...
select a.sys_id,e.row_id,COALESCE( CASE WHEN c.made_sla = 1 then 'Y' else 'N' END,'') as abc , COALESCE(e.met_sla_flag ,'') as def
...
LEFT join starwood_mdwdb.d_incident e on 
  c.sys_id=e.row_id
...
OR d.name like ('Opera-PMS%') 
  ...
  where  row_id is null and abc<>def
)i
