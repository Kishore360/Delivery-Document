 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.change_request_key' ELSE 'SUCCESS' END as Message from 
(select a.sys_id,e.row_id,c.severity AS SEVERITY_a_CODE ,COALESCE(LKP.row_key,CASE WHEN c.rfc IS NULL THEN 0 else '-1' end) abc, COALESCE(e.change_request_key,'') def
...
LEFT join starwood_mdwdb.f_incident e on
 ...
 LEFT JOIN starwood_mdwdb.d_change_request LKP 
 ON ( c.rfc= LKP.row_id 
