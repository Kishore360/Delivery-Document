...
FROM  (
select count(1) as cnt from (SELECT SRC.sys_id,TRGT.row_id,COALESCE( CASE WHEN SRC.reassignment_count >= 1 then 'Y' else 'N' END,'') as abc, COALESCE(TRGT.multiple_assignment_flag) as def
