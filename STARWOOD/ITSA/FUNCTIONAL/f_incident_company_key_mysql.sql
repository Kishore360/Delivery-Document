 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.company_key' ELSE 'SUCCESS' END as Message from 
(select a.sys_id,e.row_id,c.severity AS SEVERITY_a_CODE ,COALESCE(LKP.row_key,CASE WHEN c.company IS NULL THEN 0 else '-1' end) abc, COALESCE(e.company_key,'') def
...
LEFT JOIN starwood_mdwdb.d_internal_organization LKP 
 ON ( concat('SUBSIDIARY~',c.company)= LKP.row_id 
