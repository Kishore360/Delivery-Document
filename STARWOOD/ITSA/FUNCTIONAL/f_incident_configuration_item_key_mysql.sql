...
(SELECT SRC.sys_id,TRGT.row_id,COALESCE(LKP.row_key,CASE WHEN SRC.cmdb_ci IS NULL THEN 0 else '-1' end) as abc, COALESCE(TRGT.configuration_item_key,'')
 as def
...
LEFT JOIN starwood_mdwdb.d_configuration_item LKP 
