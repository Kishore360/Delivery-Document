...
(select a.sys_id,e.row_id,c.severity AS SEVERITY_a_CODE ,COALESCE(LKP.row_key,CASE WHEN c.sys_domain IS NULL THEN 0 else '-1' end)abc, COALESCE(e.domain_key,'')def
