...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.changed_by' ELSE 'SUCCESS' END as Message from 
(select a.sys_id,e.row_id,c.severity AS SEVERITY_SRC_CODE ,COALESCE( c.sys_updated_by,'') as abc, COALESCE(e.changed_by ,'') as def
