...
FROM  (
select count(1) as cnt from (SELECT SRC.sys_id,TRGT.row_id,COALESCE( CASE WHEN SRC.problem_id is not null then 'Y' else 'N' END,'') as abc, COALESCE(TRGT.problem_flag ,'') as def
