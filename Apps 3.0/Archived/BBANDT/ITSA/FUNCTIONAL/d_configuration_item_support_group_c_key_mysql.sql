 ...
 JOIN bbandt_mdwdb.d_internal_organization LKP
 ON ((COALESCE(CONCAT('GROUP','~~~',UPPER(SRC.support_group)),'UNSPECIFIED'))=LKP.row_id
 AND SRC.sourceinstance=LKP.source_id)
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.support_group IS NULL THEN 0 ELSE -1 END)<>(TRGT.support_group_c_key ))temp;