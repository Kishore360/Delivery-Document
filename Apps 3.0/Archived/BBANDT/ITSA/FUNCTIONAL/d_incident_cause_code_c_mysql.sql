SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.cause_code_c_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN bbandt_mdwdb.d_lov LKP 
 ON  LKP.dimension_class =  'CAUSE_CODE_C~INCIDENT' and (concat('CAUSE_CODE_C~INCIDENT~~~',upper(SRC.u_cause_code))= LKP.row_id AND SRC.sourceinstance= LKP.source_id )
