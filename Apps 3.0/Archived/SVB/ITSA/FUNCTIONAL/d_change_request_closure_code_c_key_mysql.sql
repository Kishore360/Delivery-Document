...
left  JOIN  svb_mdwdb.d_lov LKP  
 ON ( concat('CLOSURE_CODE_C~CHANGE_REQUEST~~~',upper(u_closure_code))= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN u_closure_code is null  THEN 0 else -1 end)<>(TRGT.closure_code_c_key))x