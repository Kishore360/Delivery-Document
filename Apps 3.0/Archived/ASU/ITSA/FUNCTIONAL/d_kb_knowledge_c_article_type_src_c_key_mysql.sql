...
LEFT JOIN asu_mdwdb.d_lov LKP 
 ON ( concat('ARTICLE_TYPE_C~KNOWLEDGE_C~~~',upper(article_type))= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.article_type IS NULL THEN 0 else -1 end)<> COALESCE(TRGT.article_type_src_c_key) 
