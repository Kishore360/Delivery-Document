...
( concat('STATE~INCIDENT~~~',u_sector)= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id ) 
 LEFT JOIN jnj_mdwdb.d_internal_organization TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_sector IS NULL THEN 0 else -1 end)<> (TRGT.sector_c_key)
