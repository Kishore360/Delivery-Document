 ...
 FROM(
select lov.src_rowid lov_src_rowid,map.src_rowid map_src_rowid,lov.dimension_code lov_dimension_code,
map.dimension_code lov_map_dimension_code,
...
from  mercuryins_mdwdb.d_lov_map map
 join mercuryins_mdwdb.d_lov lov on lov.src_rowid=map.src_rowid  and lov.source_id=map.source_id
left join
...
from mercuryins_workdb.lsm_ls_system_variables conf
left join mercuryins_mdsdb.sys_choice_final scf on conf.table_value= scf.name and conf.column_value = scf.element
