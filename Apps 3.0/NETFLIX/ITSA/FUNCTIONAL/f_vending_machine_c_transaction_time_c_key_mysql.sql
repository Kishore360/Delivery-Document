 ...
 join netflix_mdwdb.d_calendar_time d on COALESCE(DATE_FORMAT(CONVERT_TZ(p.u_transaction_date,'GMT','America/Los_Angeles'),'%Y%m%d'),'UNSPECIFIED')=d.row_id
 join netflix_mdwdb.f_vending_machine_c f on sys_id=f.row_id and sourceinstance=f.source_id
where 
coalesce(d.row_key,case when u_transaction_date is null then 0 else -1 end )<>f.transaction_time_c_key)b
