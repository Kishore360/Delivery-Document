...
LEFT JOIN homedepot_mdwdb.d_lov LKP 
 ON ( concat('DATA_CLASSIFICATION_C~APPLICATION~~~',u_data_classification)= LKP.row_id 
AND a.sourceinstance= LKP.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN a.u_data_classification IS NULL THEN 0 else -1 end)<>b.data_classification_src_c_key)temp; 
