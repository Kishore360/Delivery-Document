...
on concat('INTERNAL_CONTACT~',a.u_patch_director_vp) = c.row_id
and a.sourceinstance = c.source_id
where 
case when a.u_patch_director_vp is NULL then 0
when c.row_id is NULL and a.u_patch_director_vp is not NULL then -1
else
c.row_key <> b.patch_director_vp_c_key end ) b
