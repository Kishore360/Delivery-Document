...
on concat('INTERNAL_CONTACT~',a.u_subject_matter_expert) = c.row_id
and a.sourceinstance = c.source_id
where case when assigned_to is NULL then 0
when c.row_id is NULL and u_subject_matter_expert is not NULL then -1
else
c.row_key <> b.subject_matter_expert_c_key end ) b 