...
on concat('INTERNAL_CONTACT~',a.u_product_architect) = c.row_id
and a.sourceinstance = c.source_id
where 
case when u_product_architect is NULL then 0
when c.row_id is NULL and u_product_architect is not NULL then -1
else
c.row_key <> b.product_architect_c_key end ) b
