...
select case when (major_incident_flag='Y'  or saf.oldvalue=1 or saf.newvalue=1) then 'Y'  else 'N'  END as src, priority_ever_p1_c_flag trgt
 FROM svb_mdsdb.incident_final inc
  left join (select oldvalue,newvalue,documentkey from @#LDB_CLIENT_SOURC_Database@#.sys_audit_final where tablename='incident' and fieldname='priority' )saf on saf.documentkey=inc.sys_id
