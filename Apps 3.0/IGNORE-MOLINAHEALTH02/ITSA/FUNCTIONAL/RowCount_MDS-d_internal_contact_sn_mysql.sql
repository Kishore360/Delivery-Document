...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM molinahealth_mdsdb.sys_user_final ) AS count_mds ) MDS,
(SELECT COUNT(1) Count_DWH, 'd_internal_contact'  Table_DWH 
FROM molinahealth02_mdwdb.d_internal_contact WHERE row_key NOT IN (0,-1)  and current_flag='Y'
