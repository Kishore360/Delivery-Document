 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_outage_c.related_task_c_key' ELSE 'SUCCESS' END as Message
 FROM nypres_mdsdb.cmdb_ci_outage_final SRC 
 JOIN nypres_mdwdb.f_outage TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
 join nypres_mdwdb.d_task d on (SRC.u_related_task =d.row_id  
