...
inner join ( select cmdb_ci,sys_id,sourceinstance from nypres_mdsdb.change_request_final 
join nypres_mdwdb.d_calendar_date d on d.row_id=date_format(convert_tz(opened_at,'GMT','America/New_york'),'%Y%m%d')
and d.lagging_count_of_days between 0 and 29) as inc
...
where trgt.changes_in_last_30_days_c <> src.source_count;
