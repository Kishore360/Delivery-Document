...
from (select  number,opened_at,cmdb_ci,sourceinstance,sys_id from nypres_mdsdb.change_request_final 
join  nypres_mdwdb.d_calendar_date d on d.row_id=date_format(convert_tz(opened_at,'GMT','America/New_york'),'%Y%m%d')
where  d.lagging_count_of_days between 0 and 59  ) t  
...
join 
(select row_id,changes_in_last_60_days_c  from nypres_mdwdb.f_configuration_item_c) trgt
on src.cmdb_ci=trgt.row_id 
where src.cnt <>trgt.changes_in_last_60_days_c
