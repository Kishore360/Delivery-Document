...
LEFT JOIN nypres_mdwdb.d_lov LKP 
 ON ( concat('U_CLOSURE_SUBCATEGORY~CMDB_CI_OUTAGE',upper(u_closure_subcategory))= LKP.row_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_closure_subcategory IS NULL THEN 0 else -1 end)<> COALESCE(TRGT.closure_subcategory_src_c_key) 
