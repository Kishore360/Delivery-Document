...
from nypres_workdb.lsm_ls_system_variables conf
where conf.enable_flag = 'Y') SRC
left join nypres_mdwdb.d_lov TGT
ON TGT.dimension_class = SRC.dimension_class
where TGT.row_id is null and SRC.dimension_class not like '%CALL%' and SRC.dimension_class not like '%FACILITIES%' and SRC.dimension_class not like '%HR_CHANGE%')a;
