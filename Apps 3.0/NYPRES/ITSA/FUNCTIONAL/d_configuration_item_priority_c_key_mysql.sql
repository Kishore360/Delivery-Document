...
LEFT JOIN nypres_mdwdb.d_lov LKP 
 ON ( CONCAT('U_PRIORITY','~','CMDB_CI_APPL','~~~',UPPER(u_priority))= LKP.row_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_priority IS NULL THEN 0 else -1 end)<> (TRGT.priority_c_key) 
