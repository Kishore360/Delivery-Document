...
LEFT JOIN nypres_mdwdb.d_lov LKP 
 ON ( concat('U_CLOSURE_CATEGORY~CMDB_CI_OUTAGE',upper(u_closure_category))= LKP.row_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_closure_category IS NULL THEN 0 else -1 end)<> COALESCE(TRGT.closure_category_src_c_key)
