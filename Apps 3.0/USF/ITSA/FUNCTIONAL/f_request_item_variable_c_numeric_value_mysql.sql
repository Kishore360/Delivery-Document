...
a.numeric_value aa ,
 ...
 from 
(select type,sourceinstance,sys_id,name  from usf_mdsdb.item_option_new_final  where name='u_room_number'
and  
...
inner join usf_mdwdb.d_request_item c
on d.request_item=c.row_id and c.source_id=d.sourceinstance
join usf_workdb.lsm_ls_variable_list z on e.sys_id=z.row_id and z.table_name='request_item' and z.variable_type!='Reference'
join usf_mdsdb.sys_choice_final t  on t.name='question' and e.type=t.value
join usf_workdb.lsm_ls_variable_datatype lvd on t.label = lvd.variable_type 
join (SELECT substring(row_id,1,32) as r1, SUBSTRING(row_id,34,32) as r2, numeric_value  FROM  usf_mdwdb.f_request_item_variable_c) a 
