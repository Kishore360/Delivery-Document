...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM molinahealth_mdsdb.cmn_location_final)  AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_location'  Table_DWH 
FROM molinahealth02_mdwdb.d_location WHERE row_key not in (0,-1)) DWH
