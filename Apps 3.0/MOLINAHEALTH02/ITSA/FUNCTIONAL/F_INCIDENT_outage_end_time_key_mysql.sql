...
on (LKP.row_id = date_format(convert_tz(SRC.u_outage_end ,'GMT','America/Los_Angeles'),'%H%i') and LKP.source_id=0)
 WHERE coalesce(LKP.row_key,case when SRC.u_outage_end is null then 0 else -1 end)<>(TRGT.outage_end_time_key)
