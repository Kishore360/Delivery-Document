...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM molinahealth_mdsdb.sc_cat_item_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_master_item'  Table_DWH 
FROM molinahealth02_mdwdb.d_master_item WHERE row_key NOT IN (0,-1) )  DWH
