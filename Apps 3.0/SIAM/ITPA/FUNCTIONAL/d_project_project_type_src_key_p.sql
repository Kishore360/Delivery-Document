...
LEFT JOIN <<tenant>>_mdwdb.d_lov LKP 
 ON ( concat('PROJECT~','project_type~','~',SRC.project_type) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.project_type IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_type_src_key ,'')
