...
LEFT JOIN nbcu_mdwdb.d_internal_contact LKP 
 ON ( COALESCE(concat('INTERNAL_CONTACT~',SRC.taken_by),'UNSPECIFIED')= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.taken_by IS NULL THEN 0 else -1 end)<>TRGT.taken_by_c_key
