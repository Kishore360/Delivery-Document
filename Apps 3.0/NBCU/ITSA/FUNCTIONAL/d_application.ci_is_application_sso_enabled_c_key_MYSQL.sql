...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_application.ci_is_application_sso_enabled_c_key' ELSE 'SUCCESS' END as Message 
FROM (
SELECT count(1) as CNT
FROM nbcu_mdsdb.cmdb_ci_appl_final SRC
LEFT JOIN nbcu_mdwdb.d_lov LKP 
ON (coalesce(CONCAT('IS_APPLICATION_SSO_ENABLED','~','APPLICATION~~~',SRC.u_is_application_sso_enabled_),'UNSPECIFIED')=LKP.row_id AND
