...
FROM ( 
		...
		(
			select count(1) 
			FROM 
				nbcu_mdwdb.f_request_task d_inc
			LEFT JOIN 
			...
			Join 
					...
					and b.name LIKE '%CTASk%(With Pause)%' and b.collection='sc_task' 
			...
			where 
				...
				and b.name LIKE '%CTASK%(With Pause)%' and b.collection='sc_task'  
				GROUP BY a.task
			having timestampdiff(SECOND, '1970-01-01 00:00:00',max(a.duration)) <> max(d_inc.met_sla_with_pause_duration_c)
		)a
)a 
;