...
FROM ( 
		...
		(
			...
			Join 
					...
					on a.sla=b.sys_id  where a.stage='completed'
					and b.name LIKE '%Resolution%(With Pause)%' and b.collection='incident' 
			...
			where 
				...
				and b.name LIKE '%Resolution%(With Pause)%' and b.collection='incident'  
				GROUP BY a.task
			having timestampdiff(SECOND, '1970-01-01 00:00:00',max(a.duration)) <> max(d_inc.resolution_with_pause_sla_duration_c)
		)a
)a 
;