...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_application.install_status' ELSE 'SUCCESS' END as Message 
FROM (
...
JOIN nbcu_mdwdb.d_application TRGT
 ...
 WHERE COALESCE(LKP.row_key, CASE WHEN SRC.install_status IS NULL THEN 0 ELSE -1 END)
  <>TRGT.install_status
)temp;
