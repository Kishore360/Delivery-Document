...
left join  nbcu_mdwdb.d_calendar_date lkp on
 date_format(convert_tz(src.u_fulfilled_at,'GMT','America/New_York'),'%Y%m%d')=lkp.row_id --  and lkp.source_id=src.sourceinstance
left join nbcu_mdwdb.f_request_item f 
on  f.row_id=src.sys_id and f.source_id=src.sourceinstance
where coalesce(lkp.row_key,case when date_format(convert_tz(src.u_fulfilled_at,'GMT','America/New_York'),'%Y%m%d') is null then 0 else -1 end )
