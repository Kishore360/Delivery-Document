...
(select count(1) as CNT from nbcu_mdsdb.sys_audit_final SRC
 left join nbcu_mdwdb.d_lov LKP on concat('OPERATIONAL_STATUS_C~CONFIGURATION_ITEM~~~',SRC.newvalue)=LKP.row_id and 
...
coalesce(LKP.row_key, case when SRC.newvalue is null then 0 else -1 end) <> TRGT.task_attribute_wh_new_value_key
