...
from
    ...
    from
        nbcu_workdb.lsm_ls_system_variables conf
    where
        conf.enable_flag = 'Y') SRC
    left join nbcu_mdwdb.d_lov TGT ON TGT.dimension_class = SRC.dimension_class
    where
        TGT.row_id is null
            and SRC.dimension_class not like ('%CATEGORY~FACILITIES%')
			and SRC.dimension_class not like ('%CATEGORY~HR_CHANGE%')
			and SRC.dimension_class not like ('%STATE~HR_CHANGE%')) a;
