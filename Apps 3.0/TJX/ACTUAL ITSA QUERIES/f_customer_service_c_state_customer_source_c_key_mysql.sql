...
LEFT JOIN tjx_mdwdb.d_lov LKP 
 ON(COALESCE(CONCAT('STATE_C~U_CUSTOMER_SERVICE~~~',UPPER(SRC.state)),'UNSPECIFIED') = LKP.row_id 
AND SRC.sourceinstance= LKP.source_id ) 
 LEFT JOIN tjx_mdwdb.f_customer_service_c TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.state IS NULL THEN 0 else -1 end)<> (TRGT.state_customer_source_c_key);
