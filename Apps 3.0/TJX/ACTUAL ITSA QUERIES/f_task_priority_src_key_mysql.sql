...
FROM (select sys_id,sourceinstance,priority from tjx_mdsdb.task_final where sys_class_name='problem' ) src
JOIN tjx_mdwdb.f_task_c trgt 
on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
left JOIN tjx_mdwdb.d_lov lkp 
on CONCAT('PRIORITY','~','TASK','~','~','~',UPPER(src.priority)) = lkp.row_id AND src.sourceinstance =  lkp.source_id where 
