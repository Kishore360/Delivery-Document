...
THEN TIMESTAMPDIFF(SECOND, CONVERT_TZ(par.opened_on,'America/New_York','GMT'),
CONVERT_TZ(d.closed_on,'America/New_York','GMT')) else 0 END
and ( TRGT.Parent_Age_At_Task_Closure IS NULL OR TRGT.Parent_Age_At_Task_Closure <> 
CASE WHEN d.closed_on > par.opened_on THEN TIMESTAMPDIFF(SECOND, CONVERT_TZ(par.opened_on,'America/New_York','GMT'),
CONVERT_TZ(d.closed_on,'America/New_York','GMT')) else 0 END);
