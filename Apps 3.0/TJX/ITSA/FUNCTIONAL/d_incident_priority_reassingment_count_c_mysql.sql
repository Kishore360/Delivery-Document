...
from
(select 
case when src.cnt <> trgt.priority_reassingment_count_c then src.documentkey else '' end as failures,

case when src.cnt <> trgt.priority_reassingment_count_c  then 1 else 0 end as failures_cnt

from 
(select documentkey,sourceinstance,count(sys_id) as cnt
from tjx_mdsdb.sys_audit_final where tablename = 'incident' and fieldname = 'priority' and oldvalue is not null
group by documentkey,sourceinstance) src
left join tjx_mdwdb.d_incident trgt on src.documentkey = trgt.row_id and src.sourceinstance = trgt.source_id ) fnl;
