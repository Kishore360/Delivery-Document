...
on (LKP.row_id = date_format(convert_tz(SRC.opened_at,'GMT','America/New_York'),'%Y%m%d')  and LKP.source_id=0
 ...
 join tjx_mdwdb.d_request_item a19 on (TRGT.request_item_key = a19.row_key)  where category_name_c in ('ARMS') and COALESCE(LKP.row_key,case when SRC.opened_at is null then 0 else -1 end) <> (TRGT.opened_on_key))temp;
