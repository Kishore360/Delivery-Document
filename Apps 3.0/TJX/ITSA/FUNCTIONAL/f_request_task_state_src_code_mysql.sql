...
CASE WHEN CNT >0 THEN 'MDS to DWH data validation failed for f_request_task.state_src_code' ELSE 'SUCCESS' END as Message
...
on (a11.request_item_key = a19.row_key) 
 WHERE category_name_c in ('ARMS') and COALESCE( SRC.state,'')<> COALESCE(TRGT.state_src_code ,''))temp;
