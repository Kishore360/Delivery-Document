...
from  tjx_mdwdb.d_lov_map map
 join tjx_mdwdb.d_lov lov on lov.src_rowid=map.src_rowid and lov.source_id=map.source_id
left join
...
left join tjx_mdsdb.sys_choice_final scf on conf.table_value= scf.name and conf.column_value = scf.element
where conf.enable_flag = 'Y'
and scf.inactive = 0
and scf.language='en') SRC
on lov.row_id = SRC.row_id
and lov.source_id = SRC.source_id
where src_key<0 and lov.dimension_class not in ('PROJECT'))x