...
from
(select 
case when src.cnt <> trgt.group_reassignment_count_c then src.documentkey else '' end as failures,

case when src.cnt <> trgt.group_reassignment_count_c  then 1 else 0 end as failures_cnt

from 
(select documentkey,sourceinstance,count(sys_id) as cnt
from tjx_mdsdb.sys_audit_final where tablename = 'problem' and fieldname = 'assignment_group' and oldvalue is not null
