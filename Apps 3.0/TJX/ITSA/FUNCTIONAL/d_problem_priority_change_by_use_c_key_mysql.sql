...
FROM (
select count(1) as cnt from tjx_mdwdb.d_problem TGT 
inner join  tjx_mdsdb.problem_final incf on TGT.row_id=incf.sys_id and TGT.source_id = incf.sourceinstance
left join (select a.sourceinstance,a.documentkey,a.newvalue,a.oldvalue,a.user from  tjx_mdsdb.sys_audit_final a 
                                                join (select documentkey, max(sys_created_on) as sys_created_on from tjx_mdsdb.sys_audit_final where tablename='problem' and fieldname = 'priority' 
                                                group by documentkey) rec on a.documentkey = rec.documentkey and  a.sys_created_on = rec.sys_created_on
                                where  tablename='problem' and fieldname = 'priority' ) as pon
