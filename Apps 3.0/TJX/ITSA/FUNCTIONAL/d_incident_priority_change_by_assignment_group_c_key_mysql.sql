...
left join (select saf.documentkey, 
                                                                   saf.sourceinstance,
                                                                   saf.newvalue,saf.oldvalue,saf.sys_created_on 
                                                from   (select asg.documentkey, max(asg.sys_created_on) sys_created_on 
                                                                                ...
                                                                                join (select documentkey, sys_created_on,oldvalue,newvalue from tjx_mdsdb.sys_audit_final 
                                                                                                                 ...
                                                                                                                 on prt.documentkey = asg.documentkey 
                                                                                                                                and prt.mxsys_created_on > asg.sys_created_on 
                                                                                group  by asg.documentkey) tmp 
                                                                   join tjx_mdsdb.sys_audit_final saf 
                                                                                 on saf.documentkey = tmp.documentkey 
                                                                                                and tmp.sys_created_on = saf.sys_created_on 
...
inner join tjx_mdwdb.d_internal_organization LKP on LKP.row_id = coalesce(concat('GROUP~',coalesce(pasg.newvalue,pasg.oldvalue,incf.assignment_group)),'UNSPECIFIED') AND LKP.source_id=pasg.sourceinstance
