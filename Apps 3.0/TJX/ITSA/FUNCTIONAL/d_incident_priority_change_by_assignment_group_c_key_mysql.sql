...
join tjx_mdwdb.d_internal_organization dic on concat('GROUP~',coalesce(src.oldvalue,incf.assignment_group)) =dic.row_id 
where priority_change_by_assignment_group_c_key <> coalesce(TGT.row_key,case when concat('GROUP~',coalesce(src.oldvalue,incf.assignment_group)) is null then 0 else -1 end  ) 
