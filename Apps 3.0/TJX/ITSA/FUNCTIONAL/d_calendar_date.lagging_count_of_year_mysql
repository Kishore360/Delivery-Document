...
LEFT OUTER JOIN(SELECT MAX(DATE(DATE)) AS YEAR_END_DATE,MIN(DATE(DATE)) AS YEAR_START_DATE
 FROM tjx_workdb.DS_DATE_SEED
                   ...
                   GROUP BY CAST(CONCAT(DATE_FORMAT(DATE,'%Y'))AS SIGNED
)) SRC2   
...
AND IF(YEAR(SRC2.YEAR_START_DATE)
<=YEAR(tz_sys_val),YEAR(tz_sys_val)-YEAR(SRC2.YEAR_START_DATE),NULL)<>TRGT.lagging_count_of_year
