...
CASE WHEN CNT >0 THEN 'MDS to DWH data validation failed for f_request_task.changed_by' ELSE 'SUCCESS' END as Message
...
on (a11.request_item_key = a19.row_key) 
 WHERE category_name_c in ('ARMS') and COALESCE( SRC.sys_updated_by,'')<> COALESCE(TRGT.changed_by ,''))temp;
