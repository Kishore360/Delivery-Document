...
LEFT JOIN tjx_mdwdb.d_lov LKP 
 ON(COALESCE(CONCAT('U_CATEGORY_US_C~U_CUSTOMER_SERVICE~~~',UPPER(SRC.u_category_us)),'UNSPECIFIED') = LKP.row_id 
AND SRC.sourceinstance= LKP.source_id ) 
 LEFT JOIN tjx_mdwdb.f_customer_service_c TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_category_us IS NULL THEN 0 else -1 end)<> (TRGT.category_us_c_key);
