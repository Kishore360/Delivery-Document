...
on Case when c.label='Select Box'  then qc.sys_id else null  end = lkp.row_id 
where coalesce(lkp.row_key,case when qa.sys_id is null then 0 else -1 end) <> trgt.reference_c_key)c
