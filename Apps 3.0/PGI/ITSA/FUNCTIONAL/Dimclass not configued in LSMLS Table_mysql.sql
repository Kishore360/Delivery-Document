...
from pgi_mdwdb.d_lov lov where  source_id=2 and dimension_class 
not in (select class_value from pgi_workdb.lsm_ls_system_variables where enable_flag='Y'  )  and dimension_class 
...
,'CORE_SERVICE~INCIDENT'
,'NAME~POOR_TICKET_QUALITY'
,'NAME~WHAT_REQUIRES_REVIEW'
) )a;
