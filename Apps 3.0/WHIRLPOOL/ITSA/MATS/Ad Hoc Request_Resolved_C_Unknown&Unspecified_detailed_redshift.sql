select 'last_resolved_on_key '   as Table_Name, 
COUNT(a11.last_resolved_on_key ) AS TotalRows,
COUNT(DISTINCT a11.last_resolved_on_key  ) AS DistinctValues, 
sum(case when a11.last_resolved_on_key = -1 THEN 1   ELSE 0 END) as UnknownRecords, 
sum(case when a11.last_resolved_on_key =0 THEN 1 ELSE 0 END ) as UnspecifiedRecords,
case when ((sum(case when a11.last_resolved_on_key  = -1 THEN 1   ELSE 0 END)  + sum(case when a11.last_resolved_on_key  =0 THEN 1 ELSE 0 END ))= COUNT( a11.last_resolved_on_key  )) then 'Failed'  
when ((sum(case when a11.last_resolved_on_key  = -1 THEN 1   ELSE 0 END)  + sum(case when a11.last_resolved_on_key  =0 THEN 1 ELSE 0 END ))> 0.8*COUNT( a11.last_resolved_on_key  )) then 'Failed' else 'Success' end as compare
