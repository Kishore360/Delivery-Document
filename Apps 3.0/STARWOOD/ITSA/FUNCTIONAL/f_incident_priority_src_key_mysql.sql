...
select  CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.priority_src_key' ELSE 'SUCCESS' END as Message from 
(select a.sys_id,e.row_id,c.severity AS SEVERITY_a_CODE ,COALESCE(LKP.row_key,CASE WHEN c.priority IS NULL THEN 0 else '-1' end) abc , COALESCE(e.priority_src_key,'') def
...
LEFT join starwood_mdwdb.f_incident e on
 ...
 ON ( concat('PRIORITY','~','TASK','~','~','~',upper(c.priority))= LKP.src_rowid 
