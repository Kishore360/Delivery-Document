SELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM
(
select count(1) as cnt from(SELECT SRC.sys_id,c.row_id,COALESCE( SRC.description,'')as abc, COALESCE(c.description ,'')as def 
...
OR u_name  IN ('Valhalla','Central Reservation Systems') )abc
  where abc<>def
)i