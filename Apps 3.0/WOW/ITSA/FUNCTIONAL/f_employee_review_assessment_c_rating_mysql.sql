...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_employee_review_assessment_c.rating' ELSE 'SUCCESS' END as Message 
FROM ( SELECT CONCAT(src1.sys_id,'~',src3.sys_id) as sys_id,
src1.sourceinstance,COALESCE(src4.actual_value,0) as val
FROM
