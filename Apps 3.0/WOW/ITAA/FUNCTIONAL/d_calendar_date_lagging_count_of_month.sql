...
AND CASE WHEN YEAR(SRC5.MONTH_START_DATE)=YEAR(tz_sys_val) THEN IF(MONTH(tz_sys_val)-MONTH(SRC5.MONTH_START_DATE)<0,NULL, MONTH(tz_sys_val)-MONTH(SRC5.MONTH_START_DATE)) ELSE IF(TIMESTAMPDIFF(MONTH,SRC5.MONTH_START_DATE,tz_sys_val)<=0, NULL,TIMESTAMPDIFF(MONTH,SRC5.MONTH_START_DATE,tz_sys_val)) END<>TRGT.lagging_count_of_month
