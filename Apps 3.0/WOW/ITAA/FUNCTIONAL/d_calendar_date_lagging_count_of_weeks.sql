...
AND CASE WHEN YEAR(SRC6.WEEK_START_DATE)=YEAR(tz_sys_val) THEN IF(WEEK(tz_sys_val,1)-WEEK(SRC6.WEEK_START_DATE,1) <0,NULL,WEEK(tz_sys_val,1)-WEEK(SRC6.WEEK_START_DATE,1)) ELSE IF(TIMESTAMPDIFF(DAY,SRC6.WEEK_START_DATE,DATE_ADD(DATE(tz_sys_val), INTERVAL 6-WEEKDAY(DATE(tz_sys_val)) DAY))<0,NULL,TIMESTAMPDIFF(WEEK,SRC6.WEEK_START_DATE,tz_sys_val)) END <>TRGT.lagging_count_of_weeks
