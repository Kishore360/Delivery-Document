...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.week_day_flag' ELSE 'SUCCESS' END as Message
...
AND CASE WHEN(DATE_FORMAT(SRC.`DATE`,'%w')) = 0 OR(DATE_FORMAT(SRC.`DATE`,'%w')) = 6 THEN 'N' ELSE 'Y' END <>TRGT.week_day_flag
