...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.quarter_name' ELSE 'SUCCESS' END as Message
...
AND CONCAT('Q',QUARTER(SRC.`DATE`),' ',YEAR(SRC.`DATE`)) <>TRGT.quarter_name
