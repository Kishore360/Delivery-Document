...
left join tivo_mdwdb.d_internal_organization a
on a.row_id=COALESCE(CONCAT('SUBSIDIARY','~',SUBSTRING_INDEX(SUBSTRING_INDEX(TRIM(b.u_companies), ',', n.n), ',', -1)),'UNSPECIFIED')and b.sourceinstance=a.source_id
...
WHERE COALESCE(a.row_key,CASE WHEN b.sys_id IS NULL THEN 0 else -1 end)<>c.companies_c_key) temp; 
