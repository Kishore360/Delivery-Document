...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 join tivo_mdwdb.d_calendar_date d on 
   DATE_FORMAT(CONVERT_TZ(i.DUE_DATE,'GMT','America/Los_Angeles'),'%Y%m%d')=f.row_id
 join tivo_mdwdb.f_change_task_c f on sys_id=f.row_id and sourceinstance=f.source_id
 where due_on_key<>coalesce(d.row_key,case when opened_at is null then 0 else -1 end)
