...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 CONCAT('CONTACT_TYPE','~','CHANGE_TASK','~','~','~',UPPER(i.CONTACT_TYPE))  =f.row_id
 where REPORTED_TYPE_SRC_key<>coalesce(d.row_key,case when CONTACT_TYPE is null then 0 else -1 end)
