 ...
 COALESCE(DATE_FORMAT(CONVERT_TZ(i.CLOSED_AT,'GMT','America/Los_Angeles'),'%Y%m%d'),'UNSPECIFIED') =d.row_id
 join tivo_mdwdb.f_change_task_c f on sys_id=f.row_id and sourceinstance=f.source_id
 where closed_on_key<>coalesce(d.row_key,case when CLOSED_AT is null then 0 else -1 end)
