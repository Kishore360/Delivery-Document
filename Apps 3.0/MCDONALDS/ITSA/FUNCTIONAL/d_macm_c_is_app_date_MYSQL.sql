...
CASE WHEN CNT THEN 'MDS to DWH data validation failed for d_macm_c_is_app_date' ELSE 'SUCCESS' END as Message
FROM
(
...
WHERE COALESCE(CONVERT_TZ(SRC.u_is_app_date,'GMT','US/Central'),'UNSPECIFIED')<>TRGT.is_app_date
)a;