...
(SELECT  mon.row_key as month_start_date_key,
       ...
       (open_task_fss.closed_on_key is null) 
	   and (open_task_fss.age between 0 and 30) then 1 else 0 END ) AS 'age_bucket1'
                   FROM uclahealth_mdwdb.d_calendar_date mon
JOIN uclahealth_mdwdb.f_incident open_task_fss  ON 
