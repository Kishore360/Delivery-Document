SELECT CASE WHEN result = 0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 from 
 (select case when (select count(1) from cardinalhealth_workdb.fs_agent_call_summary_aux ) = 
 (select count(1) from cardinalhealth_mdwdb.f_agent_call_summary_aux) then 1 else 0 end as result) a
