...
truncate table cardinalhealth_workdb.fs_agent_call_summary_aux;
...
FROM cardinalhealth_mdsdb.synonyms_final syn
JOIN cardinalhealth_mdsdb.hagent_final agent ON (syn.acd_no = agent.acd)
...
FROM cardinalhealth_mdsdb.synonyms_final syn
JOIN cardinalhealth_mdsdb.hagent_final agent
...
SET @sql = CONCAT(
'INSERT INTO cardinalhealth_workdb.fs_agent_call_summary_aux (ROW_ID,SOURCE_ID,PARENT_ROW_ID,TIME_ID,DAY_ID,EMPLOYEE_ID,SKILL_ID,SHIFT_ID,LOCATION_ID,DEPARTMENT_ID,COST_CENTER_ID,AUX_CODE_SRC_ID,ACD_ID,AUX_TIME,CREATED_BY,CHANGED_BY,CREATED_ON,CHANGED_ON,PIVOT_DATE)
