...
from 
(select 
case when src.due_date is not null then 1 else 0 end as src_not_null_count,
case when trgt.due_on is not null then 1 else 0 end as trgt_not_null_count,
case when COALESCE(convert_tz(src.due_date,'UTC','America/New_York'),'1970-01-01 00:00:00') <> COALESCE(trgt.due_on,'1970-01-01 00:00:00') then src.sys_id else '' end as failures,
case when COALESCE(convert_tz(src.due_date,'UTC','America/New_York'),'1970-01-01 00:00:00') <> COALESCE(trgt.due_on,'1970-01-01 00:00:00') then 1 else 0 end as failures_cnt
