...
on lkp.src_rowid=COALESCE(CONCAT('U_CAH_HEALTH_SOFTWARE~CMDB_CI_APPLICATION','~','','~','','~',COALESCE(ccaf.u_cah_health_software,'')),'UNSPECIFIED')
where  coalesce(lkp.row_key,case when ccaf.u_cah_health_software is null then 0 else -1 end) <> d.cah_health_software_src_c_key)c
