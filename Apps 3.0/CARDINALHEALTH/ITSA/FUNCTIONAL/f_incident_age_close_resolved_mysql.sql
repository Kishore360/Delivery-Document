...
from cardinalhealth_mdsdb.incident_final where opened_at < coalesce(resolved_at,closed_at)) SRC 
  join cardinalhealth_mdwdb.f_incident f ON (SRC.sys_id =f.row_id  
 AND SRC.sourceinstance= f.source_id  )
JOIN cardinalhealth_mdwdb.d_lov_map br ON f.state_src_key = br.src_key
AND br.dimension_wh_code IN ('RESOLVED','CLOSED')
JOIN cardinalhealth_mdwdb.d_incident a ON a.row_key = f.incident_key
