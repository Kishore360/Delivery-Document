...
case when COALESCE(timestampdiff(second,'1970-01-01 00:00:00',src.business_duration),0) <> COALESCE(trgt.business_duration,0) then src.sys_id else '' end as failures,

case when COALESCE(timestampdiff(second,'1970-01-01 00:00:00',src.business_duration),0) <> COALESCE(trgt.business_duration,0) then 1 else 0 end as failures_cnt

from cardinalhealth_mdsdb.sc_task_final  src
left join cardinalhealth_mdwdb.f_request_task  trgt on trgt.row_id = src.sys_id and trgt.source_id = src.sourceinstance) fnl ;