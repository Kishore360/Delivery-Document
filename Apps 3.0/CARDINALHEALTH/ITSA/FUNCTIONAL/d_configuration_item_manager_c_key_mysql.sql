...
case when trgt.manager_c_key is not null and trgt.manager_c_key <> 0 then 1 else 0 end as trgt_not_null_count,
...
case when COALESCE(lkp.row_key,CASE WHEN src.u_cah_level_5_primary_contact IS NULL THEN 0 else -1 end) <> COALESCE(trgt.manager_c_key,0)  then src.sys_id else '' end as failures,
