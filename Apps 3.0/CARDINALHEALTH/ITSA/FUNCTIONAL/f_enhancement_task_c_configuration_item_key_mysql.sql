...
when sum(warnings_cnt) > 0 and length(group_concat(distinct warnings)) >= 1024 then concat('Warning. Look up does not exists for ',sum(warnings_cnt),' records. Few (sys_id || cmdb_ci) - ',group_concat(distinct warnings))
...
left join cardinalhealth_mdwdb.d_configuration_item lkp on lkp.row_id = src.cmdb_ci and lkp.source_id = src.sourceinstance)fnl;