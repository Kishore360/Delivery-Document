...
from 
(select 
case when src.business_duration is not null then 1 else 0 end as src_not_null_count,

case when trgt.business_duration_c is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(timestampdiff(second,'1970-01-01 00:00:00',src.business_duration),0) <> COALESCE(trgt.business_duration_c,0) then src.sys_id else '' end as failures,

case when COALESCE(timestampdiff(second,'1970-01-01 00:00:00',src.business_duration),0) <> COALESCE(trgt.business_duration_c,0) then 1 else 0 end as failures_cnt

from cardinalhealth_mdsdb.problem_final src
left join cardinalhealth_mdwdb.f_problem trgt on trgt.row_id = src.sys_id and trgt.source_id = src.sourceinstance) fnl ;
