...
from 
(select 
case when src.u_iceberg_access is not null then 1 else 0 end as src_not_null_count,
case when trgt.iceberg_access_c_flag is not null then 1 else 0 end as trgt_not_null_count,
case when COALESCE(IF(src.u_iceberg_access=true,'Y','N'),'N') <> COALESCE(trgt.iceberg_access_c_flag,'N') then src.sys_id else '' end as failures,
case when COALESCE(IF(src.u_iceberg_access=true,'Y','N'),'N') <> COALESCE(trgt.iceberg_access_c_flag,'N') then 1 else 0 end as failures_cnt
