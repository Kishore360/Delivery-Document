...
from 
(select 
case when src.reassignment_count is not null then 1 else 0 end as src_not_null_count,

case when trgt.reassignment_count is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.reassignment_count,0) <> COALESCE(trgt.reassignment_count,0) then src.sys_id else '' end as failures,

case when COALESCE(src.reassignment_count,0) <> COALESCE(trgt.reassignment_count,0) then 1 else 0 end as failures_cnt
