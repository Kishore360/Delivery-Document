...
case when COALESCE(IF(src.active=true,'Y','N'),'N') <> COALESCE(trgt.active_flag,'N') then src.sys_id else '' end as failures,
case when COALESCE(IF(src.active=true,'Y','N'),'N') <> COALESCE(trgt.active_flag,'N') then 1 else 0 end as failures_cnt
