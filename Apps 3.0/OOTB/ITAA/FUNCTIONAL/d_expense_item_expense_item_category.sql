...
FROM (SELECT S.sys_id,S.sourceinstance,S.category 
	FROM #MDS_TABLE_SCHEMA.fm_expense_line_final S 
	JOIN #MDS_TABLE_SCHEMA.pm_project_final SRC_P ON (SRC_P.sys_id = S.task) 
    UNION
    SELECT S.sys_id,S.sourceinstance,S.category 
	FROM #MDS_TABLE_SCHEMA.fm_expense_line_final S 
	JOIN #MDS_TABLE_SCHEMA.pm_project_task_final SRC_PT ON (SRC_PT.sys_id = S.task) 
)SRC
