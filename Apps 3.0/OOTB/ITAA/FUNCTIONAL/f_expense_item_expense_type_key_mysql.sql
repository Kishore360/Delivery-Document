...
AND LKP.sourceinstance = LKP.sourceinstance
 ...
 ON  COALESCE(CONCAT('EXPENSE_ITEM~U_EXPENSE_TYPE~~',SRC.u_expense_type),'UNSPECIFIED')  = LKP1.row_id 
AND SRC.sourceinstance = LKP1.source_id  
where COALESCE(LKP1.row_key,CASE WHEN SRC.u_expense_type IS NULL THEN 0 else '-1' end) <> TRGT.expense_type_key;