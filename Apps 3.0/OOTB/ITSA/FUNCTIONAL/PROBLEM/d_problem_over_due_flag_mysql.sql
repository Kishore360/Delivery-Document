...
FROM <<tenant>>_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%'),<<DW_TARGET_TIME_ZONE>>,<<TENANT_SSI_TIME_ZONE>>)then 'Y' else 'N' END),
 (TRGT.over_due_flag )
from
...
( CASE WHEN SRC.active = TRUE and SRC.due_date < CONVERT_TZ((SELECT MAX(lastupdated) AS lastupdated
FROM <<tenant>>_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' and etl_run_number=TRGT.etl_run_number),<<DW_TARGET_TIME_ZONE>>,<<TENANT_SSI_TIME_ZONE>>) then 'Y' else 'N' END)<>
 (TRGT.over_due_flag )
)a
