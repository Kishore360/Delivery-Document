...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.core_company_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.cmn_department_final) +
 (SELECT COUNT(1) FROM <<tenant>>_mdsdb.sys_user_group_final)

 AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_internal_organization'  Table_DWH 
FROM <<tenant>>_mdwdb.d_internal_organization WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N') DWH
