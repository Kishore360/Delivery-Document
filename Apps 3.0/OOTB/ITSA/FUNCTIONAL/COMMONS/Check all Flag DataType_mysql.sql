...
where table_schema  in ('<<tenant>>_workdb','<<tenant>>_mdwdb') and table_name IN (SELECT 	table_name
 from information_schema.TABLES where table_schema  in ('<<tenant>>_workdb','<<tenant>>_mdwdb') and table_type ='BASE TABLE')
