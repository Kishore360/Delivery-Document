...
FROM  <<tenant>>_mdwdb.d_request_task REQ_ITM 
WHERE  REQ_ITM.over_due_flag <> CASE WHEN (REQ_ITM.active_flag = 'Y') && (REQ_ITM.due_on < (SELECT max(lastupdated) AS lastupdated
FROM <<tenant>>_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' and etl_run_number=REQ_ITM.etl_run_number)) THEN 'Y' ELSE 'N' END)a;
