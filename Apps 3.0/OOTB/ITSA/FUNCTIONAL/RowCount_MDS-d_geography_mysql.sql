...
END AS Message
FROM (SELECT (
SELECT COUNT(1) FROM <<tenant>>_mdsdb.cmn_location_final ) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_geography'  Table_DWH 
FROM <<tenant>>_mdwdb.d_geography WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N') DWH 