...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.month_id' ELSE 'SUCCESS' END as Message
...
AND CAST(CONCAT(DATE_FORMAT(SRC.`DATE`,'%Y'),DATE_FORMAT(SRC.`DATE`,'%m')) AS SIGNED) <>TRGT.month_id
