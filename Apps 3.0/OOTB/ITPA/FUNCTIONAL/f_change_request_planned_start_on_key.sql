...
on (LKP.row_id  = date_format(convert_tz(SRC.start_date,source_time_zone,target_time_zone),'%Y%m%d') and LKP.source_id=0
 
)
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.start_date IS NULL THEN '' else '-1' end)<> COALESCE(TRGT.planned_start_on_key,'')
