...
and case when lm.dimension_wh_code in ('OPEN') 
    then TIMESTAMPDIFF(DAY, coalesce( SRC.opened_at,0),convert_tz(df.lastupdated, 'America/Los_Angeles','GMT'))
    else case when SRC.opened_at> SRC.closed_at then 0 else TIMESTAMPDIFF(DAY,coalesce( SRC.opened_at,0),coalesce( SRC.closed_at,0)) end
    end <>TRGT.age
