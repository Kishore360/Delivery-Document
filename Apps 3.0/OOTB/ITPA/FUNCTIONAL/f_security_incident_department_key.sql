...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_security_incident.department_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN <<tenant>>_mdwdb.d_internal_organization LKP 
	ON ( CONCAT('DEPARTMENT~',DEPARTMENT) = LKP.row_id
