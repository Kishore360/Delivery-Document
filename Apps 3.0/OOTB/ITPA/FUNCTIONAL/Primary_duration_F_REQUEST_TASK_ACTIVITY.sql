...
FROM     ( SELECT 1 FROM <<tenant>>_mdwdb.f_request_task_activity WHERE task_wh_type = 'sc_task' and task_attribute_wh_name = 'state') f1
LEFT JOIN( SELECT f2.task_key as task_key1, f2.primary_sequence_id as primary_sequence_id1, f2.created_on as created_on1 FROM <<tenant>>_mdwdb.f_request_task_activity f2 
           WHERE f2.task_wh_type = 'sc_task' and f2.task_attribute_wh_name = 'state' )f2
ON f1.task_key = f2.task_key1 AND f2.primary_sequence_id1 = f1.primary_sequence_id + 1 AND f1.task_wh_type = 'sc_task' and f1.task_attribute_wh_name = 'state' 
INNER JOIN <<tenant>>_mdwdb.d_request_task d ON f1.task_key = d.row_key
LEFT JOIN <<tenant>>_mdwdb.d_lov_map br ON f1.task_attribute_wh_new_value = br.dimension_code AND br.dimension_class = 'STATE~sc_task'
