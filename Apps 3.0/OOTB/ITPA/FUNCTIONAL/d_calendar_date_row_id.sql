...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.row_id' ELSE 'SUCCESS' END as Message
...
AND CONCAT(CONCAT(DATE_FORMAT(SRC.`DATE`,'%Y'),DATE_FORMAT(SRC.`DATE`,'%m')),DATE_FORMAT(SRC.`DATE`,'%d')) <>TRGT.row_id
