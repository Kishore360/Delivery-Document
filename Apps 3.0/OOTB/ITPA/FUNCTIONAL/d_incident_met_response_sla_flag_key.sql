...
case  when count(1)>0 then 'MDS to DWH data validation failed:d_incident.met_response_sla_flag_key' else 'SUCCESS' end as message
...
where f.task_type='incident' and SQ.strt = f.start_on and d.sla_category in ('Response')
