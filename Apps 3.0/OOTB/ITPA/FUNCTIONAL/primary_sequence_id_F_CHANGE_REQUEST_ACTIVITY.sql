...
select ftt.task_key,ftt.row_id,ftt.created_on_key, ftt.created_on,ftt.row_key,task_attribute_wh_name,task_attribute_wh_new_value,task_attribute_wh_old_value,ftt.primary_sequence_id,
                ((select count(1) from <<tenant>>_mdwdb.f_change_request_activity ftt1 where ftt1.task_wh_type = 'change_request' and ftt1.task_attribute_wh_name = 'state' 
                 AND ftt.task_key = ftt1.task_key  and ftt1.created_on < ftt.created_on ) )  as primary_sequence_id1 
          FROM <<tenant>>_mdwdb.f_change_request_activity ftt 
          WHERE ftt.task_wh_type = 'change_request' and ftt.task_attribute_wh_name = 'state' and ftt.task_key != -1 
