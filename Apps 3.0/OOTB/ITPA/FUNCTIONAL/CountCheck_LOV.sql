...
END AS Message
FROM (SELECT COUNT(1) Count_DWH , '#DWH_TABLE_NAME' Table_DWH FROM <<tenant>>_mdwdb.#DWH_TABLE_NAME 
where dimension_class='#Type') DWH,
(SELECT COUNT(1)-2 Count_VIEW, '#VIEWNAME'  Viewname FROM #VIEWNAME) VI
