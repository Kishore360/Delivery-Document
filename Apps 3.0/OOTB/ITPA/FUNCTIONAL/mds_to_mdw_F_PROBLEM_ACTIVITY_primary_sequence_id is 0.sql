...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_activity' else 'SUCCESS' end as Message
...
left join <<tenant>>_mdwdb.d_problem D 
on F.problem_key = D.row_key
where TGT.primary_sequence_id = 0 and
concat(
trim(SUBSTRING(CONCAT('problem~',TGT.task_attribute_wh_name, '~',F.row_id,'~',0),1,255)),
