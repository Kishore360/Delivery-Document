...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_task_activity' else 'SUCCESS' end as Message
...
concat(
trim(SUBSTRING(CONCAT('sc_task~',TGT.task_attribute_wh_name, '~',F.row_id,'~',0),1,255)),
