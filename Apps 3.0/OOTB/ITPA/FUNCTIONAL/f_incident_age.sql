...
and case when lm.dimension_wh_code in ('OPEN') 
    then case when coalesce( SRC.opened_at,0)> convert_tz(df.lastupdated, 'America/Los_Angeles','GMT') then 0 else
    TIMESTAMPDIFF(DAY, coalesce( SRC.opened_at,0), convert_tz(df.lastupdated, 'America/Los_Angeles','GMT')) end
               else 
