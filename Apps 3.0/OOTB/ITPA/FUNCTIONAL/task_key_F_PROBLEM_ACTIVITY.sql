...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_activity.task_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(F.problem_key,'') <> COALESCE(TGT.task_key,'')
union
select TGT.row_id
