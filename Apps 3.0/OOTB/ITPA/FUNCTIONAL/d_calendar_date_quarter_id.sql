...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.quarter_id' ELSE 'SUCCESS' END as Message
...
AND CAST(CONCAT(DATE_FORMAT(SRC.`DATE`,'%Y'),QUARTER(SRC.`DATE`)) AS SIGNED) <>TRGT.quarter_id
