...
from pan_mdsdb.u_walk_up_final where opened_at < coalesce(closed_at,u_resolved_at)) SRC 
  join pan_mdwdb.f_walk_up_c f ON (SRC.sys_id =f.row_id  
 AND SRC.sourceinstance= f.source_id  )
JOIN pan_mdwdb.d_lov_map br ON f.state_src_key = br.src_key
AND br.dimension_wh_code IN ('RESOLVED','CLOSED')
JOIN pan_mdwdb.d_walk_up_c a ON a.row_key = f.walk_up_c_key
