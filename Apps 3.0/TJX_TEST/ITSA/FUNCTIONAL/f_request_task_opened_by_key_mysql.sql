...
AND SRC.sourceinstance= LKP.source_id ) and COALESCE(CONVERT_TZ (SRC.opened_at,'GMT','America/New_York'), 
CONVERT_TZ (coalesce(SRC.closed_at,SRC.sys_updated_on),'GMT','America/New_York'),'UNSPECIFIED') 
...
on (a11.request_item_key = a19.row_key) 
 WHERE category_name_c in ('ARMS') and COALESCE(LKP.row_key,CASE WHEN SRC.opened_by IS NULL THEN 0 else -1 end)<> (TRGT.opened_by_key))temp;
