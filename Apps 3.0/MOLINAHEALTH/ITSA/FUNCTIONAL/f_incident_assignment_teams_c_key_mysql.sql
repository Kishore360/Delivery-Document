...
molinahealth_mdsdb.u_major_incident_final SRC on SRC1.sys_id =SRC.sys_id and SRC.sourceinstance=SRC1.sourceinstance
 ...
 ON ( concat('GROUP~',SRC.u_assignment_teams )= LKP.row_id
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_assignment_teams IS NULL THEN 0 else -1 end)<> (TRGT.assignment_teams_c_key);
