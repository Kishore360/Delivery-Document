...
on CONCAT('U_HR_CASE_CATEGORY~REQUEST_ITEM','~','~','~',src1.u_category)=lov.row_id and src1.sourceinstance=lov.source_id
where coalesce(lov.row_key,case when a.u_category is null then 0 else -1 )<>category_src_c_key