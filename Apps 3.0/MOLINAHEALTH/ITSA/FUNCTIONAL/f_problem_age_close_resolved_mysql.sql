...
(select count(1) cnt FROM (select * from molinahealth_mdsdb.problem_final where opened_at < closed_at) SRC 
  join molinahealth_mdwdb.f_problem f ON (SRC.sys_id =f.row_id  
...
JOIN molinahealth_mdwdb.d_problem a ON a.row_key = f.problem_key
AND f.source_id = a.source_id 

WHERE TIMESTAMPDIFF(DAY,SRC.opened_at,SRC.closed_at)<> f.age
