...
from 
(select 
case when src.meta is not null then 1 else 0 end as src_not_null_count,

case when trgt.meta is not null and trgt.meta <> 'UNSPECIFIED' then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.meta,'UNSPECIFIED') <> COALESCE(trgt.meta,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.meta,'UNSPECIFIED') <> COALESCE(trgt.meta,'UNSPECIFIED') then 1 else 0 end as failures_cnt
