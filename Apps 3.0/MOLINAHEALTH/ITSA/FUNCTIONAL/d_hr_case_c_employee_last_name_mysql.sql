...
case when COALESCE(src.last_name,'UNSPECIFIED') <> COALESCE(trgt.employee_last_name,'UNSPECIFIED') then src.record_id else '' end as failures,

case when COALESCE(src.last_name,'UNSPECIFIED') <> COALESCE(trgt.employee_last_name,'UNSPECIFIED') then 1 else 0 end as failures_cnt
