...
LEFT JOIN molinahealth_mdwdb.d_internal_contact LKP 
 ON ( concat('INTERNAL_CONTACT~',SRC.u_it_incident_facilitator)= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id ) 
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_it_incident_facilitator IS NULL THEN 0 else -1 end)<> (TRGT.it_incident_facilitator_c_key);
