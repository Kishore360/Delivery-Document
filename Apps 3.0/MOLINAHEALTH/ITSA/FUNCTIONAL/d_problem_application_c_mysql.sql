...
when sum(warnings_cnt) > 0 and length(group_concat(distinct failures)) >= 1024 then concat('Warning. Look up does not exists for ',sum(warnings_cnt),' records. Few (sys_id || cmdb_ci) - ',group_concat(distinct warnings))
...
LEFT JOIN molinahealth_mdwdb.d_problem trgt ON src.sys_id = trgt.row_id AND src.sourceinstance = trgt.source_id
