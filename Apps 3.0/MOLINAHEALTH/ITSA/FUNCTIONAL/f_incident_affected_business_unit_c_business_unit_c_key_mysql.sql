...
when sum(warnings_cnt) > 0 and length(group_concat(distinct warnings)) >= 1024 then concat('Warning. Look up does not exists for ',sum(warnings_cnt),' records. Few (sys_id || business_unit) - ',substring_index(group_concat(distinct warnings),',',-sum(warnings_cnt)))

when sum(warnings_cnt) > 0 and length(group_concat(distinct warnings)) < 1024 then concat('Warning. Look up does not exists for ',sum(warnings_cnt),' records. (sys_id || business_unit) : ',substring_index(group_concat(distinct warnings),',',-sum(warnings_cnt)))
...
case when src.business_unit is not null and lkp.row_id is null then concat(src.row_id,' || ',src.business_unit) else '' end as warnings,
...
left join molinahealth_mdwdb.d_business_unit_c lkp on src.business_unit = lkp.row_id and src.source_id = lkp.source_id) fnl ;