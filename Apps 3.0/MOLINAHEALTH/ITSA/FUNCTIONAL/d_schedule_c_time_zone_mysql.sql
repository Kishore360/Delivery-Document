...
case when COALESCE(src.time_zone,'UNSPECIFIED') <> COALESCE(trgt.time_zone,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.time_zone,'UNSPECIFIED') <> COALESCE(trgt.time_zone,'UNSPECIFIED') then 1 else 0 end as failures_cnt
