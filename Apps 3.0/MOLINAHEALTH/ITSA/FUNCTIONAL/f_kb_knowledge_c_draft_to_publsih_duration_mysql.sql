...
from 
(select 
case when CASE WHEN timestampdiff(second,src.drafted,src.published) <=0 then 0 else timestampdiff(second,src.drafted,src.published) end is not null then 1 else 0 end as src_not_null_count,

case when trgt.draft_to_publish_duration is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(CASE WHEN timestampdiff(second,src.drafted,src.published) <=0 then 0 else timestampdiff(second,src.drafted,src.published) end ,0) <> COALESCE(trgt.draft_to_publish_duration,0) then src.sys_id else '' end as failures,

case when COALESCE(CASE WHEN timestampdiff(second,src.drafted,src.published) <=0 then 0 else timestampdiff(second,src.drafted,src.published) end ,0) <> COALESCE(trgt.draft_to_publish_duration,0) then 1 else 0 end as failures_cnt
from 
