...
join(
select -- c.name,user_input,priority,month_end_date_key,
c.name name,user_input user_input,sf.label,date_format(month_end_date,'%Y%m%d') month_end_date,count(1) src from molinahealth_mdsdb.incident_final i
join  molinahealth_mdwdb.d_calendar_date d on d.row_id =(select distinct  aggregate_c_key from molinahealth_mdwdb.f_incident_backlog_aggregator_monthly_c)
...
on a.name=b.name and a.user_input=b.user_input and a.dimension_name=b.label and b.month_end_date=a.aggregate_c_key
where b.src<>a.trgt;
