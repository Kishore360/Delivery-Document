...
from
(select 
case when src.cnt <> trgt.reassignee_count_c then src.documentkey else '' end as failures,

case when src.cnt <> trgt.reassignee_count_c  then 1 else 0 end as failures_cnt

from 
(select documentkey,sourceinstance,count(sys_id) as cnt
from molinahealth_mdsdb.sys_audit_final where tablename = 'sc_req_item' and fieldname = 'assigned_to' and oldvalue is not null
