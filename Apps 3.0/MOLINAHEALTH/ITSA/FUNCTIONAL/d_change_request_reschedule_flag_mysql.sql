...
FROM molinahealth_mdsdb.change_request_final SRC
 LEFT JOIN (select row_key,row_id,source_id,rescheduled_flag from molinahealth_mdwdb.d_change_request) TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
 LEFT JOIN molinahealth_mdwdb.f_change_request TRGT1 
