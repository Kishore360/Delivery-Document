...
case when COALESCE(src.irequest,'UNSPECIFIED') <> COALESCE(trgt.irequest,'UNSPECIFIED') then src.record_id else '' end as failures,

case when COALESCE(src.irequest,'UNSPECIFIED') <> COALESCE(trgt.irequest,'UNSPECIFIED') then 1 else 0 end as failures_cnt
