...
case when COALESCE(src.type,'UNSPECIFIED') <> COALESCE(trgt.type,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.type,'UNSPECIFIED') <> COALESCE(trgt.type,'UNSPECIFIED') then 1 else 0 end as failures_cnt
