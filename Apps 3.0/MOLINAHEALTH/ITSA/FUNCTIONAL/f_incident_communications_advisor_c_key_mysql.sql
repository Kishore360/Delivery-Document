...
LEFT JOIN molinahealth_mdwdb.d_internal_contact LKP 
 ON ( concat('INTERNAL_CONTACT~',SRC.u_communications_advisor)= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id ) 
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_communications_advisor IS NULL THEN 0 else -1 end)<> (TRGT.communications_advisor_c_key);
