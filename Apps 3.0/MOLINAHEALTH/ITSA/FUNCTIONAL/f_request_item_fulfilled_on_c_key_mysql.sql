...
(select src.sys_id,lv_mp.dimension_wh_code,f_trgt.row_id,src.src_fulfilled_on_c_key ,f_trgt.fulfilled_on_c_key as trgt_fulfilled_on_c_key
from 
...
left join molinahealth_mdwdb.f_request_item f_trgt on f_trgt.row_id = src.sys_id and f_trgt.source_id = src.sourceinstance 
left join molinahealth_mdwdb.d_lov_map lv_mp on f_trgt.state_src_key = lv_mp.src_key and lv_mp.dimension_class = 'STATE~SC_REQ_ITEM') fnl
where (case when dimension_wh_code = 'OPEN' then 0 else src_fulfilled_on_c_key end) <> trgt_fulfilled_on_c_key;
