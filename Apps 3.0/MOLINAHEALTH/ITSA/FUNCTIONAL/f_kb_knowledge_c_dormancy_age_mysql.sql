...
when sum(failures_cnt) > 0 and length(group_concat(distinct failures)) >= 1024 then 
concat('FAILURE. Data does not match for ',sum(failures_cnt),' records. Few sys ids are : ',
substring_index(group_concat(distinct failures),',',-31))

when sum(failures_cnt) > 0 and length(group_concat(distinct failures)) < 1024
then concat('FAILURE. Data does not match for ',sum(failures_cnt),' records. Sys Ids : ',
substring_index(group_concat(distinct failures),',',-sum(failures_cnt)))
...
from 
(select 
case when src.dormancy_age is not null then 1 else 0 end as src_not_null_count,

case when trgt.dormancy_age is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.dormancy_age,0) <> COALESCE(trgt.dormancy_age,0) then src.sys_id else '' end as failures,

case when COALESCE(src.dormancy_age,0) <> COALESCE(trgt.dormancy_age,0) then 1 else 0 end as failures_cnt
from 
(select 
kb.sys_id,kb.sourceinstance,
case 
when sc_kb.label = 'Retired' then COALESCE(case when timestampdiff(second,kb.sys_updated_on,COALESCE(src_sa.retired_on,kb.retired)) <= 0 then 0 else timestampdiff(second,kb.sys_updated_on,COALESCE(src_sa.retired_on,kb.retired)) end ,0)
when sc_kb.label <> 'Retired' then COALESCE(case when timestampdiff(second,kb.sys_updated_on,lastupdated) <=0 then 0 else timestampdiff(second,kb.sys_updated_on,lastupdated) end ,0)
else 0 end as dormancy_age
