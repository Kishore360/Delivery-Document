...
when sum(warnings_cnt) > 0 and length(group_concat(distinct warnings)) >= 1024 then concat('Warning. Look up does not exists for ',sum(warnings_cnt),' records. Few (sys_id || u_business_area) - ',substring_index(group_concat(distinct warnings),',',-sum(warnings_cnt)))
