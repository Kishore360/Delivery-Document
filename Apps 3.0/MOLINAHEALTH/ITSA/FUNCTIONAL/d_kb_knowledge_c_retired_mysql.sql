...
from 
(select 
case when src.retired_on is not null then 1 else 0 end as src_not_null_count,

case when trgt.retired_on is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.retired_on,'1970-01-01') <> COALESCE(trgt.retired_on,'1970-01-01') then src.sys_id else '' end as failures,

case when COALESCE(src.retired_on,'1970-01-01') <> COALESCE(trgt.retired_on,'1970-01-01') then 1 else 0 end as failures_cnt
from 
