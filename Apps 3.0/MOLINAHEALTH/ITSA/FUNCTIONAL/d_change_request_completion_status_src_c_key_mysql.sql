SELECT 
-- src.sys_id,trgt.row_id,COALESCE(CONCAT('COMPLETION_STATUS_C','~','CHANGE_REQUEST','~~~',UPPER(src.u_completion_status)),'UNSPECIFIED') as src_completion_status, lkp.row_id as lkp_completion_status, COALESCE(lkp.row_key,CASE WHEN src.u_completion_status IS NULL THEN 0 else -1 end) as src_completion_status_src_c_key,completion_status_src_c_key as trgt_completion_status_src_c_key
CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_request.completion_status_src_c_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(lkp.row_key,CASE WHEN src.u_completion_status IS NULL THEN 0 else -1 end) <> COALESCE(trgt.completion_status_src_c_key,0);