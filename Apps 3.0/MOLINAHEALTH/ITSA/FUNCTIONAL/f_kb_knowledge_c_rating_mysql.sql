...
from 
(select 
case when src.rating is not null then 1 else 0 end as src_not_null_count,

case when trgt.rating is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.rating,0) <> COALESCE(trgt.rating,0) then src.sys_id else '' end as failures,

case when COALESCE(src.rating,0) <> COALESCE(trgt.rating,0) then 1 else 0 end as failures_cnt
