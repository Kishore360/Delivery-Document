...
case when COALESCE(src.employee_num,'UNSPECIFIED') <> COALESCE(trgt.employee_number,'UNSPECIFIED') then src.record_id else '' end as failures,

case when COALESCE(src.employee_num,'UNSPECIFIED') <> COALESCE(trgt.employee_number,'UNSPECIFIED') then 1 else 0 end as failures_cnt
