...
from  
(select   
case when src.u_category is not null and lkp.row_id is null then concat(src.sys_id,' || ',src.u_category) else '' end as warnings,

case when src.u_category is not null and lkp.row_id is null then 1 else 0 end as warnings_cnt,

case when COALESCE(lkp.row_key,CASE WHEN src.u_category IS NULL THEN 0 else -1 end) <> COALESCE(trgt.category_src_key,0)  then src.sys_id else '' end as failures,

case when COALESCE(lkp.row_key,CASE WHEN src.u_category IS NULL THEN 0 else -1 end) <> COALESCE(trgt.category_src_key,0) then 1 else 0 end as failures_cnt
...
left join molinahealth_mdwdb.d_lov lkp on concat('CATEGORY~HR_CASE~~~',upper(src.u_category))=  lkp.row_id and lkp.dimension_class = 'CATEGORY~HR_CASE')fnl;