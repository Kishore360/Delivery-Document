...
from 
(select 
-- src.application_id,trgt.row_id as trgt_row_id,src.impacted_departments as src_impacted_departments, trgt.impacted_departments_c as trgt_impacted_departments
case when COALESCE(src.impacted_departments,'UNSPECIFIED') <> COALESCE(trgt.impacted_departments_c,'UNSPECIFIED') then src.application_id else '' end as failures,
case when COALESCE(src.impacted_departments,'UNSPECIFIED') <> COALESCE(trgt.impacted_departments_c,'UNSPECIFIED') then 1 else 0 end as failures_cnt
