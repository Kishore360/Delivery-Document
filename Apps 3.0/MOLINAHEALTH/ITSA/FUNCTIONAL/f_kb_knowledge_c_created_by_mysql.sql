...
from 
(select 
case when src.sys_created_by is not null then 1 else 0 end as src_not_null_count,

case when trgt.created_by is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.sys_created_by,'Numerify') <> COALESCE(trgt.created_by,'Numerify') then src.sys_id else '' end as failures,

case when COALESCE(src.sys_created_by,'Numerify') <> COALESCE(trgt.created_by,'Numerify') then 1 else 0 end as failures_cnt
