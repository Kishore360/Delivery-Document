...
from 
(select 
case when COALESCE(src.u_tier,0) <> COALESCE(trgt.tier_c,0) then src.sys_id else '' end as failures,
case when COALESCE(src.u_tier,0) <> COALESCE(trgt.tier_c,0) then 1 else 0 end as failures_cnt
