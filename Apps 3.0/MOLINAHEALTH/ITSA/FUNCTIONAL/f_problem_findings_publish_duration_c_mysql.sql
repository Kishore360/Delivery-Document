...
from 
(select 
case when COALESCE(lkp.findings_publish_duration,0) <> COALESCE(trgt.findings_publish_duration_c,0) then src.sys_id else '' end as failures,
case when COALESCE(lkp.findings_publish_duration,0) <> COALESCE(trgt.findings_publish_duration_c,0) then 1 else 0 end as failures_cnt
from molinahealth_mdsdb.problem_final src
left join molinahealth_mdwdb.f_problem trgt on trgt.row_id = src.sys_id and trgt.source_id = src.sourceinstance 
left join 
(select sys_id,sourceinstance,sum(timestampdiff(second,convert_tz(state_created_on,'UTC','America/Los_Angeles'),convert_tz(Result_code_created_on,'UTC','America/Los_Angeles'))) as findings_publish_duration
from 
(select
pf.sys_id,pf.sourceinstance,pf.number,pf.state,pf.u_Result_code,
su_state.documentkey,su_state.sys_created_on as state_created_on,su_state.sequence_id as state_sequence_id,
su_Result_code.sys_created_on as Result_code_created_on,su_Result_code.sequence_id as Result_code_sequence_id
...
join molinahealth_mdsdb.sys_choice_final sc_Result on sucode.newvalue = sc_Result.value 
where sucode.tablename = 'problem' and sucode.fieldname = 'u_Result_code' and  sc_Result.name ='problem' and sc_Result.element = 'u_Result_code' and sc_Result.label = 'Initial Findings Pending Review' order by documentkey,sys_created_on) sa
join (select @prev:='NA',@rownum:=0) t)su_Result_code on su_state.documentkey = su_Result_code.documentkey and su_state.sequence_id = su_Result_code.sequence_id 
where su_Result_code.sequence_id is not null) tmp 
