...
case when COALESCE(DATE_FORMAT(CONVERT_TZ(lkp.change_successful_on,'UTC','America/Los_Angeles'),'%Y%m%d'),'19700101') <> COALESCE(trgt.change_successful_on_c_key,'19700101') then src.sys_id else '' end as failures,

case when COALESCE(DATE_FORMAT(CONVERT_TZ(lkp.change_successful_on,'UTC','America/Los_Angeles'),'%Y%m%d'),'19700101') <> COALESCE(trgt.change_successful_on_c_key,'19700101') then 1 else 0 end as failures_cnt
