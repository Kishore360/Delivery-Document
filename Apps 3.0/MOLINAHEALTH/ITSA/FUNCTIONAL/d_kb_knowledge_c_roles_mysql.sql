...
from 
(select 
case when src.roles is not null then 1 else 0 end as src_not_null_count,

case when trgt.roles is not null and trgt.roles <>  'UNSPECIFIED' then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.roles,'UNSPECIFIED') <> COALESCE(trgt.roles,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.roles,'UNSPECIFIED') <> COALESCE(trgt.roles,'UNSPECIFIED') then 1 else 0 end as failures_cnt
