...
from 
( SELECT CONCAT(sys_id,'~',COALESCE(SUBSTRING_INDEX(SUBSTRING_INDEX(TRIM(roles), ',', n.n), ',', -1),'UNSPECIFIED')) AS row_id, sourceinstance AS source_id,	sys_id as knowledge_id
	...
	CROSS JOIN
	  ...
	  (SELECT 0 AS N UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) b
		ORDER BY n) n
	 ON n.n <= 1 + (LENGTH(COALESCE(TRIM(roles),'UNSPECIFIED')) - LENGTH(REPLACE(COALESCE(TRIM(roles),'UNSPECIFIED'), ',', ''))) ) src
