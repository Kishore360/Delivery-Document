...
case when COALESCE(src.name,'UNSPECIFIED') <> COALESCE(trgt.name,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.name,'UNSPECIFIED') <> COALESCE(trgt.name,'UNSPECIFIED') then 1 else 0 end as failures_cnt
