...
case when trgt.knowledge_number is not null and trgt.knowledge_number <> 'UNSPECIFIED' then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.number,'UNSPECIFIED') <> COALESCE(trgt.knowledge_number,'UNSPECIFIED') then src.sys_id else '' end as failures,

case when COALESCE(src.number,'UNSPECIFIED') <> COALESCE(trgt.knowledge_number,'UNSPECIFIED') then 1 else 0 end as failures_cnt
