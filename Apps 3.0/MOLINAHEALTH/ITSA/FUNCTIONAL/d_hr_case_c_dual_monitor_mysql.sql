...
case when COALESCE(src.dual_monitor,'UNSPECIFIED') <> COALESCE(trgt.dual_monitor,'UNSPECIFIED') then src.record_id else '' end as failures,

case when COALESCE(src.dual_monitor,'UNSPECIFIED') <> COALESCE(trgt.dual_monitor,'UNSPECIFIED') then 1 else 0 end as failures_cnt
