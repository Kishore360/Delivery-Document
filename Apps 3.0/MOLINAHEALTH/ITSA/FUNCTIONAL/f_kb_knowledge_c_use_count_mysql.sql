...
from 
(select 
case when src.use_count is not null then 1 else 0 end as src_not_null_count,

case when trgt.use_count is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.use_count,0) <> COALESCE(trgt.use_count,0) then src.sys_id else '' end as failures,

case when COALESCE(src.use_count,0) <> COALESCE(trgt.use_count,0) then 1 else 0 end as failures_cnt
