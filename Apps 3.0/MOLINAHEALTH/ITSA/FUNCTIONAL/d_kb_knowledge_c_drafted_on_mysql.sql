...
from 
(select 
case when src.drafted is not null then 1 else 0 end as src_not_null_count,

case when trgt.drafted_on is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.drafted,'1970-01-01') <> COALESCE(trgt.drafted_on,'1970-01-01') then src.sys_id else '' end as failures,

case when COALESCE(src.drafted,'1970-01-01') <> COALESCE(trgt.drafted_on,'1970-01-01') then 1 else 0 end as failures_cnt
from 
