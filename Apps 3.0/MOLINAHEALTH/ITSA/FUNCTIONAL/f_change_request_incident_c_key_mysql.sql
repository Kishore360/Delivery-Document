...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 LEFT JOIN molinahealth_mdwdb.d_incident LKP 
 ON UPPER(u_incident)= LKP.row_id 
 
AND SRC.sourceinstance= LKP.source_id 
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_incident IS NULL THEN 0 else -1 end)<> (TRGT.incident_c_key)
