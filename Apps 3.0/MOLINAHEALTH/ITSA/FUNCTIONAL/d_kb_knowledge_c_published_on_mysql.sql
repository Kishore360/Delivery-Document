...
from 
(select 
case when src.published is not null then 1 else 0 end as src_not_null_count,

case when trgt.published_on is not null then 1 else 0 end as trgt_not_null_count,

case when COALESCE(src.published,'1970-01-01') <> COALESCE(trgt.published_on,'1970-01-01') then src.sys_id else '' end as failures,

case when COALESCE(src.published,'1970-01-01') <> COALESCE(trgt.published_on,'1970-01-01') then 1 else 0 end as failures_cnt
from 
(select sys_id,sourceinstance,COALESCE(drafted_on,convert_tz(kb.sys_created_on,'UTC','America/Los_Angeles')) as drafted,reviewed_on,provisional_published_on,COALESCE(published_on,kb.published) as published,retired_on
