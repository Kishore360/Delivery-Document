...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for d_task_sla.sla_category_c_key     ' ELSE 'SUCCESS' END as Message from
(
...
WHERE COALESCE(lkp.row_key, CASE WHEN src.name IS NULL THEN 0 ELSE -1 end)<>  coalesce(sla_category_c_key,0))a
;