...
select count(1) cnt
from 
(select sys_id,sourceinstance,newvalue,CONCAT('INTERNAL_CONTACT~',newvalue) as lkp_row_id
from cardinalhealth_mdsdb.sys_audit_final 
where
tablename = 'enhancement' ) src
left join 
(select row_id,source_id,task_attribute_wh_new_value,task_attribute_wh_new_value_key
