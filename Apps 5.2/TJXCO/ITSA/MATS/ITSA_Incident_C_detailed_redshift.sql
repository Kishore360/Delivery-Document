...
on (a112.category_src_key=a130.row_key)
union
...
on (a112.close_code_src_key=a131.row_key)
union
...
on (a112.priority_src_key=a132.row_key)
union
...
on (a112.met_resolution_sla_flag_key=a133.row_key)
union
...
on (a112.met_response_sla_flag_key=a134.row_key)
union
...
on (a112.sub_category_src_key=a137.row_key)
union
select'ldb.d_lov_incident_first_pass_reason_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident       a11
 join ldb.d_incident       a112
on (a11.incident_key=a112.row_key)
join ldb.d_lov_incident_first_pass_reason_c       a138
on (a112.first_pass_reason_c_key=a138.row_key)
union
