...
from 
(select count(1) as cnt  from (select sys_id, sourceinstance, cdctime, approver from  mcdonalds_mdsdb.sysapproval_approver_final) src 
...
where (src.cdctime<=f1.lastupdated) and  coalesce(LKP.row_key, case when src.approver is null then 0 else -1 end) <> trgt.approver_c_key) temp;