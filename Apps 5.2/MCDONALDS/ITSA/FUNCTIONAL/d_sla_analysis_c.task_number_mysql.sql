...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_sla_analysis_c.task_number' ELSE 'SUCCESS' END as Message
from 
(select count(1) as cnt  from (select sys_id, sourceinstance,cdctime, number from  mcdonalds_mdsdb.task_final) src 
...
where (src.cdctime<=f1.lastupdated) and  src.number <> trgt.task_number) temp;