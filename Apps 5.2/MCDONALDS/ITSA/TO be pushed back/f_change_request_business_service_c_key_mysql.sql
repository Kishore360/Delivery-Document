...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_change_request.business_service_c_key' ELSE 'SUCCESS' END as Message 
...
where (src.cdctime<=f1.lastupdated) and COALESCE(lkp.row_key,CASE WHEN src.u_business_service IS NULL THEN 0 else -1 end) <>business_service_c_key
;