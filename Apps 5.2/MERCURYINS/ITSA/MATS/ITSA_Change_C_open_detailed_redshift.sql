...
select'ldb.d_change_request_reported_type' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.d_change_request_reported_type       a19
...
select'ldb.d_internal_contact_change_owner_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.d_internal_contact_change_owner_c       a110
on (a11.change_owner_c_key = a110.row_key)
union
...
select'ldb.d_lov_change_request_impact_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.d_lov_change_request_impact_c       a113
...
select'ldb.d_lov_change_request_environment_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.d_lov_change_request_environment_c       a124
on (a11.environment_src_c_key = a124.row_key)
union
select'ldb.d_lov_includes_data_change_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.d_lov_includes_data_change_c       a125
on (a11.includes_data_change_c_key = a125.row_key)
union
...
on (a11.opened_by_key = a127.row_key)
union
select'ldb.o_data_freshness' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_change_request       a11 
join ldb.o_data_freshness       a128
on (a11.source_id = a128.source_id)
union
...
on (a13.week_start_date_key = a130.row_key)
union
