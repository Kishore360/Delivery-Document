...
from 
(select sys_id,sourceinstance, state from  pan6_mdsdb.u_service_request_final where CDCTYPE<>'D') SRC 
LEFT JOIN (select row_id, row_key, source_id from pan6_mdwdb.d_lov where soft_deleted_flag<>'Y') LKP 
 
ON ( concat('STATE_C~SERVICE_REQUEST_C~',upper(SRC.state))= LKP.row_id AND SRC.sourceinstance= LKP.source_id ) 
 
LEFT JOIN pan6_mdwdb.d_service_request_c TRGT  ON (SRC.sys_id =TRGT.row_id  
