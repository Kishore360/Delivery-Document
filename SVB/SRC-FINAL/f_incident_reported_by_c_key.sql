...
FROM svb_mdsdb.incident_final )a
 left JOIN  
(SELECT  reported_by_c_key,source_id,row_id FROM  svb_mdwdb.d_internal_organization
...
where b_reported_by_c_key  <>(case when a_u_contact is null then 'UNSPECIFIED' else coalesce(row_key,-1) end))E;