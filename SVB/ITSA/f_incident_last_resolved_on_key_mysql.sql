...
DATE_FORMAT(CONVERT_TZ(u_resolved,'GMT','America/Los_Angeles'),'%Y')  AS   year,
DATE_FORMAT(CONVERT_TZ(u_resolved,'GMT','America/Los_Angeles'),'%Y%m%d')  AS   last_resolved_on_key
FROM svb_mdsdb.incident_final 
#where sys_id='092bf5f2f547b9c00f4bd5728aaf9afc'
)A
 JOIN  
(SELECT  last_resolved_on_key,source_id,ROW_ID FROM  svb_mdwdb.f_incident
