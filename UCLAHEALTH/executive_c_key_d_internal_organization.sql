...
(case when a.u_executive is null then 'UNSPECIFIED' else coalesce(c.row_key,-1) end)
FROM
...
where b_executive_c_key<>(case when a_u_executive is null then 'UNSPECIFIED' else coalesce(row_key,-1) end))E;
