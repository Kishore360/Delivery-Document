...
SELECT A.SYS_ID,B.ROW_ID,A.  outage_type_src_c_id  A_outage_type_src_c_id,B.  outage_type_src_c_id B_outage_type_src_c_id FROM
(SELECT SYS_ID,sourceinstance, u_outage_type AS   outage_type_src_c_id
FROM uclahealth_mdsdb.incident_final )A
LEFT OUTER JOIN  
(SELECT   REPLACE(outage_type_src_c_id,'OUTAGE_TYPE_C~INCIDENT~~~','') AS outage_type_src_c_id
