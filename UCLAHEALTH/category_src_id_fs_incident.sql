...
SELECT A.SYS_ID,B.ROW_ID,CONCAT('CATEGORY~INCIDENT~~~',A.  category_src_id) A_category_src_id ,B.  category_src_id B_category_src_id  FROM
(SELECT SYS_ID,sourceinstance, u_category AS   category_src_id 
FROM uclahealth_mdsdb.incident_final )A
LEFT OUTER JOIN  
(SELECT   category_src_id  as category_src_id
