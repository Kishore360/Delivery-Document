...
left outer join weillcornell_mdwdb.d_configuration_item c on a.u_bsd_business_service=c.row_id
where case when  a.u_bsd_business_service is null then '0' 
else case when c.row_key is null then '-1' else c.row_key end end <>b.bsd_business_service_c_key ) c;