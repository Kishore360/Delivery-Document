...
ldb.dh_user_group_classification_hierarchy	a16
	...
	select  'dh_user_group_classification_hierarchy_level1' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	
...
ldb.d_incident_variable_agent_down_check_box	a19
	...
	select  'd_incident_variable_what_are_you_attempting_to_do_that_you_cannot' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	

ldb.d_incident_variable_what_are_you_attempting_to_do_that_you_cannot	a110
	  on 	(a11.incident_key = a110.row_key)union
	select  'd_incident_variable_has_this_functionality_ever_worked_successfully' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	

ldb.d_incident_variable_has_this_functionality_ever_worked_successfully	a111
	  on 	(a11.incident_key = a111.row_key)union
	select  'd_incident_variable_impacted_agent_information' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	

ldb.d_incident_variable_impacted_agent_information	a112
	  on 	(a11.incident_key = a112.row_key)union
	select  'd_incident_variable_how_many_agents_are_impacted' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	
...
ldb.d_incident_variable_other_info	a114
	...
	select  'd_incident_variable_what_have_you_already_tried_to_resolve_this_issue' as Table_Name,count(a11.row_key) Row_Count from ldb.f_incident a11 join	
...
ldb.d_configuration_item	a124
	  on 	(a11.business_service_key_c = a124.row_key)union
...
ldb.d_variable_lov_environment	a126
...
ldb.d_variable_lov_request_type	a127
...
ldb.d_variable_lov_tool_name	a129
...
ldb.d_varaible_lov_inin_tool	a137
...
ldb.d_incident_symptom_c	a139
	  on 	(a11.symptom_src_key_c = a139.row_key)union
...
ldb.d_incident_sub_type_c	a146
	  on 	(a11.sub_type_key_c = a146.row_key)union
