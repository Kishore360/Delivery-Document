...
on (LKP.row_id = date_format(convert_tz(coalesce(SRC.opened_at,'UNSPECIFIED'),'GMT','US/Central'),'%Y%m%d') and LKP.source_id=0
)
WHERE  case when dlm.dimension_wh_code = 'OPEN' then (LKP.row_key) else null end <> (TRGT.OPENED_ON_KEY); 