...
where case when 
when (ci.sys_class_name ='cmdb_ci_osx_server' ) then COALESCE(substring_index(osx.os_version,'.',2),'UNSPECIFIED')
when (ci.sys_class_name='cmdb_ci_linux_server') then COALESCE(substring_index(lnx.os_version,'.',2),'UNSPECIFIED')
when (ci.sys_class_name='cmdb_ci_unix_server') then COALESCE(substring_index(unx.os_version,'.',2),'UNSPECIFIED')
when (ci.sys_class_name='cmdb_ci_esx_server') then COALESCE(substring_index(esx.os_version,'.',2),'UNSPECIFIED')
when (ci.sys_class_name='cmdb_ci_mainframe') then COALESCE(substring_index(cim.os_version,'.',2),'UNSPECIFIED')
when (ci.sys_class_name='u_cmdb_ci_server_as400') then COALESCE(substring_index(cas.os_version,'.',2),'UNSPECIFIED')
 else 'UNSPECIFIED' end <>  TRGT.os_version_c)a
