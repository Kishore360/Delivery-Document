...
on (LKP.row_id  = date_format(convert_tz(SRC.start_date,'GMT','America/New_York'),'%Y%m%d')  and LKP.source_id=2
)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.start_date IS NULL THEN 0 else '-1' end)  <> COALESCE(TRGT.planned_start_on_fiscal_c_key,'') 
