SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (COALESCE( CONCAT('PROJECT','~','SCOPE','~~',SRC.u_scope))= LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_scope IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_scope_color_c_key,'')
