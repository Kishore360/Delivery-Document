SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (COALESCE( CONCAT('PROJECT','~','SCHEDULE','~~',SRC.u_timeline))= LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_timeline IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_schedule_color_c_key,'')
