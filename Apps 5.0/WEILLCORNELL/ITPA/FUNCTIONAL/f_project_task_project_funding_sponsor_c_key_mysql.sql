SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (concat('INTERNAL_CONTACT~',SRC.u_prj_sponsor_funding)= LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_prj_sponsor_funding IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_funding_sponsor_c_key,'')
