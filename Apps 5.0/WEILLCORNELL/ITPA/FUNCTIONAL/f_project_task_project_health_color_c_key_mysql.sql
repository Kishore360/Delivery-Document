SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (COALESCE( CONCAT('PROJECT','~','HEALTH','~~',SRC.u_project_status))= LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_project_status IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_health_color_c_key,'')
