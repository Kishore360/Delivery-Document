...
on (LKP.row_id  = date_format(convert_tz(SRC.work_end,'GMT','America/New_York'),'%Y%m%d')  and LKP.source_id=2
)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.work_end IS NULL THEN 0 else '-1' end)  <> COALESCE(TRGT.actual_end_on_fiscal_c_key,'') 
