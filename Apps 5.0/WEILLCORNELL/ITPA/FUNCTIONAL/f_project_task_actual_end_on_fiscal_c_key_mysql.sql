...
on LKP.row_id  = CONCAT(date_format(convert_tz(SRC.work_end,'GMT','America/New_York'),'%Y%m%d'),'~2')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.work_end IS NULL THEN 0 else '-1' end)  <> TRGT.actual_end_on_fiscal_c_key