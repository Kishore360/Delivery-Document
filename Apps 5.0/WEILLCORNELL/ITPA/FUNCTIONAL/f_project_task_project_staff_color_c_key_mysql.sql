SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (COALESCE( CONCAT('PROJECT','~','STAFF','~~',SRC.u_staff))= LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_staff IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_staff_color_c_key,'')
