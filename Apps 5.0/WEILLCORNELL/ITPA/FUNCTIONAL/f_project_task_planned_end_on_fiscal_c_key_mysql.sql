...
on LKP.row_id  = CONCAT(date_format(convert_tz(SRC.end_date,'GMT','America/New_York'),'%Y%m%d'),'~2')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.end_date IS NULL THEN 0 else '-1' end)  <> TRGT.planned_end_on_fiscal_c_key
