...
(select count(*) as src_count from weillcornell_mdwdb.d_incident where soft_deleted_flag <>'Y')A,
(select count(*)+2 as tar_count from weillcornell_mdsdb.incident_final where (sys_id,number) not in (
