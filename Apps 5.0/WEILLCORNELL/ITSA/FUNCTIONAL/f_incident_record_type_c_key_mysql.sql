...
LEFT JOIN weillcornell_mdwdb.d_lov LKP 
 ON ( concat('RECORD_TYPE','~','INCIDENT','~','~','~',upper(u_record_type))= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_record_type IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.record_type_c_key,'') 
and    SRC.sys_id not in (select B.sys_id
from weillcornell_mdsdb.incident_final B
