...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_employee_review_assessment_c.assessment_metrics_c_key' ELSE 'SUCCESS' END as Message 
FROM ( SELECT CONCAT(src1.sys_id,'~',src3.sys_id) as sys_id,
src1.sourceinstance,src3.metric as val
FROM
...
LEFT JOIN wow_mdwdb.d_assessment_metrics_c lkp
ON COALESCE(MAIN_SRC1.val,'UNSPECIFIED') = lkp.row_id and MAIN_SRC1.sourceinstance = lkp.source_id
WHERE COALESCE(lkp.row_key,CASE WHEN MAIN_SRC1.val IS NULL THEN 0 else -1 end) <>assessment_metrics_c_key
;