...
LEFT JOIN qualcomm_mdsdb.task_final task  ON SRC.sysapproval = task.sys_id -- and SRC.cdctype='X'
LEFT JOIN  qualcomm_mdwdb.d_task_approval_c TRGT ON (SRC.sys_id =TRGT.row_id  AND SRC.sourceinstance= TRGT.source_id  )
WHERE 
CONVERT_TZ(SRC.sys_updated_on,'GMT','GMT') <> TRGT.changed_on  and SRC.cdctype='X'
) temp
 ;