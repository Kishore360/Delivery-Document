...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_major_incident_c.state_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.state IS NULL THEN 0 else -1 end)<> (TRGT.state_key);