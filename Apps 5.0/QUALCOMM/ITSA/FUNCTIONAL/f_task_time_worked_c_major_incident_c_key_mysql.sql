...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_task_time_worked_c.major_incident_c_key
...
LEFT JOIN qualcomm_mdwdb.f_task_time_worked_c TRGT 
 ...
 AND SRC.sourceinstance= TRGT.source_id  )
   join qualcomm_mdwdb.d_major_incident_c LKP 
	...
	AND task.sys_class_name='u_major_incident'

 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.task IS NULL THEN 0 else -1 end) <> (TRGT.major_incident_c_key
);
