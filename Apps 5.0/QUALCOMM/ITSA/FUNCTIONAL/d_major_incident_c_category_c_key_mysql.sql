...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_major_incident_c.category_c_key' ELSE 'SUCCESS' END as Message
FROM qualcomm_mdsdb.u_major_incident_final SRC 
 ...
 ON COALESCE(CONCAT('U_CATEGORY~U_MAJOR_INCIDENT~~~',UPPER(SRC.u_category)),'UNSPECIFIED')= LKP.row_id 
