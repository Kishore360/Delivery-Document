...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_task_approval_c.approval_state_c_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN qualcomm_mdwdb.f_task_approval_c TRGT 
 ...
 ON COALESCE( CONCAT('STATE~SYSAPPROVAL_APPROVER~~~',UPPER(SRC.state)),'UNSPECIFIED')= LKP.row_id 
