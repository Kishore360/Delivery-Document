...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_survey_instance_journal_c.survey_type' ELSE 'SUCCESS' END as Message
FROM 	qualcomm_mdsdb.sys_journal_field_final SRC
JOIN	qualcomm_mdwdb.d_survey_instance_journal_c TRGT 
	 ON (SRC.sys_id=TRGT.row_id 
 AND SRC.sourceinstance=TRGT.source_id )
WHERE SRC.name<> TRGT.survey_type
