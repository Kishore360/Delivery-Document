 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project.is_leaf' ELSE 'SUCCESS' END as Message
 FROM
...
JOIN #DWH_TABLE_SCHEMA.d_project TRGT 
 ON (SRC_F.row_id =TRGT.row_id 
 AND SRC_F.source_id =TRGT.source_id )
WHERE COALESCE(CASE WHEN SRC_F.Cnt = 0 THEN 'Y' ELSE 'N' END , '')<> COALESCE(TRGT.is_leaf_flag, '')
