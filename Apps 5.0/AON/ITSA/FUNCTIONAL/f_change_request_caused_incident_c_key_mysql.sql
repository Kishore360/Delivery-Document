 ...
 ON ( COALESCE(CONCAT(CAUSED_INCIDENT_C~CHANGE_REQUEST),'UNSPECIFIED') = LKP.row_id 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_caused_incident  IS NULL THEN 0 else -1 end) <> (TRGT.caused_incident_c_key))b;
