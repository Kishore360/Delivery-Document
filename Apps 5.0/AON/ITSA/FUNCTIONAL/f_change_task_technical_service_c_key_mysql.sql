 ...
 FROM (
 select count(1) as cnt from aon_mdsdb.change_task_final SRC
...
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_technical_service  IS NULL THEN 0 else -1 end)<> (TRGT.technical_service_c_key)
 )b;