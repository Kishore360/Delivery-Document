...
UNION 
SELECT '% Priority Escalated Incidents last to last 30 days',
(X.a-Y.b)/Y.b *100.0 AS Priority_Escalated_Incidents_last_to_last_30_days FROM 
(
SELECT count(a.row_key)*1.0 AS a  
FROM ldb.f_incident a 
JOIN ldb.d_calendar_date b ON a.opened_on_key=b.row_key
JOIN ldb.d_calendar_month c ON b.month_start_date_key=c.row_key
JOIN ldb.d_incident d ON a.incident_key=d.row_key
Where (b.lagging_count_of_days between 0 and 29 and d.priority_escalated_flag='Y'))X 
CROSS JOIN 
(
SELECT count(a.row_key)*1.0 as b 
FROM ldb.f_incident a 
...
JOIN ldb.d_calendar_month c ON b.month_start_date_key=c.row_key
JOIN ldb.d_incident d ON a.incident_key=d.row_key
Where (b.lagging_count_of_days between 30 and 59 and d.priority_escalated_flag='Y')
) Y 
UNION 
SELECT '% Reopened Incidents last to last 30 days',
(X.a-Y.b)/Y.b *100.0 AS Reopened_Incidents_last_to_last_30_days FROM 
(
SELECT count(a.row_key)*1.0 AS a 
FROM ldb.f_incident a 
JOIN ldb.d_calendar_date b ON a.opened_on_key=b.row_key
JOIN ldb.d_calendar_month c ON b.month_start_date_key=c.row_key
JOIN ldb.d_incident d ON a.incident_key=d.row_key
Where (b.lagging_count_of_days between 0 and 29 and d.reopened_flag='Y')
)X 
CROSS JOIN 
(
SELECT count(a.row_key)*1.0 as b 
...
JOIN ldb.d_calendar_month c ON b.month_start_date_key=c.row_key
JOIN ldb.d_incident d ON a.incident_key=d.row_key
Where (b.lagging_count_of_days between 30 and 59 and d.reopened_flag='Y')
) Y 
UNION 
...
UNION 
SELECT 'Avg Incident Open To Close Duration (Days)',
...
UNION 
SELECT 'Current Open Incidents',
count(a.row_key) as Current_Open_count
FROM ldb.f_incident a 
JOIN ldb.d_calendar_date b ON a.opened_on_key=b.row_key
JOIN ldb.d_calendar_month c ON b.month_start_date_key=c.row_key
JOIN ldb.d_incident d ON a.incident_key=d.row_key
JOIN ldb.d_incident_state x ON d.state_src_key=x.row_key
where (b.lagging_count_of_month between 0 and 24 and x.wh_dimension_code='OPEN')
UNION 
...
where	(a13.dormant_flag = 'Y'
 ...
 and a15.lagging_count_of_month between 0 and 24)
 UNION 
 select	'Duplicate Incidents',
count(a11.row_key)  AS Duplicate_Incidents
from	ldb.f_incident	a11
join ldb.d_calendar_date	a12 on 	(a11.opened_on_key = a12.row_key)
join ldb.d_incident	a13  on 	(a11.incident_key = a13.row_key)
join ldb.d_calendar_month a14 on 	(a12.month_start_date_key = a14.row_key)
where (a13.duplicate_incident_flag = 'Y' and a14.lagging_count_of_month between 0 and 24)
UNION 
select	'Escalated Incident Backlog',count(a24.row_key) AS Escalated_Incident_backlog
from	ldb.f_incident	a24
join	ldb.d_calendar_date	a12 on 	(a24.opened_on_key = a12.row_key)
join	ldb.d_incident	a13 on 	(a24.incident_key = a13.row_key)
join	ldb.d_incident_state	a14 on 	(a13.state_src_key = a14.row_key)
join	ldb.d_calendar_month	a15 on 	(a12.month_start_date_key = a15.row_key)
...
where	(a12.backlog_flag = 'Y'
 and a14.lagging_count_of_month between 0 and 24)
 UNION 
 select	'Incident Backlog @CI',
 count(a11.row_key)  AS Incident_Backlog_CI
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	...
	join	ldb.d_calendar_month	a15
	  on 	(a12.month_start_date_key = a15.row_key)
where	(a14.wh_dimension_code = 'OPEN'
 and a15.lagging_count_of_month between 0 and 24)
 UNION 
...
where	(a14.lagging_count_of_month between 0 and 24)
 ...
 UNION 
 select	'Incident Reassignment Count',
COALESCE(sum(a11.reassignment_count), 0) AS Incident_Reassignment_Count
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
where	a13.lagging_count_of_month between 0 and 24
UNION 
select	'Incident Reopened Count',
SUM(a11.reopened_count) as CNT
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
where	(a12.knowledge_flag='Y'
 and a14.lagging_count_of_month between 0 and 24)
UNION 
select	'Incidents Closed As Duplicate', 
count(a11.row_key)*1.0 AS Incidents_Closed_As_Duplicate
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
where	(a13.duplicate_incident_flag = 'Y'
 and a14.lagging_count_of_month between 0 and 24)
 UNION 
select	'Incidents Closed with No Manual Intervention',
count(a11.row_key)*1.0 AS Incidents_Closed_with_No_Manual_Intervention
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
where	(a13.closed_without_intervention_flag = 'Y'
 and a14.lagging_count_of_month between 0 and 24)
UNION 
select	'Incidents Closed Without Action',
(count((CASE WHEN a13.closed_without_intervention_flag = 'Y' THEN a11.row_key ELSE NULL END))+
	count((CASE WHEN a13.duplicate_incident_flag = 'Y' THEN a11.row_key ELSE NULL END)))*1.0 AS Incidents_Closed_Without_Action
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
a.resolved_incidents/b.opened_incidents*100.0 as fix_rate
FROM 
...
count(distinct a24.row_key) AS Major_Incident
from	ldb.f_incident	a24
	join	ldb.d_calendar_date	a12
	  on 	(a24.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
FROM 
(select	sum((a24.open_to_resolve_duration / 86400.0)) AS open_to_resolve_duration
from	ldb.f_incident_resolved	a24
	join	ldb.d_calendar_date	a12
	  on 	(a24.last_resolved_on_key = a12.row_key)
	...
	join	ldb.d_incident	a14
	  on 	(a24.incident_key = a14.row_key)
	join	ldb.d_incident_state	a15
	  on 	(a14.state_src_key = a15.row_key)
where	a15.wh_dimension_code in ('RESOLVED', 'CLOSED')  
and a12.lagging_count_of_month between 0 and 24) a 
CROSS JOIN 
(select	count(a24.row_key) AS resolved_incidents
from	ldb.f_incident_resolved	a24
	join	ldb.d_calendar_date	a12
	  on 	(a24.last_resolved_on_key = a12.row_key)
	join	ldb.d_calendar_rolling_three_months	a13
...
from	ldb.f_incident	a11
	...
	join	ldb.d_calendar_month	a14
	  on 	(a12.month_start_date_key = a14.row_key)
where (a13.reopened_flag = 'Y' and a13.met_sla_flag='Y'
and a14.lagging_count_of_month between 0 and 24)
UNION 
...
from	ldb.f_incident	a11
	...
	join	ldb.d_calendar_month	a14
	  on 	(a12.month_start_date_key = a14.row_key)
where	(a13.reopened_flag = 'Y' and a13.met_sla_flag = 'N'
 and a14.lagging_count_of_month between 0 and 24)
UNION 
...
JOIN ldb.d_calendar_month d ON c.month_start_date_key=d.row_key 
WHERE (b.met_sla_flag='N' and d.lagging_count_of_month between 0 and 24) 
UNION 
SELECT 
'Business Duration (Days)',
SUM(a.business_stc_c)/86400.0 as SUm
FROM ldb.f_incident a
JOIN ldb.d_calendar_date b ON a.opened_on_key=b.row_key
