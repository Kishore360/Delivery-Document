...
and coalesce(concat('~',SRC.u_programmer),'UNSPECIFIED' )= LKP.row_id )
JOIN fidelity_mdwdb.f_change_request trgt1
on SRC.sys_id = trgt1.row_id and SRC.sourceinstance = trgt1.source_id
WHERE coalesce(LKP.row_key,case when SRC.u_programmer is null then 0 else -1 end) <>trgt1.programmer_c_key
