...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_time_card.time_sheet_c_key' ELSE 'SUCCESS' END as Message 
...
LEFT JOIN fidelity_mdwdb.d_time_sheet_c LKP
on COALESCE(floor(SRC.TIME_SHEET_ID),'UNSPECIFIED') = LKP.row_id and LKP.source_id = SRC.sourceinstance
WHERE coalesce(LKP.row_key,case when SRC.TIME_SHEET_ID is null then 0 else -1 end ) <> TRGT.time_sheet_c_key; 