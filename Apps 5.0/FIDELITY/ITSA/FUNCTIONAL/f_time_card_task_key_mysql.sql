...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_time_card.task_key' ELSE 'SUCCESS' END as Message 
...
LEFT JOIN fidelity_mdwdb.d_task LKP
on COALESCE(floor(SRC.WORK_ITEM_ID),'UNSPECIFIED') = LKP.row_id and LKP.source_id = SRC.sourceinstance
WHERE coalesce(LKP.row_key,case when SRC.WORK_ITEM_ID is null then 0 else -1 end ) <> TRGT.task_key; 