...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_time_card.company_key' ELSE 'SUCCESS' END as Message 
...
on  COALESCE(CONCAT(LKP_SRC.company,'~~~','CRT - Company Backup'),'UNSPECIFIED') = LKP.row_id and LKP.source_id = LKP_SRC.sourceinstance
WHERE coalesce(LKP.row_key,case when LKP_SRC.company is null then 0 else -1 end ) <> TRGT.company_key
