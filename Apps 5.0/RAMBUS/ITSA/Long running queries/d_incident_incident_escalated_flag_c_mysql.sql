 ...
 FROM rambus_mdwdb.d_incident di 
			LEFT JOIN (SELECT distinct task_key,dic.group_level_c,dio_old.level_type_c dio_old_level_type_c,
dio_new.level_type_c dio_new_level_type_c
		FROM (select * from rambus_mdwdb.f_t_task_activity where task_attribute_wh_name = 'assignment_group' and task_wh_type = 'incident') src_ft
