...
FROM (SELECT (CASE WHEN (DAYOFWEEK(CONVERT_TZ(opened_at,'GMT','America/Los_Angeles'))=6 AND TIME(CONVERT_TZ(opened_at,'GMT','America/Los_Angeles'))>='16:30') THEN 'N'
WHEN (DAYOFWEEK(CONVERT_TZ(opened_at,'GMT','America/Los_Angeles')) = 7 ) THEN 'N' 
WHEN (DAYOFWEEK(CONVERT_TZ(opened_at,'GMT','America/Los_Angeles')) = 1 AND TIME(CONVERT_TZ(opened_at,'GMT','America/Los_Angeles')) <= '15:30' ) THEN 'N'ELSE 'Y' END) , business_day_flag_c
