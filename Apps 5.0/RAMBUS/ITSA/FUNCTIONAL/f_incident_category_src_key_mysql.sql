...
LEFT OUTER JOIN rambus_mdwdb.d_lov c ON c.row_id= COALESCE(CONCAT('CATEGORY','~','INCIDENT','~','~','~',upper(b.u_application)),'UNSPECIFIED')
 AND c.source_id =  b.sourceinstance AND DATE_FORMAT(a.pivot_date, '%Y-%m-%d %H:%i:%s')
