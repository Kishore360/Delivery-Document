-- SUCCESSFUL,UNSUCCESSFUL,- formula correct data not matching
...
select	'Actual Duration (Hours)' as metrics,sum((a11.actual_duration / 3600.0)) AS value
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
join   ldb.d_calendar_date  A14
         ...
         join ldb.d_calendar_month a15
         on   (a14.month_start_date_key = a15.row_key)
where  a15.lagging_count_of_month between 0 and 11 group by 1)a where abc>0 
UNION
...
UNION
select 'Deviation Duration (hours)'as metrics,planned_duration-actual_duration as value from ( 
select	sum((a11.planned_duration / 3600.0)) AS planned_duration,
	sum((a11.actual_duration / 3600.0)) AS actual_duration
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
where  a15.lagging_count_of_month between 0 and 11
UNION
select  'Outage Duration(Days)' as metrics, (sum(outage_duration_c/60) ) as value
from ldb.f_change_request a
join   ldb.d_calendar_date  A14
         on   (a. opened_on_key = a14.row_key)
         join ldb.d_calendar_month a15
...
union
SELECT 
'percent of RFC pending_implementation' as metrics,(a.pending_impl/b.cnt)*100.0 as value
FROM 
(
...
where	a13.lagging_count_of_month between 0 and 11
	UNION
		select	'Planned Duration(Hours)'as metrics,sum((a11.planned_duration / 3600.0)) AS value 
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
join   ldb.d_calendar_date  A14
		...
		UNION
		select	'RFC_Age' as metrics,sum(a11.change_request_age) AS value
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
