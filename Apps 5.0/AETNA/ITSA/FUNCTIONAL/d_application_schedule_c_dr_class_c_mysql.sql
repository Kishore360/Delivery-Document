...
CASE WHEN count(1)>1 then 'Data mismatch for d_application_schedule_c.dr_class_c'  ELSE 'SUCCESS'  END as Message
...
where a11.DRClass<>a12.dr_class_c
union
 select case when count(1)>1 then 'FAILURE'  else 'SUCCESS'  END as result ,
CASE WHEN count(1)>1 then 'Data mismatch for d_application_schedule_c.dr_class_c'  ELSE 'SUCCESS'  END as Message
from  aetna_mdsdb.us_application_schedule_c_final a11
...
-- concat(a11.Application_SYS_ID,'~',a11.DRClass,'~',a11.Day,'~',a11.Start,'~',a11.End,'~','1')=a12.row_id