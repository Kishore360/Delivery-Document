...
CASE WHEN count(1)>1 then 'Data mismatch for d_application_schedule_c.portfolio_owner_c_key'  ELSE 'SUCCESS'  END as Message
...
on (a11.Portfolio_Owner_ID=a13.user_name and a11.sourceinstance=a13.source_id)
where coalesce(a13.row_key, case when a11.Portfolio_Owner is null then 0 else -1 end)<>a12.portfolio_owner_c_key
union
select case when count(1)>1 then 'FAILURE'  else 'SUCCESS'  END as result ,
CASE WHEN count(1)>1 then 'Data mismatch for d_application_schedule_c.portfolio_owner_c_key'  ELSE 'SUCCESS'  END as Message
from  aetna_mdsdb.us_application_schedule_c_final a11
...
join aetna_mdwdb.d_internal_contact a13
on 
(a11.Portfolio_Owner_ID=a13.user_name and
 a11.sourceinstance=a13.source_id)
where coalesce(a13.row_key, case when a11.Portfolio_Owner is null then 0 else -1 end)<>a12.portfolio_owner_c_key
union
select case when count(1)>1 then 'FAILURE'  else 'SUCCESS'  END as result ,
CASE WHEN count(1)>1 then 'Data mismatch for d_application_schedule_c.portfolio_owner_c_key'  ELSE 'SUCCESS'  END as Message
