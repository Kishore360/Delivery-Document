...
union
select 'ldb.d_intenal_contact_caller_c a113 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_incident a11 
join ldb.d_incident a110 
on (a11.incident_key = a110.row_key) 
join ldb.d_intenal_contact_caller_c a113 
on (a110.caller_key = a113.row_key) 
union
...
union
select 'ldb.d_location_configuration_item_c a129 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_incident a11 
join ldb.d_configuration_item a111 
on (a11.configuration_item_key = a111.row_key) 
join ldb.d_location_configuration_item_c a129 
on (a111.location_key = a129.row_key) 
union
...
on (a110.category_src_key = a130.row_key) 
union
...
on (a110.close_code_src_key = a131.row_key) 
union
...
on (a110.priority_src_key = a132.row_key) 
union
...
on (a110.met_resolution_sla_flag_key = a133.row_key) 
union
...
on (a110.met_response_sla_flag_key = a134.row_key) 
union
...
on (a110.severity_src_key = a135.row_key) 
union
...
on (a110.sub_category_src_key = a137.row_key) 
union
select 'ldb.d_location_caller_c a138 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_incident a11 
join ldb.d_incident a110 
on (a11.incident_key = a110.row_key) join ldb.d_intenal_contact_caller_c a113 
on (a110.caller_key = a113.row_key) 
join ldb.d_location_caller_c a138 
