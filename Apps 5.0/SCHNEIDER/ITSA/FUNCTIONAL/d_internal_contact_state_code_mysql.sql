...
WHERE (case when choice.value='Office-Based Employee' then COALESCE(SRC.Site_State_Province,'UNSPECIFIED') 
when choice.value='Home-Based Employee'  then COALESCE(SRC.home_State_Province,'UNSPECIFIED')  else COALESCE( SRC.default_State_Province, 'UNSPECIFIED') end ) <> (TRGT.state_code)
