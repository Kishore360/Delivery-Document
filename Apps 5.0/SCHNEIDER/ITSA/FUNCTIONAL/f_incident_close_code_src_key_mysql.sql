...
LEFT JOIN schneider_mdwdb.d_lov LKP 
 ON ( COALESCE(CONCAT('CLOSE_CODE~INCIDENT~~~',SRC.status_reason2) = LKP.row_id 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.status_reason IS NULL THEN 0 else -1 end)<> (TRGT.close_code_src_key)
