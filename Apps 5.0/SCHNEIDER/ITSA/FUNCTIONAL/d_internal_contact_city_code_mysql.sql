...
WHERE (case when choice.value='Office-Based Employee' then COALESCE(SRC.Site_City,'UNSPECIFIED') 
when choice.value='Home-Based Employee'  then COALESCE(SRC.Home_City,'UNSPECIFIED')  else COALESCE( SRC.Default_City, 'UNSPECIFIED') end ) <> (TRGT.city_code)
