...
join schneider_mdwdb.d_calendar_date lkp on  lkp.row_id=COALESCE(DATE_FORMAT(FROM_UNIXTIME(src.overallstarttime),'%Y%m%d'),'UNSPECIFIED')
where coalesce(lkp.row_key , case when src.overallstarttime is null then 0 else -1 end)<>stg.start_on_key;
