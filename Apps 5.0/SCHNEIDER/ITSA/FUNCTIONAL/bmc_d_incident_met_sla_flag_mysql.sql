...
JOIN schneider_mdwdb.d_incident di ON (SRC.incident_number = di.row_id AND SRC.sourceinstance = di.source_id AND SRC.sourceinstance = di.source_id )
 where (case 
                when SRC.slm_status is null then 'N' 
