 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_work_order.dormancy_age' ELSE 'SUCCESS' END as Message from (select count(1) cnt 
...
WHERE TIMESTAMPDIFF(SECOND,CONVERT_TZ(f.changed_on,'America/Los_Angeles','GMT'), CONVERT_TZ(df.lastupdated,'America/Los_Angeles','GMT')) <> f.dormancy_age) a;
