SELECT CASE WHEN count(1)  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for d_location.region_c' ELSE 'SUCCESS' END as Message 
...
where site_id  is not null group by site_id
 ...
 WHERE coalesce(AUX_SRC.region,'UNSPECIFIED')  <> (TRGT.region_c) 
