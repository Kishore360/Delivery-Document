...
FROM schneider_mdsdb.woi_workorder_final  SRC JOIN schneider_mdwdb.f_work_order TRGT ON (SRC.request_id = TRGT.row_id AND SRC.sourceinstance = TRGT.source_id ) inner join (select documentkey,sourceinstance,max(sys_updated_by) as closed_by, max(sys_created_on) as closed_on
