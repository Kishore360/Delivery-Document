...
WHERE (case when choice.value='Office-Based Employee' then COALESCE(SRC.Site_Zip_Postal_Code,'UNSPECIFIED') 
when choice.value='Home-Based Employee'  then COALESCE(SRC.home_Zip_Postal_Code,'UNSPECIFIED')  
else COALESCE( SRC.default_zip_postal_code, 'UNSPECIFIED') end ) <> (TRGT.postal_code)
