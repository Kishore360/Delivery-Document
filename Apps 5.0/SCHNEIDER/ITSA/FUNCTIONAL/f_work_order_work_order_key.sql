...
jOIN schneider_mdwdb.d_work_order LKP ON (SRC.request_id = LKP.row_id  AND SRC.sourceinstance = LKP.source_id ) 
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.request_id IS NULL THEN 0 else -1 end)  <> (TRGT.work_order_key) 
