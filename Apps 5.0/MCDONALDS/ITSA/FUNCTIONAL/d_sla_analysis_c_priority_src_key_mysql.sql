...
(select a1.priority , trgt.priority_src_code,a1.sys_id,a1.s2,a1.t,a1.s,a1.r from 
(select CONCAT(src.sys_id,'~',c.sla,'~',c.sys_id) xyz, src1.priority priority,src1.sys_id sys_id,src.sys_id s2,c.task t,c.sla s,lov.row_key r
...
on src.sys_id=src1.sys_id and src.sourceinstance=src1.sourceinstance
 join (select sys_id, sla,task,sourceinstance  from   mcdonalds_mdsdb.task_sla_final where CDCTYPE<>'D') c on src.sys_id=c.task 
 ...
 left join  mcdonalds_mdwdb.d_lov lov  on CONCAT('PRIORITY~INCIDENT~~~',UPPER(src1.priority))=lov.row_id
 
 
 )a1
 join   mcdonalds_mdwdb.d_sla_analysis_c trgt on  a1.xyz =trgt.row_id
  where trgt.soft_deleted_flag='N'  and  a1.r <> trgt.priority_src_key)a) temp;
