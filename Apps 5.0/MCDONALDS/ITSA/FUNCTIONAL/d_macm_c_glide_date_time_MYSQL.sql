...
CASE WHEN CNT THEN 'MDS to DWH data validation failed for d_macm_c_glide_date_time' ELSE 'SUCCESS' END as Message
FROM
(
SELECT count(1) as CNT from from
...
where (SRC.cdctime<=f1.lastupdated) and COALESCE(CONVERT_TZ(SRC.u_glide_date_time_1,'GMT','US/Central'),'UNSPECIFIED')<>TRGT.glide_date_time
)a;