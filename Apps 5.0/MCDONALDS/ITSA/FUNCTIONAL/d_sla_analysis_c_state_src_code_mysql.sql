...
(select a1.state , trgt.state_src_code,a1.sys_id,a1.s2,a1.t,a1.s from 
(select CONCAT(src.sys_id,'~',c.sla,'~',c.sys_id) xyz, src1.state state,src1.sys_id sys_id,src.sys_id s2,c.task t,c.sla s
from (select sys_id, sourceinstance,state from   mcdonalds_mdsdb.incident_final where CDCTYPE<>'D') src1 
...
on src.sys_id=src1.sys_id and src.sourceinstance=src1.sourceinstance
 ...
 join   mcdonalds_mdwdb.d_sla_analysis_c trgt on  a1.xyz =trgt.row_id
  where trgt.soft_deleted_flag='N'  and  a1.state <> trgt.state_src_code)a) temp;
