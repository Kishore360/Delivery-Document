...
from 
(select count(1) as cnt  from (select number,sys_id, sourceinstance,cdctime from  mcdonalds_mdsdb.task_final where sys_class_name='incident') src 
 ...
 join (select sys_id, u_interaction_type,sourceinstance from mcdonalds_mdsdb.contract_sla_final) d on c.sla=d.sys_id and src.sourceinstance=d.sourceinstance
...
and  src.number <> trgt.task_number) temp;