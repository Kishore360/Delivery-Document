...
(
select CONCAT(a.sys_id, '~',c.sla,'~',c.sys_id) sys_id,a.sourceinstance sourceinstance,number from 
(select sys_id,sys_class_name,sourceinstance,number  from mcdonalds_mdsdb.task_final 
...
join (select source_id,row_id,task_number from mcdonalds_mdwdb.d_sla_analysis_c where soft_deleted_flag='N' )e 
on a1.sys_id=e.row_id and a1.sourceinstance=e.source_id   
and  a1.number <> e.task_number
