...
ON COALESCE(CONCAT('COMPONENT~MACM','~~~',UPPER(SRC.u_component)),'UNSPECIFIED')=LKP.row_id
where (SRC.cdctime<=f1.lastupdated) and COALESCE(LKP.row_key, CASE WHEN SRC.u_component IS NULL THEN 0 ELSE -1 END) <> TRGT.component_src_key
)a;
