...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_task_tag_c.tag_name' ELSE 'SUCCESS' END as Message
...
where (src.cdctime<=f1.lastupdated)  and trgt.soft_deleted_flag='N' and lf.name <> trgt.tag_name) temp;