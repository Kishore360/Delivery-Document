...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_sla_analysis_c.start_time' ELSE 'SUCCESS' END as Message
from 
(select count(1) as cnt  from (select sys_id, sourceinstance,cdctime from  mcdonalds_mdsdb.task_final where sys_class_name='incident' ) src 
 ...
 join (select sys_id, sla,task,start_time,sourceinstance  from mcdonalds_mdsdb.task_sla_final) c on src.sys_id=c.task and src.sourceinstance=c.sourceinstance
 join (select sys_id,sourceinstance from mcdonalds_mdsdb.contract_sla_final) d on c.sla=d.sys_id and c.sourceinstance=d.sourceinstance
...
where (src.cdctime<=f1.lastupdated) and  CONVERT_TZ(c.start_time,'GMT','US/Central') <> trgt.start_time) temp;