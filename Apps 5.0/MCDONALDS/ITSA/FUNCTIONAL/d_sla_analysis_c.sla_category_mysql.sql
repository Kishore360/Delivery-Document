...
left join (select sys_id, type from mcdonalds_mdsdb.contract_sla_final) d on c.sla=d.sys_id 
...
where (src.cdctime<=f1.lastupdated) and  COALESCE(d.type ,'UNSPECIFIED') <> trgt.sla_category) temp;