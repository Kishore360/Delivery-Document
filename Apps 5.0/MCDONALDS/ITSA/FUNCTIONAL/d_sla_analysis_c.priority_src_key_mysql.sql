...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_sla_analysis_c.priority_src_key' ELSE 'SUCCESS' END as Message
from 
...
where (src.cdctime<=f1.lastupdated) and  COALESCE(CONCAT('PRIORITY~INCIDENT~~~',UPPER(src1.priority)),'UNSPECIFIED') <> trgt.priority_src_key) temp;