...
CASE WHEN count(1)<1  THEN  'SUCCESS' ELSE  'Data mismatch'  END as Message 
FROM (    
  select a.week_start_key,b.prev_week_start_key,inflow,prev_inflow from (
select week_start_key,sum(inflow_count) inflow
FROM     mcdonalds_mdwdb.f_n_request_item_hie_io_snapshot_weekly_c
Join mcdonalds_mdwdb.d_internal_contact a12 ON businessunit_custom_c_key= a12.row_key
