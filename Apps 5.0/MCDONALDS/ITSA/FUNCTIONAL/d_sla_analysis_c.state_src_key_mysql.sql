...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_sla_analysis_c.state_src_key' ELSE 'SUCCESS' END as Message
from 
...
where (src.cdctime<=f1.lastupdated) and  COALESCE(CONCAT('STATE~INCIDENT~~~',UPPER(src1.state)),'UNSPECIFIED') <> trgt.state_src_key) temp;