...
FROM mcdonalds_mdwdb.d_o_data_freshness where (SRC.cdctime<=f1.lastupdated) and sourcename like 'ServiceNow%' and etl_run_number=TRGT.etl_run_number),'US/Central','GMT')),0) and  (SRC.cdctime<=lastupdated) <> TRGT.dormancy_age )b
