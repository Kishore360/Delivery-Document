...
on src.sys_id=src1.sys_id and src.sourceinstance=src1.sourceinstance
 ...
 left join  mcdonalds_mdwdb.d_lov lov  on CONCAT('STAGE~TASK_SLA~~~',UPPER(c.stage ))=lov.row_id
 
 
 )a1
 join   mcdonalds_mdwdb.d_sla_analysis_c trgt on  a1.xyz =trgt.row_id
  where trgt.soft_deleted_flag='N'  and  a1.r <> trgt.stage_src_key)a) temp;
