...
ON (LKP.row_id=DATE_FORMAT(convert_tz(ext.expirydate,'GMT','US/Central'),'%Y%m%d')and LKP.source_id=0)
where trgt.expiry_date_src_c_key <> OALESCE (LKP.row_key,case when ext.expirydate is null then 0 else -1 end)
)a
;
