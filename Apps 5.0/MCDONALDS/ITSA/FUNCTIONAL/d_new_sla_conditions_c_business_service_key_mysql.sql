SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed' ELSE 'SUCCESS' END as Message
			FROM
                mcdonalds_mdsdb.us_new_sla_map_final us_sla                  
            LEFT OUTER JOIN
 ...
 LEFT OUTER JOIN mcdonalds_mdwdb.d_service l on    COALESCE(CONCAT('BUSINESS_SERVICE~',serv.sys_id),'UNSPECIFIED')=l.row_id
            ...
            'UNSPECIFIED')),
            255) =d1.row_id
			where coalesce(l.row_key,case when sys_user_group.sys_id is null then 0 else -1 end )<>d1.business_service_key
