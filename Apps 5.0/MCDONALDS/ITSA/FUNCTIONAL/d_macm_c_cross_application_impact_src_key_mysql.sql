...
COALESCE(CONCAT('CROSS_APPLICATION_IMPACT', '~', 'MACM', '~', '~', '~',Upper(macm.u_cross_application_impact)),'UNSPECIFIED')=LKP.row_id
WHERE COALESCE(LKP.row_key, CASE WHEN SRC.priority IS NULL THEN 0 ELSE -1 END) <> TRGT.cross_application_impact_src_key
)a;
