...
(
select CONCAT(a.sys_id, '~',c.sla,'~',c.sys_id) sys_id,a.sourceinstance sourceinstance,is_active_flag from 
(select sys_id,sys_class_name,sourceinstance  from mcdonalds_mdsdb.task_final 
 where  sys_class_name='INCIDENT' and CDCTYPE<>'D'  )a
   JOIN  (select task,sourceinstance,stage,COALESCE(sla,'UNSPECIFIED')sla,cdctime,COALESCE(sys_id,'UNSPECIFIED') sys_id, case when active=1 then 'Y' else 'N' end is_active_flag from 
...
join (select source_id,row_id,is_active_flag from mcdonalds_mdwdb.d_sla_analysis_c where soft_deleted_flag='N' )e 
on a1.sys_id=e.row_id and a1.sourceinstance=e.source_id   
and  a1.is_active_flag <> e.is_active_flag
