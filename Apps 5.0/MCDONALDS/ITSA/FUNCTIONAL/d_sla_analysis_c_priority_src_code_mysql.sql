...
(select CONCAT(src.sys_id,'~',c.sla,'~',c.sys_id) xyz, src1.priority priority,src1.sys_id sys_id,src.sys_id s2,c.task t,c.sla s
...
on src.sys_id=src1.sys_id and src.sourceinstance=src1.sourceinstance
 ...
 join   mcdonalds_mdwdb.d_sla_analysis_c trgt on  a1.xyz =trgt.row_id
  where trgt.soft_deleted_flag='N'  and  a1.priority <> trgt.priority_src_code)a) temp;
