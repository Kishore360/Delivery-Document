select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident_task_c_task.location' ELSE 'SUCCESS' END as Message
from mcdonalds_mdsdb.incident_task_final SRC
left join mcdonalds_mdwdb.f_incident_task_c TRGT 
 ON SRC.sys_id =TRGT.row_id 
 AND SRC.sourceinstance = TRGT.source_id  
 left join mcdonalds_mdwdb.d_location d
 on d.row_id = SRC.location
 where coalesce(d.row_key,CASE WHEN SRC.location is NULL then 0 else -1 end)<> TRGT.location_key;
 COALESCE(lkp.row_key,CASE WHEN s.opened_by IS NULL THEN 0 else -1 end) <> t.opened_by_key)