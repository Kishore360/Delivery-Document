...
CASE WHEN CNT >0  THEN 'MDS to DWH data validation failed for d_change_failure_c.rescheduled_flag' ELSE 'SUCCESS' END as Message 
FROM 
(
...
WHERE (CASE WHEN SRC1.cnt > 0 THEN 'Y' ELSE 'N' END)<>TRGT.rescheduled_flag
AND SRC.CDCTYPE<>'D'
) temp;
