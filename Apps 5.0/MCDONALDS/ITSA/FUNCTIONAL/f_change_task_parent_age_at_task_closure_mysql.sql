...
THEN TIMESTAMPDIFF(SECOND, CONVERT_TZ(par.opened_on,'GMT', 'America/New_York'),
CONVERT_TZ(d.closed_on,'GMT', 'America/New_York')) else 0 END
and ( TRGT.Parent_Age_At_Task_Closure IS NULL OR TRGT.Parent_Age_At_Task_Closure <> 
CASE WHEN d.closed_on > par.opened_on THEN TIMESTAMPDIFF(SECOND, CONVERT_TZ(par.opened_on,'GMT', 'America/New_York'),
CONVERT_TZ(d.closed_on,'GMT', 'America/New_York')) else 0 END);
