...
(
select CONCAT(a.sys_id, '~',c.sla,'~',c.sys_id) sys_id,a.sourceinstance sourceinstance,actual_duration from 
...
CASE WHEN duration is null  THEN NULL   
     WHEN TIMESTAMPDIFF(SECOND,'1970-01-01 00:00:00',duration) < 0 THEN NULL    
     WHEN TIMESTAMPDIFF(SECOND,'1970-01-01 00:00:00',duration) > 214748364 THEN NULL     
     ELSE TIMESTAMPDIFF(SECOND,'1970-01-01 00:00:00',duration) END actual_duration   from 
...
join (select source_id,row_id,actual_duration from mcdonalds_mdwdb.f_sla_analysis_c where soft_deleted_flag='N' )e 
on a1.sys_id=e.row_id and a1.sourceinstance=e.source_id   
and  a1.actual_duration <> e.actual_duration
