...
(select d.sys_name n,CONCAT(src.sys_id,'~',c.sla,'~',c.sys_id) xyz, src1.priority priority,src1.sys_id sys_id,src.sys_id s2,c.task t,c.sla s
...
join (select sys_id,sourceinstance,sys_name from   mcdonalds_mdsdb.contract_sla_final where CDCTYPE<>'D') d on c.sla=d.sys_id  and d.sourceinstance=src1.sourceinstance
 ...
 join   mcdonalds_mdwdb.d_sla_analysis_c trgt on  a1.xyz =trgt.row_id
  where trgt.soft_deleted_flag='N'  and  a1.n <> trgt.sla_name)a) temp;
