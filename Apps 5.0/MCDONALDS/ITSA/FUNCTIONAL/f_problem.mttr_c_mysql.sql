...
left join (SELECT documentkey,sourceinstance,convert_tz(max(sys_created_on),'GMT','US/Central')
  as completed_on from mcdonalds_mdsdb.sys_audit_final WHERE tablename = 'problem' and fieldname = 'u_rca_status' and newvalue = 'Complete'
group by 1,2) lkp on src.sys_id = lkp.documentkey and src.sourceinstance = lkp.sourceinstance
left join mcdonalds_mdwdb.f_problem trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
