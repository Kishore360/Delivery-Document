...
FROM (SELECT count(1) as CNT from
 (select * from mcdonalds_mdsdb.sc_req_item_final where cdctype<>'D') SRC 
 LEFT JOIN mcdonalds_mdwdb.f_request_item TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
 LEFT JOIN mcdonalds_mdwdb.d_internal_contact LKP 
 ON ( concat('INTERNAL_CONTACT~',SRC.opened_by)= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id
and COALESCE(CONVERT_TZ (SRC.opened_at,'GMT','US/Central'), CONVERT_TZ (coalesce(SRC.closed_at,SRC.sys_updated_on),'GMT','US/Central'),'UNSPECIFIED') 
