...
(
select CONCAT(a.sys_id, '~',c.sla,'~',c.sys_id) sys_id,a.sourceinstance sourceinstance,d.u_interaction_type u_interaction_type from 
(select sys_id,sys_class_name,sourceinstance  from mcdonalds_mdsdb.task_final 
 where  sys_class_name='INCIDENT' and CDCTYPE<>'D'  )a
  ...
  ON a.sys_id = c.task AND a.sourceinstance = c.sourceinstance 
   join (select COALESCE(u_interaction_type,'UNSPECIFIED') u_interaction_type,sys_id,sourceinstance from mcdonalds_mdsdb.contract_sla_final )d  on c.sla=d.sys_id and a.sourceinstance=d.sourceinstance
...
join (select source_id,row_id,sla_type from mcdonalds_mdwdb.d_sla_analysis_c where soft_deleted_flag='N' )e 
on a1.sys_id=e.row_id and a1.sourceinstance=e.source_id   
and  a1.u_interaction_type <> e.sla_type
