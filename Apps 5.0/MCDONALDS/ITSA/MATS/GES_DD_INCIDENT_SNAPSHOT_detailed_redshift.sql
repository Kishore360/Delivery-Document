select 'INCIDENT_IO_monthLY' as Table_Name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
...
count(1) FROM mcdonalds_mdwdb.f_n_incident_io_snapshot_monthly_c
...
union all
select 'INCIDENT_AGM_monthLY' as Table_Name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
(select count(1)  a from (
...
union all
select 'INCIDENT_VDR_monthLY' as Table_Name,(a1.cnt*1.0/b.cnt*1.0)*100.0 failure_probability from
...
-- assignment_group_key,business_service_key,priority_src_key,,age_key,,sum(distinct_inflow_count),sum(distinct_outflow_count)
 ...
 -- select distinct sum(beginning_backlog) from mcdonalds_mdwdb.f_n_incident_vdr_io_snapshot_monthly_c
...
from mcdonalds_mdwdb.f_n_incident_vdr_io_snapshot_monthly_c
 ...
 union all 
 select 'INCIDENT_IO_WEEKLY' as table_name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
(select count(1)  a from
...
(sum(beginning_backlog)+sum(inflow_count)) as validate,
(sum(outflow_count)+sum(resolved)) as validate2,
count(1) FROM mcdonalds_mdwdb.f_n_incident_io_snapshot_weekly_c
...
select 'INCIDENT_AGM_WEEKLY' as table_name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
(select count(1)  a from (
...
union all
select 'INCIDENT_VDR_weekLY' as Table_Name,(a1.cnt*1.0/b.cnt*1.0)*100.0 failure_probability from
(select count(1) cnt  from 
...
-- assignment_group_key,business_service_key,priority_src_key,,age_key,,sum(distinct_inflow_count),sum(distinct_outflow_count)
 ...
 -- select distinct sum(beginning_backlog) from mcdonalds_mdwdb.f_n_incident_vdr_io_snapshot_weekly_c
...
from mcdonalds_mdwdb.f_n_incident_vdr_io_snapshot_weekly_c
 ...
 union all
 select 'INCIDENT_BU_MONTHLY' as table_name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
(select count(1)  a from
...
(sum(beginning_backlog)+sum(inflow_count)) as validate,
(sum(outflow_count)+sum(resolved)) as validate2,
count(1) FROM mcdonalds_mdwdb.f_n_incident_hie_io_snapshot_monthly_c
-- where-- month_start_key>='20180101' and month_start_key<='20180101' and 
-- month_start_key<'20180601' and 
...
union all
 select 'INCIDENT_BU_weekLY' as table_name,(a.a*1.0/b.b*1.0)*100.0 as failure_probability from 
(select count(1)  a from
...
(sum(beginning_backlog)+sum(inflow_count)) as validate,
(sum(outflow_count)+sum(resolved)) as validate2,
count(1) FROM mcdonalds_mdwdb.f_n_incident_hie_io_snapshot_weekly_c
-- where-- week_start_key>='20180101' and week_start_key<='20180101' and 
-- week_start_key<'20180601' and 
-- businessunit_custom_c_key>0
