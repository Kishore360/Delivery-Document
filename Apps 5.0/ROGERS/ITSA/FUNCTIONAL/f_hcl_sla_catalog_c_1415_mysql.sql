...
from   rogers_mdsdb.change_request_final src
left join  rogers_mdwdb.d_change_request trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id 
left join  rogers_mdwdb.f_change_request tgt on src.sys_id =tgt.row_id and src.sourceinstance=tgt.source_id
left join rogers_mdwdb.d_internal_organization LKP on tgt.change_initiator_workgroup_c_key=LKP.row_key
left join rogers_mdwdb.d_lov lov on concat('STATE~CHANGE_REQUEST~~~', src.state)=lov.row_id
