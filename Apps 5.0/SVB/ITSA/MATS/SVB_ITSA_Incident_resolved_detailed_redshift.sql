...
on (a11.last_resolved_on_key=a17.row_key)
union
...
on (a14.category_src_key=a131.row_key)
union
...
on (a14.close_code_src_key=a132.row_key)
union
...
on (a14.priority_src_key=a133.row_key)
union
...
on (a14.severity_src_key=a134.row_key)
union
...
on (a14.state_src_key=a135.row_key)
union
select'ldb.d_internal_contact_manager_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident_resolved       a11 
 join ldb.d_internal_organization_group       a15
on (a11.assignment_group_key=a15.row_key)
join ldb.d_internal_contact_manager_c       a130
on (a15.manager_c_key=a130.row_key)
union
select'ldb.d_lov_incident_type_incident_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident_resolved       a11 
join ldb.d_lov_incident_type_incident_c       a136
on (a11.incident_type_src_c_key=a136.row_key)
union
