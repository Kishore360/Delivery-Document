...
on d.row_id = (Case when lvl.variable_type='Reference' AND q.reference='sys_user' and lvl.variable_name='user_requested_date' then COALESCE(DATE_FORMAT(CONVERT_TZ(qa.value,'GMT','America/Los_Angeles') ,'%Y%m%d'),'UNSPECIFIED') else 'UNSPECIFIED' end )
							join svb_mdwdb.f_request_item_variable_c f on concat(qa.sys_id,'~',b.request_item)=f.row_id
where coalesce(d.row_key,case when qa.value is null then 0 else -1 end )<>user_requested_on_c_key
