...
join svb_mdwdb.d_calendar_date d
on Case when lvl.variable_type='Reference' AND q.reference='sys_user' and lvl.variable_name='user_requested_date' then COALESCE(DATE_FORMAT(CONVERT_TZ(qa.value,'GMT','America/Los_Angeles') ,'%Y%m%d'),'UNSPECIFIED') else 'UNSPECIFIED' end = d.row_id
where COALESCE(d.row_key,CASE WHEN qa.value IS NULL THEN 0 else -1 end)<>f.user_requested_on_c_key
