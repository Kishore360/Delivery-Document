...
on Case when lvl.variable_type='Reference' AND q.reference='sys_user' and lvl.variable_name='requested_for' then COALESCE(concat('INTERNAL_CONTACT~',qa.value),'UNSPECIFIED') else 'UNSPECIFIED' end = d.row_id
where COALESCE(d.row_key,CASE WHEN qa.value IS NULL THEN 0 else -1 end)<>f.requested_for_c_key
 )c
