...
where (case when a.opened_on> coalesce(a.closed_on,a.changed_on)then null 
      else  TIMESTAMPDIFF(SECOND,CONVERT_TZ(a.opened_on,'America/Los_Angeles','GMT'),
      CONVERT_TZ(coalesce(a.closed_on,a.changed_on),'America/Los_Angeles','GMT')) end)<>       f.open_to_close_duration
)temp
