...
(select count(1) cnt  
 ...
 ON ( concat('U_COMPLEXITY','~','PROBLEM','~','~','~',u_complexity)= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_complexity IS NULL THEN 0 else -1 end)<> (TRGT.complexity_src_c_key) )b
