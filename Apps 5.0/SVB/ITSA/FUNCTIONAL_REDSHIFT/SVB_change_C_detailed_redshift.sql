...
UNION
select	'Age (Days)',sum((a11.change_request_age / 86400.0))/count(distinct a11.change_request_key)
	 AS WJXBFS2
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
...
where	a13.lagging_count_of_month between 0 and 12
union
select	'Avg Days Past Planned End Date',sum((a11.time_past_planned_end_date / 86400.0))/count(distinct a11.change_request_key) 
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
where	a13.lagging_count_of_month between 0 and 12
union
select	'Change Backlog',count(distinct a11.change_request_key) AS WJXBFS1
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
...
union
select	'Changes Pending Implementation',count(distinct a11.change_request_key) AS WJXBFS1
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_change_request_approval	a13
	  on 	(a11.approval_state_src_key = a13.row_key)
	join	ldb.d_change_request	a14
	  on 	(a11.change_request_key = a14.row_key)
	join	ldb.d_calendar_month	a15
	  on 	(a12.month_start_date_key = a15.row_key)
where	(a14.backlog_flag = 'Y'
 and a13.wh_dimension_code = 'APPROVED'
...
a.x/b.Y*100.0 as percent_pending_impl_changes
FROM 
(
select	count(distinct a11.change_request_key)*1.0 AS x
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_change_request_approval	a13
...
CROSS JOIN 
( 
select	count(distinct a11.change_request_key)*1.0 AS Y
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_change_request	a13
...
where	(a13.failure_flag= 'N'
 and a14.lagging_count_of_month between 0 and 12)
 union
 select	'Unplanned Change Backlog',count(distinct a11.change_request_key) AS WJXBFS1
from	ldb.f_change_request	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_change_request	a13
