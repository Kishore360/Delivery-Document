...
WHERE (SRC.cdctime<=f1.lastupdated) and coalesce(LKP.row_key,case when SRC.estimated_delivery is null then 0 else -1 end) <> (TRGT.estimated_delivery_time_key))temp;
