...
FROM (SELECT count(1) as CNT
 ...
 LEFT JOIN <<tenant>>_mdwdb.d_request_item LKP 
 ON ( SRC.request_item= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id )
left join (select source_id,max(lastupdated) as lastupdated from <<tenant>>_mdwdb.d_o_data_freshness group by source_id) f1 on (f1.source_id = SRC.sourceinstance)
 where (src.cdctime<=f1.lastupdated) and COALESCE(LKP.row_key,CASE WHEN SRC.request_item IS NULL THEN 0 else -1 end)<> (TRGT.request_item_key))temp;
