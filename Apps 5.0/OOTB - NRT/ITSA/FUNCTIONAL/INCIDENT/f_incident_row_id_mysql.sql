...
SELECT CASE WHEN cnt THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN cnt THEN 'MDS to DWH data validation failed for f_incident.row_id' ELSE 'SUCCESS' END as Message from (select count(1) cnt 
 ...
 where (SRC.cdctime<=f1.lastupdated) and COALESCE( SRC.sys_id,'')<> COALESCE(TRGT.row_id ,''))b
