SELECT CASE WHEN cnt THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN cnt THEN 'MDS to DWH data validation failed for d_incident.description' ELSE 'SUCCESS' END as Message from (select count(1) cnt 
 ...
 where (SRC.cdctime<=f1.lastupdated) and  COALESCE( SRC.description,'UNSPECIFIED')<> TRGT.description)b
