...
left join (select source_id,max(lastupdated) as lastupdated from <<tenant>>_mdwdb.d_o_data_freshness group by source_id) f1 on (f1.source_id = x.sourceinstance)
where (x.cdctime<=f1.lastupdated) and last_resolved_by_key<>
