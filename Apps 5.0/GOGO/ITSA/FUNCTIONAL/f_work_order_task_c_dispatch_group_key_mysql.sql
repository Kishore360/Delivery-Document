 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_change_request.work_start_on_key' ELSE 'SUCCESS' END as 
 Message
 FROM (select count(1) cnt from gogo_mdsdb.wm_task_final SRC
  
 LEFT JOIN gogo_mdwdb.d_internal_organization LKP 
on (LKP.row_id =  concat('GROUP~',SRC.dispatch_group)  AND SRC.sourceinstance= LKP.source_id  ) 
