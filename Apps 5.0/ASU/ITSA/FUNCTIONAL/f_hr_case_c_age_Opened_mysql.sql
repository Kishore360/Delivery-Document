...
join asu_mdwdb.f_hr_case_c f ON (SRC.sys_id =f.row_id  AND SRC.sourceinstance= f.source_id  )
JOIN asu_mdwdb.d_hr_case_c a ON a.row_key = f.hr_case_c_key AND f.source_id = a.source_id
...
where timestampdiff(Second,(convert_tz(SRC.opened_at,'GMT','US/Mountain')),df.lastupdated)<> f.age)A;
