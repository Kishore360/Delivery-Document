...
WHERE CASE WHEN SRC.published > SYSDATE() THEN 0 ELSE 
(TIMESTAMPDIFF(second,CONVERT_TZ(CONVERT_TZ(SRC.published,'GMT' ,'US/Mountain'),'US/Mountain','GMT'),
(SELECT CONVERT_TZ(MAX(lastupdated),'US/Mountain','GMT') AS lastupdated FROM asu_mdwdb.d_o_data_freshness 
WHERE sourcename like 'ServiceNow%' )))END <> f.age
 )A ; 
