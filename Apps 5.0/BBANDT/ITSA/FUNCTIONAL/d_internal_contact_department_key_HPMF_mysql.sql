...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.department_key' ELSE 'SUCCESS' END as Message
 FROM (select * from bbandt_mdsdb.sys_user_final where  cdctype<>'D') SRC 

left join (select * from bbandt_mdwdb.d_internal_contact) TRGT
on CONCAT('INTERNAL_CONTACT~', SRC.USER_ID) =TRGT.row_id and SRC.sourceinstance=TRGT.source_id

left join (select * from bbandt_mdwdb.d_internal_organization)  LKP
