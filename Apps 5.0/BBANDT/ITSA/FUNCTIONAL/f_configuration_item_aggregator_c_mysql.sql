...
from bbandt_mdsdb.cmdb_ci_final  as ser
LEFT JOIN bbandt_mdsdb.sys_db_object_final sdof ON ser.sys_class_name=sdof.name and ser.SourceInstance=sdof.SourceInstance
inner join (select value,label from bbandt_mdsdb.sys_choice_final where name = 'cmdb_ci' and element = 'operational_status' and language = 'en' and label = 'Operational' group by value) as op on op.value = ser.operational_status
inner join (select value,label from bbandt_mdsdb.sys_choice_final where name = 'cmdb_ci' and element = 'install_status' and language = 'en' and label = 'Installed' group by value) as ins on ins.value = ser.install_status
group by sdof.label
) as src
left join
...
from bbandt_mdwdb.f_configuration_item_aggregator_c cd 
inner join(select date_format(max(lastupdated) ,'%Y%m01') AS refreshday from bbandt_workdb.d_o_data_freshness) as df  
