 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_group_coordinated_with_c.group_coordinated_with_c_key' ELSE 'SUCCESS' END as Message
...
join bbandt_mdwdb.f_group_coordinated_with_c b
...
join bbandt_mdwdb.d_internal_organization c
on COALESCE(CONCAT('GROUP','~',UPPER(SUBSTRING_INDEX(SUBSTRING_INDEX(TRIM(a.u_group_coordinated_with), ',', n.n), ',', -1))),'UNSPECIFIED')=c.row_id 
and a.sourceinstance=c.source_id
where COALESCE(c.row_key,CASE WHEN a.u_group_coordinated_with IS NULL THEN 0 ELSE -1 END)<>b.group_coordinated_with_c_key 
and a.cdctype='X';