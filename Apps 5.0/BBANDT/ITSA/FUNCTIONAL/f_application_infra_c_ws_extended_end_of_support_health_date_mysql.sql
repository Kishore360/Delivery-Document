...
left outer join 
bbandt_mdsdb.sa_techportfolio_products_final ws on X.webserver_product = ws.name and X.sourceinstance = ws.sourceinstance
 JOIN bbandt_mdwdb.f_application_infra_c TRGT 
 ON (X.row_id  =TRGT.row_id  
 AND X.sourceinstance= TRGT.source_id  )
 WHERE case when (upper(ws.supplier) like '%MICROSOFT%') and ws.Extended_Support is not null then ws.Extended_Support else ws.support_end_date end<>TRGT.WS_Extended_End_of_Support_Health_Date
 and X.CDCTYPE  <>'D'