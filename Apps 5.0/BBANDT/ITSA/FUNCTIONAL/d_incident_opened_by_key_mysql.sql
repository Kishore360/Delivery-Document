...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM ( SELECT * FROM bbandt_mdsdb.incident_final WHERE CDCTYPE<>'D') SRC 
...
WHERE COALESCE(CASE WHEN SRC.caller_id IS NULL THEN 0 else LKP.row_key end)<> COALESCE(TRGT.opened_by_key ,'') 
