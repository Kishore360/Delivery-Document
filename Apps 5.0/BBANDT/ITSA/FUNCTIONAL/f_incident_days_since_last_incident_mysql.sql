...
bbandt_mdwdb.f_incident f_incident   
INNER JOIN
bbandt_mdwdb.d_incident d_incident 
ON f_incident.incident_key=d_incident.row_key  
JOIN (
select source_id,max(lastupdated) as lastupdated from bbandt_workdb.d_o_data_freshness
group by source_id
