...
(a.X/b.Y)*100 as Closed_Incident_met_all_sla_percentage
FROM 
(select	count(a24.row_key)*1.0 AS X
from	ldb.f_incident_closed	a24
	join	ldb.d_incident	a12
	  on 	(a24.incident_key = a12.row_key)
...
CROSS JOIN 
(select	count(a24.row_key)*1.0 AS Y
from	ldb.f_incident_closed	a24
	join	ldb.d_calendar_date	a12
	  on 	(a24.closed_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
UNION 
select	'Count (Calendar Month)',
Count (distinct a13.lagging_count_of_month)
from	ldb.f_incident	a24
	join	ldb.d_calendar_date	a12
	  on 	(a24.opened_on_key = a12.row_key)
	join	ldb.d_calendar_month	a13
...
UNION 
 select	'Incident Reassignment Count',sum(a11.reassignment_count) AS Reassignment_Count
...
where	a13.lagging_count_of_month between 0 and 24
UNION 
select	'Incident Reopened Count',count(a11.row_key) AS Reopened_incidents
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
...
where	a13.lagging_count_of_month between 0 and 24
UNION 
select	'Incident Reopened Count',SUM(a11.reopened_count) AS Reopened_incidents
from	ldb.f_incident	a11
	join	ldb.d_calendar_date	a12
	  on 	(a11.opened_on_key = a12.row_key)
	join	ldb.d_incident	a13
