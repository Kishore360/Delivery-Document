...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.u_subcataction_c_key' ELSE 'SUCCESS' END as Message
FROM pequalcomm_mdsdb.incident_final SRC 
 LEFT JOIN pequalcomm_mdwdb.d_incident TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
LEFT JOIN pequalcomm_mdwdb.d_lov LKP 
