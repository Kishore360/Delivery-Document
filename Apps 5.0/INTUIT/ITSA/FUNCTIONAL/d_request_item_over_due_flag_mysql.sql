...
join intuit_mdwdb.f_request_item f ON( REQ_ITM.row_key = f.request_item_key )
JOIN intuit_mdwdb.d_lov_map br ON (f.state_src_key = br.src_key AND br.dimension_wh_code = 'OPEN')
...
&& (REQ_ITM.due_on < (SELECT max(lastupdated) AS lastupdated FROM intuit_mdwdb.d_o_data_freshness 
