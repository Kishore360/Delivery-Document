...
FROM <<tenant>>_mdwdb.d_asset d
JOIN <<tenant>>_mdwdb.f_asset f ON d.row_key=f.asset_key
JOIN <<tenant>>_mdwdb.d_lov l ON f.asset_status_key=l.row_key AND dimension_name= 'Installed'
JOIN <<tenant>>_mdwdb.d_model_category  m ON f.model_category_key = m.row_key AND category_name = 'Computers'
