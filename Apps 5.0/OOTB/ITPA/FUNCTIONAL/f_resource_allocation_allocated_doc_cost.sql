...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 from (select SUBSTRING_INDEX(row_id,'~',1) as source_row_id, resource_plan_key, project_key, resource_key,sum(F.allocated_doc_cost) as allocated_doc_cost 
...
WHERE COALESCE( CAST(coalesce(round(SRC.allocated_cost),0) as decimal(20,10)),'')<> COALESCE(cast(round(TRGT.allocated_doc_cost) as decimal(20,10)),'');
