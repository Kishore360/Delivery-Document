...
on (a17.close_code_src_key = a120.row_key)
union
...
on (a17.phase_src_key = a121.row_key)
union
...
on (a17.phase_state_src_key = a122.row_key)
union
...
on (a13.approval_src_key = a123.row_key)
union
...
on (a13.reported_type_src_key = a124.row_key)
union
...
on (a13.impact_src_key = a125.row_key)
union
...
on (a13.priority_src_key = a126.row_key)
union
...
on (a13.state_src_key = a127.row_key)
union
...
on (a13.task_type_src_key = a128.row_key)
union
...
on (a13.urgency_src_key = a129.row_key)
union
select 'ldb.d_lov_segment_type_c a130 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_change_task a11
join ldb.d_tribe_c a18
on (a11.tribe_c_key = a18.row_key) join ldb.d_segment_c a19
on (a18.segment_c_key = a19.row_key)
join ldb.d_lov_segment_type_c a130
on (a19.segment_type_c_key = a130.row_key)
