...
select  CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM  watson_mdwdb.f_case f_c
 LEFT JOIN 
  (select dt.source_id, dt.row_key, dt.row_id, max(fs.created_on) changed_on
from watson_mdwdb.d_case dt JOIN watson_mdwdb.d_lov_map dlm 
...
join watson_mdwdb.f_case_activity fs 
on dt.source_id=fs.source_id and dt.row_key=fs.case_key 
join watson_mdwdb.d_lov_map d on d.src_key=fs.task_attribute_wh_new_value_key and d.source_id=fs.source_id and 
...
group by dt.source_id, dt.row_key) f
 ...
 where  f_c.last_resolved_on_key<>DATE_FORMAT(coalesce(f_c.last_resolved_on,f.changed_on,f_c.closed_on),'%Y%m%d')
