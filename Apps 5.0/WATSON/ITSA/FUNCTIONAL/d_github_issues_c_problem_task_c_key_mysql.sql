...
left join watson_mdwdb.d_problem_task d on u_problem_task=d.row_id
left join  watson_mdwdb.d_github_issues_c t on s.sys_id=t.row_id and s.sourceinstance=t.source_id
where problem_task_c_key<> coalesce(d.row_key,case when u_problem_task is null then 0 else -1 end );
