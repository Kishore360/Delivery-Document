...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_case.resolution_after_first_response_duration_c' ELSE 'SUCCESS' END as Message
FROM 
...
(select documentkey,sys_created_on,newvalue,oldvalue
 ...
 where oldvalue=10 group by 1,2,3 )r on di.row_id=r.documentkey
