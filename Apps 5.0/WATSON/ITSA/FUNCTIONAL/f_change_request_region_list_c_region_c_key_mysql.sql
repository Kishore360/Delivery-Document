 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_problem_region_list_c.pivot_date' ELSE 'SUCCESS' END as Message from ( select count(1) cnt  from watson_mdsdb.change_request_final  a
 JOIN
...
AND a.sourceinstance= d.source_id ) 
 left join watson_mdwdb.f_change_request_region_list_c b
