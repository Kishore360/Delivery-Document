 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_problem_region_list_c.pivot_date' ELSE 'SUCCESS' END as Message
...
ON n.n <= 1 + (LENGTH(COALESCE(TRIM(a.u_region_list),'UNSPECIFIED')) - LENGTH(REPLACE(COALESCE(TRIM(a.u_region_list),'UNSPECIFIED'), ',', '')))
 ...
 left join watson_mdwdb.f_problem_region_list_c b
