SELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 left join watson_mdwdb.d_lov lkp on COALESCE(CONCAT('PRIORITY','~','PAGERDUTY','~','~','~',UPPER(src.priority)),'UNSPECIFIED')=lkp.row_id
 and src.sourceinstance=lkp.source_id
 where coalesce(lkp.row_key,case when src.priority is null then 0 else -1 end)<>trgt.priority_src_key)ma;
