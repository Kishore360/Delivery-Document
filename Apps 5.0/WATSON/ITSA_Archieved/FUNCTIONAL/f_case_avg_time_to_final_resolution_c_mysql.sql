...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_case.avg_time_to_final_resolution_c' ELSE 'SUCCESS' END as Message
FROM
watson_mdwdb.f_case fi 
inner join watson_mdwdb.d_case di on di.row_key=fi.case_key
inner join ( select documentkey , max(sys_created_on) as sys_created_on from 
(select documentkey,sys_created_on,oldvalue,newvalue
...
where newvalue=21 group by 1 )r on di.row_id=r.documentkey
where fi.avg_time_to_final_resolution_c <> timestampdiff(second, di.opened_on, r.sys_created_on)
;
