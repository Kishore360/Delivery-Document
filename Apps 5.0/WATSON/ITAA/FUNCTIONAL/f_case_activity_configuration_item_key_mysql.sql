...
LEFT JOIN watson_mdwdb.f_case TRGT ON (SRC.sys_id =TRGT.row_id AND SRC.sourceinstance =TRGT.source_id )
LEFT JOIN watson_mdwdb.d_configuration_item LKP ON ( COALESCE(SRC.cmdb_ci,'UNSPECIFIED') = LKP.row_id AND SRC.sourceinstance=LKP.source_id)
