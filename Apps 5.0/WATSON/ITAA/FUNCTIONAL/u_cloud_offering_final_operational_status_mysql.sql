...
FROM (select
count(*) as cnt
from  watson_mdsdb.u_cloud_offering_final a11
 join watson_mdwdb.d_configuration_item  a12
on a12.row_id= a11.sys_id and a11.sourceinstance=a12.source_id
JOIN watson_mdwdb.d_lov LKP ON (COALESCE(CONCAT('OPERATIONAL_STATUS~U_CLOUD_OFFERING~~~',a11.operational_status),'UNSPECIFIED')= LKP.row_id
