...
from  ldb.f_incident_activity  a11 
JOIN ldb.d_calendar_date_fiscal a13 on (a11.assigned_on_key = a13.row_key)
UNION 
select 'ldb.f_incident' as Table_Name, count(1) Row_Count 
from ldb.f_incident a11
JOIN ldb.f_incident_activity a12
on (a11.assignment_group_key = a12.current_assginment_group_key 
...
from  ldb.f_incident_activity  a11 
join  ldb.d_calendar_date_opened a110 on (a11.assigned_on_key = a110.row_key)
UNION 
select 'ldb.d_calendar_month' as Table_Name, count(1) Row_Count
from ldb.f_incident_activity a11 
JOIN ldb.d_calendar_date a14 on (a11.assigned_on_key = a14.row_key)
JOIN ldb.d_calendar_month a17 on (a14.month_start_date_key = a17.row_key)
UNION 
select 'ldb.d_internal_organization_group' as Table_Name, count(1) Row_Count
from  ldb.f_incident_activity  a11 
join ldb.d_internal_organization_group  a12 on (a11.current_assginment_group_key = a12.row_key)
UNION 
...
select 'ldb.d_internal_contact_to' as Table_Name, count(1) Row_Count
from  ldb.f_incident_activity a11 
JOIN ldb.d_internal_contact_to a17 on (a11.to_assigned_to_key = a17.row_key)
UNION 
...
join ldb.d_incident_priority  a19 on (a13.priority_src_key = a19.row_key)
UNION 
select 'ldb.d_incident_state' as Table_Name, count(1) Row_Count
from ldb.f_incident_activity a11
JOIN ldb.d_incident a13 on (a11.incident_key = a13.row_key)
join ldb.d_incident_state a110 on (a13.state_src_key = a110.row_key)
