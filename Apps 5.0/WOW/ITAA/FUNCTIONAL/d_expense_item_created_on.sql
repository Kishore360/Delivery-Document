...
FROM (SELECT S.sys_id,S.sourceinstance,S.sys_created_on 
	FROM wow_mdsdb.fm_expense_line_final S 
	JOIN wow_mdsdb.pm_project_final SRC_P ON (SRC_P.sys_id = S.task) 
    UNION
    SELECT S.sys_id,S.sourceinstance,S.sys_created_on 
	FROM wow_mdsdb.fm_expense_line_final S 
	JOIN wow_mdsdb.pm_project_task_final SRC_PT ON (SRC_PT.sys_id = S.task) 
)SRC
JOIN wow_mdwdb.d_expense_item TRGT 
