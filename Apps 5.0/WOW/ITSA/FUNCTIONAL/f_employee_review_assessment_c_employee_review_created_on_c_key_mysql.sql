...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_employee_review_assessment_c.employee_review_created_on_c_key' ELSE 'SUCCESS' END as Message 
FROM ( SELECT CONCAT(src1.sys_id,'~',src3.sys_id) as sys_id,
src1.sourceinstance,COALESCE(DATE_FORMAT(CONVERT_TZ(src2.sys_created_on,'GMT','US/Central'),'%Y%m%d'),'UNSPECIFIED')as val
FROM
...
ON COALESCE(MAIN_SRC1.val,'UNSPECIFIED') = lkp.row_id 
WHERE COALESCE(lkp.row_key,CASE WHEN MAIN_SRC1.val IS NULL THEN 0 else -1 end) <>employee_review_created_on_c_key
;