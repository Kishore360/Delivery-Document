...
FROM (select count(1) as cnt from pgi_mdwdb.f_incident dfi
join pgi_mdsdb.incident_final incf on dfi.row_id = incf.sys_id and dfi.source_id= incf.sourceinstance
where
dfi.last_resolved_on_key = COALESCE(DATE_FORMAT(CONVERT_TZ(incf.closed_at,"GMT","UTC"),'%Y%m%d'),'UNSPECIFIED')
