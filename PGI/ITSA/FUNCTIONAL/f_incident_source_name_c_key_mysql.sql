...
ON COALESCE(CONCAT('SOURCE','~','INCIDENT','~','~','~',UPPER(inc.u_source)),'UNSPECIFIED')  = lkp.row_id
and finc.source_id=lkp.source_id
where coalesce(lkp.row_key,case when inc.u_source is null then 0 else -1 end)<> finc.source_name_c_key)a
