...
where 
coalesce(LKP.row_key,case when  SRC.problem_assignment_group_id is null then 0 else -1 end) <>TRGT.problem_assignment_group_key)aSELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END AS Result
,CASE WHEN cnt > 0 THEN 'Data did not Match.'
ELSE 'Data Matched' END AS Message
FROM (select count(*) as cnt 
from
