...
from
(
SELECT s1.*, LKP.dimension_wh_code FROM (SELECT * FROM ge_mdsdb.change_request_final WHERE CDCTYPE<>'D' 
...
left outer join 
(select * from ge_mdwdb.d_change_failure where current_flag='Y') TGT
