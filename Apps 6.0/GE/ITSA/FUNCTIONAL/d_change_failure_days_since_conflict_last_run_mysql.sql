...
where case 
when (COALESCE((SELECT dimension_wh_code FROM ge_mdwdb.d_lov_map WHERE
...
then (case  
      when (CONVERT_TZ ((SELECT  max(lastupdated) FROM ge_mdwdb.d_o_data_freshness 
            ...
            when (TIMESTAMPDIFF(SECOND, change_request.conflict_last_run, CONVERT_TZ((SELECT max(lastupdated) 
			      FROM ge_mdwdb.d_o_data_freshness 
			...
			then (TIMESTAMPDIFF(SECOND, change_request.conflict_last_run, CONVERT_TZ((SELECT  max(lastupdated) 
                  FROM ge_mdwdb.d_o_data_freshness   WHERE
