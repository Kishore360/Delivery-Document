...
FROM (select sys_id, sourceinstance,u_category from paloalto_mdsdb.incident_final where CDCTYPE<>'D')SRC 

LEFT JOIN (select row_key, src_rowid, source_id,dimension_class from paloalto_mdwdb.d_lov where soft_deleted_flag<>'Y') LKP 
...
LEFT JOIN paloalto_mdwdb.f_incident TRGT 
