 ...
 FROM paloalto_mdsdb.u_incident_tasks_final SRC 
  JOIN paloalto_mdwdb.f_incident_tasks_c TRGT 
 ON (SRC.sys_id =TRGT.row_id 
 AND SRC.sourceinstance= TRGT.source_id )
 JOIN paloalto_mdwdb.d_incident_tasks_c trgtf on TRGT.incident_tasks_c_key=trgtf.row_key
 left join paloalto_mdwdb.d_lov_map lm 
 ON (lm.src_key = trgtf.state_src_key)
 where trgtf.soft_deleted_flag='N' and lm.soft_deleted_flag='N' and 
 (COALESCE(TIMESTAMPDIFF(SECOND,SRC.sys_updated_on,CONVERT_TZ((SELECT MAX(lastupdated) AS lastupdated
FROM paloalto_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)- TRGT.dormancy_age) not in  (3600,-3600) and 
...
FROM paloalto_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)<> TRGT.dormancy_age )a
