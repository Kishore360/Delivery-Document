

SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.sub_category_src_key' ELSE 'SUCCESS' END as Message
 FROM paloalto_mdsdb.incident_final SRC 
 LEFT JOIN paloalto_mdwdb.f_incident TRGT 
 ON (SRC.sys_id =TRGT.row_id  
 AND SRC.sourceinstance= TRGT.source_id  )
LEFT JOIN paloalto_mdwdb.d_lov LKP 
 ON ( concat('SUBCATEGORY','~','INCIDENT','~',upper(u_subcategory))= LKP.src_rowid 
AND SRC.sourceinstance= LKP.source_id )
 WHERE LKP.soft_deleted_flag='N' and COALESCE(LKP.row_key,CASE WHEN SRC.u_subcategory IS NULL THEN 0 else -1 end)<> (TRGT.sub_category_src_key)
