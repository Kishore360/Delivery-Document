...
-- count(distinct case when upper(status_te.wi_sts) = 'FAILED' then FE.test_execution_key else null end ) as te_failed_count,
-- count(distinct case when upper(status_te.wi_sts) = 'BLOCKED' then FE.test_execution_key else null end ) as te_blocked_count
...
group by FE.test_suite_key,status_te.sor_lov_name
order by FE.test_suite_key,status_te.sor_lov_name