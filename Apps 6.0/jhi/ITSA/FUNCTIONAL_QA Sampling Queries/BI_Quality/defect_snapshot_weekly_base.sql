drop table if exists #DWH_TABLE_SCHEMA.work_item_weekly_snapshot_cube;
create table #DWH_TABLE_SCHEMA.work_item_weekly_snapshot_cube AS
select 
F.row_key,
F.work_item_key,
F.n_date,
...
TO_CHAR((D.created_on),'fmDD/fmMM/YYYY fmHH:MI:SS AM') as detected_on_date,
TO_CHAR((D.work_start_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM') in_progress_dt,
TO_CHAR((D.resolved_on),'fmDD/fmMM/YYYY fmHH:MI:SS AM') as resolved_dt,
...
from #DWH_TABLE_SCHEMA.f_work_item_snapshot_weekly F
...
on CALWK.row_key = CAL.week_start_date_key
join
(
