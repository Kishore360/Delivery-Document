...
else to_char(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_creation/86400.00 else 0.0 end)/count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end),'FM999,999,999,999,990.0') end avg_lead_time,
...
else to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_in_progress/86400.00 else 0.0 end)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.row_key else null end))),'FM999,999,999,999,990.0') end avg_cycle_time
...
group by type.row_key, type.sor_lov_name
order by type.row_key, lower(type.sor_lov_name)