...
ITE.name it_nm, 
TO_CHAR((ITE.start_date),'fmMM/fmDD/YYYY fmHH:MI:SS AM') plnd_strt_dt,
TO_CHAR((ITE.end_date),'fmMM/fmDD/YYYY fmHH:MI:SS AM') plnd_end_dt,
TO_CHAR((ITE.completion_date),'fmMM/fmDD/YYYY fmHH:MI:SS AM') actl_end_dt,
...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) then F.row_key else null end),'FM999,999,999,999,990') opened_wi,
...
group by PROJ.project_name, ITE.row_key, ITE.name, ITE.start_date, ITE.end_date, ITE.completion_date
order by lower(PROJ.project_name), ITE.row_key, upper(ITE.name), ITE.start_date, ITE.end_date, ITE.completion_date
