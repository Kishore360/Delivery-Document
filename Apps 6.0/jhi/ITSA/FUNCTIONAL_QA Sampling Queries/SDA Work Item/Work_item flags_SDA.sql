...
group by D.work_item_number, D.row_id, PROJ.project_name, D.proposed_flag, D.in_progress_flag, D.reopened_flag, D.overdue_flag, D.multiple_assignment_flag
order by D.work_item_number, D.row_id, lower(PROJ.project_name), upper(D.proposed_flag), upper(D.in_progress_flag), upper(D.reopened_flag), upper(D.overdue_flag), upper(D.multiple_assignment_flag)
