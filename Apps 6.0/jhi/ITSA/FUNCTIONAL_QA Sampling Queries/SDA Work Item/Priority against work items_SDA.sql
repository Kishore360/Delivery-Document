...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then F.row_key else null end),'FM999,999,999,999,990') crtd_wi,
...
group by priority.wi_prio, type.row_key, type.sor_lov_name
order by upper(priority.wi_prio), type.row_key, lower(type.sor_lov_name)