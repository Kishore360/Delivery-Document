...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then F.work_item_key else null end),'FM999,999,999,999,990') crtd_wi,
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990') proposed_wi,
to_char(count(case when (cal_inprog_mon.lagging_count_of_month between 0 and 12 or status.wi_sts = 'IN PROGRESS') and status.wi_sts in ('IN PROGRESS') and F.work_item_work_start_on_key is not null then F.work_item_key else null end),'FM999,999,999,999,990') inprog_wi
...
group by ITE.row_key, ITE.name, PROJ.project_name
order by ITE.row_key, upper(ITE.name), lower(PROJ.project_name)
