...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_work_item.level_3_product_management_ready_c' ELSE 'SUCCESS' END as Message
...
WHERE  CASE             
            WHEN (CASE 
                ...
                AND (CASE 
                    ...
                    ELSE 0 
                END) = 1 
                AND LKP.quarterly_planning_completed_and_first_held_c = 1 THEN 1 
                ELSE 0 
            END) = 1  
            AND LKP.funding_model_supports_prod_mgmt_c=1 
            AND LKP.product_vision_initiated_c=1 
            AND LKP.product_okrs_identified_c=1
            THEN 1             
            ELSE 0          
        END  <> (LKP.level_3_product_management_ready_c) and LKP.soft_deleted_flag ='N';