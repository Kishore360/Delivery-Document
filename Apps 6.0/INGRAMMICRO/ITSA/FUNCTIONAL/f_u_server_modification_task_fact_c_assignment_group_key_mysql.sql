...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_u_server_modification_task_fact_c.assignment_group_key' ELSE 'SUCCESS' END as Message
FROM 
(
...
JOIN ingrammicro_mdwdb.d_internal_organization LKP ON LKP.row_id=
COALESCE(concat('GROUP~',SRC.assignment_group),'UNSPECIFIED') and SRC.sourceinstance=LKP.source_id 
