...
left JOIN ingrammicro_mdwdb.d_internal_contact LKP on LKP.row_id=coalesce(concat('INTERNAL_CONTACT','~',SRC.opened_by),'UNSPECIFIED')
AND SRC.sourceinstance=LKP.source_id
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.parent is NULL THEN 0 ELSE -1 END)<>TRGT.opened_by_key ) temp;