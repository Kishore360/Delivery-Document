SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.formerly_p1_flag_c' ELSE 'SUCCESS' END as Message
...
and oldvalue=1  group by documentkey ) ref on (SRC.sys_id=ref.documentkey and SRC.sourceinstance=ref.sourceinstance)
  WHERE downgraded_from_p1_flag_c='Y'
  AND COALESCE(CASE WHEN SRC.priority  != 1 then 'Y' else 'N' END,'') <> TRGT.formerly_p1_flag_c;