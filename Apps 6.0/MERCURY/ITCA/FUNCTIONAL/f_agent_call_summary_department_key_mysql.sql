SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 from mercury_mdwdb.f_agent_call_summary TRGT
 left join mercury_mdwdb.d_internal_contact lkp1
 on TRGT.employee_key = lkp1.row_key
  left join mercury_mdwdb.d_acd d
 on d.row_key = TRGT.acd_key
 left join mercury_mdwdb.d_internal_contact lkp2
 on (lkp2.source_id = 2 AND lkp1.row_id =concat('INTERNAL_CONTACT~',d.acd_code,'~', lkp2.mdm_id))
 where lkp2.department_key
 <> TRGT.department_key
