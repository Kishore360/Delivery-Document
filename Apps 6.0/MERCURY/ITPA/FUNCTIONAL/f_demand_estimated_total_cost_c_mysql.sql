...
left join  mercury_mdsdb.hp_kcrt_req_header_details_final kcrt_req_header_details on kcrt_requests.REQUEST_ID=kcrt_req_header_details.REQUEST_ID and kcrt_requests.sourceinstance=kcrt_req_header_details.sourceinstance
join  mercury_mdwdb.f_demand TRGT on kcrt_fg_pfm_proposal.REQUEST_ID=TRGT.row_id
where concat(coalesce(replace(replace(replace(kcrt_req_header_details.VISIBLE_PARAMETER32,',',''),'$',''),'USD',''),0),'.','0000000000')<> TRGT.estimated_total_cost_c)a
