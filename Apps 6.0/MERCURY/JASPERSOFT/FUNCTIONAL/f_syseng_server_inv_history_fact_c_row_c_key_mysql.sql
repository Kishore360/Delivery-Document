...
(SELECT count(1) as CNT from
   			mercury_mdsdb.syseng_server_inv_history_final SRC 
				...
				on DATE_FORMAT(STR_TO_DATE(SRC.date_time,'%Y-%m-%d'),'%Y%m%d')=TRGT.row_id and SRC.sourceinstance=TRGT.source_id and SRC.cdctype<>'D'  
   join mercury_mdwdb.d_calendar_date LKP on coalesce(DATE_FORMAT(STR_TO_DATE(SRC.date_time,'%m/%d/%Y'),'%Y%m%d'),DATE_FORMAT(STR_TO_DATE(SRC.date_time,'%m-%d-%Y'),'%Y%m%d'))=LKP.row_id
