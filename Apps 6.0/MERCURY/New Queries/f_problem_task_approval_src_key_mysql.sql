...
LEFT JOIN mercury_mdwdb.f_problem_task TRGT ON (SRC.sys_id=TRGT.row_id AND SRC.sourceinstance=TRGT.source_id)
LEFT JOIN mercury_mdwdb.d_lov LKP ON (COALESCE( CONCAT('APPROVAL','~','PROBLEM_TASK','~','~','~',UPPER(SRC.approval)),'UNSPECIFIED')=LKP.row_id
