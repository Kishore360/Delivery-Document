...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for d_pandg_ot_processing_activities_inventory_c.control_criteria' 
...
where COALESCE(CASE   
                    WHEN src.Compliance_control = 'Data breach response'       THEN IF(src.Data_breach_response_Criteria <> '',
                    src.Data_breach_response_Criteria ,
                    NULL)  
                    WHEN src.Compliance_control = 'Data Retention'                              THEN IF(src.Data_retention_Criteria <> '',
                    src.Data_retention_Criteria ,
                    NULL)  
