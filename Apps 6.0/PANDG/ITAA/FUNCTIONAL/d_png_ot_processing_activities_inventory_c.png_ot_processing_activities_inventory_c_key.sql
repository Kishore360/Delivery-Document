...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for f_pandg_ot_processing_activities_inventory_c.pandg_ot_inventory_c_key' 
...
where COALESCE(lkp.row_key,case when src.inventory_id is NULL THEN 0 ELSE -1 END)<>trgt.pandg_ot_processing_activities_inventory_c_key;