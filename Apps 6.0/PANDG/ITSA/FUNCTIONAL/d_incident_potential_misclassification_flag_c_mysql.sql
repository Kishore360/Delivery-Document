...
FROM pandg_mdwdb.f_incident f_inc
JOIN pandg_mdwdb.f_change_request f_chg ON f_inc.change_request_key = f_chg.change_request_key OR f_chg.incident_key = f_inc.incident_key
GROUP BY 1
) X
ON trgt.row_key = X.incident_key
JOIN pandg_mdwdb.d_lov lv ON trgt.priority_src_key = lv.row_key
JOIN pandg_mdwdb.d_lov_map lv_mp ON trgt.state_src_key =lv_mp.src_key
JOIN pandg_mdwdb.d_configuration_item d_conf ON X.configuration_item_key = d_conf.row_key
LEFT JOIN (SELECT incident_cbp_c_key,count(1) cnt FROM pandg_mdwdb.f_task_cbp_c WHERE impacted_cbp_flag ='Y' AND incident_cbp_c_key >0 and soft_deleted_flag ='N' group by 1) cbp ON trgt.row_key =cbp.incident_cbp_c_key
