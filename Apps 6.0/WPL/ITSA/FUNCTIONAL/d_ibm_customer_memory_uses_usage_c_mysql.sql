SELECT CASE WHEN count(1)  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for d_ibm_customer_memory_uses.usage_c' ELSE 'SUCCESS' END as Message FROM   
...
where cm.IBM_usage<>TRGT.usage_c and cm.cdctype='X'