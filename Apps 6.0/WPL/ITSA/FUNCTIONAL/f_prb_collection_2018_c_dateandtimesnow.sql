...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_prb_collection_2018_c_fact.dateandtimesnow' ELSE 'SUCCESS' END as Message  
FROM 
(
...
where src.cdctype<>'D' and    STR_TO_DATE(src.dateandtimesnow,'%m-%d-%Y %T') <> trgt.dateandtimesnow) temp; 