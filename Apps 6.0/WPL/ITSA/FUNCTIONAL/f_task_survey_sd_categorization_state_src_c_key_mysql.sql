...
on tf.sys_id = tsd.u_parent_ticket
 ...
 ON ( COALESCE( CONCAT('U_SD_CATEGORIZATION_STATE~SURVEY~',upper(tsd.u_sd_categorization_state)),'UNSPECIFIED') = LKP.row_id 
AND tsd.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN tsd.u_sd_categorization_state IS NULL THEN 0 else -1 end)<> (ts.sd_categorization_state_src_c_key)
 and tf.CDCTYPE<>'D'
