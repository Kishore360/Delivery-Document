...
join wpl_mdwdb.d_lov lkp on COALESCE( CONCAT('ENVIRONMENT~SAP_BACKUP~',src.environment),'UNSPECIFIED') =lkp.row_id and src.sourceinstance=lkp.source_id
where src.cdctype<>'D' and coalesce(lkp.row_key,case when src.environment is null then 0 else -1 end)<>trgt.environment_c_key)ma
;