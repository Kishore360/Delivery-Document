...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_cab_performance_c.cab_date' ELSE 'SUCCESS' END as Message  
FROM 
(
...
where src.cdctype<>'D' and   STR_TO_DATE(src.cabdate,'%m/%d/%Y') <> trgt.cab_date
) temp; 
