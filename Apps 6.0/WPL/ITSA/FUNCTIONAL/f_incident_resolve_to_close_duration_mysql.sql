SELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
  CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_incidnet.resolve_to_close_duration' ELSE 'SUCCESS' END as Message from(
...
where (case 
            when lv.dimension_wh_code='CLOSED' THEN  case 
                ...
                'GMT',
                'America/New_York')) <coalesce(CONVERT_TZ( CASE 
                    when COALESCE((SELECT
                        dimension_wh_code 
                    FROM
                        wpl_mdwdb.d_lov_map 
                    ...
                    WHERE
                        ...
                        AND d_lov_map.source_id = incident.sourceinstance),
                    'UNKNOWN') in ('CLOSED',
                    'RESOLVED' ) then  coalesce(incident.closed_at,
                    incident.sys_updated_on ) 
                end ,
                'GMT',
                'America/New_York'),
            ...
            ELSE NULL 
        END) <> f.resolve_to_close_duration and incident.cdctype<>'D'
