SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.met_all_resolution_sla_flag_c' ELSE 'SUCCESS' END as Message
 from 
(
SELECT a.incident_key ,dt.target_c,b.outcome_flag,b.stage_src_code,
 sum(case when stage_src_code ='cancelled' THEN 1 ELSE 0 END) as can_count,
...
where
B.met_all_resolution_sla_flag_c <> case when A.resolution_non_can_count > 0 and A.resolution_non_can_breached_count > 0 then 'N'
when A.resolution_non_can_count > 0 and resolution_non_can_breached_count = 0 then 'Y'
when A.resolution_sla_count = 0 then 'X'
when A.resolution_sla_count > 0 and resolution_non_can_count = 0 then 'C' else 'X' end 