select CASE WHEN Count(1) > 0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN Count(1) > 0  THEN 'MDS to DWH data validation failed for f_ama_inventory_c_fact.control_count_not_started_c' ELSE 'SUCCESS' END as Message 
FROM 
png_mdwdb.f_ama_inventory_c_fact f
JOIN (SELECT coalesce(SUM(CASE 
                WHEN ( a.control_result_c like '%NOT%START%' 
                OR a.control_result_c ='UNSPECIFIED' ) 
                and a.control_in_scope_c ='TRUE' THEN 1 
                ELSE 0 
            END),0) AS cnt_notstarted, inventory_id
FROM			
...
where f.control_count_not_started_c <> t.cnt_notstarted
