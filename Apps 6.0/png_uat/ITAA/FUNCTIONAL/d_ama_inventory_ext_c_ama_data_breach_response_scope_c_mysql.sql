SELECT CASE WHEN Count(1) > 0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN Count(1) > 0  THEN 'MDS to DWH data validation failed for d_ama_inventory_ext_c.data_breach_response_scope_c' ELSE 'SUCCESS' END as Message
 ...
 WHERE SRC.cdctype<>'D' AND (CASE WHEN (((SRC.vendor_qualification = 'DATA CONTROLLER') ) 
                ...
                end) <> (TRGT.data_breach_response_scope_c);
