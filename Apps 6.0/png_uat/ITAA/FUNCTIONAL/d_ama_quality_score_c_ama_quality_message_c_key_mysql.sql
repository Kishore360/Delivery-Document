...
LEFT JOIN png_jul_mdwdb.d_lov LKP ON ( concat( 'AMA_QUALITY_MESSAGE_C~PROCESSING_ACTIVITIES~' ,upper( replace(replace(SRC.Message,'}',''),'{',''))) = LKP.row_id AND SRC.sourceinstance = LKP.source_id ) 
