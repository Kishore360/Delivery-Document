select CASE WHEN Count(1) > 0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN Count(1) > 0  THEN 'MDS to DWH data validation failed for f_ama_inventory_c_fact.not_started_perc_c' ELSE 'SUCCESS' END as Message 
FROM 
...
JOIN (SELECT (coalesce(SUM(CASE 
            ...
            END),0))/(SUM(CASE 
                ...
                ELSE 0 
            END)) AS not_started_perc_c, inventory_id
FROM			
...
where f.not_started_perc_c <> t.not_started_perc_c
