...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_rita_application_c.rita_app_technology_supplier_name' ELSE 'SUCCESS' END as Message
FROM 
(
...
(select group_concat(distinct CASE 
                WHEN u_ci_responsible_vendor.u_role='Technology Supplier' THEN d_internal_organization.organization_name 
                else '' 
            END 
        order by
            1 asc) as technology_supplier, SRC.row_key as row_key
...
where
d_rita_application_c.rita_app_technology_supplier_name=
CASE 
WHEN SRC1.technology_supplier LIKE ',%' THEN REPLACE(SRC1.technology_supplier,
',',
'')  
WHEN SRC1.technology_supplier ='' THEN 'UNSPECIFIED' ELSE SRC1.technology_supplier END
) temp;
