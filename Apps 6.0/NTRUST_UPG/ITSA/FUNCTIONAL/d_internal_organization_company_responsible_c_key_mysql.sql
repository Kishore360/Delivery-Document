...
join ntrust_mdwdb.d_internal_organization TRGT
 on coalesce(concat('GROUP~',SRC.sys_id),'UNSPECIFIED')=TRGT.row_id and SRC.sourceinstance=TRGT.source_id
  join ntrust_mdwdb.d_company_responsible_c LKP
 on SRC.u_company_responsible=LKP.row_id
where coalesce(LKP.row_key,case when SRC.u_company_responsible is null then 0 else -1 end)
         <> TRGT.company_responsible_c_key and SRC.cdctype='X' )temp;
