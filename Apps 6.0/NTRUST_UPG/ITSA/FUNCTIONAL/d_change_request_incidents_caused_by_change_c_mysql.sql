...
CASE WHEN a.CNT > 0 THEN 'MDS to DWH data validation failed for d_change_request.incidents_caused_by_change_c' ELSE 'SUCCESS' END as Message
...
(select f_incident.caused_by_change_key AS row_key,
		...
		where  TRGT2.incidents_caused_by_change_c <> f.Incs_caused_by_chng)a
