...
LEFT JOIN ntrust_mdwdb.f_task_sla TRGT2 ON (SRC.sys_id=TRGT2.row_id AND SRC.sourceinstance=TRGT2.source_id)
 Left JOIN ntrust_mdwdb.d_configuration_item LKP ON coalesce(task.cmdb_ci,'UNSPECIFIED')=LKP.row_id  AND SRC.sourceinstance=LKP.source_id
where case when task is NULL then 0 else coalesce(LKP.row_key,-1) end <> TRGT2.task_configuration_item_c_key and SRC.cdctype='X') temp;
