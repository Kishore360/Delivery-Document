...
CASE WHEN count(1) THEN 'MDS to DWH data validation failed for d_change_failure.numerify_change_failure_flag' ELSE 'SUCCESS' END as Message
from marriott_mdwdb.d_change_failure t
...
WHERE a.cdctype<>'D' and t.numerify_change_failure_flag <> (case when ((b.priority in( 1,2) and b.u_enterprise_reportable=1) OR
