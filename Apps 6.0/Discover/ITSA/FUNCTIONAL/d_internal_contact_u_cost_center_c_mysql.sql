...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_problem.u_cost_center_c' ELSE 'SUCCESS' END as Message
FROM
(
...
WHERE SRC.u_cost_center<>TRGT.u_cost_center_c
AND SRC.cdctype='X'
) temp;