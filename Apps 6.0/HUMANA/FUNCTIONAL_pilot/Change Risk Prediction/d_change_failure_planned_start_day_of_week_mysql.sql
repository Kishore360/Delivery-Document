...
where case 
when (change_request.start_date is null  and (COALESCE((SELECT  dimension_wh_code  FROM humana_mdwdb.d_lov_map 
...
then 'UNSPECIFIED' 
when (COALESCE((SELECT dimension_wh_code  FROM humana_mdwdb.d_lov_map 
      WHERE d_lov_map.dimension_class = 'STATE~CHANGE_REQUEST' 
      AND d_lov_map.dimension_code = change_request.state 
      AND d_lov_map.source_id = change_request.sourceinstance), 'UNKNOWN') = 'OPEN')  
then (DAYNAME(CONVERT_TZ(change_request.start_date, 'UTC', 'US/Central'))) 
when (change_request.work_start is null  and (COALESCE((SELECT dimension_wh_code  FROM humana_mdwdb.d_lov_map 
      WHERE d_lov_map.dimension_class = 'STATE~CHANGE_REQUEST' 
      AND d_lov_map.dimension_code = change_request.state 
