SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 LEFT JOIN (select * from humana_mdwdb.d_change_failure where current_flag='Y' and soft_deleted_flag='N') TRGT 
 ON (SRC.sys_id=TRGT.row_id 
 AND SRC.sourceinstance=TRGT.source_id )
LEFT JOIN  humana_mdwdb.d_internal_contact LKP1 
 ON ( concat('INTERNAL_CONTACT~',SRC.requested_by) = LKP1.row_id 
 AND SRC.sourceinstance = LKP1.source_id ) 
 LEFT JOIN   humana_mdwdb.d_internal_organization LKP
