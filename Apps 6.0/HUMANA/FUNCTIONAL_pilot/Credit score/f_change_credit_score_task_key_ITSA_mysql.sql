SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_credit_score.task_key' ELSE 'SUCCESS' END as Message
...
from (select * from humana_mdwdb.d_incident ) d_inc
left join (select * from humana_mdwdb.f_incident) f_inc
on f_inc.incident_key = d_inc.row_key
left join humana_mdwdb.d_internal_contact ic
on f_inc.opened_by_key = ic.row_key
join humana_mdwdb.f_caused_by_change cc -- join used to get only the matching records for caused by change
on cc.incident_key = f_inc.incident_key
left join (select source_id,max(lastupdated) as lastupdated from humana_mdwdb.d_o_data_freshness group by source_id) od
on f_inc.source_id = od.source_id
left join humana_mdwdb.d_lov_map map
on map.dimension_class = 'PRIORITY~INCIDENT' and d_inc.priority_src_key=map.src_key
left join humana_mdwdb.d_lov_map st
on st.dimension_class = 'STATE~INCIDENT' and d_inc.state_src_key=st.src_key
left join humana_mdwdb.d_lov p1_inc
...
left join humana_mdwdb.d_lov p2_inc
...
left join (select * from humana_mdwdb.f_incident) f_inc
on f_inc.incident_key = d_inc.row_key
join humana_mdwdb.f_caused_by_change cc
on cc.incident_key = f_inc.incident_key
left join humana_mdwdb.d_internal_contact ic
on f_inc.opened_by_key = ic.row_key
left join (select source_id,max(lastupdated) as lastupdated from humana_mdwdb.d_o_data_freshness group by source_id) od
...
left join humana_mdwdb.d_lov_map st
on st.dimension_class = 'STATE~INCIDENT' and d_inc.state_src_key=st.src_key
left join humana_mdwdb.d_lov p3_inc
...
from (select * from humana_mdwdb.d_problem_task) prob
left join humana_mdwdb.f_problem_task f_pro
on f_pro.row_id = prob.row_id
and f_pro.source_id = prob.source_id
join (select distinct problem_key from humana_mdwdb.f_caused_by_change where problem_key > 0)cc
on cc.problem_key = f_pro.problem_key
left join humana_mdwdb.d_lov_map lov_map
...
left join humana_mdwdb.d_lov p_prob
...
from (select * from humana_mdwdb.d_change_request ) chg
left join humana_mdwdb.f_change_request f
...
left join humana_mdwdb.d_lov p_chg
...
null as task_duration, null as caused_by_change_closed_on_key,f.state_src_key as task_state
from (select * from humana_mdwdb.d_change_request where change_type_normal_to_emergency_flag = 'Y') chg
left join humana_mdwdb.f_change_request f
on f.change_request_key = chg.row_key 
left join humana_mdwdb.d_internal_contact ic
on f.opened_by_key = ic.row_key
...
left join (select source_id,max(lastupdated) as lastupdated from humana_mdwdb.d_o_data_freshness group by source_id) od
on chg.source_id = od.source_id
left join humana_mdwdb.d_calendar_date curr_month
...
where curr_month.lagging_count_of_month = 0 and st.dimension_wh_code = 'OPEN')a) SRC 
 LEFT JOIN humana_mdwdb.f_change_credit_score TRGT 
 ON (SRC.row_id=TRGT.row_id AND SRC.source_id=TRGT.source_id AND SRC.task_type = TRGT.task_type)
 where COALESCE( SRC.task_key,'')<> COALESCE(TRGT.task_key,'')
