...
-- Duplicate Query to  irisk_category_c , Due to other Queries we are maintaining it.
SELECT 
CASE WHEN CNT > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_rita_application_c.irisk_category_c_Duplicate' ELSE 'SUCCESS' END as Message
FROM 
(
...
WHERE COALESCE(SRC.SOURCE_CATEGORY,'')<>TRGT.irisk_category_c
) temp;