...
, 	CASE WHEN sum(case when irc.MEGA_HEX_ID= ra.u_pg_source_system_id then 1 else 0 END) > 0 THEN  'Y' ELSE 'N' END 			as Privacy_Exists_in_iRisk_flag
,	CASE WHEN sum(case when ra.u_pg_source_system_id = ir.Mega_HEX_ID AND  
...
,	CASE WHEN sum(case when nonir.HexaIdAbs= ra.u_pg_source_system_id THEN 1 ELSE 0 END) > 0 THEN  'Y' ELSE 'N' END 			as privacy_Exists_in_Legacy_flag 
