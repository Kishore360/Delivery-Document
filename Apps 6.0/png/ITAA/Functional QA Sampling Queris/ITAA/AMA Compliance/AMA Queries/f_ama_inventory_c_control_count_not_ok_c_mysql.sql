select CASE WHEN Count(1) > 0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN Count(1) > 0  THEN 'MDS to DWH data validation failed for f_ama_inventory_c_fact.control_count_not_ok_c' ELSE 'SUCCESS' END as Message 
FROM 
...
JOIN (SELECT coalesce(SUM(CASE 
            ...
            END ='TRUE' THEN 1 
                ELSE 0 
            END),0) as cnt_notok, a1.inventory_id
FROM
        ...
        INNER JOIN
            png_mdsdb.pg_ot_privacy_compliance_controls_c_final a2 
                ON a2.region ='ama' 
                and a2.cdctype ='X' 
GROUP BY a1.inventory_id	)			t on f.row_id=t.inventory_id
where f.control_count_not_ok_c <> t.cnt_notok
