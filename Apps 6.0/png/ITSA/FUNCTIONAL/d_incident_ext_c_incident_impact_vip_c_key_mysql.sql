...
ON CONCAT('IMPACT_VIP~INCIDENT~',upper(src.u_impact_vip))=lkp.row_id
where COALESCE(lkp.row_key,CASE WHEN src.u_impact_vip IS NULL THEN 0 else -1 end)<> trgt.incident_impact_vip_c_key
) temp;