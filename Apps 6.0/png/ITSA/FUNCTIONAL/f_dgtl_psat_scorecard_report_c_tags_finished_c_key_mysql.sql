...
on COALESCE(DATE_FORMAT(SRC.Tags_Finish_Date,'%Y%m%d'),'UNSPECIFIED')= l.row_id
WHERE COALESCE(l.row_key,CASE WHEN SRC.Tags_Finish_Date IS NULL THEN 0 else '-1' end)<>
 TRGT.tags_finished_c_key
) temp