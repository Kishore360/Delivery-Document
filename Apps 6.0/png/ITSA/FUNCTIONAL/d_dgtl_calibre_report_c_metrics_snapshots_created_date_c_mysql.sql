...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_dgtl_calibre_report_c.metrics_snapshots_created_date_c' ELSE 'SUCCESS' END as Message
FROM 
(
...
WHERE  MetricsSnapshotsCreatedAt   <>TRGT.metrics_snapshots_created_date_c
AND SRC.cdctype='X'
) temp;
