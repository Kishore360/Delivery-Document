...
ON CONCAT('LOSS_OF_SALES~INCIDENT~',upper(src.u_loss_of_sales))=lkp.row_id
where COALESCE(lkp.row_key,CASE WHEN src.u_loss_of_sales IS NULL THEN 0 else -1 end)<> trgt.incident_loss_of_sales_c_key
) temp;