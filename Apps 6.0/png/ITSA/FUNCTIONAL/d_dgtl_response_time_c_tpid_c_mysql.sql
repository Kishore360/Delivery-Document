...
WHERE case                 
                    when SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',4),'_',-1) REGEXP '^[0-9]*$' 
					then SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',4),'_',-1)                       
                    when SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',5),'_',-1) REGEXP '^[0-9]*$' 
					then SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',5),'_',-1)                       
                    when SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',6),'_',-1) REGEXP '^[0-9]*$'
					then SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',6),'_',-1)                      
                    when SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',7),'_',-1) REGEXP '^[0-9]*$' 
					then SUBSTRING_INDEX(SUBSTRING_INDEX(SRC1.name,'_',7),'_',-1)                   
                end  <> TRGT.tpid_c
AND SRC.cdctype<>'D'
) temp;
