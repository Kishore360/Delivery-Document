...
on (a11.problem_assignment_group_key = a12.row_key)
union
...
union
select 'ldb.d_internal_contact_director_c a116 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_ag_manager_role_vp_dir_ad_c a12
on (a11.problem_assignment_group_key = a12.row_key)
join ldb.d_internal_contact_director_c a116
on (a12.director_key = a116.row_key)
union
...
on (a19.week_start_date_key = a117.row_key)
union
...
on (a111.year_start_date_key = a118.row_key)
union
...
union
select 'ldb.d_business_service_parent_c a121 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_business_service a17
on (a11.business_service_key = a17.row_key)
join ldb.d_business_service_parent_c a121
on (a17.parent_business_service_c_key = a121.row_key)
union
...
on (a12.associate_director_key = a122.row_key)
union
select 'ldb.d_calendar_fiscal_quarter a123 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_calendar_date_fiscal a13
on (a11.fiscal_date_key = a13.row_key)
join ldb.d_calendar_fiscal_quarter a123
...
union
select 'ldb.d_internal_contact_pg_opened_for_c a125 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_internal_contact_pg_opened_for_c a125
on (a11.pg_opened_for_c_key = a125.row_key)
union
...
union
select 'ldb.d_configuration_item_problem_service_offering_c a129 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_configuration_item_problem_service_offering_c a129
...
union
select 'ldb.d_internal_contact_pg_root_cause_analyst_c a131 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_internal_contact_pg_root_cause_analyst_c a131
on (a11.pg_root_cause_analyst_c_key = a131.row_key)
union
select 'ldb.d_internal_contact_pf_sfa_owner_c a132 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_internal_contact_pf_sfa_owner_c a132
on (a11.pf_sfa_owner_c_key = a132.row_key)
union
...
on (a115.department_key = a137.row_key)
union
...
on (a115.employee_mdm_key = a138.row_key)
union
...
on (a18.pg_other_impact_src_c_key = a140.row_key)
union
...
on (a18.pg_problem_identification_src_c_key = a141.row_key)
union
...
on (a18.root_cause_code_src_c_key = a142.row_key)
union
...
on (a18.state_src_key = a143.row_key)
union
...
on (a18.rca_status_c_key = a144.row_key)
union
...
on (a18.root_cause_sub_code_src_c_key = a145.row_key)
union
...
on (a18.sfa_status_c_key = a146.row_key)
union
...
on (a18.sufficient_impact_to_proceed_c_key = a147.row_key)
union
...
on (a113.business_critical_c_key = a148.row_key)
union
select 'ldb.d_business_service_grand_parent_c a149 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_problem a11
join ldb.d_business_service a17
on (a11.business_service_key = a17.row_key) join ldb.d_business_service_parent_c a121
on (a17.parent_business_service_c_key = a121.row_key)
join ldb.d_business_service_grand_parent_c a149
on (a121.parent_business_service_c_key = a149.row_key)