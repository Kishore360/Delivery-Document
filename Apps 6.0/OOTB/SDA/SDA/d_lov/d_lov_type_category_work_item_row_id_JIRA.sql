...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_type_category_work_item.row_id' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE((CONCAT('TYPE_CATEGORY~WORK_ITEM~',(SRC.referenceName)),'')<>COALESCE(TRGT.row_id,'') 