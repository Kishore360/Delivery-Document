...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_type_category_work_item.src_rowid' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE((CONCAT('TYPE_CATEGORY~WORK_ITEM~',(SRC.referenceName)),'')<>COALESCE(TRGT.src_rowid,'') 