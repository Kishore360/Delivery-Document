...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_severity_work_item.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('SEVERITY~WORK_ITEM~',(SRC.msvsts_common_severity)),'')<>COALESCE(TRGT.row_id,'')