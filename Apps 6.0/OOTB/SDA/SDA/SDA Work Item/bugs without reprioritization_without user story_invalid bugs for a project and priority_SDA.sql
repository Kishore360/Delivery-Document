...
trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'BUG' and D.orphan_bug_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990')) bugs_without_user_story,

trim(to_char(count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'BUG' and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),'FM999,999,999,999,990')) invalid_bugs,

trim(to_char(count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and type.wi_type = 'BUG' and D.reprioritization_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990')) rslvd_bugs_with_reprio
...
order by lower(PROJ.project_name), upper(priority.wi_prio)