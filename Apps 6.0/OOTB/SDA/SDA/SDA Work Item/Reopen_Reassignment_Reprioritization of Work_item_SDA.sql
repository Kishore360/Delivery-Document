...
coalesce(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then F.reopen_count
else 0 end), 0) reopn_cnt,

coalesce(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then F.reassignment_count
else 0 end), 0) reasgn_cnt,

coalesce(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then F.re_prioritization_count
else 0 end), 0) reprio_cnt

/*coalesce(sum(case when status.wi_sts in ('RESOLVED', 'COMPLETED') and cal_rslvd_mon.lagging_count_of_month between 0 and 12 then F.reopen_count 
when status.wi_sts in ('COMPLETED') and cal_cmptd_mon.lagging_count_of_month between 0 and 12 then F.reopen_count 
when status.wi_sts in ('IN PROGRESS') and cal_inprog_mon.lagging_count_of_month between 0 and 12 then F.reopen_count 
WHEN D.proposed_flag = 'Y' or cal_crtd_mon.lagging_count_of_month between 0 and 12 then F.reopen_count
else 0 end), 0) reopn_cnt,

coalesce(sum(case when status.wi_sts in ('RESOLVED', 'COMPLETED') and cal_rslvd_mon.lagging_count_of_month between 0 and 12 then F.reassignment_count
when status.wi_sts in ('COMPLETED') and cal_cmptd_mon.lagging_count_of_month between 0 and 12 then F.reassignment_count 
when status.wi_sts in ('IN PROGRESS') and cal_inprog_mon.lagging_count_of_month between 0 and 12 then F.reassignment_count 
WHEN D.proposed_flag = 'Y' or cal_crtd_mon.lagging_count_of_month between 0 and 12 then F.reassignment_count
else 0 end), 0) reasgn_cnt*/
...
group by PROJ.project_name, D.work_item_number, D.row_id, type.sor_lov_name, D.reopened_flag, D.multiple_assignment_flag
order by lower(PROJ.project_name), D.work_item_number, D.row_id, lower(type.sor_lov_name), upper(D.reopened_flag), upper(D.multiple_assignment_flag)
