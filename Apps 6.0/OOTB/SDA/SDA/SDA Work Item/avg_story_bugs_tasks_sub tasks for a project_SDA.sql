...
case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'EPIC' then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and D.story_to_epic_flag = 'Y' then F.work_item_key else null end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'EPIC' then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_stories_linkd_to_epic,

case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'STORY' then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') and D.work_item_to_story_flag = 'Y' then F.work_item_key else null end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'STORY' then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_tasks_per_story,

case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'SUBTASK' then F.work_item_key else null end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_subtasks_per_task,

case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'STORY' then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'BUG' then F.work_item_key else null end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'STORY' then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_bugs_per_story,

case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(coalesce(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') then F.story_points else 0 end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type in ('TASK') then F.work_item_key else null end),0),'FM999,999,999,999,990.00')) end as avg_weight_per_task,

case when count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'EPIC' then F.work_item_key else null end) = 0 then trim(to_char(0,'FM999,999,999,999,990.00'))
else trim(to_char(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and D.story_to_epic_flag = 'Y' then F.story_points else 0 end) * 1.00/
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and type.wi_type = 'EPIC' then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_sp_per_epic
...
order by lower(PROJ.project_name)