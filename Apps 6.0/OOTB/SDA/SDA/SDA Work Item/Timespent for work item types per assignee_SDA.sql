...
to_char((sum(coalesce(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) then F.time_spent else 0.0 end, 0)) / 3600.00),'FM999,999,999,999,990D00')as time_spent_created, -- based on the opened date
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 3600.00), 'FM999,999,999,999,990D00') as time_spent_closed -- based on the closed date
...
group by INTC_ASGN.full_name, type.row_key, type.sor_lov_name
order by upper(INTC_ASGN.full_name), type.row_key, lower(type.sor_lov_name)
