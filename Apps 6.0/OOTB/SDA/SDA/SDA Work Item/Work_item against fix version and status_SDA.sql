...
TO_CHAR((D.due_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM') due_dt,
D.overdue_flag od_flg,

to_char(coalesce(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 then F.time_until_due_date/86400.00 else 0.00 end), 0),'FM999,999,999,999,990D0') days_until_due,

trim(to_char(count(case when (cal_cmptd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and pv.name in ('UNKNOWN', 'UNSPECIFIED') and status.wi_sts = 'COMPLETED' then F.work_item_key else null end),'FM999,999,999,999,990')) wi_without_fv
...
group by PROJ.project_name, D.work_item_number, type.sor_lov_name, pv.row_key, pv.name, status.dimension_name, resolution.sor_lov_name, D.due_on, D.overdue_flag
order by lower(PROJ.project_name), D.work_item_number, lower(type.sor_lov_name), pv.row_key, upper(pv.name), upper(status.dimension_name), upper(resolution.sor_lov_name), D.due_on, upper(D.overdue_flag)