...
resolution.sor_lov_name wi_resolution,
...
case when count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) = 0 then to_char(0.0,'FM999,999,999,999,990.0')
when count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) < 0 or sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_creation/86400.00 else 0.0 end) < 0
then '('||to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_creation/86400.00 else 0.0 end) * (-1.00))/count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end),'FM999,999,999,999,990D0')||')'
else to_char(coalesce(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_creation/86400.00 else 0.0 end)/count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end),0),'FM999,999,999,999,990.0') end avg_lead_time,


case when (count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.row_key else null end)) = 0 then to_char(0.0,'FM999,999,999,999,990.0')

when sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_in_progress/86400.00 else 0.0 end) < 0 
then '('||trim(to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_in_progress/(-86400.00) else 0.0 end)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.row_key else null end))),'FM999,999,999,999,990.0'))||')'

else to_char(coalesce(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.age_since_in_progress/86400.00 else 0.0 end)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.row_key else null end)),0),'FM999,999,999,999,990.0') end avg_cycle_time
...
group by D.work_item_number, D.row_id, PROJ.project_name, priority.wi_prio, resolution.sor_lov_name, status.dimension_name, status_category.wi_sts_ctgy, wi_rsn.wi_rsn, wi_rsk.wi_rsk, wi_sev.wi_sev
order by D.work_item_number, D.row_id, lower(PROJ.project_name), upper(priority.wi_prio), upper(resolution.sor_lov_name), upper(status.dimension_name), upper(status_category.wi_sts_ctgy), upper(wi_rsn.wi_rsn), upper(wi_rsk.wi_rsk), upper(wi_sev.wi_sev)