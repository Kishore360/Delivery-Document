...
to_char(count(case when cal_cancld_mon.lagging_count_of_month between 0 and 12 and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.row_key else null end),'FM999,999,999,999,990') cancld_wi,
to_char(count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and resolution.wi_res <> 'CANCELLED' and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.row_key else null end),'FM999,999,999,999,990') rslvd_wi,
to_char(count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and resolution.wi_res <> 'CANCELLED' and status.wi_sts in ('COMPLETED') then F.row_key else null end),'FM999,999,999,999,990') cmpltd_wi
