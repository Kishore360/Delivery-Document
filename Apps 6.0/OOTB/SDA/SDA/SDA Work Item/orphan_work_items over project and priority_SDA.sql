...
and D.orphan_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990')) orphan_wi,

trim(to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED')))
and D.orphan_story_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990')) orphan_user_story
