...
to_char(count(distinct case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),'FM999,999,999,999,999') rslvd_wi,

to_char(case when count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) = 0 then 0.0
else round((sum(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_created_to_resolved else 0.0 end)/86400.00) / 
count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),1) end,'FM999,999,999,999,990D0') avg_duration_crtd_rslv,

case when count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) = 0 then to_char(0.0,'FM999,999,999,999,990D0')
when sum(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_work_start_to_resolved else 0.0 end) < 0
then 
'('||to_char(round((sum(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_work_start_to_resolved else 0.0 end)/86400.00) / 
count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),1),'FM999,999,999,999,990D0')||')'
else
to_char(round((sum(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_work_start_to_resolved else 0.0 end)/86400.00) / 
count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),1),'FM999,999,999,999,990D0') 

end avg_duration_inprog_rslv
...
group by PROJ.project_name, priority.wi_prio
order by lower(PROJ.project_name), lower(priority.wi_prio)