...
to_char(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) then coalesce(F.votes_count, 0) else 0 end),'FM999,999,999,999,990') votes
...
group by PROJ.project_name, D.work_item_number, D.row_id, type.sor_lov_name
order by lower(PROJ.project_name), D.work_item_number, D.row_id, lower(type.sor_lov_name)
