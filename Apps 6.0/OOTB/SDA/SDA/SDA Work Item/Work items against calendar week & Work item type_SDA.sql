select  -- F.*,
concat(cal_crtd_week.month_name_abbreviated::text, concat(' '::text, "substring"(cal_crtd_week.week_start_date_key::text, 7, 8))) wk_nm,
cal_crtd.year_id yr_nm,
...
-- to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 and (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) then F.row_key else null end),'FM999,999,999,999,990') opened_wi,
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.proposed_flag = 'Y' then F.row_key else null end),'FM999,999,999,999,990') proposed_wi
-- to_char(count(case when (cal_inprog_mon.lagging_count_of_month between 0 and 12 or status.wi_sts = 'IN PROGRESS') and status.wi_sts in ('IN PROGRESS') and F.work_item_work_start_on_key is not null then F.row_key else null end),'FM999,999,999,999,990') inprog_wi,
-- to_char(count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.row_key else null end),'FM999,999,999,999,990') rslvd_wi,
-- to_char(count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then F.row_key else null end),'FM999,999,999,999,990') cmpltd_wi,
-- to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 OR (status.wi_sts in ('IN PROGRESS') or status.wi_sts in ('PROPOSED'))) and D.reopened_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990') reopnd_wi
...
FROM #DWH_TABLE_SCHEMA.d_calendar_date calendar_week
  WHERE calendar_week.week_start_date_key = calendar_week.row_key AND calendar_week.source_id = 0
) cal_crtd_week
on cal_crtd.week_start_date_key = cal_crtd_week.row_key
left join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
group by cal_crtd_week.week_id, wk_nm, cal_crtd.year_id, type.row_key, type.sor_lov_name
having crtd_wi > 0
order by cal_crtd_week.week_id desc, wk_nm, cal_crtd.year_id, type.row_key, lower(type.sor_lov_name)