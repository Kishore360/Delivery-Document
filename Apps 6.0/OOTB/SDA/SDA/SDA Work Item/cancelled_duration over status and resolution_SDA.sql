...
group by status.dimension_name,status.wi_sts, resolution.sor_lov_name
order by upper(status.dimension_name), upper(status.wi_sts), upper(resolution.sor_lov_name)