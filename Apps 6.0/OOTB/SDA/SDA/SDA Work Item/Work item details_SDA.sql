...
INTC_UPDTBY.full_name chngd_by,
INTC_RSLVDBY.full_name rslvd_nm,
INTC_CLSDBY.full_name clsd_nm,
pv.name erlst_fix_ver,

to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 then coalesce(F.time_spent ,0) else 0.0 end) / 3600.0),'FM999,999,999,999,990D0')as time_spent_created, 
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 3600.0), 'FM999,999,999,999,990D0') as time_spent_closed
...
group by D.work_item_number, D.row_id, type.sor_lov_name, PROJ.project_name, D.short_description, D.description, ITE.row_key, ITE.name, D.labels_list, INTC_ASGN.full_name, INTC_RPT.full_name, INTC_CRT.full_name, INTC_UPDTBY.full_name, INTC_RSLVDBY.full_name, INTC_CLSDBY.full_name, pv.row_key, pv.name
order by D.work_item_number, D.row_id, lower(type.sor_lov_name), lower(PROJ.project_name), upper(D.short_description), upper(D.description), ITE.row_key, upper(ITE.name), upper(D.labels_list), upper(INTC_ASGN.full_name), upper(INTC_RPT.full_name), upper(INTC_CRT.full_name), upper(INTC_UPDTBY.full_name), upper(INTC_RSLVDBY.full_name), upper(INTC_CLSDBY.full_name), pv.row_key, upper(pv.name)