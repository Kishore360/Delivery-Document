select  -- F.*,
D.work_item_number,
type.sor_lov_name wi_type,
INTC_CANBY.full_name cancelled_by,
TO_CHAR((D.cancelled_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM') cancel_date,
status.dimension_name wi_status,
status.wi_sts st_status,
resolution.sor_lov_name wi_resolution,

case when count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12) 
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end) = 0 then trim(to_char(0.00,'FM999,999,999,999,990.00'))
when sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_created_to_cancelled else 0 end) < 0 then 
'('||trim(to_char(sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_created_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),'FM999,999,999,999,990.00'))||')'
else trim(to_char(coalesce(sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_created_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),0),'FM999,999,999,999,990.00')) end as avg_dur_crtd_to_cancl,

case when count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end) = 0 then trim(to_char(0.00,'FM999,999,999,999,990.00'))
when sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_work_start_to_cancelled else 0 end) < 0
then '('||trim(to_char(sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_work_start_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),'FM999,999,999,999,990.00'))||')'
else trim(to_char(coalesce(sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_work_start_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),0),'FM999,999,999,999,990.00')) end as avg_dur_inpr_to_cancl
...
group by D.work_item_number, type.sor_lov_name, INTC_CANBY.full_name, D.cancelled_on, status.dimension_name, status.wi_sts, resolution.sor_lov_name
order by upper(D.work_item_number), lower(type.sor_lov_name), upper(INTC_CANBY.full_name), upper(D.cancelled_on), upper(status.dimension_name), upper(status.wi_sts), upper(resolution.sor_lov_name)