...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and D.reopened_flag = 'Y' then F.row_key else null end),'FM999,999,999,999,990') reopnd_wi,
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and D.overdue_flag = 'Y' then F.row_key else null end),'FM999,999,999,999,990') od_wi
