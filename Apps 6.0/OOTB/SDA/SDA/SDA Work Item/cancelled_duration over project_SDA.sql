...
else trim(to_char(sum(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_created_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_dur_crtd_to_cancl,
...
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.duration_work_start_to_cancelled else 0 end) / 86400.00/
count(case when (cal_cancld_mon.lagging_count_of_month between 0 and 12)
and (resolution.wi_res = 'CANCELLED' or status.wi_sts in ('CANCELLED')) then F.work_item_key else null end),'FM999,999,999,999,990.00')) end as avg_dur_inpr_to_cancl
