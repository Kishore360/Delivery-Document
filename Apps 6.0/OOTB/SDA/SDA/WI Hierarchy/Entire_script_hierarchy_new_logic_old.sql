...
coalesce(chnglog.issuechangelogid, (issue.id - 9999)) as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, coalesce(issue.sourceinstance, 2) child_sourceinstance,
...
select chnglogitems.issuechangelogid as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, coalesce(issue.sourceinstance, 2) child_sourceinstance,
coalesce(issue_from_id.id, issue_from_key.id) parent_id, coalesce(issue_from_id.`key`, SUBSTRING_INDEX(chnglogitems.tostring, ' ', -1)) parent_wi_number, parent_wi_type.dimension_wh_code parent_type, coalesce(issue_from_id.sourceinstance, issue_from_key.sourceinstance, 2) parent_sourceinstance,
case when chnglogitems.field = 'link' then 
...
select chnglogitems.issuechangelogid as log_key, issue_child.id child_id, SUBSTRING_INDEX(COALESCE(chnglogitems.tostring, chnglogitems.fromstring), ' ', -1) child_wi_number, child_wi_type.dimension_wh_code child_type, coalesce(issue_child.sourceinstance, 2) child_sourceinstance, 
issue.id parent_id, issue.`key` parent_wi_number, parent_wi_type.dimension_wh_code parent_type, coalesce(issue.sourceinstance, 2) parent_sourceinstance,
case when chnglogitems.field = 'link' then 
TRIM(SUBSTRING_INDEX(REPLACE(REPLACE(COALESCE(chnglogitems.tostring, chnglogitems.fromstring), 'This issue ', ''), issue_child.`key`, ''), SUBSTRING_INDEX( COALESCE(chnglogitems.tostring, chnglogitems.fromstring), ' ', -1 ), 1))
else chnglogitems.field end as link_type, 
...
select chnglogitems.issuechangelogid as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, coalesce(issue.sourceinstance, 2) child_sourceinstance, 
issue_parent.id parent_id, SUBSTRING_INDEX(COALESCE(chnglogitems.fromstring, ''), ' ', -1) parent_wi_number, parent_wi_type.dimension_wh_code parent_type, coalesce(issue_parent.sourceinstance, 2) parent_sourceinstance,
case when chnglogitems.field = 'link' then 
...
(base_id, log_key, sys_id, lev_0_id, lev_1_id, lev_2_id, lev_3_id, lev_4_id, lev_5_id, lev_6_id, sourceinstance, inserted_on)

select tmp_lev1.sys_id, tmp_lev1.log_key, CONCAT(tmp_lev1.child_id, '~WORK_ITEM') as sys_id, tmp_lev1.child_id lev_0_id, tmp_lev1.parent_id lev_1_id, tmp_lev2.parent_id lev_2_id, tmp_lev3.parent_id lev_3_id, 
