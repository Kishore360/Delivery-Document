...
drop table if exists sda_63_workdb.flat_hier_temp;

create table if not exists sda_63_workdb.flat_hier_temp (
...
insert into sda_63_workdb.flat_hier_temp
(sys_id, log_key, child_id, child_wi_number, child_type, sourceinstance, parent_id, parent_wi_number, parent_type, parent_sourceinstance, parent_precedence, 
link_type, link_val, issue_field, textval, link_created, source_table, inserted_on)

select concat(issue.id, '~', coalesce(chnglogitems.issuechangelogid, 0)) as sys_id,
coalesce(chnglog.issuechangelogid, issue.id) as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, issue.sourceinstance sourceinstance,
issue_parent.id parent_id, issue_parent.`key` parent_wi_number, parent_wi_type.dimension_wh_code parent_type, issue_parent.sourceinstance parent_sourceinstance,
...
from sda_63_mdsdb.issue_final issue
left join sda_63_mdsdb.issue_final issue_parent
on issue.epic_link = issue_parent.`key` and issue.sourceinstance = issue_parent.sourceinstance
left join (
select chnglogitems.issuechangelogid, chnglog.issueid, chnglog.sourceinstance, chnglog.created, chnglogitems.`field`, chnglogitems.`to`
from sda_63_mdsdb.issue_changelog_final chnglog
left join sda_63_mdsdb.issue_changelog_items_final chnglogitems
on chnglogitems.issuechangelogid = chnglog.id and chnglogitems.sourceinstance = chnglog.sourceinstance 
where chnglogitems.`field` in ('Epic Link')
) chnglog
on chnglog.issueid = issue.id and chnglog.sourceinstance = issue.sourceinstance and chnglog.`to` = issue_parent.id
left join sda_63_mdwdb.d_lov_map link_type
on concat('LINK_TYPE~WORK_ITEM~', 'Epic Link') = link_type.src_rowid
and issue_parent.sourceinstance=link_type.source_id
left join sda_63_mdwdb.d_lov_map child_wi_type
on concat('TYPE~WORK_ITEM','~',upper(issue.issuetype_id)) = child_wi_type.src_rowid
and issue.sourceinstance=child_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_type
on concat('TYPE~WORK_ITEM','~',upper(issue_parent.issuetype_id)) = parent_wi_type.src_rowid
and issue_parent.sourceinstance=parent_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_precedence
on concat('TYPE_LINK_PRECEDENCE~WORK_ITEM','~',upper(issue_parent.issuetype_id)) = parent_wi_precedence.src_rowid
...
parent_wi_type.dimension_wh_code parent_type, issue_parent.sourceinstance parent_sourceinstance,
parent_wi_precedence.dimension_wh_code parent_precedence,
...
left join (
select chnglogitems.issuechangelogid, chnglog.issueid, chnglog.sourceinstance, chnglog.created, chnglogitems.`field`, chnglogitems.`to`, chnglogitems.tostring
from sda_63_mdsdb.issue_changelog_final chnglog
left join sda_63_mdsdb.issue_changelog_items_final chnglogitems
on chnglogitems.issuechangelogid = chnglog.id and chnglogitems.sourceinstance = chnglog.sourceinstance 
...
select concat(issue.id, '~', coalesce(chnglogitems.issuechangelogid, 0)) as sys_id,
chnglogitems.issuechangelogid as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, issue.sourceinstance sourceinstance,
...
and issue.sourceinstance=child_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_type
on concat('TYPE~WORK_ITEM','~',upper(coalesce(issue_to_id.issuetype_id, issue_to_key.issuetype_id))) = parent_wi_type.src_rowid
and coalesce(issue_to_id.sourceinstance, issue_to_key.sourceinstance)=parent_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_precedence
on concat('TYPE_LINK_PRECEDENCE~WORK_ITEM','~',upper(coalesce(issue_to_id.issuetype_id, issue_to_key.issuetype_id))) = parent_wi_precedence.src_rowid
...
select concat(issue_child.id, '~', coalesce(chnglogitems.issuechangelogid, 0)) as sys_id,
chnglogitems.issuechangelogid as log_key, issue_child.id child_id, SUBSTRING_INDEX(COALESCE(chnglogitems.tostring, chnglogitems.fromstring), ' ', -1) child_wi_number, child_wi_type.dimension_wh_code child_type, issue_child.sourceinstance sourceinstance, 
...
left join sda_63_mdsdb.issue_changelog_items_final chnglogitems
on chnglogitems.issuechangelogid = chnglog.id and chnglogitems.sourceinstance = chnglog.sourceinstance
left join sda_63_mdsdb.issue_final issue_child
...
and chnglogitems.sourceinstance=link_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_type
on concat('TYPE~WORK_ITEM','~',upper(issue.issuetype_id)) = parent_wi_type.src_rowid
and issue.sourceinstance=parent_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_precedence
on concat('TYPE_LINK_PRECEDENCE~WORK_ITEM','~',upper(issue.issuetype_id)) = parent_wi_precedence.src_rowid
and issue.sourceinstance=parent_wi_precedence.source_id
left join sda_63_mdwdb.d_lov_map child_wi_type
on concat('TYPE~WORK_ITEM','~',upper(issue_child.issuetype_id)) = child_wi_type.src_rowid
...
update sda_63_workdb.flat_hier_temp SRC
join (
...
FROM sda_63_workdb.flat_hier_temp tmp
) base
...
delete SRC 
from sda_63_workdb.flat_hier_temp as SRC
join sda_63_workdb.flat_hier_temp SRC_B
...
drop table if exists sda_63_workdb.parent_removed_temp;

create table if not exists sda_63_workdb.parent_removed_temp (
  log_key int(15) DEFAULT NULL,
...
insert into sda_63_workdb.parent_removed_temp
(log_key, child_id, child_wi_number, child_type, sourceinstance, old_parent_id, old_parent_wi_number, old_parent_type, old_parent_sourceinstance, 
link_type, link_val, issue_field, textval, link_created, inserted_on)

select chnglogitems.issuechangelogid as log_key, issue.id child_id, issue.`key` child_wi_number, child_wi_type.dimension_wh_code child_type, issue.sourceinstance sourceinstance,
...
and issue.sourceinstance=child_wi_type.source_id
left join sda_63_mdwdb.d_lov_map parent_wi_type
on concat('TYPE~WORK_ITEM','~',upper(coalesce(issue_from_id.issuetype_id, issue_from_key.issuetype_id))) = parent_wi_type.src_rowid
...
select chnglogitems.issuechangelogid as log_key, issue_child.id child_id, SUBSTRING_INDEX(COALESCE(chnglogitems.tostring, chnglogitems.fromstring), ' ', -1) child_wi_number, child_wi_type.dimension_wh_code child_type, issue_child.sourceinstance sourceinstance, 
issue.id parent_id, issue.`key` parent_wi_number, parent_wi_type.dimension_wh_code parent_type, issue.sourceinstance parent_sourceinstance,
case when chnglogitems.field = 'link' then 
TRIM(SUBSTRING_INDEX(REPLACE(REPLACE(COALESCE(chnglogitems.tostring, chnglogitems.fromstring), 'This issue ', ''), issue_child.`key`, ''), SUBSTRING_INDEX( COALESCE(chnglogitems.tostring, chnglogitems.fromstring), ' ', -1 ), 1))
else chnglogitems.field end as link_type, 
...
delete hier
from sda_63_workdb.flat_hier_temp hier
join sda_63_workdb.parent_removed_temp removed_parent
...
delete SRC
from sda_63_workdb.flat_hier_temp SRC
join (
...
drop table if exists sda_63_workdb.flat_hier_temp_bkp;

create table sda_63_workdb.flat_hier_temp_bkp select * from sda_63_workdb.flat_hier_temp;
...
from sda_63_workdb.flat_hier_temp
...
update sda_63_workdb.flat_hier_temp main
join 
(
select log_key, child_id, invalid_flag from sda_63_workdb.flat_hier_temp base 
where (child_id, parent_preference_id) in 
(select child_id, min(parent_preference_id) from sda_63_workdb.flat_hier_temp where invalid_flag = 'Y' group by child_id)
) SQ
...
alter table sda_63_workdb.flat_hier_temp
ADD PRIMARY KEY (sys_id, sourceinstance, source_table), 
ADD INDEX first_idx (child_id,sourceinstance),
ADD INDEX second_idx (parent_id,parent_sourceinstance);

drop table if exists sda_63_workdb.hier_base;

create table if not exists sda_63_workdb.hier_base (
  log_key int(15) DEFAULT NULL,
  sys_id varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
...
-- Lev 0 & 1 -- 
insert into sda_63_workdb.hier_base
...
-- Lev 2 -- 
update sda_63_workdb.hier_base bs
join sda_63_workdb.flat_hier_temp tmp
...
update sda_63_workdb.hier_base hier
join (
