...
set hier_base.is_cyclic_flag = case when SQ.cycle_level > 0 then 'Y' else 'N' end, hier_base.cyclic_level = SQ.cycle_level, base.from_cyclic_level = 0;
...
on base.base_id = SQ.base_id and base.sourceinstance = SQ.sourceinstance
set base.is_cyclic_flag = case when base.is_cyclic_flag = 'Y' then 'Y' when SQ.cycle_level > 0 then 'Y' else 'N' end, 
base.cyclic_level = case when base.cyclic_level is null then SQ.cycle_level when SQ.cycle_level < base.cyclic_level then SQ.cycle_level else base.cyclic_level end, base.from_cyclic_level = 4;
