...
from  #STG_TABLE_SCHEMA.issue_activity_temp tmp
 ...
 on lov.row_id = concat('STATUS~WORK_ITEM~', IF(TRIM(tmp.from)=TRIM(tmp.to) OR LENGTH(tmp.to)=0,NULL,TRIM(SUBSTRING_INDEX(tmp.to, ',', -1)))) and lov.dimension_class = 'STATUS~WORK_ITEM' and tmp.issue_changelog_sourceinstance = lov.source_id
...
from  #STG_TABLE_SCHEMA.issue_activity_temp tmp
 ...
 OR LENGTH(TRIM(SUBSTRING_INDEX(tmp.to, ',', -1)))=0,null,TRIM(SUBSTRING_INDEX(tmp.to, ',', -1))) and tmp.issue_changelog_sourceinstance = ITE.source_id
