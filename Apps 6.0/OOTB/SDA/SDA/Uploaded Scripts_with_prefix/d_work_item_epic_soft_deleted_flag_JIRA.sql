SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item_epic.soft_deleted_flag' ELSE 'SUCCESS' END as Message
...
where LKP.dimension_wh_code = 'EPIC') SRC
 ...
 where SRC.key is not null and coalesce(case when SRC.cdctype = 'D' then 'Y' else 'N' end,'N')<>coalesce(TRGT.soft_deleted_flag,'');