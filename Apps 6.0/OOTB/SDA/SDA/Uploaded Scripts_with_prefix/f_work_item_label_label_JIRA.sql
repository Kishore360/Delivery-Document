...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_label.label' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(SRC.label,'') <> COALESCE(TRGT.label,'');