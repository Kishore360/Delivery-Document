SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_iteration.iteration_key' ELSE 'SUCCESS' END as Message
 from 
(
 ...
 where coalesce(LKP.row_key,case when concat(SRC.sprint,'~',SRC.board) is null then 0 else -1 end)<>coalesce(TRGT.iteration_key,'')