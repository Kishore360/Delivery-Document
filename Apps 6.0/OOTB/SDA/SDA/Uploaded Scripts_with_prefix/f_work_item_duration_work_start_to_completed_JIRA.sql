...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.resolution_id))=RES.src_rowid and SRC.sourceinstance = RES.source_id
 ...
 when DM.dimension_wh_code in ('COMPLETED') and strt_dt.work_start_on is not null then
 timestampdiff(second,coalesce(strt_dt.work_start_on,0),coalesce(compltd_dt.work_complete_on,0)) else 0 end,0) <>coalesce(TRGT.duration_work_start_to_completed,'')