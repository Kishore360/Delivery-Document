SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 on LM.row_id = concat('STATUS~WORK_ITEM~', SRC2.from) and LM.dimension_class = 'STATUS~WORK_ITEM'
 
 where SRC2.field = 'status' and coalesce(LM.wh_key, case when SRC2.fromstring is null then 0 else -1 end) <>coalesce(TRGT.task_attribute_wh_old_value_key,'');