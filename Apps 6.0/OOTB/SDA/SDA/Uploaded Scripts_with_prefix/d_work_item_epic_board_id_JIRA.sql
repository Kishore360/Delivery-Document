SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item_epic.board_id' ELSE 'SUCCESS' END as Message
...
where LKP.dimension_wh_code = 'EPIC') SRC
 ...
 WHERE SRC.key is not null and COALESCE(SRC.boardid,'' )<> COALESCE(TRGT.board_id ,'')