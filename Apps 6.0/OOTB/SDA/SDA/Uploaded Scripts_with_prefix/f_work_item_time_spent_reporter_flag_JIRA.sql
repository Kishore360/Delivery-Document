...
/*select 
 COALESCE(CASE WHEN SRC.author_key = SRC.reporter_id THEN 'Y' ELSE 'N' END,'') , COALESCE(TRGT.reporter_flag,'')
...
where COALESCE(CASE WHEN SRC.author_key = SRC.reporter_id THEN 'Y' ELSE 'N' END,'') <> COALESCE(TRGT.reporter_flag,'');
