...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.created_by' ELSE 'SUCCESS' END as Message from
/*
select 
COALESCE(SRC.author_key,'') , COALESCE(TRGT.created_by,'')
...
worklog.author_key as author_key,
...
where COALESCE(SRC.author_key,'') <> COALESCE(TRGT.created_by,'');
