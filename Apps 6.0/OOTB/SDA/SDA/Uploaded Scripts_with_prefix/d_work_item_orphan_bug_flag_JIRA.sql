...
select SRC.id, SRC.sourceinstance, group_concat(case when typ.dimension_wh_code = 'BUG' then 
  case when isu_lnk.issueid is null or (isu_lnk.inward_issue_type not in ('STORY','EPIC') or isu_lnk.outward_issue_type not in ('STORY','EPIC')) then 'Y' 
  else 'N' end
...
where coalesce(IF(SQ.orphan_bug_flag like '%N%', 'N', 'Y'), 'N')<>coalesce(TRGT.orphan_bug_flag,'');