SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 where SRC2.field = 'status' and coalesce(CASE WHEN SRC2.field = 'assignee' THEN CONVERT_TZ(SRC.created, '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>') ELSE NULL END,'') <> coalesce(TRGT.assigned_to_created_on,'');