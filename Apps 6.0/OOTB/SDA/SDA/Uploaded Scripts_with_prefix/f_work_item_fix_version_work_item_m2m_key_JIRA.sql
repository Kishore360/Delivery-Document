SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_fix_version.work_item_m2m_key' ELSE 'SUCCESS' END as Message
...
case when LM.dimension_wh_code in ('COMPLETED') then case when (compltd_dt.work_complete_on >= LKP_SRC.resolutiondate or LKP_SRC.resolutiondate is null)
and compltd_dt.work_complete_on is not null then compltd_dt.work_complete_on
else coalesce(LKP_SRC.resolutiondate, LKP_SRC.created) end
when LM.dimension_wh_code in ('CANCELLED') then cancld_dt.cancelled_on
