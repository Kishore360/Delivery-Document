SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 where SRC2.field = 'status' and coalesce(CASE WHEN SRC2.field = 'assignee' THEN ADDDATE(CONVERT_TZ(SRC.created, '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), INTERVAL TRGT.primary_duration SECOND) ELSE NULL END,0) <> coalesce(TRGT.assigned_to_ended_on,'')