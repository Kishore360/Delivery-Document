...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.row_id' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(concat(SRC.worklog_id,'~', SRC.IssueId) ,'') <> COALESCE(TRGT.row_id,'');