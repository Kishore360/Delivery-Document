SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_iteration.is_closed_flag' ELSE 'SUCCESS' END as Message
 from (
...
where coalesce(case when SRC.state like '%closed%' then 'Y' else 'N' end,'N') != coalesce(TRGT.is_closed_flag,'');