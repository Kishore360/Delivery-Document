...
coalesce(case when typ.dimension_wh_code = 'STORY' then 
  case when isu_lnk.issueid is not null and (isu_lnk.inward_issue_type in ('EPIC') 
  or isu_lnk.outward_issue_type in ('EPIC')) 
  or SRC.epic_link is not null then 'Y' else 'N' end
else 'N' end, 'N') sr,
...
select SRC.id, SRC.sourceinstance, group_concat(case when typ.dimension_wh_code = 'STORY' then 
  ...
  or SRC.epic_link is not null) then 'Y' else 'N' end else 'N' end) as story_to_epic_flag
