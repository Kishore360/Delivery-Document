...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.assignee_flag' ELSE 'SUCCESS' END as Message
/*
select 
COALESCE(CASE WHEN SRC.author_key = SRC.assignee_id THEN 'Y' ELSE 'N' END,'') ,COALESCE(TRGT.assignee_flag,'')*/
...
where COALESCE(CASE WHEN SRC.author_key = SRC.assignee_id THEN 'Y' ELSE 'N' END,'') <> COALESCE(TRGT.assignee_flag,'');