...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.time_spent_sec' ELSE 'SUCCESS' END as Message
FROM 
/*select COALESCE(SRC.timespent_sec,'') ,COALESCE(TRGT.time_spent_sec,'') */
...
where COALESCE(SRC.timespent_sec,'') <>COALESCE(TRGT.time_spent_sec,'')