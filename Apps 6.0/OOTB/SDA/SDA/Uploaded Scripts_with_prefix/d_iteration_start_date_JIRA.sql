SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_iteration.start_date' ELSE 'SUCCESS' END as Message
 from (
...
and SRC.sourceinstance=TRGT.source_id
 ...
 where coalesce(convert_tz(SRC.startDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.start_date,'')