...
actv_sprnt.sprintid, clsd_sprnt.sprint_count,
TRGT.iteration_count, (case when actv_sprnt.sprintid is null then coalesce(clsd_sprnt.sprint_count,0) else 1 + coalesce(clsd_sprnt.sprint_count,0) end) ITR_CN*/
...
where (case when actv_sprnt.sprintid is null then coalesce(clsd_sprnt.sprint_count,0) else 1 + coalesce(clsd_sprnt.sprint_count,0) end) <> coalesce(TRGT.iteration_count,'');