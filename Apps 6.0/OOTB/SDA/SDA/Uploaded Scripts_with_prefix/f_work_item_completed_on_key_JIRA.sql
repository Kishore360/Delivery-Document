...
on (LKP_COMPLETED.row_id  = date_format(convert_tz(compltd_dt.work_complete_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') and LKP_COMPLETED.source_id=0)
 ...
 on concat(SRC.id, '')=D.row_id and SRC.sourceinstance=D.source_id
  
 left join #DWH_TABLE_SCHEMA.d_lov_map DM
 on D.work_item_status_src_key=DM.src_key
  
 where coalesce(case when DM.dimension_wh_code='COMPLETED' then LKP_COMPLETED.row_key else 0 end,0) <>coalesce(TRGT.completed_on_key,0)