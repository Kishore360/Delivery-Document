...
WHERE tmp.field in ('STATUS', 'ITERATION', 'STORY POINTS', 'TIMEORIGINALESTIMATE', 'BUSINESS VALUE') and COALESCE(case when tmp.issue_changelog_cdctype = 'D' then 'Y' else 'N' end,'Y')<> COALESCE(TRGT.soft_deleted_flag,'');