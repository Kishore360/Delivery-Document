...
issue.priority_id, round(EXT.customfield_10005,10), D.story_points, issue.created, issue.creatorRef, issue.updated, issue.creatorRef, issue.cdctype, 
...
where chnglog.field in ('STATUS', 'ASSIGNEE', 'PRIORITY', 'ITERATION', 'DUEDATE', 'SUMMARY', 'PROJECT', 'BUSINESS VALUE', 'STORY POINTS', 'RESOLUTION', 'FIX VERSION') and
...
where chnglog.field in ('STATUS', 'ASSIGNEE', 'PRIORITY', 'ITERATION', 'DUEDATE', 'SUMMARY', 'PROJECT', 'BUSINESS VALUE', 'STORY POINTS', 'RESOLUTION') and chnglog.`from` <> chnglog.`to`
) base
...
where chnglog.field in ('FIX VERSION') and chnglog.`from` <> chnglog.`to`
) base
...
on SRC.n_key = cal.row_key
where chnglog.field in ('STATUS', 'ASSIGNEE', 'PRIORITY', 'ITERATION', 'DUEDATE', 'SUMMARY', 'PROJECT', 'BUSINESS VALUE', 'STORY POINTS', 'RESOLUTION', 'FIX VERSION') and
date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') > date_format(cal.week_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
...
where chnglog.field in ('STATUS', 'ASSIGNEE', 'PRIORITY', 'ITERATION', 'DUEDATE', 'SUMMARY', 'PROJECT', 'BUSINESS VALUE', 'STORY POINTS', 'RESOLUTION', 'FIX VERSION');
...
where LKP2.field in ('BUSINESS VALUE') and coalesce(LKP2.next_from, '') <> coalesce(SRC.business_value, '');
...
where LKP2.field in ('STORY POINTS') and SRC.story_points <> LKP2.next_from;
...
where LKP2.field in ('SUMMARY') and LKP2.next_from <> SRC.summary;
...
on concat('STATUS~WORK_ITEM~',upper(chnglog.`to`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code='COMPLETED' and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
...
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code IN ('PROPOSED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
...
set SRC.last_updated_on = coalesce(convert_tz(tmp.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), D.work_item_updated_on), SRC.changed_by = updated_by.authorRef, SRC.updated_on = current_timestamp;
...
on concat('STATUS~WORK_ITEM~',upper(chnglog.`to`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code IN ('RESOLVED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_res
...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.work_item_resolution)) = LM_RES.src_rowid
and SRC.sourceinstance=LM_RES.source_id
where lower(chnglog.field) = 'STATUS' and (LM.dimension_wh_code = 'CANCELLED' or LM_RES.dimension_wh_code in ('CANCELLED')) and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_cncl
...
on SRC.n_key = cal.row_key
where chnglog.field in ('STATUS', 'ASSIGNEE', 'PRIORITY', 'ITERATION', 'DUEDATE', 'SUMMARY', 'PROJECT', 'BUSINESS VALUE', 'STORY POINTS', 'RESOLUTION', 'FIX VERSION') and
date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
...
on SRC.issueid = LKP2.issueid and SRC.sourceinstance = LKP2.sourceinstance and SRC.n_key = LKP2.n_key
set SRC.business_value = coalesce(LKP2.next_from, SRC.business_value), SRC.updated_on = current_timestamp
where LKP2.field in ('BUSINESS VALUE') and SRC.business_value <> LKP2.next_from;
...
on SRC.issueid = LKP2.issueid and SRC.sourceinstance = LKP2.sourceinstance and SRC.n_key = LKP2.n_key
set SRC.summary = coalesce(LKP2.next_from, SRC.summary), SRC.updated_on = current_timestamp
where LKP2.field in ('SUMMARY') and SRC.summary <> LKP2.next_from;
...
on concat('STATUS~WORK_ITEM~',upper(chnglog.`to`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code='COMPLETED' and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
...
on concat('STATUS~WORK_ITEM~',upper(chnglog.`from`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code IN ('PROPOSED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
...
on concat('STATUS~WORK_ITEM~',upper(chnglog.`to`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglog.field) = 'STATUS' and LM.dimension_wh_code IN ('RESOLVED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_res
on tmp_res.issueid = SRC.issueid and tmp_res.sourceinstance = SRC.sourceinstance and tmp_res.n_key = SRC.n_key
...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.work_item_resolution)) = LM_RES.src_rowid
and SRC.sourceinstance=LM_RES.source_id
where lower(chnglog.field) = 'STATUS' and (LM.dimension_wh_code = 'CANCELLED' or LM_RES.dimension_wh_code in ('CANCELLED')) and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and chnglog.`from` <> chnglog.`to`
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_cncl
on tmp_cncl.issueid = SRC.issueid and tmp_cncl.sourceinstance = SRC.sourceinstance and tmp_cncl.n_key = SRC.n_key
