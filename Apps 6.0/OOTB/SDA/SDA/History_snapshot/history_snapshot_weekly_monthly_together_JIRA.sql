...
issue.PriorityId, round(issue.business_value,10), issue.story_points, issue.created, issue.CreatorId, issue.updated, issue.CreatorId, issue.cdctype, 
...
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, chnglogitems.field, chnglogitems.issuechangelogid as max_chnglog_id, min(coalesce(chnglogitems.`to`, '')) as min_chnglog_id
from #STG_TABLE_SCHEMA.history_snapshot_weekly_temp SRC
left join #MDS_TABLE_SCHEMA.issue_changelog_final chnglog
on SRC.issueid=chnglog.issueid
and SRC.sourceinstance=chnglog.sourceinstance
...
on chnglogitems.issuechangelogid = chnglog.id and chnglogitems.sourceinstance = chnglog.sourceinstance
join (
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, chnglogitems.field, chnglogitems.issuechangelogid nxt_chnglog_id, min(chnglogitems.from) as nxt_min_id
from #STG_TABLE_SCHEMA.history_snapshot_weekly_temp SRC
left join #MDS_TABLE_SCHEMA.issue_changelog_final chnglog
on SRC.issueid=chnglog.issueid
and SRC.sourceinstance=chnglog.sourceinstance
...
where lower(LKP2.field) in ('summary') and LKP2.next_fromstring <> SRC.summary;
...
and LKP.issue_changelog_sourceinstance=LM.source_id
...
left join #DWH_TABLE_SCHEMA.d_calendar_date cal
on SRC.n_key = cal.row_key
where lower(LKP.field) in ('status', 'assignee', 'priority', 'sprint', 'duedate', 'summary', 'project', 'business value', 'story points', 'resolution', 'fix version') and 
(coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'') or coalesce(LKP.fromstring,'') <> coalesce(LKP.tostring,'')) and
date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
group by SRC.issueid, SRC.sourceinstance, SRC.n_key
union all 
...
convert_tz(SRC.changed_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>') as changed_on,
...
date_format(SRC.n_date, '%Y-%m-%d') as n_date,
SRC.n_key as n_key,
...
on SRC.n_key = cal.row_key
where lower(chnglogitems.field) in ('status', 'assignee', 'priority', 'sprint', 'duedate', 'project', 'resolution') and
date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and coalesce(chnglogitems.`from`, '') <> coalesce(chnglogitems.`to`, '')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, chnglogitems.field

union all

select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, chnglogitems.field, chnglogitems.issuechangelogid as max_chnglog_id, min(chnglogitems.to) as min_chnglog_id
from #STG_TABLE_SCHEMA.history_snapshot_monthly_temp SRC
left join #MDS_TABLE_SCHEMA.issue_changelog_final chnglog
on SRC.issueid=chnglog.issueid
and SRC.sourceinstance=chnglog.sourceinstance
...
on SRC.n_key = cal.row_key
where lower(chnglogitems.field) in ('business value', 'story points', 'summary') and
date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and coalesce(chnglogitems.fromstring, '') <> coalesce(chnglogitems.tostring, '')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, chnglogitems.field;
...
on chnglogitems.issuechangelogid = chnglog.id and chnglogitems.sourceinstance = chnglog.sourceinstance
join (
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, chnglogitems.field, chnglogitems.issuechangelogid nxt_chnglog_id, min(chnglogitems.from) as nxt_min_id
from #STG_TABLE_SCHEMA.history_snapshot_monthly_temp SRC
left join #MDS_TABLE_SCHEMA.issue_changelog_final chnglog
on SRC.issueid=chnglog.issueid
and SRC.sourceinstance=chnglog.sourceinstance
...
on SRC.issueid = LKP2.issueid and SRC.sourceinstance = LKP2.sourceinstance and SRC.n_key = LKP2.n_key
set SRC.business_value = coalesce(LKP2.next_fromstring, SRC.business_value), SRC.updated_on = current_timestamp
where lower(LKP2.field) in ('business value') and SRC.business_value <> LKP2.next_fromstring;
...
on SRC.issueid = LKP2.issueid and SRC.sourceinstance = LKP2.sourceinstance and SRC.n_key = LKP2.n_key
set SRC.summary = coalesce(LKP2.next_fromstring, SRC.summary), SRC.updated_on = current_timestamp
where lower(LKP2.field) in ('summary') and SRC.summary <> LKP2.next_fromstring;
...
where lower(LKP.field) in ('status', 'assignee', 'priority', 'sprint', 'duedate', 'summary', 'project', 'business value', 'story points', 'resolution', 'fix version') and 
(coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'') or coalesce(LKP.fromstring,'') <> coalesce(LKP.tostring,'')) and
date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d')
group by SRC.issueid, SRC.sourceinstance, SRC.n_key
union all 
select SRC.issueid, SRC.sourceinstance, SRC.n_key, comments.authorkey, max(convert_tz(comments.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')) as last_updated_on
from #STG_TABLE_SCHEMA.history_snapshot_monthly_temp SRC
