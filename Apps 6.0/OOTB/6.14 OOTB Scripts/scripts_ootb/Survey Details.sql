...
to_char(SUR_INST.taken_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') inst_takn,
coalesce(D.survey_instance_key, 0) sur_inst_num,
D.survey_type typ,
to_char(F.requested_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') reqd_on,
QUES.question quest,
QUES_TYPE.ques_type qu_typ,
F.response resp,
LOC.location_name loc_name,
SUR_STATE.state st,
round(avg(F.response_value*1.00),2) avg_inc_csat_scr
from 
...
a.row_id sur_id, i.row_id inc_id
...
UNION 
 ...
 a.state_src_key state_key, a.row_id sur_id, i.row_id inc_id
   FROM #DWH_TABLE_SCHEMA.f_task_survey a
   JOIN #DWH_TABLE_SCHEMA.f_incident i ON a.incident_key = i.incident_key
...
on F.opened_by_key = DIC_INC.row_key
left join
(
 SELECT lov.*, wh_lov.dimension_code AS inc_prio_code, wh_lov.dimension_name AS inc_prio
   FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key
   JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON wh_lov.row_key = br.wh_key AND br.dimension_class::text = 'PRIORITY~INCIDENT'::text
UNION 
 SELECT lov.*, lov.dimension_code AS inc_prio_code, lov.dimension_name AS inc_prio
   FROM #DWH_TABLE_SCHEMA.d_lov lov
  WHERE lov.row_key = -1 OR lov.row_key = 0
) INC_PRIO
on INC_PRIO.row_key = F.priority_src_key
left join
(
select *
FROM #DWH_TABLE_SCHEMA.d_configuration_item
) INC_CI
on F.configuration_item_key = INC_CI.row_key
where cal_mon.lagging_count_of_month between 0 and 12
group by sur_nm, inst_takn, sur_inst_num, typ, reqd_on, QUES.row_id, quest, qu_typ, resp, loc_name, st
order by sur_nm, inst_takn, sur_inst_num, typ, reqd_on, quest, qu_typ, resp, loc_name, st