...
on TRGT.project_key=LKP.row_key
and TRGT.source_id=LKP.source_id
WHERE COALESCE(LKP.total_actual_loc_cost,CASE WHEN LKP.project_key IS NULL THEN 0 else '-1' end)<> 
COALESCE(TRGT.project_actual_loc_cost,0)
