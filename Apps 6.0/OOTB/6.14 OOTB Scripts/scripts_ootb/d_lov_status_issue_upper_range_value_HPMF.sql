...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.upper_range_value' ELSE 'SUCCESS' END as Message
...
where coalesce(TRGT.upper_range_value,'')<>null;