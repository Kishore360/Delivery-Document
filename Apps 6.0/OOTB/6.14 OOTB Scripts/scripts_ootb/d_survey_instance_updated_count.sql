...
case when count(1) > 0 then 'MDS to DWH data validation failed for updated_count between survey_instance_final to d_survey_instance' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
