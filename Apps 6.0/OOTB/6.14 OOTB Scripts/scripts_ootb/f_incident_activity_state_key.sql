...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_incident_activity.state_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id, COALESCE(TGT.task_attribute_wh_new_value_key,''), COALESCE(TGT.state_key,'')
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
where TGT.task_attribute_wh_name = 'state' and TGT.incident_key <> -1
and COALESCE(TGT.task_attribute_wh_new_value_key,'') <> COALESCE(TGT.state_key,'')
union all

select TGT.row_id, 
COALESCE(LKP.task_attribute_wh_new_value_key,'') sr, COALESCE(TGT.state_key,'') tg
from 
(
select TGT.row_id, TGT.incident_key, TGT.source_id, TGT.primary_sequence_id, max(SQ.created_on) max_state_created_on
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
left join (
select TGT.row_id, TGT.incident_key, TGT.source_id, TGT.task_attribute_wh_new_value_key, TGT.created_on, TGT.sequence_id
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
where TGT.task_attribute_wh_name = 'state'
) SQ
on TGT.incident_key = SQ.incident_key and TGT.source_id = SQ.source_id and (TGT.created_on >= SQ.created_on or (TGT.created_on = SQ.created_on and TGT.sequence_id > SQ.sequence_id)) and TGT.task_attribute_wh_name = 'assigned_to'
...
where TGT.task_attribute_wh_name = 'assigned_to' and TGT.incident_key <> -1 and TGT.soft_deleted_flag = 'N' and INC.soft_deleted_flag = 'N'
and COALESCE(LKP.task_attribute_wh_new_value_key,'') <> COALESCE(TGT.state_key,'')
...
where TGT.task_attribute_wh_name = 'assignment_group' and TGT.soft_deleted_flag = 'N'
group by TGT.row_id, TGT.incident_key, TGT.source_id, TGT.primary_sequence_id
) base 
left join (
select TGT.row_id, TGT.incident_key, TGT.source_id, TGT.task_attribute_wh_new_value_key, TGT.created_on
