...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_a_change_request_smpo_dppc.successful_change_failure_requests' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.successful_change_failure_requests, '')<> COALESCE(TRGT.successful_change_failure_requests ,'');