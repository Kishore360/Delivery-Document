...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_type.dimension_name' ELSE 'SUCCESS' END as Message 
FROM
...
THEN  CONCAT(SRC.configuration_scriptType,' Test') END,'') <> COALESCE(TRGT.dimension_name,'')
