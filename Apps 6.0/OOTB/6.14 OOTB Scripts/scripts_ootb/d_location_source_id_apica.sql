...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.source_id' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.source_id,'')<>coalesce(TRGT.source_id,'') -- collate utf8_unicode_ci;
