...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project_risk.row_count' ELSE 'SUCCESS' END as Message
from (select * from 
...
from #DWH_TABLE_SCHEMA.f_project_risk TRGT where TRGT.soft_deleted_flag<>'Y')count_trgt
