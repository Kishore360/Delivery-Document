...
WHERE coalesce(substr(CONVERT_TZ(coalesce(SRC.scheduled_start_date,SRC.actual_start_date,SRC.submit_date),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),1,19),'')
<> COALESCE(TRGT.change_start_on ,'')
