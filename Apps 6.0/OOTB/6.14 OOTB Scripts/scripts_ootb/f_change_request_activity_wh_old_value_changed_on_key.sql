...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.wh_old_value_changed_on_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(null, '') <> COALESCE(TGT.wh_old_value_changed_on_key,'')
union all
select TGT.row_id
