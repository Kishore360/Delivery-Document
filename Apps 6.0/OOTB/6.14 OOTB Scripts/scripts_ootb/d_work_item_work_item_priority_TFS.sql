SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_priority' ELSE 'SUCCESS' END as Message
...
coalesce(SRC.microsoft_vsts_common_priority,'')<>coalesce(TRGT.work_item_priority,'')