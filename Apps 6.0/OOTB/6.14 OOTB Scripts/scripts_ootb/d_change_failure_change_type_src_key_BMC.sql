...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON (concat('TYPE~','CHANGE_REQUEST~',upper(SRC.change_timing)) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.change_timing IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.change_type_src_key ,'')
