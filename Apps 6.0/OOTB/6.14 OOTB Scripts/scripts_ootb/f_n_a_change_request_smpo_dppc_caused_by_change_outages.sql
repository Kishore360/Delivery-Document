...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_a_change_request_smpo_dppc.caused_by_change_outages' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.caused_by_change_outages, '')<> COALESCE(TRGT.caused_by_change_outages ,'');