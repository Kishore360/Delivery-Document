SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.PLANNED_START_ON' ELSE 'SUCCESS' END as Message
...
where COALESCE(CONVERT_TZ(SRC.SCHEDULE_START,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> COALESCE(TRGT.PLANNED_START_ON,'')