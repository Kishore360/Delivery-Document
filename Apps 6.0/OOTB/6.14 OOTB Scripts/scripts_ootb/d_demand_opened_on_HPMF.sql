...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_demand.opened_on' ELSE 'SUCCESS' END as Message
...
where COALESCE(convert_tz(SRC.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.opened_on,'');