...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_SET_ID as project_key, time_sheet.WORK_ITEM_SET_ID as work_item_set_id,sum(act_effort.total_effort) total_effort,state,time_sheet.created_by
from
