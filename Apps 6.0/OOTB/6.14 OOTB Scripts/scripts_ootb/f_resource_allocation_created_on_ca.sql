SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for f_resource_allocation.created_on'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
        prassignment.sourceinstance AS source_id,
			...
			prassignment.created_date as pr_created_date
    FROM
...
WHERE
    trg.row_id LIKE '%ASSIGNED%'
        AND COALESCE(CONVERT_TZ(src.pr_created_date,'@#TENANT_SSI_TIME_ZONE@#','@#DW_TARGET_TIME_ZONE@#'), '') <> COALESCE(trg.created_on, '');