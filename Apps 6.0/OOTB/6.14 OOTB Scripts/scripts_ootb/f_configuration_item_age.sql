SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 where
coalesce(CASE when LKP.purchase_date IS NULL THEN 0
when lov_map.dimension_wh_code = 'NON-OPERATIONAL' then 0  ELSE 
TIMESTAMPDIFF(day, coalesce(convert_tz(LKP.purchase_date, '<<DW_TARGET_TIME_ZONE>>', '<<TENANT_SSI_TIME_ZONE>>'),0), 
convert_tz(df.lastupdated, '<<DW_TARGET_TIME_ZONE>>', '<<TENANT_SSI_TIME_ZONE>>')) end,0) <> coalesce(TRGT.age,'')