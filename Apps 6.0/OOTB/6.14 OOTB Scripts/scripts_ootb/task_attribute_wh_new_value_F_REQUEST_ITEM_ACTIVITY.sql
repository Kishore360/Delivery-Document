...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_item_activity.task_attribute_wh_new_value' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (F.request_item_key=TGT2.request_item_key AND TGT2.task_attribute_wh_name = 'state' AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1)
where TGT.primary_sequence_id = 0 AND TGT.task_attribute_wh_name = 'state' and D.soft_deleted_flag = 'N'
...
on SRC.documentkey = F.row_id
left join #DWH_TABLE_SCHEMA.f_request_item_activity TGT2
on (F.request_item_key=TGT2.request_item_key AND TGT2.task_attribute_wh_name = 'stage' AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1)
where TGT.primary_sequence_id = 0 AND TGT.task_attribute_wh_name = 'stage' and D.soft_deleted_flag = 'N'
and COALESCE(null,'') <> COALESCE(TGT.task_attribute_wh_new_value,'')
