...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.postal_code' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.zip,''),coalesce(TRGT.postal_code,'')
...
where customer=1 and coalesce(SRC.zip,'')<>coalesce(TRGT.postal_code,'');