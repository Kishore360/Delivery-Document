...
round((case when sum(F.calls_offered)=0 then NULL else sum(F.abandoned_calls)*100.0/sum(F.Calls_Offered) end),1) Abandoned_calls_offered,
case when sum(F.calls_offered)=0 then NULL else sum(F.abandoned_calls)/sum(F.Calls_Offered) end Answered_incoming_calls,
round(case when sum (F.calls_offered)=0 then NULL else sum(F.acd_calls)*1.00/sum (F.calls_offered) end)Answered_calls_offered,
case when sum(F.Calls_Offered)=0 then '('||trim(to_char(round((sum(F.Target_Percent)),0), '9,999,999,990'))||')'
 when round(((sum(F.target_Met_Acd_Calls)*1.00 /sum(F.Calls_Offered)) - sum(F.Target_Percent)),0) < 0
then '('||trim(to_char(round(((sum(F.target_Met_Acd_Calls)*1.00 /sum(F.Calls_Offered)) - sum(F.Target_Percent)),0)*(-1), '9,999,999,990'))||')'
else trim(to_char(round(((sum(F.target_Met_Acd_Calls)*1.00 /sum(F.Calls_Offered)) - sum(F.Target_Percent)),0), '9,999,999,990')) end  Actual_relative_to_target
...
order by CAL_MONTH.month_id asc,CAL_MONTH.month_name_abbreviated,D.row_key asc,D.name,CAL_MONTH.year_name