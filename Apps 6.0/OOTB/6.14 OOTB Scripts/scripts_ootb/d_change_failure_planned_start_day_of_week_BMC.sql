...
WHERE coalesce(case when SRC.Scheduled_Start_Date is null and LM.dimension_wh_code = 'OPEN' then 'UNSPECIFIED'
when LM.dimension_wh_code =  'OPEN' then (DAYNAME(CONVERT_TZ(SRC.Scheduled_Start_Date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')))end,'')
<> COALESCE(TRGT.planned_start_day_of_week ,'')
