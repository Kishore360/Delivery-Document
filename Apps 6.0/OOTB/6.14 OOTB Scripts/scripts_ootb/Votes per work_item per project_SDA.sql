...
to_char(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) then coalesce(F.votes_count, 0) else 0 end),'FM999,999,999,999,990') votes
...
group by PROJ.project_name, D.work_item_number, D.row_id, type.wi_type
order by upper(PROJ.project_name), D.work_item_number, D.row_id, upper(type.wi_type)
