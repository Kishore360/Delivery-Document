...
left join(select portfolio_key, coalesce(sum(estimated_doc_cost),0) as project_planned_cost,source_id  from #DWH_TABLE_SCHEMA.f_project
group by portfolio_key)LKP
on TRGT.portfolio_key=LKP.portfolio_key
and TRGT.source_id=LKP.source_id
WHERE COALESCE(LKP.project_planned_cost,CASE WHEN LKP.portfolio_key IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_planned_doc_cost,0)
