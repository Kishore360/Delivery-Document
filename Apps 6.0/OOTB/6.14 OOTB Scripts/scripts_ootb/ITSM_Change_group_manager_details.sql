select  organization_name as group_name,full_name as assignment_group_manager-- , dimension_code
, '' as metrics ,
opened_changes-- , unplanned_changes, planned_changes, rescheduled_changes
from
(select  dio.organization_name,dic.full_name , -- lovcategory.dimension_code, 
...
on dio.row_key =fcr.assignment_group_key

join #DWH_TABLE_SCHEMA.d_internal_contact dic
on dic.row_key =dio.manager_key

group by   dio.organization_name,dic.full_name-- , lovcategory.dimension_code
order by  dio.organization_name,dic.full_name-- , lovcategory.dimension_code
);
