...
WHERE COALESCE(DATE_ADD(CONVERT_TZ(SRC7.START_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), INTERVAL coalesce(SRC6.effort_order,0) DAY),'')<>COALESCE(TRGT.TIMECARD_ON,'')
