...
ON COALESCE(concat('GROUP~',SRC.asgrpid),'UNSPECIFIED') = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.asgrpid IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.assignment_group_key,'')
