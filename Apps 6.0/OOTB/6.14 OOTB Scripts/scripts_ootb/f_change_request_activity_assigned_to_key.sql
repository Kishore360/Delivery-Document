...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.change_request_key, LKP.source_id, LKP.state_created_on, LKP.assigned_to_key 
from #DWH_TABLE_SCHEMA.f_change_request_activity LKP
where LKP.task_attribute_wh_name = 'assigned_to'
group by LKP.change_request_key, LKP.source_id, LKP.state_created_on) base
