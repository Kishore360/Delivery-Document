...
case when coalesce(resource_allocated_hour,0)=0 then null 
	 when (resource_allocated_hour is not null and resource_actual_hours is null) then '0.0%'
...
project_number, 
project_task, 
resource_type,primary_role,resource_role,
...
project_number, 
project_task, 
resource_type,resource_key,primary_role,resource_role
)
-- where resource not in ('UNKNOWN')
order by upper(resource),upper(project_number), upper(project_task),upper(resource_type),upper(primary_role),upper(resource_role),resource_actual_hours,
resource_planned_effort_hours
