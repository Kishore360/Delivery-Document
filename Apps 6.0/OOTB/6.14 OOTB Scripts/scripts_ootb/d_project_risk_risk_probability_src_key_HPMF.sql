...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_risk.risk_probability_src_key' ELSE 'SUCCESS' END as Message
...
on lov.row_id=coalesce(CONCAT('PROJECT_RISK~PROBABILITY~',SRC1.PROBABILITY_CODE),'UNSPECIFIED')
and lov.source_id=SRC1.sourceinstance
where coalesce(lov.row_key,case when SRC1.PROBABILITY_CODE is null then 0 else -1 end)<>coalesce(TRGT.risk_probability_src_key,'');