 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster_keyword.changed_on' ELSE 'SUCCESS' END as Message
 FROM (select 
 'X' as cdctype
...
-- select * from #DWH_TABLE_SCHEMA.d_cluster_keyword;
