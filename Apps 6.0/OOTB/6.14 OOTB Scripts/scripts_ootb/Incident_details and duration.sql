...
to_char(D.last_resolved_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') reslvd_on,
to_char(D.closed_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') clsd_on,
priority.dimension_name prio,
D.major_incident_flag mi_flag,
D.reopened_flag ro_flag,
state.dimension_name st,
IOG.organization_name curr_org,
IOG.man_nm man_nm,

trim(to_char(coalesce(round(sum(INCACT.assigned_duration / 86400.00), 1), 0), '9,999,999,990D9')) dur,

case when round((sum(F.update_count)),0) >=1000
then concat(trim(to_char(coalesce(round(((sum(F.update_count)*1.00)/1000),0),0), '9,999,999,990')),'K') 
else (trim(to_char(coalesce(round((sum(F.update_count))),0), '9,999,999,990'))) end updts
...
group by inc_key, inc_num, D.opened_on, D.last_resolved_on, D.closed_on, prio, mi_flag, ro_flag, st, IOG.organization_name, IOG.man_nm
order by inc_key, inc_num, D.opened_on, D.last_resolved_on, D.closed_on, prio, mi_flag, ro_flag, st, upper(IOG.organization_name), upper(IOG.man_nm)