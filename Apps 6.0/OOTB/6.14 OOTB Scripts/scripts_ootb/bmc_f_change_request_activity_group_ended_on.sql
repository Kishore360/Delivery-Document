...
CASE WHEN COUNT(1) >0 THEN 'group_ended_on did not match for f_change_request_activity' 
...
COALESCE(case when TGT.task_attribute_wh_name ='assignment_group' then adddate(TGT.created_on, INTERVAL TGT.primary_duration SECOND) 
when TGT.task_attribute_wh_name = 'assigned_to' then TGT.group_ended_on 
when TGT.task_attribute_wh_name = 'state' then TGT.group_ended_on 
else null
end,'') <> COALESCE(TGT.group_ended_on,'')
union
select TGT.row_id
from ( SELECT * FROM #MDS_TABLE_SCHEMA.bmc_chg_infrastructure_change_sys_audit_final WHERE CDCTYPE<>'D') SRC
left join #DWH_TABLE_SCHEMA.f_change_request_activity TGT
