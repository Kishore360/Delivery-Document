select 
Resource,skill_name,
TO_CHAR((calendar_date),'fmMM/fmDD/YYYY'), 
...
from 
(
select calendar_date,resource_key, Resource,skill_name,  
...
group by calendar_date,resource_key, Resource,skill_name
)
where resource not in ('UNKNOWN') 
order by upper(resource),upper(skill_name),to_char(calendar_date,'yyyy-mm-dd'),resource_allocated_hour,resource_actual_hours,capacity_hours