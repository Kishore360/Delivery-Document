...
case when count(1) > 0 then 'MDS to DWH data validation failed for pause_duration from task_sla_final to f_task_sla' else 'SUCCESS' end as Message
...
where ifnull(CASE WHEN S.pause_duration is null  THEN NULL   
	 ...
	 ELSE TIMESTAMPDIFF(SECOND,'1970-01-01 00:00:00', S.pause_duration) end,'') <> ifnull(DWH.pause_duration,'');
