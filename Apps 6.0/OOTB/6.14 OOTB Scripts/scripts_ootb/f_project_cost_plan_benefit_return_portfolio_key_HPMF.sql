...
FROM 
(select TRGT.portfolio_key from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
on COALESCE(SRC.prj_portfolio_id,'')=COALESCE(LKP.row_id,'')
and COALESCE(SRC.sourceinstance,'')=COALESCE(LKP.source_id,'')
WHERE planned_benefit.period_id is not null
and COALESCE(LKP.row_key,CASE WHEN SRC.prj_portfolio_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key,0)
union
select TRGT.portfolio_key from(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
on SRC.PRJ_FINANCIAL_SUMMARY_ID=fin_summary.FINANCIAL_SUMMARY_ID 
