...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_resource_plan.PLANNED_END_ON_KEY' ELSE 'SUCCESS' END as Message
...
WHERE DATE_FORMAT(coalesce(CONVERT_TZ(SRC2.SCHED_FINISH_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')
