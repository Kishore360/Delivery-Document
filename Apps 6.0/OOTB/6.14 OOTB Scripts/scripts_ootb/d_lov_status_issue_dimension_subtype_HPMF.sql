...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.dimension_subtype' ELSE 'SUCCESS' END as Message
...
where coalesce(TRGT.dimension_subtype,'')<>null;