SELECT   COALESCE(LKP.row_key,CASE WHEN src.src_resource_plan_key IS NULL THEN 0 else '-1' end),COALESCE(trg.resource_plan_key ,'')
FROM
(SELECT
 ...
 COALESCE(concat(prassignment.PRRESOURCEID,'~',prassignment.PRTASKID,'~',prtask.PRPROJECTID),'UNSPECIFIED') as src_resource_plan_key
FROM
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_resource_plan LKP 
 ON ( src.src_resource_plan_key = LKP.row_id 
AND src.prassignment_sourceinstance = LKP.source_id )
 WHERE trg.row_id like '%ASSIGNED%' and
 COALESCE(LKP.row_key,CASE WHEN src.src_resource_plan_key IS NULL THEN 0 else '-1' end)<> COALESCE(trg.resource_plan_key ,'');
