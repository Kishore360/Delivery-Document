...
ON (date_format(convert_tz(SRC.system_changeddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i') = LKP.row_id AND LKP.source_id=0)
 WHERE coalesce(LKP.row_key, 0) <> coalesce(TRGT.created_time_key,'');