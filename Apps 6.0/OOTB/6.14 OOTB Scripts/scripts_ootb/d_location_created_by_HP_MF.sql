SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.created_by' ELSE 'SUCCESS' END as Message
...
AND SRC.SOURCEINSTANCE=TRGT.SOURCE_ID
 where COALESCE(TRGT.created_by,'')<>COALESCE(SRC.created_by,'');