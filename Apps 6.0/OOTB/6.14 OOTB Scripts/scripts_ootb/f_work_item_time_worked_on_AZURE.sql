SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item.time_worked_on' ELSE 'SUCCESS' END as Message
...
when LKP.work_item_key is null and STS.dimension_wh_code = 'IN PROGRESS' then timestampdiff(second, SRC.createdDate, convert_tz(FRESH.lastupdated, '<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>')) else 0 end else 0 end,0) sr, coalesce(TRGT.time_worked_on,'') tg*/
...
case when LKP.work_item_key is not null and STS.dimension_wh_code in ('RESOLVED', 'COMPLETED', 'CANCELLED') and LKP.time_worked_on = 0 then timestampdiff(second,dates.created_on,coalesce(dates.resolved_on, dates.completed_on, dates.cancelled_on))
when LKP.work_item_key is not null then LKP.time_worked_on 
when LKP.work_item_key is null and STS.dimension_wh_code = 'IN PROGRESS' then timestampdiff(second, SRC.createdDate, convert_tz(FRESH.lastupdated, '<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>')) else 0 end else 0 end,0) <>coalesce(TRGT.time_worked_on,'');