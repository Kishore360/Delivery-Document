...
trim(to_char(count(case when D.over_due_flag = 'Y' and F.task_key is not null then F.task_key else null end),'FM999,999,999')) as odtasks,

case when count(case when D.over_due_flag = 'Y' and F.task_key is not null then F.task_key else null end) = 0 then null 
else to_char(round(((sum(case when D.over_due_flag='Y' and F.task_key is not null then F.overdue_duration else 0 end))/86400.00)
/ (count(case when D.over_due_flag='Y' then F.task_key else null end )::decimal(10,1)),1),'FM999,999,999,999,990D0') end as avg_od_duration
...
(
 ...
 join 
 (
 SELECT assigned_to.*, fco_d_internal_contact.full_name man_nm
