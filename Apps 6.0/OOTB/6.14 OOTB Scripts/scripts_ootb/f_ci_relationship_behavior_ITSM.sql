SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for behavior column of f_ci_relationship' ELSE 'SUCCESS' END as Message
from (
...
where coalesce(SRC.behavior, '') <> coalesce(TRGT.behavior, '')
) SQ;