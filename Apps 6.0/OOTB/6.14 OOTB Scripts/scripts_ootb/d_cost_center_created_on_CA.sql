...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cost_center.created_on' ELSE 'SUCCESS' END as Message
FROM  
...
where COALESCE(SRC.created_date,'')<> COALESCE(TRGT.created_on,'')