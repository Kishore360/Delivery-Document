...
from 
(
select resource_key, Resource, manager_name,location_name, 
...
group by resource_key, Resource,  location_name, manager_name
)
where resource not in ('UNKNOWN') 
order by upper(resource),upper(manager_name),upper(location_name),resource_cost,resource_allocated_cost