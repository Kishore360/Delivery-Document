SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_availability.changed_by' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(SRC1.last_updated_by,'')<> coalesce(TRGT.changed_by,'')