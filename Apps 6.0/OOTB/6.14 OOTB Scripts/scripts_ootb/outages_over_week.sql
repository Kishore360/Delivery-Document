select 
concat(cal_week.month_name_abbreviated, concat(' ', "substring"(cal_week.week_start_date_key, 7, 8))) WK,
count(F.outage_key) Outages,
SUM(case when outage_type.outage_type = 'PLANNED OUTAGE' THEN 1 ELSE 0 END) planned,
count(case when outage_type.outage_type = 'UNPLANNED OUTAGE' THEN F.outage_key ELSE null END) unplanned,
SUM(case when outage_type.outage_type = 'DEGRADATION' THEN 1 ELSE 0 END) DEGRADATION
from 
(
...
on 	(CAL.week_start_date_key = cal_week.row_key)
...
group by WK, cal_week.week_start_date_key
order by cal_week.week_start_date_key, WK