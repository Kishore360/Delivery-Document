...
impact.dimension_name imp, 
state.dimension_name st, 
approval.dimension_name apr, 
urgency.dimension_name urg, 
reported_type.dimension_name rpt,

round(sum(distinct case when state.state = 'CLOSED' then 0 else F.update_count end) * 1.00, 0) updt_cnt,
...
on cal.row_key = (case when state.state != 'CLOSED' then F.opened_on_key else F.closed_on_key end)
join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
group by D.problem_task_number, DPROB.problem_number, D.short_description, pr, imp, st, apr, urg, rpt
order by D.problem_task_number, DPROB.problem_number, D.short_description, pr, imp, st, apr, urg, rpt