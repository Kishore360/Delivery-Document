...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.domain_key' ELSE 'SUCCESS' END as Message
FROM 
...
left outer join
(select * from  #DWH_TABLE_SCHEMA.f_resource_allocation where row_key not in (0,-1))TRGT   
ON CONCAT('ALLOCATION','~',SRC.prid,date_format(SRC1.slice_date,'%Y%m%d')) =TRGT.row_id 
and TRGT.source_id = SRC.sourceinstance
where  TRGT.domain_key<> 0