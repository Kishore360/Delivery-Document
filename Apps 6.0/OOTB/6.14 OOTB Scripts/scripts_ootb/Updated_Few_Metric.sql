...
case when coalesce(resource_allocated_hour,0)=0 then null
	  when resource_allocated_hour is not null and resource_actual_hours is null then '100.0%'
	  when (resource_allocated_hour-resource_actual_hours)=0 then '0.0%'
	  when (resource_allocated_hour-resource_actual_hours)<0 then
	  '('||(cast((coalesce((round(((((resource_allocated_hour-resource_actual_hours)*100.0)/resource_allocated_hour)),1)*(-1)),0)) as varchar(200)))||'%)'
	  else concat(trim(to_char(coalesce(round(((((resource_allocated_hour-resource_actual_hours)*100.0)/resource_allocated_hour)),1),0), '9,999,999,990D9')),'%') 
	  end	as allocation_gap_percentage,
...
case when round(((coalesce(resource_allocated_hour,0)-coalesce(resource_actual_hours,0))),1) = 0 then '0.0'
when round(((coalesce(resource_allocated_hour,0)-coalesce(resource_actual_hours,0))),1) <= 0 	
then '('||trim(to_char(round(((coalesce(resource_allocated_hour,0)-coalesce(resource_actual_hours,0))),1)*(-1), '9,999,999,990D9'))||')'
else trim(to_char(round(((coalesce(resource_allocated_hour,0)-coalesce(resource_actual_hours,0))),1), '9,999,999,990D9')) end as Effort_variance,

case when coalesce(resource_allocated_hour,0)=0 then null
when (coalesce(resource_actual_hours,0)<=resource_allocated_hour) then '0.0%' else 
concat(trim(to_char(coalesce(round(((((resource_actual_hours-resource_allocated_hour)*100)/resource_allocated_hour)),1),0), '9,999,999,990D9')),'%') end as Overtime_percentage,

case when round(((coalesce(resource_actual_hours,0)-coalesce(resource_allocated_hour,0))),1) <= 0 then '0.0'
-- then '('||trim(to_char(round(((resource_planned_effort_hours-coalesce(resource_allocated_hour,0))),1)*(-1), '9,999,999,990D9'))||')'
else trim(to_char(round(((resource_actual_hours-coalesce(resource_allocated_hour,0))),1), '9,999,999,990D9')) end as Overtime_hours,


case when round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost)),0) <0
-- then concat('$',(trim(to_char(coalesce(round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost))),0), '9,999,999,999'))))
then '-$'||(cast((coalesce((round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost)),0)*(-1)),0)) as varchar(200)))
when round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_allocated_cost))),0), '9,999,999,999')))) end as RCV_Actual_Vs_Allocated,

case when round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost)),0) <0
then '-$'||(cast((coalesce((round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost)),0)*(-1)),0)) as varchar(200)))
-- then concat('$',(trim(to_char(coalesce(round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost))),0), '9,999,999,999'))))
when round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((((coalesce(resource_actual_hours,0)*Resource_Per_Hour_Cost)-resource_planned_effort_cost))),0), '9,999,999,999')))) end as RCV_Actual_Vs_Planned,

case when round(((resource_allocated_cost-resource_planned_effort_cost)),0) <0
then '($'||(cast((coalesce((round(((resource_allocated_cost-resource_planned_effort_cost)),0)*(-1)),0)) as varchar(200)))||')'
when round(((resource_allocated_cost-resource_planned_effort_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round((((resource_allocated_cost-resource_planned_effort_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round(((resource_allocated_cost-resource_planned_effort_cost))),0), '9,999,999,999')))) end as RCV_Allocated_Vs_Planned, 

case when coalesce(resource_allocated_hour,0)=0 then '100.0%' when resource_actual_hours is null then '100.0%' 
when (100-((resource_actual_hours*100)/resource_allocated_hour))=0 then '0.0%'
when (100-((resource_actual_hours*100)/resource_allocated_hour))>0 then
concat(trim(to_char(coalesce(round((100-((resource_actual_hours*100)/resource_allocated_hour)),1),0), '9,999,999,990D9')),'%')
else '('||(cast((coalesce((round((100-((resource_actual_hours*100)/resource_allocated_hour)),1)*(-1)),0)) as varchar(200)))||'%)'
end  as Timecard_Non_Compliance_percentage 

from
(
select distinct calendar_date, Resource,primary_role,
...
order by upper(resource),upper(primary_role),to_char(calendar_date,'yyyy-mm-dd'),4,5,6,7,8,9,10,11
