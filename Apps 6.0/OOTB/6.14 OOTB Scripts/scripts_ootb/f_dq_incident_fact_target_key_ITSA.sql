 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_incident_fact.target_key' ELSE 'SUCCESS' END as Message
 FROM 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=NEW' and rule_type='Complex' and inc.dimension_wh_code='OPEN'  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=CLOSED' and rule_type='Complex' and inc.dimension_wh_code in ('CLOSED', 'RESOLVED')  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=INPROGRESS' and rule_type='Complex' and inc.dimension_wh_code='OPEN' and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=INPROGRESS' and rule_type='Individual' and inc.dimension_wh_code='OPEN'  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=NEW' and rule_type='Individual' and inc.dimension_wh_code='OPEN'  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=CLOSED' and rule_type='Individual' and inc.dimension_wh_code in ('CLOSED', 'RESOLVED')  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
...
where inc.dimension_wh_code in ('CLOSED','OPEN') and  /*(cal1.lagging_count_of_month between 0 and 12) */ cast(inc.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) 
