SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_asset.quarter_bucket' ELSE 'SUCCESS' END as Message
...
on FRSH.source_id=SRC.sourceinstance
 ...
 DATEDIFF(FRSH.lastupdated,convert_tz(SRC.asset_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'))/120
 else 0 end 
 <> TRGT.quarter_bucket 
