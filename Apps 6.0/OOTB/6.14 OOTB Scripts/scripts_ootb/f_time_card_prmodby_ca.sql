...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.prmodby' ELSE 'SUCCESS' END as Message
FROM
(select * from 
...
where 
COALESCE(src.changed_by,'')<>COALESCE(trg.prmodby,'');