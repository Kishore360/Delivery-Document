...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.met_resolution_sla_flag' ELSE 'SUCCESS' END as Message
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.hlx_hpd_help_desk_final WHERE CDCTYPE<>'D') SRC
 ...
 WHERE COALESCE( 'Y' ,'')<> COALESCE(TRGT.met_resolution_sla_flag ,'');