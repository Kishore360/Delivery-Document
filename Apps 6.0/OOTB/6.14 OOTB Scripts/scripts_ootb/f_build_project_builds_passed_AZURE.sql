...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_build_project.builds_passed' ELSE 'SUCCESS' END as Message
...
count(case when build.__numerify__primary_key is not null and  sts.dimension_wh_code = 'SUCCESS' and rslt.dimension_wh_code = 'SUCCESS' 
then rslt.row_key else null end) as builds_passed_count
...
where  COALESCE(SQ.builds_passed_count,0) <> COALESCE(TRGT.builds_passed,'');
