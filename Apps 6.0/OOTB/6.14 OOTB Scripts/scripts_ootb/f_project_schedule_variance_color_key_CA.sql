...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.schedule_variance_color_key' ELSE 'SUCCESS' END as Message
...
ON coalesce(TRGT.schedule_variance,0) BETWEEN LKP.lower_range_value AND LKP.upper_range_value
AND LKP.dimension_class = 'WH_PROJECT~COLOR~SCHEDULE_VARIANCE'
...
WHERE COALESCE(LKP.row_key,CASE WHEN TRGT.schedule_variance IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.schedule_variance_color_key,'0')  