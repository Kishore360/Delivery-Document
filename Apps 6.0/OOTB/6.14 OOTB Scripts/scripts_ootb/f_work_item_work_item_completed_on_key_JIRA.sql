...
on SRC.id=compltd_dt.issueid and SRC.sourceinstance=compltd_dt.sourceinstance

LEFT JOIN #DWH_TABLE_SCHEMA.d_calendar_date LKP_COMPLETED
on (LKP_COMPLETED.row_id  = date_format(convert_tz(compltd_dt.completed_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') and LKP_COMPLETED.source_id=0)
...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.resolution_id))=RES.src_rowid and SRC.sourceinstance=RES.source_id and RES.dimension_class = 'RESOLUTION~WORK_ITEM'
 ...
 where coalesce(case when LM.dimension_wh_code = 'COMPLETED' and (RES.dimension_wh_code<>'CANCELLED' or RES.dimension_wh_code is null) then LKP_COMPLETED.row_key else 0 end,0) <>coalesce(TRGT.work_item_completed_on_key, '');