...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.CREATED_BY' ELSE 'SUCCESS' END as Message
...
where customer=1 and coalesce(SRC.sys_created_by,'')<>coalesce(TRGT.CREATED_BY,'');