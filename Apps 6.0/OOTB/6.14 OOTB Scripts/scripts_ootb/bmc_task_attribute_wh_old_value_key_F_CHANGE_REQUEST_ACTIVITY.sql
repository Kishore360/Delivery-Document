...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.task_attribute_wh_old_value_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
left join #DWH_TABLE_SCHEMA.d_lov LOV
on LOV.row_id = concat('STATE~CHANGE_REQUEST~', SRC.oldvalue)
