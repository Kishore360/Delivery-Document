...
case when count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) = 0 then null
...
count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.proposed_flag = 'Y' then F.row_key else null end)) = 0 then null
...
count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.proposed_flag = 'Y' then F.row_key else null end))),'FM999,999,999,999,990.0'))||')'
...
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then F.row_key else null end) -
count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.proposed_flag = 'Y' then F.row_key else null end))),'FM999,999,999,999,990.0') end avg_cycle_time
...
group by type.row_key, type.wi_type
order by type.row_key, upper(type.wi_type)