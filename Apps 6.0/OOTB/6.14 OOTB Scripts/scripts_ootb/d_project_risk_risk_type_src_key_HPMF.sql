...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_risk.risk_type_src_key' ELSE 'SUCCESS' END as Message
...
on lov.row_id=coalesce(CONCAT('PROJECT_RISK~TYPE~',SRC2.PARAMETER5),'UNSPECIFIED')
and lov.source_id=SRC2.sourceinstance
where coalesce(lov.row_key,case when SRC2.PARAMETER5 is null then 0 else -1 end)<>coalesce(TRGT.risk_type_src_key,'');