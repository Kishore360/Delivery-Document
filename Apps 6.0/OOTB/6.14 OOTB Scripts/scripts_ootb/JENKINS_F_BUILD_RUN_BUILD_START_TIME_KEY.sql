...
on lkp.row_id = DATE_FORMAT( CONVERT_TZ(FROM_UNIXTIME(SRC.timestamp/1000), '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i')
WHERE COALESCE(lkp.row_key, case when SRC.timestamp is null then 0 else -1 end)<> COALESCE(TRG.build_START_TIME_KEY,'');