...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.discount_rate_percent' ELSE 'SUCCESS' END as Message
FROM
...
where  COALESCE(round(TRGT.discount_rate_percent),'') <> 0