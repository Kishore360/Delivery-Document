...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_ID as demand_key,
 time_sheet.WORK_ITEM_ID as work_item_id,sum(act_effort.total_effort) total_effort,state,max(time_sheet.last_updated_by) as last_updated_by
from
...
and  COALESCE(time_sheet.last_updated_by,'')<> COALESCE(TRGT.changed_by,'')