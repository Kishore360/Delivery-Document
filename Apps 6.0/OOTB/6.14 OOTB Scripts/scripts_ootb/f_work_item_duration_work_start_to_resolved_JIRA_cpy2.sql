SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 then case when (coalesce(case when DM.dimension_wh_code='RESOLVED' and SRC1.work_start_on is not null then 
 timestampdiff(second,coalesce(SRC1.work_start_on,0),coalesce(SRC.resolutiondate,0)) else 0 end,0) - coalesce(TRGT.duration_work_start_to_resolved,'')) in (-3600, 3600) 
