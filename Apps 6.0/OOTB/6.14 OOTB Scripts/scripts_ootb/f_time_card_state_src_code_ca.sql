...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.state_src_code' ELSE 'SUCCESS' END as Message
FROM
    (SELECT 
        prt.PRID as prt_PRID,prt.sourceinstance as prt_sourceinstance,prstatus
    FROM
...
WHERE
    pr_slices.REQUEST_NAME = 'DAILYRESOURCETIMECURVE'
        AND COALESCE(src.prstatus, '') <> COALESCE(trg.state_src_code, '');            