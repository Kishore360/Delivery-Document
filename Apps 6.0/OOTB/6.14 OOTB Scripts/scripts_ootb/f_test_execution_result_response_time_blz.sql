...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.response_time' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.duration,'')<>coalesce(TRGT.response_time,'')
