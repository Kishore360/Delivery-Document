...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.ci_name' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(cmdb_ci.name,'UNSPECIFIED') <> COALESCE(TRGT.ci_name,'')
