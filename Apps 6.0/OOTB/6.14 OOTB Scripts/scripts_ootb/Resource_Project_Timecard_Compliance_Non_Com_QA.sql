...
from
(
select distinct  Resource, parent_project_name,project,
...
group by Resource, 
parent_project_name,project,resource_key
)
-- where resource not in ('UNKNOWN')
order by upper(Resource), upper(parent_project_name),upper(project)
