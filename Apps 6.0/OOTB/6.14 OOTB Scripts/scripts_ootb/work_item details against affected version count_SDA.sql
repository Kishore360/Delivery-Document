...
type.dimension_name type_nm,
INTC_ASGN.full_name asgn_to,
TO_CHAR((FWI.created_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM'),
status.dimension_name sts_nm,
priority.dimension_name prio_nm,
trim(to_char(coalesce(round((count(distinct DAV.row_key)),0),0), '9,999,999,990')) av_count

from #DWH_TABLE_SCHEMA.f_work_item_affected_version F
...
join #DWH_TABLE_SCHEMA.d_work_item_version DAV
on DAV.row_key = F.project_affected_version_key
join
(
...
group by D.work_item_number, type.dimension_name, INTC_ASGN.full_name, FWI.created_on, status.dimension_name, priority.dimension_name
order by upper(D.work_item_number), upper(type.dimension_name), upper(INTC_ASGN.full_name), FWI.created_on, upper(status.dimension_name), upper(priority.dimension_name)