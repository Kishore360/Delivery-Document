...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.src_rowid' ELSE 'SUCCESS' END as Message
...
where 
coalesce(concat('PROJECT_ISSUE~STATUS~',SRC.STATUS_ID),'')
