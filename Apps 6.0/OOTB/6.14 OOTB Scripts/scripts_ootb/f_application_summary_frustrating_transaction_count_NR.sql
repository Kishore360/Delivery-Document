SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.frustrating_transaction_count' ELSE 'SUCCESS' END as Message
	from 
    (select SRC.sourceinstance,  SUM(CASE WHEN SRC.duration <= app.enduserapdexthreshold THEN 1 ELSE 0 END) avg_du, SRC.sys_id from  
 ...
 where coalesce( t.avg_du ,'')<> COALESCE(round(TRGT.frustrating_transaction_count,5),'');