...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.pivot_date' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.pivot_date,'')<>coalesce(TRGT.pivot_date,'')  -- collate utf8_unicode_ci;
