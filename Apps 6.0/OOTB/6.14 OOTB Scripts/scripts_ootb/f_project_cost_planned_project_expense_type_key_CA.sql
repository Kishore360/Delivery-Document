...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.planned_operating_loc_cost' ELSE 'SUCCESS' END as Message
...
on CONCAT('Project~Expense_Type~',SRC2.TRANSACTION_CLASS_ID) = LKP.src_rowid
...
and
COALESCE(LKP.row_key,CASE WHEN SRC2.TRANSACTION_CLASS_ID IS NULL THEN 0 else '-1' end)
<> COALESCE(TRGT.project_expense_type_key,'');