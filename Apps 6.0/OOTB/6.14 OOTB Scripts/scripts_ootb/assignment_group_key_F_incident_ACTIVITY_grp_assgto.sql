...
where case when TRGT_A.task_attribute_wh_name ='assignment_group'then coalesce(TRGT_A.task_attribute_wh_new_value_key ,-1)
when TRGT_A.task_attribute_wh_name = 'assigned_to' then coalesce(TRGT_S.assignment_group_key,-1)
else -1
end <> coalesce(TRGT_A.assignment_group_key,'') ;
