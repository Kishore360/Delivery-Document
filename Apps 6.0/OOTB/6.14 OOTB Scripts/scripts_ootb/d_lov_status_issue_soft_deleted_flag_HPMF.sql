...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.soft_deleted_flag' ELSE 'SUCCESS' END as Message
...
where 
coalesce(case when SRC.cdctype='D' then 'Y' else 'N' end)
<>coalesce(TRGT.soft_deleted_flag,'');