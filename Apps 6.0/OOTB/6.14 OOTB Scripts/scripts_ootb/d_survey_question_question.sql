...
case when count(1) > 0 then 'MDS to DWH data validation failed for question between survey_question_new_final & asmt_metric_final to d_survey_question' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(replace(S.question_text,'\n',''),'') <> ifnull(DWH.question,'')
