...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.avg_response_time' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.avgresponsetime,'')<>round(coalesce(TRGT.avg_response_time,''),1)
