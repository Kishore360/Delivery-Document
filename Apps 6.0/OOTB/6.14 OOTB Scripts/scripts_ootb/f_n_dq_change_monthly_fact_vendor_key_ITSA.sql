SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_dq_change_monthly_fact.vendor_key' ELSE 'SUCCESS' END as Message
FROM #STG_TABLE_SCHEMA.f_n_dq_change_monthly_fact_final_base SRC
/*left join #DWH_TABLE_SCHEMA.f_change f_cr
 on f_cr.change_key=SRC.target_key
