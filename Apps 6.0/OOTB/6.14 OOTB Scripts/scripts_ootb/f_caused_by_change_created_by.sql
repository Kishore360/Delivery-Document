...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.created_by' ELSE 'SUCCESS' END as Message 
from(select coalesce(chg.sys_created_by,''),coalesce(TRGT.created_by,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(chg.sys_created_by,'')<> coalesce(TRGT.created_by,'')

union all

select coalesce(chg.sys_created_by,''),coalesce(TRGT.changed_by,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(chg.sys_created_by,'')<> coalesce(TRGT.created_by,''))ca