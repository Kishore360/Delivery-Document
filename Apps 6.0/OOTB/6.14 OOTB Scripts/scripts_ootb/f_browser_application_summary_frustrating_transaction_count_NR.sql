SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.satisfied_transaction_count' ELSE 'SUCCESS' END as Message
   ...
   -- select TRGT.row_id, t.du, TRGT.frustrating_transaction_count
    from
    (select SRC.sourceinstance,sum(CASE WHEN SRC.duration > ( 4*app.enduserapdexthreshold ) THEN 1 ELSE 0 END)du,
    CONCAT( COALESCE((DATE_FORMAT(FROM_UNIXTIME(SRC.timestamp/1000-MOD(SRC.timestamp/1000,600)),'%Y%m%d%H%i')),'UNSPECIFIED'),
