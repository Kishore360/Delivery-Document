...
sum(case when D.task_type in ('TASK', 'PROJECT') and D.is_in_budget_flag = 'Y' and d.is_in_schedule_flag= 'Y' then 1 else 0 end) No_of_tasks_proj_ovr_budget_sch,
concat(round((case when sum(case when D.is_milestone_flag = 'Y' then 1 else 0 end) = 0 then null
else (Sum (case when D.is_milestone_flag = 'Y' and d.is_milestone_slipped_flag = 'Y' then 1 else 0 end)/
sum(case when D.is_milestone_flag = 'Y' then 1.00 else 0.00 end)) end)*100,0),'%') prct_Project_MS_slipped,
concat(round((case when sum(case when D.is_milestone_flag = 'Y' then 1 else 0 end) = 0 then null
else (Sum (case when D.is_milestone_flag = 'Y' and STATE.state = 'Closed' then 1.00 else 0.00 end)/
sum(case when D.is_milestone_flag = 'Y' then 1.00 else 0.00 end))*100 end),0),'%') milestone_prct_complete,
sum(case when D.is_milestone_flag = 'Y' then 1 else 0 end) No_of_milestone,
sum(case when D.is_milestone_flag = 'Y' and STATE.state = 'Closed' then 1 else 0 end) No_of_milestone_closed,
sum(case when D.is_milestone_flag = 'Y' and d.is_milestone_slipped_flag = 'Y' then 1 else 0 end) No_of_milestone_closed_slipped
...
D2.project_name ,
SRC_Project_Task_Approval,
SRC_Project_TASK_Contact_type,
SRC_Project_Task_escalation,
SRC_Project_Task_impact,
SRC_Project_Task_phase,
SRC_Project_Task_phase_type,
SRC_Project_Task_priority,
SRC_Project_Task_risk,
proj_task_type,
SRC_Project_Task_urgency,
milestone
order by D_P.row_key, lower(D_P.name), D.row_key, lower(D.Project_name)