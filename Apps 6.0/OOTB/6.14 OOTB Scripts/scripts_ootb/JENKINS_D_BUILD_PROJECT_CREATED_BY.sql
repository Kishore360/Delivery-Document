SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_build_project.CREATED_BY' ELSE 'SUCCESS' END as Message
...
-- WHERE COALESCE(SRC.creatorid,'')<> COALESCE(TRG.CREATED_BY,'');