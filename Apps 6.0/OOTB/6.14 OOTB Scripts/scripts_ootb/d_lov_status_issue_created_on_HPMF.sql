...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.created_on' ELSE 'SUCCESS' END as Message
...
where 
coalesce(convert_tz(SRC.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
