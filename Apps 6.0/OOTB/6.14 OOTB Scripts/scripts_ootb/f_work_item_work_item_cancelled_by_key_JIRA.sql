...
(case when DM.dimension_wh_code in ('CANCELLED') then CONCAT('INTERNAL_CONTACT~',cncld_by.authorRef)
  when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') then case when RES.dimension_wh_code in ('CANCELLED') then CONCAT('INTERNAL_CONTACT~',coalesce(resolved_by.authorRef,completed_by.authorRef)) else 'UNSPECIFIED' end
