select app.name as app_name,d.device_name as device_name,t.hour_12_format_char as hour,loc.location_name as location,
loc.country_code as country, loc.state_code as state, loc.city_code as city,
t.minute_number as time,
trim(to_char(coalesce(count(distinct f.device_uuid),0), '9,999,999,990')) as mobile_device_count,
...
trim(to_char(coalesce(sum(f.request_response_time),0), '9,999,999,990')) as http_error
