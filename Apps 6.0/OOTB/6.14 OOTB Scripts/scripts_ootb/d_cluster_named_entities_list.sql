...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster.named_entities_list' ELSE 'SUCCESS' END as Message
 FROM (select 
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.named_entities_list,'')<>coalesce(TRGT.named_entities_list,'');	
