SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_resource_role.description' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(SRC.full_name,'')<> coalesce(TRGT.description,'')