...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.cdctype' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.cdctype,'')<>coalesce(TRGT.cdctype,'') ; -- collate utf8_unicode_ci;
