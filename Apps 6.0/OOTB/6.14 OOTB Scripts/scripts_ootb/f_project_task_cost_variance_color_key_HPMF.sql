...
ON COALESCE(TRGT.cost_variance,'') BETWEEN LKP.lower_range_value AND LKP.upper_range_value
AND LKP.dimension_class = 'WH_PROJECT~COLOR~COST_VARIANCE'
WHERE COALESCE(LKP.row_key, CASE WHEN TRGT.cost_variance is null then 0 else -1 end )<>COALESCE(TRGT.cost_variance_color_key,0)