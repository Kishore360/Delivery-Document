SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 when DM.dimension_wh_code in ('COMPLETED') and dates.work_start_on is not null then
 timestampdiff(second,coalesce(dates.work_start_on,0),coalesce(dates.completed_on,0)) else 0 end,0) - coalesce(TRGT.duration_work_start_to_completed,'')) in (-3600, 3600) 
 then 'Failing due to timezone conversion issue for f_work_item.duration_work_start_to_completed'
 ELSE  'MDS to DWH data validation failed for f_work_item.duration_work_start_to_completed' END ELSE 'SUCCESS' END as Message
...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.resolution_id))=RES.src_rowid and SRC.sourceinstance = RES.source_id
 ...
 when DM.dimension_wh_code in ('COMPLETED') and dates.work_start_on is not null then
 timestampdiff(second,coalesce(dates.work_start_on,0),coalesce(dates.completed_on,0)) else 0 end,0) <>coalesce(TRGT.duration_work_start_to_completed,'')