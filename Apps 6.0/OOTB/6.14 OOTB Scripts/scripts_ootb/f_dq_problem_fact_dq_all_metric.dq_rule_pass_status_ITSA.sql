 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_problem_fact_dq_all_metric.dq_rule_pass_status' ELSE 'SUCCESS' END as Message
-- select distinct row_id_der, row_id,dq_target_value, target_column_value,dq_rule_pass_status,rule_pass_status FROM --  (select * from #DWH_TABLE_SCHEMA.f_dq_problem_fact  where target_column like'dq_%' and target_column<>'dq_age' and  soft_deleted_flag='N')dq_fact  left join
 from 
(
...
backlog_flag as dq_target_value, case when Dq_issue=0 then 1.0000000000
...
left join #STG_TABLE_SCHEMA.dq_issue_prob 
on dq_issue_prob.target_key=prob.row_key
...
case when prob.reassignment_count>3 then 'Y' else 'N' end as dq_target_value, case when Dq_issue=0 then 1.0000000000
when Dq_issue>0 then 0.0000000000
 end as dq_rule_pass_status-- , dq_fct.row_id, dq_fct.row_key
from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0') ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
...
case when reopened_count_sum>0 then 'Y' else 'N'end as dq_target_value, case when Dq_issue=0 then 1.0000000000
when Dq_issue>0 then 0.0000000000
 end as dq_rule_pass_status-- , dq_fct.row_id, dq_fct.row_key
from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0') ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 left join (select * from #DWH_TABLE_SCHEMA.f_dq_problem_fact  where target_column like'dq_%' and target_column<>'dq_age' and  soft_deleted_flag='N')dq_fact
