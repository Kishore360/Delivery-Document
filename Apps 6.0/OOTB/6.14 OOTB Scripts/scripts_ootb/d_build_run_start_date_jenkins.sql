...
WHERE COALESCE(date_format(CONVERT_TZ(FROM_UNIXTIME(round(SRC.timestamp/1000)),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y-%m-%d %H:%i:%s'),0) 
<> COALESCE(TRG.start_time,'');
