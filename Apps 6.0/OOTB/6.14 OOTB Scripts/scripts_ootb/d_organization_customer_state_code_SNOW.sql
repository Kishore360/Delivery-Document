...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.state_code' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.state,''),coalesce(TRGT.state_code,'')
...
where customer=1 and coalesce(SRC.state,'')<>coalesce(TRGT.state_code,'');