...
resolution.wi_res wi_resolution,
status.dimension_name wi_status,
status_category.wi_sts_ctgy wi_status_category,
wi_rsn.wi_rsn wi_reason,
wi_rsk.wi_rsk wi_risk,
wi_sev.wi_sev wi_severity,

/*to_char(coalesce(avg(F.age_since_creation/86400.00),0),'FM999,999,999,999,990.0') avg_lead_time,

case when (count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end)) = 0 then null
else to_char((sum(F.age_since_in_progress/86400.00)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end))),'FM999,999,999,999,990.0') end avg_cycle_time*/

case when count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) = 0 then to_char(0.0,'FM999,999,999,999,990D0')
else to_char(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_creation/86400.00 else 0.0 end)/count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end),'FM999,999,999,999,990D0') end avg_lead_time,


case when (count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and status.wi_sts = 'PROPOSED' then F.row_key else null end)) = 0 or 
sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_in_progress/86400.00 else 0.0 end) is null
then to_char(0.0,'FM999,999,999,999,990D0')

when (count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and status.wi_sts = 'PROPOSED' then F.row_key else null end)) < 0 or 
sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_in_progress/86400.00 else 0.0 end) < 0
then '('||to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_in_progress/86400.00 else 0.0 end)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and status.wi_sts = 'PROPOSED' then F.row_key else null end))) * (-1.0),'FM999,999,999,999,990.0')||')'

else to_char((sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.age_since_in_progress/86400.00 else 0.0 end)/
(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS') then F.row_key else null end) -
count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')) and status.wi_sts = 'PROPOSED' then F.row_key else null end))),'FM999,999,999,999,990.0') end avg_cycle_time
...
group by D.work_item_number, D.row_id, PROJ.project_name, priority.wi_prio, resolution.wi_res, status.dimension_name, status_category.wi_sts_ctgy, wi_rsn.wi_rsn, wi_rsk.wi_rsk, wi_sev.wi_sev
order by D.work_item_number, D.row_id, upper(PROJ.project_name), upper(priority.wi_prio), upper(resolution.wi_res), upper(status.dimension_name), upper(status_category.wi_sts_ctgy), upper(wi_rsn.wi_rsn), upper(wi_rsk.wi_rsk), upper(wi_sev.wi_sev)