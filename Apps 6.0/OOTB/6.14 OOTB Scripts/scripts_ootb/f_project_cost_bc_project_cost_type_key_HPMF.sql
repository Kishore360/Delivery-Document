...
on CONCAT('PROJECT~COST_TYPE~',UPPER(SRC2.expense_type_code)) = lov.row_id
and lov.source_id=TRGT.source_id
where coalesce(lov.row_key,case when SRC2.expense_type_code is null then 0 else -1 end ,'' )<>coalesce(TRGT.project_cost_type_key,0)