SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_skill.weight' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(SRC.weight,'')<> coalesce(TRGT.weight,'')
