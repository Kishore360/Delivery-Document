...
,case  when count(1)>0 then 'MDS to DWH data validation failed: d_case.met_resolution_sla_flag' else 'SUCCESS' end as message
from #DWH_TABLE_SCHEMA.d_case D
left join 
(
...
left join #DWH_TABLE_SCHEMA.d_task_sla d
...
left join 
(select max(f.start_on) strt, f.case_key case_key
from #DWH_TABLE_SCHEMA.f_task_sla f
left join #DWH_TABLE_SCHEMA.d_task_sla d
on d.row_key=f.sla_key and f.task_type='SN_CUSTOMERSERVICE_CASE'
left join #DWH_TABLE_SCHEMA.d_lov LOV
on LOV.row_key = f.stage_src_key
where d.sla_category in ('Resolution') and f.stage_src_key is not null and LOV.dimension_code not in ('cancelled')
group by f.case_key
) SQ
on SQ.case_key = f.case_key
where f.task_type='SN_CUSTOMERSERVICE_CASE' and SQ.strt = f.start_on and d.sla_category in ('Resolution')
...
left join #DWH_TABLE_SCHEMA.d_lov LOV
on D.met_Resolution_sla_flag = LOV.dimension_code
