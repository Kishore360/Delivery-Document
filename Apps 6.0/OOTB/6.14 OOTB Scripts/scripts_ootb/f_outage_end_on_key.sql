...
on (LKP.row_id  = date_format(convert_tz(SRC.end,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d')  and LKP.source_id=0
)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.end IS NULL THEN '' else '' end)  <> COALESCE(TRGT.end_on_key,'0') 
