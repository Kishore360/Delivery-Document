...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.city_code' ELSE 'SUCCESS' END as Message
from (select coalesce(SRC.city,''),coalesce(TRGT.city_code,'')
...
where (TRGT.city_code)is not null
...
where (TRGT.city_code)is not null)inter_org