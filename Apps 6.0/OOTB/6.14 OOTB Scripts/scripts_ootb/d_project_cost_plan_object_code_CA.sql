...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project_cost_plan.OBJECT_CODE' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(SRC.OBJECT_CODE,'')<>coalesce(TRGT.OBJECT_CODE,'')