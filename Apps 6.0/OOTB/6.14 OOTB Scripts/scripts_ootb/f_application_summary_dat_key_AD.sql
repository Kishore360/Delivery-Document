...
ON CONCAT(COALESCE(SRC.application , 'UNSPECIFIED'),
'~',COALESCE(SRC.transactionName , 'UNSPECIFIED'),
'~',COALESCE(SRC.userExperience , 'UNSPECIFIED'),
'~',COALESCE(SRC.time_series, 'UNSPECIFIED'))=TRGT.row_id  AND SRC.sourceinstance=TRGT.source_id 
...
COALESCE(SRC.time_series, 'UNSPECIFIED'))=trns.row_id  AND SRC.sourceinstance=trns.source_id 
...
COALESCE(SRC.userExperience , 'UNSPECIFIED'),'~',COALESCE(SRC.time_series, 'UNSPECIFIED')) is null then 0 else -1 end)<>coalesce(TRGT.DAY_KEY,'') 
