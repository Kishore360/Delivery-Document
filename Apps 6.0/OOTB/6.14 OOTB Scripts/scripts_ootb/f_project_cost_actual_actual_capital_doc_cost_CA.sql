SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.actual_capital_doc_cost' ELSE 'SUCCESS' END as Message
...
and ROUND(COALESCE(TRGT.actual_capital_doc_cost,''),6) <> ROUND(COALESCE(CASE WHEN UPPER(SRC.COST_TYPE) = 'CAPITAL' THEN SRC2.TOTALCOST END,0),6)
