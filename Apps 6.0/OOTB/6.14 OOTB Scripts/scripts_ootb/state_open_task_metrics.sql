...
tsk_st.source_dimension_name ST_NM,

trim(to_char(count (case when F.task_key is not null then F.task_key else null end),'FM999,999,999')) as tasks,

to_char((sum(coalesce(F.time_worked ,0)::bigint) /86400.00) /(count (case when F.task_key is not null then F.task_key else null end)),'FM999,999,999,999,990D0') as time_worked, 
...
(
 ...
 or D.backlog_flag = 'Y'
 group by tsk_typ.source_dimension_name, tsk_st.row_key, tsk_st.source_dimension_name
 order by upper(tsk_typ.source_dimension_name), tsk_st.row_key, upper(tsk_st.source_dimension_name)