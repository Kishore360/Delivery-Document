...
SUM(AMOUNT_BSE) AMOUNT_BSE, SUM(AMOUNT_LCL) AMOUNT_LCL
...
left join(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)prop
on prop.prop_project_id=SRC.PRJ_PROJECT_ID
and prop.sourceinstance=SRC.sourceinstance
...
on LKP.row_id = prop.request_id 
and SRC.sourceinstance = LKP.source_id
where COALESCE(LKP.row_key,case when prop.request_id is null then 0 else -1 end)<> COALESCE(TRGT.demand_key,0)