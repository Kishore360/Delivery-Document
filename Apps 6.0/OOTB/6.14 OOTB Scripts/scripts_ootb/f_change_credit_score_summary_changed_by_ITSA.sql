SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_credit_score_summary_base.changed_by' ELSE 'SUCCESS' END as Message
 ...
 where COALESCE( SRC.changed_by,'')<> COALESCE(TRGT.changed_by,'')
