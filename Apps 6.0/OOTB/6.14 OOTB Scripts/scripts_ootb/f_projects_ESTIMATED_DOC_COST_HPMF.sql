...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ESTIMATED_DOC_COST' ELSE 'SUCCESS' END as Message
...
where  COALESCE(SRC4.PLAN_TOTAL_BSE,0)
<> COALESCE(TRGT.ESTIMATED_DOC_COST ,'');