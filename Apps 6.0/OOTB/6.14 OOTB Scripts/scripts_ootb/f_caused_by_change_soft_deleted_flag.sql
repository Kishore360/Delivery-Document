...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.soft_deleted_flag' ELSE 'SUCCESS' END as Message 
from(select coalesce(case when chg.cdctype='D' then 'Y' else 'N' end,''),coalesce(TRGT.soft_deleted_flag,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(case when chg.cdctype='D' then 'Y' else 'N' end,'')<> coalesce(TRGT.soft_deleted_flag,'')

union all

select coalesce(case when chg.cdctype='D' then 'Y' else 'N' end,''),coalesce(TRGT.soft_deleted_flag,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(case when chg.cdctype='D' then 'Y' else 'N' end,'')<> coalesce(TRGT.soft_deleted_flag,''))ca