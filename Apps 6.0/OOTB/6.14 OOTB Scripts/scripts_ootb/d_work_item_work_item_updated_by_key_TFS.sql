SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_updated_by_key' ELSE 'SUCCESS' END as Message
FROM (
...
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key, case when SRC.System_ChangedBy__PersonSK is null then 0 else -1 end)<> COALESCE(TRGT.work_item_updated_by_key ,'');