...
on cast(ch_req.changed_on as date)=cal1.calendar_date
 ...
 where d_dq_rules.rule='State=NEW' and rule_type='Complex' and ch_req.dimension_wh_code='OPEN' and 
 /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH)
 
 /*and (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=CLOSED' and rule_type='Complex' and ch_req.dimension_wh_code in ('CLOSED', 'RESOLVED') and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) /*and  (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=INPROGRESS' and rule_type='Complex' and ch_req.dimension_wh_code='OPEN' and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) /*and  (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=INPROGRESS' and rule_type='Individual' and ch_req.dimension_wh_code='OPEN'  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) /*and (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=NEW' and rule_type='Individual' and ch_req.dimension_wh_code='OPEN'  and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH) /*and (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where d_dq_rules.rule='State=CLOSED' and rule_type='Individual' and ch_req.dimension_wh_code in ('CLOSED', 'RESOLVED') and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH)  /*and (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'*/
...
-- where /*ch_req.dimension_wh_code in ('CLOSED', 'RESOLVED') and */  (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
-- when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'
...
cross join (SELECT MAX(lastupdated) AS lastupdated from #DWH_TABLE_SCHEMA.d_o_data_freshness WHERE source_id=2 group by source_id)  d_o
 ...
 where ch_req.dimension_wh_code in ('CLOSED', 'RESOLVED') and /*(cal1.lagging_count_of_month between 0 and 12) */ cast(ch_req.changed_on as date)>=ADDDATE(CAST(lastupdated AS DATE), INTERVAL -13 MONTH)  /*and (case when Conf_dim.run_on ='MDW' then f_cr.pivot_date 
when Conf_dim.run_on ='MDS' then ch_req.opened_on, d_chg.changed_on end)   >'2018-11-05 16:00:00'  */
...
-- where row_id_der is null and dq_fact.row_id is not null
 where coalesce(row_id_der,'')<>coalesce(row_id,'')
