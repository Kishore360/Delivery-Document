...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.reopened_flag' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(null, '') <> COALESCE(TGT.reopened_flag,'')
union all
select TGT.row_id
...
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' 
and COALESCE('N', '')  <> COALESCE(TGT.reopened_flag,'') 
) SQ;