...
on  SRC.__numerify__primary_key=run.row_id and SRC.sourceinstance=run.source_id
WHERE COALESCE(run.row_key,case when run.row_key is null then 0 else -1 end, '')<> COALESCE(TRG.build_run_key,'');