...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_task.update_count' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(null,'') <> COALESCE(TRGT.update_count,'')
