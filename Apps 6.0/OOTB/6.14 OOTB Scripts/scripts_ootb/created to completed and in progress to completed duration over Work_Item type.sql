...
to_char(count(distinct case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then F.work_item_key else null end),'FM999,999,999,999,999') cmpltd_wi,

to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 3600.0), 'FM999,999,999,999,990D0') time_spent_comptd,

to_char(case when count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.work_item_key else null end) = 0 then null
else round((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.duration_created_to_completed else 0.0 end)/86400.00)/
count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.work_item_key else null end),1) end,'FM999,999,999,999,990D0') avg_duration_crtd_compd,

to_char(case when count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.work_item_key else null end) = 0 then null
else round((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.duration_work_start_to_completed else 0.0 end)/86400.00)/
count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.work_item_key else null end),1) end,'FM999,999,999,999,990D0') avg_duration_inprog_compd
