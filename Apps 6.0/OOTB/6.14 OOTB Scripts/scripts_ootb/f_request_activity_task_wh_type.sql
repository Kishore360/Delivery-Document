...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_activity.task_wh_type' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE('sc_request', '') <> COALESCE(TGT.task_wh_type,'')
union all
select TGT.row_id
