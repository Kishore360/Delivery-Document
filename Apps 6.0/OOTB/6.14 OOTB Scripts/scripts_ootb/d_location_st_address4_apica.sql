...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.st_address4' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.st_address4,'')<>coalesce(TRGT.st_address4,'')  -- collate utf8_unicode_ci;
