...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.PROJECT_RISK_COLOR_KEY' ELSE 'SUCCESS' END as Message
...
on concat('PROJECT~COLOR~RISK~',SRC2.PROJECT_HEALTH_CODE)=LKP.row_id
and SRC1.sourceinstance=LKP.source_id
WHERE COALESCE(LKP.row_key,CASE WHEN SRC2.PROJECT_HEALTH_CODE IS NULL THEN 0 else '-1' end)
<> COALESCE(TRGT.PROJECT_RISK_COLOR_KEY ,'');