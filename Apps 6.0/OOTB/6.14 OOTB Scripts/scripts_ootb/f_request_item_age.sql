select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_request_item.age' ELSE 'SUCCESS' END as Message
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.sc_req_item_final WHERE CDCTYPE<>'D') SRC 
 ...
 left join apps4_4_2int_mdwdb.d_lov_map lm 
  ON (lm.src_key = TRGT.state_src_key and lm.dimension_class = 'STATE~SC_REQ_ITEM')
 LEFT JOIN (
