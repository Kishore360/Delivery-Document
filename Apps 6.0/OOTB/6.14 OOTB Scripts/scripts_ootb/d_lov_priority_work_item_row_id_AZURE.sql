...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_priority_work_item_work_item.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('PRIORITY~WORK_ITEM~',(SRC.msvsts_common_priority)),'')<>COALESCE(TRGT.row_id,'')