...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_response.response' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(replace(S.response,'\n',''),'') <> ifnull(DWH.response,'')
...
where coalesce(replace(D.string_value,'\n',''),'') <> ifnull(DWH.response,'')
)SQ;