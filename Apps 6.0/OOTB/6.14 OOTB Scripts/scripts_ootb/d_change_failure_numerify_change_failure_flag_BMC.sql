...
LEFT JOIN  #DWH_TABLE_SCHEMA.d_lov_map LKP1
 ON  CONCAT('STATE','~','CHANGE_REQUEST','~',UPPER(SRC_main.change_request_status)) = LKP1.row_id 
 and TRGT.source_id=LKP1.source_id
LEFT JOIN  #DWH_TABLE_SCHEMA.d_lov_map LKP2
 ON CONCAT('CLOSE_CODE','~','CHANGE_REQUEST','~',UPPER(SRC_main.status_reason)) = LKP2.row_id
 and TRGT.source_id=LKP2.source_id
  where  COALESCE(CASE WHEN SRC_main.change_request_status is  null or SRC_main.approval_status is null then 'X' 
when   LKP1.dimension_wh_code = 'CLOSED' THEN CASE WHEN (LKP2.dimension_wh_code IN ('SUCCESSFUL_WITH_ISSUES','UNSUCCESSFUL')
 or COALESCE(src_temp1.problem_count,0)>0 or COALESCE(SRC_temp.incident_count,0)>0) THEN 'Y' ELSE 'N' END
