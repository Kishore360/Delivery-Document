...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.row_id' ELSE 'SUCCESS' END as Message
from (select coalesce(CONCAT('SUBSIDIARY~',SRC.sys_id),''),coalesce(TRGT.row_id,'')
...
where coalesce(CONCAT('SUBSIDIARY~',SRC.sys_id),'')<>coalesce(TRGT.row_id,'')

union all

select coalesce(CONCAT('DEPARTMENT~',SRC.sys_id),''),coalesce(TRGT.row_id,'')
...
where coalesce(CONCAT('DEPARTMENT~',SRC.sys_id),'')<>coalesce(TRGT.row_id,'')

union all

select coalesce(CONCAT('GROUP~',SRC.sys_id),''),coalesce(TRGT.row_id,'')
...
where coalesce(CONCAT('GROUP~',SRC.sys_id),'')<>coalesce(TRGT.row_id,''))inter_org