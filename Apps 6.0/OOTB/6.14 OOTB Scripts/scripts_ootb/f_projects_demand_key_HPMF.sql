...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.demand_key' ELSE 'SUCCESS' END as Message
...
left join(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)proposal
on proposal.prop_project_id=SRC2.PRJ_PROJECT_ID
and proposal.sourceinstance=SRC.sourceinstance
left join #DWH_TABLE_SCHEMA.d_demand LKP
on LKP.row_id = proposal.request_id 
and SRC.sourceinstance = LKP.source_id
WHERE COALESCE(LKP.row_key,case when proposal.request_id is null then 0 else -1 end)<> COALESCE(TRGT.demand_key,0)