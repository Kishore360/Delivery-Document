...
-- select distinct row_id_der, row_id,dq_target_value, target_column_value,dq_rule_pass_status,rule_pass_status  FROM   (select * from #DWH_TABLE_SCHEMA.f_dq_change_fact  where target_column not like 'dq%') dq_fact left join
 from
 (
...
ch_req.Short_Description as dq_target_value,
 cast(case when (ch_req.Short_Description in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or ch_req.Short_Description is null) then 0 else 1 end as decimal(28,10))as dq_rule_pass_status
...
, null as dq_target_value, null as dq_rule_pass_status -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0') and module='change' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
case when f_ca.change_request_key>0 then 'Y' else 'N' end  as dq_target_value, null as dq_rule_pass_status-- , dq_fct.row_id, dq_fct.row_key
 ...
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
cross join (Select d_chg.change_request_number,d_chg.row_key,d_chg.row_id,dimension_wh_code,opened_on,d_chg.changed_on from (Select * from #DWH_TABLE_SCHEMA.d_change_request where row_key not in ('-1','0')) d_chg
left join (SELECT * FROM #MDS_TABLE_SCHEMA.change_request_final WHERE CDCTYPE<>'D')f_chg
on f_chg.sys_id=d_chg.row_id
left join (select * from #DWH_TABLE_SCHEMA.d_change_failure  where soft_deleted_flag='N' and current_flag='Y') dcf
on d_chg.row_key=dcf.change_request_key
...
left join (select * from #DWH_TABLE_SCHEMA.f_dq_change_fact  where target_column not like 'dq%' and soft_deleted_flag='N') dq_fact
