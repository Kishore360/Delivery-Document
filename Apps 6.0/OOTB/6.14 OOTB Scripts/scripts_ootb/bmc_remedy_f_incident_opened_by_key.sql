...
FROM ( SELECT hpd.submitter,hpd.sourceinstance,hpd.incident_number,submitter_lkp.person_id
FROM #MDS_TABLE_SCHEMA.hlx_hpd_help_desk_final hpd
left join #MDS_TABLE_SCHEMA.hlx_ctm_people_final submitter_lkp 
on hpd.submitter = submitter_lkp.remedy_login_id 
...
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.submitter IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.opened_by_key ,'')