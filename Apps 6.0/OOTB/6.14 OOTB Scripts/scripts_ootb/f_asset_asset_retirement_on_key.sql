...
on (LKP.row_id  = date_format(convert_tz(SRC.retired,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d')  and LKP.source_id=0
)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.retired IS NULL THEN NULL else '' end)  <> COALESCE(TRGT.asset_retirement_on_key,'') 
