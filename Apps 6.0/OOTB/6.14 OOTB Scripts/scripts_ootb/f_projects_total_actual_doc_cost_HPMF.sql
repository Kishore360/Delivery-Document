...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.TOTAL_ACTUAL_DOC_COST' ELSE 'SUCCESS' END as Message
...
ON SRC2.PRJ_FINANCIAL_SUMMARY_ID=SRC3.FINANCIAL_SUMMARY_ID AND SRC2.sourceinstance=SRC3.sourceinstance 
...
where  ROUND(coalesce(SRC4.ACTUAL_TOTAL_BSE,0),7)
<> ROUND(COALESCE(TRGT.total_actual_doc_cost,''),7);
