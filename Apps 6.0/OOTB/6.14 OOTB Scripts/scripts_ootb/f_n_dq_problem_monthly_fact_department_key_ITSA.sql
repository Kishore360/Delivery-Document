SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_dq_problem_monthly_fact.department_key' ELSE 'SUCCESS' END as Message
FROM -- #DWH_TABLE_SCHEMA.f_n_dq_problem_monthly_fact TRGT left join 
#STG_TABLE_SCHEMA.f_n_dq_problem_monthly_fact_final_base SRC
...
WHERE COALESCE(SRC.department_key, '') <> COALESCE(TRGT.department_key, '');  
