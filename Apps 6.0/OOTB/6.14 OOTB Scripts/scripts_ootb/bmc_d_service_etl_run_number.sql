...
CASE WHEN count(1)>0  THEN 'MDS to DWH data validation failed for d_service.etl_run_number' ELSE 'SUCCESS' END as Message 
...
WHERE COALESCE(@#PARENT_ETL_RUN_ID@#,'UNSPECIFIED') <> COALESCE(TRGT.etl_run_number,'UNSPECIFIED')
