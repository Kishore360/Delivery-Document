 select tran.transaction_name as transaction_name ,tran.key_transaction_flag as key_transaction_flag,
trim(to_char(coalesce(sum(f.browser_page_visits),0), '9,999,999,990')) as browser_page_visits, 
 trim(to_char(coalesce(((sum(f.satisfied_transaction_count) + 
 (sum(f.tolerating_transaction_count) * 0.5)) / NULLIF(sum(f.browser_page_visits), 0)),0), '9,999,999,990D99')) as satisfaction_score, 
trim(to_char(coalesce((sum(f.key_transaction_response_time)/sum(f.browser_page_visits)),0), '9,999,999,990D99')) as key_tran_res_time,
 trim(to_char(coalesce((sum(f.network_time)/sum(f.browser_page_visits)),0), '9,999,999,990D99')) as avg_ntwk_tm,
  trim(to_char(coalesce((sum(f.page_rendering_time)/sum(f.browser_page_visits)),0), '9,999,999,990D99')) as avg_pg_rndr_tm,
  trim(to_char(coalesce(count(distinct f.session_id),0), '9,999,999,990')) as active_users
