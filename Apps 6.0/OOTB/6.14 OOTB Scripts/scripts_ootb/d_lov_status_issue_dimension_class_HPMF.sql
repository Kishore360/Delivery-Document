...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.dimension_class' ELSE 'SUCCESS' END as Message
...
where 'PROJECT_ISSUE~STATUS'<>coalesce(TRGT.dimension_class,'');