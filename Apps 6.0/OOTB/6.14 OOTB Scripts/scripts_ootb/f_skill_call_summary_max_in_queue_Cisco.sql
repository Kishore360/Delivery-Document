...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_skill_call_summary.max_in_queue' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.routermaxcallsqueued,'')<> COALESCE(TRGT.max_in_queue,'')
