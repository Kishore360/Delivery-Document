...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.st_address1' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.st_address1,'')<>coalesce(TRGT.st_address1,'')  -- collate utf8_unicode_ci;
