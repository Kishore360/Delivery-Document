SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 left join #DWH_TABLE_SCHEMA.d_case SRC1
 on SRC1.row_id = SRC.sys_id and SRC1.source_id = SRC.sourceinstance
 LEFT JOIN #DWH_TABLE_SCHEMA.f_case TRGT 
 ON (SRC.sys_id=TRGT.row_id 
 AND SRC.sourceinstance=TRGT.source_id )
LEFT JOIN #DWH_TABLE_SCHEMA.d_calendar_date LKP 
on (LKP.row_id  = date_format(SRC1.last_resolved_on,'%Y%m%d')  and LKP.source_id=0
)
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map LKP3
 ON (concat('STATE~CASE~',upper(SRC.state))=LKP3.src_rowid
