...
ON (date_format(convert_tz(SRC.microsoft_vsts_common_resolveddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') = LKP.row_id)
WHERE coalesce(LKP.row_key, case when SRC.microsoft_vsts_common_resolveddate is null then 0 else 0 end) <> coalesce(TRGT.work_item_resolved_on_key,'');