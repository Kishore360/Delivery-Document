...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.department_flag' ELSE 'SUCCESS' END as Message
from (select (TRGT.department_flag)
...
where (TRGT.department_flag)<>'N'

union all

select (TRGT.department_flag)
...
where (TRGT.department_flag)<>'N')inter_org