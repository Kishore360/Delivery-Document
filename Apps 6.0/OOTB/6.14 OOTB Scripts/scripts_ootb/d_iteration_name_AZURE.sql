...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_iteration.name' ELSE 'SUCCESS' END as Message
/*select COALESCE(SRC.Iname ,'') ,COALESCE(TRGT.name,'') */
FROM (select 
...
where COALESCE(SRC.Iname ,'') <> COALESCE(TRGT.name,'')
