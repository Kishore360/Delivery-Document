...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.response_time_90_perc' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.90line,'')<>coalesce(TRGT.response_time_90_perc,'')
