...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.planned_effort' ELSE 'SUCCESS' END as Message
...
where  ROUND(COALESCE(T2.plan_eff,0),7)
<> ROUND(COALESCE(TRGT.planned_effort,0),7);
