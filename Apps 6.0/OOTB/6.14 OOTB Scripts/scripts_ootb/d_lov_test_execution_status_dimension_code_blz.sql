...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.dimension_code' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE(CASE WHEN SRC.passed=1 THEN 'PASS' WHEN SRC.passed=0 then 'FAIL' END ,'') <> COALESCE(TRGT.dimension_code ,'');
