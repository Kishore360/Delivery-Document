SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (CONCAT('CATEGORY~PROJECT~',UPPER(SRC.visibility)) = LKP.row_id 
 AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.visibility IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_category_src_key,'')