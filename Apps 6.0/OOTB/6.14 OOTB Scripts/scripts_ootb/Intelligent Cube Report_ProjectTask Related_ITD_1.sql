...
approve.dimension_name SRC_Project_Task_Approval,
contact_type.dimension_name SRC_Project_Task_Contact_type, 
ESCALATION.dimension_name SRC_Project_Task_escalation,
IMPACT.dimension_name SRC_Project_Task_impact,
PHASE.dimension_name SRC_Project_Task_phase,
PHASE_TYPE.dimension_name SRC_Project_Task_phase_type,
PRIORITY.dimension_name SRC_Project_Task_priority,
RISK.dimension_name SRC_Project_Task_risk,
URGENCY.dimension_name SRC_Project_Task_urgency,
D.task_type proj_task_type,
sum(case when D.task_type in ('TASK', 'PROJECT') and D.is_in_schedule_flag = 'N' then 1 else 0 end) No_of_tasks_proj_delayed,
sum(case when D.task_type in ('TASK', 'PROJECT') and d.is_in_budget_flag= 'N' then 1 else 0 end) No_of_tasks_proj_ovr_budget,
sum(case when D.task_type in ('TASK', 'PROJECT') and D.is_in_effort_flag = 'N' then 1 else 0 end) No_of_tasks_proj_ovr_efrt,
sum(case when D.task_type in ('TASK', 'PROJECT') and d.is_in_budget_flag= 'Y' and D.is_in_schedule_flag = 'Y' then 1 else 0 end) No_of_tasks_proj_in_budget_sch,
sum(case when D.task_type = 'TASK' and STATE.state = 'Closed' then 1 else 0 end) No_of_closed_tasks,
sum(case when D.task_type = 'TASK' and STATE.state <> 'Closed' then 1 else 0 end) No_of_not_closed_tasks,
sum(case when D.task_type = 'TASK' and STATE.state = 'Closed' and D.is_in_schedule_flag = 'N' then 1 else 0 end) No_of_closed_tasks_sch_slipped,
sum(case when D.task_type = 'TASK' and STATE.state = 'Closed' and D.is_in_schedule_flag = 'Y' then 1 else 0 end) No_of_closed_tasks_in_sch,
sum(case when D.task_type = 'TASK' then 1 else 0 end) No_of_tasks,
sum(case when D.task_type in ('TASK', 'PROJECT') then 1 else 0 end) No_of_tasks_proj,
case when sum(case when D.task_type = 'TASK' then 1.000 else 0.000 end) = 0 then null
else concat(round((sum(case when D.task_type = 'TASK' and STATE.STATE = 'Closed' then 1.000 else 0.000 end)/sum(case when D.task_type = 'TASK' then 1.000 else 0.000 end))*100,0), '%') 
end as prct_closed_task
...
group by D_P.name , D_P.row_key,Portfolio_category ,D.project_name , D.row_key,D2.project_name , D2.row_key,
SRC_Project_Task_Approval,SRC_Project_TASK_Contact_type,SRC_Project_Task_escalation,SRC_Project_Task_impact,
SRC_Project_Task_phase,SRC_Project_Task_phase_type,SRC_Project_Task_priority,SRC_Project_Task_risk,
proj_task_type,SRC_Project_Task_urgency
