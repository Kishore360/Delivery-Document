SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.priority_escalated_flag' ELSE 'SUCCESS' END as Message

/*select SRC.system_id,
coalesce(case when SRC.final_esc_flg like '%Y%' then 'Y' else 'N' end, 'N'), coalesce(TRGT.priority_escalated_flag,'')*/
from 
(
select SRC_LKP.system_id, SRC_LKP.teamprojectsk, SRC_LKP.sourceinstance,
group_concat(SRC_LKP.esc_flg separator ',') final_esc_flg
from
(
...
case when base.microsoft_vsts_common_priority < next.microsoft_vsts_common_priority then 'Y' else 'N' end esc_flg
from
(
...
where coalesce(case when SRC.final_esc_flg like '%Y%' then 'Y' else 'N' end, 'N') <> coalesce(TRGT.priority_escalated_flag,'');