SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.DOC_CURRENCY_CODE' ELSE 'SUCCESS' END as Message
...
where COALESCE(SRC.CURRENCY_CODE,'') <> COALESCE(TRGT.DOC_CURRENCY_CODE,'')