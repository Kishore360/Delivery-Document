...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.row_id' ELSE 'SUCCESS' END as Message
...
where CONCAT(SRC.masterid,'_',SRC.labelid)<>coalesce(TRGT.row_id,'')
