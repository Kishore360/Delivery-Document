...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_priority_work_item_work_item.dimension_code' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.msvsts_common_priority,'')<>COALESCE(TRGT.dimension_code,'')