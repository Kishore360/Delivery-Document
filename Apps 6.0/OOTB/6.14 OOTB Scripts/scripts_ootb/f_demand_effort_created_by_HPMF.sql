...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_ID as demand_key,
 time_sheet.WORK_ITEM_ID as work_item_id,sum(act_effort.total_effort) total_effort,state,max(time_sheet.created_by) as created_by
from
...
and  COALESCE(time_sheet.created_by,'')<> COALESCE(TRGT.created_by,'')