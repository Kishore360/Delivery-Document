SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_to_story_flag' ELSE 'SUCCESS' END as Message

/*select SQ.__numerify__primary_key, SQ.sourceinstance, SQ.work_item_to_story_flag val,
if(SQ.work_item_to_story_flag like '%Y%', 'Y', 'N') sr,
coalesce(TRGT.work_item_to_story_flag,'') tgt*/
from 
(
select SRC.__numerify__primary_key, SRC.sourceinstance, 
group_concat(case when typ.dimension_wh_code in ('TASK') then 
case when (LNK_PAR.targetKey is null and LNK_CHL.sourceKey is null) then 'N' 
when LNK_PAR.targetKey is not null and LNK_CHL.sourceKey is null then case when LNK_PAR.par_lnk_type in ('STORY') then 'Y' else 'N' end
when LNK_PAR.targetKey is null and LNK_CHL.sourceKey is not null then case when LNK_CHL.ch_lnk_type in ('STORY') then 'Y' else 'N' end
when (LNK_PAR.targetKey is not null and LNK_CHL.sourceKey is not null) then case when LNK_PAR.par_lnk_type in ('STORY') then 'Y' when LNK_CHL.ch_lnk_type in ('STORY') then 'Y' else 'N' end
else 'N' end
else 'N' end) as work_item_to_story_flag
...
where coalesce(if(SQ.work_item_to_story_flag like '%Y%', 'Y', 'N'), 'N')<>coalesce(TRGT.work_item_to_story_flag,'');