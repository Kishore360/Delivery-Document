...
create table app_test.ds_location
	...
	(SELECT sys_id,sourceinstance
	 FROM  #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D'
	 UNION
	 SELECT A.sys_id, A.sourceinstance
	 FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') A
	...
	LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') c ON c.sys_id = Driver.sys_ID AND c.sourceinstance = Driver.sourceinstance
	LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') B ON c.contact = B.sys_id AND c.sourceinstance = B.sourceinstance;	