...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_timespent.time_spent_updated_on' ELSE 'SUCCESS' END as Message 
/*select COALESCE(SRC.authorRef, 0) ,COALESCE(TRGT.time_spent_updated_on,0) */
from
...
issue.reporterRef as reporter_id,
...
where COALESCE(CONVERT_TZ(SRC.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), 0) <> COALESCE(TRGT.time_spent_updated_on,0)