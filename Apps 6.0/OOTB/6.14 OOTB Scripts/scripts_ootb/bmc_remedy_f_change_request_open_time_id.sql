SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed' ELSE 'SUCCESS' END as Message
from (SELECT * FROM #MDS_TABLE_SCHEMA.hlx_chg_infrastructure_change_final WHERE CDCTYPE<>'D') SRC
left join (SELECT * FROM #DWH_TABLE_SCHEMA.f_change_request where soft_deleted_flag='N') TRGT 
ON (SRC.infrastructure_change_id=TRGT.row_id 
 AND SRC.sourceinstance =TRGT.source_id)
left join #DWH_TABLE_SCHEMA.d_calendar_time cal
 on cal.row_id = DATE_FORMAT(convert_tz(SRC.submit_date, '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i')
 where coalesce(cal.row_key,'') <> coalesce(TRGT.opened_time_key,'');
 

