...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.max_response_time' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.maxresponsetime,'')<>coalesce(TRGT.max_response_time,'')
