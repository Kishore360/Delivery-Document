select concat(concat(CAL_MONTH.month_name_abbreviated,' '), CAL_MONTH.year_name), SRVC.name As BS,CI.name As CI_item,			
...
group by  CAL_MONTH.month_name_abbreviated, CAL_MONTH.month_id,CAL_MONTH.year_name,SRVC.row_key,CI.row_key,SRVC.name,CI.name
order by CAL_MONTH.month_id desc,CAL_MONTH.month_name_abbreviated,CAL_MONTH.year_name,SRVC.row_key,CI.row_key,lower(SRVC.name),lower(CI.name)