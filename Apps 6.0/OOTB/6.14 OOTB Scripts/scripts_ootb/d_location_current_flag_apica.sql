...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.current_flag' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.current_flag,'')<>coalesce(TRGT.current_flag,'')  -- collate utf8_unicode_ci;
