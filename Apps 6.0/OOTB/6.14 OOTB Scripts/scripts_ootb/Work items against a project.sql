...
to_char(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.reopened_flag = 'Y' then F.row_key else null end),'FM999,999,999,999,990') reopnd_wi,
to_char(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.overdue_flag = 'Y' then F.row_key else null end),'FM999,999,999,999,990') od_wi
...
order by upper(PROJ.project_name)