...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_environment.monitor_environment_key' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.monitor_environment_key, 'Unspecified')<>coalesce(trgt.monitor_environment_key, 'Unspecified')
