...
FROM 
(select TRGT.row_id from (select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
and COALESCE(CONCAT('PLAN_RETURN_BENEFIT~',SRC.PRJ_PROJECT_ID, '~',SRC.PRJ_FINANCIAL_SUMMARY_ID,
'~',fiscal_period.FISCAL_PERIOD_ID),'UNSPECIFIED')<> COALESCE(TRGT.row_id,'')
union
select TRGT.row_id from(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
on SRC.PRJ_FINANCIAL_SUMMARY_ID=fin_summary.FINANCIAL_SUMMARY_ID 
...
and COALESCE(CONCAT('PLAN_RETURN_COST~',SRC.PRJ_PROJECT_ID, '~',SRC.PRJ_FINANCIAL_SUMMARY_ID,
'~',fiscal_period.FISCAL_PERIOD_ID),'UNSPECIFIED')<> COALESCE(TRGT.row_id,'')
)SQ
