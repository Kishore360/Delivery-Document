...
on LM.row_id = concat('STATUS~WORK_ITEM~', SRC.System_State) and LM.dimension_class = 'STATUS~WORK_ITEM'
 ...
 when LM.dimension_wh_code in ('RESOLVED', 'COMPLETED') then
 timestampdiff(second,coalesce(inprog_dt.inprog_strtdt,0),coalesce(SRC.microsoft_vsts_common_resolveddate,SRC.microsoft_vsts_common_closeddate))
 else 0 end,0) <> coalesce(TRGT.duration_work_start_to_resolved,'');