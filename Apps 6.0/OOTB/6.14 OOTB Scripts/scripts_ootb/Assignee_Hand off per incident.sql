...
D.reassignee_count tot_hnd_off_per_inc,

coalesce(max(distinct D.reassignee_count), 0) asgn_hand_off,
coalesce(count(distinct INCACT.to_assigned_to_key), 0) num_asgn_invld
...
group by inc_key, inc_num, tot_hnd_off_per_inc
order by inc_key, inc_num, tot_hnd_off_per_inc