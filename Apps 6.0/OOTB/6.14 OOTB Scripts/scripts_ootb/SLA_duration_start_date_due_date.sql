select 
sla_stage.dimension_code stage_name,
to_char(F.start_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') strt,
to_char(F.due_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') due,
to_char(F.end_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') enddt,
to_char(round(coalesce(avg( F.actual_duration ),0.00)/86400.00, 1),'FM999999990.0') as sla_duration
from 
(
...
left join
(
select	IOG.row_key AS lev4_ag_key,
...
group by	IOG.row_key
) lev4_hier
on lev4_hier.lev4_ag_key = F.assignment_group_key
left join
(
select	IOG.row_key AS lev3_ag_key,
...
left join
(
select	IOG.row_key AS lev2_ag_key,
...
left join
(
select	IOG.row_key AS lev1_ag_key,
...
group by stage_name, F.start_on, F.due_on, F.end_on
order by stage_name, F.start_on, F.due_on, F.end_on