SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.browser_page_visits' ELSE 'SUCCESS' END as Message
    from 
    (select count(1) cnt, SRC.sourceinstance, round(avg(SRC.queueDuration),5) du, CONCAT( COALESCE((DATE_FORMAT(FROM_UNIXTIME(SRC.timestamp/1000-MOD(SRC.timestamp/1000,600)),'%Y%m%d%H%i')),'UNSPECIFIED'),
 ...
 where coalesce( t.cnt ,'')<> COALESCE(round(TRGT.browser_page_visits,5),'');
