SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_cost_center.row_count' ELSE 'SUCCESS' END as Message
  from ( select * from
 ...
 left join (select count(*),source_id from  #DWH_TABLE_SCHEMA.d_cost_center)trgt
