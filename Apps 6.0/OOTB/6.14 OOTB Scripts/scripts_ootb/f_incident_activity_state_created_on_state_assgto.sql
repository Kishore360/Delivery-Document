...
CASE WHEN COUNT(1) >0 THEN 'state_created_on did not match for f_incident_activity' 
ELSE 'SUCCESS' END as Message
from #DWH_TABLE_SCHEMA.f_incident_activity TRGT_A
join (select l.state_created_on,l.incident_key,l.primary_sequence_id
from #DWH_TABLE_SCHEMA.f_incident_activity l
where l.task_attribute_wh_name ='assigned_to' 
...
where case when TRGT_A.task_attribute_wh_name ='state'then TRGT_A.created_on
when TRGT_A.task_attribute_wh_name = 'assigned_to' then TRGT_S.state_created_on 
else null
end <> TRGT_A.state_created_on ;