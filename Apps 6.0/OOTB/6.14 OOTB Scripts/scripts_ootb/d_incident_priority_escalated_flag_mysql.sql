...
SELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for d_request_item.priority_escalated_flag' ELSE 'SUCCESS' END as Message from (select count(1) cnt  from (
...
AND fieldname =  'priority' and oldvalue is not null and newvalue is not null
 ...
 join #MDS_TABLE_SCHEMA.incident_final c on b.documentkey=c.sys_id
join #DWH_TABLE_SCHEMA.d_incident d on c.sys_id = d.row_id and c.sourceinstance = d. source_id
) e  where priority_escalation<>target )b 
