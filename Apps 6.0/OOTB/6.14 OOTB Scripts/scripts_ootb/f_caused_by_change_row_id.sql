...
from(select coalesce(coalesce(outage.sys_id,inc.sys_id),'') , coalesce(TRGT.row_id,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and 
coalesce(coalesce(outage.sys_id,inc.sys_id),'') <> coalesce(TRGT.row_id,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and 
coalesce(outage.sys_id,'')<> coalesce(TRGT.row_id,''))ca