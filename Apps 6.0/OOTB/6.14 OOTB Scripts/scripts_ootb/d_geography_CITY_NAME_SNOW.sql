...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_geography.CITY_NAME' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.city,''),coalesce(TRGT.CITY_NAME,'')
...
where coalesce(SRC.city,'')<>coalesce(TRGT.CITY_NAME,'')