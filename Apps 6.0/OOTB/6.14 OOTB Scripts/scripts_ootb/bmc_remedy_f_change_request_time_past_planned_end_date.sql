...
on  SRC.change_request_status=LKP.dimension_code  
 ...
 group by source_id 
  ...
  -- AND TRGT.etl_run_number = df.etl_run_number)
	...
	where  case when LKP.dimension_wh_code in ('OPEN') 
			then case when TIMESTAMPDIFF(SECOND,coalesce(SRC.scheduled_end_date,0),convert_tz(df.lastupdated,'<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>')) > 0 
				then TIMESTAMPDIFF(SECOND,coalesce(SRC.scheduled_end_date,0),convert_tz(df.lastupdated,'<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>'))
				else 0 end
		else case when TIMESTAMPDIFF(SECOND,coalesce(SRC.scheduled_end_date,0),coalesce(SRC.actual_end_date,SRC.closed_date,SRC.last_modified_date,0))>0
				then TIMESTAMPDIFF(SECOND,coalesce(SRC.scheduled_end_date,0),coalesce(SRC.actual_end_date,SRC.closed_date,SRC.last_modified_date,0))
				else 0 end
