...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.project_manager_key' ELSE 'SUCCESS' END as Message
FROM 
...
ON CONCAT('INTERNAL_CONTACT~',SRC.MANAGER_ID) = lkp.row_id and SRC.sourceinstance=lkp.source_id
where COALESCE(lkp.row_key,case when SRC.MANAGER_ID is null then 0 else -1 end ) <> COALESCE(TRGT.project_manager_key,0);