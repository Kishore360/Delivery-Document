...
ON LKP.row_id=concat('PROJECT~STATE~',SRC10.STATUS_ID)
...
WHERE COALESCE(CASE WHEN LKP.dimension_wh_code IN ('OPEN') THEN 0
       WHEN TRGT.estimated_doc_cost=0 AND TRGT.total_actual_doc_cost=0 THEN 0
		   ...
		   WHEN TRGT.planned_percent_complete = 0 AND TRGT.total_actual_doc_cost=0  and TRGT.actual_percent_complete=0 THEN 0
           WHEN TRGT.estimated_doc_cost <>0 AND LKP.dimension_wh_code IN ('CLOSED', 'IN PROGRESS', 'CANCELLED')
