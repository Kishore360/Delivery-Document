...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ACTUAL_START_ON_KEY' ELSE 'SUCCESS' END as Message
...
where  COALESCE(DATE_FORMAT(CONVERT_TZ(SRC7.ACT_START_DATE,'<<DW_TARGET_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED')
<> COALESCE(TRGT.ACTUAL_START_ON_key,'UNSPECIFIED');
