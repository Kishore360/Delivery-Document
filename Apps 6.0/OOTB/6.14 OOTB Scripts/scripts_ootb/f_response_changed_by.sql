...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_response.changed_by' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(A.sys_updated_by,'') <> ifnull(DWH.changed_by,'')
)SQ;