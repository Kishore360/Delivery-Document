...
and COALESCE(CONCAT('PLAN_RETURN_BENEFIT~',SRC.REQUEST_ID, '~',SRC1.PROP_FINANCIAL_SUMMARY_ID,
'~',fiscal_period.FISCAL_PERIOD_ID),'UNSPECIFIED')<>COALESCE(TRGT.row_id,'')
union
select TRGT.row_id from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
on SRC.request_id=SRC1.request_id
and SRC.sourceinstance=SRC1.sourceinstance
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
...
and COALESCE(CONCAT('PLAN_RETURN_COST~',SRC.REQUEST_ID, '~',SRC1.PROP_FINANCIAL_SUMMARY_ID,
'~',fiscal_period.FISCAL_PERIOD_ID),'UNSPECIFIED')<>COALESCE(TRGT.row_id,'')
)SQ