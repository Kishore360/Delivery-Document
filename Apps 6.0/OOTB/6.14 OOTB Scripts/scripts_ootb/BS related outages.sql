select coalesce(srvc.name, '') SVC_NAME,
srvc_criticality.dimension_name SVC_CRITICALITY,
srvc_used_for.dimension_name SVC_USED_FOR_NAME,
coalesce(srvc.version, '') SVC_VER,
SUM(case when srvc_criticality.srvc_criticality_code = 'CRITICAL' THEN 1 ELSE 0 end) Outages_Affecting_Critical_BS,
...
group by SRVC.name, SVC_CRITICALITY, SVC_USED_FOR_NAME, SVC_VER
order by upper(SRVC.name), SVC_CRITICALITY, SVC_USED_FOR_NAME, SVC_VER