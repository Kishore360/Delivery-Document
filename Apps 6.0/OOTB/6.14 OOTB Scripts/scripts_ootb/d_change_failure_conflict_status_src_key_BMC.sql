...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.conflict_status_src_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(TRGT.conflict_status_src_key,'')<> coalesce(0,'')
