...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_skill_call_summary.other_calls' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.othercalls,'')<> COALESCE(TRGT.other_calls,'')
