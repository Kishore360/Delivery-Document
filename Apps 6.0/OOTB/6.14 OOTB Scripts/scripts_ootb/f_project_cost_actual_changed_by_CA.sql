SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.changed_by' ELSE 'SUCCESS' END as Message
...
and COALESCE(TRGT.changed_by,'') <> COALESCE(SRC.LASTUPDTDBY_RESOURCE_CODE,'')