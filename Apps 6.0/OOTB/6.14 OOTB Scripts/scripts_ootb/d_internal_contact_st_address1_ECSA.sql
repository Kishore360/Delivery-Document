SELECT CASE WHEN COUNT(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END AS Result,
    CASE WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.st_address1' ELSE 'SUCCESS' END AS Message
    FROM
	...
	WHERE COALESCE(sys_user.street,'') <> COALESCE(TRGT.st_address1, '')
	union 
	...
	WHERE COALESCE(csm_consumer.street,'') <> COALESCE(TRGT.st_address1, ''))IC;