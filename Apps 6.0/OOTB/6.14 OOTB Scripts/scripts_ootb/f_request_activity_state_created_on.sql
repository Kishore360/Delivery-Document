...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_activity.state_created_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.request_key, LKP.source_id, LKP.state_created_on, LKP.state_key 
from #DWH_TABLE_SCHEMA.f_request_activity LKP
where LKP.task_attribute_wh_name in ('request_state', 'stage')
group by LKP.request_key, LKP.source_id, LKP.state_created_on) base
