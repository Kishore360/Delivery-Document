...
to_char(coalesce(sum(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED')) then D.iteration_count else 0 end), 0),'FM999,999,999,999,990') ITE_cnt
