...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_customer.monitor_customer_key' ELSE 'SUCCESS' END as Message
...
left outer join #DWH_TABLE_SCHEMA.d_organisation_customer D_ORGCUST
...
where coalesce(SRC.monitor_customer_key, 'Unspecified')<>coalesce(trgt.monitor_customer_key, 'Unspecified')
