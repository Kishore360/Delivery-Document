...
from
(
SELECT OUTR.infrastructure_change_id, OUTR.ci_key, OUTR.sourceinstance, OUTR.dimension_wh_code, OUTR.scheduled_start_date,
OUTR.closed_date,
case when OUTR.scheduled_start_date is NULL then 0
...
from
(
SELECT s1.*, LKP.dimension_wh_code, tgt_for_key.ci_key FROM (SELECT * FROM  #MDS_TABLE_SCHEMA.hlx_chg_infrastructure_change_final WHERE CDCTYPE<>'D' 
...
where coalesce(INN.ci_key,1)=coalesce(OUTR.ci_key,1) 
AND INN.closed_date< OUTR.scheduled_start_date
AND INN.infrastructure_change_id<>OUTR.infrastructure_change_id
) end AS ci_prior 
from
(
select SRC.infrastructure_change_id, SRC.closed_date, tgt_for_key1.ci_key, SRC.scheduled_start_date, SRC.sourceinstance, LKP.dimension_wh_code
...
where SRC.ci_prior<>TGT.ci_prior_changes -- and work_start is not null 
