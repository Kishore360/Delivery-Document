...
WHERE COALESCE(convert_tz(SRC.pravailstart,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),convert_tz(SRC3.schedule_start,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'))<> COALESCE(TRGT.planned_start_on,'')
