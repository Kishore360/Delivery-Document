...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.group_ended_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(null,'') <> COALESCE(TGT.group_ended_on,'')
union all
select TGT.row_id
