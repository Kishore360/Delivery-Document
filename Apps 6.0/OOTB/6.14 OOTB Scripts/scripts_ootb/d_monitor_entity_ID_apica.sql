...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_monitor.entity_ID' ELSE 'SUCCESS' END as Message
 FROM (select 
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.entity_ID,'')<>coalesce(TRGT.entity_ID,'');