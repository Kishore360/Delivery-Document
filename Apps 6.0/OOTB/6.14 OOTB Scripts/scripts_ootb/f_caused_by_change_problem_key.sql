...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.problem_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(prob_d.row_key,case when prob.sys_id is null then 0 else -1 end),coalesce(TRGT.problem_key,'')
from 
...
left join (select * from #DWH_TABLE_SCHEMA.d_problem where soft_deleted_flag='N')prob_d
on prob_d.row_id=coalesce(prob.sys_id,'UNSPECIFIED') and prob_d.source_id=prob.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(prob_d.row_key,case when prob.sys_id is null then 0 else -1 end)<>coalesce(TRGT.problem_key,'')

union all

select TRGT.problem_key,0
...
where UPPER(map.dimension_wh_code)='CLOSED' and TRGT.problem_key<>0)ca