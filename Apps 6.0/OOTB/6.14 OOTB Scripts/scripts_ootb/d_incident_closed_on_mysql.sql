SELECT CASE WHEN cnt THEN 'FAILURE' ELSE 'SUCCESS' END as Result, 
CASE WHEN cnt THEN 'MDS to DWH data validation failed for d_incident.closed_on' ELSE 'SUCCESS' END as Message from (select count(1) cnt   from
#MDS_TABLE_SCHEMA.incident_final SRC  
JOIN #DWH_TABLE_SCHEMA.d_incident TRGT  ON 
(SRC.sys_id =TRGT.row_id   AND SRC.sourceinstance= TRGT.source_id  ) 
join  #DWH_TABLE_SCHEMA.f_incident TRGTF  ON TRGTF.incident_key=TRGT.row_key
JOIN #DWH_TABLE_SCHEMA.d_lov_map lkp
on TRGTF.state_src_key = lkp.src_key
WHERE convert_tz(coalesce(SRC.closed_at,sys_updated_on),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')<> (TRGT.closed_on)
AND dimension_wh_code = 'CLOSED')b
