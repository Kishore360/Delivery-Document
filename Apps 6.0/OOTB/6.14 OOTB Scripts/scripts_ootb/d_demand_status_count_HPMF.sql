...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_demand.status_count' ELSE 'SUCCESS' END as Message
from (select 
...
where coalesce(SRC.STATUS_COUNT,'')<>coalesce(TRGT.status_count,'');