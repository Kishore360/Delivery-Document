...
COALESCE(LKP.row_key, case when DM.dimension_wh_code not in ('RESOLVED') then 0 else -1 end) sr, coalesce(TRGT.work_item_resolved_by_key,'') tg*/
 ...
 when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') and ASCII(COALESCE(SRC.msvsts_common_resolvedbykey, dates.resolved_by, SRC.msvsts_common_closedbykey, dates.closed_by)) = 0 THEN 'UNSPECIFIED' 
 when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') then CONCAT('INTERNAL_CONTACT~',COALESCE(SRC.msvsts_common_resolvedbykey, dates.resolved_by, SRC.msvsts_common_closedbykey, dates.closed_by, ''))
 else 'UNKNOWN' end = LKP.row_id 
 AND (SRC.sourceinstance = LKP.source_id or LKP.source_id = 0))
  
 WHERE TRGT.soft_deleted_flag <> 'Y' and COALESCE(LKP.row_key, case when DM.dimension_wh_code not in ('RESOLVED') then 0 else -1 end)<>coalesce(TRGT.work_item_resolved_by_key,'');