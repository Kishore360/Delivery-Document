...
tsk_aprv.source_dimension_name approval_name,
tsk_cnt_typ.source_dimension_name contact_type,
trim(to_char(count (case when F.task_key is not null then F.task_key else null end),'999,999,999')) as tasks,

case when count (case when tsk_aprv.wh_dimension_name = 'APPROVED' and F.task_key is not null then F.task_key else null end) = 0 then null
else trim(to_char((sum(case when tsk_aprv.wh_dimension_name = 'APPROVED' then f.approval_duration else 0 end) / 86400.00) /
count (case when tsk_aprv.wh_dimension_name = 'APPROVED' and F.task_key is not null then F.task_key else null end)
,'999,999,990D9')) end av_approval_duration
...
(
 ...
 or D.backlog_flag = 'Y'
 group by tsk_typ.source_dimension_name, tsk_aprv.source_dimension_name, tsk_cnt_typ.source_dimension_name
 order by upper(tsk_typ.source_dimension_name), upper(tsk_aprv.source_dimension_name), upper(tsk_cnt_typ.source_dimension_name)