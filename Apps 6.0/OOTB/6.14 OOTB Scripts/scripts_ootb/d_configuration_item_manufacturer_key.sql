SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_configuration_item.manufacturer_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_internal_organization LKP 
	ON ( COALESCE(CONCAT('SUBSIDIARY~',SRC.manufacturer),'')= LKP.row_id
	AND SRC.sourceinstance = LKP.source_id )
 LEFT JOIN #DWH_TABLE_SCHEMA.d_configuration_item TRGT 
 ON (SRC.sys_id =TRGT.row_id 
 AND SRC.sourceinstance=TRGT.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.manufacturer IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.manufacturer_key ,'')