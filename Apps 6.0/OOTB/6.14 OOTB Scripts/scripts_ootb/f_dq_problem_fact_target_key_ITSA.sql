 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_problem_fact.target_key' ELSE 'SUCCESS' END as Message
 FROM 
...
cross join (Select d_prob.problem_number,d_prob.row_key,d_prob.row_id,dimension_wh_code,d_prob.changed_on,opened_on from (Select * from #DWH_TABLE_SCHEMA.d_problem where row_key not in (-1,0) and soft_deleted_flag='N' ) d_prob
...
select concat('dq_backlog_problem','~',prob.row_key) as row_id_der,rule,rule_type,'dq_backlog_problem' as dq_target_column , prob.row_key as target_key-- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in (-1,0) and soft_deleted_flag='N' and active_flag<>'N' and module='problem' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
cross join (Select d__prob.problem_number,d__prob.row_key,d__prob.row_id,dimension_wh_code,d__prob.changed_on,opened_on from (Select * from #DWH_TABLE_SCHEMA.d_problem where row_key not in (-1,0) and soft_deleted_flag='N' ) d__prob
...
select concat('dq_reassignment_problem','~',prob.row_key) as row_id_der ,rule,rule_type,'dq_reassignment_problem' as dq_target_column , prob.row_key as target_key-- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in (-1,0) and soft_deleted_flag='N' and active_flag<>'N' and module='problem' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
cross join (Select d__prob.problem_number,d__prob.row_key,d__prob.row_id,dimension_wh_code,d__prob.changed_on,opened_on from (Select * from #DWH_TABLE_SCHEMA.d_problem where row_key not in (-1,0) and soft_deleted_flag='N' ) d__prob
...
select concat('dq_reopen_problem','~',prob.row_key) as row_id_der ,rule,rule_type,'dq_reopen_problem' as dq_target_column , prob.row_key as target_key-- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in (-1,0) and soft_deleted_flag='N' and active_flag<>'N' and module='problem' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
cross join (Select d__prob.problem_number,d__prob.row_key,d__prob.row_id,dimension_wh_code,d__prob.changed_on,opened_on from (Select * from #DWH_TABLE_SCHEMA.d_problem where row_key not in (-1,0) and soft_deleted_flag='N' ) d__prob
...
where coalesce(src.target_key,'')<>coalesce(dq_fact.target_key,'')
