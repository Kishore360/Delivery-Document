...
left join(select * from #DWH_TABLE_SCHEMA.d_project)proj
on COALESCE(SRC.prj_project_id,'')=COALESCE(proj.row_id,'')
and COALESCE(SRC.sourceinstance,'')=COALESCE(proj.source_id,'')
...
AND COALESCE(proj.row_key,CASE WHEN SRC.prj_project_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.project_key,0)
