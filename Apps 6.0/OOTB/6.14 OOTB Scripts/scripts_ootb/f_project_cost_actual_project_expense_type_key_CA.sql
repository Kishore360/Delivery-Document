SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.project_expense_type_key' ELSE 'SUCCESS' END as Message
...
on CONCAT('Project~Expense_Type~',SRC6.ID) = LKP.src_rowid
and SRC6.sourceinstance = LKP.source_id
...
and COALESCE(LKP.row_key,CASE WHEN SRC6.ID IS NULL THEN 0 else '-1' end) <> COALESCE(TRGT.project_expense_type_key,'')
