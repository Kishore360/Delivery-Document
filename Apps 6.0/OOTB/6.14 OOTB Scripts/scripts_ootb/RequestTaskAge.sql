SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_request_task.age' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_request_task TRGTD
ON TRGTD.row_key = TRGT.request_task_key

LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map lm 
ON (lm.src_key = TRGT.state_src_key and lm.dimension_class = 'STATE~SC_TASK')
LEFT JOIN (
