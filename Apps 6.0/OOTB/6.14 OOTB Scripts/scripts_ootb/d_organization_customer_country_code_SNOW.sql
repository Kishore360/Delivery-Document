...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.country_code' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.country,''),coalesce(TRGT.country_code,'')
...
where customer=1 and coalesce(SRC.country,'')<>coalesce(TRGT.country_code,'');