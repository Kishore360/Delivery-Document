...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_task_activity.task_attribute_wh_new_value' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (F.problem_task_key=TGT2.task_key AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1)
