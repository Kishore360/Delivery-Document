select DIC.full_name,sum(F.I_ACD_TIME + F.I_DA_ACD_TIME) as ACD_time_intreval_based,sum(F.I_ACW_TIME + F.I_DA_ACW_TIME)acw_time_intreval_based,
sum(F.INBOUND_ACD_CALLS)/max(F.employee_key),
case when Sum(F.ACD_CALLS) = 0 then null else SUM(F.ACD_TIME+F.HOLD_TIME+F.ACW_TIME)/ SUM(F.ACD_CALLS) end avg_handle_time,
SUM(F.I_STAFF_TIME)/SUM(F.TIME_INTERVAL*60) as avg_positions_staffed,
round(Sum(F.INBOUND_ACD_CALLS*1.00),2)/365 as call_volume_per_day,
sum(F.NO_ANSWER_REDIRECT_CALLS) as no_answer_redirect_calls,
sum(F.TI_AUX_TIME) as TI_AUX_TIME, sum(TI_AVAIL_TIME) as TI_AVAIL_TIME, sum(F.TI_STAFF_TIME) as TI_STAFF_TIME
from 
(
...
group by DIC.full_name
order by DIC.full_name
