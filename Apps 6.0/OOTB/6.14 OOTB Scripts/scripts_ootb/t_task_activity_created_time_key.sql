...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for t_task_activity.created_time_key ' 
...
JOIN  #DWH_TABLE_SCHEMA.d_calendar_time CT 
ON (date_format(convert_tz(SRC.sys_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i')   
= CT.row_id 
AND CT.source_id=0)
...
AND COALESCE(CT.row_key ,CASE WHEN SRC.sys_created_on IS NULL THEN 0 ELSE -1 END) 
<> COALESCE(TRGT.created_time_key,'') 
