...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_task.state_src_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(inc.state_src_key,prob.state_src_key,prob_tas.state_src_key,req_item.state_src_key
,req_tas.state_src_key,req.state_src_key,chan_req.state_src_key,chan_tas.state_src_key,LKP.row_key
,CASE WHEN SRC.state IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.state_src_key ,'')
