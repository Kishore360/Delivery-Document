...
LEFT JOIN #DWH_TABLE_SCHEMA.d_internal_contact LKP 
 ON ( CONCAT('INTERNAL_CONTACT~',SRC.system_assignedto__personsk) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.system_assignedto__personsk IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.work_item_assigned_to_key ,''); 