...
-- select TRGT.row_id,case when COALESCE(SRC.problem_count,0)>0 then 'Y' else 'N' end,COALESCE(TRGT.caused_problem_flag,'')
from (
...
left join (SELECT * FROM #DWH_TABLE_SCHEMA.d_change_failure where year(effective_to)=2999 and current_flag='Y' and soft_deleted_flag='N'
 ...
 WHERE (case when COALESCE(SRC.problem_count,0)>0 then 'Y' 
 else 'N' end)<> COALESCE(TRGT.caused_problem_flag,'');