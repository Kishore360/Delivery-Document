...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_timespent.time_spent' ELSE 'SUCCESS' END as Message 
/*select COALESCE(SRC.time_spent,'') , COALESCE(TRGT.time_spent,'') */
from
...
where COALESCE(SRC.Time_spent,'') <> COALESCE(TRGT.time_spent,'');
