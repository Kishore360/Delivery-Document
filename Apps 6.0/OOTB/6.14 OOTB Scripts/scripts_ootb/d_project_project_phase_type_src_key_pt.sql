...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON ( concat('PROJECT~','phase_type~',phase_type) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.phase_type IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_phase_type_src_key ,'')
