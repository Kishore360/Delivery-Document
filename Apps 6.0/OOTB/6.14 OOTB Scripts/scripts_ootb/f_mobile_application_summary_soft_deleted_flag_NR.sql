...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where COALESCE(case when mobile.cdctype = 'D' then 'Y' else 'N' end, '')<> COALESCE(TRGT.soft_deleted_flag ,'') <> coalesce(TRGT.soft_deleted_flag,'')
 union
 select TRGT.row_id 
 ...
 where COALESCE(case when MobileRequestError.cdctype = 'D' then 'Y' else 'N' end, '')<> COALESCE(TRGT.soft_deleted_flag ,''))SQ
