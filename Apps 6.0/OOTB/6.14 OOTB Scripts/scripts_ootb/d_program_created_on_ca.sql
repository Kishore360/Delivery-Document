SELECT CASE
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_program.created_on'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
...
where COALESCE(convert_tz(src.created_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')  <> COALESCE(trg.created_on, '')
;
