 ...
 from (
 select * from
(select count(*) count_src from #STG_TABLE_SCHEMA.burn_down_temp SRC where SRC.work_item_key <> 0) count_src,
 ...
 (select count(*) count_trgt from #DWH_TABLE_SCHEMA.f_work_item_burn_down TRGT
  ...
  where TRGT.soft_deleted_flag = 'N' and TRGT.work_item_key <> 0) count_trgt
