...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.subsidary_fiscal_calendar' ELSE 'SUCCESS' END as Message
from (select TRGT.subsidary_fiscal_calendar
...
where TRGT.subsidary_fiscal_calendar is not null

union all

select TRGT.subsidary_fiscal_calendar
...
where TRGT.subsidary_fiscal_calendar is not null)inter_org;