...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_master_item.item_category_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON ( concat('SC~ITEM~CATEGORY','~',upper(cat.sys_id)) = LKP.row_id  
AND SRC.sourceinstance = LKP.source_id )
 WHERE cat.sys_id is not null and
 COALESCE(LKP.row_key,CASE WHEN cat.sys_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.item_category_key ,'')
