...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_service.type' ELSE 'SUCCESS' END as Message
FROM 
(
...
where field.fieldName ='ServiceType' AND schma.NAME='BMC.CORE:BMC_BusinessService') SRC2
ON SRC2.enumId=SRC1.ServiceType and SRC2.sourceinstance=SRC1.sourceinstance
) SRC
...
WHERE COALESCE(SRC.value,'UNSPECIFIED') <>COALESCE(TRGT.type,'UNSPECIFIED')
