select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.missed_resolution_sla_on' ELSE 'SUCCESS' END as Message
...
coalesce(TRGT.missed_resolution_sla_on, '') actual, case when SQ.stage in ('BREACHED') then coalesce(SQ.start_on, '') else  NULL end expected*/
...
and coalesce(TRGT.missed_resolution_sla_on, '') <> case when SQ.stage in ('BREACHED') then coalesce(SQ.start_on, '') else  NULL end;