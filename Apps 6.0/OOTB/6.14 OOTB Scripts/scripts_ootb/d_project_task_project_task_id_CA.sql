SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project_task.project_task_id' ELSE 'SUCCESS' END as Message
...
where COALESCE(SRC.prexternalid,'') <> COALESCE(TRGT.project_task_id,'')