...
case when SUR_STATE.state = 'Completed' then DIO_DEPT.organization_name else null end dept,
SUR.survey_name sur_nm,
ASGN_TO.full_name asn_to, 
concat(trim(round(
case when count(distinct F.rowkey) is null then 0.00
when count(distinct case when SUR_STATE.state = 'Completed' then F.rowkey else null end) is null then 0.00
else count(distinct case when SUR_STATE.state = 'Completed' then F.rowkey else null end)*100.00
/count(distinct F.rowkey)*1.00 
end
, 2)),'%') response_rate,
count(distinct F.rowkey) survey_sent
from 
