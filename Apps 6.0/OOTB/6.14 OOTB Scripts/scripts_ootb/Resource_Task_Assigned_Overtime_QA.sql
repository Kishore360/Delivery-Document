...
group by Resource, 
project, 
project_task,resource_key, resource_role
)
-- where resource not in ('UNKNOWN')
order by upper(resource),upper(project), upper(project_task),upper(resource_role),resource_assigned_hours,resource_actual_hours,resource_allocated_hour
