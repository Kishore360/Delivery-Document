...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_type_category_work_item.dimension_code' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE(SRC.referenceName,'')<>COALESCE(TRGT.dimension_code,'') 