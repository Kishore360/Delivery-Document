SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_test_execution.work_item_key' ELSE 'SUCCESS' END as Message
 ...
 join #DWH_TABLE_SCHEMA.d_lov_map lov TYP 
...
and SRC.sourceinstance=TRGT.source_id
 ...
 coalesce(LKP.row_key,case when SRC.testCaseKey is null then 0 else -1 end )<>coalesce(TRGT.work_item_key,'');