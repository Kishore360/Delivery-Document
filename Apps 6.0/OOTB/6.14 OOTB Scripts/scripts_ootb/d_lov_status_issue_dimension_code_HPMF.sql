...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.dimension_code' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.status_id,'')<>coalesce(TRGT.dimension_code,'');