...
WHERE	(SRC3.WORKING_DAY_FLAG = 'Y') AND SRC4.EVENT_DATE is NULL AND (SRC3.WORKING_DAY_FLAG = 'Y')  AND  round(coalesce(SRC5.DURATION,''),0) <> round(COALESCE(TRGT.capacity,''),0);