...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_0
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_1
ON(lev_0.manager = lev_1.sys_id AND lev_0.sys_id <> lev_1.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_2
ON(lev_1.manager = lev_2.sys_id AND lev_1.sys_id <> lev_2.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_3
ON(lev_2.manager = lev_3.sys_id AND lev_2.sys_id <> lev_3.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_4
ON(lev_3.manager = lev_4.sys_id AND lev_3.sys_id <> lev_4.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') lev_5
ON(lev_4.manager = lev_5.sys_id AND lev_4.sys_id <> lev_5.sys_id)
)a
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_0
ON CONCAT('INTERNAL_CONTACT~',a.s0)  = level_0.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_1
ON CONCAT('INTERNAL_CONTACT~',a.s1)  = level_1.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_2
ON CONCAT('INTERNAL_CONTACT~',a.s2)  = level_2.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_3
ON CONCAT('INTERNAL_CONTACT~',a.s3)  = level_3.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_4
ON CONCAT('INTERNAL_CONTACT~',a.s4)  = level_4.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_contact level_5
