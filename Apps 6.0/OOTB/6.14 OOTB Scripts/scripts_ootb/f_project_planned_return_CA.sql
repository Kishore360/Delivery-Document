...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.PLANNED_RETURN' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(TRGT.PLANNED_RETURN,'') <> COALESCE(NULL,'');
