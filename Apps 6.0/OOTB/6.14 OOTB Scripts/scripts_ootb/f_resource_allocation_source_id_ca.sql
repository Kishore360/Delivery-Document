...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.source_id' ELSE 'SUCCESS' END as Message
FROM
(SELECT
...
where 
COALESCE(src.prassignment_sourceinstance,'')<> COALESCE(trg.source_id,'');
