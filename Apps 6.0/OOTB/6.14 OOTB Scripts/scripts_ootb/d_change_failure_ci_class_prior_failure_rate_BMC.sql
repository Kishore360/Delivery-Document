...
from
(
select infrastructure_change_id,  numerify_change_failure_flag, ci_class_prior_changes, ci_class_key, ci_class_prior_Y,
case when ci_class_prior_changes=0 then -1 when ci_class_prior_changes=-1 then -2 
else round((ci_class_prior_Y/ci_class_prior_changes),6) end as prior_failure_rate_qa
from
(
SELECT OUTR.infrastructure_change_id, numerify_change_failure_flag, ci_class_prior_changes, OUTR.ci_class_key, 
...
from
(
select SRC.infrastructure_change_id, SRC.closed_date, chg.ci_class_key, SRC.scheduled_start_date, SRC.sourceinstance, LKP.dimension_wh_code, 
