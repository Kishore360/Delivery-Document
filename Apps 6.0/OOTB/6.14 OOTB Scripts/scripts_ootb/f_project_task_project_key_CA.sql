SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_task.project_key' ELSE 'SUCCESS' END as Message 
...
on  SRC.prid = SRC2.object_id and SRC.sourceinstance = SRC2.sourceinstance
 ...
 on LKP.row_id  = SRC.prprojectid and LKP.source_id = SRC.sourceinstance   
    where coalesce(LKP.row_key, case when SRC.prprojectid is null then 0 else -1 end) <> coalesce(TRGT.project_key, '');
