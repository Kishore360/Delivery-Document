...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.resource_key' ELSE 'SUCCESS' END as Message
...
on (LKP.row_id  = CONCAT('INTERNAL_CONTACT~',SRC.user_id) and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.user_id is null THEN 0 else '-1' end)<> 
COALESCE(TRGT.resource_key,'');      