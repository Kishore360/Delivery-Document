...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.PERCENT_ROI' ELSE 'SUCCESS' END as Message
...
where  ROUND(COALESCE(SRC2.PRJ_RETURN_ON_INVESTMENT/SRC4.PROJECTED_TOTAL,0),7) <> ROUND(COALESCE(TRGT.percent_roi ,0),7);