...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_facility_request.impact_src_key' ELSE 'SUCCESS' END as Message
 ...
 ON ( CONCAT('IMPACT','~','FACILITIES','~',UPPER(impact)) = LKP.src_rowid 
