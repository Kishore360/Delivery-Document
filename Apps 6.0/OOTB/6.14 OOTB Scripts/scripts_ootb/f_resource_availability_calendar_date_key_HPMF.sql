...
WHERE	(SRC3.WORKING_DAY_FLAG = 'Y') AND SRC4.EVENT_DATE is NULL AND COALESCE(DATE_FORMAT(CONVERT_TZ(SRC3.WORKING_DAY_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') <> COALESCE(TRGT.calendar_date_key,'');