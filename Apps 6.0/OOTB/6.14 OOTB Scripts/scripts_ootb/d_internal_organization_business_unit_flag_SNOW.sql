...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.business_unit_flag' ELSE 'SUCCESS' END as Message
from (select TRGT.business_unit_flag
...
where TRGT.business_unit_flag<>'N'

union all

select TRGT.business_unit_flag
...
where TRGT.business_unit_flag<>'N')inter_org