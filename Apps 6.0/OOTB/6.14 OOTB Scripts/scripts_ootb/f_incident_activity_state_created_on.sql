...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_incident_activity.state_created_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id, COALESCE(TGT.created_on,''), COALESCE(TGT.state_created_on,'')
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
where TGT.task_attribute_wh_name = 'state' and TGT.incident_key <> -1
and COALESCE(TGT.created_on, '') <> COALESCE(TGT.state_created_on,'')
union all
select TGT.row_id, 
COALESCE(base.max_state_created_on,'') sr, COALESCE(TGT.state_created_on,'') tg
from 
(
...
and COALESCE(base.max_state_created_on,'') <> COALESCE(TGT.state_created_on,'')
union all
select TGT.row_id, 
COALESCE(base.max_state_created_on,'') sr, COALESCE(TGT.state_created_on,'') tg
from 
