...
case when count(1)>0 then 'MDS to MDW data validation  failed  for d_configuration_item.row_id' else 'Success' end as message2
from
...
where coalesce(src.ci_name,'') <> coalesce(trgt.row_id,'');
