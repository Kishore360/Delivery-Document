select loc.city_code as city, loc.country_code as country, loc.state_code as state,
...
trim(to_char(coalesce((sum(f.network_time)/sum(f.browser_page_visits)),0), '9,999,999,990D99')) as avg_ntwk_tm,
  ...
  to_char(coalesce(count(distinct f.session_id),0),'9,999,999,990')  active_users
