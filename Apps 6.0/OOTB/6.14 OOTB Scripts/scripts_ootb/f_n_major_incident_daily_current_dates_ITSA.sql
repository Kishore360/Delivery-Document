 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_major_incident_daily.current_dates' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE(SRC.current_dates, '0') <> COALESCE(TRGT.current_dates, '0'); 