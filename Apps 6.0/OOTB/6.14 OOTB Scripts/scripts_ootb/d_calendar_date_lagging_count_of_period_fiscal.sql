...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_calendar_date.lagging_count_of_period' ELSE 'SUCCESS' END as Message
FROM  
(SELECT * FROM #DWH_TABLE_SCHEMA.d_calendar_date WHERE calendar_name <> 'Greogrian Calendar') tgt,
(
select distinct src1.calendar_date, src1.rolling_count_of_period 
...
where (src.rolling_count_of_period-tgt.rolling_count_of_period) <> tgt.lagging_count_of_period