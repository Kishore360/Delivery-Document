SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.changed_by' ELSE 'SUCCESS' END as Message
FROM
...
where COALESCE(SRC2.last_updated_by,'')<>COALESCE(TRGT.changed_by,'')
