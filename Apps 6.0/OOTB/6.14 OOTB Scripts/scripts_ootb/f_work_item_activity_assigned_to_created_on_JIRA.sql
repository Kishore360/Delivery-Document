SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 where tmp.field in ('STATUS', 'ITERATION', 'STORY POINTS', 'TIMEORIGINALESTIMATE', 'BUSINESS VALUE') and coalesce(CASE WHEN tmp.field = 'assignee' THEN CONVERT_TZ(tmp.created, '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>') ELSE NULL END,'') <> coalesce(TRGT.assigned_to_created_on,'');