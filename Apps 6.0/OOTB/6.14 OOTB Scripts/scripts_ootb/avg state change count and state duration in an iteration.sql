...
trim(to_char(coalesce(round(sum(FA.primary_sequence_id)* (1.00)/count(FA.work_item_key)),0), '999,999,999')) avg_state_chang_count,

case when (sum(FA.primary_duration)/86400.00) < 0 
then '-'||trim(to_char(coalesce(round((sum(FA.primary_duration * (-1))/86400.00)/count(FA.work_item_key),1),0), '999,999,990.9'))
else trim(to_char(coalesce(round((sum(FA.primary_duration)/86400.00)/count(FA.work_item_key),1),0), '999,999,990.9')) end avg_state_dur

from #DWH_TABLE_SCHEMA.f_work_item_activity FA
LEFT JOIN 
( SELECT f_work_item_activity.work_item_key, max(f_work_item_activity.primary_sequence_id + 1) AS state_count
  FROM #DWH_TABLE_SCHEMA.f_work_item_activity where primary_sequence_id <> 0
  GROUP BY f_work_item_activity.work_item_key) state_count 
ON state_count.work_item_key = FA.work_item_key
