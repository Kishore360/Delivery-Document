...
WHERE COALESCE(CONCAT('APPLICATION~',SRC.appKey),'')<> COALESCE(TRGT.ROW_ID,''))
...
AND SRC.sourceinstance=TRGT.source_id 
WHERE COALESCE(CONCAT('APPLICATION~',SRC.internalName),'')<> COALESCE(TRGT.ROW_ID,''))
...
AND SRC.sourceinstance=TRGT.source_id 
WHERE COALESCE(CONCAT('APPLICATION~',SRC.name),'')<> COALESCE(TRGT.ROW_ID,'')) ) SQ
