SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 WHERE COALESCE(DATE_FORMAT(CONVERT_TZ(SRC3.SCHED_START_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED')
  <> COALESCE(TRGT.planned_start_on_key,'UNSPECIFIED');