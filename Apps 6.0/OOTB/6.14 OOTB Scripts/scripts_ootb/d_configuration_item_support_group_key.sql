SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_configuration_item.support_group_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.support_group IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.support_group_key ,'')