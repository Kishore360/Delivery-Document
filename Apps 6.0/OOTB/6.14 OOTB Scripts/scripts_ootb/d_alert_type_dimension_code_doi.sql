...
case when count(1)>0 then 'MDS to MDW data validation  failed  for d_alert_type.dimension_code' else 'Success' end as message2
from
...
where coalesce(src.alarmtype,'') <> coalesce(trgt.dimension_code,'');
