...
left join #DWH_TABLE_SCHEMA.d_task_sla D on D.row_id = S.sla
...
left join #DWH_TABLE_SCHEMA.d_problem PROB on PROB.row_id = COALESCE(S.task,'UNSPECIFIED') and upper(task.sys_class_name)  ='PROBLEM'

left join #DWH_TABLE_SCHEMA.d_request REQ on REQ.row_id = COALESCE(S.task,'UNSPECIFIED') and upper(task.sys_class_name)  ='REQUEST'

left join #DWH_TABLE_SCHEMA.d_request_item REQ_ITEM on REQ_ITEM.row_id = COALESCE(S.task,'UNSPECIFIED') and upper(task.sys_class_name)  ='REQUEST_ITEM'

left join #DWH_TABLE_SCHEMA.d_request_task REQ_TASK on REQ_TASK.row_id = COALESCE(S.task,'UNSPECIFIED') and upper(task.sys_class_name)  ='REQUEST_TASK'

left join #DWH_TABLE_SCHEMA.d_problem_task PROB_TASK on PROB_TASK.row_id = COALESCE(S.task,'UNSPECIFIED') and upper(task.sys_class_name)  ='PROBLEM_TASK'
...
)) from #DWH_TABLE_SCHEMA.f_task_sla DWH);
