...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_skill_call_summary.outbound_acd_time' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.agentoutcallstalktime,'')<> COALESCE(TRGT.outbound_acd_time,'')
