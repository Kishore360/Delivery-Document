 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for D_DEVICE.DEVICE_OS_VERSION' ELSE 'SUCCESS' END as Message
 FROM
 ...
 WHERE COALESCE(SRC.osversion, '') <> COALESCE(TRGT.device_os_version,'')