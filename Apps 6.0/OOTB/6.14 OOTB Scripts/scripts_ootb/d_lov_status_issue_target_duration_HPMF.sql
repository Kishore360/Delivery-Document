...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.target_duration' ELSE 'SUCCESS' END as Message
...
where coalesce(TRGT.target_duration,'')<>null;