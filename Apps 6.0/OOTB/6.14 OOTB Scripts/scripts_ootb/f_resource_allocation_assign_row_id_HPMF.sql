...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.row_id' ELSE 'SUCCESS' END as Message
...
where coalesce(concat('ASSIGN','~',SRC.row_id,'~',SRC.WORKING_DAY_DATE),'UNSPECIFIED')<>coalesce(TRGT.row_id,'');