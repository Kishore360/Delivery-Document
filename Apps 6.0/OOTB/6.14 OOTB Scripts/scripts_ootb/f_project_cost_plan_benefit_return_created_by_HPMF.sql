SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.created_by' ELSE 'SUCCESS' END as Message
FROM 
...
inner join(select fs.FINANCIAL_SUMMARY_ID,period_id,sum(pc.PLAN_TOTAL_BSE) as plan_cost,pc.created_by from
...
left join(select fs.FINANCIAL_SUMMARY_ID,period_id,sum(bene.PLANNED_VALUE_BSE) as plan_bene,bene.created_by from
 #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final fs 
...
and COALESCE(planned_benefit.created_by,'')<> COALESCE(TRGT.created_by,'')
union
select TRGT.created_by
from (select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
...
and COALESCE(plan_cost.created_by,'')<> COALESCE(TRGT.created_by,'')
)SQ