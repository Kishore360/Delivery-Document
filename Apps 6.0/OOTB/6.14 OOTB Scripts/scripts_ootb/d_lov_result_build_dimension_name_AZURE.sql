...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_result_build.dimension_name' ELSE 'SUCCESS' END as Message
...
WHERE  coalesce( SRC.result,'')<>coalesce(TRGT.dimension_name,'')