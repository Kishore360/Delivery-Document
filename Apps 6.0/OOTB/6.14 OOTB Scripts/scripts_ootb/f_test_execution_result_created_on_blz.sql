...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.created_on' ELSE 'SUCCESS' END as Message
...
where coalesce(convert_tz(from_unixtime(SRC1.created),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.created_on,'')
