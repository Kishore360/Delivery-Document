select 
CAL.year_name yr, 
DIO_DEPT.organization_name org, 
DIO_GRP.organization_name grp, 
...
case when SUR_STATE.state = 'Completed' then SUR_STATE.state else null end sur_st,
round(avg(F.response_value*1.00),2) avg_inc_csat_scr,
case when max(CAL.lagging_count_of_days) between 0 and 29 then round(avg(F.response_value*1.00),2) else null end avg_inc_csat_scr_lst30,
case when max(CAL.lagging_count_of_days) between 30 and 59 then round(avg(F.response_value*1.00),2) else null end avg_inc_csat_scr_lst30_60
from 
...
group by yr, org, grp, inc, sur_nm, sur_inst_num, inst_takn, sur_st
order by yr, org, grp, inc, sur_nm, sur_inst_num, inst_takn, sur_st