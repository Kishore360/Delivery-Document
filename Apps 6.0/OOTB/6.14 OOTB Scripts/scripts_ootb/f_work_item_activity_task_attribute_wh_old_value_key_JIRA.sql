...
else IF(LENGTH(COALESCE(tmp.from,tmp.fromstring))=0, NULL, TRIM(SUBSTRING_INDEX(COALESCE(tmp.from,tmp.fromstring), ',', -1))) end, 'UNSPECIFIED') and (tmp.sourceinstance = ITE.source_id or ITE.source_id = 0)
