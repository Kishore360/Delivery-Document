SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_problem_task.overdue_duration' ELSE 'SUCCESS' END as Message
...
WHERE case when SRC.over_due_flag ='Y' and LM.dimension_wh_code='OPEN' then
		...
		when SRC.over_due_flag ='Y' and LM.dimension_wh_code='CLOSED' then
