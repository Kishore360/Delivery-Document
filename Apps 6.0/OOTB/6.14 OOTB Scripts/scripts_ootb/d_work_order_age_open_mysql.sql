...
WHERE TIMESTAMPDIFF(DAY,SRC.SUBMIT_DATE,(SELECT CONVERT_TZ(max(lastupdated),<<DW_TARGET_TIME_ZONE>>,<<TENANT_SSI_TIME_ZONE>>) AS lastupdated FROM schneider_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' and etl_run_number=f.etl_run_number))<> f.age
