...
CASE WHEN count(1)>0  THEN 'MDS to DWH data validation failed for d_service.name' ELSE 'SUCCESS' END as Message 
from (
SELECT requestid, sourceinstance, CASE WHEN markasdeleted = 1 THEN 'Y' ELSE 'N' END SOFT_DELETED_FLAG , name
FROM #MDS_TABLE_SCHEMA.hlx_bmc_core_businessservice_final where datasetid = 'BMC.ASSET'

UNION
 
SELECT A.requestid, A.sourceinstance, CASE WHEN A.markasdeleted = 1 THEN 'Y' ELSE 'N' END SOFT_DELETED_FLAG, name FROM #MDS_TABLE_SCHEMA.hlx_bmc_core_businessservice_final A
