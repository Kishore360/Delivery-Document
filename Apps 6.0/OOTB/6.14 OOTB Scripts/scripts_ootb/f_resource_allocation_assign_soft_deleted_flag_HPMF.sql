...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.soft_deleted_flag' ELSE 'SUCCESS' END as Message
from (select SRC23.cdctype,SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,SRC23.resource_id as user_id,temp.WORKING_DAY_DATE,
