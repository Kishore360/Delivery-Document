...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.project_cost_type_key' ELSE 'SUCCESS' END as Message
...
on CONCAT('Project~Cost_Type~',SRC5.ID) = LKP.src_rowid
...
and COALESCE(LKP.row_key,CASE WHEN SRC5.ID IS NULL THEN 0 else '-1' end) <> COALESCE(TRGT.project_cost_type_key,'')
