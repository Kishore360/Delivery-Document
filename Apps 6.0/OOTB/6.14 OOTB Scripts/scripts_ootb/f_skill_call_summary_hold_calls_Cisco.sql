...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_skill_call_summary.hold_calls' ELSE 'SUCCESS' END as Message
...
WHERE -- COALESCE(SRC.incomingcallsonhold,'')
coalesce(SRC.incomingcallsonhold,0)+coalesce(SRC.agentoutcallsonhold,0)
<> COALESCE(TRGT.hold_calls,'')
