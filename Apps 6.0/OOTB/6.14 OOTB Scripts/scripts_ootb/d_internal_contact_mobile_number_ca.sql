...
SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.mobile_number'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
    ...
    FROM
        (SELECT 
        sourceinstance AS srm_sourceinstance, ID, cdctype, sms_email
    FROM
...
WHERE
    COALESCE(src.sms_email, '') <> COALESCE(trg.mobile_number, '');
