select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.met_resolution_sla_on' ELSE 'SUCCESS' END as Message
 
/*select TRGT.row_key, TRGT.incident_number, SQ.category, inc_state.dimension_wh_code, SQ.stage, SQ.outcome_flag, SQ.start_on,
...
coalesce(TRGT.met_resolution_sla_on, '') actual, case when SQ.stage in ('COMPLETED','ACHIEVED') then coalesce(SQ.start_on, '') else  NULL end expected*/
...
and coalesce(TRGT.met_resolution_sla_on, '') <> case when SQ.stage in ('COMPLETED','ACHIEVED') then coalesce(SQ.start_on, '') else  NULL end;