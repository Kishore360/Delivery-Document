...
select a.name as name,a.build_number as build_number,
concat(a.month_name_abbreviated::text, concat(' '::text, "substring"(a.week_start_date_key::text, 7, 8))) wk_nm, 
...
(case when a._test_skip is NUll then Null else (
...
sum(f.AUTOMATED_TEST_TOTAL_COUNT) _test_total,
sum(case  when  f.AUTOMATED_TEST_TOTAL_COUNT =0 then Null
                             ...
                             sum( case when  f.AUTOMATED_TEST_TOTAL_COUNT =0 then null
		                            when  coalesce (f.AUTOMATED_TEST_TOTAL_COUNT,0)<>0 
		                            then  (f.AUTOMATED_TEST_PASS_COUNT)*1.00/f.AUTOMATED_TEST_TOTAL_COUNT *100 end) as  _test_pass,
