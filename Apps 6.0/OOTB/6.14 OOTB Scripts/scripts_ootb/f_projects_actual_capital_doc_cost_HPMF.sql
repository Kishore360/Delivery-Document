...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.actual_capital_doc_cost' ELSE 'SUCCESS' END as Message
...
where  ROUND(coalesce(SRC4.ACTUAL_CAPEX_TOTAL_BSE,0),7)
<> ROUND(COALESCE(TRGT.actual_capital_doc_cost,''),7);
