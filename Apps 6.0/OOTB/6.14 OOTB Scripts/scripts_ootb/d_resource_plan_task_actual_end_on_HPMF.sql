...
left join (select max(timecard_on) as actual_end, resource_plan_key from  #DWH_TABLE_SCHEMA.f_time_card group by resource_plan_key)LKP
on TRGT.row_key=LKP.resource_plan_key
where coalesce(LKP.actual_end,'')<>coalesce(TRGT.actual_end_on,'')