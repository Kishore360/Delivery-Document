...
join (select l.state_key,l.incident_key,l.primary_sequence_id
...
where case when TRGT_A.task_attribute_wh_name ='state'then TRGT_A.task_attribute_wh_new_value_key 
when TRGT_A.task_attribute_wh_name = 'assigned_to' then TRGT_S.state_key 
else null
end <> TRGT_A.state_key ;
