SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'data validation failed for f_problem_task_activity.primary_duration'  ELSE 'SUCCESS' END as Message
FROM (
...
join #DWH_TABLE_SCHEMA.f_problem_task F on F.problem_task_key = d.row_key
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON F.state_src_key = br.src_key AND br.dimension_class = 'STATE~PROBLEM_TASK'
...
WHERE A.primary_duration <> (case when A.problem_task_key = -1 then null when A.primary_duration1 < 0 then 0 else A.primary_duration1 end)