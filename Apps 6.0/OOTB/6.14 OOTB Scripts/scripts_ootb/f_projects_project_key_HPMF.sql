...
ON SRC.PROJECT_ID=SRC3.PROJECT_ID AND SRC.sourceinstance=SRC3.sourceinstance 

left join  #DWH_TABLE_SCHEMA.f_project TRGT
on SRC.project_id=TRGT.row_id
and SRC.sourceinstance=TRGT.source_id
left join #DWH_TABLE_SCHEMA.d_project SRC2
on SRC2.row_id = SRC.project_id 
and SRC2.source_id = SRC.sourceinstance
where coalesce(SRC2.row_key,'')<>coalesce(TRGT.project_key,'');