...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.project_task_key' ELSE 'SUCCESS' END as Message
from (select SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,SRC23.resource_id as user_id,temp.WORKING_DAY_DATE,
...
LEFT JOIN (select * from #DWH_TABLE_SCHEMA.d_project_task) LKP
ON LKP.ROW_ID = SRC.task_id and LKP.source_id=SRC.sourceinstance
WHERE COALESCE(LKP.row_key,'0')<>COALESCE(TRGT.project_task_key,'')  