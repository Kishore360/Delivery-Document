SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.calendar_date_key' ELSE 'SUCCESS' END as Message
...
where  date_format(COALESCE(convert_tz(E.start_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')<> COALESCE(TRGT.calendar_date_key,'')