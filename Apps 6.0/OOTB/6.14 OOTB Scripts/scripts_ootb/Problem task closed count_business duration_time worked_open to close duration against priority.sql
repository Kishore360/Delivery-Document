...
round(coalesce(sum(distinct F.open_to_close_duration / 86400.00),0),1) open_2_close_dur,

case when count(case when state.state in ('CLOSED', 'RESOLVED') then F.problem_task_key else null end) = 0 then null
when count(case when state.state in ('CLOSED', 'RESOLVED') then F.problem_task_key else null end) <> 0 and sum(F.time_worked / 86400.00) is null then 0
else round(sum(F.time_worked / 86400.00)/count(case when state.state in ('CLOSED', 'RESOLVED') then F.problem_task_key else null end),1) end avg_tim_wkd
