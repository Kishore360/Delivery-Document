...
(select MF.*,round(AVG(MF.interactionDuration),5) int_dur from #MDS_TABLE_SCHEMA.Mobile_final MF
...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where coalesce(mobile.int_dur ,'')  <> coalesce(round(TRGT.request_response_time,5),'')
 union
 select TRGT.row_id 
 from (select me.*, round(avg(me.responseTime),5) int_dur from #MDS_TABLE_SCHEMA.MobileRequestError_final me group by sessionId) MobileRequestError
 left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(MobileRequestError.int_dur,'') <> coalesce(round(TRGT.request_response_time,5),''))SQ
 ;