...
count(case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('CLOSED', 'RESOLVED') and D.met_sla_flag = 'Y' then F.incident_key else null end) rslvd_inc,

case when count(distinct case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('CLOSED', 'RESOLVED') then F.incident_key else null end) = 0 then null

else concat(cast(round((sum(case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('CLOSED', 'RESOLVED') and D.met_sla_flag = 'Y' then 1.0000 else 0.0000 end)/
count(distinct case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('CLOSED', 'RESOLVED') then F.incident_key else null end)*1.0000)*100.0000,0) as varchar),'%') end rslvd_sla_met_prct
