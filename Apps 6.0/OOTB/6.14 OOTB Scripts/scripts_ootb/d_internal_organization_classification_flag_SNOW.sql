...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.classification_flag' ELSE 'SUCCESS' END as Message
from (select (TRGT.classification_flag)
...
where (TRGT.classification_flag)<>'N'

union all

select (TRGT.classification_flag)
...
where (TRGT.classification_flag)<>'N')inter_org