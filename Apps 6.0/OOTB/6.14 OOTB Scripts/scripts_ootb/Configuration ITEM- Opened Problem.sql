select dci.name  , dci.category , dci.ci_type ,   dci.class ,
'' as metrics, 
count(fp.row_key)as "Opened_Problem"
...
group by dci.row_key,dci.name , dci.category , dci.ci_type ,   dci.class
order by dci.row_key,lower(dci.name) ,lower(dci.category) , lower(dci.ci_type) ,  lower(dci.class) asc 