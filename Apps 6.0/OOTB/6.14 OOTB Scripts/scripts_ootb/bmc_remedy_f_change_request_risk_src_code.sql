...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for risk_src_code' ELSE 'SUCCESS' END as Message
 ...
 WHERE COALESCE(SRC.risk_level,'') <> COALESCE(TRGT.risk_src_code,'')
