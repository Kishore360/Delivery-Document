...
/*
select 
COALESCE(CASE WHEN SRC.authorRef = SRC.assignee_id THEN 'Y' ELSE 'N' END,'') ,COALESCE(TRGT.assignee_flag,'')*/
...
where COALESCE(CASE WHEN SRC.authorRef = SRC.assignee_id THEN 'Y' ELSE 'N' END,'') <> COALESCE(TRGT.assignee_flag,'');