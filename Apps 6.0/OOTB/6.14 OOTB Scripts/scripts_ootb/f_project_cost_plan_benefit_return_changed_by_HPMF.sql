...
FROM 
(select TRGT.changed_by from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
and COALESCE(planned_benefit.last_updated_by,'') <> COALESCE(TRGT.changed_by,'')
union
select TRGT.changed_by from(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
on SRC.PRJ_FINANCIAL_SUMMARY_ID=fin_summary.FINANCIAL_SUMMARY_ID 
...
and COALESCE(plan_cost.last_updated_by,'')<> COALESCE(TRGT.changed_by,'')
)SQ
