...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.change_credit_score' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(0,'') <> COALESCE(round(TRGT.change_credit_score,0),'')
