...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.changed_on' ELSE 'SUCCESS' END as Message
...
where 
coalesce(convert_tz(SRC.last_update_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
