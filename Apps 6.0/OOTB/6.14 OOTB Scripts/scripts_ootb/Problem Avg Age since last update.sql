...
round(case when sum(case when dp.backlog_flag='Y' and fp.row_key is not NULL then 1 else 0 end) = 0 then NULL else sum(fp.dormancy_age/ 86400.00)
/sum(case when dp.backlog_flag='Y' and fp.row_key is not NULL then 1 else 0 end) end,1) as "Avg Age since last update"
...
order by dio.row_key asc,dio.organization_name 