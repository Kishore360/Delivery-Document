...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.changed_on' ELSE 'SUCCESS' END as Message
from (select SRC23.last_update_date,SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,
...
where COALESCE(convert_tz(SRC.last_update_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<> 
