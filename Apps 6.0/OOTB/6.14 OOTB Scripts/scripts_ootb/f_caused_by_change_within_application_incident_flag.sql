...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.within_application_incident_flag' ELSE 'SUCCESS' END as Message 
from(select (case when (inc.cmdb_ci=chg.cmdb_ci) then 'Y' else 'N' end),coalesce(TRGT.within_application_incident_flag,'') from 
...
where UPPER(dimension_wh_code)='CLOSED' and (case when (inc.cmdb_ci=chg.cmdb_ci) then 'Y' else 'N' end)<> coalesce(TRGT.within_application_incident_flag,'')

union all

select coalesce(TRGT.within_application_incident_flag,''),'N'
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(TRGT.within_application_incident_flag,'')<>'N')ca