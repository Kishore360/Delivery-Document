...
--CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.incidents_caused_outside_application' ELSE 'SUCCESS' END as Message
--from
--(select change_request,change_request_key as change_request_key,count(incidents_for_diff_application) as incidents_for_diff_application
--from
--(select chgd.change_request_number as change_request,chgd.row_key as change_request_key,
--case when (UPPER(map.dimension_name)='CLOSED' and inc.opened_on_key>=chg.closed_on_key and inc.caused_by_change_key<>0 and chg.application_key<>inc.application_key) then (inc.incident_key) else null end as incidents_for_diff_application
--
...
--on cal.row_key=chg.closed_on_key)incidents_outs_application
--group by incidents_outs_application.change_request_key)SRC
--left join #DWH_TABLE_SCHEMA.d_change_failure chg_failure
--on chg_failure.change_request_key=SRC.change_request_key
--where SRC.incidents_for_diff_application<>chg_failure.incidents_caused_outside_application;


-- using flag from d_incident. Count of Incidents where Caused by Change Numerify Flag = 'Y'
select chg_d.row_key,SRC.incidents_caused_outside_application,TRGT.incidents_caused_outside_application 
...
case when ((inc_d.caused_by_change_numerify_flag='N') and (inc_d.caused_by_change_numerify_key<>0)) then count(inc_d.caused_by_change_numerify_flag) else 0 end as incidents_caused_outside_application 
...
where coalesce(SRC.incidents_caused_outside_application,0)<>coalesce(TRGT.incidents_caused_outside_application,0);
select * from #DWH_TABLE_SCHEMA.d_incident where caused_by_change_numerify_key=790;