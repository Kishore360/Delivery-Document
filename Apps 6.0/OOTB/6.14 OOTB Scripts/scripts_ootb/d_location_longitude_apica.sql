...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.longitude' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.longitude,'')<>coalesce(TRGT.longitude,'')  -- collate utf8_unicode_ci;
