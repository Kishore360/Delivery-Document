...
FROM (select * from  #DWH_TABLE_SCHEMA.d_change_failure where current_flag='Y' and row_id not in (0,-1))TGT
left join (
...
where coalesce(TGT.ci_manufacturer,'') <>  coalesce(lkp_ci.manufacturername,'UNSPECIFIED')