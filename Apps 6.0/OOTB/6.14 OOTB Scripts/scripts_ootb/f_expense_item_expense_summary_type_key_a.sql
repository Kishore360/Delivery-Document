...
AND LKP.dimension_class ='EXPENSE_ITEM~SUMMARY_TYPE' 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.summary_type IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.expense_summary_type_key,'')and SRC.asset is not null;
