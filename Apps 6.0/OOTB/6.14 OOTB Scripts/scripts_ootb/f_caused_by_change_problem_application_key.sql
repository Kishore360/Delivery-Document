...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.outage_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(prob_f.application_key,0),coalesce(TRGT.problem_application_key,'')
from 
...
left join (select * from #DWH_TABLE_SCHEMA.f_problem where soft_deleted_flag='N')prob_f
on prob_f.row_id=coalesce(prob.sys_id,'UNSPECIFIED') and prob_f.source_id=prob.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and  coalesce(prob_f.application_key,0)<>coalesce(TRGT.problem_application_key,'')

union all

select coalesce(out_d.row_key,case when outage.sys_id is null then 0 else -1 end),coalesce(TRGT.outage_key,'')
...
left join (select * from #DWH_TABLE_SCHEMA.d_lov_map where upper(wh_dimension_class) like '%WH_STATE~CHANGE_REQUEST%' and UPPER(dimension_wh_code)='CLOSED')map
ON CONCAT('STATE','~','CHANGE_REQUEST','~',UPPER(chg.state)) = map.row_id and chg.sourceinstance=map.source_id
left join (select * from #DWH_TABLE_SCHEMA.d_outage where soft_deleted_flag='N')out_d
...
where UPPER(map.dimension_wh_code)='CLOSED' and TRGT.problem_application_key<>0)ca