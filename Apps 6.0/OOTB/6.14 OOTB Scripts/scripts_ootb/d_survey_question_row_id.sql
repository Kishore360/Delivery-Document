...
case when count(1) > 0 then 'MDS to DWH data validation failed for row_id between survey_question_new_final & asmt_metric_final to d_survey_question' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
ifnull(A.sys_id,'') <> ifnull(DWH.row_id,'')
) SQ;