...
on CONCAT('DEMAND~CATEGORY~',upper(SRC1.prop_project_class_code)) = lov.row_id
where coalesce(lov.row_key, case when SRC1.prop_project_class_code is null then 0 else -1 end)<>coalesce(TRGT.demand_category_src_key,0)