...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_task.opened_by_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(inc.opened_by_key,prob.opened_by_key,prob_tas.opened_by_key,req_item.opened_by_key
,req_tas.opened_by_key,req.opened_by_key,chan_req.opened_by_key,chan_tas.opened_by_key
,LKP.row_key
,CASE WHEN SRC.opened_by IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.opened_by_key ,'')
