...
case when D.is_active_flag = 'Y' and state.state = 'In Progress' then 'Y' else 'N' end actv,
D.is_top_flag top_proj,
D.is_upcoming_flag upcoming,
D.is_in_budget_flag in_budget,
D.is_in_effort_flag in_efrt,
D.is_in_schedule_flag in_sch,
case when sum(F.planned_duration) = 0 then null
when sum(F.planned_duration) <> 0 
then trim(to_char(coalesce(round(((sum(datediff(day, F.planned_start_on, F.actual_start_on))/sum(F.planned_duration))*100),0),0),'9999999999'))||'%'
else trim(to_char(0, '9999999999'))||'%' end prct_Delay_in_strt,
concat(round(coalesce((max(F.actual_percent_complete/100)*100),0),0),'%') act_prct,
concat(round(coalesce(max(F.planned_percent_complete)*100,0),0),'%') plnd_prct,
sum(case when D.is_active_flag = 'Y' and state.state = 'In Progress' then 1 else 0 end) No_Of_Active_Project_task,
...
round(sum(case when D.is_upcoming_flag = 'Y' and D.task_type in ('TASK', 'PROJECT') then 1 else 0 end),0) no_of_upcoming_proj_task
from 
(
 SELECT f.*
...
group by D_P.name , D_P.row_key,Portfolio_category ,D.project_name , D.row_key,D2.project_name, D2.row_key, actv,
top_proj, upcoming, in_budget, in_efrt, in_sch
