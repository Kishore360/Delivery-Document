...
trim(to_char(coalesce(round((count(distinct DCMP.row_key)),0),0), '9,999,999,990')) cmp_count
...
group by D.work_item_number, type.dimension_name, INTC_ASGN.full_name, status.dimension_name, priority.dimension_name
order by upper(D.work_item_number), upper(type.dimension_name), upper(INTC_ASGN.full_name), upper(status.dimension_name), upper(priority.dimension_name)