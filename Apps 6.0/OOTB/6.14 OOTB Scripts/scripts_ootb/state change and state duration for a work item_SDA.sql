...
count(CASE WHEN (FA.task_attribute_wh_next_value_key <> 0 and (status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS' or cal_mon.lagging_count_of_month between 0 and 12)) THEN FA.row_key ELSE NULL END) AS state_chang_count,

case when (sum(FA.primary_duration)/86400.00) < 0 
then '('||trim(to_char(coalesce(round((sum(FA.primary_duration * (-1))/86400.00),1),0), '999,999,990.0'))||')'
else trim(to_char(coalesce(round((sum(FA.primary_duration)/86400.00),1),0), '999,999,990.0')) end state_dur
...
group by D.work_item_number, D.row_key
order by D.work_item_number, D.row_key
