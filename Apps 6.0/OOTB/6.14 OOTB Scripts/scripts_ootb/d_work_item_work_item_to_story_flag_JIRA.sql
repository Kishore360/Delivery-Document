...
/*select SQ.id,SQ.sourceinstance, SQ.work_item_to_story_flag val,
if(SQ.work_item_to_story_flag like '%Y%', 'Y', 'N') sr,
coalesce(TRGT.work_item_to_story_flag,'') tgt*/
from 
(
select SRC.id, SRC.sourceinstance, group_concat(case when typ.dimension_wh_code in ('TASK') then 
case when isu_lnk.issueid is not null and (isu_lnk.inward_issue_type in ('STORY') or isu_lnk.outward_issue_type in ('STORY')) then 'Y' else 'N' end
