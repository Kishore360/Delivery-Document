...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.city_code' ELSE 'SUCCESS' END as Message
  FROM ( SELECT ctm_people.SourceInstance,value,Site_City,Home_City,Default_City,person_id from #MDS_TABLE_SCHEMA.hlx_ctm_people_final ctm_people                  
            LEFT OUTER JOIN
...
WHERE COALESCE(case WHEN SRC.value = 'Office-Based Employee' THEN SRC.Site_City 
WHEN  SRC.value = 'Home-Based Employee' THEN Home_City  else SRC.Default_City end
,'UNSPECIFIED')<> COALESCE(TRGT.city_code,'UNSPECIFIED')