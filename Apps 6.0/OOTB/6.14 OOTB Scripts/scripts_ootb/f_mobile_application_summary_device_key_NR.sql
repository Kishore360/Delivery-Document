SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM 
 (select TRGT.row_id from #MDS_TABLE_SCHEMA.Mobile_final mobile LEFT OUTER JOIN #MDS_TABLE_SCHEMA.MobileRequestError_final merr
...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.device_key,'')
 union
 select TRGT.row_id 
 from #MDS_TABLE_SCHEMA.MobileRequestError_final MobileRequestError
left join #DWH_TABLE_SCHEMA.d_device lov
 on (CONCAT(MobileRequestError.device,'~',MobileRequestError.deviceModel,'~',MobileRequestError.osVersion, '~',MobileRequestError.deviceType) = lov.row_id and lov.source_id =MobileRequestError.sourceinstance)
 left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.device_key,''))SQ
 ;