...
ON F.row_id =TRGT.row_id and F.source_id=TRGT.source_id
 
where TRGT.row_key not in (0,-1) and TRGT.soft_deleted_flag = 'N' and
coalesce(CASE WHEN F.reassignment_count > 0 THEN 'Y' ELSE 'N' end,'N' )<>coalesce(TRGT.multiple_assignment_flag,'');