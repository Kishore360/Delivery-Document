...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster.cluster_description' ELSE 'SUCCESS' END as Message
 FROM (select 
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.cluster_description,'')<>coalesce(TRGT.cluster_description,'');	
