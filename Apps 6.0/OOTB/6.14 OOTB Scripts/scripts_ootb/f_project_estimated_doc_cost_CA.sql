...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.ESTIMATED_DOC_COST' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC2.PLANNED_CST_TOTAL,0) <> COALESCE(TRGT.ESTIMATED_DOC_COST,'')								