...
to_char((sum(coalesce(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) then F.time_spent else 0.0 end, 0)) / 3600.0),'FM999,999,999,999,990D0')as time_spent_created, -- based on the opened date
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 3600.0), 'FM999,999,999,999,990D0') as time_spent_closed -- based on the closed date
...
D.proposed_flag = 'Y')
group by INTC_ASGN.full_name, type.wi_type
order by upper(INTC_ASGN.full_name), upper(type.wi_type)
