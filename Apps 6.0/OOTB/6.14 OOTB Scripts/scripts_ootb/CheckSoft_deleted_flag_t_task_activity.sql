...
FROM (select SRC.cdctype,SRC.sys_id,SRC.sourceinstance from ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_audit_bwd_delta WHERE CDCTYPE<>'D') SRC 
...
AND SRC.jobid =(select max(jobid) from ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_audit_bwd_delta WHERE CDCTYPE<>'D')
