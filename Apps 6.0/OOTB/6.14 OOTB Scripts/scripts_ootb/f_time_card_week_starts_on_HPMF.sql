SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.week_starts_on' ELSE 'SUCCESS' END as Message
...
left join (select * from  #MDS_TABLE_SCHEMA.hp_tm_time_sheet_lines_final) SRC2
 ...
 left join(select * from  #MDS_TABLE_SCHEMA.hp_ktmg_periods_final)SRC7
 on SRC1.period_id=SRC7.period_id
...
WHERE coalesce(CONVERT_TZ(SRC7.start_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.WEEK_STARTS_ON,'')
