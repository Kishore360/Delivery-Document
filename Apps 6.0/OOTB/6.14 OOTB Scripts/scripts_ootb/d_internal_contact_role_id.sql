...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.role_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('ROLE~',ROLE) ,'')<> COALESCE(TRGT.role_id ,'')
