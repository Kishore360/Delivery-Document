...
order by D.work_item_number, D.row_id, upper(PROJ.project_name), upper(D.proposed_flag), upper(D.in_progress_flag), upper(D.reopened_flag), upper(D.overdue_flag), upper(D.multiple_assignment_flag)
