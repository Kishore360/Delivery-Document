...
coalesce(SRC.work_start,SRC.start_date,SRC.opened_at) AS chg_dt
...
left outer join
(
select sys_id,sourceinstance,change_request, sys_created_on as task_dt from #MDS_TABLE_SCHEMA.change_task_final where change_request is not null 
) task
