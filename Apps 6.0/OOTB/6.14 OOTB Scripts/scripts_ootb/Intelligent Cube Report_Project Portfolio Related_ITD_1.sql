...
sum(case when D.is_active_flag = 'Y' and state.state in ('In Progress') and D.task_type in ('PROJECT', 'TASK') then 1 else 0 end) No_Of_Active_Project_Task,
sum(case when D.is_top_flag = 'Y' and state.state in ('In Progress') and D.is_active_flag = 'Y' and D.task_type in ('PROJECT') then 1 else 0 end) No_of_Active_top,
sum(case when D.task_type = 'PROJECT' then 1 else 0 end) Project_Count,
sum(case when D.task_type = 'TASK' then 1 else 0 end) Task_Count,
...
else (Sum (case when D.is_milestone_flag = 'Y' and D.is_milestone_slipped_flag = 'Y' then 1.0000 else 0.0000 end)/
sum(case when D.is_milestone_flag = 'Y' then 1.00 else 0.00 end)) end)*100,0),'%') prct_Project_MS_slipped,
concat(round((sum(case when D.is_in_schedule_flag = 'N' then 1.0000 else 0.0000 end)/sum(case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end))*100,0),'%') No_of_proj_task_behind_sch,
concat(round((sum(case when D.is_in_effort_flag = 'N' then 1.0000 else 0.0000 end)/sum(case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end))*100,0),'%') No_of_proj_task_excess_efrt,
concat(round((sum (case when D.is_in_budget_flag = 'Y' and D.is_in_schedule_flag = 'Y' and D.task_type in ('PROJECT', 'TASK') 
then 1.0000 else 0.0000 end)/sum (case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end))*100,0),'%') Success_ratio,
concat(round((1.0000-(sum (case when D.is_in_budget_flag = 'Y' and D.is_in_schedule_flag = 'Y' and D.task_type in ('PROJECT', 'TASK') 
then 1.0000 else 0.0000 end)/sum (case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end)))*100,0),'%') Failure_ratio
...
group by D_P.row_key, Portfolio_name, Portfolio_category
