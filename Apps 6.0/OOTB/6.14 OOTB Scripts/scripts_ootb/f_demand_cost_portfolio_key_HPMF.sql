...
left join(select * from #DWH_TABLE_SCHEMA.d_portfolio)portfolio
on SRC1.prop_portfolio_id=portfolio.row_id
and TRGT.source_id=portfolio.source_id
WHERE planned_benefit.period_id is not null
and COALESCE(portfolio.row_key,CASE WHEN SRC1.prop_portfolio_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key ,0)
union
select TRGT.portfolio_key
from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
