...
WHERE date_format(COALESCE(convert_tz(SRC.PRAVAILFINISH,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),
convert_tz(SCHEDULE_FINISH,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')<>
 COALESCE(TRGT.planned_end_on_key,'')
