...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.application_key' ELSE 'SUCCESS' END as Message 
from(select 
...
left join (select * from #DWH_TABLE_SCHEMA.d_application where soft_deleted_flag='N')app
on app.row_id=coalesce(concat('APPLICATION~',chg.cmdb_ci),'UNSPECIFIED') and app.source_id=chg.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(app.row_key,case when chg.cmdb_ci is null then 0 else -1 end)<> coalesce(TRGT.application_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(app.row_key,case when chg.cmdb_ci is null then 0 else -1 end)<> coalesce(TRGT.application_key,''))ca