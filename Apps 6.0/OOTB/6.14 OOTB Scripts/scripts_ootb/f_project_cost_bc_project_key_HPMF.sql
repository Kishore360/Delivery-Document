...
on COALESCE(SRC1.project_id,'')=COALESCE(LKP.row_id,'')
and COALESCE(SRC.sourceinstance,'')=COALESCE(LKP.source_id,'')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC1.project_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.project_key,'')
