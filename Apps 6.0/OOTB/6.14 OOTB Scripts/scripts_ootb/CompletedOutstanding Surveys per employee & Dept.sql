select 
DIC.full_name emp_name, 
SUR.survey_name sur_nm,
DIO_DEPT.organization_name dept,
ASGN_TO.full_name asn_to, 
case when max(SUR_STATE.state) != 'Completed' then null else count(distinct case when SUR_STATE.state = 'Completed' then F.rowkey else null end) end completed_surveys,
-- case when max(SUR_STATE.state) = 'Requested' then count(distinct case when SUR_STATE.state != 'Completed' then F.rowkey else null end) else null end outstanding_surveys
case when max(SUR_STATE.state) != 'Requested' then null else count(distinct case when SUR_STATE.state = 'Requested' then F.rowkey else null end) end outstanding_surveys
from 
...
NULL AS response, a.incident_key as inc_key, a.row_key rowkey, a.state_src_key state_key
  ...
  WHERE (a.survey_instance_key = 0 OR a.survey_instance_key = -1) AND a.soft_deleted_flag = 'N' AND i.soft_deleted_flag = 'N' AND q.exclude_flag = 'N'
UNION 
 ...
 a.state_src_key state_key
  ...
  WHERE a.survey_instance_key <> 0 AND a.survey_instance_key <> -1 AND a.soft_deleted_flag = 'N' AND i.soft_deleted_flag = 'N' AND r.soft_deleted_flag = 'N'
) F
left join 
(
SELECT a.*, upper(a.task_type) AS task_typ, i.*, r.response_value AS response_value, 
...
on DIO_DEPT.row_key = F.taken_by_department_key
join
(
select iog.*
...
group by emp_name, dept, sur_nm, asn_to
order by emp_name, dept, sur_nm, asn_to