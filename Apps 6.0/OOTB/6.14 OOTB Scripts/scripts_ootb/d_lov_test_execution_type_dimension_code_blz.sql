...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_type.dimension_code' ELSE 'SUCCESS' END as Message 
FROM
...
WHERE COALESCE(CASE WHEN SRC.configuration_testMode='http' THEN 'URL_TEST' WHEN SRC.configuration_testMode='script' 
THEN  CONCAT(UPPER(SRC.configuration_scriptType),'_TEST') END,'') <> COALESCE(TRGT.dimension_code,'')
