SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_availability.calendar_date_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN (select * from #DWH_TABLE_SCHEMA.f_resource_availability)TRGT   
 ...
 WHERE COALESCE(date_format(slice_date,'%Y%m%d'),'')<> COALESCE(TRGT.calendar_date_key,'');    
