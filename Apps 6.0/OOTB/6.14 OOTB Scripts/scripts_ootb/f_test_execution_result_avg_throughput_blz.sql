...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.avg_throughput' ELSE 'SUCCESS' END as Message
...
where COALESCE(SRC.avgthroughput,'')<>round(coalesce(TRGT.avg_throughput,''),1)
