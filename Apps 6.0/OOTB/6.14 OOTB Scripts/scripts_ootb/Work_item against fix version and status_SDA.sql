...
group by PROJ.project_name, D.work_item_number, D.row_id, type.wi_type, pv.row_key, pv.name, status.dimension_name, resolution.wi_res, D.due_on, D.overdue_flag
order by upper(PROJ.project_name), D.work_item_number, D.row_id, upper(type.wi_type), pv.row_key, upper(pv.name), upper(status.dimension_name), upper(resolution.wi_res), D.due_on, upper(D.overdue_flag)