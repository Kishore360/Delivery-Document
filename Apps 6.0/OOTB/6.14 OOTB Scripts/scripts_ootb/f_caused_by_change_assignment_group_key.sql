...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.assignment_group_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(int_org.row_key,case when chg.assignment_group is null then 0 else -1 end),coalesce(TRGT.assignment_group_key,'') from 
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_internal_organization int_org
ON coalesce(concat('GROUP~',chg.assignment_group),'UNSPECIFIED') = int_org.row_id  AND chg.sourceinstance = int_org.source_id
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(int_org.row_key,case when chg.assignment_group is null then 0 else -1 end)<> coalesce(TRGT.assignment_group_key,'')

union all

select coalesce(int_org.row_key,case when chg.assignment_group is null then 0 else -1 end),coalesce(TRGT.assignment_group_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(int_org.row_key,case when chg.assignment_group is null then 0 else -1 end)<> coalesce(TRGT.assignment_group_key,''))ca