...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.city_code' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.city,''),coalesce(TRGT.city_code,'')
...
where customer=1 and coalesce(SRC.city,'')<>coalesce(TRGT.city_code,'');