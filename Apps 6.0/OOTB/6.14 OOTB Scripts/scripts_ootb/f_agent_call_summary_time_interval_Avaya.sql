...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.time_interval' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.f_skill_call_summary TRGT 
 ...
 WHERE COALESCE(SRC.intrvl,'') <> COALESCE(TRGT.time_interval,'')