...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ACTUAL_OPERATING_DOC_COST' ELSE 'SUCCESS' END as Message
...
where  coalesce(SRC4.ACTUAL_OPEX_TOTAL_BSE,0) <> COALESCE(TRGT.actual_operating_doc_cost,0);