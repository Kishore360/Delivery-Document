select distinct Resource,project, 
...
group by Resource,project,resource_key
)
-- where resource not in ('UNKNOWN')
order by upper(resource),upper(project), resource_actual_hours, resource_allocated_hour
