...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_response.created_on' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(convert_tz(A.sys_created_on, '<<TENANT_SSI_TIME_ZONE>>', '<<DW_TARGET_TIME_ZONE>>'),'') <> ifnull(DWH.created_on,'')
)SQ;
