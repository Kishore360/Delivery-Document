SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_burn_down.business_value_burn' ELSE 'SUCCESS' END as Message
...
FROM (
 select SR.work_item_key, SR.source_id, SR.burn_on_key burn_on_key, max(coalesce(LKP.primary_sequence_id,0)) max_primary_sequence_id, COALESCE(LKP.task_attribute_wh_new_value,0) business_value_burn, SR.sys_id
 from #STG_TABLE_SCHEMA.burn_down_temp SR
 left join #DWH_TABLE_SCHEMA.f_work_item_activity LKP
 on SR.work_item_key = LKP.work_item_key
 and SR.source_id = LKP.source_id
 and LKP.task_attribute_wh_name = 'BUSINESS VALUE'
...
WHERE TRGT.soft_deleted_flag = 'N' and cast(COALESCE(SRC.business_value_burn,0) as decimal(28,10)) <> COALESCE(TRGT.business_value_burn ,'');