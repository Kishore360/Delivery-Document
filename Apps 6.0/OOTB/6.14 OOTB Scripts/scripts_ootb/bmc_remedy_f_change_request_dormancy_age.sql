select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_request.dormancy_age' ELSE 'SUCCESS' END as Message
...
THEN TIMESTAMPDIFF(SECOND,coalesce(SRC.last_modified_date,0), convert_tz(df.lastupdated, '<<DW_TARGET_TIME_ZONE>>', '<<TENANT_SSI_TIME_ZONE>>') )
