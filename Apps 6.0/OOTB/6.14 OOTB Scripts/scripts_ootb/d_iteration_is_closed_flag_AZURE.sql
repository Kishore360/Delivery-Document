...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_iteration.is_closed_flags' ELSE 'SUCCESS' END as Message
FROM (select 
...
where COALESCE(case when SRC.timeFrame = 'past' then 'Y' else 'N' end ,'') <> COALESCE(TRGT.is_closed_flag,'')