...
case when count(1) > 0 then 'MDS to DWH validation failed for d_change_request.state_count' else 'SUCCESS' end as Message
...
left join (
select base.change_request_key, base.source_id, 
...
group by base.change_request_key, base.source_id 
) state
on state.change_request_key = TRGT.row_key and state.source_id = TRGT.source_id
