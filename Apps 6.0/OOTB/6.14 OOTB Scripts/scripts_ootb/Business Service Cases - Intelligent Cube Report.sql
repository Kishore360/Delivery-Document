select 
SRVC.name name, 
crit.dimension_name critical, 
used.dimension_name used_for,
SRVC.version version,
sum(case when F.case_key is not null then 1 else 0 end) opened_cases
...
group by name, critical, used_for, version
order by name
--limit 20