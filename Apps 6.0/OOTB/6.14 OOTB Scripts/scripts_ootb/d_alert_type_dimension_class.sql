...
from
(select distinct 'TYPE~ALERT', alarmtype,sourceinstance  from #MDS_TABLE_SCHEMA.alarm_ext_final where alarmtype is not null) src
left outer join #DWH_TABLE_SCHEMA.d_lov trgt
on trgt.row_id =CONCAT('TYPE~ALERT~',src.alarmType)
and trgt.source_id = src.sourceinstance
where coalesce('TYPE~ALERT','') <> coalesce(trgt.dimension_class,'');