...
join(select chg_assoc.request_id02, chg_assoc.sourceinstance, min(chg_assoc.submit_date) as latest_date, assoc_type.value 
from  #MDS_TABLE_SCHEMA.hlx_chg_associations_final chg_assoc
...
on chg_assoc.association_type01 = assoc_type.value and chg_assoc.sourceinstance = assoc_type.sourceinstance

join (SELECT enum_values.value,enum_values.enumId, enum_values.sourceinstance
FROM  #MDS_TABLE_SCHEMA.hlx_field_enum_values_final enum_values
JOIN  #MDS_TABLE_SCHEMA.hlx_field_final field ON (enum_values.fieldId=field.fieldId AND enum_values.schemaId=field.schemaId and enum_values.sourceinstance = field.sourceinstance)
JOIN  #MDS_TABLE_SCHEMA.hlx_arschema_final schma ON (field.schemaId=schma.schema_Id and field.sourceinstance = schma.sourceinstance and schma.overlaygroup=0 ) where field.fieldName ='Request Type01' AND schma.NAME='CHG:Associations' and enum_values.value = 'Configuration Item') req_type 
on chg_assoc.request_type01 = req_type.value and chg_assoc.sourceinstance = req_type.sourceinstance

where chg_assoc.data_set_id01 = 'BMC.ASSET' 
group by chg_assoc.request_id02, chg_assoc.sourceinstance,value
)SRC
...
left join  #MDS_TABLE_SCHEMA.hlx_bmc_core_businessservice_final lkp_service_ci on base.latest_ci = lkp_service_ci.reconciliationidentity 
and base.sourceinstance = lkp_service_ci.sourceinstance and lkp_service_ci.datasetid = 'BMC.ASSET'

left join  #DWH_TABLE_SCHEMA.d_service lkp_d_service
on coalesce(concat('BUSINESS_SERVICE~',lkp_service_ci.requestid),'UNSPECIFIED') = lkp_d_service.row_id

WHERE COALESCE(lkp_d_service.row_key,CASE WHEN lkp_service_ci.requestid IS NULL THEN 0 else '-1' end)<> COALESCE(TGT.business_service_key,'')