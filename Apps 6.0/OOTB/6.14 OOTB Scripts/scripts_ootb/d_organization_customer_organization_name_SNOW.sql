...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.organization_name' ELSE 'SUCCESS' END as Message
...
where customer=1 and coalesce(SRC.name,'')<>coalesce(TRGT.organization_name,'');