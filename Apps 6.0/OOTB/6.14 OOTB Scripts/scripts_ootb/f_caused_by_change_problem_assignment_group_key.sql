...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.problem_assignment_group_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(int_org.row_key,case when prob.assignment_group is null then 0 else -1 end),coalesce(TRGT.problem_assignment_group_key,'')
from 
...
ON coalesce(concat('GROUP~',prob.assignment_group),'UNSPECIFIED') = int_org.row_id  AND prob.sourceinstance = int_org.source_id
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(int_org.row_key,case when prob.assignment_group is null then 0 else -1 end)<> coalesce(TRGT.problem_assignment_group_key,'')

union all

select TRGT.problem_assignment_group_key,0
...
where UPPER(map.dimension_wh_code)='CLOSED' and TRGT.problem_assignment_group_key<>0)ca