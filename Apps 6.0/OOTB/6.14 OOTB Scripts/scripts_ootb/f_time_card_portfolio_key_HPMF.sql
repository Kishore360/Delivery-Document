SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.portfolio_key' ELSE 'SUCCESS' END as Message
...
on SRC7.prj_portfolio_id=LKP.row_id
and TRGT.source_id=LKP.source_id
WHERE COALESCE(LKP.row_key,CASE WHEN SRC7.prj_portfolio_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key ,0)