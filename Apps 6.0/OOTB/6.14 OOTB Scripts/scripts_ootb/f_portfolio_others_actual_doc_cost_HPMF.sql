SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_portfolio.others_actual_doc_cost' ELSE 'SUCCESS' END as Message
...
sum(fm_forecast.ACTUAL_TOTAL_BSE)  as project_cost_actual
...
where coalesce((coalesce(portfolio.total_actual_doc_cost,0)-coalesce(project.project_cost_actual,0)-coalesce(demand.demand_cost_actual,0)),0)<>coalesce(port.others_actual_doc_cost,'');