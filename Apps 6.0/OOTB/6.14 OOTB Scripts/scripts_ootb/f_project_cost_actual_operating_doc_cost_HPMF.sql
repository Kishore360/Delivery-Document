SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.actual_operating_doc_cost' ELSE 'SUCCESS' END as Message
...
and E.sourceinstance=SRC.sourceinstance
...
WHERE COALESCE(CASE WHEN D.expense_type_code='OPERATING' and D.labor_type_code='LABOR' then C.ACTUAL_OPEX_LABOR_BSE  
WHEN D.expense_type_code='OPERATING' and D.labor_type_code='NON_LABOR' then C.ACTUAL_OPEX_NONLABOR_BSE 
ELSE 0  END,'')<>round(COALESCE(TRGT.actual_operating_doc_cost,''),10)