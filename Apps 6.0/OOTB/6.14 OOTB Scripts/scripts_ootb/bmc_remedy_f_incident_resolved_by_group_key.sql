...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.resolved_by_group_key' ELSE 'SUCCESS' END as Message
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.hlx_hpd_help_desk_final WHERE CDCTYPE<>'D') SRC
 ...
 WHERE COALESCE( 0 ,'')<> COALESCE(TRGT.resolved_by_group_key ,'');