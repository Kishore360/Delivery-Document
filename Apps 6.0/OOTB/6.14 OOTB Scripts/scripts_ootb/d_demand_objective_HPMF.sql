...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_demand.objective' ELSE 'SUCCESS' END as Message
...
where COALESCE(SRC1.prop_business_objective_name,'')<>coalesce(TRGT.objective,'') 