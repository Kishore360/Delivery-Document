...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.concurrency' ELSE 'SUCCESS' END as Message
...
where COALESCE(SRC.concurrency,'')<>coalesce(TRGT.concurrency,'')
