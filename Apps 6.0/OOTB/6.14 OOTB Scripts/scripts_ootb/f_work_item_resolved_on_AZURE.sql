...
WHERE coalesce(case when LM.dimension_wh_code in ('RESOLVED' , 'COMPLETED') then convert_tz(SRC.msvsts_common_resolveddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>') else '' end, '') <> coalesce(TRGT.resolved_on,'');