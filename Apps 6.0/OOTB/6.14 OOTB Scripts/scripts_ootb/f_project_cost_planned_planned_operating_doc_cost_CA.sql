...
and COALESCE(CASE when  (select dimension_wh_code 
from #DWH_TABLE_SCHEMA.d_lov_map 
where UPPER(dimension_class) = 'PROJECT~COST_TYPE'
and SRC2.COST_TYPE_ID = dimension_code
and SRC2.sourceinstance = source_id) = 'OPERATING' then COALESCE(DATEDIFF(SRC3.FINISH_DATE,SRC3.START_DATE)*SRC3.SLICE,0) end,0)
<> COALESCE(TRGT.planned_operating_doc_cost,0);