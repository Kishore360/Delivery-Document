...
#MDS_TABLE_SCHEMA.web_session_records_raw_final SRC 
 ...
 WHERE COALESCE(CONCAT(COALESCE(SRC.device,''),'~',COALESCE(SRC.deviceos,'')))<>COALESCE(TRGT.row_id ,'')
union
 select TRGT.row_id from 
#MDS_TABLE_SCHEMA.mobile_session_records_raw_final SRC 
 ...
 AND SRC.sourceinstance=TRGT.source_id 
 WHERE COALESCE(CONCAT(COALESCE(SRC.devicename,''),'~',COALESCE(SRC.devicemodel,''),'~',COALESCE(SRC.osversion,'')))<>COALESCE(TRGT.row_id ,'')) SQ