...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.i_avail_time' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.availtime,'')<> COALESCE(TRGT.i_avail_time,'')
