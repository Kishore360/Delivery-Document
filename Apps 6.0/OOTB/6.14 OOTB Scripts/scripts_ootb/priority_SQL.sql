...
left join
(
select SQ.issue_changelog_issueid, SQ.issue_changelog_sourceinstance, count(case when SQ.prio_esc_flg = 'Y' then 1 else 0 end) prio_esc_cnt
from
(
select base.issue_changelog_issueid, base.issue_changelog_sourceinstance, base.issuechangelogid, base.to, base.tostring,
case when base.to < next.to then 'Y' else 'N' end prio_esc_flg
from
(
...
on base.issue_changelog_issueid = next.issue_changelog_issueid and base.issue_changelog_sourceinstance = next.issue_changelog_sourceinstance 
and base.issuechangelogid < next.issuechangelogid
)SQ
...
where coalesce(case when prio_count.prio_esc_cnt > 0 then 'Y' else 'N' end,'N' )<>coalesce(TRGT.priority_deescalated_flag,'');