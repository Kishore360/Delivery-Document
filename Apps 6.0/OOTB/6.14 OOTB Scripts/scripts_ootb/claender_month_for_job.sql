select name,build_number,month_name,
(case when _test_fail is NULL then Null else _test_fail end) _test_fail ,
 (case when _test_pass is Null then null else _test_pass end )_test_pass,
(case when _test_skip is Null then Null else _test_skip end  )as _test_skip from 
(
select a.name as name,a.build_number as build_number,concat(substring(a.month_name,1,3) ,a.year_name) as month_name,
round(a._test_fail,2) _test_fail,
round(a._test_pass,2) _test_pass,
round (a._test_skip,2) _test_skip 
from 
(select 	dp.name,d.build_number,dc.month_name,dy.year_name,sum(f.AUTOMATED_TEST_TOTAL_COUNT)test_total,
