...
as planned_row_id,PRJ_PROJECT_ID,sum(forecast_value),position_forecast.forecast_date
...
and date_format(coalesce(convert_tz(planned_effort.forecast_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')<> COALESCE(TRGT.calendar_date_key,0)
