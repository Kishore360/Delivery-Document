...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_activity.assigned_to_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.request_key, LKP.source_id, LKP.state_created_on, LKP.assigned_to_key 
