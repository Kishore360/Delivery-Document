...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.week_id' ELSE 'SUCCESS' END as Message
...
where COALESCE(TRGT.week_id)is not null;