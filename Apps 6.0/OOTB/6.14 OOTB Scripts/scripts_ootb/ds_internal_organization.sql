...
SELECT 
    ...
    FROM 
     ...
     FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D'
	 UNION
	 SELECT A.sys_id, A.sourceinstance
     FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') A
	 JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') B ON A.contact = B.sys_id AND A.sourceinstance = B.sourceinstance
	 )Driver
	 LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') cc ON cc.sys_id = Driver.sys_ID AND cc.sourceinstance = Driver.sourceinstance
	 LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') B ON cc.contact = B.sys_id AND cc.sourceinstance = B.sourceinstance
	...
	FROM 
	 (SELECT sys_id, sourceinstance
     FROM  #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D'
	 UNION
	 SELECT A.sys_id, A.sourceinstance
	 FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') A
	 JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') B ON A.primary_contact = B.sys_id AND A.sourceinstance = B.sourceinstance
	 )Driver
	LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') cd ON cd.sys_id = Driver.sys_ID AND cd.sourceinstance = Driver.sourceinstance
	LEFT OUTER JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.sys_user_final WHERE CDCTYPE<>'D') B ON cd.primary_contact = B.sys_id AND cd.sourceinstance = B.sourceinstance
	...
	FROM  #MDS_TABLE_SCHEMA.sys_user_group_final sug WHERE CDCTYPE<>'D' ;	 
