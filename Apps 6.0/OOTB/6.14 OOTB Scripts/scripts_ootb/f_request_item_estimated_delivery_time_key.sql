...
JOIN #DWH_TABLE_SCHEMA.d_calendar_time LKP 
on (LKP.row_id  = date_format(convert_tz(SRC.estimated_delivery,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i') ) 
and LKP.source_id=0

-- WHERE COALESCE(LKP.row_key,CASE WHEN SRC.estimated_delivery IS NULL THEN 0 else '-1' end)<> coalesce(TRGT.estimated_delivery_time_key,'')
