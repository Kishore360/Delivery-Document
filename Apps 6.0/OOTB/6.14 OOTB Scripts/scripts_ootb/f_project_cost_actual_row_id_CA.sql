...
and COALESCE(TRGT.row_id,'') <> COALESCE(CONCAT('ACT~',COALESCE(SRC.TRANSNO,'UNSPECIFIED'),'~',COALESCE(SRC2.ID,'UNSPECIFIED'),'~',COALESCE(DATE_FORMAT(SRC.TRANSDATE,'%Y%m%d'),'UNSPECIFED')),'UNSPECIFIED')