...
from(select coalesce(out_d.row_key,case when outage.sys_id is null then 0 else -1 end),coalesce(TRGT.outage_key,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(out_d.row_key,case when outage.sys_id is null then 0 else -1 end)<> coalesce(TRGT.outage_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(out_d.row_key,case when outage.sys_id is null then 0 else -1 end)<> coalesce(TRGT.outage_key,''))ca