...
on PROP.prop_portfolio_id=portfolio.row_id
and TRGT.source_id=portfolio.source_id
WHERE  COALESCE(LKP.dimension_wh_code IN ('APPROVED','COMPLETED'))
and  COALESCE(portfolio.row_key,CASE WHEN PROP.prop_portfolio_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key ,0)