...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.dimension_class' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE('TEST_EXECUTION~STATUS' ,'') <> COALESCE(TRGT.dimension_class ,'');