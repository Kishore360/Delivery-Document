SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'd_h_all_hierarchies for  MASTER ITEM failed' ELSE 'SUCCESS' END as Message
 FROM 
(
SELECT CONCAT(a.s0,'~MASTER_ITEM') AS row_id,
...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_0
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_1
ON(lev_0.parent_row_id = lev_1.row_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_2
ON(lev_1.parent_row_id = lev_2.row_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_3
ON(lev_2.parent_row_id = lev_3.row_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_4
ON(lev_3.parent_row_id = lev_4.row_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.us_d_master_item_final WHERE CDCTYPE<>'D') lev_5
...
lev_3_id,lev_3_key,lev_4_id,lev_4_key,lev_5_id,lev_5_key, hierarchy_class
FROM #DWH_TABLE_SCHEMA.d_h_all_hierarchies
WHERE hierarchy_class ='MASTER_ITEM' ) TRGT
