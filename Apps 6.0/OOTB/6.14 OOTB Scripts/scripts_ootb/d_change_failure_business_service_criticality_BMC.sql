...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.business_service_criticality' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(lkp_service_ci.priority,'')<> COALESCE(TGT.business_service_criticality,'')