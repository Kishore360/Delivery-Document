...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_organization.current_flag' ELSE 'SUCCESS' END as Message
FROM  
...
where COALESCE(TRGT.current_flag,'') <> 'Y'