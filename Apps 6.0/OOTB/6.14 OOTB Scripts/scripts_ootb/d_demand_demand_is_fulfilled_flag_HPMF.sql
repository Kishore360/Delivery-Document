...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_demand.demand_is_fulfilled_flag' ELSE 'SUCCESS' END as Message
...
where (CASE WHEN LKP.dimension_wh_code IN ('CLOSED')  then 'Y'
else 'N' end)<>COALESCE(TRGT.is_fulfilled_flag,'')
