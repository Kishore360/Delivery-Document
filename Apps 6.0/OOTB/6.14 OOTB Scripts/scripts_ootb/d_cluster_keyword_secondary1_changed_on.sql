...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster_keyword.secondary1_changed_on' ELSE 'SUCCESS' END as Message
 FROM (select 
 'X' as cdctype
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.secondary1_changed_on,'')<>coalesce(TRGT.secondary1_changed_on,'');	