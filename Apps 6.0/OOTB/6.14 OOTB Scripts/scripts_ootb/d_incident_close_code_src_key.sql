...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.close_code_src_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON ( concat('CLOSE_CODE','~','INCIDENT','~',upper(close_code)) = LKP.src_rowid 
