SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_availability.created_by' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(SRC1.created_by,'')<> coalesce(TRGT.created_by,'')