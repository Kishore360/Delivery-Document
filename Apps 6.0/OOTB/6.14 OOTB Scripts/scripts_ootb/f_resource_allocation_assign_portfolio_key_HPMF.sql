...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.portfolio_key' ELSE 'SUCCESS' END as Message
from (select SRC7.prj_portfolio_id,SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,
...
on pm_work_plans.project_id=SRC7.prj_project_id and pm_work_plans.sourceinstance=SRC7.sourceinstance
...
ON LKP.ROW_ID = SRC.prj_portfolio_id
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.prj_portfolio_id is null THEN 0 else '-1' end)
