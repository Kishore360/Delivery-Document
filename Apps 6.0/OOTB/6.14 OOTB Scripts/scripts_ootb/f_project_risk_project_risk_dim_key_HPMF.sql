...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project_risk.project_risk_dim_key' ELSE 'SUCCESS' END as Message
...
where coalesce(lov.row_key,case when SRC.request_id is null then 0 else -1 end)<>coalesce(TRGT.project_risk_dim_key,'');