...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project.PLANNED_END_ON' ELSE 'SUCCESS' END as Message
...
on SRC1.FINISH_DATE_PERIOD=SRC2.FISCAL_PERIOD_ID 
...
where coalesce(CONVERT_TZ(SRC2.END_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.PLANNED_END_ON,'');