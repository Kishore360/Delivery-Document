...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_resource_plan.planned_start_on' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(CONVERT_TZ(SRC2.SCHED_START_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
