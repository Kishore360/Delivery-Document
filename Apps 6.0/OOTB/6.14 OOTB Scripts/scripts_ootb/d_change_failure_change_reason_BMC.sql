...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.change_reason' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(LKP.dimension_name,'UNSPECIFIED') <> COALESCE(TRGT.change_reason,'')
