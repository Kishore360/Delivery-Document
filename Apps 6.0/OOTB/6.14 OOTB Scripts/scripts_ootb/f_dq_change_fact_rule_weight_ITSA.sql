 ...
 FROM
 -- select distinct row_id_der, dq_fact.row_id,dq_fact.row_key, target_column,dq_target_column, rule_weight_der, rule_weight from  #DWH_TABLE_SCHEMA.f_dq_change_fact dq_factleft join 
(
select distinct  
...
cross join (Select d_chg.change_request_number,d_chg.row_key,d_chg.row_id,dimension_wh_code,d_chg.changed_on from (Select * from #DWH_TABLE_SCHEMA.d_change_request where row_key not in (-1,0) and soft_deleted_flag='N') d_chg
...
select concat('dq_age','~',ch_req.row_key)  as row_id_der ,rule,rule_type,'dq_age' as dq_target_column, age as rule_weight_der -- , dq_fct.row_id, dq_fct.row_key
 ...
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
cross join (Select d_chg.change_request_number,d_chg.row_key,d_chg.row_id,dimension_wh_code,d_chg.changed_on,age from (Select * from #DWH_TABLE_SCHEMA.d_change_request where row_key not in (-1,0) and soft_deleted_flag='N') d_chg
left join (SELECT * FROM #MDS_TABLE_SCHEMA.change_request_final WHERE CDCTYPE<>'D')f_chg
on f_chg.sys_id=d_chg.row_id
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
ON ( concat('STATE','~','CHANGE_REQUEST','~',upper(state))) = LKP.src_rowid 
