SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.web_application_time' ELSE 'SUCCESS' END as Message
    from 
    (select SRC.sourceinstance, round(avg(SRC.webappduration),5) du, CONCAT( COALESCE((DATE_FORMAT(FROM_UNIXTIME(SRC.timestamp/1000-MOD(SRC.timestamp/1000,600)),'%Y%m%d%H%i')),'UNSPECIFIED'),
 ...
 where coalesce( t.du ,'')<> COALESCE(round(TRGT.web_application_time,5),'');
