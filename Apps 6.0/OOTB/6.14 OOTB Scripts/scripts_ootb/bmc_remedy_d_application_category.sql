...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_application.category' ELSE 'SUCCESS' END as Message
 FROM 
( SELECT SRC.requestid,SRC.SourceInstance,field_enum_values.value FROM
    (SELECT * from #MDS_TABLE_SCHEMA.hlx_bmc_core_application_final WHERE datasetid = 'BMC.ASSET' AND CDCTYPE<>'D') SRC                  
            LEFT OUTER JOIN
                #MDS_TABLE_SCHEMA.hlx_field_enum_values_final field_enum_values 
                    ...
                    AND field_enum_values.enumid=SRC.ApplicationType 
            LEFT OUTER JOIN
            ...
            WHERE
                ...
                AND  field.fieldName in ('ApplicationType')) SRC1
 ...
 WHERE COALESCE( SRC1.value,'')<> COALESCE(TRGT.category,'')