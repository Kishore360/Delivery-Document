SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_resource_role.soft_deleted_flag' ELSE 'SUCCESS' END as Message
...
where COALESCE(CASE WHEN SRC.is_active=1 THEN 'N' ELSE 'Y' END,'') <> COALESCE(TRGT.soft_deleted_flag ,'')