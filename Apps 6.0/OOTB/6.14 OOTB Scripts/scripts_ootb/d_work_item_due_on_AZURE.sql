...
where COALESCE(convert_tz(COALESCE(SRC.msvsts_scheduling_targetdate, SRC.msvsts_cmmi_targetresolvedate),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
<>coalesce(TRGT.due_on,'');