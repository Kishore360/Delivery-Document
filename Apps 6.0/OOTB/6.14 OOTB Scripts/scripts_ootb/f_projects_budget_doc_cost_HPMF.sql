...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.budget_doc_cost' ELSE 'SUCCESS' END as Message
...
left join 
(
select sourceinstance,FINANCIAL_SUMMARY_ID,count(1) from #MDS_TABLE_SCHEMA.hp_fm_approved_budgets_final
group by 1,2
) T1
on T1.FINANCIAL_SUMMARY_ID = SRC2.PRJ_FINANCIAL_SUMMARY_ID
and T1.sourceinstance = SRC2.sourceinstance
left join 
(
select sourceinstance,FINANCIAL_SUMMARY_ID,sum(AMOUNT_BSE) as bud_doc_cst from #MDS_TABLE_SCHEMA.hp_fm_approved_budgets_final
group by 1,2
) T2
on T2.FINANCIAL_SUMMARY_ID = T1.FINANCIAL_SUMMARY_ID 
...
where  ROUND(COALESCE(T2.bud_doc_cst,0),7)
<> ROUND(COALESCE(TRGT.budget_doc_cost,0),7);
