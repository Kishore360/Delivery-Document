...
on (F.request_item_key=TGT2.request_item_key AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1 and TGT2.task_attribute_wh_name = 'state')
...
where TGT.primary_sequence_id = 0 and TGT.task_attribute_wh_name = 'state' and F.soft_deleted_flag = 'N'
...
on SRC.documentkey = F.row_id
left join #DWH_TABLE_SCHEMA.f_request_item_activity TGT2
on (F.request_item_key=TGT2.request_item_key AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1 and TGT2.task_attribute_wh_name = 'stage')
left join #DWH_TABLE_SCHEMA.d_lov LOV
