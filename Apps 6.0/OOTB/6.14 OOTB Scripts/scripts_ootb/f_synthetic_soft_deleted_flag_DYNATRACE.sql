select distinct soft_deleted_flag from #DWH_TABLE_SCHEMA.f_synthetic;

SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_synthetic.soft_deleted_flag' ELSE 'SUCCESS' END as Message
FROM (
select coalesce(b.__numerify__primary_key,c.__numerify__primary_key) as __numerify__primary_key, 
