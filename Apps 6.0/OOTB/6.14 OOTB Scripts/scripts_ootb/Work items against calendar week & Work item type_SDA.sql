...
to_char(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'PROPOSED' then F.row_key else null end),'FM999,999,999,999,990') proposed_wi
...
on cal_crtd_week.row_key = cal_crtd.week_start_date_key
left join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
where ((cal_crtd_mon.lagging_count_of_month between 0 and 12) or
status.wi_sts = 'PROPOSED' or status.wi_sts = 'IN PROGRESS')
group by cal_crtd_week.week_id, wk_nm, cal_crtd_week.year_id, type.row_key, type.wi_type
order by cal_crtd_week.week_id desc, wk_nm, cal_crtd_week.year_id desc, type.row_key, upper(type.wi_type)