...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.upper_range_value' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE(TRGT.upper_range_value,'') <> COALESCE(NULL ,'')