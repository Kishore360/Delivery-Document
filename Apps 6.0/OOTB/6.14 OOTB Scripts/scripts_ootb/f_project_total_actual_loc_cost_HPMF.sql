...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.total_actual_loc_cost' ELSE 'SUCCESS' END as Message
...
where  0
<> ROUND(COALESCE(TRGT.total_actual_loc_cost,''),7);