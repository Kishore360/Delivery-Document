...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.src_rowid' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE(CONCAT('TEST_EXECUTION~STATUS~',CASE WHEN SRC.passed=1 THEN 'PASS' WHEN SRC.passed=0 then 'FAIL' END),'')
