SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.role_id'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
    ...
    FROM
        (SELECT 
        sourceinstance AS srm_sourceinstance, ID, cdctype
    FROM
    ...
    WHERE
        ...
        LEFT JOIN
    (SELECT 
        prPrimaryRoleID, sourceinstance AS prj_sourceinstance, prid
    FROM
        #MDS_TABLE_SCHEMA.prj_resources_final
    WHERE
        cdctype <> 'D') prj ON src.ID = prj.prid
...
WHERE
    COALESCE(prj.prPrimaryRoleID, '') <> COALESCE(trg.role_id, '')
;