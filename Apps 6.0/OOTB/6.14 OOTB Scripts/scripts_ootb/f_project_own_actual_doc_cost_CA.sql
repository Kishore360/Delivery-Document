...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.own_actual_doc_cost' ELSE 'SUCCESS' END as Message
from
...
where  COALESCE(round(TRGT.own_actual_doc_cost) ,'') <> 0