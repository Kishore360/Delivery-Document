...
then '('||trim(to_char(coalesce(round((sum(FA.primary_duration * (-1))/86400.00)/count(distinct FA.work_item_key),1),0), '999,999,990D0'))||')'
else trim(to_char(coalesce(round((sum(FA.primary_duration)/86400.00)/count(distinct FA.work_item_key),1),0), '999,999,990D0')) end avg_state_dur
...
group by INTC_ASGN.row_key, INTC_ASGN.full_name
order by upper(INTC_ASGN.full_name), INTC_ASGN.row_key