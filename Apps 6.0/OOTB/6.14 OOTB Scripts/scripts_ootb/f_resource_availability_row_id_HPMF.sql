...
WHERE	(SRC3.WORKING_DAY_FLAG = 'Y') AND SRC4.EVENT_DATE is NULL AND concat(SRC.USER_ID,'~',COALESCE(DATE_FORMAT(CONVERT_TZ(SRC3.WORKING_DAY_DATE,'@#TENANT_SSI_TIME_ZONE@#','@#DW_TARGET_TIME_ZONE@#'),
'%Y%m%d'),'UNSPECIFIED') ) <> TRGT.row_id