...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.hold_time' ELSE 'SUCCESS' END as Message
...
WHERE -- COALESCE(SRC.incomingcallsonholdtime,'')
coalesce(SRC.incomingcallsonholdtime+0) + coalesce(SRC.agentoutcallsonholdtime,0)
<> COALESCE(TRGT.hold_time,'')
