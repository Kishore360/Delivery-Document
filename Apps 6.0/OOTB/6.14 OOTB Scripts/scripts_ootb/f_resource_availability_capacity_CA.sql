SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_availability.capacity' ELSE 'SUCCESS' END as Message
...
LEFT JOIN (select * from #DWH_TABLE_SCHEMA.f_resource_availability)TRGT   
  ...
  WHERE COALESCE(SRC.SLICE,'0')<> round(COALESCE(TRGT.capacity,''),6);    
