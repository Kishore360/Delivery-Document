...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster_keyword.secondary2_changed_on' ELSE 'SUCCESS' END as Message
 FROM (select 
 'X' as cdctype
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.secondary2_changed_on,'')<>coalesce(TRGT.secondary2_changed_on,'');	