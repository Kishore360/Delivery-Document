...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.actual_end_on_numerify' ELSE 'SUCCESS' END as Message
...
on coalesce(concat('Project~State~',SRC10.STATUS_ID),'UNSPECIFIED') = LKP.src_rowid
and SRC.sourceinstance = LKP.source_id
left join #DWH_TABLE_SCHEMA.d_lov_map MAP
on SRC.sourceinstance = MAP.source_id
and  COALESCE(LKP.row_key,CASE WHEN SRC10.STATUS_ID IS NULL THEN 0 else '-1' end) = MAP.src_key
where CASE 
            ...
            ELSE COALESCE(CONVERT_TZ(SRC7.ACT_FINISH_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') END <> COALESCE(TRGT.actual_end_on_numerify,'')