SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item.defect_age' ELSE 'SUCCESS' END as Message
...
coalesce(case when TYP.dimension_wh_code in ('BUG') then case when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') 
 ...
 else timestampdiff(second,coalesce(SRC.createdDate,0),coalesce(convert_tz(FRESH.lastupdated,'<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>'),0)) end
 else 0 end,0) sr,
coalesce(TRGT.defect_age,'') tg*/
...
on concat('STATUS~WORK_ITEM~',upper(SRC.workItemType))=TYP.src_rowid and SRC.sourceinstance = TYP.source_id
...
on concat('RESOLUTION~WORK_ITEM~',upper(SRC.msvsts_common_resolvedreason))=RES.src_rowid and SRC.sourceinstance = RES.source_id
 ...
 where coalesce(case when TYP.dimension_wh_code in ('BUG') then case when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') 
  then timestampdiff(second,coalesce(SRC.createdDate,0),coalesce(SRC.msvsts_common_resolveddate, case when DM.dimension_wh_code in ('COMPLETED','RESOLVED') then coalesce(dates.completed_on, SRC.msvsts_common_closeddate) else null end, dates.cancelled_on))
 when DM.dimension_wh_code in ('CANCELLED') then timestampdiff(second,coalesce(SRC.createdDate,0),coalesce(SRC.msvsts_common_resolveddate, dates.cancelled_on))
 else timestampdiff(second,coalesce(SRC.createdDate,0),coalesce(convert_tz(FRESH.lastupdated,'<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>'),0)) end
 else 0 end,0)<> coalesce(TRGT.defect_age,'');