...
left join(select  count(row_id) as sub_project_count,row_id from #DWH_TABLE_SCHEMA.d_project where top_project_key<>row_key group by top_project_key)LKP
on TRGT.row_id=LKP.row_id
WHERE COALESCE(LKP.sub_project_count,'0')<>COALESCE(TRGT.sub_project_count,'0')
