set @lov_column= (select column_value from lsm_ls_system_variables where class_value='STATE~PROBLEM' and source_entity='sys_choice' and enable_flag='Y');

-- select @lov_column;

set @script=concat('SELECT CASE WHEN count(1) > 0 THEN ''FAILURE'' ELSE ''SUCCESS'' END as Result,
...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.problem_final WHERE CDCTYPE<>'D') SRC
 ...
 ON (SRC.',@lov_column,'=L.dimension_code
...
on (LKP.row_id = date_format(convert_tz(COALESCE( SRC.closed_at,SRC.sys_updated_on),''<<TENANT_SSI_TIME_ZONE>>'',''<<DW_TARGET_TIME_ZONE>>''),''%Y%m%d'') 
	and LKP.source_id=0)
WHERE CASE WHEN L.dimension_wh_code=''CLOSED'' THEN COALESCE(LKP.row_key,'') ELSE '' END 
