...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_build_project.builds_not_executed' ELSE 'SUCCESS' END as Message
...
count(case when build.__numerify__primary_key is not null and sts.dimension_wh_code in ('ABORTED', 'IN PROGRESS', 'NEW')
then sts.row_key else null end) as builds_not_executed_count
...
where  COALESCE(SQ.builds_not_executed_count,0) <> COALESCE(TRGT.builds_not_executed,'');
