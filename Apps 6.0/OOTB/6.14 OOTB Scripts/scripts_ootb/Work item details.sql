...
D.description srt_desc,
ITE.name it_nm,
D.iteration_list it_lst,
pv.description fx_ver,
D.fix_version_list fx_ver_lst,
D.labels_list lbl_lst,
INTC_ASGN.full_name asgn_nm,
INTC_RPT.full_name rpt_nm,
INTC_CRT.full_name cret_nm,
case when D.workflow_key = -1 then 'UNKNOWN' when D.workflow_key = 0 then 'UNSPECIFIED' end wf,
D.changed_by chngd_by,
INTC_RSLVDBY.full_name rslvd_nm,
INTC_CLSDBY.full_name clsd_nm,

to_char((sum(coalesce(F.time_spent ,0)) / 1.0),'FM999,999,999,999,990')as time_spent_created, 
to_char((sum(case when status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 1.0), 'FM999,999,999,999,990') as time_spent_closed
...
D.proposed_flag = 'Y')
group by D.work_item_number, D.row_id, type.wi_type, PROJ.project_name, D.short_description, D.description, ITE.row_key, ITE.name, D.iteration_list, pv.description, D.fix_version_list, D.labels_list, INTC_ASGN.full_name, INTC_RPT.full_name, INTC_CRT.full_name, D.workflow_key, D.changed_by, INTC_RSLVDBY.full_name, INTC_CLSDBY.full_name
order by D.work_item_number, D.row_id, upper(type.wi_type), upper(PROJ.project_name), upper(D.short_description), upper(D.description), ITE.row_key, upper(ITE.name), upper(D.iteration_list), upper(pv.description), upper(D.fix_version_list), upper(D.labels_list), upper(INTC_ASGN.full_name), upper(INTC_RPT.full_name), upper(INTC_CRT.full_name), upper(D.workflow_key), upper(D.changed_by), upper(INTC_RSLVDBY.full_name), upper(INTC_CLSDBY.full_name)
