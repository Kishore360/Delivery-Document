...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ACTUAL_EFFORT' ELSE 'SUCCESS' END as Message
from (select *from  #MDS_TABLE_SCHEMA.hp_pm_projects_final where cdctype <> 'D') SRC1
left join     #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final SRC5
on SRC1.PROJECT_ID = SRC5.PRJ_PROJECT_ID and SRC1.sourceinstance = SRC5.sourceinstance 
left join (select * from #MDS_TABLE_SCHEMA.hp_rsc_staffing_profiles_final where is_active_flag='Y')SRC11
on SRC11.STAFFING_PROFILE_ID=SRC5.PRJ_STAFF_PROF_ID and SRC11.sourceinstance=SRC5.sourceinstance
...
WHERE COALESCE(SRC4.ACT_EFFORT,0)
<> COALESCE(TRGT.ACTUAL_EFFORT ,'');