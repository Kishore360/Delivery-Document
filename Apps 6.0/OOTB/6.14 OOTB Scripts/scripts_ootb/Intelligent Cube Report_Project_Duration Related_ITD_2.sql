...
case when (sum(F.planned_duration* F.planned_percent_complete) = 0 or sum(F.planned_duration* F.planned_percent_complete) is null) or sum(F.actual_percent_complete) = 0 then null else
trim(to_char(coalesce(round((sum(F.planned_duration * F.planned_percent_complete * 100) /sum(F.actual_percent_complete)),0),0), '9,999,999,990')) end EAC_Time,

trim(to_char(coalesce(round((sum((1.0000+ coalesce(f.planned_percent_complete, 0) - coalesce(f.actual_percent_complete, 0)/50.000)*f.planned_duration)),0),0), '9,999,999,990')) ETC_time,

trim(to_char(coalesce(round(((sum(f.planned_duration) + sum(f.actual_duration)) - sum(coalesce(f.planned_percent_complete, 0) * f.planned_duration)),0),0), '9,999,999,990')) LRE,

trim(to_char(round(coalesce(sum(F.planned_duration),0) - (case when sum(F.actual_percent_complete) = 0 or sum(F.actual_percent_complete) is null then 0 
else coalesce((sum(F.planned_duration)*(sum(F.planned_percent_complete)*100) /sum(F.actual_percent_complete)),0) end),0), '9,999,999,990')) VAC_Time
