...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_risk_work_item_work_item.src_rowid' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('RISK~WORK_ITEM~',(SRC.msvsts_common_risk)),'')<>COALESCE(TRGT.src_rowid,'')