...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_priority_work_item_work_item.dimension_class' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE('PRIORITY~WORK_ITEM','')<>COALESCE(TRGT.dimension_class,'')