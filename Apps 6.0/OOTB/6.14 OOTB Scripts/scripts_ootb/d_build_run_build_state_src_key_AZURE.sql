...
on (LM.src_rowid = CONCAT('STATE~BUILD~',SRC.status) AND SRC.sourceinstance = LM.source_id )
WHERE COALESCE(LM.src_key,CASE WHEN SRC.status IS NULL THEN 0 else -1 end)<> COALESCE(TRG.build_state_src_key,'');