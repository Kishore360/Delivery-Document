...
on cal_cmptd.row_key = FWI.completed_on_key
...
on cal_rslvd.row_key = FWI.work_item_resolved_on_key
left join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
on cal_inprog.row_key = FWI.work_item_work_start_on_key
/*case when status.wi_sts in ('COMPLETED') then coalesce(F.completed_on_key,0) 
when status.wi_sts in ('RESOLVED') then coalesce(F.work_item_resolved_on_key,0) 
when status.wi_sts in ('IN PROGRESS') then coalesce(F.work_item_work_start_on_key,0) 
else coalesce(F.created_on_key,0) end*/
...
where ((cal_dte_mon.lagging_count_of_month between 0 and 12 or cal_inprog_mon.lagging_count_of_month between 0 and 12 or
