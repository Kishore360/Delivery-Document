...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.allocation_on' ELSE 'SUCCESS' END as Message
...
where COALESCE(convert_tz(SRC.WORKING_DAY_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
<> COALESCE(TRGT.allocation_on,'');