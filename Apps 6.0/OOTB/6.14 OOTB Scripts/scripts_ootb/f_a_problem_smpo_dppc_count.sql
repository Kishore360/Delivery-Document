...
CASE WHEN count(1) >0 THEN 'Row count failed for f_a_problem_smpo_dppc' ELSE 'SUCCESS' END as Message
from (
select * from
(select count(*) count_src from  #STG_TABLE_SCHEMA.f_a_problem_smpo_dppc_base SRC)count_src,

 (select count(*) count_trgt from  #DWH_TABLE_SCHEMA.f_a_problem_smpo_dppc TRGT
