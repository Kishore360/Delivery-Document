select 
-- to_char(cal.calendar_date, 'fmmm/fmdd/fmyyyy fmHH:MI:SS AM') cald,
...
count(case when CAL.lagging_count_of_days between 0 and 29 and D.major_incident_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) maj_inc,
...
count(case when CAL.lagging_count_of_days between 0 and 29 and D.reopened_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) reopnd_inc,

count(case when CAL.lagging_count_of_days between 0 and 29 and F.assigned_to_key in (0,-1) then F.incident_number else null end) unasgnd_inc,

count(case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) bcklg_inc,

count(case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('OPEN') and D.over_due_flag = 'Y' and D.backlog_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) od_bck_inc,

count(case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('OPEN') and D.priority_escalated_flag = 'Y' and D.backlog_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) esc_bck_inc,

count(case when CAL.lagging_count_of_days between 0 and 29 and D.priority_escalated_flag = 'Y' and F.incident_key is not null then F.incident_key else null end) pe_inc,

sum(case when CAL.lagging_count_of_days between 0 and 29 then F.priority_escalation_count else 0 end) pe_cnt
...
group by pr
order by pr
-- order by CAL.lagging_count_of_days limit 10