SELECT CASE
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for f_resource_allocation.allocation_on'
        ELSE 'SUCCESS'
    END AS Message
FROM
...
AND COALESCE(CONVERT_TZ(src.SLICE_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> COALESCE(trg.allocation_on, '');
