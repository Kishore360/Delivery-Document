...
ON (date_format(convert_tz(inprog_dt.inprog_strtdt,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') = LKP.row_id)
 WHERE coalesce(LKP.row_key, case when inprog_dt.inprog_strtdt is null then 0 else 0 end) <> coalesce(TRGT.work_item_work_start_on_key,'');