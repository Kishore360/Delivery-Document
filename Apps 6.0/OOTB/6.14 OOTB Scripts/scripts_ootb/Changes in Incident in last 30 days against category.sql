select CATEGORY.dimension_name cat_name,

count(case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.incident_key else null end) a,
sum((case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) b,


count(case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.incident_key else null end) c,
sum((case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) d,


(case when count(case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.incident_key else null end) = 0 then null

when count(case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.incident_key else null end) = 0 then 
('('||trim(to_char(round(100 , 2) , '9,999,999,999D99'))||'%)')

when ((sum((case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) /
count(case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.incident_key else null end)) - 
(sum((case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) /
count(case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.incident_key else null end))) < 0 then

('('|| trim(to_char(round(
((sum((case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) /
count(case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.incident_key else null end)) - 
(sum((case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) /
count(case when CAL.lagging_count_of_days between 0 and 29 and D.backlog_flag = 'Y' then F.incident_key else null end))) /
(sum((case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.age else null end * 1.000)/86400.000) /
count(case when CAL.lagging_count_of_days between 30 and 59 and D.backlog_flag = 'Y' then F.incident_key else null end)) , 2)
 , '9,999,999,999D99'))||'%)')
...
(
   ...
   FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key
   JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON wh_lov.row_key = br.wh_key AND br.dimension_class = 'CONTACT_TYPE~INCIDENT'
UNION ALL 
...
(
 SELECT lov.*, lov.dimension_code subcat_code, lov.dimension_name as subcat
   ...
   AND parent.src_rowid = concat(concat('CATEGORY~INCIDENT', '~~~'), upper(lov.dimension_type))
...
(
 SELECT dh_all_hierarchies_dimension.lev_5_key, dh_user_group_level2_bi.organization_name AS lev_2_organization_name, assignment_group.organization_name, dh_user_group_level3_bi.organization_name AS lev_3_organization_name, dh_all_hierarchies_dimension.lev_4_key, dh_user_group_level5_bi.organization_name AS lev_5_organization_name, dh_user_group_level4_bi.organization_name AS lev_4_organization_name, dh_all_hierarchies_dimension.lev_2_key, dh_all_hierarchies_dimension.lev_0_key, assignment_group.emp_count, dh_user_group_level1_bi.organization_name AS lev_1_organization_name, dh_all_hierarchies_dimension.lev_1_key, dh_all_hierarchies_dimension.lev_3_key, fco_d_internal_contact.full_name AS manager, assignment_group.soft_deleted_flag, assignment_group.row_current_key, assignment_group.changed_by, assignment_group.dw_updated_on, assignment_group.changed_on, assignment_group.current_flag, assignment_group.source_id, assignment_group.secondary2_changed_on, assignment_group.effective_from, assignment_group.created_by, assignment_group.row_dn_key, assignment_group.location_key, assignment_group.secondary1_changed_on, assignment_group.cdctype, assignment_group.effective_to, assignment_group.created_on, assignment_group.row_id, assignment_group.etl_run_number, assignment_group.row_key, assignment_group.dw_inserted_on
   ...
   JOIN #DWH_TABLE_SCHEMA.d_h_all_hierarchies dh_all_hierarchies_dimension ON assignment_group.row_dn_key = dh_all_hierarchies_dimension.lev_0_key AND (dh_all_hierarchies_dimension.hierarchy_class = 'USER GROUP' OR dh_all_hierarchies_dimension.hierarchy_class = 'UNSPECIFIED' OR dh_all_hierarchies_dimension.hierarchy_class = 'UNKNOWN') AND dh_all_hierarchies_dimension.current_flag = 'Y'
   JOIN #DWH_TABLE_SCHEMA.d_internal_organization dh_user_group_level1_bi ON dh_all_hierarchies_dimension.lev_1_key = dh_user_group_level1_bi.row_key AND dh_user_group_level1_bi.group_flag = 'Y' AND dh_user_group_level1_bi.current_flag = 'Y'
   JOIN #DWH_TABLE_SCHEMA.d_internal_organization dh_user_group_level2_bi ON dh_all_hierarchies_dimension.lev_2_key = dh_user_group_level2_bi.row_key AND dh_user_group_level2_bi.group_flag = 'Y' AND dh_user_group_level2_bi.current_flag = 'Y'
   JOIN #DWH_TABLE_SCHEMA.d_internal_organization dh_user_group_level3_bi ON dh_all_hierarchies_dimension.lev_3_key = dh_user_group_level3_bi.row_key AND dh_user_group_level3_bi.group_flag = 'Y' AND dh_user_group_level3_bi.current_flag = 'Y'
   JOIN #DWH_TABLE_SCHEMA.d_internal_organization dh_user_group_level4_bi ON dh_all_hierarchies_dimension.lev_4_key = dh_user_group_level4_bi.row_key AND dh_user_group_level4_bi.group_flag = 'Y' AND dh_user_group_level4_bi.current_flag = 'Y'
   JOIN #DWH_TABLE_SCHEMA.d_internal_organization dh_user_group_level5_bi ON dh_all_hierarchies_dimension.lev_5_key = dh_user_group_level5_bi.row_key AND dh_user_group_level5_bi.group_flag = 'Y' AND dh_user_group_level5_bi.current_flag = 'Y'
  WHERE assignment_group.group_flag = 'Y' OR assignment_group.row_key = 0 OR assignment_group.row_key = -1
 ) DIO
