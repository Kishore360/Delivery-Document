...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.legal_entity_flag' ELSE 'SUCCESS' END as Message
from (select (TRGT.legal_entity_flag)
...
where (TRGT.legal_entity_flag)<>'Y'

union all

select (TRGT.legal_entity_flag)
...
where (TRGT.legal_entity_flag)<>'N'
...
where (TRGT.legal_entity_flag)<>'N')inter_org