...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_efford.project_manager_key' ELSE 'SUCCESS' END as Message
FROM 
...
on concat('INTERNAL_CONTACT~',SRC.PRJ_PROJECT_MANAGER_USER_ID) = INT_CON.row_id
...
and COALESCE(INT_CON.row_key,CASE WHEN SRC.PRJ_PROJECT_MANAGER_USER_ID IS NULL THEN 0 else '-1' end)
