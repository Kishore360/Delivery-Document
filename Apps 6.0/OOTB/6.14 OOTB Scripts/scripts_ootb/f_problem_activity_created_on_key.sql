...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_activity.created_on_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' and CAL.calendar_code = 0
