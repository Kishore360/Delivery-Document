SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_iteration.changed_by' ELSE 'SUCCESS' END as Message
FROM (
 select SRCD.*
...
left join #DWH_TABLE_SCHEMA.f_work_item_iteration TRGT
 ...
 where coalesce(SRC.System_ChangedBy__PersonSK,'')<>coalesce(TRGT.changed_by,'')