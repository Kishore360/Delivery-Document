...
coalesce(TRGT.work_item_epic_key,'') actl*/
FROM (
...
where coalesce(LKP.row_key, case when SRC.System_WorkItemType = 'Epic' and CONCAT(fhist.TargetWorkItemID, '~', SRC.TeamProjectSK) is not null then -1 else 0 end)<>coalesce(TRGT.work_item_epic_key,'');