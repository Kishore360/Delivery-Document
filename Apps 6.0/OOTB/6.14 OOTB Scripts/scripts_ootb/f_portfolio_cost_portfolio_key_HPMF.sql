SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_portfolio_cost.portfolio_key' ELSE 'SUCCESS' END as Message
FROM
...
on COALESCE(SRC.portfolio_id,'')=COALESCE(LKP.row_id,'')
and COALESCE(SRC.sourceinstance,'')=COALESCE(LKP.source_id,'')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.portfolio_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key,0)
