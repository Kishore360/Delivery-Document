...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_risk.risk_status_src_key' ELSE 'SUCCESS' END as Message
...
on lov.row_id=coalesce(CONCAT('PROJECT_RISK~STATUS~',SRC.STATUS_ID),'UNSPECIFIED')
and lov.source_id=SRC.sourceinstance
where coalesce(lov.row_key,case when SRC.STATUS_ID is null then 0 else -1 end)<>coalesce(TRGT.risk_status_src_key,'');