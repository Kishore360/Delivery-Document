...
FROM 
(
select base.row_id, base.task_attribute_wh_next_value_key, next.task_attribute_wh_new_value_key,base.source_id
...
union all
select base.row_id, base.task_attribute_wh_next_value_key, 0 as task_attribute_wh_new_value_key,base.source_id
from #DWH_TABLE_SCHEMA.f_work_item_activity base 
join
(
select max(F.primary_sequence_id) max_sequence_id, F.work_item_key, F.task_attribute_wh_name,F.source_id
from #DWH_TABLE_SCHEMA.f_work_item_activity F
group by F.work_item_key, F.task_attribute_wh_name,F.source_id
)max_id
