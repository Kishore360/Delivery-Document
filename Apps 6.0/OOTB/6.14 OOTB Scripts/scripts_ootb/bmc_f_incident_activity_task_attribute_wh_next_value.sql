...
where -- case when SRC.expose_flag = 'Y' then COALESCE(SRC.task_attribute_wh_new_value,0) else 0 end <> COALESCE(TRGT.task_attribute_wh_next_value,0)
COALESCE(null,'')<> COALESCE(TRGT.task_attribute_wh_next_value,'')
union all
select base.row_id
