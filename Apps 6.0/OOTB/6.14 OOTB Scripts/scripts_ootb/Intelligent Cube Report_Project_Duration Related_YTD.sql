...
TO_CHAR((F.planned_end_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM')  plnd_end_date,
round(sum(F.actual_duration),1) Act_duration,
round(sum(F.planned_duration),1) plnd_duration,
case when sum(F.actual_duration - F.planned_duration) < 0 then '('||cast(round(sum(F.planned_duration - F.actual_duration),1) as varchar(200))||')'
else cast(round(sum(F.actual_duration - F.planned_duration),1)as varchar(200)) end duration_variation,
case when(sum(datediff(day, F.planned_start_on, F.actual_start_on))) is null then cast(0.0 as varchar(200))
when (sum(datediff(day, F.planned_start_on, F.actual_start_on))) < 0 then '('||cast(round(sum(datediff(day, F.actual_start_on, F.planned_start_on))*1.0,1) as varchar(200))||')'
else cast(round(sum(datediff(day, F.planned_start_on, F.actual_start_on))*1.0,1) as varchar(200)) end as strt_delay_duration,
case when F.planned_duration = 0 then null
when F.planned_duration <> 0 
then trim(to_char(coalesce(round(((sum(datediff(day, F.planned_start_on, F.actual_start_on))/sum(F.planned_duration))*100),1),0.0),'9999999999'))||'%'
else trim(to_char(0.0, '9999999999'))||'%' end prct_Delay_in_strt,
round(sum(F.remaining_duration),1) Remaining_duration,
case when sum(datediff(day, F.planned_start_on, F.actual_start_on)) is null then cast(0.0 as varchar(200))
when sum(datediff(day, F.planned_start_on, F.actual_start_on)) < 0 then '('||cast(round(sum(datediff(day, F.planned_start_on, F.actual_start_on))*(-1.0),1) as varchar(200))||')'
else cast(round(sum(datediff(day, F.planned_start_on, F.actual_start_on))*1.0,1) as varchar(200)) end as "Project Kick-off slack time",
round(case when sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end) = 0.0 then null
else (sum(case when STATE.state = 'Closed' then F.actual_duration*1.0 else 0.0 end)/
sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end)) end,1) AVG_time_Deliver,
round(case when sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end) = 0.0 then null
else (sum(case when STATE.state = 'Closed' then F.planned_duration*1.0 else 0.0 end)/
sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end)) end,1) AVG_Projected_time_Deliver,
case when round((coalesce(sum(datediff(day, F.actual_start_on, COALESCE(f.actual_end_on, FRESH.lastupdated)))*1.0,0.0)),1) < 0
then '('||trim(to_char(round((coalesce(sum(datediff(day, F.actual_start_on, COALESCE(f.actual_end_on, FRESH.lastupdated))),0)),1)*(-1.0), '9,999,999,990D9'))||')'
else trim(to_char(round((coalesce(sum(datediff(day, F.actual_start_on, COALESCE(f.actual_end_on, FRESH.lastupdated)))*1.0,0.0)),1), '9,999,999,990D9')) end Actual_Duration_Timeline
...
group by D_P.row_key, Portfolio_name, Portfolio_category, D.row_key, D.Project_name, D2.row_key, Top_Project_Name,
act_strt_date,act_end_date,plnd_end_date,F.actual_end_on,F.planned_duration
