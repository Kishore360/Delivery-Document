...
select A.incident_key, A.incident_key1, A.task_attribute_wh_name, A.dimension_wh_code, A.created_on, A.created_on1, A.lastupdated, A.closed_on, 
A.primary_duration, A.primary_duration1, A.a, A.b, A.c
FROM (
SELECT f1.incident_key, f2.incident_key1, f1.task_attribute_wh_name, br.dimension_wh_code, f1.created_on, f2.created_on1, dat.lastupdated, coalesce(d.closed_on,d.last_resolved_on) as closed_on, 
timestampdiff( SECOND ,f1.created_on,coalesce(d.closed_on,d.last_resolved_on) ) a,
timestampdiff( SECOND ,f1.created_on,dat.lastupdated) b,
timestampdiff( SECOND ,f1.created_on,f2.created_on1 ) c,
f1.primary_duration,
CASE WHEN  f2.created_on1 IS NULL AND br.dimension_wh_code = 'CLOSED' THEN timestampdiff( SECOND ,f1.created_on,coalesce(d.closed_on,d.last_resolved_on) )
...
FROM     ( SELECT * FROM #DWH_TABLE_SCHEMA.f_incident_activity WHERE task_wh_type = 'incident' and task_attribute_wh_name = 'state') f1
...
FROM #DWH_TABLE_SCHEMA.f_incident_activity f2 
           WHERE f2.task_wh_type = 'incident' and f2.task_attribute_wh_name = 'state' )f2
...
INNER JOIN #DWH_TABLE_SCHEMA.d_incident d ON f1.incident_key = d.row_key
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON f1.state_key = br.src_key AND br.dimension_class = 'state~incident'
...
FROM     ( SELECT * FROM #DWH_TABLE_SCHEMA.f_incident_activity WHERE task_wh_type = 'incident' and task_attribute_wh_name = 'assignment_group') f1
LEFT JOIN( SELECT f2.incident_key as incident_key1, f2.primary_sequence_id as primary_sequence_id1, f2.created_on as created_on1 

FROM #DWH_TABLE_SCHEMA.f_incident_activity f2 
           WHERE f2.task_wh_type = 'incident' and f2.task_attribute_wh_name = 'assignment_group' )f2
ON f1.incident_key = f2.incident_key1 AND f2.primary_sequence_id1 = f1.primary_sequence_id + 1 AND f1.task_wh_type = 'incident' 
and f1.task_attribute_wh_name = 'assignment_group' 
INNER JOIN #DWH_TABLE_SCHEMA.d_incident d ON f1.incident_key = d.row_key
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON f1.state_key = br.src_key AND br.dimension_class = 'state~incident'
INNER JOIN ( SELECT MAX(lastupdated) as lastupdated, source_id FROM #DWH_TABLE_SCHEMA.d_o_data_freshness group by source_id ) dat 
on f1.source_id = dat.source_id
