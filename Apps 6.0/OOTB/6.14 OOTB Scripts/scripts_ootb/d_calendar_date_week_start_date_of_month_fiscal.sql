...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for fiscal d_calendar_date.week_start_date_of_month' ELSE 'SUCCESS' END as Message
FROM  
(
select distinct src.calendar_date, src1.src_week_start_date AS src_week_start_date
from
...
LEFT OUTER JOIN
(SELECT  rolling_count_of_weeks, min(calendar_date) src_week_start_date
...
WHERE SRC_1.src_week_start_date <> tgt.week_start_date_of_month 