...
FROM
    ...
    AND prsheet.sourceinstance = prtask.sourceinstance
    WHERE
        prt.CDCTYPE <> 'D'
    ...
    #DWH_TABLE_SCHEMA.f_time_card trg ON CONCAT(src.prt_prid,
        ...
        LEFT JOIN
    #DWH_TABLE_SCHEMA.d_project_task lkp ON (src.pr_task_prid = lkp.row_id
        AND src.prtask_sourceinstance = lkp.source_id)
WHERE
    pr_slices.REQUEST_NAME = 'DAILYRESOURCETIMECURVE'
        AND COALESCE(lkp.row_key,
            CASE
                WHEN src.pr_task_prid IS NULL THEN 0
                ELSE '-1'
            END) <> COALESCE(trg.project_task_key, '');
