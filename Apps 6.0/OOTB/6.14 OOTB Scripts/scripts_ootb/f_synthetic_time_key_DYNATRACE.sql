...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_synthetic.time_key' ELSE 'SUCCESS' END as Message

FROM (select coalesce(b.__numerify__primary_key,c.__numerify__primary_key) as __numerify__primary_key, 
coalesce(a.GEOLOCATION,b.GEOLOCATION), coalesce(b.timestamp,c.timestamp) as timestamp,
...
on a.__numerify__primary_key=c.__numerify__primary_key
 ...
 left join #DWH_TABLE_SCHEMA.d_calendar_time trns
 ON COALESCE(DATE_FORMAT(CONVERT_TZ(from_unixtime(timestamp/1000),'#TENANT_SSI_TIME_ZONE','#DW_TARGET_TIME_ZONE'),'%H%i'),'UNSPECIFIED')
 =trns.row_id 
 left join #DWH_TABLE_SCHEMA.f_synthetic TRGT
on SRC.__numerify__primary_key =TRGT.row_id
and SRC.sourceinstance=TRGT.source_id
where coalesce(TRGT.time_key,'')<>COALESCE(trns.row_key,'')
