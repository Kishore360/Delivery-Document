SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.pivot_date' ELSE 'SUCCESS' END as Message
FROM
...
WHERE COALESCE(CONVERT_TZ(SRC2.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>COALESCE(TRGT.pivot_date,'')
