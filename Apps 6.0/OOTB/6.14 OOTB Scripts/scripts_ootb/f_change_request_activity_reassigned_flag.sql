...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.reassigned_flag' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(null, '') <> COALESCE(TGT.reassigned_flag,'')
union all
select TGT.row_id
...
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' 
and COALESCE('N', '')  <> COALESCE(TGT.reassigned_flag,'') 
) SQ;