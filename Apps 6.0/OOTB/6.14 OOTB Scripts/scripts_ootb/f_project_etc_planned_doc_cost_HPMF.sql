...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.etc_planned_doc_cost' ELSE 'SUCCESS' END as Message
...
where coalesce(round(project_cost.project_planned_cost,0),'')<>coalesce(TRGT.etc_planned_doc_cost,'');