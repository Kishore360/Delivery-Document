SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.demand_key' ELSE 'SUCCESS' END as Message
...
on SRC3.id=LKP.row_id
WHERE  COALESCE(CASE when SRC3.odf_object_code='idea' then LKP.row_key else null end,'0')<>COALESCE(TRGT.demand_key,'')
