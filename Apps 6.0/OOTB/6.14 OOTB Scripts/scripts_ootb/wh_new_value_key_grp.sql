
select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'data validation failed for wh_new_value_key' ELSE 'SUCCESS' END as Message
from #DWH_TABLE_SCHEMA.f_incident_activity T
join #DWH_TABLE_SCHEMA.d_incident D
on (T.task_key= D.row_key
                                and T.task_wh_type='incident'
                                and T.task_attribute_wh_name ='assignment_group'
                                               )
left join #DWH_TABLE_SCHEMA.d_internal_organization ICF
on( concat('GROUP~',T.task_attribute_wh_new_value) = ICF.row_id)
WHERE coalesce(T.task_attribute_wh_new_value_key,'')
<>coalesce(case when T.task_attribute_wh_new_value is null then 0 end, 
case when T.task_attribute_wh_new_value = 'unspecified' then 0 else ICF.row_key end,-1);
