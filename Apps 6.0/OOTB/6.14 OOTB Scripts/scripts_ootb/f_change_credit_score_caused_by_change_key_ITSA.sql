SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_credit_score.caused_by_change_key' ELSE 'SUCCESS' END as Message
 ...
 where COALESCE( SRC.caused_by_change_key,'')<> COALESCE(TRGT.caused_by_change_key,'')
