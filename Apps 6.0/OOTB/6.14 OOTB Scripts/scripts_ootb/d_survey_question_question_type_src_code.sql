...
case when count(1) > 0 then 'MDS to DWH data validation failed for question_type_src_code between survey_question_new_final & asmt_metric_final to d_survey_question' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(S.type,'') <> ifnull(DWH.question_type_src_code,'')
...
ifnull(A.datatype,'') <> ifnull(DWH.question_type_src_code,'')
) SQ;