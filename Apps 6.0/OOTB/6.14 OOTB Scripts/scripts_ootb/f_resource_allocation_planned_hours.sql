SELECT  CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.planned_hours' ELSE 'SUCCESS' END as Message
/*select RP.planned_hours, TRGT.planned_hours,
...
where coalesce(RP.planned_hours, 0) <> round(coalesce((TRGT.planned_hours * TRGT.week_day_count), 0), 2);