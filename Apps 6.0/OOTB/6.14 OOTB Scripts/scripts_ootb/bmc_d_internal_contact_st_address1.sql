...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.city_code' ELSE 'SUCCESS' END as Message
  FROM ( SELECT ctm_people.SourceInstance,value,Site_street ,home_street ,Default_street,person_id from  #MDS_TABLE_SCHEMA.hlx_ctm_people_final ctm_people                  
            LEFT OUTER JOIN
...
WHERE COALESCE(case WHEN SRC.value = 'Office-Based Employee' THEN SRC.Site_street 
                    WHEN SRC.value = 'Home-Based Employee' THEN SRC.home_street 
                    ELSE SRC.Default_street end,'')<> COALESCE(TRGT.st_address1,'') 
