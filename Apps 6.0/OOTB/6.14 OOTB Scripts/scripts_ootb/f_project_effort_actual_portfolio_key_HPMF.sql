...
left join(select * from #DWH_TABLE_SCHEMA.d_portfolio)port
on COALESCE(SRC.prj_portfolio_id,'')=COALESCE(port.row_id,'')
and COALESCE(SRC.sourceinstance,'')=COALESCE(port.source_id,'')
...
and COALESCE(port.row_key,CASE WHEN SRC.prj_portfolio_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_key,0)
