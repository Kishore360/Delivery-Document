...
select F.row_key,
D.incident_number,
D.major_incident_flag, 

round(sum(INCACT.assigned_duration/86400.00), 1) duration,
max(D.reassignee_count) no_of_asignee_handoff,
max(D.reassignment_count) no_of_asignment_handoff,
sum(F.update_count) updates
...
group by F.row_key, D.incident_number, D.major_incident_flag
order by D.incident_number, F.row_key, D.major_incident_flag
)SQ
...
count(distinct INCACT.to_assigned_to_key) no_of_asignee_involved,
count(distinct INCACT.to_assignment_group_key) no_of_asignment_grps_involved
...
on cal.row_key = coalesce(INCACT.assigned_on_key, F.opened_on_key)
join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
round(avg(INCACT.assigned_duration / 86400.00), 1) avg_duration_old_assignee
...
select IOG_FROM.organization_name old_assignment_group_name, 
...
on IOG_FROM.row_key = INCACT.from_assignment_group_key
where cal_mon.lagging_count_of_month between 0 and 12
group by old_assignment_group_name
order by upper(IOG_FROM.organization_name)
