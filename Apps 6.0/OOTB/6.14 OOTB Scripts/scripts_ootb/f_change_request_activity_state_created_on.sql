...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.state_created_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.change_request_key, LKP.source_id, LKP.state_created_on, LKP.state_key 
