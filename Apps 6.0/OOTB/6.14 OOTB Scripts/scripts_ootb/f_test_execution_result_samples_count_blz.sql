...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.samples_count' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.samples,'')<>coalesce(TRGT.samples_count,'')
