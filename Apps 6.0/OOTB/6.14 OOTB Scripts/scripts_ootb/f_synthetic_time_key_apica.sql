...
left outer join #DWH_TABLE_SCHEMA.d_calendar_time D_TIM
on COALESCE(DATE_FORMAT(CONVERT_TZ(check_results.timestamp_utc,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i'),'UNSPECIFIED')=D_TIM.row_id
...
where SRC.time_key<>trgt.time_key
