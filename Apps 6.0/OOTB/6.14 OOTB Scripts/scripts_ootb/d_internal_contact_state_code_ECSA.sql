SELECT CASE WHEN COUNT(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END AS Result,
    CASE WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.state_code' ELSE 'SUCCESS' END AS Message
    FROM
	...
	WHERE COALESCE(sys_user.state,'') <> COALESCE(TRGT.state_code, '')
	union 
	...
	WHERE COALESCE(csm_consumer.state,'') <> COALESCE(TRGT.state_code, ''))IC;