...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_ID as demand_key, time_sheet.WORK_ITEM_ID as work_item_id,
act_effort.total_effort total_effort,state,time_sheet.last_update_date as changed_on
from
...
and  COALESCE(convert_tz(time_sheet.changed_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'))<> COALESCE(TRGT.changed_on,'')