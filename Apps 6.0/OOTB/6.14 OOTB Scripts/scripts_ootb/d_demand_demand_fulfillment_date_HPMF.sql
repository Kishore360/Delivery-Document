...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_demand.demand_fulfillment_date' ELSE 'SUCCESS' END as Message
...
where (CASE WHEN LKP.dimension_wh_code IN ('CLOSED') THEN d_proj.actual_end_on else coalesce(null,'') end)<>COALESCE(TRGT.demand_fulfillment_date,'')
