...
where CAL.lagging_count_of_weeks between (CAL_CURR.lagging_count_of_weeks + 1) and
(select retention
...
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.field as `field`, max(wi_update.primary_sequence_id) as max_chnglog_id, min(wi_update.primary_sequence_id) as min_chnglog_id
...
and date_format(convert_tz(wi_update.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
and coalesce(wi_update.`from`, wi_update.fromstring, '') <> coalesce(wi_update.`to`, wi_update.tostring,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.field;
...
next_id.n_key, next_id.lagging_count_of_weeks
from #STG_TABLE_SCHEMA.issue_activity_azure_base wi_update
join (
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.field as `field`, 
...
date_format(convert_tz(wi_update.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') > date_format(cal.week_end_date, '%Y-%m-%d') 
and coalesce(wi_update.`from`, wi_update.fromstring, '') <> coalesce(wi_update.`to`, wi_update.tostring,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.field
) next_id
...
-- Weekly base table severity update --
...
set SRC.completed_on = case when LM.dimension_wh_code IN ('COMPLETED') then coalesce(case when date_format(tmp.completed_on, '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') 
...
and LKP.sourceinstance=LM.source_id
where lower(LKP.field) = 'status' and LM.dimension_wh_code IN ('PROPOSED') and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
...
left join #DWH_TABLE_SCHEMA.d_calendar_date cal
on SRC.n_key = cal.row_key
where date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
group by SRC.issueid, SRC.sourceinstance, SRC.n_key
) SQ
...
where lower(LKP.field) = 'status' and LM.dimension_wh_code = 'CANCELLED' and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_cncl
...
-- Weekly base table deferred_flag update --
...
where upper(LKP.field) = 'RESOLUTION' and LM_TO.dimension_wh_code in ('DEFERRED') and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) deferred_cn
...
-- Weekly base table defect_regression_flag update --
...
else timestampdiff(second,coalesce(SRC.created_on,''),coalesce(convert_tz(timestamp(CAL.week_end_date),'<<DW_TARGET_TIME_ZONE>>','<<TENANT_SSI_TIME_ZONE>>'),0)) + 86400 end,0), SRC.updated_on = current_timestamp;
