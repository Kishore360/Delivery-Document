select D.problem_task_number pt_num, 
DPROB.problem_number pnum, 
D.short_description sh_des,
app.name app_nm,
srvc.name srvc_nm,
to_char(D.opened_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') opnd_on,
to_char(D.due_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') dueon,
to_char(D.closed_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') clsd_on,
asgn_to.full_name asgn_to_nm,
org.organization_name asgn_grp,
CI.name ci_nm,
LOC.location_name loc_nm,

round(sum(distinct F.reassignment_count),0) reasgn_cnt
-- round(sum(distinct F.update_count) * 1.00, 0) updt_cnt
...
group by D.problem_task_number, DPROB.problem_number, D.short_description, app_nm, srvc_nm, D.opened_on, D.due_on, D.closed_on, asgn_to_nm, asgn_grp, ci_nm, loc_nm
order by D.problem_task_number, DPROB.problem_number, D.short_description, app_nm, srvc_nm, D.opened_on, D.due_on, D.closed_on, asgn_to_nm, asgn_grp, ci_nm, loc_nm