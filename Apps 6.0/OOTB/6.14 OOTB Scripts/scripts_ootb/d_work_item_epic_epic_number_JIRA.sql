SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item_epic.epic_number' ELSE 'SUCCESS' END as Message
...
where LKP.dimension_wh_code = 'EPIC') SRC
 ...
 WHERE SRC.key is not null and COALESCE(SRC.id,'' )<> COALESCE(TRGT.epic_number ,'')