...
round(sum(F.I_Aux_Time)*1.00/sum(F.I_Staff_Time)) time_spent_aux
...
group by CAL_MONTH.month_id,CAL_MONTH.month_name_abbreviated,F.skill_key,D.name,cal_month.year_name
order by CAL_MONTH.month_id asc,CAL_MONTH.month_name_abbreviated,F.skill_key asc ,D.name,cal_month.year_name