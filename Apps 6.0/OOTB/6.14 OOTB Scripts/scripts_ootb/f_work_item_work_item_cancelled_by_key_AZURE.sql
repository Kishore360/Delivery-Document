...
dates.cancelled_by, resolved_by.authorkey resolved_by, completed_by.authorkey completed_by,

(case when DM.dimension_wh_code in ('CANCELLED') then CONCAT('INTERNAL_CONTACT~',dates.cancelled_by)
  when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') then case when RES.dimension_wh_code in ('CANCELLED') then CONCAT('INTERNAL_CONTACT~',coalesce(dates.authorkey,dates.authorkey)) else 'UNSPECIFIED' end
...
COALESCE(LKP.row_key, case when (DM.dimension_wh_code in ('CANCELLED') or RES.dimension_wh_code in ('CANCELLED')) and CONCAT('INTERNAL_CONTACT~',dates.cancelled_by) is not null then -1 else 0 end) sr, 
