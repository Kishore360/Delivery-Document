...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster_keyword.keyword_weight' ELSE 'SUCCESS' END as Message
FROM (select 
 'X' as cdctype
...
-- ,tp.keyword as keyword
 ,cast(tp.keyword_weight as decimal(28,10)) as keyword_weight
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.keyword_weight,'')<>coalesce(TRGT.keyword_weight,'');	
 
 
 -- desc #DWH_TABLE_SCHEMA.d_cluster_keyword ;
 
 -- desc #STG_TABLE_SCHEMA.AS_PYSPARK_LATENTDIRICHLETALLOCATION
