...
on (LKP.row_id  = date_format(convert_tz(SRC.requested_by_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') and LKP.source_id=0
 
)
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.requested_by_date IS NULL THEN '' else '' end)<> COALESCE(TRGT.requested_completion_on_key,'')
