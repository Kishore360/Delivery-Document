...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.outages_caused_within_application' ELSE 'SUCCESS' END as Message
from
(select change_request_key as change_request_key,
count(distinct(outages_directly_for_change_request_same_application))+count(distinct(outages_caused_for_change_request_same_application)) as outages_for_same_application
from
...
case when (UPPER(map.dimension_name)='CLOSED' and (outa.start_on_key>=chgf.closed_on_key) and outa.change_request_key<>0 and chgf.application_key=outa.application_key) then outa.outage_key else null end as outages_directly_for_change_request_same_application,

case when (UPPER(map.dimension_name)='CLOSED' and (outa.start_on_key>=chgf.closed_on_key) and outa1.caused_by_change_key<>0 and outa1.incident_key<>0 and chgf.application_key=outa1.application_key) then outa1.outage_key else null end as outages_caused_for_change_request_same_application
...
where SRC.outages_for_same_application<>chg_failure.outages_caused_within_application;
...
case when outd.caused_by_change_flag='Y' then count(outd.caused_by_change_flag) else 0 end as outages_caused_within_application 
...
where SRC.outages_caused_within_application<>TRGT.outages_caused_within_application;