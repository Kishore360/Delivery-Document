...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.organization_active_flag' ELSE 'SUCCESS' END as Message
from (select coalesce(TRGT.organization_active_flag,''),'Y'
...
where coalesce(TRGT.organization_active_flag,'')<>'Y'

union all

select coalesce(TRGT.organization_active_flag,''),'Y'
...
select coalesce(CASE WHEN SRC.active = 1 THEN 'Y' ELSE 'N' END),coalesce(TRGT.organization_active_flag,'')
...
where coalesce(CASE WHEN SRC.active = 1 THEN 'Y' ELSE 'N' END)<>coalesce(TRGT.organization_active_flag,''))inter_org