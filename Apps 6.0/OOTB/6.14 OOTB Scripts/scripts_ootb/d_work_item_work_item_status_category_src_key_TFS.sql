SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 on concat('STATUS_CATEGORY~WORK_ITEM','~',upper(SRC.system_boardcolumn))=LKP.src_rowid
...
coalesce(LKP.row_key, case when SRC.system_boardcolumn is null then 0 else -1 end,'')<>coalesce(TRGT.work_item_status_category_src_key,'')