...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.customer_prospect_rating' ELSE 'SUCCESS' END as Message
-- select TRGT.customer_prospect_rating
...
where customer=1 and TRGT.customer_prospect_rating is not null;