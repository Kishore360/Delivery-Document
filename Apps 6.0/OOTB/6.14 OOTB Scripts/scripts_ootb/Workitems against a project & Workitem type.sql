...
count(case when (cal_mon.lagging_count_of_month between 0 and 13 or status.wi_sts in ('PROPOSED', 'IN PROGRESS')) then F.work_item_key else null end) opened_wi,
count(case when status.wi_sts in ('PROPOSED') then F.work_item_key else null end) proposed_wi,
count(case when status.wi_sts in ('IN PROGRESS') then F.work_item_key else null end) inprog_wi,
count(case when status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) rslvd_wi,
count(case when status.wi_sts in ('COMPLETED') then F.work_item_key else null end) cmpltd_wi,
count(case when D.reopened_flag = 'Y' then F.work_item_key else null end) reopnd_wi

from appsqa_tfs22_mdwdb.f_work_item F
join appsqa_tfs22_mdwdb.d_work_item D
on F.work_item_key = D.row_key and F.soft_deleted_flag = 'N'
join appsqa_tfs22_mdwdb.d_project PROJ
on D.project_key = PROJ.row_key
join appsqa_tfs22_mdwdb.d_iteration ITE
on ITE.row_key = D.iteration_key
join
(
 SELECT lov.*, lov.dimension_code AS wi_res_code, lov.dimension_name AS wi_res
   FROM appsqa_tfs22_mdwdb.d_lov lov
   LEFT JOIN appsqa_tfs22_mdwdb.d_lov_map br ON lov.row_key = br.src_key AND br.dimension_class = 'RESOLUTION~WORK_ITEM'
...
(
 SELECT lov.*, lov.dimension_code AS wi_prio_code, lov.dimension_name AS wi_prio
   FROM appsqa_tfs22_mdwdb.d_lov lov
   LEFT JOIN appsqa_tfs22_mdwdb.d_lov_map br ON lov.row_key = br.src_key
...
(
 SELECT lov.*, wh_lov.dimension_code AS wi_sts_code, wh_lov.dimension_name AS wi_sts, lov.dimension_name sor_lov_name
   FROM appsqa_tfs22_mdwdb.d_lov lov
   LEFT JOIN appsqa_tfs22_mdwdb.d_lov_map br ON lov.row_key = br.src_key
   JOIN appsqa_tfs22_mdwdb.d_lov wh_lov ON wh_lov.row_key = br.wh_key AND br.dimension_class = 'STATUS~WORK_ITEM'
UNION 
...
FROM appsqa_tfs22_mdwdb.d_calendar_date d_calendar_date
   LEFT JOIN appsqa_tfs22_mdwdb.d_calendar_date prev_day ON d_calendar_date.previous_day_key = prev_day.row_key
   LEFT JOIN appsqa_tfs22_mdwdb.d_calendar_date prev_week ON d_calendar_date.previous_week_key = prev_week.row_key AND prev_week.row_key = prev_week.week_start_date_key
   LEFT JOIN appsqa_tfs22_mdwdb.d_calendar_date prev_month ON d_calendar_date.previous_month_key = prev_month.row_key AND prev_month.row_key = prev_month.month_start_date_key
   LEFT JOIN appsqa_tfs22_mdwdb.d_calendar_date prev_quarter ON d_calendar_date.previous_quarter_key = prev_quarter.row_key AND prev_quarter.row_key = prev_quarter.quarter_start_date_key
   LEFT JOIN appsqa_tfs22_mdwdb.d_calendar_date prev_year ON d_calendar_date.previous_year_key = prev_year.row_key AND prev_year.row_key = prev_year.year_start_date_key
) cal
...
(
select *
from appsqa_tfs22_mdwdb.d_internal_contact
) INTC_ASGN
...
from appsqa_tfs22_mdwdb.d_o_data_freshness
group by source_id
) fresh
...
join appsqa_tfs22_mdwdb.d_project_version pv
on pv.row_key = D.earliest_fixed_version_key
where (cal_mon.lagging_count_of_month between 0 and 13 or 
D.proposed_flag = 'Y')
group by PROJ.project_name, type.wi_type
order by upper(PROJ.project_name), upper(type.wi_type)