SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_risk_src_key' ELSE 'SUCCESS' END as Message
 ...
 on concat('RISK~WORK_ITEM','~',upper(SRC.microsoft_vsts_common_risk))=LKP.src_rowid
...
coalesce(LKP.row_key, case when SRC.microsoft_vsts_common_risk is null then 0 else -1 end,'')<>coalesce(TRGT.work_item_risk_src_key,'');