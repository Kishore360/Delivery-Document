 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_facility_request.priority_de_escalated_flag' ELSE 'SUCCESS' END as Message
...
where TA.tablename = 'facilities_request' 
...
THEN 'Y' ELSE 'N' end,'N' )<>coalesce(TRGT.priority_de_escalated_flag,'');
