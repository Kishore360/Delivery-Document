...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.city_code' ELSE 'SUCCESS' END as Message
  FROM ( SELECT ctm_people.SourceInstance,value,Site_Country,Home_country,Default_Country,person_id from #MDS_TABLE_SCHEMA.hlx_ctm_people_final ctm_people                  
            LEFT OUTER JOIN
...
WHERE COALESCE(case WHEN SRC.value = 'Office-Based Employee' THEN SRC.Site_Country 
                    WHEN SRC.value = 'Home-Based Employee' THEN Home_country ELSE SRC.Default_Country end,
                    'UNSPECIFIED')
                    <> COALESCE(TRGT. country_code,'UNSPECIFIED') 
