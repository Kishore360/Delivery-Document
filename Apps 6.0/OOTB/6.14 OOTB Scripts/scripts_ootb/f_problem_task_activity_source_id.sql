...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_task_activity.source_id' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' 
and COALESCE(SRC.sys_id,'')  <> COALESCE(TGT.source_id,'') 
) SQ;