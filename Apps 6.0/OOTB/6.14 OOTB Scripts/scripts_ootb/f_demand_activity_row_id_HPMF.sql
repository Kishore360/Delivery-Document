SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_demand_activity.row_id' ELSE 'SUCCESS' END as Message
from (select TRGT.row_id,SRC.transaction_id from (select * from #MDS_TABLE_SCHEMA.hp_kcrt_transactions_final  where cdctype<>'D') SRC
left join #DWH_TABLE_SCHEMA.f_demand_activity TRGT
on SRC.transaction_id=TRGT.row_id
and SRC.sourceinstance=TRGT.source_id
where TRGT.primary_sequence_id<>0
and coalesce(SRC.transaction_id,'')<>coalesce(TRGT.row_id,'')
UNION 
select coalesce(concat('kcrt_requests','~',TRGT1.task_attribute_wh_name,'~',d_dem.row_id,'~',0),''),TRGT1.row_id 
from #DWH_TABLE_SCHEMA.f_demand_activity TRGT1
left join #DWH_TABLE_SCHEMA.d_demand d_dem
on TRGT1.demand_key=d_dem.row_key
where TRGT1.primary_sequence_id=0 and 
coalesce(concat('kcrt_requests','~',TRGT1.task_attribute_wh_name,'~',d_dem.row_id,'~',0),'')<>coalesce(TRGT1.row_id,''))a;