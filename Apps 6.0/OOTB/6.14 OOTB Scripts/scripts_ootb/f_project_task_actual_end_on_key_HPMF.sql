SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 AND SRC.sourceinstance=TRGT.source_id )
  WHERE COALESCE(DATE_FORMAT(CONVERT_TZ(SRC2.ACT_FINISH_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') 
  <> COALESCE(TRGT.actual_end_on_key,'UNSPECIFIED');