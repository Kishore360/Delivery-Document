...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.state_src_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(lov.row_key,case when chg.state is null then 0 else -1 end),coalesce(TRGT.state_src_key,'')
from 
...
on lov.row_id=coalesce(concat('STATE~CHANGE_REQUEST~',chg.state),'UNSPECIFIED') and lov.source_id=chg.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(lov.row_key,case when chg.state is null then 0 else -1 end)<> coalesce(TRGT.state_src_key,'')

union all

select coalesce(lov.row_key,case when chg.state is null then 0 else -1 end),coalesce(TRGT.state_src_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(lov.row_key,case when chg.state is null then 0 else -1 end)<> coalesce(TRGT.state_src_key,''))ca