...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ACTUAL_OPERATING_LOC_COST' ELSE 'SUCCESS' END as Message
...
where  0
<> ROUND(COALESCE(TRGT.ACTUAL_OPERATING_LOC_COST,''),7);