/*Report Name: Project_Program_Portfolio_Project_Allocation_QA_Resource_Task*/
...
from
(
select distinct project,program, portfolio, parent_project_name,row_key,
...
res_role.role_name primary_role, program.program_name as program,proj.row_key as row_key, proj.project_name as project, 
...
group by project,program, portfolio,parent_project_name,row_key
)
-- where resource not in ('UNKNOWN')
order by row_key,upper(project),upper(program), upper(portfolio),upper(parent_project_name)
