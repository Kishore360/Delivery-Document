...
on (LKP.row_id  = (Case when LM.dimension_wh_code='OPEN' then date_format(convert_tz(SRC.start_date, '<<TENANT_SSI_TIME_ZONE>>', '<<DW_TARGET_TIME_ZONE>>'),'%H%i') 
when LM.dimension_wh_code='CLOSED' THEN date_format(convert_tz(SRC.work_start, '<<TENANT_SSI_TIME_ZONE>>', '<<DW_TARGET_TIME_ZONE>>'),'%H%i') end) and LKP.source_id=0)
