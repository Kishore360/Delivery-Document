...
case when count(1)>0 then 'MDS to MDW data validation  failed  for d_alert.cluster_name' else 'Success' end as message2
from
...
where coalesce(src.cluster_name,'') <> coalesce(trgt.cluster_name,'');
