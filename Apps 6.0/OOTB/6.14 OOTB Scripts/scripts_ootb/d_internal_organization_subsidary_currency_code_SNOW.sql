...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.subsidary_currency_code' ELSE 'SUCCESS' END as Message
from (select TRGT.subsidary_currency_code
...
where TRGT.subsidary_currency_code is not null

union all

select TRGT.subsidary_currency_code
...
where TRGT.subsidary_currency_code is not null)inter_org;