...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM #MDS_TABLE_SCHEMA.cmn_location_final )  AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_location'  Table_DWH 
FROM #DWH_TABLE_SCHEMA.d_location WHERE row_key not in (0,-1) and soft_deleted_flag='N') DWH