...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.inventory_org_flag' ELSE 'SUCCESS' END as Message
from (select (TRGT.inventory_org_flag)
...
where (TRGT.inventory_org_flag)<>'N'

union all

select (TRGT.inventory_org_flag)
...
where (TRGT.inventory_org_flag)<>'N')inter_org