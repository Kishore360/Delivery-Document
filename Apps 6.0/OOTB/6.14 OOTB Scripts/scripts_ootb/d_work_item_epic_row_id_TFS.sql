SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item_epic.row_id' ELSE 'SUCCESS' END as Message
...
coalesce(CONCAT(SRC.system_id, '~', SRC.teamprojectsk),'')<>coalesce(TRGT.row_id,'')