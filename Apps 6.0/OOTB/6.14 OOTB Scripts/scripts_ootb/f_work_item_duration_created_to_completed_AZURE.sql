...
where coalesce(case when DM.dimension_wh_code in ('COMPLETED') and (RES.dimension_wh_code<>'CANCELLED' or RES.dimension_wh_code is null) and SRC.createdDate is not null then
 timestampdiff(second,coalesce(SRC.createdDate,0),coalesce(dates.completed_on, SRC.msvsts_common_closeddate,0)) else 0 end,0) <>coalesce(TRGT.duration_created_to_completed,'')