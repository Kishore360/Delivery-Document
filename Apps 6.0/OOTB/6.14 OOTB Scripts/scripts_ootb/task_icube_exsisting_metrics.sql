select
tsk_typ.source_dimension_name task_type,

trim(to_char(count(case when F.task_key is not null then F.task_key else null end),'FM999,999,999')) as tasks,
to_char(sum(case when D.over_due_flag='Y' and F.task_key is not null then 1 else 0 end ),'FM999,999,999,999,999')  as overdue, 
to_char(sum(case when D.backlog_flag='Y' then 1 else 0 end ),'FM999,999,999,999,999')  as backlog,
to_char(sum(F.reassignment_count_numerify),'FM999,999,999,999,999')  as reassignment_count_numerify,
to_char(sum(F.reassignment_count),'FM999,999,999,999,999')  as reassignment_count,
to_char((sum(coalesce(F.reassignment_count ,0)::bigint)/1.00) /(count(case when F.task_key is not null then F.task_key else null end)),'FM999,999,999,999,990D00')as avg_reassignment_count,
to_char((sum(coalesce(F.age ,0)::bigint) /86400.0) /(count(case when F.task_key is not null then F.task_key else null end)),'FM999,999,999,999,990D0')as age,
to_char((sum(coalesce(F.business_duration ,0)::bigint) /86400.0) ,'FM999,999,999,999,990D0')as business_duration,
to_char((sum(coalesce(F.time_worked ,0)::bigint) /86400.0) /(count(case when F.task_key is not null then F.task_key else null end)),'FM999,999,999,999,990D0')as time_worked,
to_char((sum(coalesce(F.update_count ,0)::bigint)) ,'FM999,999,999,999,990')as update_count,
to_char((sum(coalesce(F.update_count ,0)::bigint)/1.00) /(count(case when F.task_key is not null then F.task_key else null end)),'FM999,999,999,999,990D00')as avg_update_count,
to_char(round((sum( F.age_of_parent_at_task_open )/86400.0),1),'FM999,999,999,999,990.0')as age_of_parent_at_closure
...
(
 ...
 or D.backlog_flag = 'Y'
 group by tsk_typ.source_dimension_name
 order by upper(tsk_typ.source_dimension_name)