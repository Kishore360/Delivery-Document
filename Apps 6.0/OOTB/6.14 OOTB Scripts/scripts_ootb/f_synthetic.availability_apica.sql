...
-- ,case when lower(check_tags.tagname) = 'application' then coalesce(D_APP.row_key, -1) else 0 end as application_key
,CASE WHEN check_results.severity = 'I' THEN 'AVAILABLE' ELSE 'UNAVAILABLE' END AS availability
...
where SRC.availability<>trgt.availability