SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.allocation_end_on' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE COALESCE(convert_tz(SRC.PRAVAILFINISH,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),convert_tz(SRC3.SCHEDULE_FINISH,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'))<> COALESCE(TRGT.allocation_end_on,'')
