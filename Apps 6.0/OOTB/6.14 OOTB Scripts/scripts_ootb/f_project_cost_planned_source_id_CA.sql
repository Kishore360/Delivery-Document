SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.source_id' ELSE 'SUCCESS' END as Message
 FROM (select * from #MDS_TABLE_SCHEMA.fin_plans_final  where cdctype <> 'D')SRC
...
and coalesce(SRC.sourceinstance,'')
