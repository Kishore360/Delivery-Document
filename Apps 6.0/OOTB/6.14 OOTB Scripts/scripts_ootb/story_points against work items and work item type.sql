...
to_char(sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) then coalesce(F.story_points ,0) else 0 end) , 'FM999,999,999,999,990') as story_points
...
D.proposed_flag = 'Y')
group by D.work_item_number, D.row_id, type.wi_type
order by D.work_item_number, D.row_id, upper(type.wi_type)