SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for f_resource_allocation.allocation_on_key'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
...
WHERE
    trg.row_id LIKE '%ASSIGNED%'
        AND COALESCE(DATE_FORMAT(CONVERT_TZ(src.SLICE_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y%m%d'),
            '')  <> COALESCE(trg.allocation_on_key, ''); 