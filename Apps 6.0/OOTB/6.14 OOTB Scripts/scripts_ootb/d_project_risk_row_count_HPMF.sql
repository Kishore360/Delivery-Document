...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_risk.row_count' ELSE 'SUCCESS' END as Message
from (select * from 
...
from #DWH_TABLE_SCHEMA.d_project_risk TRGT where TRGT.soft_deleted_flag<>'Y' and row_key not in (-1,0))count_trgt
