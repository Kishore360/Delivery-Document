...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status.dimension_short_name' ELSE 'SUCCESS' END as Message
...
where coalesce(TRGT.dimension_short_name,'')<>null;