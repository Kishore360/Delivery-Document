select cust.organization_name cust_name,

D.multiple_assignment_flag multiple_assignment_flag,

sum(coalesce(F.reassignment_count,0)) reassignment_count,

count( case when F.case_key is not null and D.multiple_assignment_flag = 'Y' then F.case_key else null end) multiple_assignment_cases,

count( case when F.case_key is not null and D.multiple_categorization_flag = 'Y' then F.case_key else null end) multiple_categorization_cases,

concat(round(case when count( case when F.case_key is not null then F.case_key else null end) = 0 then 0
else count( case when F.case_key is not null and D.multiple_categorization_flag = 'Y' then F.case_key else null end) * 100.00/
count( case when F.case_key is not null then F.case_key else null end) end, 0), '%') multiple_categorization_cases_prct
...
group by cust_name, multiple_assignment_flag
order by upper(cust.organization_name), multiple_assignment_flag
