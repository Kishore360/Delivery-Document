...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_SET_ID as project_key, max(time_sheet.last_update_date) as last_update_date,time_sheet.WORK_ITEM_SET_ID as work_item_set_id,sum(act_effort.total_effort) total_effort,state
from
...
and  (COALESCE(convert_tz(time_sheet.last_update_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),''))<>COALESCE(TRGT.changed_on,'')
