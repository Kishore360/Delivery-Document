...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_business_service.monitor_business_service_key' ELSE 'SUCCESS' END as Message
...
,D_SERV.row_key as monitor_business_service_key
...
left outer join #DWH_TABLE_SCHEMA.d_service D_SERV
on COALESCE(CONCAT('BUSINESS_SERVICE~',check_tags.tagvalue),'UNSPECIFIED')=D_SERV.row_id
and D_SERV.source_id=checks.sourceinstance
where checks.cdctype<>'D'
and lower(check_tags.tagname) = 'business_service'
)SRC

left outer join #DWH_TABLE_SCHEMA.f_monitor_business_service trgt
on SRC.row_id=trgt.row_id
and SRC.source_id=trgt.source_id
where coalesce(SRC.monitor_business_service_key, 'Unspecified')<>coalesce(trgt.monitor_business_service_key, 'Unspecified')
