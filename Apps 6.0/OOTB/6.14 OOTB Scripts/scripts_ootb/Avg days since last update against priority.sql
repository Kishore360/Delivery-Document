...
case when count(case when D.backlog_flag = 'Y' then F.change_task_key else null end) <> 0
then round((sum(case when D.backlog_flag = 'Y' and cal_mon.lagging_count_of_month between 0 and 12 then F.dormancy_age else 0 end/86400.00))/
count(case when D.backlog_flag = 'Y' then F.change_task_key else null end),1)
else null end avg_days_snc_lst_updt
