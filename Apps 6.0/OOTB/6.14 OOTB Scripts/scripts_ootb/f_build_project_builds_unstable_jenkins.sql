...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_build_project.builds_unstable' ELSE 'SUCCESS' END as Message
...
count(case when build.__numerify__primary_key is not null and  LKP.dimension_wh_code = 'UNSTABLE'
then LKP.row_key else null end) as builds_unstable_count
...
where  COALESCE(SQ.builds_unstable_count,0) <> COALESCE(TRGT.builds_unstable,'');