...
left join(select FINANCIAL_SUMMARY_ID,FISCAL_PERIOD_ID,LABOR_TYPE_CODE,EXPENSE_TYPE_CODE,sourceinstance,max(creation_date) as creation_date,
...
WHERE COALESCE(convert_tz(SRC2.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> coalesce(TRGT.created_on,'')
