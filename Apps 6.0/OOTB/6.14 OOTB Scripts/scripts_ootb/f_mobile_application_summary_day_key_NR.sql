...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 on (DATE_FORMAT(FROM_UNIXTIME(mobile.timestamp/1000),'%Y%m%d') = lov.row_id and lov.source_id =0)
 ...
 left join #DWH_TABLE_SCHEMA.d_calendar_date lov
 on (DATE_FORMAT(FROM_UNIXTIME(MobileRequestError.timestamp/1000),'%Y%m%d') = lov.row_id and lov.source_id =0)
 left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.day_key,''))SQ
 ;