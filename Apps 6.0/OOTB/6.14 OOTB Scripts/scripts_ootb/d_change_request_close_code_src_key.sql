...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_request.close_code_src_key' ELSE 'SUCCESS' END as Message
 ...
 ON ( CONCAT('CLOSE_CODE','~','CHANGE_REQUEST','~',UPPER(SRC.close_code)) = LKP.src_rowid 
