SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_demand_cost.planned_doc_cost' ELSE 'SUCCESS' END as Message
FROM 
(select TRGT.planned_doc_cost
...
and COALESCE(plan_cost.planned_doc_cost,'')<>round(COALESCE(TRGT.planned_doc_cost,''),5)
union
select TRGT.planned_doc_cost from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
on SRC.request_id=SRC1.request_id
and SRC.sourceinstance=SRC1.sourceinstance
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
