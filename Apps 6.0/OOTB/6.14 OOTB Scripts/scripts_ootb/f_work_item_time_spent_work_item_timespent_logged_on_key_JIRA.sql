...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.work_item_timespent_logged_on_key' ELSE 'SUCCESS' END as Message from
/*
select 
...
where COALESCE(DATE_FORMAT(CONVERT_TZ(SRC.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),0)<> COALESCE(TRGT.work_item_timespent_logged_on_key,0);
