...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_task.source_url' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.Task_ID,'') <> COALESCE(TRGT.source_url,'')
