...
WHERE COALESCE(LKP.row_key,CASE WHEN (SRC.Approval_Status2 IS NULL and SRC.change_request_status not in (6,7,10,11)) 
THEN '0' else '-1'
end)<> COALESCE(TRGT.change_approval_state_src_key,'') ; ;