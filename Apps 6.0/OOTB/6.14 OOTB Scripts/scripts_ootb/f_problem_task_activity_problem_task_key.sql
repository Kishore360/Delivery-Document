...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_task_activity.problem_task_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(F.problem_task_key,'') <> COALESCE(TGT.problem_task_key,'')
union
select TGT.row_id
...
else D.row_key end,'') <> COALESCE(TGT.problem_task_key,'')
) SQ;