...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) and d.priority_escalated_flag='Y' then F.row_key else null end),'FM999,999,999,999,999')  as priority_escalated_work_item,

to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) and d.priority_deescalated_flag='Y' then F.row_key else null end),'FM999,999,999,999,999')  as priority_deescalated_work_item
