...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON (concat('PROJECT','~','STATE','~',SRC.progress) = LKP.src_rowid 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.progress IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_state_src_key,'')