 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for D_DEVICE.DEVICE_OS' ELSE 'SUCCESS' END as Message
 FROM
 ...
 WHERE COALESCE((SRC.deviceos) ,'')<>COALESCE(TRGT.device_os,'')
union
select TRGT.row_id from 
 ...
 AND SRC.sourceinstance=TRGT.source_id 
 WHERE COALESCE((SRC.platform) ,'')<>COALESCE(TRGT.device_os ,'') ) SQ
