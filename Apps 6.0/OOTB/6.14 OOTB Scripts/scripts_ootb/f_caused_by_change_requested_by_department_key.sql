...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.requested_by_department_key' ELSE 'SUCCESS' END as Message 
...
ON coalesce(concat('DEPARTMENT~',inc_contact.department_code),'UNSPECIFIED') = int_org.row_id  AND inc_contact.source_id = int_org.source_id
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(int_org.row_key,case when inc_contact.department_code is null then 0 else -1 end)
<> coalesce(TRGT.requested_by_department_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(int_org.row_key,case when inc_contact.department_code is null then 0 else -1 end)
<> coalesce(TRGT.requested_by_department_key,''))ca