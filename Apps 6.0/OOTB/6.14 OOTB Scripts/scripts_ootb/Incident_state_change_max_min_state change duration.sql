select t_state.source_dimension_name Old_State,'' as metrics,
	concat(round((max(t.primary_duration)),0),' sec') as "Max Duration For State",
	concat(round((min(t.primary_duration)),0),' sec') as "Min Duration For State"/*,
	concat(round((avg(t.primary_duration)),0),' sec') as "Avg Duration For State"*/
