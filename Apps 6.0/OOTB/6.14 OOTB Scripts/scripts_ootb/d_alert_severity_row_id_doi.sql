...
case when count(1)>0 then 'MDS to MDW data validation  failed  for d_alert_severity.row_id' else 'Success' end as message2
from 
...
where coalesce(CONCAT('SEVERITY~ALERT~',src.severity),'') <> coalesce(trgt.row_id,'');
