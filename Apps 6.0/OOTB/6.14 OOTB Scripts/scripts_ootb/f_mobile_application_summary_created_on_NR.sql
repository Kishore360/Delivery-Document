SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_mobile_application_summary.created_on' ELSE 'SUCCESS' END as Message
FROM 
(select TRGT.row_id from 
(select MF.*,round(FROM_UNIXTIME(MF.timestamp/1000-MOD(MF.timestamp/1000,600)),5) int_dur from #MDS_TABLE_SCHEMA.Mobile_final MF
...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where coalesce(mobile.int_dur ,'')  <> coalesce(round(TRGT.created_on,5),'')
 union
 select TRGT.row_id 
 from (select MF.*,round(FROM_UNIXTIME(MF.timestamp/1000-MOD(MF.timestamp/1000,600)),5) int_dur from #MDS_TABLE_SCHEMA.MobileRequestError_final MF group by MF.sessionId) MobileRequestError
left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(MobileRequestError.int_dur,'') <> coalesce(round(TRGT.created_on,5),''))SQ