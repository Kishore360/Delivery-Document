...
where COALESCE(CONVERT_TZ(sf.requested_date, '<<TENANT_SSI_TIME_ZONE>>', '<<DW_TARGET_TIME_ZONE>>'),'') <> (TRGT.requested_on)
...
ifnull(convert_tz(A.sys_created_on, '<<TENANT_SSI_TIME_ZONE>>', '<<DW_TARGET_TIME_ZONE>>'),'') <> ifnull(DWH.requested_on,'')
) SQ;
