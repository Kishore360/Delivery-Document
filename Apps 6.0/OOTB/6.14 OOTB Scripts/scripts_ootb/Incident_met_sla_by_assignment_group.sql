...
sum(case when F.outcome_flag='N' and F.exclude_flag='N' then 1 else 0 end ) as incident_met_sla
,sum(case when F.outcome_flag='Y' and F.exclude_flag='N' then 1 else 0 end ) as incident_missed_sla
,count( distinct F.row_key) as incidents_asssociated,
sum(distinct case when DIO.lev_1_organization_name not in ('UNSPECIFIED', 'UNKNOWN') then 1 else null end) hier1_cnt,
sum(distinct case when DIO.lev_2_organization_name not in ('UNSPECIFIED', 'UNKNOWN') then 1 else null end) hier2_cnt,
sum(distinct case when DIO.lev_3_organization_name not in ('UNSPECIFIED', 'UNKNOWN') then 1 else null end) hier3_cnt,
sum(distinct case when DIO.lev_4_organization_name not in ('UNSPECIFIED', 'UNKNOWN') then 1 else null end) hier4_cnt
...
group by ORG.organization_name, hier1, hier2, hier3, hier4
order by upper(ORG.organization_name), hier1, hier2, hier3, hier4