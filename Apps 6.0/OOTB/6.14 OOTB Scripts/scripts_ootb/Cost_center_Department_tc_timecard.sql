...
/*****Hours spent by  a company ***/
SELECT
d_cost.cost_center_name,
DINT_ORG.ORGANIZATION_NAME
...
ON FTC.department_key=DINT_ORG.ROW_KEY
 ...
 inner join #DWH_TABLE_SCHEMA.d_project proj
   ON FTC.project_key=proj.ROW_KEY
     inner JOIN  #DWH_TABLE_SCHEMA.d_cost_center d_cost
 ON FTC.cost_center_key=d_cost.ROW_KEY
group by d_cost.cost_center_name,DINT_ORG.ORGANIZATION_NAME
order by  d_cost.cost_center_name,DINT_ORG.ORGANIZATION_NAME;
