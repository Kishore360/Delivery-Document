...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_case_activity.task_attribute_wh_next_value_key' else 'SUCCESS' end as Message
from
...
where
COALESCE(SRC.task_attribute_wh_new_value_key,0) <> COALESCE(TRGT.task_attribute_wh_next_value_key,0)
union all
select base.row_id
