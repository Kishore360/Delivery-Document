...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.within_application_problem_flag' ELSE 'SUCCESS' END as Message 
from(select 
(case when (chg.cmdb_ci=inc.cmdb_ci and inc.cmdb_ci=prob.cmdb_ci) then 'Y' else 'N' end),coalesce(TRGT.within_application_problem_flag,'') 
from 
...
where UPPER(dimension_wh_code)='CLOSED' and (case when (chg.cmdb_ci=inc.cmdb_ci and inc.cmdb_ci=prob.cmdb_ci) then 'Y' else 'N' end)
<>coalesce(TRGT.within_application_problem_flag,'')

union all
-- direct outage
select coalesce(TRGT.within_application_problem_flag,''),'N'
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(TRGT.within_application_problem_flag,'')<>'N')ca