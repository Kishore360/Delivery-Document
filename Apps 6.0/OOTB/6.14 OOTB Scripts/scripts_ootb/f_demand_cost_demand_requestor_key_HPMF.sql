...
FROM 
(select TRGT.demand_requestor_key
...
on concat('INTERNAL_CONTACT~',SRC.created_by) = INT_CON.row_id
and SRC.sourceinstance=INT_CON.source_id
WHERE planned_benefit.period_id is not null
and COALESCE(INT_CON.row_key,CASE WHEN SRC.created_by IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.demand_requestor_key,0)
union
select TRGT.demand_requestor_key
from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
