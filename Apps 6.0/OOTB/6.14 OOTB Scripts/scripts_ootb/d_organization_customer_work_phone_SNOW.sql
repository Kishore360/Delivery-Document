...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.work_phone' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.phone,''),coalesce(TRGT.work_phone,'')
...
where customer=1 and coalesce(SRC.phone,'')<>coalesce(TRGT.work_phone,'');