...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_organization.pivot_date' ELSE 'SUCCESS' END as Message
FROM  
...
where  COALESCE(TRGT.pivot_date,'') <>null