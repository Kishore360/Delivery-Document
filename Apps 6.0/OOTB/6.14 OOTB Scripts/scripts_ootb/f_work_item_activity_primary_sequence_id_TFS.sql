...
CASE WHEN count(1) >0 THEN 'data validation failed for f_work_item_activity.primary_sequence_id'  ELSE 'SUCCESS' END as Message
from (
select TRGT.work_item_key,TRGT.task_attribute_wh_name,TRGT.primary_sequence_id,
(
select count(*) from 
(
...
where SRC.System_Id = SRC_NEXT.System_Id and SRC.TeamProjectSK = SRC_NEXT.TeamProjectSK and SRC_NEXT.System_Rev < SRC.System_Rev and TRGT.task_attribute_wh_name = 'STATUS'
)  as primary_sequence_id1 
 FROM  (
 select DWI.*, SQ.prev_state as prev_state, SQ.current_state as current_state
 from #MDS_TABLE_SCHEMA.dimworkitem_final DWI
 join 
...
select base.system_id, base.teamprojectsk, base.sourceinstance, prev.System_State prev_state, base.System_State current_state, /*next.System_State next_state,*/ base.System_Rev sys_rev
 ...
 WHERE d.soft_deleted_flag = 'N' and TRGT.task_attribute_wh_name = 'STATUS'
 ORDER BY TRGT.work_item_key,TRGT.primary_sequence_id,TRGT.task_attribute_wh_name
 ) A
