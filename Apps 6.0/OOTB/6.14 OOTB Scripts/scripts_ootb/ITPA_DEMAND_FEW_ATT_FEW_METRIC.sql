select  demand_goal,demand_manager,demand_priority,demand_requestor,demand_tshirt_size, 
...
round(((sum(no_of_fulfilled_demand+no_of_pending_demands)*100.0)/sum(no_of_demands)),1) end , '9,999,999,990D9')),'%') 
...
round(((sum(no_of_fulfilled_demand)*100.0)/sum(no_of_fulfilled_demand+no_of_pending_demands)),1) end , '9,999,999,990D9')),'%') 
end as fulfilled_to_approved_ratio,

case when (sum(no_of_fulfilled_demand)=0 and sum(no_of_demands)=0) then null
else
concat(trim(to_char(case when sum(no_of_demands)=0 then 0 else
round(((sum(no_of_fulfilled_demand)*100.0)/sum(no_of_demands)),1) end , '9,999,999,990D9')),'%') end as fulfillment_ratio,
...
group by demand_goal,demand_manager,demand_priority,demand_requestor,demand_tshirt_size, 
department, domain_name
order by UPPER(demand_goal),UPPER(demand_manager),UPPER(demand_priority),UPPER(demand_requestor),demand_tshirt_size, department, domain_name