SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_a_request_item_smpo_dppc.leader_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.leader_key, 0)<> COALESCE(TRGT.leader_key ,'');