select D.change_task_number ctn ,
D.change_task_number ctn1,
D.short_description descp,
cr.change_request_number crn,
cr.change_request_number crn1,
OPN_DIC.full_name opnd_by,
CLS_DIC.full_name clsd_by,
to_char(D.opened_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') opnd_on,
to_char(D.due_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') due_on_dt,
to_char(D.closed_on,'FMmm/FMdd/yyyy FMhh:mi:ss AM') clsd_on,
app.name app_nm,
srvc.name srvc_nm,
CI.name ci_nm,
case when F.closed_by_key in (0,-1) then OPN_DIC.full_name else CLS_DIC.full_name end emp,
LOC.location_name ct_loc,
aprvl.dimension_name approval,
rprt_typ.dimension_name cnt_typ,
impct.dimension_name impact,
prio.dimension_name priority,
state.dimension_name st,
tsk_typ.dimension_name tsktyp,
urg.dimension_name urgcy,
round(sum(distinct case when state.state = 'CLOSED' then 0 else F.update_count end), 0) updt_cnt
from
(
...
(
 SELECT lov.*, wh_lov.dimension_code AS state_code, wh_lov.dimension_name AS state
   ...
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class = 'STATE~CHANGE_TASK'
) state
...
FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.dimension_class = 'APPROVAL~CHANGE_TASK' OR lov.row_key = -1 OR lov.row_key = 0
) aprvl
...
FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class = 'PRIORITY~CHANGE_TASK'
) prio
...
FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class = 'IMPACT~CHANGE_TASK'
) impct
...
FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class = 'CONTACT_TYPE~CHANGE_TASK'
) rprt_typ
...
(
 SELECT lov.*, wh_lov.dimension_code AS tsk_typ_code, wh_lov.dimension_name AS tsk_typ
   ...
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class::text = 'TASK_TYPE~TASK'
) tsk_typ
...
FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key AND lov.row_key <> -1 AND lov.row_key <> 0
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON br.wh_key = wh_lov.row_key
  WHERE lov.row_key = -1 OR lov.row_key = 0 OR lov.dimension_class = 'URGENCY~CHANGE_TASK'
) urg
...
join #DWH_TABLE_SCHEMA.d_internal_contact asgn_to_mgr
on asgn_to_mgr.row_key = asgn_to.manager_key
join #DWH_TABLE_SCHEMA.d_internal_contact asgn_grp_mgr
on asgn_grp.manager_key = asgn_grp_mgr.row_key
join  
(
...
or D.backlog_flag = 'Y'
group by ctn, ctn1, descp, crn, crn1, opnd_by, clsd_by, D.opened_on, D.due_on, D.closed_on, app_nm, srvc_nm, ci_nm,
emp, ct_loc, approval, cnt_typ, impact, priority, st, tsktyp, urgcy
order by ctn, ctn1, descp, crn, crn1, opnd_by, clsd_by, D.opened_on, D.due_on, D.closed_on, app_nm, srvc_nm, ci_nm,
emp, ct_loc, approval, cnt_typ, impact, priority, st, tsktyp, urgcy