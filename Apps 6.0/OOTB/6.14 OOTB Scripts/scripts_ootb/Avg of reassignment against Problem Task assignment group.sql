select org.organization_name asgn_grp,
org.man_nam asgn_grp_man,

round(sum(distinct F.reassignment_count) * 1.00/count(distinct case when F.problem_task_key is not null then F.problem_task_key else null end), 0) avg_reasgn_cnt,

round(sum(F.update_count) * 1.00 / count(F.problem_task_key), 0) avg_updt_cnt
...
group by asgn_grp, asgn_grp_man
order by asgn_grp, asgn_grp_man