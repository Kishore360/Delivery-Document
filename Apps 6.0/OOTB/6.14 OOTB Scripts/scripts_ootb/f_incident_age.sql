...
and case when lm.dimension_wh_code in ('OPEN') 
    then case when coalesce( SRC.opened_at,0)> convert_tz(df.lastupdated, '<<DW_TARGET_TIME_ZONE>>', '<<TENANT_SSI_TIME_ZONE>>') then 0 else
    TIMESTAMPDIFF(SECOND, coalesce( SRC.opened_at,0), convert_tz(df.lastupdated, '<<DW_TARGET_TIME_ZONE>>', '<<TENANT_SSI_TIME_ZONE>>')) end
               else 
                case when  SRC.opened_at>coalesce( SRC.resolved_at,  SRC.closed_at, SRC.sys_updated_on) then 0 else
                TIMESTAMPDIFF(SECOND, coalesce(SRC.opened_at,0),coalesce( SRC.resolved_at,  SRC.closed_at, SRC.sys_updated_on,0))
		end end  <>TRGT.age
