...
FROM (
SELECT f1.row_key,f1.row_id,f1.change_request_key ,f2.change_request_key1, f1.primary_sequence_id,f2.primary_sequence_id1,f1.created_on, f2.created_on1,dat.lastupdated,d.closed_on,
...
LEFT JOIN( SELECT f2.change_request_key as change_request_key1, f2.primary_sequence_id as primary_sequence_id1, f2.created_on as created_on1 FROM #DWH_TABLE_SCHEMA.f_change_request_activity f2 
           WHERE f2.task_wh_type = 'change_request' and f2.task_attribute_wh_name = 'state' )f2
ON f1.change_request_key = f2.change_request_key1 AND f2.primary_sequence_id1 = f1.primary_sequence_id + 1 AND f1.task_wh_type = 'change_request' and f1.task_attribute_wh_name = 'state' 
INNER JOIN #DWH_TABLE_SCHEMA.d_change_request d ON f1.change_request_key = d.row_key and d.soft_deleted_flag = 'N'
join #DWH_TABLE_SCHEMA.f_change_request F on F.change_request_key = d.row_key
...
ORDER BY f1.change_request_key,f1.primary_sequence_id
)A 
