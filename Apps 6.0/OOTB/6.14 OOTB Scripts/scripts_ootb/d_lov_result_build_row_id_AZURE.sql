...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_result_build.row_id' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(CONCAT('RESULT~BUILD~',SRC.result), '')<>coalesce(TRGT.row_id,'')