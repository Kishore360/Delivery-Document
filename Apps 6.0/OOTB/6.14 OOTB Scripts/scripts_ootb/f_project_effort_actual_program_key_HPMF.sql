...
left join(select * from #DWH_TABLE_SCHEMA.d_program)prog
on program.program_id=prog.row_id
and SRC.sourceinstance=prog.source_id
...
and COALESCE(prog.row_key,CASE WHEN program.program_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.program_key,0)
