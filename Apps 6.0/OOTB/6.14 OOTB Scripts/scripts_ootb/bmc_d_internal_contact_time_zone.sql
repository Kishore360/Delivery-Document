...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.city_code' ELSE 'SUCCESS' END as Message
  FROM ( SELECT ctm_people.SourceInstance,value,home_time_zone,Default_time_time_zone,person_id from #MDS_TABLE_SCHEMA.hlx_ctm_people_final ctm_people                  
            LEFT OUTER JOIN
...
WHERE COALESCE(CASE WHEN SRC.value = 'Home-Based Employee' THEN SRC.home_time_zone ELSE SRC.Default_time_time_zone END,'UNSPECIFIED')
<> COALESCE(TRGT.time_zone,'UNSPECIFIED') 
