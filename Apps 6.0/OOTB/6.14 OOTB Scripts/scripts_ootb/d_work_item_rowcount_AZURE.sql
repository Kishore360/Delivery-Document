 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_work_item' ELSE 'SUCCESS' END as Message
 from (
 select *
 from
 (select count(*) count_src from #STG_TABLE_SCHEMA.workitem_temp_azure SRC where SRC.cdctype <> 'D') count_src,
 
 (select count(*) count_trgt from #DWH_TABLE_SCHEMA.d_work_item TRGT 
