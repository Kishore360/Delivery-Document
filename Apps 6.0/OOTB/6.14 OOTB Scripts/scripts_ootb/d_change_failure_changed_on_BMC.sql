...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.changed_on' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(substr(CONVERT_TZ(SRC.last_modified_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),1,19),'')
