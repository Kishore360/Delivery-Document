...
to_char(case when count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) = 0 then null
...
to_char(case when count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) = 0 then null
else round((sum(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_work_start_to_resolved else 0.0 end)/86400.00) / 
count(case when cal_rslvd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),1) end,'FM999,999,999,999,990D0') avg_duration_inprog_rslv
