...
case when count(1) > 0 then 'MDS to DWH data validation failed for problem_task_key from task_sla_final to f_task_sla' else 'SUCCESS' end as Message
...
left join #DWH_TABLE_SCHEMA.d_problem_task PROB_TASK on PROB_TASK.row_id = COALESCE(S.task,'UNSPECIFIED') and PROB_TASK.source_id = S.sourceinstance and upper(task.sys_class_name)  ='PROBLEM_TASK'
