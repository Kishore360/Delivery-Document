...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.renewalstatus' ELSE 'SUCCESS' END as Message
-- select TRGT.renewalstatus
...
where customer=1 and TRGT.renewalstatus is not null;