...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.opened_for_self_flag' ELSE 'SUCCESS' END as Message
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.hlx_hpd_help_desk_final WHERE CDCTYPE<>'D') SRC
 ...
 WHERE COALESCE( 'N' ,'')<> COALESCE(TRGT.opened_for_self_flag ,'');