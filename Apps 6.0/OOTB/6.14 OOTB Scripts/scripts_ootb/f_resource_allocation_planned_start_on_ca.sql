...
SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for f_resource_allocation.planned_start_on'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
        prassignment.sourceinstance AS source_id,
            ...
            prassignment.PRSTART AS assign_PRSTART,
            prtask.PRSTART AS prtask_PRSTART
    FROM
...
WHERE
    trg.row_id LIKE '%ASSIGNED%'
        AND COALESCE(CONVERT_TZ(coalesce(src.assign_PRSTART, src.prtask_PRSTART),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')  <> COALESCE(trg.planned_start_on, '');        