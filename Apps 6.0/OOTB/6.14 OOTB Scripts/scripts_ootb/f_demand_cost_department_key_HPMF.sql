...
left join  #DWH_TABLE_SCHEMA.d_internal_organization DEPT
on concat('DEPARTMENT~',SRC.department_code) = DEPT.row_id
and SRC.sourceinstance=DEPT.source_id
WHERE planned_benefit.period_id is not null
and COALESCE(DEPT.row_key,CASE WHEN SRC.department_code IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.department_key,0)
union
select TRGT.department_key
from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
