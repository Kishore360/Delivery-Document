...
on coalesce(date_format(COALESCE(convert_tz(SRC.scheduled_end_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d'))
 = LKP.row_id 
WHERE COALESCE( LKP.row_key, case when SRC.scheduled_end_date is null then 0 else -1 end)<> COALESCE(TRGT.due_on_key,'')