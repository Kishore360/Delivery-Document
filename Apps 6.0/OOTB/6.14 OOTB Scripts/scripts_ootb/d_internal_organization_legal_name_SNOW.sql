...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.legal_name' ELSE 'SUCCESS' END as Message
from (select (TRGT.legal_name),null
...
where (TRGT.legal_name)is not null

union all

select (TRGT.legal_name),null
...
where (TRGT.legal_name)is not null)inter_org