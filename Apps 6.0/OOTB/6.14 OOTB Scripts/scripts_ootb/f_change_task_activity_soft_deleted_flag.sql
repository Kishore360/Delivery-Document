...
LEFT JOIN #DWH_TABLE_SCHEMA.f_change_task_activity TRGT 
ON (SRC.sys_id =TRGT.row_id 
AND SRC.sourceinstance= TRGT.source_id)
WHERE TRGT.change_task_key <> -1 and TRGT.primary_sequence_id <>0 and CASE WHEN SRC.cdctype='D' THEN 'Y' ELSE 'N' END <> COALESCE(TRGT.soft_deleted_flag ,'')
union 
select TGT.row_id
...
where TGT.change_task_key <> -1 and TGT.primary_sequence_id = 0 and D.soft_deleted_flag = 'N' and COALESCE(F.soft_deleted_flag,'') <> COALESCE(TGT.soft_deleted_flag ,'')
) SQ