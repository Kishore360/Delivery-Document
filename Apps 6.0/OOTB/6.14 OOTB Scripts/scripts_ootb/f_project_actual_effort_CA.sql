...
on SRC.ID=SRC1.PRID
left join (select min(prj_blb_slices.slice_date),max(prj_blb_slices.slice_date),sum(prj_blb_slices.slice) as effort,SRC4.prprojectid,
...
where COALESCE(SRC10.effort,'0') <>round(COALESCE(TRGT.actual_effort,'0'),6)