SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_program.program_status_src_key'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
   ...
   LEFT JOIN #MDS_TABLE_SCHEMA.inv_projects_final inv_pr ON srm.id =inv_pr.prid
    ...
    #DWH_TABLE_SCHEMA.d_program trg ON (src.id = trg.row_id
        ...
        LEFT JOIN
    #DWH_TABLE_SCHEMA.d_lov LKP ON (COALESCE(CONCAT('Program~Status~', src.status),
...
WHERE
    COALESCE(LKP.row_key,
            CASE
                ...
                ELSE '-1'
            END) <> COALESCE(trg.program_status_src_key, '')
;