...
LEFT JOIN #DWH_TABLE_SCHEMA.d_internal_contact D
 on (D.row_id  = CONCAT('INTERNAL_CONTACT~',SRC.prresourceid) and D.source_id  = SRC.sourceinstance)
 WHERE COALESCE(D.row_key,CASE WHEN SRC.prresourceid is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_key,'');            
