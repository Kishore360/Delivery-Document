SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_asset.doc_currency_code' ELSE 'SUCCESS' END as Message
...
and SRC.sourceinstance=cst.sourceinstance
 ...
 ON (concat('ASSET~',SRC.sys_id)=TRGT.row_id 
  ...
  where coalesce(cst.reference_currency ,'')<>coalesce(TRGT.doc_currency_code ,'')
;
