...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.changed_on' ELSE 'SUCCESS' END as Message 
from(select convert_tz(chg.sys_updated_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),coalesce(TRGT.changed_on,'') 
from 
...
where UPPER(dimension_wh_code)='CLOSED' and convert_tz(chg.sys_updated_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')
<> coalesce(TRGT.changed_on,'')

union all

select convert_tz(chg.sys_updated_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),coalesce(TRGT.changed_on,'') 
...
where UPPER(map.dimension_wh_code)='CLOSED' and convert_tz(chg.sys_updated_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')
<> coalesce(TRGT.changed_on,''))ca