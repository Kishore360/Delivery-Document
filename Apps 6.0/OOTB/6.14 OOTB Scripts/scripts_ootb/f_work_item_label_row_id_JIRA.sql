...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_label.row_id' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(CONCAT(SRC.issueid,'~',SRC.label),'')<> COALESCE(TRGT.row_id,'');