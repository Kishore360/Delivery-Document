SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON (concat(tmp.issueid, '')=F.row_id 
 AND tmp.sourceinstance=F.source_id)

 WHERE tmp.field in ('STATUS', 'ITERATION', 'STORY POINTS', 'TIMEORIGINALESTIMATE', 'BUSINESS VALUE') and COALESCE(F.work_item_key, case when tmp.issueid is null then 0 else -1 end)<> COALESCE(TRGT.work_item_key,'');