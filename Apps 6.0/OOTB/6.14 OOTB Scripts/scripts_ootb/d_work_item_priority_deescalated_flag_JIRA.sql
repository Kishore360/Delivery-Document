...
/*select SRC.id,prio_count.issueid, prio_count.sourceinstance, prio_count.prio_esc_cnt,
...
left join
(
select SQ.issueid, SQ.sourceinstance, count(case when SQ.prio_esc_flg = 'Y' then 1 else null end) prio_esc_cnt
from
(
select base.issueid, base.sourceinstance, base.issuechangelogid, base.from, base.fromstring, base.to, base.tostring,
case when base.from > base.to then 'Y' else 'N' end prio_esc_flg
from
(
select tmp.issueid,tmp.sourceinstance, tmp.issuechangelogid, tmp.to, tmp.tostring, tmp.from, tmp.fromstring
from #STG_TABLE_SCHEMA.issue_activity_temp tmp
where lower(tmp.field) = 'priority' and tmp.from is not null
order by tmp.issueid,tmp.sourceinstance, tmp.issuechangelogid, tmp.tostring
) base
)SQ
group by SQ.issueid, SQ.sourceinstance
) prio_count

on SRC.id=prio_count.issueid
