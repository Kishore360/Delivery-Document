...
case when count(1) > 0 then 'MDS to DWH data validation failed for min_value between survey_question_new_final & asmt_metric_final to d_survey_question' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(S.scale_min,'') <> ifnull(DWH.min_value,'')
...
ifnull(A.min,'') <> ifnull(DWH.min_value,'')
) SQ;