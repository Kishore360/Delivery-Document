SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_a_request_item_smpo_dppc.calendar_date_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.calendar_date_key, -1)<> COALESCE(TRGT.calendar_date_key ,'');