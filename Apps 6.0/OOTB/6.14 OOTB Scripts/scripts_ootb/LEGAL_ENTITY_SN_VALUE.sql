...
CASE WHEN count(1) >0 THEN 'd_h_all_hierarchies f,  LEGAL_ENTITY failed' ELSE 'SUCCESS' END as Message
 ...
 FROM 
(
SELECT CONCAT('SUBSIDIARY~',a.s0,'~LEGAL ENTITY') AS row_id,
...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_0
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_1
ON(lev_0.parent = lev_1.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_2
ON(lev_1.parent = lev_2.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_3
ON(lev_2.parent = lev_3.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_4
ON(lev_3.parent = lev_4.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.core_company_final WHERE CDCTYPE<>'D') lev_5
