...
changed_on,created_on,changed_by,outage_deduction,source_id,created_by,de_prioritised_incident_deduction)
 ...
 select concat(f.assignment_group_key,f.assigned_to_key,f.department_key ,f.application_key,f.business_service_key,f.configuration_item_key) as row_id,sum(case when task_type = 'INC_MAJOR' and task_priority in ('CRITICAL','HIGH') then debit_score else 0 end) as major_incident_deduction,
sum(case when task_type = 'INC_MINOR' and task_priority in ('LOW','MEDIUM') then debit_score else 0 end) as minor_incident_deduction,
sum(case when task_type = 'CHG_CT' then debit_score else 0 end) as change_type_change_deduction,
sum(case when task_type = 'PRB_TASK' and task_priority in ('HIGH','MEDIUM','LOW') then debit_score else 0 end) as problem_task_deduction,
sum(case when task_type = 'CHG_OVERDUE' then debit_score else 0 end) as overdue_change_deduction,
f.assignment_group_key, f.assigned_to_key, f.department_key
 ,f.application_key,f.business_service_key,f.configuration_item_key, '850' as default_credit_score,
'300' minimum_credit_score,SQ.unsuccessful_changes,SQ.successful_changes,SQB.unapproved_changes,SQB.production_changes,SQB.opened_changes,
...
and timestampdiff(second,d_chg.closed_on,o_d.lastupdated)/86400  between 0 and 120
group by
...
,SUM(case when d_chg.approval_flag='N' then 1 else null end) as unapproved_changes
...
left join (select max(lastupdated) as lastupdated,source_id from #DWH_TABLE_SCHEMA.d_o_data_freshness o_d group by source_id) o_d
on o_d.source_id = f_chg.source_id
where timestampdiff(second,d_chg.opened_on,o_d.lastupdated)/86400  between 0 and 120
group by
f_chg.assignment_group_key,f_chg.assigned_to_key,inc.department_key,
