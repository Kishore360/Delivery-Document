...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.pivot_date' ELSE 'SUCCESS' END as Message
...
where COALESCE(convert_tz(SRC.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<> 
