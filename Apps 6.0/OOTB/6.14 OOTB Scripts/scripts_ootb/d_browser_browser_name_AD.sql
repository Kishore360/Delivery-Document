...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_browser.BROWSER_NAME' ELSE 'SUCCESS' END as Message
FROM
...
WHERE COALESCE(SRC.browser,'')<> COALESCE(TRGT.BROWSER_NAME,'') 