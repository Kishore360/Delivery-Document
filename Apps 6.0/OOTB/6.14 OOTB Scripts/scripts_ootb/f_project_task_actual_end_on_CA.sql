...
on SRC.prprojectid=SRC1.id
  ...
  where case when  LKP.dimension_wh_code = 'CLOSED' then COALESCE(convert_tz(SRC2.ACTUAL_END_ON,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
  else null END <>  coalesce(TRGT.actual_end_on,'')