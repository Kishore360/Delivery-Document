SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.tolerating_transaction_count' ELSE 'SUCCESS' END as Message
    from
    (select SRC.sourceinstance, SUM(CASE WHEN SRC.duration > app.enduserapdexthreshold AND SRC.duration <= ( 4*app.enduserapdexthreshold ) THEN 1 ELSE 0 END)du, CONCAT( COALESCE((DATE_FORMAT(FROM_UNIXTIME(SRC.timestamp/1000-MOD(SRC.timestamp/1000,600)),'%Y%m%d%H%i')),'UNSPECIFIED'),
 ...
 where coalesce( t.du ,'')<> COALESCE(round(TRGT.tolerating_transaction_count,5),'');
