...
where
-- case when SRC.expose_flag = 'Y' then COALESCE(SRC.task_attribute_wh_new_value,0) else 0 end <> COALESCE(TRGT.task_attribute_wh_next_value,0)
COALESCE(null,0) <> COALESCE(TRGT.task_attribute_wh_next_value,0)
union all
select base.row_id
