...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_activity.request_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(F.request_key,'') <> COALESCE(TGT.request_key,'')
union
select TGT.row_id
...
else D.row_key end,'') <> COALESCE(TGT.request_key,'')
) SQ;