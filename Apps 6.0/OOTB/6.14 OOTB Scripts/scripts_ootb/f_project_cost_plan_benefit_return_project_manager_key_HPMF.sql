...
FROM 
(select TRGT.project_manager_key from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
on concat('INTERNAL_CONTACT~',SRC.PRJ_PROJECT_MANAGER_USER_ID) = LKP.row_id
and SRC.sourceinstance=LKP.source_id
WHERE planned_benefit.period_id is not null
and COALESCE(LKP.row_key,CASE WHEN SRC.PRJ_PROJECT_MANAGER_USER_ID IS NULL THEN 0 else '-1' end)
...
and SRC.sourceinstance=TRGT.source_id
left join  #DWH_TABLE_SCHEMA.d_internal_contact LKP
on concat('INTERNAL_CONTACT~',substring(PRJ_PROJECT_MANAGER_USER_ID,1,6)) = LKP.row_id
and SRC.sourceinstance=LKP.source_id
