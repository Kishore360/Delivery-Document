...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.synonyms_final) syn
JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.hagent_final) agent ON (syn.acd_no = agent.acd)
...
JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.hagent_final where CDCTYPE <> 'D') agent
