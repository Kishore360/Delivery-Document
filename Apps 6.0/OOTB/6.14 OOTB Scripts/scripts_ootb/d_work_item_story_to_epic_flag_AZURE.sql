...
/*select SQ.__numerify__primary_key,SQ.sourceinstance, 
coalesce(if(SQ.story_to_epic_flag like '%Y%', 'Y', 'N'), 'N') sr,
coalesce(TRGT.story_to_epic_flag,'') tgt*/
from 
(
select SRC.__numerify__primary_key, SRC.sourceinstance, group_concat(case when typ.dimension_wh_code = 'STORY' then 
--  case when (isu_lnk.issueid is not null and (isu_lnk.outward_issue_type in ('EPIC') or isu_lnk.inward_issue_type in ('EPIC'))) then 'Y' else 'N' end else 'N' end) as story_to_epic_flag
case when (isu_lnk.issueid is not null and isu_lnk.inward_issue_type in ('EPIC')) then 'Y' else 'N' end else 'N' end) as story_to_epic_flag
