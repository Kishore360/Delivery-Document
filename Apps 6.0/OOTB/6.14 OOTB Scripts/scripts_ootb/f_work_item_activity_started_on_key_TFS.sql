...
ON DATE_FORMAT(convert_tz(SRC.System_ChangedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y%m%d') = CAL.row_id
 
WHERE coalesce(CAL.row_key,0) <> coalesce(TRGT.started_on_key,-1);