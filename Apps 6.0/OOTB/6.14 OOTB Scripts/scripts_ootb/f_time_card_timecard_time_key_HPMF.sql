SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.timecard_time_key' ELSE 'SUCCESS' END as Message
FROM
...
left join(select * from #DWH_TABLE_SCHEMA.d_calendar_time)LKP
on COALESCE(000)=LKP.row_id
WHERE COALESCE(LKP.row_key,0)<> COALESCE(TRGT.timecard_time_key,'0')