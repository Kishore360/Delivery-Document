SELECT CASE WHEN COUNT(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END AS Result,
    CASE WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.mobile_number' ELSE 'SUCCESS' END AS Message
    FROM
	...
	WHERE COALESCE(sys_user.mobile_phone,'') <> COALESCE(TRGT.mobile_number, '')
	union 
	...
	WHERE COALESCE(csm_consumer.mobile_phone,'') <> COALESCE(TRGT.mobile_number, ''))IC;