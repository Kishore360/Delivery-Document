...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.allocation_on_key' ELSE 'SUCCESS' END as Message
...
where date_format(COALESCE(convert_tz(SRC.working_day_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')<>
COALESCE(TRGT.allocation_on_key,'');