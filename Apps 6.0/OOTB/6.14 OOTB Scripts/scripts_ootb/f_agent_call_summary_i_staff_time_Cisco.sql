...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.i_staff_time' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.loggedOntime,'')<> COALESCE(TRGT.i_staff_time,'')
