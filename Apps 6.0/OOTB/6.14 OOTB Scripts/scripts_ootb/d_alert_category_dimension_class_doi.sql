...
case when count(1)>0 then 'MDS to MDW data validation  failed  for d_alert_category.dimension_class' else 'Success' end as message2
from
...
where coalesce(src.alarm_data_type,'') <> coalesce(trgt.dimension_class,'');
