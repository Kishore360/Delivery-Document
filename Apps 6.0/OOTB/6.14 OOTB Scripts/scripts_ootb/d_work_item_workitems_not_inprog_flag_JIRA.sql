...
coalesce(case when inprog_wi.issueid is null and STS.dimension_wh_code <> 'IN PROGRESS' then 'Y' else 'N' end, 'N') sr,
...
where coalesce(case when inprog_wi.issueid is null and STS.dimension_wh_code <> 'IN PROGRESS' then 'Y' else 'N' end, 'N')<>coalesce(TRGT.workitems_not_inprog_flag,'');