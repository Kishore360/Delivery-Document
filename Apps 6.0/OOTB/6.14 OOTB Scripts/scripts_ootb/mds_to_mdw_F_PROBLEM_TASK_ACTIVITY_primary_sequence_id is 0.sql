...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_task_activity' else 'SUCCESS' end as Message
from #DWH_TABLE_SCHEMA.f_problem_task_activity TGT
left join #DWH_TABLE_SCHEMA.f_problem_task F 
on F.problem_task_key = TGT.task_key
...
concat(
trim(SUBSTRING(CONCAT('problem_task~',TGT.task_attribute_wh_name,'~',F.row_id,'~',0),1,255)),
