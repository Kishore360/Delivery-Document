...
and E.sourceinstance=C.sourceinstance
...
WHERE COALESCE(CASE WHEN D.expense_type_code='CAPITAL' and D.labor_type_code='LABOR' then C.PLAN_CAPEX_LABOR_BSE  
WHEN D.expense_type_code='CAPITAL' and D.labor_type_code='NON_LABOR' then C.PLAN_CAPEX_NONLABOR_BSE 
WHEN D.expense_type_code='OPERATING' and D.labor_type_code='LABOR' then C.PLAN_OPEX_LABOR_BSE
WHEN D.expense_type_code='OPERATING' and D.labor_type_code='NON_LABOR' then C.PLAN_OPEX_NONLABOR_BSE else 0  END,'')<>ROUND(COALESCE(TRGT.estimated_doc_cost,''),5)
