...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.task_wh_type' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE('change_request', '') <> COALESCE(TGT.task_wh_type,'')
union all
select TGT.row_id
...
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' 
and COALESCE(SRC.tablename, '')  <> COALESCE(TGT.task_wh_type,'') 
) SQ;