...
(select count(*) + 1 from #DWH_TABLE_SCHEMA.f_incident_activity ftt1 where ftt1.task_wh_type = 'incident' AND ftt.incident_key = ftt1.incident_key and (ftt1.created_on < ftt.created_on or (ftt1.created_on = ftt.created_on and ftt1.task_attribute_wh_name < ftt.task_attribute_wh_name)))  as sequence_id1 
FROM #DWH_TABLE_SCHEMA.f_incident_activity ftt 
join #DWH_TABLE_SCHEMA.d_incident d on d.row_key = ftt.incident_key
WHERE ftt.task_wh_type = 'incident' and ftt.incident_key != -1 and d.soft_deleted_flag = 'N'
ORDER BY ftt.incident_key,ftt.sequence_id,task_attribute_wh_name
) A
