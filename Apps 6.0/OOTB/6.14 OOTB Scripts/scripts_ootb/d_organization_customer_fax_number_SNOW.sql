...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.fax_number' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.fax_phone,''),coalesce(TRGT.fax_number,'')
...
where customer=1 and coalesce(SRC.fax_phone,'')<>coalesce(TRGT.fax_number,'');