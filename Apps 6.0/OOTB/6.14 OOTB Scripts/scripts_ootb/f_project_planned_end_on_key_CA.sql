...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.planned_end_on_key' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(DATE_FORMAT(CONVERT_TZ(SRC.SCHEDULE_FINISH,
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),0) <> COALESCE(TRGT.planned_end_on_key,0);