...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.created_on' ELSE 'SUCCESS' END as Message 
from(select convert_tz(chg.sys_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),coalesce(TRGT.created_on,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and convert_tz(chg.sys_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')
<> coalesce(TRGT.created_on,'')

union all

select convert_tz(chg.sys_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),coalesce(TRGT.created_on,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and convert_tz(chg.sys_created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')
<> coalesce(TRGT.created_on,''))ca