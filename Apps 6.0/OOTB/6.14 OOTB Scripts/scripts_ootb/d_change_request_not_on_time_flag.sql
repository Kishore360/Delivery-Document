...
WHERE COALESCE( CASE WHEN (-- SRC.active= TRUE AND 
 (SRC.closed_at > SRC.end_date) -- or (datediff(SRC.closed_at,SRC.END_DATE)>0) -- updated as per change  in APP-1774
 ) THEN 'Y' ELSE 'N' END  ,'')<> COALESCE(TRGT.not_on_time_flag ,'')
