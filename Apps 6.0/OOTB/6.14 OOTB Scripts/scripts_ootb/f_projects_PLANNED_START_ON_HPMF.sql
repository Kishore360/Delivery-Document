...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.pivot_date' ELSE 'SUCCESS' END as Message
...
on SRC.START_DATE_PERIOD=SRC9.FISCAL_PERIOD_ID
and SRC.sourceinstance=SRC9.sourceinstance
left join #DWH_TABLE_SCHEMA.f_project TRGT
on TRGT.row_id = SRC.project_id 
and TRGT.source_id = SRC.sourceinstance
where   COALESCE(CONVERT_TZ(SRC9.START_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
<> COALESCE(TRGT.PLANNED_START_ON ,'');