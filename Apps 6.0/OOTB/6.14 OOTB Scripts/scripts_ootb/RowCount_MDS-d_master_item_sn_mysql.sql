...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM #MDS_TABLE_SCHEMA.sc_cat_item_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_master_item'  Table_DWH 
FROM #DWH_TABLE_SCHEMA.d_master_item WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N')  DWH
