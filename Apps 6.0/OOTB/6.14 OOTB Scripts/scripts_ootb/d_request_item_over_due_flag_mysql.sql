...
FROM #DWH_TABLE_SCHEMA.d_request_item REQ_ITM 
join #DWH_TABLE_SCHEMA.f_request_item f ON( REQ_ITM.row_key = f.request_item_key )
JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON (f.state_src_key = br.src_key AND br.dimension_wh_code = 'OPEN')
...
&& (REQ_ITM.due_on < (SELECT max(lastupdated) AS lastupdated FROM asu_mdwdb.d_o_data_freshness 
