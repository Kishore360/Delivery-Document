...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_customer.monitor_cust_count' ELSE 'SUCCESS' END as Message
...
left outer join #DWH_TABLE_SCHEMA.d_organization_customer D_ORGCUST
on COALESCE(CONCAT('CUST_ACC~',check_tags.tagvalue),'UNSPECIFIED')=D_ORGCUST.row_id
and D_ORGCUST.source_id=checks.sourceinstance
where checks.cdctype<>'D'
and lower(check_tags.tagname) = 'customer'
group by 1,2,3,4,5,6
)SRC

left outer join #DWH_TABLE_SCHEMA.f_monitor_customer trgt
on SRC.row_id=trgt.row_id
and SRC.source_id=trgt.source_id
where coalesce(SRC.monitor_cust_count, 'Unspecified')<>coalesce(trgt.monitor_cust_count, 'Unspecified')
