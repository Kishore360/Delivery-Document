...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for  f_build_project.tests_failed' ELSE 'SUCCESS' END as Message
...
left join 
(
SELECT build.__numerify__parent_key,build.sourceinstance ,sum(coalesce(TRP.failcount,TRS1.failcount,0)) failcount
...
where  COALESCE(SQ.failcount,0) <> COALESCE(TRGT.tests_failed,'');
