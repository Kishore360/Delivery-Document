...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_iteration.end_date' ELSE 'SUCCESS' END as Message
FROM (select 
...
where COALESCE(convert_tz(SRC.finishdate, '<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> COALESCE(TRGT.end_date,'')
