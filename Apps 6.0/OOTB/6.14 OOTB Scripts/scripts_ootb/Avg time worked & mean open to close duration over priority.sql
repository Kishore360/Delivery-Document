...
coalesce(case when count(F.change_task_key) <> 0 then round((sum(F.time_worked)/86400.00)/count(F.change_task_key),1) else null end, 0.0) avg_time_wkd,
case when count(F.change_task_key) <> 0 then round((sum(F.open_to_close_duration)/86400.00)/count(F.change_task_key),1) else null end mean_opn_cls_durtn
