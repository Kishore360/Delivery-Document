...
on CONCAT('PROJECT~EXPENSE_TYPE~',UPPER(SRC2.labor_type_code)) = lov.row_id
and lov.source_id=TRGT.source_id
where coalesce(lov.row_key,case when SRC2.labor_type_code is null then 0 else -1 end ,'' )<>coalesce(TRGT.project_expense_type_key,0)