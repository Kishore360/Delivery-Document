...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_work_item.work_item_reported_by_key' ELSE 'SUCCESS' END as Message
...
where coalesce(contact.row_key,case when SRC.createdByKey is null then 0 else -1 end)
<>coalesce(TRGT.work_item_reported_by_key,'');