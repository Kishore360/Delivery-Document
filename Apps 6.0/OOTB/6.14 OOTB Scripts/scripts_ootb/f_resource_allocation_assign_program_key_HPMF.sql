...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.program_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(LKP.row_key,CASE WHEN program.program_id IS NULL THEN 0 else '-1' end)<>
COALESCE(TRGT.program_key,0)