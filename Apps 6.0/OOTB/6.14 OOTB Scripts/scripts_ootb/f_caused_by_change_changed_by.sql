...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.changed_by' ELSE 'SUCCESS' END as Message 
from(select coalesce(chg.sys_updated_by,''),coalesce(TRGT.changed_by,'')
from 
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(chg.sys_updated_by,'')<> coalesce(TRGT.changed_by,'')

union all

select coalesce(chg.sys_updated_by,''),coalesce(TRGT.changed_by,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(chg.sys_updated_by,'')<> coalesce(TRGT.changed_by,''))ca