...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_issue.due_date' ELSE 'SUCCESS' END as Message
...
where coalesce(substring(convert_tz(SRC2.parameter7,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),1,19),'')<>coalesce(TRGT.due_date,'');