...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.fax_phone_number' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.fax_phone_number,'')<>coalesce(TRGT.fax_phone_number,'')  -- collate utf8_unicode_ci;
