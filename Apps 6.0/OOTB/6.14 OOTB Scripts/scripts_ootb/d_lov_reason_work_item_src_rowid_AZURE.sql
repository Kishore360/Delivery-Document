...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_reason_work_item.src_rowid' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('REASON~WORK_ITEM~',(SRC.reason)),'')<>COALESCE(TRGT.src_rowid,'')