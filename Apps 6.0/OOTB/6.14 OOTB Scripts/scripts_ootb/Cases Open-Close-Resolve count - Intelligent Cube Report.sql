select GRP.organization_name org_name, 

count(F.case_key) all_cases,

count(case when F.case_key is not null then F.case_key else null end) opened_cases,

concat(round(case when max(GRP.emp_count) = 0 then null else (count(case when F.case_key is not null then F.case_key else null end)*100.00/max(GRP.emp_count)) end, 0), '%') opened_cases_ratio,

count(case when F.case_key is not null and CAL.lagging_count_of_days <30 then F.case_key else null end) opened_cases_last_30_days,

concat(round(((count(case when F.case_key is not null and state.dimension_wh_code = 'OPEN' then F.case_key else null end) *100.00)/
(count(case when F.case_key is not null then F.case_key else null end))), 0), '%') as current_open_case_prct,

case when count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end) = 0 then null 

when count(case when F.case_key is not null and CAL.lagging_count_of_days <30 then F.case_key else null end) < 
count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end)

then concat('(', concat(round(((count(case when F.case_key is not null and CAL.lagging_count_of_days <30 then F.case_key else null end) - 
count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end))*(-100.00))/
count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end), 0), '%)'))

else concat(round(((count(case when F.case_key is not null and CAL.lagging_count_of_days <30 then F.case_key else null end) - 
count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end))*100.00)/
count(case when F.case_key is not null and CAL.lagging_count_of_days between 30 and 59 then F.case_key else null end), 0), '%') end prct_opened_cases_last_to_last_30_days,

count(case when closed.closed_case is not null and state.dimension_wh_code = 'CLOSED' then F.case_key else null end) closed_cases,

concat(round(case when count(case when F.case_key is not null then F.case_key else null end) = 0 then 0 
else (count(case when resolved.resolved_case is not null then F.case_key else null end)*100.00/count(case when F.case_key is not null then F.case_key else null end)) end, 0), '%') case_fix_rate,

count(case when resolved.resolved_case is not null and state.dimension_wh_code in ('RESOLVED', 'CLOSED') then F.case_key else null end) resolved_cases
...
group by org_name
order by upper(GRP.organization_name)
