...
(issueid, wi_number, n_date, n_key, lagging_count_of_weeks, title, wi_type, sourceinstance, due_on,
...
select issue.__numerify__primary_key, issue.id, calendar.calendar_date as n_date, calendar.row_key as n_key, calendar.lagging_count_of_weeks, 
issue.title, issue.workItemType, issue.sourceinstance, convert_tz(coalesce(issue.msvsts_scheduling_targetdate, issue.msvsts_cmmi_targetresolvedate),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>') as due_on, issue.assignedToKey, issue.projectKey, issue.state, issue.msvsts_common_resolvedreason, D.iteration_key, D.earliest_fixed_version_key,
...
where CAL.lagging_count_of_weeks between 1 and 
(select retention
from #STG_TABLE_SCHEMA.lsm_ls_dw_aggregator where subject_area = 'AZURE' and target_table = 'f_work_item_snapshot_weekly')) calendar;
...
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
when wi_update.fieldName = 'Microsoft_VSTS_Scheduling_StoryPoints' then 'STORY POINTS' when wi_update.fieldName = 'Microsoft_VSTS_scheduling_OriginalEstimate' then 'TIMEORIGINALESTIMATE' 
when wi_update.fieldName = 'Microsoft_VSTS_Common_BusinessValue' then 'BUSINESS VALUE' when wi_update.fieldName = 'Microsoft_VSTS_Common_Priority' then 'PRIORITY' 
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end) as `field`, 
max(wi_update.rev) as max_chnglog_id, min(wi_update.rev) as min_chnglog_id
from #STG_TABLE_SCHEMA.history_snapshot_weekly_temp SRC
left join #MDS_TABLE_SCHEMA.azure_workitem_update_final wi_update
on SRC.issueid=wi_update.workItemKey and SRC.sourceinstance=wi_update.sourceinstance
left join #DWH_TABLE_SCHEMA.d_calendar_date cal
on SRC.n_key = cal.row_key
where wi_update.fieldName in ('State', 'AssignedTo', 'Microsoft_VSTS_Common_Priority', 'IterationId', 'TeamProject') 
and date_format(convert_tz(wi_update.revisedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.fieldName
...
left join #DWH_TABLE_SCHEMA.d_calendar_date cal
on SRC.n_key = cal.row_key
where wi_update.fieldName in ('Microsoft_VSTS_Common_BusinessValue', 'Microsoft_VSTS_Scheduling_StoryPoints') and
date_format(convert_tz(wi_update.revisedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d')
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.fieldName;
...
from 
(select wi_update.rev as id, wi_update.workItemKey as issueid, wi_update.sourceinstance, wi_update.revisedDate as created, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
when wi_update.fieldName = 'Microsoft_VSTS_Scheduling_StoryPoints' then 'STORY POINTS' when wi_update.fieldName = 'Microsoft_VSTS_scheduling_OriginalEstimate' then 'TIMEORIGINALESTIMATE' 
when wi_update.fieldName = 'Microsoft_VSTS_Common_BusinessValue' then 'BUSINESS VALUE' when wi_update.fieldName = 'Microsoft_VSTS_Common_Priority' then 'PRIORITY' 
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end) as `field`, 
wi_update.oldValue as fromstring, coalesce(wi_update.oldValueKey, wi_update.oldValue) as `from`, coalesce(wi_update.newValueKey, wi_update.newValue) as `to`, wi_update.newValue as tostring
...
wi_update.oldValue as fromstring, coalesce(wi_update.oldValueKey, wi_update.oldValue) as `from`, coalesce(wi_update.newValueKey, wi_update.newValue) as `to`, wi_update.newValue as tostring
from #MDS_TABLE_SCHEMA.azure_workitem_update_final wi_update
where wi_update.fieldName in ('Microsoft_VSTS_Common_BusinessValue', 'Microsoft_VSTS_Scheduling_StoryPoints') 
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
) base
...
select wi_update.rev AS changelogid, wi_update.workItemKey as issueid, wi_update.sourceinstance, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
when wi_update.fieldName = 'Microsoft_VSTS_Scheduling_StoryPoints' then 'STORY POINTS' when wi_update.fieldName = 'Microsoft_VSTS_scheduling_OriginalEstimate' then 'TIMEORIGINALESTIMATE' 
when wi_update.fieldName = 'Microsoft_VSTS_Common_BusinessValue' then 'BUSINESS VALUE' when wi_update.fieldName = 'Microsoft_VSTS_Common_Priority' then 'PRIORITY' 
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end) as `field`, 
wi_update.oldValue as next_fromstring, coalesce(wi_update.oldValueKey, wi_update.oldValue) as next_from, next_id.n_key, next_id.lagging_count_of_weeks
from #MDS_TABLE_SCHEMA.azure_workitem_update_final wi_update
join (
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
...
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end) as `field`, 
min(wi_update.rev) as nxt_chnglog_id
from #STG_TABLE_SCHEMA.history_snapshot_weekly_temp SRC
left join #MDS_TABLE_SCHEMA.azure_workitem_update_final wi_update
on SRC.issueid=wi_update.workItemKey and SRC.sourceinstance=wi_update.sourceinstance
left join #DWH_TABLE_SCHEMA.d_calendar_date cal
on SRC.n_key = cal.row_key
where wi_update.fieldName in ('State', 'AssignedTo', 'Microsoft_VSTS_Common_Priority', 'IterationId', 'TeamProject') and
date_format(convert_tz(wi_update.revisedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') > date_format(cal.week_end_date, '%Y-%m-%d') 
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_weeks, wi_update.fieldName
) next_id
on next_id.nxt_chnglog_id = wi_update.rev and next_id.sourceinstance = wi_update.sourceinstance and next_id.issueid = wi_update.workItemKey 
and next_id.field = upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
when wi_update.fieldName = 'Microsoft_VSTS_Scheduling_StoryPoints' then 'STORY POINTS' when wi_update.fieldName = 'Microsoft_VSTS_scheduling_OriginalEstimate' then 'TIMEORIGINALESTIMATE' 
when wi_update.fieldName = 'Microsoft_VSTS_Common_BusinessValue' then 'BUSINESS VALUE' when wi_update.fieldName = 'Microsoft_VSTS_Common_Priority' then 'PRIORITY' 
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end)
where wi_update.fieldName in ('State', 'AssignedTo', 'Microsoft_VSTS_Common_Priority', 'IterationId', 'TeamProject') 
...
set SRC.due_on = convert_tz(coalesce(LKP.`to`, coalesce(issue.msvsts_scheduling_targetdate, issue.msvsts_cmmi_targetresolvedate)),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), SRC.updated_on = current_timestamp;
...
-- Weekly base table title update --
...
where lower(LKP.field) = 'status' and LM.dimension_wh_code='COMPLETED' and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'')
...
where lower(chnglogitems.field) = 'status' and LM.dimension_wh_code IN ('PROPOSED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(chnglogitems.`from`,'') <> coalesce(chnglogitems.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
...
on concat('STATUS~WORK_ITEM~',upper(LKP.`to`))=LM.src_rowid
and LKP.sourceinstance=LM.source_id
where lower(LKP.field) = 'status' and LM.dimension_wh_code IN ('RESOLVED') and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_res
...
where lower(LKP.field) = 'status' and (LM.dimension_wh_code = 'CANCELLED' or LM_RES.dimension_wh_code in ('CANCELLED')) and date_format(convert_tz(LKP.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') and coalesce(LKP.`from`,'') <> coalesce(LKP.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp_cncl
...
then coalesce(case when date_format(convert_tz(issue.msvsts_common_resolveddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.week_end_date, '%Y-%m-%d') 
...
reopened_flag, overdue_flag, last_updated_date, n_date, n_key, project_key, reassignment_count, reopen_count, row_id, soft_deleted_flag, source_id, title, 
...
where CAL.lagging_count_of_month between 1 and 
(select retention
...
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
when wi_update.fieldName = 'Microsoft_VSTS_Scheduling_StoryPoints' then 'STORY POINTS' when wi_update.fieldName = 'Microsoft_VSTS_scheduling_OriginalEstimate' then 'TIMEORIGINALESTIMATE' 
when wi_update.fieldName = 'Microsoft_VSTS_Common_BusinessValue' then 'BUSINESS VALUE' when wi_update.fieldName = 'Microsoft_VSTS_Common_Priority' then 'PRIORITY' 
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
...
on SRC.n_key = cal.row_key
where wi_update.fieldName in ('State', 'AssignedTo', 'Microsoft_VSTS_Common_Priority', 'IterationId', 'TeamProject') 
and date_format(convert_tz(wi_update.revisedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d')
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, wi_update.fieldName
...
on SRC.n_key = cal.row_key
where wi_update.fieldName in ('Microsoft_VSTS_Common_BusinessValue', 'Microsoft_VSTS_Scheduling_StoryPoints') and
date_format(convert_tz(wi_update.revisedDate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d')
and coalesce(wi_update.oldValueKey, wi_update.oldValue, '') <> coalesce(wi_update.newValueKey, wi_update.newValue,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, wi_update.fieldName;
...
when wi_update.fieldName = 'AssignedTo' then 'ASSIGNEE' when wi_update.fieldName = 'WorkItemType' then 'ISSUETYPE'  
when wi_update.fieldName = 'TeamProject' then 'PROJECT' end) as `field`, 
wi_update.oldValue as next_fromstring, coalesce(wi_update.oldValueKey, wi_update.oldValue) as next_from, next_id.n_key, next_id.lagging_count_of_month
from #MDS_TABLE_SCHEMA.azure_workitem_update_final wi_update
join (
select SRC.issueid,SRC.sourceinstance, SRC.n_key, SRC.lagging_count_of_month, 
upper(case when wi_update.fieldName = 'State' then 'STATUS' when wi_update.fieldName = 'IterationId' then 'ITERATION' 
...
on concat('STATUS~WORK_ITEM~',upper(chnglogitems.`from`))=LM.src_rowid
and chnglogitems.sourceinstance=LM.source_id
where lower(chnglogitems.field) = 'status' and LM.dimension_wh_code IN ('PROPOSED') and date_format(convert_tz(chnglog.created,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '%Y-%m-%d') <= date_format(cal.month_end_date, '%Y-%m-%d') and coalesce(chnglogitems.`from`,'') <> coalesce(chnglogitems.`to`,'')
group by SRC.issueid,SRC.sourceinstance, SRC.n_key
) tmp
on tmp.issueid = SRC.issueid and tmp.sourceinstance = SRC.sourceinstance and tmp.n_key = SRC.n_key
