SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_board_column_done_flag' ELSE 'SUCCESS' END as Message
...
coalesce(case when SRC.system_boardcolumndone = 1 then 'Y' else 'N' end,'N')<>coalesce(TRGT.work_item_board_column_done_flag,'')