...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.project_task_key' ELSE 'SUCCESS' END as Message
FROM
(SELECT
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_project_task LKP 
 ON ( src.prtask_prid = LKP.row_id 
AND src.prassignment_sourceinstance = LKP.source_id )
 WHERE trg.row_id like '%ASSIGNED%' and
 COALESCE(LKP.row_key,CASE WHEN src.prtask_prid IS NULL THEN 0 else '-1' end)<> COALESCE(trg.project_task_key ,'');
