...
((select count(*) from #DWH_TABLE_SCHEMA.f_problem_activity ftt1 where ftt1.task_wh_type = 'problem' and ftt1.task_attribute_wh_name = 'problem_state' 
 AND ftt.problem_key = ftt1.problem_key and (ftt1.created_on < ftt.created_on or (ftt1.created_on = ftt.created_on and ftt1.task_attribute_wh_name < ftt.task_attribute_wh_name))))  as primary_sequence_id1 
FROM #DWH_TABLE_SCHEMA.f_problem_activity ftt 
join #DWH_TABLE_SCHEMA.d_problem d on d.row_key = ftt.problem_key
WHERE ftt.task_wh_type = 'problem' and ftt.task_attribute_wh_name = 'problem_state' and ftt.problem_key != -1 and d.soft_deleted_flag = 'N'
