...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status_issue.dimension_name' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.status_name,'')<>coalesce(TRGT.dimension_name,'');