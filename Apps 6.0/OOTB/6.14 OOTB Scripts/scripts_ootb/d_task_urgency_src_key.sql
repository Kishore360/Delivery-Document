...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_task.urgency_src_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
	ON ( CONCAT('URGENCY~TASK~',URGENCY) = LKP.row_id
...
WHERE COALESCE(inc.urgency_src_key,prob.urgency_src_key,prob_tas.urgency_src_key,req_item.urgency_src_key
,req_tas.urgency_src_key,req.urgency_src_key,chan_req.urgency_src_key,chan_tas.urgency_src_key,LKP.row_key
,CASE WHEN SRC.urgency IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.urgency_src_key ,'')
