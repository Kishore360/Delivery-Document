/*known issue for few records seconds difference comes*/
...
'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED') AS Calendar_Date_key,time_sheet.WORK_ITEM_ID as demand_key,
 time_sheet.WORK_ITEM_ID as work_item_id,sum(act_effort.total_effort) total_effort,state,
 max(time_sheet.creation_date) as created_on
 from
...
and  COALESCE(convert_tz(time_sheet.created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'))<> COALESCE(TRGT.created_on,'')