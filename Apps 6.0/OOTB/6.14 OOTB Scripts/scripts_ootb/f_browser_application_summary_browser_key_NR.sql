SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_browser_application_summary.browser_key' ELSE 'SUCCESS' END as Message
	 ...
	 left join #DWH_TABLE_SCHEMA.d_browser lov
	 on lov.row_id =CONCAT(SRC.userAgentName, '~', SRC.userAgentVersion, '~', SRC.userAgentOS, '~', SRC.deviceType)
and lov.source_id = SRC.sourceinstance
 WHERE COALESCE(lov.row_key, case when SRC.userAgentName is null then 0 else '-1' end)<> COALESCE(TRGT.browser_key,0);
