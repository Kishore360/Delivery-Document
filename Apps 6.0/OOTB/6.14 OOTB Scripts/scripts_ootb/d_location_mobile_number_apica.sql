...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_location.mobile_number' ELSE 'SUCCESS' END as Message
...
and SRC.source_id=TRGT.source_id -- collate utf8_unicode_ci
 where coalesce(SRC.mobile_number,'')<>coalesce(TRGT.mobile_number,'')  -- collate utf8_unicode_ci;
