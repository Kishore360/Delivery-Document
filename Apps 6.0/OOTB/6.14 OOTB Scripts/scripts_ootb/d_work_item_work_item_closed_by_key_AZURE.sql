SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 /*SELECT TRGT.row_id, TRGT.row_key,DM.dimension_wh_code, SRC.assignedToKey, dates.closed_by,
(case when DM.dimension_wh_code not in ('COMPLETED') then 'UNSPECIFIED'
 when DM.dimension_wh_code='COMPLETED' then CONCAT('INTERNAL_CONTACT~',dates.closed_by)
 when ASCII(COALESCE(SRC.assignedToKey,'')) = 0 THEN 'UNSPECIFIED'
