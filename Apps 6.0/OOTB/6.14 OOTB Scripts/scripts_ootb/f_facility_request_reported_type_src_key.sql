...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_facility_request.reported_type_src_key' ELSE 'SUCCESS' END as Message
 ...
 ON ( CONCAT('CONTACT_TYPE','~','FACILITIES','~',UPPER(contact_type)) = LKP.src_rowid 
