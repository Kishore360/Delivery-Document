...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.priority_de_escalation_count' ELSE 'SUCCESS' END as Message
...
THEN 1 ELSE 0 END) AS priority_de_escalation_cnt
 ...
 AND TA.tablename = 'hpd_help_desk' 
...
FROM #MDS_TABLE_SCHEMA.bmc_field_enum_values_final EV
JOIN #MDS_TABLE_SCHEMA.bmc_field_final F ON (EV.fieldId=F.fieldId AND EV.schemaId=F.schemaId and EV.sourceinstance = F.sourceinstance)
JOIN #MDS_TABLE_SCHEMA.bmc_arschema_final S ON (F.schemaId = S.schemaId AND F.sourceinstance = S.sourceinstance)
WHERE  F.fieldName  ='priority'
