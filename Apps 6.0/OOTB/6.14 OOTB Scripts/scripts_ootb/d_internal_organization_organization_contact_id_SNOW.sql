...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.organization_contact_id' ELSE 'SUCCESS' END as Message
from (select coalesce(SRC.contact,''),coalesce(TRGT.organization_contact_id,'')
...
where coalesce(SRC.contact,'')<>coalesce(TRGT.organization_contact_id,'')

union all

select coalesce(SRC.primary_contact,''),coalesce(TRGT.organization_contact_id,'')
...
where  coalesce(SRC.primary_contact,'')<>coalesce(TRGT.organization_contact_id,'')

union all

select coalesce(TRGT.organization_contact_id),null
...
where  (TRGT.organization_contact_id)is not null)inter_org