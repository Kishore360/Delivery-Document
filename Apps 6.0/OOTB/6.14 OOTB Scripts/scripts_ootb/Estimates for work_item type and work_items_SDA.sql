...
to_char((sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then coalesce(F.time_estimate ,0) else 0.0 end) / 3600.00), 'FM999,999,999,999,990D00') as original_estimate_created, -- based on the opened date
to_char((sum(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (status.wi_sts = 'IN PROGRESS' or status.wi_sts in ('PROPOSED'))) then coalesce(F.time_remaining ,0) else 0.0 end) /3600.00),'FM999,999,999,999,990D00') as current_estimate_created, -- based on the opened date
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_estimate ,0) else 0.0 end) / 3600.00), 'FM999,999,999,999,990D00') as original_estimate_closed, -- based on the closed date
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_remaining ,0) else 0.0 end) / 3600.00), 'FM999,999,999,999,990D00') as current_estimate_closed -- based on the closed date
