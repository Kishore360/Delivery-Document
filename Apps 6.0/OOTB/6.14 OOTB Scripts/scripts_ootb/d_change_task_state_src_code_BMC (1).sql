...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_task.state_src_code' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.status,'') <> COALESCE(TRGT.state_src_code,'')
