...
,coalesce(SRC.changed_by,''),coalesce(convert_tz(SRC.created_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),''),coalesce(convert_tz(SRC.changed_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') ) 
...
inventory_org_flag,''),coalesce(TRGT.subsidiary_flag,''),coalesce(TRGT.classification_flag,''),coalesce(TRGT.group_flag,''),coalesce(TRGT.created_by,''),coalesce(TRGT.changed_by,''),coalesce(TRGT.created_on,''),coalesce(TRGT.changed_on,'')) COLLATE utf8_unicode_ci ;
