...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.day_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.row_date / starttime_utc IS NULL THEN 0 else '-1' end) <> COALESCE(TRGT.day_id,'')
