...
on (LKP.row_id  = date_format(convert_tz(coalesce(dates.completed_on, SRC.msvsts_common_closeddate),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') and LKP.source_id=0)
...
on concat('STATUS~WORK_ITEM~',upper(SRC.`state`))=DM.src_rowid and SRC.sourceinstance = DM.source_id
 ...
 where coalesce(case when DM.dimension_wh_code='COMPLETED' and (RES.dimension_wh_code<>'CANCELLED' or RES.dimension_wh_code is null) then LKP_COMPLETED.row_key else 0 end,0) <>coalesce(TRGT.work_item_completed_on_key,'0');