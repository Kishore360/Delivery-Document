select  -- F.*,
concat(cal_week.month_name_abbreviated::text, concat(' '::text, "substring"(cal_week.week_start_date_key::text, 7, 8))) wk_nm,
cal_week.year_id yr_nm,
type.wi_type wi_typ,
...
on cal.row_key = case when status.wi_sts in ('COMPLETED') and F.soft_deleted_flag = 'N' then coalesce(F.work_item_completed_on_key,0) 
...
else coalesce(F.work_item_created_on_key,0) end
join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
on cal_week.row_key = cal.week_start_date_key
/*join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
where (cal_mon.lagging_count_of_month between 0 and 12 or 
D.proposed_flag = 'Y')
group by cal_week.week_id, wk_nm, cal_week.year_id, type.wi_type
order by cal_week.week_id desc, wk_nm, cal_week.year_id desc, upper(type.wi_type)