...
CASE WHEN COUNT(1) >0 THEN 'state_created_on did not match for f_change_request_activity' 
...
/*COALESCE(case when TGT.task_attribute_wh_name ='state' then TGT.created_on
when TGT.task_attribute_wh_name = 'assigned_to' then TGT.state_created_on 
when TGT.task_attribute_wh_name = 'assignment_group' then TGT.state_created_on 
else null
end,'')*/
COALESCE(null,'') <> COALESCE(TGT.state_created_on,'')
union
select TGT.row_id
from ( SELECT * FROM #MDS_TABLE_SCHEMA.bmc_chg_infrastructure_change_sys_audit_final SRC
join (select max(etl_run_number) as etl_run_no FROM #MDS_TABLE_SCHEMA.bmc_chg_infrastructure_change_sys_audit_final WHERE CDCTYPE<>'D') etl_run_no on etl_run_no.etl_run_no = SRC.etl_run_number
...
COALESCE(null,'')  <> COALESCE(TGT.state_created_on,'') )SQ;