...
join (select l.assigned_to_key,l.incident_key,l.primary_sequence_id
...
where case when TRGT_A.task_attribute_wh_name ='assigned_to'then TRGT_A.task_attribute_wh_new_value_key 
when TRGT_A.task_attribute_wh_name = 'assignment_group' then TRGT_S.assigned_to_key 
else null
end <> TRGT_A.assigned_to_key ;
