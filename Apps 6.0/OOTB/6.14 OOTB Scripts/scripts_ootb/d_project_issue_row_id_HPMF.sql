...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project_issue.row_id' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.request_id,'')<>coalesce(TRGT.row_id,'');