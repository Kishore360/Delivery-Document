...
/*****No. of hours spent for  a particular task/project***/
select project_name,task_type,
case when effort<0 then '('||to_char(abs(effort),'FM99,999')||')' else ''||to_char(abs(effort),'FM99,999') end as time_entered
from (
SELECT 
proj.row_key,
proj.project_name,
...
--D2.task_number
,round(SUM(FTC.TIME_ENTERED)) ::integer as effort
...
( lo.row_key in (0,-1))
 ...
 on lo_type.row_key=D2.task_type_src_key
...
group by proj.row_key ,
proj.project_name,lo_type.dimension_name --,D2.task_number

order by -- proj.row_key  ,
lower(proj.project_name) ,lo_type.dimension_name--,D2.task_number
,effort asc 

)order by lower(replace(project_name,'_','0')) ,
lower(task_type),effort-- ,time_entered asc