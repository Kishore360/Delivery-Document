...
left join
(select 
CONCAT(time_sheet.WORK_ITEM_SET_ID,'~',COALESCE(DATE_FORMAT(CONVERT_TZ(DATE_ADD(ktmg_period.START_DATE, INTERVAL COALESCE(act_effort.effort_order,0) DAY),'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED')) as actual_row_id,
 time_sheet.WORK_ITEM_SET_ID as work_item_set_id,act_effort.total_effort
 from
...
left join
(SELECT CONCAT(tm_actuals.TIME_SHEET_LINE_ID,'~',COALESCE(tm_actuals_effort.EFFORT_ORDER,'UNSPECIFIED')) AS row_id,
...
tm_actuals_effort.EFFORT_ORDER AS effort_order, tm_actuals.TIME_SHEET_LINE_ID FROM
#MDS_TABLE_SCHEMA.hp_tm_actuals_final tm_actuals  
LEFT OUTER JOIN
#MDS_TABLE_SCHEMA.hp_tm_actuals_effort_final tm_actuals_effort 
...
on time_sheet.time_sheet_line_id=act_effort.timesheet_line_id group by actual_row_id)time_sheet
on SRC.PRJ_PROJECT_ID=time_sheet.work_item_set_id
left join(select * from #DWH_TABLE_SCHEMA.f_project_effort)TRGT
on CONCAT('ACTUAL','~',SRC.PRJ_PROJECT_ID,'~',time_sheet.actual_row_id)=TRGT.row_id
WHERE time_sheet.actual_row_id is not null and TRGT.row_id not in (CONCAT('PLANNED','~',SRC.PRJ_PROJECT_ID,'~',time_sheet.actual_row_id))
and  COALESCE(TRGT.application_key,0)<>0
