...
on concat('RESOLUTION~WORK_ITEM','~',upper(SRC.resolution_id))=RES.src_rowid and SRC.sourceinstance=RES.source_id and RES.dimension_class = 'RESOLUTION~WORK_ITEM'
 ...
 where coalesce(case when LM.dimension_wh_code = 'COMPLETED' and (RES.dimension_wh_code<>'CANCELLED' or RES.dimension_wh_code is null) then LKP_COMPLETED.row_key else 0 end,0) <>coalesce(TRGT.completed_on_key,0);