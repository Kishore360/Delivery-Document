...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.planned_end_on_key' ELSE 'SUCCESS' END as Message
FROM (select S.*,cd.calendar_date  from ( SELECT * FROM #MDS_TABLE_SCHEMA.resource_allocation_final WHERE CDCTYPE<>'D') S 
JOIN #DWH_TABLE_SCHEMA.d_calendar_date cd ON cd.calendar_date BETWEEN S.end_date AND S.end_date
...
ON (concat(SRC.sys_id,'~', date_format(SRC.calendar_date,'%Y%m%d'))  =TRGT.row_id  
 ...
 ON ( date_format(RP.end_date,'%Y%m%d') = LKP.row_id 
AND 0= LKP.source_id )

 WHERE COALESCE(LKP.row_key,CASE WHEN RP.end_date IS NULL THEN '' else '' end)<> COALESCE(TRGT.planned_end_on_key,'')
