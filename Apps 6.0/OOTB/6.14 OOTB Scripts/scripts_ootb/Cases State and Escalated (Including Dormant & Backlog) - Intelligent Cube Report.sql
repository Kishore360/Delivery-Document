...
D.backlog_flag backlog,

sum(coalesce(F.reassignment_count,0)) reassignment_count,

count( case when F.case_key is not null and state.dimension_wh_code = 'OPEN' and D.priority_escalated_flag = 'Y' then F.case_key else null end) open_case_escalated,

count( case when F.case_key is not null and D.dormant_flag = 'Y' then F.case_key else null end) dormant_cases,

count( case when resolved.resolved_case is not null and D.first_call_resolution_flag = 'Y' then F.case_key else null end) first_call_res_cases,

count( case when F.case_key is not null and D.backlog_flag = 'Y' then F.case_key else null end) backlog_cases
...
group by case_no, state, backlog
order by case_no, state, backlog
