SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure.reschedule_flag' ELSE 'SUCCESS' END as Message
FROM 
(SELECT CASE WHEN TA.original_request_id IS NULL THEN 0 ELSE COUNT(*) END as reschedule_count
,SRC.infrastructure_change_id,SRC.sourceinstance
...
WHERE (case when FSRC.reschedule_count>0  then 'Y' else 'N' end) <>COALESCE(TRGT.rescheduled_flag,'')