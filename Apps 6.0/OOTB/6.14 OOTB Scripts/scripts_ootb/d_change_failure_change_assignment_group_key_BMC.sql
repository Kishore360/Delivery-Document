...
ON (CONCAT('GROUP~', SRC.asgrpid) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.asgrpid IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.change_assignment_group_key,'')
