...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.subsidiary_flag' ELSE 'SUCCESS' END as Message
from (select (TRGT.subsidiary_flag)
...
where (TRGT.subsidiary_flag)<>'N'

union all

select (TRGT.subsidiary_flag)
...
where (TRGT.subsidiary_flag)<>'N')inter_org