...
left join #DWH_TABLE_SCHEMA.d_demand demand
on demand.row_id = SRC.request_id 
and SRC.sourceinstance = demand.source_id
WHERE planned_benefit.period_id is not null
and COALESCE(demand.row_key,case when SRC.request_id is null then 0 else -1 end)<> COALESCE(TRGT.demand_key,0)
union
select TRGT.demand_key
from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
