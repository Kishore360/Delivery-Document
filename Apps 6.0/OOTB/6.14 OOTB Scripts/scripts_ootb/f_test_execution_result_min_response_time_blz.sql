...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.min_response_time' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.minresponsetime,'')<>coalesce(TRGT.min_response_time,'')
