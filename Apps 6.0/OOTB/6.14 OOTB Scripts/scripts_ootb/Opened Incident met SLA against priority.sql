...
on CAL.row_key = F.opened_on_key -- case when state.state in ('CLOSED', 'RESOLVED') then F.last_resolved_on_key else F.opened_on_key end
join 
(
 SELECT d_calendar_date.*
   FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
...
(
  ...
  WHERE d_lov.dimension_class = 'SLA_FLAG_WH~TASK_SLA' OR d_lov.row_key = -1 OR d_lov.row_key = 0
) d_task_sla_reso
