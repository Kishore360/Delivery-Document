...
ON (date_format(convert_tz(SRC.system_createddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') = LKP.row_id 
 AND SRC.sourceinstance=LKP.source_id)
 WHERE coalesce(LKP.row_key, case when SRC.system_createddate is null then 0 else 0 end) <> coalesce(TRGT.work_item_created_on_key,'');