...
to_char(round((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_estimate ,0.00) else 0.00 end) / 3600.00), 2), 'FM999,999,999,999,990D00') as original_estimate_closed, -- based on the closed date
