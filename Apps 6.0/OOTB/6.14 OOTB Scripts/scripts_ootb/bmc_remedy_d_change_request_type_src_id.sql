...
on(CONCAT('TYPE','~','CHANGE_REQUEST','~',UPPER(SRC.change_timing))= lov.row_id)
 WHERE coalesce(lov.row_key,case when SRC.change_timing is null then 0 else -1 end)<>COALESCE(TRGT.type_src_key,'')