SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.row_id' ELSE 'SUCCESS' END as Message
FROM
...
on (LKP.row_id  = CONCAT('RES_TASK~',SRC1.resource_id,'~',SRC2.work_item_id) and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC1.resource_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_plan_key,'');   
