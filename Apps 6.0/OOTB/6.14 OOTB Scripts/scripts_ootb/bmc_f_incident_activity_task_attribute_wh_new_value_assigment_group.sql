...
on (TGT.incident_key=TGT2.incident_key and TGT2.primary_sequence_id = 1 and TGT2.task_attribute_wh_name = 'assignment_group')
where
...
join (select max(etl_run_number) as etl_run_no FROM #MDS_TABLE_SCHEMA.bmc_hpd_help_desk_sys_audit_final WHERE CDCTYPE<>'D') etl_run_no on etl_run_no.etl_run_no = SRC.etl_run_number
 WHERE SRC.CDCTYPE<>'D') SRC
left join 
(
