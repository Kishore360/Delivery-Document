 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_problem_fact.rule_pass_status' ELSE 'SUCCESS' END as Message
 from
-- select distinct row_id_der, row_id,dq_target_value, target_column_value,dq_rule_pass_status,rule_pass_status FROM --  (select * from #DWH_TABLE_SCHEMA.f_dq_problem_fact  where target_column not like 'dq%') dq_fact left join
