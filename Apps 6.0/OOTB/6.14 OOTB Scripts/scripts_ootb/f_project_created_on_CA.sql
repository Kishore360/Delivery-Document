...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.created_on' ELSE 'SUCCESS' END as Message
FROM
...
where COALESCE(convert_tz(SRC1.created_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> COALESCE(TRGT.created_on,'')