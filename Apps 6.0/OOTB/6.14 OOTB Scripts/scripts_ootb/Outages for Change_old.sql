...
(case when REQ.change_request_key is null then 'UNSPECIFIED' else D_REQ.short_description end) req_des,
--'' as metrics,
sum(case when outage_key is not null then 1 else 0 end) outages,
sum(case when INC.caused_by_change_key is not null and REQ.change_request_key is not null
and INC.caused_by_change_key = REQ.change_request_key and outage_key is not null
...
left join #DWH_TABLE_SCHEMA.d_change_request D_REQ
on D_REQ.row_key = REQ.change_request_key
...
group by F.outage_key, D.short_description, req_des
