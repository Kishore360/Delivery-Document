...
from
(
select infrastructure_change_id,  numerify_change_failure_flag, ci_prior_changes, ci_key, ci_prior_Y,
case when ci_prior_changes=0 then -1 when ci_prior_changes=-1 then -2 
else round((ci_prior_Y/ci_prior_changes),6) end as prior_failure_rate_qa
from
(
SELECT OUTR.infrastructure_change_id, numerify_change_failure_flag, ci_prior_changes, OUTR.ci_key, 
...
from
(
SELECT s1.*, chg.numerify_change_failure_flag, LKP.dimension_wh_code,chg.ci_key FROM 
...
from
(
select SRC.infrastructure_change_id, SRC.closed_date, chg.ci_key, SRC.scheduled_start_date, SRC.sourceinstance, LKP.dimension_wh_code, 
