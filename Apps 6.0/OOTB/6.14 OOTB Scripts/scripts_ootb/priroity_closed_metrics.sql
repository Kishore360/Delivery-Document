...
tsk_prio.wh_dimension_code prio_cde,

trim(to_char(count (case when tsk_st.wh_dimension_name in ('RESOLVED', 'CLOSED') and F.task_key is not null then F.task_key else null end),'999,999,999')) as closed_tasks,
to_char((sum(coalesce(F.open_to_close_duration ,0)) ::bigint/86400.0),'FM999,999,999,999,990D0') as open_to_close,

to_char((sum(coalesce(F.open_to_close_duration ,0)) /86400.0) /(count(1)::decimal(10,1)),'FM99,999,999,999,990.0')as open_to_close_mean,  

to_char(round((sum(case when  tsk_st.wh_dimension_name in ('RESOLVED', 'CLOSED') then f.age_of_parent_at_task_closure else 0 end)/86400.0),1) ,'FM999,999,999,999,990D0')as age_of_parent_at_closure
...
(
 ...
 or D.backlog_flag = 'Y'
 group by tsk_typ.source_dimension_name, tsk_prio.wh_dimension_code
 order by upper(tsk_typ.source_dimension_name), upper(tsk_prio.wh_dimension_code)