 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_incident_fact_dq_age.dq_rule_pass_status' ELSE 'SUCCESS' END as Message
 -- select distinct row_id_der, row_id,dq_target_value, target_column_value,dq_rule_pass_status,rule_pass_status  FROM  (select * from #DWH_TABLE_SCHEMA.f_dq_incident_fact  where target_column='dq_age' and soft_deleted_flag='N')dq_fact  left join
from 
(
select distinct concat('dq_age','~',inc.row_key)  as row_id_der ,rule,rule_type,'dq_age' as dq_target_column, inc.row_key as target_key
...
left join #STG_TABLE_SCHEMA.dq_issue_inc dq_issue
on dq_issue.target_key=inc.row_key
...
left join (select * from #DWH_TABLE_SCHEMA.f_dq_incident_fact  where target_column='dq_age' and soft_deleted_flag='N')dq_fact
