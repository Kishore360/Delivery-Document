...
replace(D2.project_name ,' – ',' - ') Top_Project_Name,
...
concat(round(((case when sum(F.estimated_doc_cost) = 0 then null 
when max(d.is_in_budget_flag) = 'N' then (sum((F.total_actual_doc_cost)- (F.estimated_doc_cost))/sum(F.estimated_doc_cost))
end)*100),0),'%') Avg_cost_overrun,

concat(round(((case when sum(F.estimated_doc_cost) = 0 then null 
when max(d.is_in_budget_flag) = 'N' then (sum((F.total_actual_doc_cost)- (F.estimated_doc_cost))/sum(F.estimated_doc_cost))
end)*100),0),'%') Avg_cost_overrun_all_PM,

case when (sum(F.actual_percent_complete* F.estimated_doc_cost/100) = 0 or (sum(F.total_actual_doc_cost)*100) = 0) then null
else (case when round((sum(F.budget_doc_cost)*sum(F.total_actual_doc_cost)*100/sum(F.actual_percent_complete* F.estimated_doc_cost)),0) <0
then concat('-$',(cast((coalesce((round((sum(F.budget_doc_cost)*sum(F.total_actual_doc_cost)*100/sum(F.actual_percent_complete* F.estimated_doc_cost)),0)*(-1)),0)) as varchar(200))))
when round((sum(F.budget_doc_cost)*sum(F.total_actual_doc_cost)*100/sum(F.actual_percent_complete* F.estimated_doc_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((sum(F.budget_doc_cost)*sum(F.total_actual_doc_cost)*100/sum(F.actual_percent_complete* F.estimated_doc_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((sum(F.budget_doc_cost)*sum(F.total_actual_doc_cost)*100/sum(F.actual_percent_complete* F.estimated_doc_cost))),0), '9,999,999,999')))) end) end min_Fund, 

case when round((Sum(F.total_actual_doc_cost- F.estimated_doc_cost)),0) <0
then concat('-$',(cast((coalesce((round((Sum(F.total_actual_doc_cost- F.estimated_doc_cost)),0)*(-1)),0)) as varchar(200))))
when round((Sum(F.total_actual_doc_cost- F.estimated_doc_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((Sum(F.total_actual_doc_cost- F.estimated_doc_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((Sum(F.total_actual_doc_cost- F.estimated_doc_cost))),0), '9,999,999,999')))) end Over_budget,

case when (sum(F.actual_percent_complete* F.estimated_doc_cost) = 0 or (sum(F.total_actual_doc_cost)*100) = 0) then null else
(case when round(((sum(F.total_actual_doc_cost)*100)/sum(F.actual_percent_complete)),0) <0
then concat('-$',(cast((coalesce((round(((sum(F.total_actual_doc_cost)*100)/sum(F.actual_percent_complete)),0)*(-1)),0)) as varchar(200))))
when round(((sum(F.total_actual_doc_cost)*100)/sum(F.actual_percent_complete)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round((((sum(F.total_actual_doc_cost)*100)/sum(F.actual_percent_complete))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round(((sum(F.total_actual_doc_cost)*100)/sum(F.actual_percent_complete))),0), '9,999,999,999')))) end) end EAC$,

case when round((sum(F.estimated_doc_cost)- sum(F.total_actual_doc_cost)),0) <0
then concat('-$',(cast((coalesce((round((sum(F.estimated_doc_cost)- sum(F.total_actual_doc_cost)),0)*(-1)),0)) as varchar(200))))
when round((sum(F.estimated_doc_cost)- sum(F.total_actual_doc_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((sum(F.estimated_doc_cost)- sum(F.total_actual_doc_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((sum(F.estimated_doc_cost)- sum(F.total_actual_doc_cost))),0), '9,999,999,999')))) end ETC$,

case when (sum(F.total_actual_doc_cost) is null or sum(F.total_actual_doc_cost) = 0) or
(sum(F.planned_percent_complete * F.estimated_doc_cost) = 0 or sum(F.planned_percent_complete * F.estimated_doc_cost) is null) then null
when (sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.total_actual_doc_cost)) is null or
(sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.planned_percent_complete * F.estimated_doc_cost))is null
then Null 
when ((sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.total_actual_doc_cost)) * (sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.planned_percent_complete * F.estimated_doc_cost))) = 0 then null
else case when round(((sum(F.budget_doc_cost)/((sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.total_actual_doc_cost)) * (sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.planned_percent_complete * F.estimated_doc_cost))))),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round((((sum(F.budget_doc_cost)/((sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.total_actual_doc_cost)) * (sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.planned_percent_complete * F.estimated_doc_cost)))))/1000),0),0), '9,999,999,990')),'K')) 
else concat('$',(trim(to_char(coalesce(round(((sum(F.budget_doc_cost)/((sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.total_actual_doc_cost)) * (sum(F.actual_percent_complete * F.estimated_doc_cost/100) / sum(F.planned_percent_complete * F.estimated_doc_cost)))))),0), '9,999,999,990')))) end
End Funds_needed,

case when round((sum(coalesce(F.net_value,0) - F.total_actual_doc_cost)),0) <0
then concat('-$',(cast((coalesce((round((sum(coalesce(F.net_value,0) - F.total_actual_doc_cost)),0)*(-1)),0)) as varchar(200))))
when round((sum(coalesce(F.net_value,0) - F.total_actual_doc_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((sum(coalesce(F.net_value,0) - F.total_actual_doc_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((sum(coalesce(F.net_value,0) - F.total_actual_doc_cost))),0), '9,999,999,999')))) end gross_merg,

round((case when sum(F.planned_percent_complete* F.estimated_doc_cost) = 0 then null else
Sum(F.actual_percent_complete/100)/ sum(F.planned_percent_complete) end),2) SPI,


case when (sum(F.actual_percent_complete/100) = 0 or sum(F.actual_percent_complete/100) is null or (sum(F.actual_percent_complete/100)* sum(F.estimated_doc_cost)) = 0) then 
(case 
when round((Sum(F.estimated_doc_cost)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((Sum(F.estimated_doc_cost))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round((Sum(F.estimated_doc_cost))),0), '9,999,999,999')))) end)
else (
case when round(((Sum(F.estimated_doc_cost)*sum(F.actual_percent_complete/100) - sum(F.total_actual_doc_cost))/sum(F.actual_percent_complete/100)),0) <0
then concat('-$',(cast((coalesce((round(((Sum(F.estimated_doc_cost)*sum(F.actual_percent_complete/100) - sum(F.total_actual_doc_cost))/sum(F.actual_percent_complete/100)),0)*(-1)),0)) as varchar(200))))
when round(((Sum(F.estimated_doc_cost)*sum(F.actual_percent_complete/100) - sum(F.total_actual_doc_cost))/sum(F.actual_percent_complete/100)),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round((((Sum(F.estimated_doc_cost)*sum(F.actual_percent_complete/100) - sum(F.total_actual_doc_cost))/sum(F.actual_percent_complete/100))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round(((Sum(F.estimated_doc_cost)*sum(F.actual_percent_complete/100) - sum(F.total_actual_doc_cost))/sum(F.actual_percent_complete/100))),0), '9,999,999,999')))) end)
end VAC
