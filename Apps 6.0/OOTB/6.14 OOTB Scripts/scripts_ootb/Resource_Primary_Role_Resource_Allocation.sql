...
case when (coalesce(resource_planned_effort_hours,0)=0 or resource_allocated_hour is null) then null 
when resource_planned_effort_hours = 0 then '0.0'
else concat(trim(to_char(coalesce(round((((resource_allocated_hour/resource_planned_effort_hours)*100) ),1),0), '9,999,999,990D9')),'%') end as project_allocation_percentage,
...
/*case when (coalesce(capacity_hours,0)=0 or resource_allocated_hour is null) then null 
when resource_allocated_hour = 0 then '0.0%'
else concat(trim(to_char(coalesce(round((((resource_allocated_hour/capacity_hours)*100) ),1),0), '9,999,999,990D9')),'%') 
end as resource_allocation_percentage*/


case when  resource_allocated_hour is null and capacity_hours >0 then '0.0%' 
when capacity_hours is null or capacity_hours = 0 then null 
when resource_allocated_hour = 0 then '0.0%'
else concat(trim(to_char(coalesce(round((((resource_allocated_hour/capacity_hours)*100) ),1),0), '9,999,999,990D9')),'%') 
end as resource_allocation_percentage
...
from 
(
select resource_key, Resource,   role_name, 
...
group by resource_key, Resource,  role_name
)
where resource not in ('UNKNOWN') 
order by upper(resource),upper(role_name), project_allocation_percentage,resource_allocated_hour,resource_allocated_cost,capacity_hours