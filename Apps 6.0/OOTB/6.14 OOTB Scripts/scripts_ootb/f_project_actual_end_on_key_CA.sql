...
where CASE WHEN LKP.dimension_wh_code IN ('CLOSED') then DATE_FORMAT(CONVERT_TZ(SRC10.actual_end_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') else null end <>
COALESCE(TRGT.actual_end_on_key,'')