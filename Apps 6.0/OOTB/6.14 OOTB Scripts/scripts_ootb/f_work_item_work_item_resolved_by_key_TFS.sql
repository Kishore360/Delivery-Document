...
LEFT JOIN #DWH_TABLE_SCHEMA.d_internal_contact LKP 
 ON ( CONCAT('INTERNAL_CONTACT~',SRC.microsoft_vsts_common_resolvedby__personsk) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.microsoft_vsts_common_resolvedby__personsk IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.work_item_resolved_by_key ,'');