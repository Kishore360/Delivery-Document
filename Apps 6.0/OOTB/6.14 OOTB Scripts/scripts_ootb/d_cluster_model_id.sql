...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_cluster.model_id' ELSE 'SUCCESS' END as Message
 FROM (select 
...
and SRC.source_id=TRGT.source_id 
 where coalesce(SRC.model_id,'')<>coalesce(TRGT.model_id,'');	
