...
left join(select * from #DWH_TABLE_SCHEMA.d_lov)LKP
 ON (concat('PROJECT','~','EXPENSE_TYPE','~',D.labor_type_code) = LKP.row_id 
AND SRC.sourceinstance = LKP.source_id )
WHERE COALESCE(LKP.row_key,CASE WHEN D.labor_type_code IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_expense_type_key,'')
