...
(
select cr.infrastructure_change_id, cr.sourceinstance, count(distinct task.assignee_id) as child_assig, sum(case when task.rootrequestid is not null then 1 else 0 end) as child_flg
from
(
...
left outer join
(
select Task_ID,sourceinstance,rootrequestid,create_date as task_dt, assignee_id from 
