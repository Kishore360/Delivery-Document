select CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_incident.met_resolution_sla_flag_key' ELSE 'SUCCESS' END as Message

/*select TRGT.row_key, TRGT.incident_number, SQ.category, inc_state.dimension_wh_code, SQ.stage, SQ.outcome_flag, 
TRGT.met_resolution_sla_flag_key tr,
res_key.row_key sr,
trgt_key.row_id tr_id,
...
else 'WH_SLA_FLAG~TASK_SLA~X' end as met_resolution_sla_flag_id*/
...
on trgt_key.row_key = TRGT.met_resolution_sla_flag_key
left join #DWH_TABLE_SCHEMA.d_lov res_key
on res_key.row_id = (case when inc_state.dimension_wh_code in ('RESOLVED','CLOSED') and SQ.outcome_flag = 'Y' then 'WH_SLA_FLAG~TASK_SLA~N'
when inc_state.dimension_wh_code in ('RESOLVED','CLOSED') and SQ.outcome_flag = 'N' then 'WH_SLA_FLAG~TASK_SLA~Y'
when inc_state.dimension_wh_code not in ('RESOLVED','CLOSED')  and SQ.outcome_flag = 'Y' then 'WH_SLA_FLAG~TASK_SLA~N'
when inc_state.dimension_wh_code not in ('RESOLVED','CLOSED') and SQ.stage in ('PAUSED','IN_PROGRESS') then 'WH_SLA_FLAG~TASK_SLA~P'
when inc_state.dimension_wh_code not in ('RESOLVED','CLOSED') and SQ.stage in ('COMPLETED','ACHIEVED') then 'WH_SLA_FLAG~TASK_SLA~Y'
when inc_state.dimension_wh_code not in ('RESOLVED','CLOSED') and SQ.stage = 'BREACHED' then 'WH_SLA_FLAG~TASK_SLA~N'
else 'WH_SLA_FLAG~TASK_SLA~X' end)

where SQ.category = 'RESOLUTION'
and coalesce(TRGT.met_resolution_sla_flag_key, '') <> coalesce(res_key.row_key, '');