SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_credit_score.opened_by_key' ELSE 'SUCCESS' END as Message
 ...
 where COALESCE( SRC.opened_by_key,'')<> COALESCE(TRGT.opened_by_key,'')
