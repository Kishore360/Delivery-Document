...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project_issue.project_issue_key' ELSE 'SUCCESS' END as Message
...
left join  #DWH_TABLE_SCHEMA.d_project_issue lov
on lov.row_id=SRC.request_id
and lov.source_id=SRC.sourceinstance
where coalesce(lov.row_key,case when SRC.request_id is null then 0 else -1 end)<>coalesce(TRGT.project_issue_key,'');