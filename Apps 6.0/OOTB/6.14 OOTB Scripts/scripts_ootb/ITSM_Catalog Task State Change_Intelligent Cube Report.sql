...
PRIORITY.dimension_name priority, 
CNT.full_name asignee, 
ORG.organization_name asgn_grp,
upon_aproval.dimension_name upon_aprv,
concat(round(sum(fa.primary_duration)/3600.00,1), ' Hours') state_dur,
...
group by task, task_num, req, req_num, req_item, req_item_num, shrt, due_date, opened, act_strt, act_end, closed, new_state, curr_state, old_state, upon_aprv,
priority, asignee, asgn_grp
order by d.request_task_number, task_num, req, req_num, req_item, req_item_num, shrt, due_date, opened, act_strt, act_end, closed, new_state, curr_state, old_state, upon_aprv,
priority, asignee, asgn_grp;