...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.assignment_group_key' ELSE 'SUCCESS' END as Message
...
on concat('GROUP~',SRC11.RESOURCE_POOL_ID) = LKP.row_id
and SRC11.sourceinstance = LKP.source_id
left join #DWH_TABLE_SCHEMA.f_project TRGT
on TRGT.row_id = SRC.project_id 
and TRGT.source_id = SRC.sourceinstance
where  COALESCE(LKP.row_key,CASE WHEN SRC11.RESOURCE_POOL_ID IS NULL THEN 0 else '-1' end)
<> COALESCE(TRGT.assignment_group_key,'');
