...
FROM 
(select TRGT.planned_return from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
and  round(COALESCE((planned_benefit.plan_bene-plan_cost.plan_cost),''))<> round(COALESCE(TRGT.planned_return,0))
union
select round(TRGT.planned_return) from(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
on SRC.PRJ_FINANCIAL_SUMMARY_ID=fin_summary.FINANCIAL_SUMMARY_ID 
and SRC.sourceinstance=fin_summary.sourceinstance
...
and COALESCE((0-round(plan_cost.plan_cost)),'')<> round(COALESCE(TRGT.planned_return,0))
)SQ
