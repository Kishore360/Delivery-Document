...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.current_year_start_date' ELSE 'SUCCESS' END as Message
...
left join (select * from #MDS_TABLE_SCHEMA.hp_rsc_staffing_profiles_final where is_active_flag='Y')SRC12
on SRC12.STAFFING_PROFILE_ID=SRC2.PRJ_STAFF_PROF_ID and SRC12.sourceinstance=SRC2.sourceinstance
LEFT JOIN
...
LEFT JOIN #MDS_TABLE_SCHEMA.hp_pm_work_plans_final SRC5
...
on SRC11.STAFFING_PROFILE_ID = SRC2.PRJ_STAFF_PROF_ID
and SRC11.sourceinstance = SRC2.sourceinstance
inner join #DWH_TABLE_SCHEMA.d_calendar_date CAL
on CAL.source_id = 0 and CAL.calendar_code = 0 and CAL.lagging_count_of_days = 0
left join #DWH_TABLE_SCHEMA.f_project TRGT
on TRGT.row_id = SRC.project_id 
and TRGT.source_id = SRC.sourceinstance
where COALESCE(DATE_FORMAT(TRGT.current_year_start_date,'%Y%m%d'),'') <> COALESCE(DATE_FORMAT(CAL.year_start_date,'%Y%m%d'),'')
