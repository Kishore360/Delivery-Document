 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_dq_incident_fact.target_column_value' ELSE 'SUCCESS' END as Message
-- select distinct row_id_der, row_id,dq_target_value, target_column_value,dq_rule_pass_status,rule_pass_status  FROM   (select * from #DWH_TABLE_SCHEMA.f_dq_incident_fact  where /*target_column not like 'dq%' and*/ soft_deleted_flag='N') dq_fact left join
from
 (
...
cast(case when (dio.dimension_name in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or dio.dimension_name is null)  then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
...
cast(case when (dio.full_name in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or dio.full_name is null)  then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
-- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
inc.Short_Description as dq_target_value,
 cast(case when (inc.Short_Description in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.Short_Description is null) then 0 else 1 end as decimal(28,10))as dq_rule_pass_status
...
when (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)>50 then 'Above 50 words'
 ...
 ,cast(case when (inc.Description in(null,'','UNSPECIFIED','UNALLOCATED','UNDEFINED','UNKONWN','-1','0') or inc.Description is null)  then 0
 when (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)<=10 then 0
when (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)<=20 and (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)>10 then 0.2
when (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)<=30 and (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)>20 then 0.4
when (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)<=50 and (LENGTH(inc.Description) - LENGTH(REPLACE(REPLACE(REPLACE(inc.Description,'\n',''), ' ', ''), ' ', ''))+1)>30 then 0.6
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column, 
 ...
 ,cast(case when (inc.opened_on in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN')  or inc.opened_on is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column, 
 ...
 ,cast(case when (inc.sys_domain  in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN')  or inc.sys_domain is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column, 
 ...
 ,cast(case when (inc.due_on in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN')  or inc.due_on is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
select case when Conf_dim.run_on ='MDW' then concat(Conf_dim.row_id,'~',inc.row_key) 
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 inc.row_key, case when (inc.sla_due in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.sla_due is null) then 'DQ-YES' else 'DQ-NO' end  as dq_target_value
,cast(case when (inc.sla_due in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.sla_due is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
,cast(case when  (inc.met_sla_flag in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.met_sla_flag is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
select case when Conf_dim.run_on ='MDW' then concat(Conf_dim.row_id,'~',inc.row_key) 
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 inc.row_key, case when (inc.due_on in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.due_on is null) then 'DQ-YES' else 'DQ-NO' end as dq_target_value
,cast(case when (inc.due_on in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.due_on is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 ...
 ,cast(case when (d_lov_severity.dimension_name in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or d_lov_severity.dimension_name is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 ...
 ,cast(case when (inc.resolved_at in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.resolved_at is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 ...
 ,cast(case when (inc.closed_on in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or inc.closed_on is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when Conf_dim.run_on ='MDS' then concat(Conf_dim.row_id,'~',inc.row_id) end  as row_id_der,rule,rule_type, dq_target_column,
 ...
 ,cast(case when (f_cr.close_code_src_code in('-1','0','null','UNSPECIFIED','UNALLOCATED',' ','UNDEFINED','UNKONWN') or f_cr.close_code_src_code is null) then 0 else 1 end as decimal(28,10)) as dq_rule_pass_status
 -- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0')  and soft_deleted_flag='N' and  active_flag<>'N' and module='incident' ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
...
when (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)>50 then 'Above 50 words'
 ...
 ,cast(case when (inc.close_notes in(null,'','UNSPECIFIED','UNALLOCATED','UNDEFINED','UNKONWN','-1','0') or inc.close_notes is null)  then 0
 when (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)<=10 then 0
when (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)<=20 and (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)>10 then 0.2
when (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)<=30 and (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)>20 then 0.4
when (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)<=50 and (LENGTH(inc.close_notes) - LENGTH(REPLACE(REPLACE(REPLACE(inc.close_notes,'\n',''), ' ', ''), ' ', ''))+1)>30 then 0.6
...
on cast(inc.changed_on as date)=cal1.calendar_date
 ...
 select distinct concat('dq_age','~',inc.row_key) as row_id_der,rule,rule_type,'dq_age' as dq_target_column, inc.row_key,
...
on cast(inc.changed_on as date)=cal1.calendar_date
 ...
 where  /*(cal1.lagging_count_of_month between 0 and 12) */ inc.changed_on>=ADDDATE(lastupdated , INTERVAL -13 MONTH)
...
case when inc.reassignment_count>3 then 'Y' else 'N' end as dq_target_value, null as dq_rule_pass_status-- , dq_fct.row_id, dq_fct.row_key
 from (select * from #DWH_TABLE_SCHEMA.d_dq_rules_configuration_dim where row_key not in ('-1','0') ) Conf_dim
 left join #DWH_TABLE_SCHEMA.d_dq_rules_dim d_dq_rules
 on d_dq_rules.row_key=Conf_dim.rule_key
