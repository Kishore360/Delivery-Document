...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_response.source_id' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(A.sourceinstance,'') <> ifnull(DWH.source_id,'')
)SQ;