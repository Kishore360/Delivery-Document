SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_agent_call_summary.outbound_acd_calls' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.agentoutcalls,'')<> COALESCE(TRGT.outbound_acd_calls,'')