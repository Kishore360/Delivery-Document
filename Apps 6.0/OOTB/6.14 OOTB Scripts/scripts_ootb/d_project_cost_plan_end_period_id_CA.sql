...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project_cost_plan.END_PERIOD_ID' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(SRC.END_PERIOD_ID,'')<>coalesce(TRGT.END_PERIOD_ID,'')