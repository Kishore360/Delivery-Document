/*-2 record which is a system record is excluded as per the JIRA DELV-5093 */
...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM #MDS_TABLE_SCHEMA.sys_user_final ) AS count_mds ) MDS,
(SELECT COUNT(1) Count_DWH, 'd_internal_contact'  Table_DWH 
FROM #DWH_TABLE_SCHEMA.d_internal_contact WHERE row_key NOT IN (0,-1,-2)  and current_flag='Y'
