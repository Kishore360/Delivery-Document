...
(select count(*) count_3 from ( SELECT * FROM #MDS_TABLE_SCHEMA.bmc_hpd_help_desk_sys_audit_final WHERE CDCTYPE<>'D') SRC join #DWH_TABLE_SCHEMA.f_incident F on F.row_id = SRC.documentkey 
where SRC.tablename = 'bmc_hpd_help_desk' and SRC.fieldname in ('Assignee', 'Assigned Group', 'Status') and F.soft_deleted_flag = 'N') C
