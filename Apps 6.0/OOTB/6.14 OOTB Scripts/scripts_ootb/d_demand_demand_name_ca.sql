SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_demand.demand_name' ELSE 'SUCCESS' END as Message
FROM
...
where  COALESCE(src.name,'')<> COALESCE(trg.demand_name,'')
;
