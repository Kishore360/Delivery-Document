...
where coalesce(case when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') and RES.dimension_wh_code in ('CANCELLED') then 0
 ...
 timestampdiff(second,coalesce(dates.work_start_on,0),coalesce(SRC.msvsts_common_resolveddate,coalesce(dates.completed_on, SRC.msvsts_common_closeddate))) else 0 end,0) <>coalesce(TRGT.duration_work_start_to_resolved,'');