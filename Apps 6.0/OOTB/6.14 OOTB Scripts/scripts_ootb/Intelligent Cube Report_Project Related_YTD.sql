...
case when D.is_active_flag = 'Y' and state.state = 'In Progress' then 'Y' else 'N' end active,
round(COALESCE(f.ytd_sub_project_count, 0::numeric),0) sub_proj,
...
case when round((sum(F.risk_doc_cost)),0) <0
then concat('-$',(cast((coalesce((round((sum(F.risk_doc_cost)),0)*(-1)),0)) as varchar(200))))
when round((sum(F.risk_doc_cost)),0) >=1000
...
round(sum(case when d.is_upcoming_flag = 'Y' and D.task_type in ('TASK', 'PROJECT') then 1 else 0 end),0) no_of_upcoming_proj
