...
select concat(SRC.issueid, '~', case when upper(SRC2.field) = 'SPRINT' THEN 'ITERATION' ELSE upper(SRC2.field) END, '~', SRC2.issuechangelogid) as sys_id,
SRC.authorRef ,
SRC.issueid ,
SRC.created ,
SRC.sourceinstance ,
...
where TRIM(SUBSTRING_INDEX(COALESCE(SRC2.from,SRC2.fromstring,0),',',-1)) <> TRIM(SUBSTRING_INDEX(COALESCE(SRC2.to,SRC2.tostring,0),',',-1));