select to_char(cal.calendar_date,'FMmm/FMdd/yyyy') dt,

case when count(case when F.change_task_key is not null then change_task_key else null end) = 0 then 0 
else round(sum(F.reassignment_count) * 1.00/count(case when F.change_task_key is not null then change_task_key else null end),0) end avg_reasgn_ct,

case when count(case when F.change_task_key is not null then change_task_key else null end) = 0 then 0 
else round(sum(update_count) * 1.00/count(case when F.change_task_key is not null then change_task_key else null end),0) end avg_updt_ct

-- count(case when state.state = 'CLOSED' and F.change_task_key is not null then change_task_key else null end) closed_ct
-- count(case when D.overdue_flag = 'Y' and F.change_task_key is not null then change_task_key else null end) ovrdue_ct,
-- count(case when D.backlog_flag = 'Y' and F.change_task_key is not null then change_task_key else null end) bklog_ct
...
join #DWH_TABLE_SCHEMA.d_internal_contact asgn_to
...
join #DWH_TABLE_SCHEMA.d_internal_organization asgn_grp
