...
FROM     ( SELECT * FROM #DWH_TABLE_SCHEMA.f_request_item_activity WHERE task_wh_type = 'sc_req_item' and task_attribute_wh_name = 'state') f1
LEFT JOIN( SELECT f2.request_item_key as request_item_key1, f2.primary_sequence_id as primary_sequence_id1, f2.created_on as created_on1 FROM #DWH_TABLE_SCHEMA.f_request_item_activity f2 
           WHERE f2.task_wh_type = 'sc_req_item' and f2.task_attribute_wh_name = 'state' )f2
ON f1.request_item_key = f2.request_item_key1 AND f2.primary_sequence_id1 = f1.primary_sequence_id + 1 AND f1.task_wh_type = 'sc_req_item' and f1.task_attribute_wh_name = 'state' 
INNER JOIN #DWH_TABLE_SCHEMA.d_request_item d ON f1.request_item_key = d.row_key
INNER JOIN #DWH_TABLE_SCHEMA.f_request_item f ON f.request_item_key = d.row_key
...
WHERE f1.request_item_key <> -1 and d.soft_deleted_flag = 'N'
ORDER BY f1.request_item_key,f1.primary_sequence_id
)A 
WHERE A.primary_duration <> (case when A.request_item_key = -1 then null when A.primary_duration1 < 0 then 0 else A.primary_duration1 end) -- OR A.primary_duration is null;