...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_incident_activity.group_created_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id, COALESCE(TGT.created_on,''), COALESCE(TGT.group_created_on,'')
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
where TGT.task_attribute_wh_name = 'assignment_group' and TGT.incident_key <> -1
and COALESCE(TGT.created_on, '') <> COALESCE(TGT.group_created_on,'')
union all
select TGT.row_id, 
COALESCE(base.max_group_created_on,'') sr, COALESCE(TGT.group_created_on,'') tg
from 
(
select TGT.row_id, TGT.incident_key, TGT.source_id, TGT.primary_sequence_id, max(SQ.created_on) max_group_created_on
from #DWH_TABLE_SCHEMA.f_incident_activity TGT
left join (
...
and COALESCE(base.max_group_created_on,'') <> COALESCE(TGT.group_created_on,'')
union all
select TGT.row_id, 
COALESCE(base.max_group_created_on,'') sr, COALESCE(TGT.group_created_on,'') tg
from 
