...
on SRC.ID=SRC1.PRID
left join (select B.INVESTMENT_ID,B.sourceinstance,count(1) as capital_cost from #MDS_TABLE_SCHEMA.ppa_wip_values_final A
left join #MDS_TABLE_SCHEMA.ppa_wip_final B
on A.TRANSNO = B.TRANSNO
and A.sourceinstance = B.sourceinstance
where A.CURRENCY_TYPE = 'HOME'and B.COST_TYPE = 'CAPITAL'
group by 1,2
) T1
on SRC.ID = T1.INVESTMENT_ID
and SRC.sourceinstance = T1.sourceinstance
LEFT JOIN (select B.INVESTMENT_ID,B.sourceinstance,SUM( A.TOTALCOST )as capital_cost 
...
where COALESCE(T2.capital_cost,0) <> COALESCE(TRGT.ACTUAL_CAPITAL_DOC_COST,'');