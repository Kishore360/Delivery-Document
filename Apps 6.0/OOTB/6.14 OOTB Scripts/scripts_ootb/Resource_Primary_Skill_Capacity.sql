...
case when coalesce(capacity_hours,0) = 0 THEN '0.0'
when round((capacity_hours),1) < 0 then '('||trim(to_char(round((capacity_hours),1)*(-1), '9,999,999,990D9'))||')'
else trim(to_char(round((capacity_hours),1), '9,999,999,990D9')) end capacity_hours,


case when coalesce(capacity_hours,0)=0 then null 
when coalesce(resource_actual_hours,0)=0 then '0.0%'
else concat(trim(to_char(coalesce(round(((resource_actual_hours*100)/capacity_hours),1),0), '9,999,999,990D9')),'%') end as Capacity_utilization,
 
 
trim(to_char(coalesce(round((case when (coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0))<0 then '0.0' 
else (coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0)) end),1),0), '9,999,999,990D9')) Capacity_Overtime_hours,  


case when (coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0)) <=0 then '$0'
when round(((coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0))*Resource_Per_Hour_Cost),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round((((coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0))*Resource_Per_Hour_Cost)/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(trim(to_char(coalesce(round(((coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0))*Resource_Per_Hour_Cost)),0), '9,999,999,999')))) end as Capactiy_Overtime_Cost,
 
 
case when (coalesce(resource_allocated_hour,0)=0 or coalesce(capacity_hours,0)=0) then null
when (coalesce(resource_allocated_hour,0)-coalesce(capacity_hours,0))=0 then '0.0%' 
when (coalesce(resource_allocated_hour,0)-coalesce(capacity_hours,0))<0  
then '('||(cast((coalesce((round((((resource_allocated_hour-capacity_hours)*100)/resource_allocated_hour),1)*(-1)),0)) as varchar(200)))||'%)'
else concat(trim(to_char(coalesce(round((((resource_allocated_hour-capacity_hours)*100)/resource_allocated_hour),1),0), '9,999,999,990D9')),'%') end as Capacity_gap_percentage,

case when (coalesce(capacity_hours,0)-coalesce(resource_planned_effort_hours,0))=0 then '0.0' 
when (coalesce(capacity_hours,0)-coalesce(resource_planned_effort_hours,0))>0 then 
trim(to_char(coalesce(round((coalesce(capacity_hours,0)-coalesce(resource_planned_effort_hours,0)),1),0), '9,999,999,990D9'))
when (coalesce(capacity_hours,0)-coalesce(resource_planned_effort_hours,0))<0 then 
'('||trim(to_char(round(((coalesce(capacity_hours,0)-coalesce(resource_planned_effort_hours,0))),1)*(-1), '9,999,999,990D9'))||')' end as net_available_capacity


from 
(
select resource_key, Resource,  primary_skill_key,skill_name, 
...
case when sum(resource_planned_effort_hours) is null  or sum(resource_planned_effort_cost) is null then null 
...
else sum(coalesce(resource_planned_effort_cost,0))/sum(coalesce(resource_planned_effort_hours,0)) end as Resource_Per_Hour_Cost
from
(
...
group by resource_key, Resource, primary_skill_key,skill_name
)
where resource not in ('UNKNOWN') 
order by upper(resource),upper(skill_name), resource_actual_hours,capacity_hours,capacity_overtime_hours