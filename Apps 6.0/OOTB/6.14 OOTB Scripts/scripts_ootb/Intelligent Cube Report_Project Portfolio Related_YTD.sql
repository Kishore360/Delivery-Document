...
sum(case when D.task_type = 'PROJECT' then 1 else 0 end) Project_Count,
sum(case when upper(D.task_type) = 'TASK' then 1 else 0 end) Task_Count,
sum(case when D.task_type in ('PROJECT', 'TASK') then 1 else 0 end) Project_task_Count,
sum(case when D.is_active_flag = 'Y' and state.state = 'In Progress' then 1 else 0 end) No_Of_Active_Project,
sum(case when D.is_upcoming_flag = 'Y' then 1 else 0 end) No_of_Upcoming,
sum(case when D.is_top_flag = 'Y' and D.is_active_flag = 'Y' and state.state = 'In Progress' then 1 else 0 end) No_of_Active_top,
sum(case when D.is_top_flag = 'Y' and state.state = 'Open' then 1 else 0 end) No_of_Open_top,
sum(case when D.is_top_flag = 'Y' and D.is_closed_on_time_flag = 'Y' then 1 else 0 end) No_of_Closed_Ontime_top,
sum(case when D.is_top_flag = 'Y' and state.state = 'Closed' then 1 else 0 end) No_of_Closed_top,
sum(case when D.is_top_flag = 'Y' and D.is_upcoming_flag = 'Y' then 1 else 0 end) No_of_Upcoming_top,
concat(round(case when sum(case when D.is_top_flag = 'Y' and state.state = 'Closed' then 1 else 0 end) = 0 then null else
(sum(case when D.is_top_flag = 'Y' and D.is_closed_on_time_flag = 'Y' then 1.0000 else 0.0000 end)/
sum(case when D.is_top_flag = 'Y' and state.state = 'Closed' then 1.0000 else 0.0000 end))*100 end,0),'%') Prct_Closed_top,
concat(round((100.0000 - case when sum(case when D.is_top_flag = 'Y' and state.state = 'Closed' then 1 else 0 end) = 0 then 0.0000 else
(sum(case when D.is_top_flag = 'Y' and D.is_closed_on_time_flag = 'Y' then 1.0000 else 0.0000 end)/
sum(case when D.is_top_flag = 'Y' and state.state = 'Closed' then 1.0000 else 0.0000 end))*100 end),0),'%') Prct_not_Closed_top,
concat((round((sum (case when D.is_in_budget_flag <> 'Y' or D.is_in_schedule_flag <> 'Y' and D.task_type in ('PROJECT', 'TASK') 
then 1.0000 else 0.0000 end)/sum (case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end))*100,1)),'%') Failure_ratio,
concat(round((sum (case when D.is_in_budget_flag = 'Y' and D.is_in_schedule_flag = 'Y' and D.task_type in ('PROJECT', 'TASK') 
then 1.0000 else 0.0000 end)/sum (case when D.task_type in ('PROJECT', 'TASK') then 1.0000 else 0.0000 end))*100,1),'%') Success_ratio
from 
(
 SELECT f.*
...
group by D_P.row_key, D_P.name, Portfolio_category
order by D_P.row_key, lower(D_P.name)