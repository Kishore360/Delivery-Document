...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_state_build.dimension_code' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE(SRC.status,'')<>COALESCE(TRGT.dimension_code,'') 