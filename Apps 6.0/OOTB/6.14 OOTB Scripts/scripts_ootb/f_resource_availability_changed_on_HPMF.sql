...
WHERE	(SRC3.WORKING_DAY_FLAG = 'Y') AND SRC4.EVENT_DATE is NULL AND CONVERT_TZ(SRC.LAST_UPDATE_DATE,'@#TENANT_SSI_TIME_ZONE@#','@#DW_TARGET_TIME_ZONE@#') <> COALESCE(TRGT.changed_on,'')