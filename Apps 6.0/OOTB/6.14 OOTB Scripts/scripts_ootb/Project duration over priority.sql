...
count(case when (D.overdue_flag = 'Y' or fresh.lstupdt < D.due_on) then F.work_item_key else null end) od_wi,

case when count(case when D.proposed_flag = 'Y' then F.work_item_key else null end) =0 then null
else round((sum(case when status.wi_sts in ('PROPOSED') then F.duration_since_last_update else 0.0 end)/86400.00)/count(case when D.proposed_flag = 'Y' 
then F.work_item_key else null end),1) end avg_days_since_lst_upd,

case when count(case when status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end) = 0 then null
else round((sum(case when status.wi_sts in ('RESOLVED', 'COMPLETED') then F.duration_created_to_resolved else 0.0 end)/86400.00) / 
count(case when status.wi_sts in ('RESOLVED', 'COMPLETED') then F.work_item_key else null end),1) end avg_duration_crtd_rslv,

case when count(case when status.wi_sts = 'COMPLETED' then F.work_item_key else null end) = 0 then null
else round((sum(case when status.wi_sts = 'COMPLETED' then F.duration_created_to_completed else 0.0 end)/86400.00)/
count(case when status.wi_sts = 'COMPLETED' then F.work_item_key else null end),1) end avg_duration_crtd_compd,

case when count(case when status.wi_sts = 'COMPLETED' then F.work_item_key else null end) = 0 then null
else round((sum(case when status.wi_sts = 'COMPLETED' then F.duration_work_start_to_completed else 0.0 end)/86400.00)/
count(case when status.wi_sts = 'COMPLETED' then F.work_item_key else null end),1) end avg_duration_inprog_compd
...
join -- data complete filter; change the left join to join at the time of real testing
(
