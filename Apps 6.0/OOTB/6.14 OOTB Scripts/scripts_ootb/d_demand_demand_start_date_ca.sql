SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_demand.demand_start_date' ELSE 'SUCCESS' END as Message
FROM
...
where  COALESCE(src.schedule_start,'')<> COALESCE(trg.demand_start_date,'')
;
