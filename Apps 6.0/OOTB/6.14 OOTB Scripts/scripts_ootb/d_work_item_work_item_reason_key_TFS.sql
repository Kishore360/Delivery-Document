 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.work_item_reason_key' ELSE 'SUCCESS' END as Message
...
coalesce(LKP.row_key, case when SRC.system_reason is null then 0 else -1 end,'')<>coalesce(TRGT.work_item_reason_key,'')