...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_lov_status.created_by' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.created_by,'')
<>coalesce(TRGT.created_by,'');