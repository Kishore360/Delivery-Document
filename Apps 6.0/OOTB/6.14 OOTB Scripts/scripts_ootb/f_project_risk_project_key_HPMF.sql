...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project_risk.project_key' ELSE 'SUCCESS' END as Message
...
and project.source_id=SRC2.sourceinstance
where coalesce(project.row_key,case when SRC2.PARAMETER1 is null then 0 else -1 end)<>coalesce(TRGT.project_key,'');