...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.portfolio_effort_variance_color_key' ELSE 'SUCCESS' END as Message
...
avg(effort_variance) as effort_variance, avg(schedule_variance) as schedule_variance,portfolio_effort_variance_color_key
 from #DWH_TABLE_SCHEMA.f_project 
 group by portfolio_key)TRGT
 on TRGT.row_id = SRC.project_id 
and TRGT.source_id = SRC.sourceinstance
 LEFT JOIN   #DWH_TABLE_SCHEMA.d_lov LKP 
ON TRGT.effort_variance BETWEEN LKP.lower_range_value AND LKP.upper_range_value
AND LKP.dimension_class = 'WH_PROJECT~COLOR~EFFORT_VARIANCE'
WHERE COALESCE(LKP.row_key,CASE WHEN TRGT.effort_variance IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.portfolio_effort_variance_color_key,'0') 
