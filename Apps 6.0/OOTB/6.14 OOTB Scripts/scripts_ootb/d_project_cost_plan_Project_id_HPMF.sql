SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project_cost_plan.project_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC1.prj_project_id,'')<>COALESCE(TRGT.project_id,'')
