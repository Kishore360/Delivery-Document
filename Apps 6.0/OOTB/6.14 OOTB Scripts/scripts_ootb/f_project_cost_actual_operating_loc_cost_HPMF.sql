SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project_cost.actual_operating_loc_cost' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(TRGT.actual_operating_loc_cost,'') <> 0