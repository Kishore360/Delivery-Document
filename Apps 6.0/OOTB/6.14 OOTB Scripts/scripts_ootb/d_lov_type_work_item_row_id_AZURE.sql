...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_type_work_item.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT('TYPE~WORK_ITEM~',(SRC.workItemType)),'')<>COALESCE(TRGT.row_id,'')