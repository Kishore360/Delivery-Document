...
TO_CHAR((D.planned_end_on),'fmMM/fmDD/YYYY fmHH:MI:SS AM')  plnd_strt_date,
max(sch_ver_color.dimension_name) sch_variance_color,
concat(round((sum(coalesce(F.schedule_variance,0))*100),0),'%') Sch_Variance,
max(sch_ver_color.dimension_code) Sch_Variance_clr_cde,
...
else null end)*100),0),'%') Sch_Growth
from 
(
   Select f.*
...
TOP_PRJ.row_key, Top_Project_Name, WK,act_strt_date,act_end_date,plnd_strt_date -- ,F.is_in_schedule
