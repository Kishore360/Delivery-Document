...
to_char(count(case when (cal_crtd_mon.lagging_count_of_month between 0 and 12 or (D.proposed_flag = 'Y' or status.wi_sts in ('PROPOSED'))) then F.row_key else null end),'FM999,999,999,999,990') opened_wi,
to_char(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('PROPOSED') then F.row_key else null end),'FM999,999,999,999,990') proposed_wi,
to_char(count(case when cal_inprog_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('IN PROGRESS') then F.row_key else null end),'FM999,999,999,999,990') inprog_wi,
...
to_char(count(case when cal_crtd_mon.lagging_count_of_month between 0 and 12 and D.reopened_flag = 'Y' then F.work_item_key else null end),'FM999,999,999,999,990') reopnd_wi
...
on cal_rslvd.row_key = F.work_item_resolved_on_key
/*case when status.wi_sts in ('COMPLETED') then coalesce(F.completed_on_key,0) 
when status.wi_sts in ('RESOLVED') then coalesce(F.work_item_resolved_on_key,0) 
when status.wi_sts in ('IN PROGRESS') then coalesce(F.work_item_work_start_on_key,0) 
else coalesce(F.created_on_key,0) end*/
...
D.proposed_flag = 'Y')
group by cal_crtd_week.week_id, wk_nm, cal_crtd_week.year_id, type.wi_type
order by cal_crtd_week.week_id desc, wk_nm, cal_crtd_week.year_id desc, upper(type.wi_type)