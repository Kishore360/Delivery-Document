SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for  f_build_run.AUTOMATED_TEST_PASS_COUNT' ELSE 'SUCCESS' END as Message
from
(SELECT SUM(coalesce(TR1.passCount,TR.passCount,0)) AS passCount,build.__numerify__primary_key
...
WHERE COALESCE(passCount,'')<> COALESCE(TRG.AUTOMATED_TEST_PASS_COUNT,'')
