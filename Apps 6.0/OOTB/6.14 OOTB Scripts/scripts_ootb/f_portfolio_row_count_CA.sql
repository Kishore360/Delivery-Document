SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 (select count(*) count_src,sourceinstance from  #MDS_TABLE_SCHEMA.pfm_portfolios_final) count_src,
 (select count(*)  count_trgt,source_id from
 (select * from   #MDS_TABLE_SCHEMA.pfm_portfolios_final) sd
 left join (select count(*),source_id from  #DWH_TABLE_SCHEMA.f_portfolio)trgt
