...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.source_id' ELSE 'SUCCESS' END as Message
FROM
(select * from 
...
COALESCE(src.sourceinstance,'')<>COALESCE(trg.source_id,'');