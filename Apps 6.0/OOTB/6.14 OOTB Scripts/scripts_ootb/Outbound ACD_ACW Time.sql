select cal_mon.year_name,cal_mon.month_name,acd.name,grp_lvl.organization_name
,Round(case when SUM(F.OUTBOUND_ACD_CALLS)=0 then NULL else 
SUM(F.OUTBOUND_ACD_TIME)*1.0/SUM(F.OUTBOUND_ACD_CALLS) end) Avg_Outbound_ACD_Time,
Round(case when SUM(F.OUTBOUND_ACD_CALLS)=0 then NULL else
SUM(F.OUTBOUND_ACW_TIME)*1.0/SUM(F.OUTBOUND_ACD_CALLS) end) Avg_Outbound_ACW_Time,
Round(case when SUM(F.INBOUND_ACD_CALLS)=0 then NULL else
SUM(F.INBOUND_ACD_TIME)*1.0/SUM(F.INBOUND_ACD_CALLS) end) Avg_Inbound_ACD_Time,
Round(case when SUM(F.INBOUND_ACD_CALLS)=0 then NULL else
SUM(F.INBOUND_ACW_TIME)*1.0/SUM(F.INBOUND_ACD_CALLS) end) Avg_Inbound_ACW_Time,
Round((Sum(F.INBOUND_ACD_CALLS)*1.00),2) Inbound_ACD_Calls 
from 
(
...
group by cal_mon.month_id,cal_mon.year_name,cal_mon.month_name,acd.name,grp_lvl.organization_name,F.group_lvl_key
order by  cal_mon.month_id asc,cal_mon.year_name,cal_mon.month_name,acd.name ,F.group_lvl_key asc,grp_lvl.organization_name
