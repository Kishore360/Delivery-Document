...
to_char((sum(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 3600.000), 'FM999,999,999,999,990D00') time_spent_comptd,

to_char(case when count(case when cal_cmptd_mon.lagging_count_of_month between 0 and 12 and status.wi_sts = 'COMPLETED' then F.work_item_key else null end) = 0 then 0.0
