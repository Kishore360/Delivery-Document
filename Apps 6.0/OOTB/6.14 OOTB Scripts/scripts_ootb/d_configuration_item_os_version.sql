SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_configuration_item.os_version' ELSE 'SUCCESS' END as Message
...
left join ( SELECT * FROM #MDS_TABLE_SCHEMA.cmdb_ci_server_final WHERE CDCTYPE<>'D') SRC2
 ...
 WHERE COALESCE( SRC2.os_version,'')<> COALESCE(TRGT.os_version,'')