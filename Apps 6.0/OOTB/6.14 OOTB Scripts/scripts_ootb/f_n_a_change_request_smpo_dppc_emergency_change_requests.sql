...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_a_change_request_smpo_dppc.emergency_change_requests' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.emergency_change_requests, '')<> COALESCE(TRGT.emergency_change_requests ,'');