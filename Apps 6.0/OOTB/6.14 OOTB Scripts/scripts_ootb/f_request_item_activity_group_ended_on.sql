...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_item_activity.group_ended_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and COALESCE(adddate(TGT.group_created_on, INTERVAL TGT.primary_duration SECOND),'') <> COALESCE(TGT.group_ended_on,'')
union all
select TGT.row_id
