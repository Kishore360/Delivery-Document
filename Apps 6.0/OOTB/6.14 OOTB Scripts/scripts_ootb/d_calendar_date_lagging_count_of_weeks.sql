...
JOIN (SELECT DATE(MAX(lastupdated)) AS convertz_dat_val  FROM #STG_TABLE_SCHEMA.o_data_freshness) s1
...
WHERE  ROUND(TIMESTAMPDIFF(DAY,SRC6.WEEK_START_DATE,convertz_dat_val)/7)
