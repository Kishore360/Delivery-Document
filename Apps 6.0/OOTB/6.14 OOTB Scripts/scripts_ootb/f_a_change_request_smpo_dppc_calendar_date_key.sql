...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_a_change_request_smpo_dppc.calendar_date_key' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.calendar_date_key, '')<> COALESCE(TRGT.calendar_date_key ,'');