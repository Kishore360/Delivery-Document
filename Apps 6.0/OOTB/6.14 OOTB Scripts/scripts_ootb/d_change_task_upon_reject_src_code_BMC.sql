...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_task.upon_reject_src_code' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(null,'') <> COALESCE(TRGT.upon_reject_src_code,'')
