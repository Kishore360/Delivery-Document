SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_configuration_item.ip_address' ELSE 'SUCCESS' END as Message
...
AND SRC.sourceinstance=SRC3.sourceinstance 
 ...
 WHERE COALESCE( SRC.ip_address ,'')<> COALESCE(TRGT.ip_address ,'')