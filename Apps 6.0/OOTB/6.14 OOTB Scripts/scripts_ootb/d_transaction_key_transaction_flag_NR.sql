SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_transaction.key_transaction_flag' ELSE 'SUCCESS' END as Message from
(
SELECT DISTINCT pageview.name, pageview.appId, pageview.sourceinstance
    ...
    FROM
        #MDS_TABLE_SCHEMA.key_transactions_final
) SRC
LEFT JOIN #DWH_TABLE_SCHEMA.d_transaction TRGT 
 ON (CONCAT(SRC.name, '~', SRC.appId)  =TRGT.row_id  
 AND SRC.sourceinstance=TRGT.source_id )
 left join #MDS_TABLE_SCHEMA.key_transactions_final k
ON (CONCAT(k.name, '~', k.applicationid) =TRGT.row_id 
AND SRC.sourceinstance=TRGT.source_id )
 WHERE COALESCE((case when concat(k.name,'~', k.applicationid)=TRGT.row_id then 'Y' else 'N' end),'')<> COALESCE(TRGT.key_transaction_flag ,'')