...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_resource_plan.project_task_key' ELSE 'SUCCESS' END as Message
...
on SRC1.task_id=LKP.row_id
and SRC1.sourceinstance=LKP.source_id
where coalesce(LKP.row_key,case when SRC1.task_id is null then 0 else 1 end)<>coalesce(TRGT.project_task_key,'');