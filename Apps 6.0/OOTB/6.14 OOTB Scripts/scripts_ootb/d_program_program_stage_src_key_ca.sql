SELECT CASE
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_program.program_stage_src_key'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
   ...
   LEFT JOIN #MDS_TABLE_SCHEMA.inv_projects_final inv_pr ON srm.id =inv_pr.prid
    ...
    #DWH_TABLE_SCHEMA.d_program trg ON (src.id = trg.row_id
        ...
        LEFT JOIN
    #DWH_TABLE_SCHEMA.d_lov LKP ON (COALESCE(CONCAT('Program~Stage~', src.stage_code),
            'UNSPECIFIED') = LKP.src_rowid
        AND src.sourceinstance = LKP.source_id)
WHERE
    COALESCE(LKP.row_key,
            CASE
                WHEN src.stage_code IS NULL THEN 0
                ELSE '-1'
            END) <> COALESCE(trg.program_stage_src_key, '')
;
