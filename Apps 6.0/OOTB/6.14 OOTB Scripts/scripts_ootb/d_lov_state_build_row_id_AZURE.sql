...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_state_build.row_id' ELSE 'SUCCESS' END as Message
...
WHERE  COALESCE(CONCAT('STATE~BUILD~',SRC.status),'')<>COALESCE(TRGT.row_id,'') 