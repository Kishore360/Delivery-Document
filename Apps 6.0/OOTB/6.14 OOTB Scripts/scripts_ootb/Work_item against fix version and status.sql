...
D.proposed_flag = 'Y')
group by PROJ.project_name, D.work_item_number, D.row_id, type.wi_type, pv.description, D.fix_version_list, status.wi_sts, resolution.wi_res, D.due_on, D.overdue_flag
order by upper(PROJ.project_name), D.work_item_number, D.row_id, upper(type.wi_type), upper(pv.description), upper(D.fix_version_list), upper(status.wi_sts), upper(resolution.wi_res), D.due_on, upper(D.overdue_flag)
