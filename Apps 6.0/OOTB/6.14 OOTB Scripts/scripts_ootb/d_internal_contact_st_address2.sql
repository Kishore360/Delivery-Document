...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_contact.st_address2' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.,'')<> COALESCE(TRGT.st_address2,'')
