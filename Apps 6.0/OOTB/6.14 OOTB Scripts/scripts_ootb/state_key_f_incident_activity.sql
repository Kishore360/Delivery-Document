...
CASE WHEN COUNT(1) >0 THEN 'state_key did not match for f_incident_activity' 
/*select TRGT.row_key, TRGT.incident_key, TRGT.task_attribute_wh_name, TRGT.state_key,
case when TRGT.task_attribute_wh_name = 'state' then TRGT.task_attribute_wh_new_value_key
  when TRGT.task_attribute_wh_name = 'assigned_to' then (select ia.task_attribute_wh_new_value_key
     ...
     and ia.task_attribute_wh_name = 'state'
...
coalesce(case when TRGT.task_attribute_wh_name = 'state' then TRGT.task_attribute_wh_new_value_key
  when TRGT.task_attribute_wh_name = 'assigned_to' then (select ia.task_attribute_wh_new_value_key
     from #DWH_TABLE_SCHEMA.f_incident_activity ia
     where TRGT.incident_key = ia.incident_key 
     and (TRGT.created_on > ia.created_on or (TRGT.created_on = ia.created_on and TRGT.row_key > ia.row_key))
  ...
  else -1 end, -1) <> coalesce(TRGT.state_key, -1)