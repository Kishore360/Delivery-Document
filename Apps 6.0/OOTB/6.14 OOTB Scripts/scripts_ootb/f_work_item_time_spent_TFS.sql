SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item.time_spent' ELSE 'SUCCESS' END as Message
FROM (
 ...
 WHERE COALESCE(round((SRC.microsoft_vsts_scheduling_completedwork  * 8 * 60 * 60),0), 0)<> COALESCE(TRGT.time_spent, '');