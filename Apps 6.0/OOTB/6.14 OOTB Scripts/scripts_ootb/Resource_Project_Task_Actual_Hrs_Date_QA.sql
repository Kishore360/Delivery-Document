...
from
(
select distinct Resource,project,project_task,resource_role,
calendar_date, 
...
group by resource_key,Resource,resource_role,project,project_task,calendar_date
)
where resource_actual_hours <>0
order by to_char(calendar_date,'yyyy-mm-dd'),upper(Resource),upper(resource_role),upper(project),upper(project_task),resource_actual_hours
