...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.remaining_effort' ELSE 'SUCCESS' END as Message
...
left join 
(
select sourceinstance,STAFFING_PROFILE_ID,count(1) from #MDS_TABLE_SCHEMA.hp_rsc_position_forecast_final
group by 1,2
)T1
on T1.STAFFING_PROFILE_ID = SRC2.PRJ_STAFF_PROF_ID
and T1.sourceinstance = SRC2.sourceinstance
left join 
(
select sourceinstance,STAFFING_PROFILE_ID,sum(FORECAST_VALUE) as plan_eff from #MDS_TABLE_SCHEMA.hp_rsc_position_forecast_final
group by 1,2
) T2
on T2.STAFFING_PROFILE_ID = T1.STAFFING_PROFILE_ID 
...
where  IF(COALESCE(SRC7.ACT_EFFORT,0) < COALESCE(T2.plan_eff,0),COALESCE(T2.plan_eff,0)-COALESCE(SRC7.ACT_EFFORT,0),0)
<> ROUND(COALESCE(TRGT.remaining_effort,0),7);
