SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_time_card.resource_role_key' ELSE 'SUCCESS' END as Message
...
left join (select * from  #MDS_TABLE_SCHEMA.hp_tm_time_sheet_lines_final) SRC2
 ...
 left join(select * from  #MDS_TABLE_SCHEMA.hp_rsc_resources_final)SRC7
 on SRC1.resource_id=SRC7.resource_id
...
on COALESCE(SRC7.primary_role_id,'')=LKP.row_id
and COALESCE(SRC.sourceinstance,'')=COALESCE(TRGT.source_id,'')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC7.primary_role_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.resource_role_key ,'')
