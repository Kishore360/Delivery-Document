...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.PLANNED_BENEFIT' ELSE 'SUCCESS' END as Message
...
where  COALESCE(SRC8.PLANNED_VALUE_BSE,0)
<> COALESCE(TRGT.PLANNED_BENEFIT ,0);
