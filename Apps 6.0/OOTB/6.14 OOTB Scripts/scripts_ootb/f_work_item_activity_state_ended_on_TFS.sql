...
/*SELECT TRGT.row_id,
coalesce(convert_tz(SRC.system_changeddate,'UTC','<<DW_TARGET_TIME_ZONE>>'), '') a, coalesce(TRGT.state_ended_on,'') b*/
FROM (
...
select base.system_id, base.teamprojectsk, base.sourceinstance, prev.System_State prev_state, base.System_State current_state, /*next.System_State next_state,*/ base.System_Rev sys_rev
 ...
 ON (TRGT.work_item_key =  TRGT_NXT.work_item_key and TRGT_NXT.task_attribute_wh_name = TRGT.task_attribute_wh_name and TRGT_NXT.primary_sequence_id = TRGT.primary_sequence_id + 1 
 AND TRGT_NXT.source_id=TRGT.source_id )
WHERE coalesce(TRGT_NXT.state_created_on, '') <> coalesce(TRGT.state_ended_on,'');