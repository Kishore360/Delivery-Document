...
left join #DWH_TABLE_SCHEMA.f_demand LKP
on TRGT.row_key=LKP.demand_key
where (case when LKP.demand_approval_delay=0 then 'N' else 'Y'  end )<> COALESCE(TRGT.approval_delay_flag,'')