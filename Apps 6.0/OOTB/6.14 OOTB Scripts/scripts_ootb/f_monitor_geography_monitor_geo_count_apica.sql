...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_geography.monitor_geo_count' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.monitor_geo_count, 'Unspecified')<>coalesce(trgt.monitor_geo_count, 'Unspecified')
