...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.changed_on' ELSE 'SUCCESS' END as Message
...
where  CONVERT_TZ(SRC.LAST_UPDATED_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')<> COALESCE(TRGT.changed_on,'')