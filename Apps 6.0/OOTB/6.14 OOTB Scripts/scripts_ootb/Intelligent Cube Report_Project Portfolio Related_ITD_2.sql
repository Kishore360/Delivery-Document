...
(sum(case when D.is_top_flag = 'Y' and D.is_Closed_on_time_flag = 'Y' then 1.000 else 0.000 end)/
sum(case when D.is_top_flag = 'Y' and STATE.state = 'Closed' then 1.000 else 0.000 end))*100 end,0),'%') Prct_Closed_top,
concat(round((100 - case when sum(case when d.is_top_flag = 'Y' and STATE.state = 'Closed' then 1 else 0 end) = 0 then 0 else
(sum(case when D.is_top_flag = 'Y' and D.is_Closed_on_time_flag = 'Y' then 1.000 else 0.000 end)/
sum(case when D.is_top_flag = 'Y' and STATE.state = 'Closed' then 1.000 else 0.000 end))*100 end),0),'%') Prct_not_Closed_top,
sum(case when D.task_type = 'PROJECT' and D.is_in_budget_flag = 'Y' then 1 else 0 end) No_of_Project_in_Budget,
sum(case when D.task_type = 'PROJECT' and D.is_in_effort_flag = 'Y' then 1 else 0 end) No_of_Project_in_effort,
sum(case when D.task_type = 'PROJECT' and D.is_in_schedule_flag = 'Y' then 1 else 0 end) No_of_Project_in_schedule,
sum(case when D.task_type in ('PROJECT', 'TASK') and D.is_in_schedule_flag = 'N' then 1 else 0 end) No_of_Project_delayed,
sum(case when D.task_type in ('PROJECT', 'TASK') and D.is_in_budget_flag = 'N' then 1 else 0 end) No_of_Project_over_Budget,
sum(case when D.task_type in ('PROJECT', 'TASK') and D.is_in_effort_flag = 'N' then 1 else 0 end) No_of_Project_over_effort,
sum(case when D.task_type in ('PROJECT', 'TASK') and D.is_in_budget_flag = 'Y' and D.is_in_schedule_flag = 'Y' then 1 else 0 end) No_of_Project_in_budget_schedule
