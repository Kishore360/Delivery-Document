...
from (select err.*, round(SUM(case WHEN err.errorType = 'NetworkFailure' THEN 1 ELSE 0 end),5) dur from #MDS_TABLE_SCHEMA.MobileRequestError_final err
...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where coalesce(merr.dur, '') <> coalesce(TRGT.network_failure_count)