...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_task.description' ELSE 'SUCCESS' END as Message
...
WHERE coalesce(SRC.summary,'') <> COALESCE(TRGT.description,'')
