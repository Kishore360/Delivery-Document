...
join(select hpd_assoc.request_id02, hpd_assoc.sourceinstance, assoc_type.value as assoc_type_val, assoc_type.enumId as assoc_type_id
, min(hpd_assoc.submit_date) as latest_date
from #MDS_TABLE_SCHEMA.hlx_hpd_associations_final hpd_assoc
join (SELECT enum_values.value,enum_values.value as enumId, enum_values.sourceinstance
...
AND enum_values.value = ('Related To')
) assoc_type 
on hpd_assoc.association_type01 = assoc_type.enumId and hpd_assoc.sourceinstance = assoc_type.sourceinstance
...
on hpd_assoc.request_type01 = req_type.enumId and hpd_assoc.sourceinstance = req_type.sourceinstance

group by hpd_assoc.request_id02, hpd_assoc.sourceinstance,assoc_type.value,assoc_type.enumId) base
...
where coalesce(TGT.change_flag,'') <>  case when assoc_type_val='Related To' then 'Y' else 'N' end;
