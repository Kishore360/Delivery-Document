SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_availability.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(concat(SRC.prj_object_id,'~',date_format(slice_date,'%Y%m%d')),'') <> COALESCE(TRGT.row_id,'')