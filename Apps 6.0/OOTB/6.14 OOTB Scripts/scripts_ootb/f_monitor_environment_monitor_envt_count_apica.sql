...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_monitor_environment.monitor_envt_count' ELSE 'SUCCESS' END as Message
...
left outer join #DWH_TABLE_SCHEMA.d_monitor_environment D_MONENV
on COALESCE(CONCAT('ENVIRONMENT~',check_tags.tagvalue),'UNSPECIFIED')=D_MONENV.row_id
and D_MONENV.source_id=checks.sourceinstance
where checks.cdctype<>'D'
and lower(check_tags.tagname) = 'environment'
group by 1,2,3,4,5,6
)SRC

left outer join #DWH_TABLE_SCHEMA.f_monitor_environment trgt
on SRC.row_id=trgt.row_id
and SRC.source_id=trgt.source_id
where coalesce(SRC.monitor_envt_count, 'Unspecified')<>coalesce(trgt.monitor_envt_count, 'Unspecified')
