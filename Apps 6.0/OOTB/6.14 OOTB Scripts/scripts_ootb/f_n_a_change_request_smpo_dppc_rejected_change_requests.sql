...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_n_a_change_request_smpo_dppc.rejected_change_requests' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.rejected_change_requests, '')<> COALESCE(TRGT.rejected_change_requests ,'');