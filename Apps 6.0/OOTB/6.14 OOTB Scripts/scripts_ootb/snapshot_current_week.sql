-- current and prev
drop table  if exists app_test.curr_snap_week;
CREATE TABLE app_test.curr_snap_week AS (SELECT fin.calendar_code,
    fin.n_date,
    fin.created_by,
    fin.assigned_to_key,
    ...
    (SELECT 
        f.incident_key,
            ...
            f.opened_by_key,
            f.source_id,
            der.current_week_n_key AS n_key,
            DATE_FORMAT(der.current_week_n_key, '%Y-%m-%d 00:00:00') AS n_date,
            f.changed_on,
    ...
    INNER JOIN (SELECT DISTINCT
        df.source_id,
            ...
            DATE_FORMAT(dcal.week_start_date, '%Y%m%d') AS current_week_n_key
    FROM
        (SELECT 
	...
	ON fin.age/86400 BETWEEN dage.lower_range_value AND dage.upper_range_value
	AND dage.dimension_class = 'WH_AGEBUCKET~INCIDENT'
	LEFT JOIN  #DWH_TABLE_SCHEMA.d_lov d_dage 
    ON fin.dormancy_age/86400 BETWEEN d_dage.lower_range_value AND d_dage.upper_range_value
