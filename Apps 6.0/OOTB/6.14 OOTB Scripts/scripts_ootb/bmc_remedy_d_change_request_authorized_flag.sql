...
LEFT JOIN  #DWH_TABLE_SCHEMA.d_lov_map LM
 on concat('AUTHORIZATION~CHANGE_REQUEST~',upper(SRC.change_request_status) )= LM.src_rowid
and LM.source_id=SRC.sourceinstance
WHERE COALESCE(CASE WHEN LM.dimension_wh_code = 'AUTHORIZED' THEN 'Y' ELSE 'N' END) <> COALESCE(TRGT.authorized_flag,'')