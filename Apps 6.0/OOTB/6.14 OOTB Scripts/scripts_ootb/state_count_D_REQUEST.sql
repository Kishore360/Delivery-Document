...
case when count(1) > 0 then 'MDS to DWH validation failed for d_request.state_count' else 'SUCCESS' end as Message
...
left join (
select base.request_key, base.source_id, 
...
where base.task_attribute_wh_name = 'request_state' and base.primary_sequence_id <> 0
group by base.request_key, base.source_id 
) state
on state.request_key = TRGT.row_key and state.source_id = TRGT.source_id
