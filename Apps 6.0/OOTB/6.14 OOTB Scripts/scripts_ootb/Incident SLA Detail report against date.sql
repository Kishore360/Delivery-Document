...
case when count(distinct case when CAL.lagging_count_of_days between 0 and 29 then F.row_key else null end) = 0 then null else
concat(round(cast(sum(case when CAL.lagging_count_of_days between 0 and 29 and D.met_sla_flag ='Y' then 1 else 0 end ) as numeric)
/count(distinct case when CAL.lagging_count_of_days between 0 and 29 then F.row_key else null end)::numeric*100,0),'%') end sla_met_prct,

case when count(distinct case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_reso.dimension_name not in ('CANCELLED') then F.incident_key else null end) = 0 then null

else concat(cast(round((sum(case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_reso.dimension_name in ('ACHIEVED') then 1.0000 else 0.0000 end)/
count(distinct case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_reso.dimension_name not in ('CANCELLED') then F.incident_key else null end)*1.0000)*100.0000,2) as varchar),'%') end sla_met_prct_res,

case when count(distinct case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_resp.dimension_name not in ('CANCELLED') then F.incident_key else null end) = 0 then null

else concat(cast(round((sum(case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_resp.dimension_name in ('ACHIEVED') then 1.0000 else 0.0000 end)/
count(distinct case when CAL.lagging_count_of_days between 0 and 29 and d_task_sla_resp.dimension_name not in ('CANCELLED') then F.incident_key else null end)*1.0000)*100.0000,2) as varchar),'%') end sla_met_prct_resp,

round(cast(sum(case when CAL.lagging_count_of_days between 0 and 29 and D.met_sla_flag ='Y' and state.state in ('RESOLVED', 'CLOSED') then 1 else 0 end ) as numeric),0) rslvd_inc_met_sla,

case when count(distinct case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('RESOLVED', 'CLOSED') then F.row_key else null end) = 0 then null else
concat(round(cast(sum(case when CAL.lagging_count_of_days between 0 and 29 and D.met_sla_flag ='Y' and state.state in ('RESOLVED', 'CLOSED') then 1 else 0 end ) as numeric)
/count(distinct case when CAL.lagging_count_of_days between 0 and 29 and state.state in ('RESOLVED', 'CLOSED') then F.row_key else null end)::numeric*100,0),'%') end rslvd_sla_met_prct
...
group by cal.calendar_date, met_sla, d_task_sla_resp.dimension_name, d_task_sla_reso.dimension_name
order by cal.calendar_date, met_sla, upper(d_task_sla_resp.dimension_name), upper(d_task_sla_reso.dimension_name)