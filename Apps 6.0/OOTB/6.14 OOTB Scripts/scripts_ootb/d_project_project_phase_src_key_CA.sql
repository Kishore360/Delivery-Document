...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON (concat('PROJECT','~','PHASE','~',stage_code) = LKP.src_rowid 
AND SRC.sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.stage_code IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.project_phase_src_key,'')