...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_task_activity.assigned_to_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.request_task_key, LKP.source_id, LKP.state_created_on, LKP.assigned_to_key 
from #DWH_TABLE_SCHEMA.f_request_task_activity LKP
where LKP.task_attribute_wh_name = 'assigned_to'
group by LKP.request_task_key, LKP.source_id, LKP.state_created_on) base
