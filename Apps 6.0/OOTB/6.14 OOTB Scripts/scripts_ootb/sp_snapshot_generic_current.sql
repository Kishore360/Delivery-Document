...
from (
select * from information_schema.columns where table_name = dwh_table COLLATE utf8_unicode_ci and table_schema = mdwdb COLLATE utf8_unicode_ci
and COLUMN_NAME not in ('row_key', 'calendar_date_key', 'dw_inserted_on', 'dw_updated_on', 'etl_run_number', 'secondary1_changed_on', 'secondary2_changed_on', 'assignment_group_key', 'department_key', 'executive_leader_key', 'vendor_key', 'assigned_to_key', 'senior_leader_key', 'leader_key')
order by COLUMN_NAME) SQ
group by TABLE_NAME) table_struc;
...
from (
select SQ.TABLE_NAME, 
group_concat(concat(case when SQ.COLUMN_NAME in ('assignment_group_key', 'department_key', 'executive_leader_key', 'vendor_key', 'assigned_to_key', 'senior_leader_key', 'leader_key') then '0' 
...
/*select distinct
SRC.target_key,f_cr.opened_by_department_key as department_key
, vend.vendor_key as vendor_key
,lev1.row_key as leader_key
,lev2.row_key as senior_leader_key
,lev3.row_key as executive_leader_key
,f_cr.assignment_group_key as assignment_group_key
,f_cr.assigned_to_key as assigned_to_key
FROM 
smpo_master_mdwdb.f_dq_problem_fact SRC
left join smpo_master_mdwdb.f_problem f_cr
 on f_cr.problem_key=SRC.target_key
    left join smpo_master_mdwdb.d_internal_contact vend
 on f_cr.assigned_to_key=vend.row_key
 left join smpo_master_mdwdb.d_internal_organization d_or
on f_cr.assignment_group_key=d_or.row_key
left join smpo_master_mdwdb.d_internal_contact lev0
on lev0.row_key=d_or.manager_key
left join smpo_master_mdwdb.d_internal_contact lev1
on lev1.row_key=lev0.manager_key
left join smpo_master_mdwdb.d_internal_contact lev2
 on lev2.row_key=lev1.manager_key
left join smpo_master_mdwdb.d_internal_contact lev3
...
select @dwh_table := case when @is_smpo = 'Y' then @SN_BASE when @is_dq = 'Y' then @DQ_BASE else @dwh_table end;
...
', @column_final_list ,'
select ' ,@column_insert_list, '
from ', (case when @is_smpo = 'Y' or @is_dq = 'Y' then workdb else mdwdb end) COLLATE utf8_unicode_ci, '.', @dwh_table, ' F 
cross join (
select distinct case when @sn_type = \'MONTHLY\' then CAL_MN.calendar_date when @sn_type = \'WEEKLY\' then CAL_WK.calendar_date when @sn_type = \'DAILY\' then CAL.calendar_date end as calendar_date,
case when @sn_type = \'MONTHLY\' then CAL_MN.row_key when @sn_type = \'WEEKLY\' then CAL_WK.row_key when @sn_type = \'DAILY\' then CAL.row_key end as row_key,
CAL.',@lagging_count_var,', CAL.calendar_code
