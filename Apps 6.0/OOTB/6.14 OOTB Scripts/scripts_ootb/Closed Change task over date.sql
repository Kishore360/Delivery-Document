...
-- count(case when F.change_task_key is not null then change_task_key else null end) opened_ct,
...
on cal.row_key = F.closed_on_key -- case when state.state = 'CLOSED' then F.closed_on_key else F.opened_on_key end -- coalesce(F.closed_on_key, F.opened_on_key)
join 
(
select d_calendar_date.*
FROM #DWH_TABLE_SCHEMA.d_calendar_date d_calendar_date
