SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 /*SELECT TRGT.row_id, TRGT.row_key,DM.dimension_wh_code, resolved_by.authorRef,
case when DM.dimension_wh_code not in ('RESOLVED', 'COMPLETED') then 'UNSPECIFIED'
 when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') and ASCII(COALESCE(resolved_by.authorRef,'')) = 0 THEN 'UNSPECIFIED' 
 when DM.dimension_wh_code in ('RESOLVED', 'COMPLETED') then CONCAT('INTERNAL_CONTACT~',resolved_by.authorRef)
