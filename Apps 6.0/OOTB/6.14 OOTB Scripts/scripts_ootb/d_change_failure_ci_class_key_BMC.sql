...
left join #DWH_TABLE_SCHEMA.d_lov lkp_d_lov
on coalesce(concat('CLASS~CMDB_CI','~',lkp_ci.classid),'UNSPECIFIED') = lkp_d_lov.row_id
and TGT.source_id=lkp_d_lov.source_id

WHERE COALESCE(lkp_d_lov.row_key,CASE WHEN lkp_ci.classid IS NULL THEN 0 else '-1' end)<> COALESCE(TGT.ci_class_key,'')