SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM 
(
SELECT CONCAT(a.s0,'~LOCATION REGION CLASSIFICATION') AS row_id,
...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_0
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_1
ON(lev_0.parent = lev_1.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_2
ON('OTHERS' = lev_2.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_3
ON('OTHERS' = lev_3.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_4
ON('OTHERS' = lev_4.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_location_final WHERE CDCTYPE<>'D') lev_5
