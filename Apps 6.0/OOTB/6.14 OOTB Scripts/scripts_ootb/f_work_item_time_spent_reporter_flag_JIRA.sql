...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item_time_spent.reporter_flag' ELSE 'SUCCESS' END as Message from

/*select 
 COALESCE(CASE WHEN SRC.authorRef = SRC.reporter_id THEN 'Y' ELSE 'N' END,'') , COALESCE(TRGT.reporter_flag,'')
...
where COALESCE(CASE WHEN SRC.authorRef = SRC.reporter_id THEN 'Y' ELSE 'N' END,'') <> COALESCE(TRGT.reporter_flag,'');
