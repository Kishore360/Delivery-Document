...
CASE WHEN count(1)>0  THEN 'MDS to DWH data validation failed for d_service.row_id' ELSE 'SUCCESS' END as Message 
...
WHERE COALESCE(CONCAT('BUSINESS_SERVICE','~',SRC.requestid),'') <> COALESCE(TRGT.row_id,'')