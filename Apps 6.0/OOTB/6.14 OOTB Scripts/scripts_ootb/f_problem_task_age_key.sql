 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_problem_task.age_key' ELSE 'SUCCESS' END as Message
...
ON ((FLOOR(f.age/(24*60*60)) BETWEEN L.lower_range_value AND L.upper_range_value)
	AND L.dimension_class = 'WH_AGEBUCKET~PROBLEM_TASK' )
