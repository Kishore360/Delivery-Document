...
on lkp.row_id = coalesce(DATE_FORMAT(CONVERT_TZ(SRC.finishTime,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%H%i'), 'UNSPECIFIED')
WHERE COALESCE(lkp.row_key, case when SRC.finishTime is null then 0 else -1 end)<> COALESCE(TRG.build_end_time_key,'');
