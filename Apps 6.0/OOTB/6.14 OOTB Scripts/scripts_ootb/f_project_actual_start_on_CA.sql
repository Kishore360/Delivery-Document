...
on SRC.ID=SRC1.PRID
left join (select min(prj_blb_slices.slice_date) as actual_start_date,max(prj_blb_slices.slice_date) as actual_end_date,sum(prj_blb_slices.slice) as effort,SRC4.prprojectid,
...
where  COALESCE(CONVERT_TZ(SRC10.actual_start_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>COALESCE(TRGT.actual_start_on,'')