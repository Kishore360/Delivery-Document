...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.soft_deleted_flag' ELSE 'SUCCESS' END as Message
from (select * from  #MDS_TABLE_SCHEMA.blz_master_aggregate_report_final) SRC
