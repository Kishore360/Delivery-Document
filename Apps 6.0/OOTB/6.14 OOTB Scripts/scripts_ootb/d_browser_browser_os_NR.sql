...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_browser.browser_os' ELSE 'SUCCESS' END as Message
FROM
...
WHERE COALESCE(SRC.useragentos,'')<> COALESCE(TRGT.browser_os,'')