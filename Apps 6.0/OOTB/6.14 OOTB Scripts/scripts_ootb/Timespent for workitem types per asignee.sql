...
to_char((sum(coalesce(F.time_spent ,0)) / 1.0),'FM999,999,999,999,990')as time_spent_created, -- based on the opened date
to_char((sum(case when status.wi_sts in ('COMPLETED') then coalesce(F.time_spent ,0) else 0.0 end) / 1.0), 'FM999,999,999,999,990') as time_spent_closed -- based on the closed date
