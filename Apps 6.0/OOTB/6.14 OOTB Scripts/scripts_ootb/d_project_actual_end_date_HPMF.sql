...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_project.ACTUAL_END_DATE' ELSE 'SUCCESS' END as Message
...
where coalesce(CONVERT_TZ(SRC4.ACT_FINISH_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')
