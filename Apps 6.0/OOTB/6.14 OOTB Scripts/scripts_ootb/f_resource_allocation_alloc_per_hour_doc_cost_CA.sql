SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.per_hour_doc_cost' ELSE 'SUCCESS' END as Messagse
FROM 
...
WHERE COALESCE(CASE WHEN SRC.PRALLOCSUM=0 then 0.0000000000 ELSE (SRC.ALLOCCOST_SUM)/(SRC.PRALLOCSUM/3600)
END, '0.0000000000')<>COALESCE(TRGT.PER_HOUR_DOC_COST,'')
