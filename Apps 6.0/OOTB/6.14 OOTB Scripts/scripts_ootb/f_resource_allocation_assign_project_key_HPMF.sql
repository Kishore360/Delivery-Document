...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.project_key' ELSE 'SUCCESS' END as Message
from (select pm_work_plans.project_id,SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,SRC23.resource_id as user_id,temp.WORKING_DAY_DATE,
...
on concat('ASSIGN','~',SRC.row_id,'~',SRC.WORKING_DAY_DATE)=TRGT.row_id and SRC.sourceinstance=TRGT.source_id
LEFT JOIN (select * from #DWH_TABLE_SCHEMA.d_project) LKP
ON LKP.ROW_ID = SRC.project_id and LKP.source_id=SRC.sourceinstance
WHERE COALESCE(LKP.row_key,'0')<>COALESCE(TRGT.project_key,'');