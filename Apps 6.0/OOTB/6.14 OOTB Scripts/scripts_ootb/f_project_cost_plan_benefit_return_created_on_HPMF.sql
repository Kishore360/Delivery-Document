...
FROM 
(select TRGT.created_on from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
...
left join(select fs.FINANCIAL_SUMMARY_ID,period_id,sum(bene.PLANNED_VALUE_BSE) as plan_bene, bene.creation_date as creation_date from
  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final fs 
...
and (COALESCE(convert_tz(planned_benefit.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')) <> COALESCE(TRGT.created_on,'')
union
select TRGT.created_on from(select * from  #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_project_final)SRC
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
on SRC.PRJ_FINANCIAL_SUMMARY_ID=fin_summary.FINANCIAL_SUMMARY_ID 
and SRC.sourceinstance=fin_summary.sourceinstance
inner join(select fs.FINANCIAL_SUMMARY_ID,period_id,sum(pc.PLAN_TOTAL_BSE) as plan_cost,pc.creation_date from
  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final fs 
left outer join  #MDS_TABLE_SCHEMA.hp_fm_forecast_actual_period_sum_final pc on pc.FORECAST_ACTUAL_ID=fs.FORECAST_ACTUAL_ID and pc.PERIOD_ID<>-1
where pc.cdctype<>'D'
group by 1,2)plan_cost
...
and (COALESCE(convert_tz(plan_cost.creation_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),''))<> COALESCE(TRGT.created_on,'')
)SQ
