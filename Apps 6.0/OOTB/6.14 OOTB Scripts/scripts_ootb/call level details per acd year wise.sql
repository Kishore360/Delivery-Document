select cal_mon.year_name, acd.name, 
to_char(SUM(F.ACD_CALLS),'999,999,990') as acd_calls,
sum(f.abandoned_Calls) as abandoned_calls
,Round((case when SUM(F.ACD_CALLS)  =0 then NULL else SUM(F.ACD_TIME)*1.0/ SUM(F.ACD_CALLS) end),2) Avg_ACD_Talk_Time
,Round(case when SUM(F.ACD_CALLS)  =0 then NULL else SUM(F.ACW_TIME)*1.0/ SUM(F.ACD_CALLS) end) Avg_ACW_Time
,Round(case when SUM(F.HOLD_CALLS)  =0 then NULL else SUM(F.HOLD_TIME)*1.0 / SUM(F.HOLD_CALLS) end) Avg_Hold_Time
,Round(case when SUM(F.ABANDONED_CALLS)=0 then NULL else SUM(F.ABANDON_TIME)*1.0/SUM(F.ABANDONED_CALLS) end ) Avg_time_to_Abandon
from 
(
...
group by cal_mon.year_name, acd.name
order by  acd.name, cal_mon.year_name
