...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_build_project.last_build_run_key' ELSE 'SUCCESS' END as Message
from 
...
(select definitionKey, __numerify__primary_key,sourceinstance,starttime from #MDS_TABLE_SCHEMA.azure_build_final 
 where (definitionKey, starttime) IN (select definitionKey,max(starttime) 
...
WHERE COALESCE(LKP.row_key,case when LKP.row_key is null then 0 else -1 end) <> COALESCE(TRG.last_build_run_key,'');