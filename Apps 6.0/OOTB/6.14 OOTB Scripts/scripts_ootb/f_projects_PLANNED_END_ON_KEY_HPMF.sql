...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.PLANNED_END_ON' ELSE 'SUCCESS' END as Message
...
where  COALESCE(DATE_FORMAT(CONVERT_TZ(END_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d'),'UNSPECIFIED')<> COALESCE(TRGT.PLANNED_END_ON_KEY ,'');