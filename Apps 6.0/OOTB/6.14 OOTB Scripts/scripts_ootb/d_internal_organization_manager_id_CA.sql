...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_internal_organization.manager_id' ELSE 'SUCCESS' END as Message
FROM  
...
where COALESCE(TRGT.manager_id,'') <> COALESCE(NULL,'')