...
case when round((coalesce(sum(datediff(day, F.actual_start_on, f.actual_end_on)*1.0),0.0)),1) < 0
then '('||trim(to_char(round((coalesce(sum(datediff(day, F.actual_start_on, f.actual_end_on)),0.0)),1)*(-1.0), '9,999,999,990D9'))||')'
else trim(to_char(round((coalesce(sum(datediff(day, F.actual_start_on, f.actual_end_on)*1.0),0.0)),1), '9,999,999,990D9')) end Actual_Duration_Timeline,
...
round(case when sum(case when STATE.state = 'Closed' then 1.00 else 0.00 end)*1.00 = 0.00 then null
else (sum(case when STATE.state = 'Closed' then F.actual_duration else 0.00 end)*1.00/
sum(case when STATE.state = 'Closed' then 1 else 0 end)*1.00) end,1) AVG_time_Deliver,

round(case when sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end)*1.0 = 0.0 then null
else (sum(case when STATE.state = 'Closed' then F.planned_duration else 0.0 end)/
sum(case when STATE.state = 'Closed' then 1.0 else 0.0 end)) end,1) AVG_Projected_time_Deliver,

case when(sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0)) is null then cast(0.0 as varchar(200))
when (sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0)) < 0 then '('||cast(sum(datediff(day, F.actual_start_on, F.planned_start_on)*1.0) as varchar(200))||')'
else cast(sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0) as varchar(200)) end as strt_delay_duration,

case when sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0) is null then cast(0.0 as varchar(200))
when sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0) < 0 then '('||cast(sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0)*(-1.0) as varchar(200))||')'
else cast(sum(datediff(day, F.planned_start_on, F.actual_start_on)*1.0) as varchar(200)) end as "Project Kick-off slack time"
...
act_strt_date,act_end_date,plnd_end_date, project_planned_end_weeks_due
