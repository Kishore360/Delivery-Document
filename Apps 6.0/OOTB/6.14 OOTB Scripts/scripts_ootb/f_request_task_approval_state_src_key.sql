SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_request_task.approval_state_src_key' ELSE 'SUCCESS' END as Message
...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON ( concat('APPROVAL','~','SC_TASK','~',upper(SRC.approval))= LKP.src_rowid
