...
trim(to_char(coalesce(round((count(distinct DIT.row_key)),0),0), '9,999,999,990')) ITR_count
...
group by D.work_item_number, status.dimension_name, priority.dimension_name, INTC_RSLV.full_name, D.resolved_on
order by upper(D.work_item_number), upper(status.dimension_name), upper(priority.dimension_name), upper(INTC_RSLV.full_name), D.resolved_on