...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_project_cost_plan.PERIOD_TYPE_CODE' ELSE 'SUCCESS' END as Message
FROM 
...
where COALESCE(SRC.PERIOD_TYPE_CODE,'')<>coalesce(TRGT.PERIOD_TYPE_CODE,'')
