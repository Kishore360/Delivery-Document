select dr.opened_by_department, dr.opened_by ,'' as metric ,  count(frt.row_key) from 
...
group by dr.opened_by_department, dr.opened_by
order by lower(dr.opened_by_department),lower( dr.opened_by)