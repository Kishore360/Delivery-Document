...
from 
(select distinct (case when cdctype = 'D' then 'Y' else 'N' end) as soft , max(sourceinstance) as sourceinstance, alarm_data_type as alarmType from #MDS_TABLE_SCHEMA.alarm_final ) src
