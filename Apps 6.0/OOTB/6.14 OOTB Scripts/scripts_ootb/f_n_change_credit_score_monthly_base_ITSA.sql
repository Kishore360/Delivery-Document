...
(select case when current_flag = 'N' then 1 else 0 end as start_month_lag_count
from #STG_TABLE_SCHEMA.lsm_ls_dw_aggregator where subject_area = 'ITSA_CREDIT_SCORE' and target_table = 'f_n_change_credit_score_monthly') and
(select distinct case when max_lagging_count > 0 then 0 else 1 end from (select max(lagging_count_of_month) max_lagging_count from #STG_TABLE_SCHEMA.f_n_change_credit_score_monthly_final_base) SQ )
) CAL
