...
case when count(1) > 0 then 'MDS to DWH data validation failed for survey_name between survey_final & asmt_metric_type_final to d_survey' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(S.name,'') <> ifnull(DWH.survey_name,'')
...
ifnull(A.name,'') <> ifnull(DWH.survey_name,'')
)SQ;