...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_configuration_item.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(SRC.requestid,'')<> COALESCE(TRGT.row_id,'')
