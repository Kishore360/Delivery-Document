...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_build_project.tests_skipped' ELSE 'SUCCESS' END as Message
from
 #MDS_TABLE_SCHEMA.job_final job
...
left join 
(
SELECT build.__numerify__parent_key,build.sourceinstance ,sum(coalesce(TRP.skipCount,TRS1.skipCount,0)) skipcount
...
where  COALESCE(SQ.skipcount,0) <> COALESCE(TRGT.tests_skipped,'');