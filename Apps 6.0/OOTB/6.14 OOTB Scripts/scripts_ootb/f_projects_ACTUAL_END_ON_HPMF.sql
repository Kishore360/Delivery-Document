...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project.ACTUAL_END_ON' ELSE 'SUCCESS' END as Message
...
where COALESCE(CONVERT_TZ(SRC7.ACT_FINISH_DATE,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> COALESCE(TRGT.ACTUAL_END_ON,'');