...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_request_activity.assignment_group_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.row_id, LKP.change_request_key, LKP.source_id, LKP.state_created_on, LKP.assignment_group_key 
from #DWH_TABLE_SCHEMA.f_change_request_activity LKP
where LKP.task_attribute_wh_name = 'assignment_group'
group by LKP.row_id, LKP.change_request_key, LKP.source_id, LKP.state_created_on) base
ON TGT.change_request_key = base.change_request_key and TGT.state_created_on = base.state_created_on
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' and SRC.fieldname <> 'assignment_group' 
and COALESCE(null,'') <> COALESCE(TGT.assignment_group_key,'') 
) SQ;
