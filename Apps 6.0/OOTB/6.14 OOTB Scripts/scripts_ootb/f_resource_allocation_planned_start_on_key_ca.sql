SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for f_resource_allocation.planned_start_on_key'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
...
WHERE
    trg.row_id LIKE '%ASSIGNED%'
        AND COALESCE(DATE_FORMAT(COALESCE(src.assign_PRSTART, src.prtask_PRSTART),
                    '%Y%m%d'),
            '') <> COALESCE(trg.planned_start_on_key, '');