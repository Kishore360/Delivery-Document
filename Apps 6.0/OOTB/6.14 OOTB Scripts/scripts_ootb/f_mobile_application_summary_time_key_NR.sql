...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 on (DATE_FORMAT(FROM_UNIXTIME(mobile.timestamp/1000-MOD(mobile.timestamp/1000,600)),'%H%i') = lov.row_id and lov.source_id =0)
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.time_key,'')
 union
 select TRGT.row_id 
 from #MDS_TABLE_SCHEMA.MobileRequestError_final MobileRequestError
left join #DWH_TABLE_SCHEMA.d_calendar_time lov
 on (DATE_FORMAT(FROM_UNIXTIME(MobileRequestError.timestamp/1000-MOD(MobileRequestError.timestamp/1000,600)),'%H%i') = lov.row_id and lov.source_id =0)
 left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.time_key,''))SQ