...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.customer_number' ELSE 'SUCCESS' END as Message
-- select coalesce(SRC.name,''),coalesce(TRGT.customer_number,'')
...
where customer=1 and coalesce(SRC.name,'')<>coalesce(TRGT.customer_number,'');