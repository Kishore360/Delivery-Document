...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_change_task_activity.task_attribute_wh_name' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
from #DWH_TABLE_SCHEMA.f_change_task_activity TGT
where TGT.primary_sequence_id = 0 and TGT.soft_deleted_flag = 'N'
and COALESCE(TGT.task_attribute_wh_name,'') not in ('state', 'assignment_group')
union all
select TGT.row_id
