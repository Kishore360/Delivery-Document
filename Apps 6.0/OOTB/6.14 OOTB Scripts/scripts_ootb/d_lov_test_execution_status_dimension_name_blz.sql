...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.dimension_name' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE(CASE WHEN SRC.passed=1 THEN 'Pass' WHEN SRC.passed=0 then 'Fail' END ,'') <> COALESCE(TRGT.dimension_name ,'');