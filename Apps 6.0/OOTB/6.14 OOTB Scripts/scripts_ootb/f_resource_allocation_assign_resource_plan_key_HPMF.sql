...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.resource_plan_key' ELSE 'SUCCESS' END as Message
from (select SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,
...
LEFT JOIN (select * from #DWH_TABLE_SCHEMA.d_resource_plan)LKP
on (LKP.row_id  = CONCAT('RES_TASK~',SRC.user_id,'~',SRC.task_id) and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.user_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_plan_key,''); 