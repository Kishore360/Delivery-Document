...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.CHANGED_ON' ELSE 'SUCCESS' END as Message
...
where customer=1 and coalesce(convert_tz(SRC.sys_updated_on,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'')<>coalesce(TRGT.CHANGED_ON,'');