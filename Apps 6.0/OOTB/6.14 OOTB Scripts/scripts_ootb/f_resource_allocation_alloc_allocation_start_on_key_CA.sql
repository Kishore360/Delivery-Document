...
WHERE date_format(COALESCE(convert_tz(SRC.PRAVAILSTART,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),convert_tz(SCHEDULE_START,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>')),'%Y%m%d')<> COALESCE(TRGT.allocation_start_on_key,'')
