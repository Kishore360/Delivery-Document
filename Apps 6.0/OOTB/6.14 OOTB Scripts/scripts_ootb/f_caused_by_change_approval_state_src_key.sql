...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.approval_state_src_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(lov.row_key,case when chg.approval is null then 0 else -1 end),coalesce(TRGT.approval_state_src_key,'') from 
...
left join (select * from #DWH_TABLE_SCHEMA.d_lov) lov
on lov.row_id=coalesce(concat('APPROVAL~CHANGE_REQUEST~',chg.approval),'UNSPECIFIED') and lov.source_id=chg.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(lov.row_key,case when chg.approval is null then 0 else -1 end)<>coalesce(TRGT.approval_state_src_key,'')

union all

select coalesce(lov.row_key,case when chg.approval is null then 0 else -1 end),coalesce(TRGT.approval_state_src_key,'') 
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(lov.row_key,case when chg.approval is null then 0 else -1 end)<> coalesce(TRGT.approval_state_src_key,''))ca