SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 LEFT JOIN #DWH_TABLE_SCHEMA.d_project_task STM
 on STM.row_id = SRC.TASK_ID
 and STM.source_id=SRC.sourceinstance
 WHERE coalesce(STM.row_key,case when SRC.TASK_ID is null then 0 else -1 end)<>coalesce(TRGT.project_task_key,'')