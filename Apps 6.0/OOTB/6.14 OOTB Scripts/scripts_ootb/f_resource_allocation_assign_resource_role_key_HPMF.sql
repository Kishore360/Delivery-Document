...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_resource_allocation.resource_role_key' ELSE 'SUCCESS' END as Message
from (select SRC28.role_id,SRC23.cdctype,SRC23.sourceinstance,concat(SRC23.resource_id,'~',SRC23.task_id) as row_id,SRC23.task_id,SRC23.resource_id as user_id,temp.WORKING_DAY_DATE,
...
LEFT JOIN(select * from #MDS_TABLE_SCHEMA.hp_wp_task_info_final)SRC28
on SRC24.task_info_id=SRC28.task_info_id
and SRC24.sourceinstance=SRC28.sourceinstance
...
on (LKP.row_id  = COALESCE(SRC.role_id,'') and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.role_id is null THEN 0 else -1 end)<> COALESCE(TRGT.resource_role_key,''); 