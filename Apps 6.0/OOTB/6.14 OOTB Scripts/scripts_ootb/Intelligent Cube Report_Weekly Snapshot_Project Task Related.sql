...
sum(case when D.task_type = 'TASK' and STATE.dimension_name = 'Closed' then 1 else 0 end) No_of_tasks,
sum(case when D.task_type = 'TASK' and f.planned_end_on >= cal.week_start_date AND f.planned_end_on <= cal.week_end_date and CAL.lagging_count_of_weeks = 0
then 1 else 0 end) task_due_current_week,
sum(case when D.task_type = 'TASK' and f.planned_end_on >= cal.week_start_date AND f.planned_end_on <= cal.week_end_date
then 1 else 0 end) task_due_weekly
from 
(
   Select f.*
...
TOP_PRJ.project_name, TOP_PRJ.row_key,
WK,
