...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_activity.state_created_on' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
on (SRC.sys_id=TGT.row_id AND SRC.sourceinstance=TGT.source_id)
left join 
(select LKP.problem_key, LKP.source_id, LKP.state_created_on, LKP.state_key 
from #DWH_TABLE_SCHEMA.f_problem_activity LKP
where LKP.task_attribute_wh_name = 'problem_state'
group by LKP.problem_key, LKP.source_id, LKP.state_created_on) base
ON TGT.problem_key = base.problem_key and TGT.state_created_on = base.state_created_on
where TGT.primary_sequence_id <> 0 and TGT.soft_deleted_flag = 'N' and SRC.fieldname <> 'state' 
