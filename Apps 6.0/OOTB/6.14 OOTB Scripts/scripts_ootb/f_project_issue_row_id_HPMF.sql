...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_project_issue.row_id' ELSE 'SUCCESS' END as Message
...
where concat(coalesce(SRC.request_id,'UNSPECIFIED'),'~',coalesce(SRC2.PARAMETER1,'UNSPECIFIED'))<>coalesce(TRGT.row_id,'');