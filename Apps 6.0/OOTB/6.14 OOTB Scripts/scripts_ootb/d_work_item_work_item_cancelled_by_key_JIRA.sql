SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 /*SELECT TRGT.row_id, TRGT.row_key,DM.dimension_wh_code, SRC.assigneeRef, SQ.authorRef,
(case when (DM.dimension_wh_code not in ('CANCELLED') and RES.dimension_wh_code not in ('CANCELLED')) then 'UNSPECIFIED'
 when (DM.dimension_wh_code in ('CANCELLED') or RES.dimension_wh_code in ('CANCELLED')) then CONCAT('INTERNAL_CONTACT~',SQ.authorRef)
 when ASCII(COALESCE(SRC.assigneeRef,'')) = 0 THEN 'UNSPECIFIED'
...
COALESCE(LKP.row_key, 0) sr, coalesce(TRGT.cancelled_by_key,'') tg*/
 ...
 ON (case when (DM.dimension_wh_code not in ('CANCELLED') and RES.dimension_wh_code not in ('CANCELLED')) then 'UNSPECIFIED'
 when (DM.dimension_wh_code in ('CANCELLED') or RES.dimension_wh_code in ('CANCELLED')) then CONCAT('INTERNAL_CONTACT~',dates.cancelled_by)
