...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_skill_call_summary.row_id' ELSE 'SUCCESS' END as Message
...
WHERE COALESCE(CONCAT(SRC.skilltargetid,'~',peripheralid,'~',timezone,'~',TRIM(datetime)),'')<> COALESCE(TRGT.row_id,'')