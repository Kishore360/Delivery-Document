...
concat('$',coalesce(case when round(sum(F.budget_doc_cost * f.planned_ytd_coeff),0)>=1000
then concat(trim(to_char(round(sum(F.budget_doc_cost * f.planned_ytd_coeff/1000),0), '9,999,999,999')),'K') 
else cast(round(sum(F.budget_doc_cost * f.planned_ytd_coeff),0) as varchar(200)) end, cast(0 as varchar(200)))) budgeted_cost,

coalesce(sum(case when d.is_in_budget_flag = 'Y' and D.task_type = 'PROJECT' then 1 else 0 end),0) No_of_proj_in_bud,

concat(round(((case when sum(F.estimated_doc_cost* COALESCE(f.planned_ytd_coeff, 0)) = 0 then null else
sum((F.total_actual_doc_cost* COALESCE(F.actual_ytd_coeff, 0))::numeric(12,3)-((F.actual_percent_complete* F.estimated_doc_cost)::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0)/100))/
sum(F.estimated_doc_cost* COALESCE(f.planned_ytd_coeff, 0))::numeric(12,3)
end)*100),0),'%') Prct_incrs_budget, 

concat(round(((case when sum(F.estimated_doc_cost* COALESCE(F.planned_ytd_coeff, 0)) = 0 then null else
sum((F.total_actual_doc_cost* COALESCE(F.actual_ytd_coeff, 0))- (F.estimated_doc_cost* COALESCE(F.planned_ytd_coeff, 0)))/
sum(F.estimated_doc_cost* COALESCE(F.planned_ytd_coeff, 0))
end)*100),0),'%') Prct_over_budget,

concat(round(((case when sum(F.estimated_doc_cost* COALESCE(f.planned_ytd_coeff, 0)) = 0 then null 
when max(D.is_in_budget_flag) = 'N' then
(sum((F.total_actual_doc_cost* COALESCE(F.actual_ytd_coeff, 0))- (F.estimated_doc_cost* COALESCE(f.planned_ytd_coeff, 0)))/
sum(F.estimated_doc_cost* COALESCE(f.planned_ytd_coeff, 0)))
end
)*100),0),'%') Avg_cost_overrun,

round(((case when sum((F.actual_percent_complete/100)::decimal(12,5)* F.estimated_doc_cost::decimal(12,5)* COALESCE(f.planned_ytd_coeff, 0)::decimal(12,5)) = 0 then null else
sum(F.total_actual_doc_cost::decimal(12,6)* COALESCE(F.actual_ytd_coeff, 0)::decimal(12,6))/
sum((F.actual_percent_complete/100)::decimal(12,5)* F.estimated_doc_cost::decimal(12,5)* COALESCE(f.planned_ytd_coeff, 0)::decimal(12,5))
end
)),0) Burn_rate,

case when round((sum(F.actual_percent_complete/100::numeric(12,3)* F.estimated_doc_cost::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0))),0) <0
then concat('-$',(cast((coalesce((round((sum(F.actual_percent_complete/100::numeric(12,3)* F.estimated_doc_cost::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0))),0)*(-1)),0)) as varchar(200))))
when round((sum(F.actual_percent_complete/100::numeric(12,3)* F.estimated_doc_cost::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0))),0) >=1000
then concat('$',concat(trim(to_char(coalesce(round(((sum(F.actual_percent_complete/100::numeric(12,3)* F.estimated_doc_cost::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0)))/1000),0),0), '9,999,999,999')),'K')) 
else concat('$',(cast(coalesce(round((sum(F.actual_percent_complete/100::numeric(12,3)* F.estimated_doc_cost::numeric(12,3)* COALESCE(f.planned_ytd_coeff, 0)))),0) as varchar(200)))) end Earned_Value,

concat(coalesce(round((sum(F.percent_roi)*100),0),0),'%') ROI
...
group by D_P.row_key, Portfolio_name, Portfolio_category, D.row_key, D.Project_name, D2.row_key, Top_Project_Name, cost_variance_color
