...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_organization_customer.estimated_annual_revenue' ELSE 'SUCCESS' END as Message
-- select TRGT.estimated_annual_revenue
...
where customer=1 and TRGT.estimated_annual_revenue is not null;