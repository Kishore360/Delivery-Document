...
when F.project_risk_color_key = -1 or (F.project_risk_color_key is not null and risk_ver_color.row_key <> F.project_risk_color_key) then 'UNKNOWN'
else risk_ver_color.risk_color end risk_color_cde,

sum(case when D.task_type = 'PROJECT' and f.actual_end_on >= cal.month_start_date 
AND f.actual_end_on <= cal.month_end_date then 1 else 0 end) Closed_Project_Count,

case when (sum(case when D.task_type = 'PROJECT' and f.planned_start_on >= cal.month_start_date AND f.planned_start_on <= cal.month_end_date then 1 else 0 end) - 
sum(case when D.task_type = 'PROJECT' and f.actual_end_on >= cal.month_start_date AND f.actual_end_on <= cal.month_end_date then 1 else 0 end))<0
then '('||trim(to_char(((sum(case when D.task_type = 'PROJECT' and f.planned_start_on >= cal.month_start_date AND f.planned_start_on <= cal.month_end_date then 1 else 0 end) - 
sum(case when D.task_type = 'PROJECT' and f.actual_end_on >= cal.month_start_date AND f.actual_end_on <= cal.month_end_date then 1 else 0 end))*(-1)), '9,999,999,999'))||')'
else trim(to_char(((sum(case when D.task_type = 'PROJECT' and f.planned_start_on >= cal.month_start_date AND f.planned_start_on <= cal.month_end_date then 1 else 0 end) - 
sum(case when D.task_type = 'PROJECT' and f.actual_end_on >= cal.month_start_date AND f.actual_end_on <= cal.month_end_date then 1 else 0 end))), '9,999,999,999')) end Planned_vs_Closed,

sum(case when D.task_type = 'PROJECT' and f.actual_start_on >= cal.month_start_date 
AND f.actual_start_on <= cal.month_end_date then 1 else 0 end) No_of_projects_started,

max(risk_ver_color.risk_ver_color) risk_color
...
UNION 
 SELECT lov.*, lov.dimension_code as risk_ver_color, lov.dimension_name risk_color
   FROM #DWH_TABLE_SCHEMA.d_lov lov
  WHERE lov.row_key = -1 OR lov.row_key = 0
) risk_ver_color
...
group by D_P.name , D_P.row_key, Portfolio_category,D.project_name, D.row_key, D2.project_name, month_nm, cal.month_id, Parent_project_name,
Project_Manager, Status_src, BS_name, CI_name, comp_name, dom_name, Loc_name, is_top, risk_color_cde
order by D_P.row_key, lower(D_P.name), D.row_key, lower(D.Project_name), cal.month_id