...
from
(select f.primary_sequence_id AS task_rank, f.incident_key task, f.created_on_key, f.wh_old_value_changed_on_key
from #DWH_TABLE_SCHEMA.f_incident_activity f
where f.task_attribute_wh_name ='assigned_to'
order by f.incident_key, f.created_on_key
) O
join
(select f.primary_sequence_id AS task_rank, f.incident_key task, f.created_on_key, f.wh_old_value_changed_on_key
from #DWH_TABLE_SCHEMA.f_incident_activity f
