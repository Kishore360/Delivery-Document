...
left join(select FINANCIAL_SUMMARY_ID,FISCAL_PERIOD_ID,LABOR_TYPE_CODE,EXPENSE_TYPE_CODE,sourceinstance,max(last_update_date) last_update_date,
...
on TRGT.row_id=concat(SRC.PRJ_PROJECT_ID,'~',SRC.PRJ_FINANCIAL_SUMMARY_ID,'~',SRC3.fiscal_period_id,'~',COALESCE(SRC2.LABOR_TYPE_CODE,'UNSPECIFIED'),'~',COALESCE(SRC2.EXPENSE_TYPE_CODE,'UNSPECIFIED'))
AND SRC.SOURCEINSTANCE=TRGT.SOURCE_ID
WHERE COALESCE(convert_tz(SRC2.last_update_date,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'') <> coalesce(TRGT.changed_on,'')