...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_project.weeks_to_due_date' ELSE 'SUCCESS' END as Message
...
WHERE round(DATEDIFF(TRGT.planned_end_on,FRSH.lastupdated)/7,2)  <> round(coalesce(TRGT.weeks_to_due_date,''),2)