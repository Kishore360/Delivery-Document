...
round(coalesce(sum(distinct F.business_duration/86400.00),0),1) bsns_dur,
round(coalesce(sum(distinct F.open_to_close_duration/86400.00),0),1) opn_to_close_dur,
round(sum(distinct case when D.backlog_flag = 'Y' and cal_mon.lagging_count_of_month between 0 and 12 then F.dormancy_age/86400.00 else 0.0 end),1) days_since_lst_updt
from
(
...
or D.backlog_flag = 'Y'
group by ctn, ctn1, D.opened_on, D.due_on, D.closed_on
order by ctn, ctn1, D.opened_on, D.due_on, D.closed_on