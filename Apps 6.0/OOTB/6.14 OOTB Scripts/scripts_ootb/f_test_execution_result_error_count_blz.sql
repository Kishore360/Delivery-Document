...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.error_count' ELSE 'SUCCESS' END as Message
...
where coalesce(SRC.errorscount,'')<>coalesce(TRGT.error_count,'')
