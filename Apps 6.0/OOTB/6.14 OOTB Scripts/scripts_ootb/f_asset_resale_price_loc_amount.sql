SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_asset.resale_price_loc_amount' ELSE 'SUCCESS' END as Message
...
on cst.id=SRC.sys_id  and cst.field ='cost'
 ...
 AND SRC.sourceinstance=TRGT.source_id )
  WHERE cast(coalesce(SRC.resale_price,0) * ROUND (coalesce(cst.amount,0) / SRC.cost , 2 )as decimal(20,10)) <> COALESCE(TRGT.resale_price_loc_amount ,'')
