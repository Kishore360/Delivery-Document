SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_work_item.assigned_to_key' ELSE 'SUCCESS' END as Message
...
COALESCE(LKP.row_key,CASE WHEN SRC.system_assignedto__personsk IS NULL THEN 0 else -1 end)<> COALESCE(TRGT.assigned_to_key ,'');