SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_major_incident.days_since_last_major_incident' ELSE 'days_since_last_major_incident-SUCCESS' END as Message
...
mjr_inc_ct,d_o.lastupdated,mjr_inc_dt,
case when mjr_inc_ct>0 then datediff(cast(d_o.lastupdated as date),mjr_inc_dt) 
when mjr_inc_ct is null then -1
end as days_since_last_major_incident
...
where SRC.days_since_last_major_incident<>trgt.days_since_last_major_incident;