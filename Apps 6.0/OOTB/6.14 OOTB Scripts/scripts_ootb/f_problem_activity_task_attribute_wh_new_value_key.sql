...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_problem_activity.task_attribute_wh_new_value_key' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
from #DWH_TABLE_SCHEMA.f_problem_activity TGT
left join #DWH_TABLE_SCHEMA.d_problem D
on D.row_key = TGT.problem_key
...
left join #DWH_TABLE_SCHEMA.f_problem_activity TGT2
on (F.problem_key=TGT2.problem_key AND SRC.sourceinstance=TGT2.source_id and TGT2.primary_sequence_id = 1)
left join #DWH_TABLE_SCHEMA.d_lov LOV
on LOV.row_id = concat('STATE~PROBLEM~',coalesce(TGT2.task_attribute_wh_old_value, F.state_src_code)) and LOV.dimension_class = 'STATE~PROBLEM'
...
left join #DWH_TABLE_SCHEMA.d_lov LOV
on LOV.row_id = concat('STATE~PROBLEM~', SRC.newvalue)
