...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.source_id' ELSE 'SUCCESS' END as Message 
from(select 
...
coalesce(inc.sourceinstance,'') <> coalesce(TRGT.source_id,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(outage.sourceinstance,'') <> coalesce(TRGT.source_id,''))ca