...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 on (CONCAT (mobile.city,'~',mobile.regionCode,'~',mobile.countryCode) = lov.row_id and lov.source_id =mobile.sourceinstance)
...
left join #DWH_TABLE_SCHEMA.d_location lov
 on (CONCAT (MobileRequestError.city,'~',MobileRequestError.regionCode,'~',MobileRequestError.countryCode) = lov.row_id and lov.source_id =MobileRequestError.sourceinstance)
 left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(lov.row_key,'') <> coalesce(TRGT.location_key,''))SQ
 ;