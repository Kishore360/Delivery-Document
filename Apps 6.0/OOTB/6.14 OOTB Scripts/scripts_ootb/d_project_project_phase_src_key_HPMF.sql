...
on concat('PROJECT~PHASE','~',PRJ_PHASE_CODE)=LKP.row_id
and SRC.sourceinstance=LKP.source_id
where coalesce(LKP.row_key,CASE WHEN SRC1.PRJ_PHASE_CODE IS NULL THEN 0 else '-1' end)
<> COALESCE(TRGT.PROJECT_PHASE_SRC_KEY ,'');