...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_request_task_activity.row_id' else 'SUCCESS' end as Message
from 
(
select TGT.row_id
...
and trim(SUBSTRING(CONCAT('sc_task~',TGT.task_attribute_wh_name,'~',F.row_id,'~',0),1,255)) <> COALESCE(TGT.row_id,'')
union all
select TGT.row_id
