...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_lov_test_execution_status.changed_on' ELSE 'SUCCESS' END as Message
FROM 
...
WHERE  COALESCE(TRGT.changed_on,'') <> COALESCE(NULL ,'')