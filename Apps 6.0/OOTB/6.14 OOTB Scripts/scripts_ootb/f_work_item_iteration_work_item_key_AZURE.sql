...
/* select COALESCE(concat(rev.workItemKey,'~',rev.newValue,'~',rev.organizationKey),'') , COALESCE(TRGT.row_id,'') */
FROM (select 
rev.workItemKey,
rev.newValue,
rev.organizationKey,
...
ON SRC.workItemKey = LKP.row_id and SRC.sourceinstance = LKP.source_id
where TRGT.soft_deleted_flag <> 'Y' and 
COALESCE(LKP.row_key,case when SRC.workItemKey is null then 0 else -1 end ) <> COALESCE(TRGT.work_item_key,'')