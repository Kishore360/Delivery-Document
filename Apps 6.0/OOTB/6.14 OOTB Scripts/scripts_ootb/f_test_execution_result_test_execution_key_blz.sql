...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for f_test_execution_result.test_execution_key' ELSE 'SUCCESS' END as Message
...
on SRC.masterid=LKP.row_id
and SRC.sourceinstance=LKP.source_id
where COALESCE(LKP.row_key, case when SRC.masterid is null then 0 else -1 end,'')<> COALESCE(TRGT.test_execution_key,'')
