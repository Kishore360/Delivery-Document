...
on concat('INTERNAL_CONTACT~',SRC1.resource_id)=LKP.row_id
and COALESCE(SRC.sourceinstance,'')=COALESCE(LKP.source_id,'')
WHERE COALESCE(LKP.row_key,CASE WHEN SRC1.resource_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.employee_key ,'')