SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_mobile_application_summary.device_uuid' ELSE 'SUCCESS' END as Message
FROM 
(select TRGT.row_id from 
#MDS_TABLE_SCHEMA.Mobile_final mobile LEFT OUTER JOIN #MDS_TABLE_SCHEMA.MobileRequestError_final merr
...
merr.devicetype), 'UNSPECIFIED'), '~', COALESCE(merr.deviceuuid, 'UNSPECIFIED'), '~',
 ...
 where coalesce(mobile.uuid,'') <> coalesce(TRGT.device_uuid)
 union
 select TRGT.row_id 
...
left join #DWH_TABLE_SCHEMA.f_mobile_application_summary TRGT
 ...
 where coalesce(MobileRequestError.uuid,'') <> coalesce(TRGT.device_uuid,''))SQ