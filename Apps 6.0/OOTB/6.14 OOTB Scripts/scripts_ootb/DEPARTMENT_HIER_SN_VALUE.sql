SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'd_h_all_hierarchies for DEPARTMENT failed' ELSE 'SUCCESS' END as Message
 FROM 
(
SELECT  CONCAT(a.s0,'~DEPARTMENT') AS row_id,
...
FROM ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_0
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_1
ON(lev_0.parent = lev_1.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_2
ON(lev_1.parent = lev_2.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_3
ON(lev_2.parent = lev_3.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_4
ON(lev_3.parent = lev_4.sys_id)
LEFT JOIN ( SELECT * FROM #MDS_TABLE_SCHEMA.cmn_department_final WHERE CDCTYPE<>'D') lev_5
ON(lev_4.parent = lev_5.sys_id)
)a
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_0
ON concat('DEPARTMENT~',a.s0)  = level_0.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_1
ON concat('DEPARTMENT~',a.s1)  = level_1.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_2
ON concat('DEPARTMENT~',a.s2)  = level_2.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_3
ON concat('DEPARTMENT~',a.s3)  = level_3.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_4
ON concat('DEPARTMENT~',a.s4)  = level_4.row_id 
LEFT JOIN  #DWH_TABLE_SCHEMA.d_internal_organization level_5
ON concat('DEPARTMENT~',a.s5)  = level_5.row_id  ) SRC
LEFT JOIN 
(
...
WHERE hierarchy_class ='DEPARTMENT' AND current_flag='Y') TRGT
