...
LEFT JOIN #DWH_TABLE_SCHEMA.d_resource_role LKP 
 ON ( coalesce(src.role_id,'UNSPECIFIED') = LKP.row_id 
AND src.prassignment_sourceinstance = LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN src.role_id IS NULL THEN 0 else '-1' end)<> COALESCE(trg.resource_role_key ,'')
;