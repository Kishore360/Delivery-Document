SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_work_item.due_on' ELSE 'SUCCESS' END as Message
FROM (
...
WHERE coalesce(convert_tz(SRC.microsoft_vsts_scheduling_duedate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'), '') <> coalesce(TRGT.due_on,'');