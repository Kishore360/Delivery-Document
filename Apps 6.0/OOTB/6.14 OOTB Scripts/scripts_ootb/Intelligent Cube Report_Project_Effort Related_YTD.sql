...
D.is_in_effort_flag in_effort,
effort_color.dimension_name efrt_color,

round(sum(F.actual_effort),0) act_efrt,

round(sum(F.planned_effort),0) plnd_efrt,

sum(case when D.is_in_effort_flag = 'Y' and D.task_type in ('PROJECT') then 1 else 0 end) no_of_proj_in_efrt,

case when sum(case when D.is_in_effort_flag = 'N' then f.planned_effort else 0 end) = 0 then null
else trim(to_char(round(((sum(case when D.is_in_effort_flag = 'N' then f.actual_effort else 0.00 end)-sum(case when D.is_in_effort_flag = 'N' then f.planned_effort else 0.00 end))/
sum(case when D.is_in_effort_flag = 'N' then f.planned_effort else 0.00 end))*100,0), '9,999,999,999'))||'%' end AVG_efrt_overrun,

case when  (round(sum(F.planned_percent_complete * 100.00),2) * round(sum(F.planned_duration),1)) = 0 then null else
round((round(sum(F.actual_percent_complete),0) * round(sum(F.planned_duration),1))/(round(sum(F.planned_percent_complete * 100.00),2) * round(sum(F.planned_duration),1)),2) end efrt_burn_rate,
...
max(effort_color.effort_color) efrt_vrnc_color,

round(sum(coalesce(F.remaining_effort,0)),0) remaining_efrt
from 
(
 SELECT f.*
...
group by D_P.row_key, Portfolio_name, Portfolio_category, D.row_key, D.Project_name, D2.row_key, Top_Project_Name, in_effort, efrt_color
