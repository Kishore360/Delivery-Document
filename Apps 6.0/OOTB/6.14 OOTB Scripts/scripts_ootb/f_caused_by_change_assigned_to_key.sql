...
from(select coalesce(inc_contact.row_key,case when chg.assigned_to is null then 0 else -1 end),coalesce(TRGT.row_id,'') from 
...
on inc_contact.row_id=coalesce(concat('INTERNAL_CONTACT~',chg.assigned_to),'UNSPECIFIED') and inc_contact.source_id=chg.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(inc_contact.row_key,case when chg.assigned_to is null then 0 else -1 end)<> coalesce(TRGT.assigned_to_key,'')

union all

select coalesce(inc_contact.row_key,case when chg.assigned_to is null then 0 else -1 end),coalesce(TRGT.assigned_to_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and coalesce(inc_contact.row_key,case when chg.assigned_to is null then 0 else -1 end)<> coalesce(TRGT.assigned_to_key,''))ca