...
CAL.month_name_abbreviated MN,
--'' as Metrics,
trim(to_char(count(F.outage_key), '9,999,999,990')) Outages,
trim(to_char(SUM(case when outage_type.dimension_name = 'Planned Outage' THEN 1 ELSE 0 END), '9,999,999,990')) planned,
trim(to_char(SUM(case when outage_type.dimension_name = 'Outage' THEN 1 ELSE 0 END), '9,999,999,990')) unplanned,
trim(to_char(SUM(case when outage_type.dimension_name = 'Degradation' THEN 1 ELSE 0 END), '9,999,999,990')) DEGRADATION
from 
(
