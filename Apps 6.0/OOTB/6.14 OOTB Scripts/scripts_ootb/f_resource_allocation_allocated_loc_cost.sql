SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_resource_allocation.allocated_loc_cost' ELSE 'SUCCESS' END as Message
 from (select SUBSTRING_INDEX(row_id,'~',1) as source_row_id, resource_plan_key, project_key, resource_key,(sum(distinct F.allocated_loc_cost) *F.week_day_count) as allocated_loc_cost 
...
WHERE COALESCE( CAST(coalesce(FX1.amount,0) as decimal(20,10)),'')<> COALESCE(cast(round(TRGT.allocated_loc_cost) as decimal(20,10)),'');