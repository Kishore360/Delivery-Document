...
on (date_format(convert_tz(SRC.microsoft_vsts_common_closeddate,'<<TENANT_SSI_TIME_ZONE>>','<<DW_TARGET_TIME_ZONE>>'),'%Y%m%d') = LKP.row_id)
 WHERE coalesce(LKP.row_key, 0) <> coalesce(TRGT.completed_on_key,'');