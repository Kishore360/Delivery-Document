...
Resource, 
employment_type, 
round(coalesce(Tenure_Years,0),1) as Tenure_Years, 
...
trim(to_char(coalesce(round((case when (coalesce(resource_actual_hours,0)-coalesce(resource_allocated_hour,0))<0 then '0.0' 
else (coalesce(resource_actual_hours,0)-coalesce(resource_allocated_hour,0)) end),1),0), '9,999,999,990D9')) Overtime_hours,
 


case when (coalesce(resource_allocated_hour,0)=0 or coalesce(resource_actual_hours,0)=0) then null
when (coalesce(resource_actual_hours,0)-coalesce(capacity_hours,0))<=0 then '0.0%' 
else concat(trim(to_char(coalesce(round((((resource_actual_hours-resource_allocated_hour)*100)/resource_allocated_hour),1),0), '9,999,999,990D9')),'%') end as Overtime_percentage


from 
(
select resource_key, Resource, employment_type,
Tenure_Years, 
...
group by resource_key, Resource, employment_type,
 Tenure_Years
)
where resource not in ('UNKNOWN') 
order by upper(resource),upper(employment_type),tenure_years,resource_allocated_hour,resource_actual_hours