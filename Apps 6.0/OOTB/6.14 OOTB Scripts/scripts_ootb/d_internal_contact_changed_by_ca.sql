SELECT 
    ...
    END AS Result,
    CASE
        WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed for d_internal_contact.changed_by'
        ELSE 'SUCCESS'
    END AS Message
FROM
    (SELECT 
    ...
    FROM
        (SELECT 
        sourceinstance AS srm_sourceinstance, ID, last_updated_By
    FROM
...
WHERE
    COALESCE(src.last_updated_By, '') <> COALESCE(trg.changed_by, '')
;
