...
FROM 
(select TRGT.planned_capital_doc_cost
...
and COALESCE(plan_cost.planned_capital_doc_cost,'')<>round(COALESCE(TRGT.planned_capital_doc_cost,0),5)
union
select TRGT.planned_capital_doc_cost from (select * from  #MDS_TABLE_SCHEMA.hp_kcrt_requests_final)SRC
inner join(select * from #MDS_TABLE_SCHEMA.hp_kcrt_fg_pfm_proposal_final)SRC1
on SRC.request_id=SRC1.request_id
and SRC.sourceinstance=SRC1.sourceinstance
left join(select * from  #MDS_TABLE_SCHEMA.hp_fm_financial_summary_final)fin_summary
