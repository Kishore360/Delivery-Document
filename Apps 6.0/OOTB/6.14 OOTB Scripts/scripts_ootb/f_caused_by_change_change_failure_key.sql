...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_caused_by_change.change_failure_key' ELSE 'SUCCESS' END as Message 
from(select coalesce(chg_fai.row_key,case when chg_fai.row_id is null then 0 else -1 end),coalesce(TRGT.change_failure_key,'')
from 
...
on chg_fai.row_id=coalesce(chg.sys_id,'UNSPECIFIED') and chg_fai.source_id=chg.sourceinstance
...
where UPPER(dimension_wh_code)='CLOSED' and coalesce(chg_fai.row_key,case when chg_fai.row_id is null then 0 else -1 end)<>coalesce(TRGT.change_failure_key,'')

union all

select coalesce(chg_fai.row_key,case when chg_fai.row_id is null then 0 else -1 end),coalesce(TRGT.change_failure_key,'')
...
where UPPER(map.dimension_wh_code)='CLOSED' and  coalesce(chg_fai.row_key,case when chg_fai.row_id is null then 0 else -1 end)<>coalesce(TRGT.change_failure_key,''))ca