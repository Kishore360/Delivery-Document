...
case when count(1) > 0 then 'MDS to DWH data validation failed for f_response.updated_count' else 'SUCCESS' end as Message
from 
(
select DWH.row_id
...
where ifnull(A.sys_mod_count,'') <> ifnull(DWH.updated_count,'')
)SQ;