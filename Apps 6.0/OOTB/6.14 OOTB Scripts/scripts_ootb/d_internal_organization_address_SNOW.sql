...
CASE WHEN count(1) >0 THEN 'MDS to DWH row count failed for d_internal_organization.address' ELSE 'SUCCESS' END as Message
from (select coalesce(SRC.street,''),coalesce(TRGT.address,'')
...
where coalesce(SRC.street,'')<>coalesce(TRGT.address,'')

union all

select (TRGT.row_id),null
...
where (TRGT.address)is not null
...
where (TRGT.address)is not null)inter_org