...
to_char((sum(coalesce(F.time_estimate ,0)) / 1.0), 'FM999,999,999,999,990') as original_estimate_opened, -- based on the opened date
to_char((sum(case when status.wi_sts in ('COMPLETED') then coalesce(F.time_estimate ,0) else 0.0 end) / 1.0), 'FM999,999,999,999,990') as original_estimate_closed, -- based on the closed date
to_char((sum(coalesce(F.time_remaining ,0)) /1.0),'FM999,999,999,999,990')as current_estimate_created, -- based on the opened date
to_char((sum(case when status.wi_sts in ('COMPLETED') then coalesce(F.time_remaining ,0) else 0.0 end) / 1.0), 'FM999,999,999,999,990') as current_estimate_closed -- based on the closed date
...
join 
(
 SELECT lov.*, wh_lov.dimension_code AS wi_sts_code, wh_lov.dimension_name AS wi_sts
   FROM #DWH_TABLE_SCHEMA.d_lov lov
   LEFT JOIN #DWH_TABLE_SCHEMA.d_lov_map br ON lov.row_key = br.src_key
   JOIN #DWH_TABLE_SCHEMA.d_lov wh_lov ON wh_lov.row_key = br.wh_key AND br.dimension_class = 'STATUS~WORK_ITEM'
UNION 
 SELECT lov.*, lov.dimension_code AS wi_sts_code, lov.dimension_name AS wi_sts
   FROM #DWH_TABLE_SCHEMA.d_lov lov
  WHERE lov.row_key = -1 OR lov.row_key = 0
) status
