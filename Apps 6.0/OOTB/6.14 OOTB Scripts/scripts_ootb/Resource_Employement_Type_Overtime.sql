/*Resource_Employement_Type_Overtime*/
select  
Resource, 
coalesce(employment_type,'UNKNOWN') as employment_type, 
round(coalesce(Tenure_Years,0),1) as Tenure_Years,
trim(to_char(coalesce(round((resource_allocated_hour),0),0), '9,999,999,990D9')) as resource_allocated_hour ,
trim(to_char(coalesce(round((resource_actual_hours),0),0), '9,999,999,990D9')) as resource_actual_hours,

round(case when coalesce((resource_actual_hours-resource_allocated_hour),0)<=0 then 0 else
(resource_actual_hours-resource_allocated_hour) end ,1) as Overtime_hours,

concat(round(coalesce(case when ((resource_actual_hours-capacity_hours)<=0 or resource_allocated_hour=0) then 0 else
((((resource_actual_hours-resource_allocated_hour)*100)/resource_allocated_hour)) end,0),1) ,'%')
as Overtime_percentage
from
(
select Resource,employment_type,Tenure_Years/*,calendar_date, resource_key,  , location_name, manager_name,
department, primary_role_key, role_name, primary_skill_key, skill_name*/, 
...
sum(resource_per_hour_cost) as resource_per_hour_cost
from
(
...
) group by Resource,employment_type,Tenure_Years/*,calendar_date, resource_key, Resource, , location_name, manager_name,
department, primary_role_key, role_name, primary_skill_key, skill_name*/
)order by  1,2