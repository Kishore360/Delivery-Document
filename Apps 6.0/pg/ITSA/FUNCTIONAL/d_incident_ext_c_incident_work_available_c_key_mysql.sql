...
ON CONCAT('WORK_AVAILABLE~INCIDENT~',upper(src.u_work_around_available))=lkp.row_id
where COALESCE(lkp.row_key,CASE WHEN src.u_work_around_available IS NULL THEN 0 else -1 end)<> trgt.incident_work_available_c_key
) temp;
