...
on COALESCE(DATE_FORMAT(SRC.Privacy_Finish_Date,'%Y%m%d'),'UNSPECIFIED')= l.row_id
WHERE COALESCE(l.row_key,CASE WHEN SRC.Privacy_Finish_Date IS NULL THEN 0 else '-1' end)<>
 TRGT.privacy_finish_c_key
) temp