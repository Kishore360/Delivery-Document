 ...
 FROM pan6_mdsdb.u_incident_tasks_final SRC 
  JOIN pan6_mdwdb.f_incident_tasks_c TRGT 
 ON (SRC.sys_id =TRGT.row_id 
 AND SRC.sourceinstance= TRGT.source_id )
 JOIN pan6_mdwdb.d_incident_tasks_c trgtf on TRGT.incident_tasks_c_key=trgtf.row_key
...
FROM pan6_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)- TRGT.dormancy_age) not in  (3600,-3600) and 
...
FROM pan6_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)<> TRGT.dormancy_age )a
