...
union
select 'ldb.dh_user_group_level2 a125 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_task a11 
join ldb.d_internal_organization_group a14 
on (a11.assignment_group_key = a14.row_key) join ldb.dh_user_group_hierarchy a15 
on (a14.row_dn_key = a15.lev_0_key) 
...
union
select 'ldb.dh_user_group_level3 a126 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_task a11 
join ldb.d_internal_organization_group a14 
on (a11.assignment_group_key = a14.row_key) join ldb.dh_user_group_hierarchy a15 
on (a14.row_dn_key = a15.lev_0_key) 
...
union
select 'ldb.dh_user_group_level4 a127 ' as Table_name, count(a11.row_key) Row_Count
from ldb.f_task a11 
join ldb.d_internal_organization_group a14 
on (a11.assignment_group_key = a14.row_key) join ldb.dh_user_group_hierarchy a15 
on (a14.row_dn_key = a15.lev_0_key) 
...
union
select 'd_task_category_c'as Table_name,count(1) from ldb.f_task f join ldb.d_task d on 
f.task_key=d.row_key join ldb.d_task_category_c dc on d.task_category_c_key=dc.row_key