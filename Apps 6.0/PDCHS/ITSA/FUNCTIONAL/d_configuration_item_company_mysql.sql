...
JOIN pdchs_mdwdb.d_internal_organization d 
ON COALESCE(CONCAT('SUBSIDIARY~',a.company),'UNSPECIFIED')=d.row_id 
AND a.sourceinstance=d.source_id
WHERE COALESCE(d.row_key,CASE WHEN a.company IS NULL THEN 0 ELSE -1 END)<>c.company_key
) temp;
