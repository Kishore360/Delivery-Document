...
select'ldb.d_incident_agebucket' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident_resolved       a11 
join ldb.d_incident_agebucket       a15
...
select'ldb.d_incident_change_request_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident_resolved       a11 
join ldb.d_incident_change_request_c       a115
on (a11.incident_change_request_c_key = a115.row_key)
union
...
on (a18.parent_incident_key = a118.row_key)
union
...
on (a18.incident_category_c_key = a134.row_key)
union
...
on (a18.severity_src_key = a139.row_key)
union
...
on (a18.state_src_key = a140.row_key)
union
...
on (a18.sub_category_src_key = a141.row_key)
union
...
on (a18.incident_action_c_key = a132.row_key)
union
...
on (a12.customer_mdm_key = a120.row_key)
union
...
union
select'ldb.d_incident_domain_c' as Table_Name, count(a11.row_key) Row_Count
 from  ldb.f_incident_resolved       a11 
join ldb.d_incident_domain_c       a125
on (a11.incident_domain_c_key = a125.row_key)
union
...
on (a17.incident_assignment_group_parent_c_key = a133.row_key)
union
...
on (a110.ci_used_for_c_key = a131.row_key)
