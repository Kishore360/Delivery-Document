...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for incident.incident_assignment_group_parent_c_key' ELSE 'SUCCESS' END as Message 
from pdchs_mdsdb.incident_final src 
left join pdchs_mdwdb.f_incident trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
