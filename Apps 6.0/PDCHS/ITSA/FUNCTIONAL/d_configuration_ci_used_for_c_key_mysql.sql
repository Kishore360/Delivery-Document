...
FROM (select sys_id,sourceinstance from pdchs_mdsdb.cmdb_ci_final where CDCTYPE<>'D') src
left join (select sys_id,sourceinstance,u_used_for from pdchs_mdsdb.cmdb_ci_service_final where CDCTYPE<>'D') src2
on src.sys_id=src2.sys_id and src.sourceinstance=src2.sourceinstance
