...
ON COALESCE(CONCAT('USED_FOR_C~CI_SERVICE~',src.u_used_for),'UNSPECIFIED') =lkp.row_id 
where COALESCE(lkp.row_key,CASE WHEN src.u_used_for IS NULL THEN 0 else -1 end)<> trgt.ci_used_for_c_key
;
