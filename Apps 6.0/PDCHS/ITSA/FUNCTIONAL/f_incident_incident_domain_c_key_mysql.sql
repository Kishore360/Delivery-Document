...
CASE WHEN CNT>0   THEN 'MDS to DWH data validation failed for f_incident.incident_domain_c_key' ELSE 'SUCCESS' END as Message 
...
where coalesce(domain.row_key, case when src.sys_domain is null then 0 else -1 end) <> f.incident_domain_c_key) temp; 