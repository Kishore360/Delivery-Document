...
on COALESCE(CONCAT('KB_CATEGORY_C','~','KNOWLEDGE','~',UPPER(a.category)),'UNSPECIFIED')=c.row_id
WHERE coalesce(b.row_key,case when a.category is null then 0 else -1 end )<>c.category_c_key 
) temp;