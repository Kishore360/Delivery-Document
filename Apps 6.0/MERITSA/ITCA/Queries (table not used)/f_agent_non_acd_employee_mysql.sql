...
FROM
(select TRGT.row_id from mercuryins_mdsdb.new_call_final SRC
...
union
select TRGT.row_id from mercuryins_mdsdb.incident_final SRC
LEFT JOIN qa_int_hf5_mdwdb.f_agent_non_acd TRGT
            ON (SRC.sqa_int_hf5_mdwdbys_id=TRGT.row_id
            AND SRC.sourceinstance =TRGT.source_id )
LEFT JOIN mercuryins_mdwdb.d_internal_contact LKP
            ON ( COALESCE(CONCAT('INTERNAL_CONTACT~',trim(SRC.opened_by)),'UNSPECIFIED') = LKP.row_id
            AND SRC.sourceinstance=LKP.source_id)
