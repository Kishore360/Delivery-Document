...
select case when newvalue<oldvalue then 'Y' else 'N' end as flag,documentkey from meritsa_mdsdb.sys_audit_final 
 where
tablename = 'incident' 
AND fieldname =  'priority' and oldvalue is not null and newvalue is not null
)a group by a.documentkey
)b
 join meritsa_mdsdb.u_asc_ticket_final c on b.documentkey=c.sys_id
join meritsa_mdwdb.d_incident_asc_c d on c.sys_id = d.row_id and c.sourceinstance = d. source_id
