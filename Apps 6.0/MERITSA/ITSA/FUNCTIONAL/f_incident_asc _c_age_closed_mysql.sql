...
FROM meritsa_mdwdb.f_incident_asc_c f
JOIN meritsa_mdwdb.d_lov_map br ON f.asc_incident_state_c_key = br.src_key
AND br.dimension_wh_code IN ('CLOSED','RESOLVED')
JOIN meritsa_mdwdb.d_incident_asc_c a ON a.row_key = f.incident_asc_c_key
AND f.source_id = a.source_id
where f.age <> timestampdiff(second, a.opened_on, coalesce(a.last_resolved_on, a.closed_on))