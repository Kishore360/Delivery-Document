...
FROM meritsa_mdsdb.u_asc_ticket_final x  left outer join 
(select source_id,row_key,row_id  from meritsa_mdwdb.d_internal_contact
 )y on 
CONCAT('INTERNAL_CONTACT~',x.u_resolved_by)=y.row_id  AND sourceinstance= source_id 
)A
 JOIN  
(SELECT  asc_incident_last_resolved_by_c_key AS B_asc_incident_last_resolved_by_c_key,source_id,ROW_ID FROM  meritsa_mdwdb.f_incident_asc_c
