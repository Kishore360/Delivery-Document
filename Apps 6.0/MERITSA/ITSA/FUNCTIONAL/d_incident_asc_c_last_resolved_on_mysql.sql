...
CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for d_incident_asc_c.last_resolved_on' ELSE 'data matched' END as
 Message  from (
select count(1) as cnt  FROM meritsa_mdsdb.u_asc_ticket_final SRC  LEFT JOIN meritsa_mdwdb.d_incident_asc_c TRGT  
ON (SRC.sys_id =TRGT.row_id   AND SRC.sourceinstance= TRGT.source_id  )
left join   meritsa_mdwdb.f_incident_asc_c f on f.incident_asc_c_key=TRGT.row_key
