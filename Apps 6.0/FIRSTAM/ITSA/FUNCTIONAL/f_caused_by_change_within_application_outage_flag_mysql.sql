...
from
(select COALESCE(cmdb_ci_outage.sys_id, incident.sys_id) AS row_id,change_request.sourceinstance AS source_id,IF((incident.cmdb_ci = change_request.cmdb_ci)  AND (cmdb_ci_outage.cmdb_ci = incident.cmdb_ci), 'Y', 'N') AS within_application_outage_flag from
...
union  all
select cmdb_ci_outage.sys_id AS row_id ,change_request.sourceinstance AS source_id,IF(cmdb_ci_outage.cmdb_ci = change_request.cmdb_ci, 'Y', 'N') AS within_application_outage_flag FROM
...
where 
SRC.within_application_outage_flag <>TRGT.within_application_outage_flag)a