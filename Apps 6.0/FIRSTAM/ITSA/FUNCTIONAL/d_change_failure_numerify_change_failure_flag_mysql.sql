...
FROM firstam_mdsdb.change_request_final a
JOIN firstam_mdwdb.d_change_request d ON a.sys_id=d.row_id AND a.sourceinstance=d.source_id
LEFT JOIN
(SELECT DISTINCT c.sourceinstance AS source_id,c.sys_id AS row_id,c.cdctype
FROM firstam_mdsdb.change_request_final c
JOIN firstam_mdsdb.incident_final i ON c.sys_id=i.caused_by AND c.sourceinstance=i.sourceinstance
WHERE i.caused_by IS NOT NULL AND (i.opened_at > c.end_date OR i.opened_at < c.start_date)) b
ON a.sys_id = b.row_id and a.sourceinstance = b.source_id
LEFT JOIN
(SELECT DISTINCT c.sourceinstance AS source_id,c.sys_id AS row_id,c.cdctype
FROM firstam_mdsdb.change_request_final c
JOIN firstam_mdsdb.label_final l ON c.number = l.name AND c.sourceinstance = l.sourceinstance
JOIN firstam_mdsdb.label_entry_final le ON le.label = l.sys_id AND le.sourceinstance = l.sourceinstance
JOIN firstam_mdsdb.incident_final i ON le.table_key = i.sys_id AND le.sourceinstance = i.sourceinstance
WHERE l.name LIKE 'GECHG%' AND le.id_type = 'Incident' AND (i.opened_at > c.end_date OR i.opened_at < c.start_date)) c
ON a.sys_id = c.row_id and a.sourceinstance = c.source_id
LEFT JOIN firstam_mdwdb.d_lov_map lm ON lm.dimension_class = 'STATE~CHANGE_REQUEST' AND a.state=lm.dimension_code
