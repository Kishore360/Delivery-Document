...
from
(select COALESCE(cmdb_ci_outage.sys_id, incident.sys_id) AS row_id,change_request.sourceinstance AS source_id,COALESCE(problem.sys_id,'UNSPECIFIED') AS problem_id from
...
union  all
select cmdb_ci_outage.sys_id AS row_id ,change_request.sourceinstance AS source_id,'UNSPECIFIED' AS problem_id FROM
...
ON cmdb_ci_outage.task_number = change_request.sys_id AND cmdb_ci_outage.sourceinstance = change_request.sourceinstance 
 ...
 join
 firstam_mdwdb.d_problem LKP
 on SRC.problem_id=LKP.row_id and SRC.source_id  =LKP.source_id
where 
coalesce(LKP.row_key,case when  SRC.problem_id is null then 0 else -1 end) <>TRGT.problem_key)a