...
from
(select COALESCE(cmdb_ci_outage.sys_id, incident.sys_id) AS row_id,change_request.sourceinstance AS source_id,IF((incident.cmdb_ci = change_request.cmdb_ci) AND (problem.cmdb_ci = incident.cmdb_ci), 'Y', 'N') AS within_application_problem_flag from
...
union  all
select cmdb_ci_outage.sys_id AS row_id ,change_request.sourceinstance AS source_id,'N' AS within_application_problem_flag FROM
...
where 
SRC.within_application_problem_flag<>TRGT.within_application_problem_flag)a