...
-- select count(*) 
 FROM ( SELECT * FROM firstam_mdsdb.sys_user_group_final WHERE CDCTYPE<>'D') SRC 
  JOIN firstam_mdwdb.d_internal_organization TRGT 
 ON (CONCAT('GROUP~',SRC.sys_id)=TRGT.row_id 
 AND SRC.sourceinstance=TRGT.source_id )
 join
 firstam_mdwdb.d_internal_contact LKP
