...
from
(select COALESCE(cmdb_ci_outage.sys_id, incident.sys_id) AS row_id,change_request.sourceinstance AS source_id,COALESCE(change_request.sys_id, 'UNSPECIFIED') as change_request_id from
...
union  all
select cmdb_ci_outage.sys_id AS row_id ,change_request.sourceinstance AS source_id,COALESCE(change_request.sys_id, 'UNSPECIFIED') as change_request_id FROM
...
ON cmdb_ci_outage.task_number = change_request.sys_id AND cmdb_ci_outage.sourceinstance = change_request.sourceinstance 
 ...
 join
 firstam_mdwdb.d_change_request LKP
 on SRC.change_request_id=LKP.row_id and SRC.source_id  =LKP.source_id
where 
coalesce(LKP.row_key,case when  SRC.change_request_id is null then 0 else -1 end) <>TRGT.change_request_key)a