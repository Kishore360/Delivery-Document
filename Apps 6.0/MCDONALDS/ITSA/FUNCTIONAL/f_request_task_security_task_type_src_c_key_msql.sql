 ...
 select count(1) as cnt 
  from mcd_mdsdb.sc_task_final s
left join mcd_mdwdb.f_request_task f on s.sys_id=f.row_id and s.sourceinstance=f.source_id
LEFT  join mcd_mdwdb.d_lov d on d.row_id= COALESCE(concat('SECURITY_TASK_TYPE_C~SC_TASK~',(case when short_description like '%SMART SSL%' then 'SMART SSL'
