...
from 
(select count(1) as cnt  from (select sys_id, sourceinstance, cdctime, document_id,cdctype from  mcd_mdsdb.sysapproval_approver_final where cdctype='X') src 
 join  mcd_mdwdb.d_sysapproval_approver_c trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
 join mcd_mdwdb.d_change_request LKP on src.document_id=LKP.row_id
