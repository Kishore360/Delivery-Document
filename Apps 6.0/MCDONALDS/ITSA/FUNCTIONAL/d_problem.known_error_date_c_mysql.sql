...
(select count(1) as cnt  from  (SELECT x.documentkey,x.prbnumber,x.sourceinstance,x.oldvalue,x.newvalue, x.sys_created_on , x.cdctime                
            ...
            FROM mcd_mdsdb.sys_audit_final t
			JOIN mcd_mdsdb.problem_final f
...
left join mcd_mdwdb.d_problem trgt  ON src.documentkey= trgt.row_id and src.sourceinstance=trgt.source_id 
