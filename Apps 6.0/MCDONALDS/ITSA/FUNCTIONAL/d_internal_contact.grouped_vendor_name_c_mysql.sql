...
from 
(select count(1) as cnt  from (SELECT  grouped_vendor_name, cdctime,sourceinstance,assignee_vendor_name FROM mcd_mdsdb.us_assignee_vendor_map_c_final) src 
left join mcd_mdwdb.d_internal_contact trgt  ON trgt.vendor_name_c = src.assignee_vendor_name
