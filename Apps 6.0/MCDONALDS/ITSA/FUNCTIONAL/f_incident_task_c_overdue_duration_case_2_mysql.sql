...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for f_incident_task_c.overdue_duration' ELSE 'SUCCESS' END as Message 
...
WHERE IF(CONVERT_TZ(src.due_date,'GMT','US/Central') < CONVERT_TZ(src.closed_at,'GMT','US/Central') , TIMESTAMPDIFF(SECOND,CONVERT_TZ(src.due_date,'GMT','US/Central'),CONVERT_TZ(src.closed_at,'GMT','US/Central')),0)  <> trgt.overdue_duration
;