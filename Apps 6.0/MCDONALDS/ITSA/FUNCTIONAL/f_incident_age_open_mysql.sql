 ...
 FROM (select sys_id,sourceinstance,opened_at from mcd_mdsdb.incident_final where opened_at < coalesce(resolved_at,closed_at)) SRC 
...
WHERE TIMESTAMPDIFF(SECOND,SRC.opened_at,(SELECT CONVERT_TZ(max(lastupdated),'US/Central','GMT') AS lastupdated FROM mcd_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' and etl_run_number=f.etl_run_number))<> f.age
