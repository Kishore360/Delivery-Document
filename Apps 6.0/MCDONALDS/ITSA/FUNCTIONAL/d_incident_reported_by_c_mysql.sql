...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_incident.reported_by_c' ELSE 'SUCCESS' END as Message
from 
...
where (src.cdctime<=f1.lastupdated) and  coalesce(src1.name,'UNSPECIFIED') <> trgt.reported_by_c) temp;