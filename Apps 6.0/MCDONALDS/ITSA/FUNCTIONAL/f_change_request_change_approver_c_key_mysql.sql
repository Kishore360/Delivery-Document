...
mcd_mdsdb.change_request_final src1
join
mcd_mdsdb.sysapproval_approver_final src2
on
...
(select number, coalesce(max(u_approval_completed),max(a1.sys_created_on)) created from 
  mcd_mdsdb.change_request_final a 
  join mcd_mdsdb.sysapproval_approver_final a1 on a.sys_id=a1.document_id where a1.state="approved" 
  ...
  left join mcd_mdwdb.d_internal_contact d1 on concat('INTERNAL_CONTACT~',x.approver)=d1.row_id
  left join mcd_mdwdb.f_change_request f on x.sys_id=f.row_id and x.sourceinstance=f.source_id
where  change_approver_c_key<>d1.row_key 
 and x.number not in (
select number from 
 (select number, a1.sys_created_on created,count(1) from    mcd_mdsdb.change_request_final a    join 
mcd_mdsdb.sysapproval_approver_final a1 on a.sys_id=a1.document_id where a1.state="approved"    group by 1,2   having count(1)>1
) 
a11
