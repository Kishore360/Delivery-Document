 ...
 JOIN mcd_mdwdb.d_incident_task_c trgtf on TRGT.incident_task_c_key=trgtf.row_key
...
FROM mcd_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)- TRGT.dormancy_age) not in  (3600,-3600) and 
...
FROM mcd_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%' ),'America/Los_Angeles','GMT')),0)<> TRGT.dormancy_age )a
