...
FROM (select count(1) as cnt from(select sys_id, sourceinstance, cdctime,label from mcd_mdsdb.label_entry_final )  src      
LEFT JOIN (select sys_id,name ,sourceinstance from mcd_mdsdb.label_final ) lf ON src.label = lf.sys_id 
Left join mcd_mdwdb.d_task_tag_c  trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
