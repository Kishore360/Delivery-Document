...
from 
(select count(1) as cnt  from (select sys_id, sourceinstance, cdctime, approver,cdctype from  mcd_mdsdb.sysapproval_approver_final) src 
left join  mcd_mdwdb.d_sysapproval_approver_c trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
left join mcd_mdwdb.d_internal_contact LKP on COALESCE(CONCAT('INTERNAL_CONTACT~',src.approver),'UNSPECIFIED')=LKP.row_id
