 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_incident_region_list_c.pivot_date' ELSE 'SUCCESS' END as Message from ( select count(1) cnt  
...
AND a.sourceinstance= d.source_id )
 left join ibmwatson_mdwdb.f_incident_environment_list_c b
 on  CONCAT(a.sys_id,'~',COALESCE(SUBSTRING_INDEX(SUBSTRING_INDEX(TRIM(a.u_environment), ',', n.n), ',', -1),'UNSPECIFIED')) =b.row_id  
 where  coalesce(d.row_key,case when u_environment is null then 0 else -1 end)<>coalesce(b.environment_c_key,0))a
