...
FROM  
        ...
        left join ibmwatson_mdwdb.d_change_request d on  led.table_key=d.row_id and d.source_id=2
        join   ibmwatson_mdwdb.d_task_tag_c f on led.sys_id=f.row_id and lf.sourceinstance=f.source_id 
		where upper(t.sys_class_name)  ='change_request' and coalesce(d.row_key,case when table_key is null then 0 else -1 end) <>task_tag_change_request_c_key;
