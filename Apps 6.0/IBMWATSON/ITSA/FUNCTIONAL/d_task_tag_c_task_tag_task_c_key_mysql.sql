SELECT request_task WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 request_task WHEN count(1) >0 THEN 'MDS to DWH data validation failed' ELSE 'SUCCESS' END as Message from 
-- select table_key,coalesce(d.row_key,case when table_key is null then 0 else -1 end) ,task_tag_task_c_key FROM  
ibmwatson_mdsdb.label_entry_final   led
        ...
        left join ibmwatson_mdwdb.d_task d on  led.table_key=d.row_id and d.source_id=2 and upper(t.sys_class_name)  ='incident'
