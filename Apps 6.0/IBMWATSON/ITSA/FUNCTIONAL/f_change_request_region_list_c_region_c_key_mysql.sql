 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_problem_region_list_c.pivot_date' ELSE 'SUCCESS' END as Message from ( select count(1) cnt  from ibmwatson_mdsdb.change_request_final  a
 JOIN
...
ON n.n <= 1 + (LENGTH(COALESCE(TRIM(a.u_region_list),'UNSPECIFIED')) - LENGTH(REPLACE(COALESCE(TRIM(a.u_region_list),'UNSPECIFIED'), ',', '')))
 left join ibmwatson_mdwdb.d_region_c d 
...
AND a.sourceinstance= d.source_id ) 
 left join ibmwatson_mdwdb.f_change_request_region_list_c b
