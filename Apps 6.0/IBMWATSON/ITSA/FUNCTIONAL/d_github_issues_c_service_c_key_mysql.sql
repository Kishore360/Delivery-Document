...
left join ibmwatson_mdwdb.d_github_service_map_c d on u_service=d.row_id
left join   ibmwatson_mdwdb.d_github_issues_c t on s.sys_id=t.row_id and s.sourceinstance=t.source_id
where service_c_key<> coalesce(d.row_key,case when u_service is null then 0 else -1 end );
