SELECT CASE WHEN cnt > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 from(select count(1) as cnt from ibmwatson_mdsdb.task_final src
...
left join ibmwatson_mdwdb.d_tribe_c d on a.u_tribe=d.row_id and a.sourceinstance=d.source_id and a.cdctype<>'D'
 join ibmwatson_mdwdb.d_task trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
 where COALESCE(d.row_key,case when u_tribe is null then 0 else -1 end )<>trgt.task_tribe_c_key )ma
