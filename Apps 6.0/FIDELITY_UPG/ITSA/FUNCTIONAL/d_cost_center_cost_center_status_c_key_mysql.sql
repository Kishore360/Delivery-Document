...
on COALESCE(CONCAT('STATUS~COST_CENTER~', SRC.u_status), 'UNSPECIFIED') = LKP.row_id and SRC.sourceinstance=LKP.source_id
 WHERE  coalesce(LKP.row_key, case when SRC.u_status is null then 0 else -1 end )<>(TRGT.cost_center_status_c_key) and SRC.cdctype<>'D';
