...
join fidelity_mdsdb.u_fmr_division_final di on cc.u_division=di.sys_id and di.sourceinstance=cc.sourceinstance
join fidelity_mdwdb.d_internal_organization trg on trg.row_id=concat('GROUP~',f.sys_id) and f.sourceinstance=trg.source_id
where trg.division_code_c<>coalesce(di.u_division,'UNSPECIFIED')   and f.cdctype='X'
