...
on COALESCE(CONCAT('OVERRIDE_DB2_MODULE_C~CHANGE_REQUEST~', SRC.u_override_db2_module), 'UNSPECIFIED') = LKP.row_id and
SRC.sourceinstance=LKP.source_id
 WHERE coalesce(LKP.row_key, case when SRC.u_override_db2_module is null then 0 else -1 end )  <>(TRGT.override_db2_module_src_c_key) and SRC.cdctype<>'D';
