...
on COALESCE(CONCAT('INTERNAL_CONTACT~', SRC.u_business_product_manager), 'UNSPECIFIED')  = LKP.row_id and SRC.sourceinstance=LKP.source_id
 WHERE coalesce(LKP.row_key, case when SRC.u_business_product_manager is null then 0 else -1 end ) <>(TRGT.product_manager_c_key) and SRC.cdctype<>'D';
