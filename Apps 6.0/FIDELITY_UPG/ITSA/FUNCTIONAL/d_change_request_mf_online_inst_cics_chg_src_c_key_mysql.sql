...
on COALESCE(CONCAT('MF_ONLINE_INST_CICS_CHG_C~CHANGE_REQUEST~', SRC.u_mf_online_inst_cics_chg), 'UNSPECIFIED') = LKP.row_id and
SRC.sourceinstance=LKP.source_id
 WHERE coalesce(LKP.row_key, case when SRC.u_mf_online_inst_cics_chg is null then 0 else -1 end )  <>(TRGT.mf_online_inst_cics_chg_src_c_key) and SRC.cdctype<>'D';
