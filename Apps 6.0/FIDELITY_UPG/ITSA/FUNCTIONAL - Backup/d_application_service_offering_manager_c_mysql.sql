...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for d_application.service_offering_manager_c' ELSE 'SUCCESS' END as Message
...
left join fidelity_mdsdb.sys_user_final sys on prd.u_service_offering_manager=sys.sys_id
  where trg.service_offering_manager_c<>coalesce(sys.name,'UNSPECIFIED') and app.cdctype<>'D'
