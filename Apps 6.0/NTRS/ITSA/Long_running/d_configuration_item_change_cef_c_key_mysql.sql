...
Left JOIN ntrust_mdwdb.d_lov LKP ON CONCAT('CEF_SRC_C~CMDB_CI','~',UPPER(SRC.u_cef))=LKP.row_id  AND SRC.sourceinstance=LKP.source_id
where case when u_cef is NULL then 0 else coalesce(LKP.row_key,-1) end <> TRGT2.change_cef_c_key and SRC.cdctype='X') temp;