...
FROM   ntrust_mdsdb.problem_task_final SRC
 ...
 COALESCE(CONCAT('PROBLEM_TASK_TYPE_C~PROBLEM_TASK~',UPPER(SRC.problem_task_type)),'UNSPECIFIED')=TRGT2.row_id
  AND SRC.sourceinstance=TRGT2.source_id
where coalesce(TRGT2.row_key,case when SRC.problem_task_type is null then 0 else -1 end)
         <> TRGT.problem_task_type_c_key and SRC.cdctype='X' )temp;
