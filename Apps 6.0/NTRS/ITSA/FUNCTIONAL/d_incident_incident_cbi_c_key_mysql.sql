...
(select count(1) as cnt  from ntrust_mdsdb.incident_final a
left join ntrust_mdwdb.d_incident b11 on b11.row_id=a.sys_id and a.sourceinstance=b11.source_id

left join ntrust_mdwdb.d_lov b1 on b1.row_id=COALESCE(CONCAT('CBI_SRC_C~INCIDENT',
                '~',
                UPPER(a.u_cbi)),
