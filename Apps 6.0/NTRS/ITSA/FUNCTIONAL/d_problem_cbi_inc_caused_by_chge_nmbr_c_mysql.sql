...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for d_problem.cbi_inc_caused_by_chge_nmbr_c' ELSE 'SUCCESS' END as Message
...
LEFT JOIN 
(select group_concat(distinct chg.number) as chg,TRGT2.row_key,TRGT2.problem_number
from ntrust_mdsdb.problem_final SRC
JOIN ntrust_mdwdb.d_problem TRGT2 ON (SRC.sys_id=TRGT2.row_id AND SRC.sourceinstance=TRGT2.source_id)
...
left join ntrust_mdsdb.change_request_final chg on inc.caused_by=chg.sys_id and inc.sourceinstance=chg.sourceinstance
...
where coalesce(LKP.chg,'UNSPECIFIED') <> TRGT2.cbi_inc_caused_by_chge_nmbr_c and SRC.cdctype='X') temp;