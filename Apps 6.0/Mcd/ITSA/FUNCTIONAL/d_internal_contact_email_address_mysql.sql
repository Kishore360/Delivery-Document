...
FROM (select request,sys_id,sourceinstance,cdctime from   mcd_mdsdb.sc_req_item_final where cdctype<>'D') SRC
left join mcd_mdsdb.sc_request_final LKP1 on SRC.request=LKP1.sys_id and SRC.sourceinstance=LKP1.sourceinstance
left join mcd_mdsdb.sys_user_final LKP2 on LKP1.requested_for=LKP2.sys_id and SRC.sourceinstance=LKP2.sourceinstance
join mcd_mdwdb.f_request_item f on SRC.sys_id=f.row_id and SRC.sourceinstance=f.source_id
join  mcd_mdwdb.d_internal_contact f2 on f.requested_for_key=f2.row_key
