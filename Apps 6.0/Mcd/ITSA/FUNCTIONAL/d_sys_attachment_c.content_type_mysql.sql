...
FROM (select sys_id,sourceinstance,sys_created_by,cdctime,content_type from mcd_mdsdb.sys_attachment_final where table_name not like ('%sys_email%') 
)a 
JOIN (select row_id,source_id,content_type from mcd_mdwdb.d_sys_attachment_c where soft_deleted_flag='N') X 
