...
-- SELECT  coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_is_approver) is null then 0 else -1 end) , TRGT.is_approver_key,SRC.CDCTYPE 
-- ,user.sys_id,SRC.u_is_approver,SRC.sys_id
...
where tablename = 'x_scafe_mcdcr_chan_mcdcr_change_request' and old.value = 18) tmp
...
COALESCE(Concat('INTERNAL_CONTACT~', coalesce(user.sys_id,SRC.u_is_approver)), 'UNSPECIFIED')= LKP.row_id
...
where (SRC.cdctime<=f1.lastupdated) and 
 coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_is_approver) is null then 0 else -1 end) <> TRGT.is_approver_key
 and SRC.CDCTYPE='X';