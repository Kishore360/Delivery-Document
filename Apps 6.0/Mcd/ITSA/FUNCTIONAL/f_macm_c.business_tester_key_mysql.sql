...
-- SELECT  coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_business_tester) is null then 0 else -1 end) , TRGT.business_tester_key ,SRC.CDCTYPE 
-- ,user.sys_id,SRC.u_business_tester,SRC.sys_id
...
where tablename = 'x_scafe_mcdcr_chan_mcdcr_change_request' and old.value = 16) tmp
...
COALESCE(Concat('INTERNAL_CONTACT~', coalesce(user.sys_id,SRC.u_business_tester)), 'UNSPECIFIED')= LKP.row_id
...
where (SRC.cdctime<=f1.lastupdated) and 
 coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_business_tester) is null then 0 else -1 end) <> TRGT.business_tester_key  ;