...
from mcd_mdsdb.change_request_final SRC1
left join (SELECT *, CONCAT('SUBSIDIARY~',left(vendors,32)) as jn1 FROM mcd_mdsdb.sys_user_group_final) SRC 
on SRC1.assignment_group=SRC.sys_id and SRC1.sourceinstance=SRC.sourceinstance
join  (SELECT *, RIGHT(row_id,32) as jn2  FROM mcd_mdwdb.d_internal_organization) TRGT
