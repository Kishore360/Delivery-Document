...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_request_item.date_created_c' ELSE 'SUCCESS' END as Message
from 
...
where (src.cdctime<=f1.lastupdated) and  CONVERT_TZ(src1.date_created,'GMT','US/Central') <> trgt.date_created_c) temp;