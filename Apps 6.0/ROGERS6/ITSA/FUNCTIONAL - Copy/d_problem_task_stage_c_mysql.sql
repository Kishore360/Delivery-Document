...
FROM rogers_mdwdb.d_problem_task a 
join rogers_mdwdb.f_problem_task f 
on a.row_key=f.problem_task_key AND a.source_id=f.source_id
  join rogers_mdwdb.d_lov_map  b on f.state_src_key = b.src_key
 JOIN rogers_mdwdb.d_lov c  on f.state_src_key = c.row_key AND c.source_id=f.source_id
 LEFT JOIN (
