...
left join rogers6_mdwdb.d_internal_organization d 
on COALESCE(CONCAT('SUBSIDIARY~',ci.company),'UNSPECIFIED')=d.row_id
where d.soft_deleted_flag='N' and company_c_key <> coalesce(d.row_key,case when ci.company is null then 0 else -1 end) and  ci.cdctype='X' and d.soft_deleted_flag='N')a;
