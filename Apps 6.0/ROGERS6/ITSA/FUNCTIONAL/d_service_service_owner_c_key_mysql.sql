...
FROM rogersdev6_mdsdb.cmdb_ci_service_final SRC
left JOIN rogersdev6_mdwdb.d_service TRGT
ON (CONCAT('BUSINESS_SERVICE','~',SRC.sys_id) = TRGT.row_id AND SRC.sourceinstance = TRGT.source_id ) and SRC.cdctype<>'D'
LEFT JOIN rogersdev6_mdwdb.d_internal_contact LKP ON COALESCE(CONCAT('INTERNAL_CONTACT~',SRC.u_service_owner),'UNSPECIFIED')=LKP.row_id
