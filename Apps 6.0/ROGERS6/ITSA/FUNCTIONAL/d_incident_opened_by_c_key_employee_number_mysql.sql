...
coalesce(b2.row_key,case when caller_id is null then 0 else -1 end ) as b
 from rogers6_mdwdb.d_incident d join rogers6_mdsdb.incident_final a on d.row_id=a.sys_id and d.source_id=a.sourceinstance
left  join rogers6_mdwdb.d_internal_contact b2 on coalesce(concat('INTERNAL_CONTACT~',a.caller_id),'UNSPECIFIED')=b2.row_id
where opened_by_c_key<>coalesce(b2.row_key,case when caller_id is null then 0 else -1 end )
and a.CDCTYPE='X' and b2.soft_deleted_flag='N' and d.soft_deleted_flag='N') a;
