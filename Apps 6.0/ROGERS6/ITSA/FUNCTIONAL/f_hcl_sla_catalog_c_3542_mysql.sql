...
left join rogers6_mdwdb.d_internal_organization d11 on d12.assignment_group_for_pit_lead_c_key=d11.row_key 
left join rogers6_mdwdb.d_internal_organization d112 on f.assignment_group_key=d112.row_key 
where d32.lagging_count_of_month between 0 and 11 and
has_breached=0 and 
d11.organization_name like '%HCL%' and 
d112.organization_name like '%HCL%' and 
b34.name='Rogers Priority 4 - Resolution' 
...
group by 1 )a
cross join 
( select  month_start_date_key month_start_date_key,count(1) cnt,b.cdctype from      rogers6_mdsdb.incident_final b
join    rogers6_mdsdb.task_sla_final a on a.task=b.sys_id 
join rogers6_mdsdb.contract_sla_final b34 on a.sla=b34.sys_id
join   rogers6_mdwdb.f_incident f on b.sys_id=f.row_id 
join rogers6_mdwdb.d_problem d12 on f.problem_key=d12.row_key
...
left join rogers6_mdwdb.d_internal_organization d11 on d12.assignment_group_for_pit_lead_c_key=d11.row_key 
left join rogers6_mdwdb.d_internal_organization d112 on f.assignment_group_key=d112.row_key 
where month_start_date_key>='20180101' and d32.lagging_count_of_month between 0 and 11 and
d11.organization_name like '%HCL%' and 
d112.organization_name like '%HCL%' and 
b34.name='Rogers Priority 4 - Resolution' 
and u_sub_status not in ('Cancelled','Out Of Scope') 
