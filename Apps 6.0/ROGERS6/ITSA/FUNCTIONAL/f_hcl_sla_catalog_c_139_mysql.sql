...
where sla='1.3.9' and d.hcl_schedule_ac_attachment_ref ='Availability' )a1
right join 
(
...
from (
select  d32.month_start_date_key yz,(f2.available_duration*ta.number_of_days) ABC,(incident_duration)   DEF
...
join rogers6_mdwdb.f_task_ci_c c2 on d.problem_key=c2.task_key
...
-- join rogers6_mdwdb.d_application  d1 on f2.application_key=d1.row_key
...
-- New addition 
join rogers6_mdwdb.d_lov g1 on dc.status_src_c_key=g1.row_key 
...
where component_criticality_c='Vital'   and dc.subcategory in ('Database','MSSQL','Oracle')
