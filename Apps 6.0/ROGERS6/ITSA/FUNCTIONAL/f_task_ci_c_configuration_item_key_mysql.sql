...
join rogers6_mdwdb.f_task_ci_c trgt on src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id 
left join rogers6_mdwdb.d_configuration_item lkp on src.ci_item=lkp.row_id  and src.sourceinstance=lkp.source_id
where sys_class_name='problem' and coalesce(lkp.row_key,case when src.ci_item is null then 0 else -1 end) <> trgt.configuration_item_key
