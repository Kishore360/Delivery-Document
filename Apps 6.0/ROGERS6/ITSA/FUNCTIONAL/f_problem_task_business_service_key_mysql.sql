...
join rogers6_mdsdb.problem_final SRC1
on SRC.u_pm_number=SRC1.sys_id 
JOIN rogers6_mdwdb.f_problem_task TRGT 
ON (SRC.sys_id = TRGT.row_id AND SRC.sourceinstance = TRGT.source_id )  
LEFT JOIN rogers6_mdwdb.d_service LKP 
ON (COALESCE(CONCAT('BUSINESS_SERVICE','~',SRC1.cmdb_ci ),'UNSPECIFIED') = LKP.row_id AND SRC.sourceinstance = LKP.source_id ) 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC1.cmdb_ci IS NULL THEN 0 else -1 end)<> (TRGT.business_service_key) 
