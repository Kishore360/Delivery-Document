...
left join ibmwatson_mdwdb.d_lov d on concat('ACCOUNT_TYPE_C~U_IBM_ACCOUNTS~~~',u_account_type)=d.row_id
left join  ibmwatson_mdwdb.d_ibm_accounts_c t on s.sys_id=t.row_id and s.sourceinstance=t.source_id
where account_type_src_c_key<> coalesce(d.row_key,case when u_account_type is null then 0 else -1 end )and s.cdctype <>'D'