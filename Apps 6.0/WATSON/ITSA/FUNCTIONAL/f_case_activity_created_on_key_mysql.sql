...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_case.changed_on' ELSE 'SUCCESS' END as Message
  FROM ibmwatson_mdsdb.sys_audit_final src
left join ibmwatson_mdwdb.d_calendar_date lkp on date_format(CONVERT_TZ (sys_created_on,'GMT','UTC'),'%Y%m%d') =lkp.row_id  and tablename='sn_customerservice_case'
join ibmwatson_mdwdb.f_case_activity trgt on 
