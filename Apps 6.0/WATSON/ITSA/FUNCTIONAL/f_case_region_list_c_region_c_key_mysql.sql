 ...
 CASE WHEN cnt >0 THEN 'MDS to DWH data validation failed for f_problem_region_list_c.pivot_date' ELSE 'SUCCESS' END as Message from ( select count(1) cnt  from ibmwatson_mdsdb.sn_customerservice_case_final  a
 JOIN
...
AND a.sourceinstance= d.source_id )
 left join ibmwatson_mdwdb.f_case_region_list_c b
