...
LEFT JOIN whirlpool_mdwdb.d_location LKP 
 ON ( COALESCE(tf.location,'UNSPECIFIED') = LKP.row_id 
AND tf.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN tf.location IS NULL THEN 0 else -1 end)<> (ts.location_c_key)
