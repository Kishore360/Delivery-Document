SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON ( COALESCE(CONCAT('LKP_REGION_C~INCIDENT~~~',UPPER(LKP.u_region_lkp )),'UNSPECIFIED')= LKP1.row_id  )
left join   whirlpool_mdwdb.d_internal_contact f on CONCAT('INTERNAL_CONTACT~',LKP.sys_id)=f.row_id and LKP.sourceinstance=f.source_id
 WHERE COALESCE(LKP1.row_key,CASE WHEN LKP.u_region_lkp   IS NULL THEN 0 else -1 end)<> (f.lkp_region_c_key)
;
