...
CASE WHEN cnt>0 THEN 'MDS to DWH data validation failed for d_asset.managed_by_c_key' ELSE 'SUCCESS' END as Message 
 ...
 on COALESCE(CONCAT('INTERNAL_CONTACT~',b.managed_by),'UNSPECIFIED') =lkp.row_id and a.sourveinstance=lkp.source_id
 where coalesce(lkp.row_key,case when b.managed_by is null then 0 else -1 end)<>b.managed_by_c_key  and b.soft_deleted_flag='N')ma;
