select 'ldb.f_problem' as Table_Name, count(1) Row_Count
...
union
select'ldb.d_pit_lead_support_group_business_unit_c' as Table_Name, count(1) Row_Count
 from  ldb.f_problem       a11 
join ldb.d_internal_organization_pit_lead_c       a110
on (a11.assignment_group_for_pit_lead_c_key=a110.row_key)
join	ldb.d_pit_lead_support_group_business_unit_c	a124
	  on 	(a110.business_unit_c_key = a124.row_key)
union
...
union
select'ldb.d_lov_problem_contact_type_c' as Table_Name, count(1) Row_Count
 from  ldb.f_problem       a11 

join	ldb.d_lov_problem_contact_type_c	a124
