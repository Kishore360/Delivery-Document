...
where d.soft_deleted_flag='N' and certificate_requester_c_key <> coalesce(d.row_key,case when ci.u_certificate_requestor is null then 0 else -1 end) and ci.cdctype='X' and d.soft_deleted_flag='N')a