...
select count(1) cnt  from  rogers6_mdwdb.f_response f
JOIN rogers6_mdsdb.survey_response_final s
on f.row_id=s.sys_id and f.source_id=s.sourceinstance
LEFT JOIN rogers6_mdsdb.question_final q on s.question=q.sys_id and s.sourceinstance=q.sourceinstance
left join rogers6_mdwdb.d_question_c q1 on q1.row_id=case when f.survey_type ='LEGACY' then coalesce(q.sys_id,'UNSPECIFIED')
...
and q.CDCTYPE='X' and q1.soft_deleted_flag='N')a;