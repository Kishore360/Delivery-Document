SELECT CASE WHEN cnt>0  THEN 'FAILURE' ELSE 'SUCCESS' END as Result, CASE WHEN cnt>0
 ...
 FROM (
select count(1) cnt
from (select SRC.sourceinstance,SRC.u_assignment_group_for_pit_lead u_assignment_group_for_pit_lead,TRGT.assignment_group_for_pit_lead_c_key,source_id source_id,SRC.cdctype
...
left join ( select right(row_id,32) row_id,row_key,soft_deleted_flag from rogers_mdwdb.d_internal_organization    ) LKP
 on a.u_assignment_group_for_pit_lead=LKP.row_id and a.sourceinstance = a.source_id
WHERE 
 COALESCE(LKP.row_key,CASE WHEN a.u_assignment_group_for_pit_lead IS NULL THEN 0 else -1 end) <> (a.assignment_group_for_pit_lead_c_key) 
