...
FROM rogers_mdsdb.problem_final  SRC JOIN rogers_mdwdb.d_problem TRGT ON (SRC.sys_id = TRGT.row_id AND SRC.sourceinstance = TRGT.source_id )  
LEFT JOIN rogers_mdwdb.d_lov LKP ON ( concat( 'SUBSTATUS~PROBLEM~' ,upper( SRC.u_substatus)) = LKP.row_id AND SRC.sourceinstance = LKP.source_id ) 
