...
join
fidelity_mdwdb.d_resource_role LKP
on COALESCE(rsc_positions.ROLE_ID,'UNSPECIFIED') = LKP.row_id 
where COALESCE(LKP.row_key,case when rsc_positions.ROLE_ID is null then 0 else -1 end)<>TRGT.resource_role_key