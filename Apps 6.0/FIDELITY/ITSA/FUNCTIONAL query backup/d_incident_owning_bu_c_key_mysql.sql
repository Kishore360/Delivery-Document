...
LEFT JOIN fidelity_mdwdb.d_fmr_business_unit_c LKP on SRC.u_owning_bu =LKP.row_id and SRC.sourceinstance=LKP.source_id
JOIN  fidelity_mdwdb.d_incident TRGT 
ON (SRC.sys_id = TRGT.row_id AND SRC.sourceinstance= TRGT.source_id)
WHERE coalesce(LKP.row_key,case when SRC.u_owning_bu  is null then 0 else -1 end)<> TRGT.owning_bu_c_key; 
