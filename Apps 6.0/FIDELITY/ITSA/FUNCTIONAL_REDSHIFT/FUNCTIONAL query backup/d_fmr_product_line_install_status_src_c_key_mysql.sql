...
on COALESCE(CONCAT('INSTALL_STATUS~FMR_PRODUCT_LINE_C~',SRC.install_status),'UNSPECIFIED')=LKP.row_id
WHERE coalesce(LKP.row_key,case when SRC.install_status is null then 0 else -1 end) <>TRGT.install_status_src_c_key; 