...
COALESCE(a.vendor_name,'UNSPECIFIED'),'~',COALESCE(a.corp_id,'UNSPECIFIED'),'~ACTUAL') as row_id , ITG_PROJECT_NUMBER
,CALENDAR_PERIOD_NUMBER,sourceinstance,
sum(WORKED_HOURS)  actual_hours_c
...
group by 1,2,3,4)a
left join 
(select row_id,actual_hours_c,source_id from  fidelity_mdwdb.f_project_cost) b 
on a.row_id=b.row_id and a.sourceinstance=b.source_id where a.actual_hours_c<>b.actual_hours_c;