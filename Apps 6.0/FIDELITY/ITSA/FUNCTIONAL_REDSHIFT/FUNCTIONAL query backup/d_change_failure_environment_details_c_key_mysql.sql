...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_change_failure.environment_details_src_c_key' ELSE 'SUCCESS' END as Message 
...
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_environment_details IS NULL THEN 0 else -1 end)<> (TRGT.environment_details_src_c_key) ) temp;
