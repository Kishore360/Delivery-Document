...
ORDER BY n) n
ON n.n <= 1 + (LENGTH(COALESCE(TRIM(inc.u_caused_by_change),'UNSPECIFIED')) - LENGTH(REPLACE(COALESCE(TRIM(inc.u_caused_by_change),'UNSPECIFIED'), ',', ''))))a12
...
where a13.row_key <>a11.change_request_key and a11.soft_deleted_flag='N' )a 
