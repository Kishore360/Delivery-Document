...
on COALESCE(CONCAT('INTERNAL_CONTACT~',SRC.u_business_service_manager),'UNSPECIFIED')=LKP.row_id
WHERE coalesce(LKP.row_key,case when SRC.u_business_service_manager is null then 0 else -1 end) <>TRGT.business_service_manager_c_key; 