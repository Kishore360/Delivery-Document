...
on (LKP.row_id  = CONCAT(SRC.resource_id,'~',SRC.task_id) and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.resource_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_plan_key,''); 