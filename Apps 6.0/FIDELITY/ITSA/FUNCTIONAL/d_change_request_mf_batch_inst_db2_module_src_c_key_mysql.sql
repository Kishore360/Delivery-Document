...
on COALESCE(CONCAT('MF_BATCH_INST_DB2_MODULE_C~CHANGE_REQUEST~',  SRC.u_mf_batch_inst_db2_module), 'UNSPECIFIED') = LKP.row_id and
SRC.sourceinstance=LKP.source_id
 WHERE coalesce(LKP.row_key, case when SRC.u_mf_batch_inst_db2_module is null then 0 else -1 end )  <>(TRGT.mf_batch_inst_db2_module_src_c_key) and SRC.cdctype<>'D';
