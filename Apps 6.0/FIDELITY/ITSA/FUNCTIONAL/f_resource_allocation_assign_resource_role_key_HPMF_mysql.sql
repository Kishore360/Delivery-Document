...
LEFT JOIN(select * from fidelity_mdsdb.hp_wp_task_info_final)SRC6
on SRC1.task_info_id=SRC6.task_info_id
...
on (LKP.row_id  = COALESCE(SRC6.role_id,'') and LKP.source_id  = SRC.sourceinstance)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC6.role_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_role_key,''); 