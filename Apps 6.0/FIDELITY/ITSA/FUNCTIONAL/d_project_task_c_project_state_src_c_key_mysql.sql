...
FROM  (select Driver.TASK_ID,
                    Driver.sourceinstance,Driver.STATUS  from( SELECT
                    TASK_ID,
                    sourceinstance,
                    null as STATUS,
                    CASE 
...
on COALESCE(CONCAT(SRC.STATUS,'~~~PM - TASK STATUSES'),'UNSPECIFIED')=LKP.row_id
WHERE COALESCE(LKP.row_key,case when SRC.STATUS is null then 0 else -1 end ) <> TRGT.project_state_src_c_key; 