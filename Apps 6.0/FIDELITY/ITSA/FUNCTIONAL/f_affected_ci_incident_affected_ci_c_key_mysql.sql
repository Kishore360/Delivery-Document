...
join  fidelity_mdwdb.d_incident LKP
on  COALESCE(CONCAT('APPLICATION~', SRC.task ), 'UNSPECIFIED')= LKP.row_id and
SRC.sourceinstance=LKP.source_id
 WHERE coalesce(LKP.row_key, case when SRC.task is null then 0 else -1 end )  <>(TRGT.incident_affected_ci_c_key) and SRC.cdctype<>'D';
