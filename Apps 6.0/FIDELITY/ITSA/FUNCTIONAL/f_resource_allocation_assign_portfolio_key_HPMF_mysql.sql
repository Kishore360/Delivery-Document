...
LEFT JOIN(select * from fidelity_mdsdb.hp_pm_work_plans_final)SRC6
on SRC1.work_plan_id=SRC6.work_plan_id
LEFT JOIN(select * from fidelity_mdsdb.hp_kcrt_fg_pfm_project_final)SRC7
on SRC6.project_id=SRC7.prj_project_id
...
LEFT JOIN (select * from fidelity_mdwdb.d_portfolio) LKP
ON LKP.ROW_ID = SRC7.prj_portfolio_id
WHERE COALESCE(LKP.row_key,'0')<>COALESCE(TRGT.portfolio_key,'')  