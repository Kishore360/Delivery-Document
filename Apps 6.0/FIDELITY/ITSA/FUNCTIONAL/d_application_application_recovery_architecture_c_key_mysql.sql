...
ON (COALESCE(CONCAT('RECOVERY_ARCHITECTURE~APPLICATION~',upper(SRC.u_recovery_architecture)),'UNSPECIFIED') = LKP.row_id AND SRC.sourceinstance = LKP.source_id ) 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_recovery_architecture IS NULL THEN 0 else -1 end)<>(TRGT.application_recovery_architecture_c_key) and SRC.cdctype<>'D';
