...
ON (COALESCE(CONCAT('DISCOVERABLE~APPLICATION~',upper(SRC.u_application_discoverable)),'UNSPECIFIED') = LKP.row_id AND SRC.sourceinstance = LKP.source_id ) 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.u_application_discoverable IS NULL THEN 0 else -1 end)<>(TRGT.application_discoverable_c_key) and SRC.cdctype<>'D'
