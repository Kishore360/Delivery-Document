...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for d_internal_organization.u_business_unit' ELSE 'SUCCESS' END as Message
...
left join fidelity_mdwdb.d_internal_organization trg on trg.row_id=concat('GROUP~',f.sys_id)
 where trg.business_unit_code_c<>coalesce(bu.u_business_unit ,'UNSPECIFIED');