...
CASE WHEN count(1)>0 THEN 'MDS to DWH data validation failed for d_internal_organization.business_group_description_c' ELSE 'SUCCESS' END as Message
...
left join fidelity_mdwdb.d_internal_organization trg on trg.row_id=concat('GROUP~',f.sys_id)
 where trg.business_group_description_c<>coalesce(bg.u_business_group_description ,'UNSPECIFIED');