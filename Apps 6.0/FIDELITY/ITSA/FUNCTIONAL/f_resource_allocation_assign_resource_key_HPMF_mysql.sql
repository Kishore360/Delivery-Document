...
LEFT JOIN fidelity_mdwdb.d_internal_contact LKP
 on (LKP.row_id  = CONCAT('INTERNAL_CONTACT~',SRC.resource_id) and LKP.source_id  = SRC.sourceinstance)
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.resource_id is null THEN 0 else '-1' end)<> COALESCE(TRGT.resource_key,'');      