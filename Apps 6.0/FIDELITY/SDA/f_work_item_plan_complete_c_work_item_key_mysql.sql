...
union
select  distinct 
cdctype,source_id,row_id,work_item_id,sum(is_reopened) from (
SELECT 
ics.cdctype,
...
LEFT JOIN fidelity_mdwdb.d_work_item LKP 
ON SRC.work_item_id = LKP.row_id AND SRC.source_id = LKP.source_id 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.work_item_id IS NULL THEN 0 else -1 end)<>(TRGT.work_item_key) 
