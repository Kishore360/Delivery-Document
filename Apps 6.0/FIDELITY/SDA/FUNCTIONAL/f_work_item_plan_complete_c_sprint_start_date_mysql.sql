...
date(ics2.startdate) as sprint_start_date,
1
FROM
...
union
select  distinct 
cdctype,source_id,row_id,sprint_start_date,sum(is_reopened) from (
SELECT 
ics.cdctype,
ics.sourceinstance as source_id,
concat(ics.issueid,'~',ics.id,'~',date(cf.created),'~0') as row_id,
date(ics.startdate) as sprint_start_date,
...
WHERE SRC.sprint_start_date<>(TRGT.sprint_start_date) 
