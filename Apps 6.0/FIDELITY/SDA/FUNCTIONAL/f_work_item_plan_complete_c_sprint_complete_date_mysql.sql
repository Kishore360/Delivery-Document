...
date(ics2.completedate) as sprint_complete_date,
1
FROM
...
union
select  distinct 
cdctype,source_id,row_id,sprint_complete_date,sum(is_reopened) from (
SELECT 
ics.cdctype,
ics.sourceinstance as source_id,
concat(ics.issueid,'~',ics.id,'~',date(cf.created),'~0') as row_id,
date(ics.completedate) as sprint_complete_date,
...
WHERE SRC.sprint_complete_date<>(TRGT.sprint_complete_date) 
