...
concat(ics2.id,'~',ics2.boardid) iteration_id,
1
FROM
...
union
select  distinct 
cdctype,source_id,row_id,iteration_id,sum(is_reopened) from (
SELECT 
ics.cdctype,
ics.sourceinstance as source_id,
concat(ics.issueid,'~',ics.id,'~',date(cf.created),'~0') as row_id,
concat(ics.id,'~',ics.boardid) iteration_id,
...
LEFT JOIN fidelity_mdwdb.d_iteration LKP 
ON SRC.iteration_id = LKP.row_id AND SRC.source_id = LKP.source_id 
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.iteration_id IS NULL THEN 0 else -1 end)<>(TRGT.iteration_key) 
