...
DATE_FORMAT(CONVERT_TZ(coalesce(closed_at,resolved_at,sys_updated_on),'GMT','America/New_york'),'%Y')  AS   year,
DATE_FORMAT(CONVERT_TZ(coalesce(closed_at,resolved_at,sys_updated_on),'GMT','America/New_york'),'%Y%m%d')  AS   last_resolved_on_key
FROM whirlpool_mdsdb.incident_final 

)A
 JOIN  
(SELECT  f.last_resolved_on_key,f.source_id,f.ROW_ID FROM  whirlpool_mdwdb.f_incident f
