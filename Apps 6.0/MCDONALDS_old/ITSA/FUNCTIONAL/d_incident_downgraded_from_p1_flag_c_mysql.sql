...
CASE WHEN count(1)  THEN 'DWH to DWH data validation failed for d_incident.downgraded_from_p1_flag_c' ELSE 'SUCCESS' END as Message 
...
join mcd_mdwdb.d_lov l2 on f.task_attribute_wh_new_value_key=l2.row_key
 ...
 where dim.downgraded_from_p1_flag_c<>(case when cnt>1 and tmp.downgraded>0 then 'Y' else 'N' end) and dim.soft_deleted_flag='N';