...
-- SELECT  coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_authorizor) is null then 0 else -1 end) , TRGT.authorizor_key,SRC.CDCTYPE 
-- ,user.sys_id,SRC.u_authorizor,SRC.sys_id
...
where tablename = 'x_scafe_mcdcr_chan_mcdcr_change_request' and old.value = 12) tmp
...
COALESCE(Concat('INTERNAL_CONTACT~', coalesce(user.sys_id,SRC.u_authorizor)), 'UNSPECIFIED')= LKP.row_id
and user.sourceinstance = LKP.source_id

left join (select source_id,max(lastupdated) as lastupdated from mcdonalds_mdwdb.d_o_data_freshness group by source_id) f1 on (f1.source_id = SRC.sourceinstance)

where (SRC.cdctime<=f1.lastupdated) and 
 coalesce(LKP.row_key, case when coalesce(user.sys_id,SRC.u_authorizor) is null then 0 else -1 end) <> TRGT.authorizor_key
 and SRC.CDCTYPE='X';
