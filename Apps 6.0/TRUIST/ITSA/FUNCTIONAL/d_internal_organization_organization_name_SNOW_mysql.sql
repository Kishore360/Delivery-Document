...
from (SELECT * FROM truist_mdsdb.core_company_final where cdctype<>'D')SRC
left join (select * from truist_mdwdb.d_internal_organization WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N')TRGT
...
from (SELECT * FROM truist_mdsdb.cmn_department_final where cdctype<>'D')SRC
left join (select * from truist_mdwdb.d_internal_organization WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N')TRGT
...
from (SELECT * FROM truist_mdsdb.sys_user_group_final where cdctype<>'D')SRC
left join (select * from truist_mdwdb.d_internal_organization WHERE row_key NOT IN (0,-1) and soft_deleted_flag='N')TRGT
