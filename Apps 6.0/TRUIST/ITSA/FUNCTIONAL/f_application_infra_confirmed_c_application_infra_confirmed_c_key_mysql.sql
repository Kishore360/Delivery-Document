...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_application_infra_confirmed_c.application_infra_confirmed_c_key' ELSE 'SUCCESS' END as Message
from
...
join truist_mdwdb.d_application_infra_confirmed_c lkp on src.infra_id=lkp.row_id and src.sourceinstance=lkp.source_id
where COALESCE(lkp.row_key,(case when src.infra_id is null then 0 else -1 end)) <>tgt.application_infra_confirmed_c_key)tmp;				