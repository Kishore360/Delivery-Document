...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_application_infra_confirmed_c.db_end_of_support_health_duration' ELSE 'SUCCESS' END as Message
from
...
left join truist_mdsdb.sa_techportfolio_products_final src1 on src.Database_Product=src1.name
left join truist_mdwdb.d_o_data_freshness df on tgt.source_id=df.source_id and df.soft_deleted_flag='N'
where timestampdiff(SECOND,CONVERT_TZ(df.lastupdated,'America/New_York','GMT'),support_end_date) <>tgt.db_end_of_support_health_duration)tmp;
