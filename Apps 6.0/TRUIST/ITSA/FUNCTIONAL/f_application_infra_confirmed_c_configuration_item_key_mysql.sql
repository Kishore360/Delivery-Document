...
CASE WHEN CNT > 0 THEN 'MDS to DWH data validation failed for f_application_infra_confirmed_c.configuration_item_key' ELSE 'SUCCESS' END as Message
from
...
left join truist_mdsdb.cmdb_ci_final src1 on src.u_cmdb_sys_id=src1.sys_id
left join truist_mdwdb.d_configuration_item lkp on src1.sys_id=lkp.row_id and src1.sourceinstance=lkp.source_id
where COALESCE(lkp.row_key,(case when src.u_cmdb_sys_id is null then 0 else -1 end)) <>tgt.configuration_item_key)tmp;