...
from (SELECT sys_id,sourceinstance,closed_at, sys_updated_on, assignment_group, work_start, start_date,opened_at FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' and 
...
left outer join 
(select row_id,source_id,assignment_group_prior_changes FROM fidelity_mdwdb.d_change_failure where current_flag='Y') TGT
