...
WHERE  SRC.CDCTYPE<>'D'  and coalesce(work_start,start_date,closed_at)>'2019-01-01'
                and u_environment='Production' and year(TRGT.effective_to)=2999 and  COALESCE(LKP.row_key,CASE WHEN SRC.u_reason_for_application_outage IS NULL THEN 0 else -1 end) <> (TRGT.reason_for_application_outage_src_c_key)and SRC.cdctype<>'D' ) temp;
