...
CASE WHEN cnt>0  THEN 'MDS to DWH data validation failed for d_change_failure.server_outage_src_c_key' ELSE 'SUCCESS' END as Message 
...
WHERE  SRC.CDCTYPE<>'D'and    coalesce(work_start,start_date,closed_at)>'2019-01-01'
                and u_environment='Production' and  year(TRGT.effective_to)=2999 and  COALESCE(LKP.row_key,CASE WHEN SRC.u_server_outage IS NULL THEN 0 else -1 end) <> (TRGT.server_outage_src_c_key)and SRC.cdctype<>'D') temp; 
