...
from (SELECT sys_id,sourceinstance,type FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D'
  and coalesce(work_start,start_date,closed_at)>'2019-01-01'
                and u_environment='Production' ) SRC
left join (SELECT row_id,source_id,change_type_src_key FROM fidelity_mdwdb.d_change_failure where year(effective_to)=2999 
) TRGT 
