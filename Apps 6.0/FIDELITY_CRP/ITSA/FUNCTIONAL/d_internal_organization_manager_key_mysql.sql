...
FROM fidelity_mdwdb.d_internal_organization E_TRGT 
LEFT OUTER JOIN ( SELECT * FROM fidelity_mdsdb.sys_user_group_final WHERE CDCTYPE<>'D') STG
ON CONCAT('GROUP~',STG.SYS_ID) = E_TRGT.ROW_ID
LEFT OUTER JOIN fidelity_mdwdb.d_internal_contact M_TRGT
ON CONCAT('INTERNAL_CONTACT~',STG.MANAGER) = M_TRGT.ROW_ID
where E_TRGT.ROW_KEY not in (0,-1) and E_TRGT.source_id = 2
) SRC
LEFT JOIN 
fidelity_mdwdb.d_internal_organization TRGT 
