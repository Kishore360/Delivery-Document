...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 FROM ( SELECT sys_id,sourceinstance,category FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D') SRC
left join (SELECT row_id,source_id,change_category_src_key FROM fidelity_mdwdb.d_change_failure where year(effective_to)=2999 
 ...
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.category IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.change_category_src_key,'')
