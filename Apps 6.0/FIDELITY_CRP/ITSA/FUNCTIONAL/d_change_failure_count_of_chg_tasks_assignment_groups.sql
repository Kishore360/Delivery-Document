...
left outer join
(
select sys_id,sourceinstance,change_request,COALESCE(opened_at,work_start,planned_start_date) as task_dt, assignment_group from fidelity_mdsdb.change_task_final where change_request is not null 
) task
...
left outer join 
(select row_id,source_id,count_of_chg_tasks_assignment_groups FROM fidelity_mdwdb.d_change_failure where current_flag='Y') TGT
