...
CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for d_change_failure_ci_prior_changes' ELSE 'SUCCESS' END as Message
from
(
SELECT OUTR.sys_id, OUTR.cmdb_ci, OUTR.sourceinstance, OUTR.dimension_wh_code, OUTR.work_start,
...
from
(
SELECT s1.*, LKP.dimension_wh_code FROM (SELECT sys_id,sourceinstance FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' 
...
from
(
select SRC.sys_id, SRC.closed_at, SRC.sys_updated_on, SRC.cmdb_ci, SRC.work_start, SRC.start_date, SRC.opened_at, SRC.sourceinstance, LKP.dimension_wh_code
from (SELECT sys_id,sourceinstance,closed_at, sys_updated_on, cmdb_ci, work_start, start_date, opened_at FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' and 
...
left outer join 
(select row_id,source_id,ci_prior_changes FROM fidelity_mdwdb.d_change_failure where current_flag='Y') TGT
on SRC.sys_id = TGT.row_id
and SRC.sourceinstance = TGT.source_id

where coalesce(SRC.ci_prior,0)<>coalesce(TGT.ci_prior_changes,'')
