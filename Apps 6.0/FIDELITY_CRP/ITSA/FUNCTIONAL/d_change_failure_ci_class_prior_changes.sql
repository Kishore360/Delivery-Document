...
from
(
SELECT s1.*, LKP.dimension_wh_code, tgt_for_key.ci_class_key FROM (SELECT sys_id,sourceinstance FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' 
...
left outer join 
(select row_id,source_id FROM fidelity_mdwdb.d_change_failure where current_flag='Y') tgt_for_key
...
from
(
select SRC.sys_id, SRC.closed_at, SRC.sys_updated_on, tgt_for_key1.ci_class_key, SRC.work_start, SRC.start_date, SRC.opened_at, SRC.sourceinstance, LKP.dimension_wh_code
from (SELECT sys_id,sourceinstance,closed_at, sys_updated_on, work_start, start_date, opened_at FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' and 
coalesce(work_start,start_date,opened_at)<coalesce(closed_at,'9999-01-01')) SRC
LEFT JOIN fidelity_mdwdb.d_lov_map LKP 
 ON ( concat('STATE','~','CHANGE_REQUEST','~',upper(state)) = LKP.src_rowid 
AND SRC.sourceinstance = LKP.source_id )
left outer join (select row_id,source_id FROM fidelity_mdwdb.d_change_failure where current_flag='Y') tgt_for_key1
...
left outer join 
(select row_id,source_id,ci_class_prior_changes FROM fidelity_mdwdb.d_change_failure where current_flag='Y') TGT
on SRC.sys_id = TGT.row_id
and SRC.sourceinstance = TGT.source_id

where coalesce(SRC.ci_class_prior,0)<>coalesce(TGT.ci_class_prior_changes,'')
