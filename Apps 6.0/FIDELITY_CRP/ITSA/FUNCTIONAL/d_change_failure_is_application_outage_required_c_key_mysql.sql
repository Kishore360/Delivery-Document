...
WHERE  SRC.CDCTYPE<>'D'and year(TRGT.effective_to)=2999 and TRGT.soft_deleted_flag='N' and TRGT.current_flag='Y'  and COALESCE(LKP.row_key,CASE WHEN SRC.u_is_application_outage_required IS NULL THEN 0 else -1 end) <> (TRGT.is_application_outage_required_src_c_key) and SRC.cdctype<>'D') temp; 
