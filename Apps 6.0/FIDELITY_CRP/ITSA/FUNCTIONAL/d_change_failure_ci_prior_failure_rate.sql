...
from (SELECT sys_id,sourceinstance,closed_at, cmdb_ci, work_start, start_date, opened_at FROM fidelity_mdsdb.change_request_final WHERE CDCTYPE<>'D' and 
...
left outer join (select row_id,source_id,ci_prior_failure_rate FROM fidelity_mdwdb.d_change_failure where current_flag='Y') tgt
