...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for d_change_failure.unique_assignees_aggr_c' ELSE 'SUCCESS' END as Message 
...
LEFT JOIN (select chg.sys_id , chg.sourceinstance,count(distinct chgtsk.assigned_to) as chg_tsk_assignee_cnt  from equifax_mdsdb.change_request_final chg
