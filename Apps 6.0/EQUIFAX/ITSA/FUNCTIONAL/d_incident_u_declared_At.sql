...
(
select count(1) as cnt from(select sys_id, sourceinstance,u_declared_at from equifax_mdsdb.incident_final where CDCTYPE<>'D') src
 left join  equifax_mdwdb.d_incident trgt on 
src.sys_id=trgt.row_id and src.sourceinstance=trgt.source_id
where  src.u_declared_at <> trgt.u_declared_at_c and src.cdctype<>'D'
) temp;