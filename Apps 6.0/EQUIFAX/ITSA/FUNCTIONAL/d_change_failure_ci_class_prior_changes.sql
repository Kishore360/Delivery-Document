...
from
(
SELECT s1.*, LKP.dimension_wh_code, tgt_for_key.ci_class_key FROM (SELECT * FROM equifax_mdsdb.change_request_final WHERE CDCTYPE<>'D' 
...
left outer join 
(select * from equifax_mdwdb.d_change_failure where current_flag='Y') tgt_for_key
...
left outer join (select * from equifax_mdwdb.d_change_failure where current_flag='Y') tgt_for_key1
