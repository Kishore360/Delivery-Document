...
CASE WHEN count(1)  THEN 'MDS to DWH data validation failed for d_change_failure.modification_count_c' ELSE 'SUCCESS' END as Message 
...
LEFT JOIN (select chg.sys_id , chg.sourceinstance,sum(chgtsk.sys_mod_count) as mod_cnt from equifax_mdsdb.change_request_final chg
