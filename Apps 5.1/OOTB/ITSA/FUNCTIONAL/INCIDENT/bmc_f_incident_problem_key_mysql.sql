...
ELSE 'MDS to DWH data validation passed for f_incident.problem_key' END as Message from 
...
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.problem_id IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.problem_key,''))a;