...
THEN 'MDS to DWH data validation failed for d_incident.opened_by_key' ELSE 'SUCCESS' END as Message 
 ...
 ON (SRC.sys_id = TRGT.row_id AND SRC.sourceinstance = TRGT.sourceid AND SRC.sourceinstance = TRGT.sourceid ) 
 LEFT JOIN schneider_mdwdb.d_internal_contact LKP ON ( concat( 'INTERNAL_CONTACT~' ,upper( SRC.submitter)) = LKP.row_id 
 AND SRC.sourceinstance = LKP.sourceid ) WHERE COALESCE(LKP.row_key,CASE WHEN SRC.submitter IS NULL THEN 0 else -1 end)<> (TRGT.opened_by_key) 
