...
WHERE (case when when SRC.client_type='Home-Based Employee'  then COALESCE(SRC.Home_time_zone,'UNSPECIFIED') else COALESCE(SRC.Default_time_time_zone,'UNSPECIFIED') end ) <> (TRGT.time_zone)
