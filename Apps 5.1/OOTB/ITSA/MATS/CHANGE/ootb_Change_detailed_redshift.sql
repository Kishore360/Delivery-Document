...
union
  ...
  union
  select'd_change_request_approval' as Table_Name, count(1) Row_Count
  from         ldb.f_change_request         a11
  join        ldb.d_change_request_approval               a12
  on         (a11.approval_state_src_key = a12.row_key)
  union
  select'd_change_request_impact' as Table_Name, count(1) Row_Count
  from         ldb.f_change_request         a11
  join        ldb.d_change_request_impact  a12
  on         (a11.impact_src_key = a12.row_key)
  union
  select'd_change_request_priority' as Table_Name, count(1) Row_Count
  from         ldb.f_change_request         a11
  join        ldb.d_change_request_priority  a12
  on         (a11.priority_src_key = a12.row_key)
  union
  select'd_change_request_urgency' as Table_Name, count(1) Row_Count
  from         ldb.f_change_request         a11
