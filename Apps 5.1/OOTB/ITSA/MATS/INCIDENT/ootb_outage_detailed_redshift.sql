...
from ldb.f_incident_outage       a11 				  
				  ...
				  union
				       select 'ldb.d_outage_type' as Table_name, count(1) Row_Count
from ldb.f_incident_outage       a11 				  
				  join        ldb.d_outage     a14
                  on         (a11.outage_key = a14.row_key)
...
from ldb.f_incident_outage       a11 				  
				  join        ldb.d_incident_contacttype     a14
                  on         (a11.reported_type_src_key = a14.row_key)
				  union
				   select 'ldb.d_task_impact' as Table_name, count(1) Row_Count
from ldb.f_incident_outage       a11 				  
				  join        ldb.d_task_impact     a14
                  on         (a11.impact_src_key = a14.row_key)
				  union
				   select 'ldb.d_task_priority' as Table_name, count(1) Row_Count
from ldb.f_incident_outage       a11 				  
				  join        ldb.d_task_priority     a14
                  on         (a11.priority_src_key = a14.row_key)
				  union
				     select 'ldb.d_task_urgency' as Table_name, count(1) Row_Count
from ldb.f_incident_outage       a11 				  
