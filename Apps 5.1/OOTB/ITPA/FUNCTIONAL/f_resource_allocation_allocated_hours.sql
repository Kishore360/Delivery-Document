SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 from (select SUBSTRING_INDEX(row_id,'~',1) as source_row_id, resource_plan_key, project_key, resource_key,sum(F.allocated_hours) as allocated_hours 
...
WHERE COALESCE( CAST(coalesce(SRC.allocated_hours,0) as decimal(20,10)),'')<> COALESCE(cast(round(TRGT.allocated_hours) as decimal(20,10)),'');
