...
case when count(1) > 0 then CONCAT(CAST(COUNT(1) AS CHAR(10)), 'invalid multiple_categorization_flag set for fs_incident') else 'SUCCESS' end as Message
from (select 1 from <<tenant>>_mdwdb.fs_incident 
where CONCAT(row_id ,',' ,source_id) in (select CONCAT( ta.task_row_id, ',' , source_id) 
from <<tenant>>_mdwdb.ts_task_activity ta where ta.task_wh_type = 'incident' 
