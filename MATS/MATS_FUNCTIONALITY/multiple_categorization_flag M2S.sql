...
case when count(1) > 0 then CONCAT(CAST(COUNT(1) AS CHAR(10)), 'invalid multiple_categorization_flag set for stg_<<tablename>>') else 'SUCCESS' end as Message
from (select 1 from <<tenant>>_workdb.stg_<<tablename>> 
...
union
select 1 from <<tenant>>_workdb.stg_<<tablename>> 
