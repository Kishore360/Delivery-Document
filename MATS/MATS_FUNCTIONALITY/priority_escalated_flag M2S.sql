...
case when count(1) > 0 then CONCAT( CAST(COUNT(1) AS CHAR(10)), 'invalid priority_escalated_flag set for #STG_TABLE_NAME') else 'SUCCESS' end as Message 
from (select 1
from  <<tenant>>_workdb.#STG_TABLE_NAME 
where (concat(row_id , ',' ,source_id)) in (select concat(ta.task_id, ',' ,source_id)   
from <<tenant>>_workdb.stg_ts_task_activity ta where ta.task_wh_type = '#TYPE' and ta.task_attribute_wh_name = 'priority' 
