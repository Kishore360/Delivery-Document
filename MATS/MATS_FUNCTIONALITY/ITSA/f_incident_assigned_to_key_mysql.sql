 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_incident.assigned_to_key' ELSE 'SUCCESS' END as Message from 
(select a.sys_id,e.row_id,c.severity AS SEVERITY_SRC_CODE ,COALESCE(LKP.row_key,CASE WHEN a.assigned_to IS NULL THEN 0 else '-1' end) as abc,
COALESCE(e.assigned_to_key,'') as def
...
LEFT JOIN starwood_mdwdb.d_internal_contact LKP 
 ON ( concat('INTERNAL_CONTACT~',a.assigned_to)= LKP.row_id 
