...
from (select f.1 from <<tenant>>_mdwdb.<<tablename>> f 
join <<tenant>>_mdwdb.d_calendar_date dat 
on f.opened_on_key = dat.row_key 
join <<tenant>>_mdwdb.d_lov_map br 
on f.state_src_key = br.src_key 
JOIN <<tenant>>_mdwdb.d_lov wh_lov 
...
union
select f.1 from <<tenant>>_mdwdb.<<tablename>> f 
join <<tenant>>_mdwdb.d_calendar_date dat 
on f.opened_on_key = dat.row_key 
join <<tenant>>_mdwdb.d_lov_map br 
on f.state_src_key = br.src_key 
