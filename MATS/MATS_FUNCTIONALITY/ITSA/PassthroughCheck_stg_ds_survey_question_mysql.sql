...
FROM <<tenant>>_mdsdb.survey_question_new_final 
WHERE CONCAT( IFNULL(sys_id,''), IFNULL(sourceinstance,''), IFNULL(question_text,''), IFNULL(`order`,''), 
...
NOT IN (SELECT CONCAT( IFNULL(row_id,''), IFNULL(source_id,''), IFNULL(question,''), IFNULL(question_order,''), 
...
FROM <<tenant>>_workdb.ds_survey_question)