...
FROM (
select count(*) as cnt  from(select * from (SELECT CONCAT(a.s0,'~1~ASSIGNMENT GROUP TIER') AS s_row_id,
a.s0 AS s_lev_0_id,coalesce(level_0.row_key ,0) AS s_lev_0_key,
a.s1 AS s_lev_1_id,coalesce(level_1.row_key ,0) AS s_lev_1_key,
a.s2 AS s_lev_2_id,coalesce(level_2.row_key ,0) AS s_lev_2_key,
a.s3 AS s_lev_3_id,coalesce(level_3.row_key ,0) AS s_lev_3_key,
a.s4 AS s_lev_4_id,coalesce(level_4.row_key ,0) AS s_lev_4_key,
a.s5 AS s_lev_5_id,coalesce(level_5.row_key ,0) AS s_lev_5_key
FROM
...
WHERE hierarchy_class ='ASSIGNMENT GROUP TIER' )BB
