...
FROM <<tenant>>_mdsdb.cmdb_ci_final WHERE (CONCAT( IFNULL(sys_id,'UNSPECIFIED'), IFNULL(sourceinstance,''), 
...
NOT IN (SELECT (CONCAT( IFNULL(row_id,'UNSPECIFIED'), IFNULL(source_id,'UNSPECIFIED'), IFNULL(asset_tag,'UNSPECIFIED'), IFNULL(category,''), 
...
IFNULL(company_id,'UNSPECIFIED'))) FROM <<tenant>>_workdb.ds_configuration_item)