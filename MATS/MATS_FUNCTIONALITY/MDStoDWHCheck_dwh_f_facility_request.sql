...
CASE WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed between facilities_final and dwh_f_facilty_request' ELSE 'SUCCESS' END AS Message 
...
LEFT JOIN dwh_d_internal_organization DIO_AG
...
LEFT JOIN dwh_d_configuration_item DCI
ON (DCI.row_id   =  S.cmdb_ci )
LEFT JOIN dwh_d_domain DD
ON (DD.row_id   =  S.sys_domain )
LEFT JOIN dwh_d_location DL
...
JOIN dwh_d_facility_request DFR
...
IFNULL(DWH.time_worked,''))) FROM <<tenant>>_mdwdb.dwh_f_facility_request DWH)
