...
CASE WHEN COUNT(1) > 0 THEN 'MDS to DWH data validation failed between MDS_EMPLOYEE_TERMINATION and dwh_f_employee_termination ' ELSE 'SUCCESS' END AS Message 
FROM  <<tenant>>_mdsdb.MDS_EMPLOYEE_TERMINATION met 
LEFT JOIN <<tenant>>_mdwdb.lsm_ls_source_timezone tz 
ON (tz.sourceid  = met.source_id  
AND met.source_id IN (0) )
left outer join <<tenant>>_mdwdb.dwh_d_calendar_date  last_working_day 
on last_working_day.row_id= DATE_FORMAT(CONVERT_TZ(met.last_working_day,tz.source_time_zone,tz.target_time_zone),'%Y%m%d')
and last_working_day.source_id=0
left outer join <<tenant>>_mdwdb.dwh_d_calendar_date  termination_date 
on termination_date.row_id=DATE_FORMAT(CONVERT_TZ(met.termination_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d') 
and termination_date.source_id=0
left outer join <<tenant>>_mdwdb.dwh_d_calendar_date  notice_date
on notice_date.row_id=DATE_FORMAT(CONVERT_TZ(met.notice_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d') 
and notice_date.source_id=0 
left outer join <<tenant>>_mdwdb.dwh_d_calendar_date  transaction_date 
on transaction_date.row_id=DATE_FORMAT(CONVERT_TZ(met.transaction_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d')
and transaction_date.source_id=0  
LEFT OUTER JOIN <<tenant>>_mdwdb.dwh_d_hr_employee employee 
...
FROM <<tenant>>_mdwdb.dwh_f_employee_termination)
