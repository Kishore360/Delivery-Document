...
on (LKP.row_id = date_format(convert_tz(SRC.retired,source_time_zone,target_time_zone),'%Y%m%d')  and LKP.source_id=0
)
WHERE COALESCE(LKP.row_key,CASE WHEN SRC.retired IS NULL THEN 0 else '-1' end) <> COALESCE(TRGT.asset_retirement_on_key,'') 
