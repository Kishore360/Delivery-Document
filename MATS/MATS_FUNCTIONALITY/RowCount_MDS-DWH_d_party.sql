...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.contactsearchrowbasic_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_party_person'  Table_DWH 
FROM <<tenant>>_mdwdb.d_party_person WHERE row_key not in (0,-1)) DWH
