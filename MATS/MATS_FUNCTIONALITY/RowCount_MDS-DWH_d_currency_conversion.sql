...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.currencyrate_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.datedconversionrate_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.fx_rate_final) AS count_mds) MDS,
