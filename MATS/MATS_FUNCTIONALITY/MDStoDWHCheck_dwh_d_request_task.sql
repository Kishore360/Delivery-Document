...
case when count(1) > 0 then 'MDS to DWH data validation failed between sc_task_final and dwh_d_request_task' else 'SUCCESS' end as Message
from <<tenant>>_mdsdb.sc_task_final S
join <<tenant>>_mdwdb.lsm_ls_source_timezone L 
on (S.sourceinstance = L.sourceid )

left join dwh_t_task_activity TA_PR 
on (S.sys_id = TA_PR.task_id 
...
left join dwh_t_task_activity TA_MC
on (S.sys_id = TA_MC.task_id 
...
) from <<tenant>>_mdwdb.dwh_d_request_task DWH);
