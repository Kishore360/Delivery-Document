...
, CASE WHEN X.CNT = 0 THEN 'Tranformation check succeed for dwh_f_ap_payment.applied_doc_amount'
ELSE 'Tranformation check failed for dwh_f_ap_payment.applied_doc_amount' END AS MESSAGE
FROM
(
SELECT COUNT(1) CNT
FROM
