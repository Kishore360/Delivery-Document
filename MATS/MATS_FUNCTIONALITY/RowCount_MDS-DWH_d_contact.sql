...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.contact_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_contact'  Table_DWH 
FROM <<tenant>>_mdwdb.d_contact WHERE row_key not in (0,-1)) DWH
