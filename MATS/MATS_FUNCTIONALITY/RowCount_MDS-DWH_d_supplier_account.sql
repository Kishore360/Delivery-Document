...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.vendor_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_supplier_account'  Table_DWH 
FROM <<tenant>>_mdwdb.d_supplier_account WHERE row_key not in (0,-1)) DWH
