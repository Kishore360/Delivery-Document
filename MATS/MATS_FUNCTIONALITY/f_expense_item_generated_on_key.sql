...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 ON ( SRC.date= LKP.row_id 
AND SRC.sourceinstance= LKP.source_id )
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.date IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.generated_on_key,'')and SRC.asset is not null;
