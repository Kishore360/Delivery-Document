...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.inventoryitem_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.product2_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.sc_cat_item_final) AS count_mds) MDS,
