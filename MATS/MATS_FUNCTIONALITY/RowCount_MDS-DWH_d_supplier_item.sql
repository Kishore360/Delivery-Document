...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.inventoryitem_itemvendor_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.pc_vendor_cat_item_final) AS count_mds) MDS,
