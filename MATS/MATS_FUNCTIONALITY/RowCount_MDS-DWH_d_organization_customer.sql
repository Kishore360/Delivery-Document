...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.account_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.customer_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.partner_final) AS count_mds) MDS,
