...
, CASE WHEN X.CNT = 0 THEN 'Tranformation check succeed for dwh_f_ap_credit_memo_summary.unapplied_doc_amount'
ELSE 'Tranformation check failed for dwh_f_ap_credit_memo_summary.unapplied_doc_amount' END AS MESSAGE
FROM
(
SELECT COUNT(1) CNT
FROM
...
FROM <<tenant>>_mdwdb.dwh_f_ap_credit_memo_summary
