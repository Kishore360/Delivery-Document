...
SELECT CONCAT(a.s0,'~-1~DOMAIN') AS s_row_id,
...
FROM uclahealth_mdsdb.sys_user_group_final lev_0
LEFT JOIN uclahealth_mdsdb.sys_user_group_final lev_1
ON(lev_0.parent = lev_1.sys_id)
LEFT JOIN uclahealth_mdsdb.sys_user_group_final lev_2
ON(lev_1.parent = lev_2.sys_id)
LEFT JOIN uclahealth_mdsdb.sys_user_group_final lev_3
ON(lev_2.parent = lev_3.sys_id)
LEFT JOIN uclahealth_mdsdb.sys_user_group_final lev_4
ON(lev_3.parent = lev_4.sys_id)
LEFT JOIN uclahealth_mdsdb.sys_user_group_final lev_5
ON(lev_4.parent = lev_5.sys_id)
)a
LEFT JOIN  uclahealth_mdwdb.d_domain level_0
ON a.s0 = RIGHT(level_0.row_id,32)
LEFT JOIN  uclahealth_mdwdb.d_domain level_1
ON a.s1 = RIGHT(level_1.row_id,32)
LEFT JOIN  uclahealth_mdwdb.d_domain level_2
ON a.s2 = RIGHT(level_2.row_id,32)
LEFT JOIN  uclahealth_mdwdb.d_domain level_3
ON a.s3 = RIGHT(level_3.row_id,32)
LEFT JOIN  uclahealth_mdwdb.d_domain level_4
ON a.s4 = RIGHT(level_4.row_id,32)
LEFT JOIN  uclahealth_mdwdb.d_domain level_5
ON a.s5 = RIGHT(level_5.row_id,32) )AA
left JOIN(
SELECT row_id,lev_0_key,lev_1_key,lev_2_key,
lev_3_key,lev_4_key,lev_5_key
FROM uclahealth_mdwdb.d_h_all_hierarchies
WHERE hierarchy_class ='DOMAIN')BB
on AA.s_row_id=BB.row_id where AA.s_lev_0_key <> BB.lev_0_key
or AA.s_lev_1_key<>BB.lev_1_key
or AA.s_lev_2_key<>BB.lev_2_key
or AA.s_lev_3_key<>BB.lev_3_key
or AA.s_lev_4_key<>BB.lev_4_key
or AA.s_lev_5_key<>BB.lev_5_key)ds)abc