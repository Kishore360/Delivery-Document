...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.cmn_location_final) + (SELECT COUNT(1) FROM <<tenant>>_mdsdb.cmn_location_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_location'  Table_DWH 
FROM <<tenant>>_mdwdb.d_location WHERE row_key not in (0,-1)) DWH
