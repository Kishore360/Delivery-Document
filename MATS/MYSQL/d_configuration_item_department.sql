...
select s.sys_id sys_id ,s.sourceinstance sourceinstance,concat(coalesce(s.sys_id,''),coalesce(s.sourceinstance,''),coalesce(CDF.name,'') ) as abc FROM <<tenant>>_mdsdb.cmdb_ci_final s 
 join <<tenant>>_mdsdb.cmn_department_final CDF
...
from <<tenant>>_mdwdb.d_configuration_item DWH)fgh
