...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM <<tenant>>_mdsdb.core_company_final) + 
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.core_company_final WHERE customer=1 )
 + ( SELECT COUNT(1) FROM <<tenant>>_mdsdb.sys_user_final ) AS count_mds) MDS,
