...
on (convert(LKP.row_id using utf8) = convert(date_format(convert_tz(SRC.estimated_delivery,source_time_zone,target_time_zone),'%Y%m%d') using utf8) and LKP.source_id=0
)
WHERE CONVERT(COALESCE(LKP.row_key,case when SRC.estimated_delivery is null then 0 else -1 end) using utf8) <> CONVERT(COALESCE(TRGT.estimated_delivery_on_key,'') using utf8)
