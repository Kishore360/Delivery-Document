...
case when count(1) > 0 then 'MDS to DWH data validation failed between sc_req_item_final and d_request_item' else 'SUCCESS' end as Message
from #MDS_TABLE_SCHEMA.sc_req_item_final S
...
ifnull(DWH.request_item_number,''),
...
using utf8)) from #TABLE_SCHEMA.d_request_item DWH);
