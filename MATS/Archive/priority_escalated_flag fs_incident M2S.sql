...
case when count(*) > 0 then CONCAT( CAST(COUNT(*) AS CHAR(10)), 'invalid priority_escalated_flag set for fs_incident') else 'SUCCESS' end as Message 
from (select *
from  #TABLE_SCHEMA.fs_incident 
where (concat(row_id  ',' source_id)) in (select concat(ta.task_row_id ','  source_id)   
from #TABLE_SCHEMA.ts_task_activity ta where ta.task_wh_type = 'incident' and ta.task_attribute_wh_name = 'priority' 
