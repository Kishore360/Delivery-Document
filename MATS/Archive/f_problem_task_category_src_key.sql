...
LEFT JOIN #DWH_TABLE_SCHEMA.d_lov LKP 
 ON ( CONVERT(concat('CATEGORY','~','','~','~','~',upper(null)) using utf8)= convert(LKP.src_rowid using utf8)
AND CONVERT(SRC.sourceinstance using utf8)= convert(LKP.source_id using utf8))
 WHERE COALESCE(LKP.row_key,CASE WHEN null IS NULL THEN 0 else '-1' end)<> COALESCE(CONVERT(TRGT.category_src_key using utf8),'')
