...
FROM #MDS_TABLE_SCHEMA.time_card_final TCD
...
LEFT OUTER JOIN  #MDS_TABLE_SCHEMA.sys_user_final SUF
...
on tc.time_card_on_id=convert(dt.row_id using utf8)

left outer join 
(select row_id,row_key from 
#DWH_TABLE_SCHEMA.d_calendar_time) dtt
on tc.time_card_time_id =convert(dtt.row_id using utf8)
