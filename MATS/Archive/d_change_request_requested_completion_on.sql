...
ON (convert(SRC.sourceinstance using utf8)  = convert(L.sourceid using utf8))
 WHERE COALESCE( CONVERT(convert_tz(SRC.requested_by_date,source_time_zone,target_time_zone) using utf8),'')<> COALESCE(CONVERT(TRGT.requested_completion_on using utf8),'')
