...
on (convert(LKP.row_id using utf8) = convert(date_format(convert_tz(SRC.approval_set,source_time_zone,target_time_zone),'%Y%m%d') using utf8)and LKP.source_id=0

 
)
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC.approval_set IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.approval_on_key,'')
