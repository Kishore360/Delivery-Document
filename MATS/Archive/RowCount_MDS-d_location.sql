...
END AS Message
FROM (SELECT (SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.cmn_location_final) + (SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.cmn_location_final) AS count_mds) MDS,
(SELECT COUNT(*) Count_DWH, 'd_location'  Table_DWH 
FROM #TABLE_SCHEMA.d_location WHERE row_key not in (0,-1)) DWH
