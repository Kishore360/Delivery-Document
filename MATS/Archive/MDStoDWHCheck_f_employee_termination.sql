...
FROM  #MDS_TABLE_SCHEMA.MDS_EMPLOYEE_TERMINATION met 
...
left outer join #TABLE_SCHEMA.d_calendar_date  last_working_day 
on convert(last_working_day.row_id using utf8)= convert(DATE_FORMAT(CONVERT_TZ(met.last_working_day,tz.source_time_zone,tz.target_time_zone),'%Y%m%d')using utf8)
and convert(last_working_day.source_id using utf8)=0
left outer join #TABLE_SCHEMA.d_calendar_date  termination_date 
on convert(termination_date.row_id using utf8)=convert(DATE_FORMAT(CONVERT_TZ(met.termination_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d') using utf8)
and convert(termination_date.source_id using utf8)=0
left outer join #TABLE_SCHEMA.d_calendar_date  notice_date
on convert(notice_date.row_id using utf8)=convert(DATE_FORMAT(CONVERT_TZ(met.notice_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d') using utf8)
and convert(notice_date.source_id using utf8)=0 
left outer join #TABLE_SCHEMA.d_calendar_date  transaction_date 
on convert(transaction_date.row_id using utf8)=convert(DATE_FORMAT(CONVERT_TZ(met.transaction_date,tz.source_time_zone,tz.target_time_zone),'%Y%m%d')using utf8)
and convert(transaction_date.source_id using utf8)=0  
LEFT OUTER JOIN #TABLE_SCHEMA.d_hr_employee employee 
