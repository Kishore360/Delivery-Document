...
END AS Message
FROM (SELECT (SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.vendor_final) AS count_mds) MDS,
(SELECT COUNT(*) Count_DWH, 'd_supplier_account'  Table_DWH 
FROM #TABLE_SCHEMA.d_supplier_account WHERE row_key not in (0,-1)) DWH
