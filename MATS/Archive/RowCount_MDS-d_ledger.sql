...
END AS Message
FROM (SELECT (SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.subsidiary_final) AS count_mds) MDS,
(SELECT COUNT(*) Count_DWH, 'd_ledger'  Table_DWH 
FROM #TABLE_SCHEMA.d_ledger WHERE row_key not in (0,-1)) DWH
