...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_request.opened_on_key' ELSE 'SUCCESS' END as Message
...
WHERE CONVERT(COALESCE(LKP.row_key,'') using utf8) <> CONVERT(COALESCE(TRGT.opened_on_key,'') using utf8)
