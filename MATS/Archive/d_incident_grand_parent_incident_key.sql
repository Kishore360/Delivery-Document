...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 LEFT JOIN #MDS_TABLE_SCHEMA.incident_final SRC_P
 ON(SRC_P.sys_id=SRC.parent)
...
AND CONVERT(SRC.sourceinstance using utf8)= convert(LKP.source_id using utf8))
 WHERE COALESCE(LKP.row_key,CASE WHEN SRC_P.parent IS NULL THEN 0 else '-1' end)<> COALESCE(TRGT.grand_parent_incident_key,'')
