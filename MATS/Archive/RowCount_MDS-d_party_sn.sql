...
END AS Message
FROM (SELECT (SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.core_company_final) + 
(SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.core_company_final WHERE customer=1 )
 + ( SELECT COUNT(*) FROM #MDS_TABLE_SCHEMA.sys_user_final ) AS count_mds) MDS,
