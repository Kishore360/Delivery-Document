...
case when count(1) > 0 then' dh_all_hierarchies not populated correctly for ''USER GROUP CLASSIFICATION'' ' else 'SUCCESS' end as Message

from <<tenant>>_mdsdb.c_mds_usergrouphierarchy SRC
left join <<tenant>>_mdwdb.d_internal_organization IO1
on Level1=IO1.row_id 
left join <<tenant>>_mdwdb.d_internal_organization IO2
on Level2=IO2.row_id
left join <<tenant>>_mdwdb.d_internal_organization IO3
on Level3=IO3.row_id
left join <<tenant>>_mdwdb.d_internal_organization IO4
...
from <<tenant>>_mdwdb.dh_all_hierarchies  DH
