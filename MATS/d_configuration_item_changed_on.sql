...
ifnull(convert_tz(S.sys_updated_on,source_time_zone,target_time_zone),'') )using utf8)not in (
select convert(concat(
ifnull(DWH.row_id,''),
ifnull(DWH.source_id,''),
ifnull(DWH.changed_on,'') )using utf8) from <<tenant>>_mdwdb.d_configuration_item DWH);
