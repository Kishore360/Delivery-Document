...
AND  LM.dimension_wh_code IN('RESOLVED','CLOSED') THEN 'Y' ELSE 'N' END abc, COALESCE(TRGT.first_call_resolution_flag ,'') def
...
LEFT JOIN <<tenant>>_mdwdb.d_o_data_freshness FRESH  ON(FRESH.source_id=SRC.sourceinstance) )A
 WHERE abc<>def
