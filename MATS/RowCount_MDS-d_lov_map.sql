...
END AS Message
FROM ( SELECT(
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_currency_rate_type_map_final )
+
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_accounttype_map_final )
+
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_units_map_final )
+
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_fin_status_lov_map_final )
+
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_om_lov_map_final )
+
(SELECT COUNT(1) FROM <<tenant>>_mdsdb.us_finbucket_map_final )
+
