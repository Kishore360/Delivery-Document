...
case when count(1)> 0 then 'MDS to DWH data validation failed between problem_final and f_problem: impact_src_ket' else 'SUCCESS' end as Message;
select sys_id from <<tenant>>_mdsdb.problem_final S
...
using utf8)) from <<tenant>>_mdwdb.f_problem DWH);
