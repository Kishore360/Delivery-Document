...
case when count(1)> 0 then 'MDS to DWH data validation failed between request_final and f_request: passthroughMDSDWH' else 'SUCCESS' end as Message
...
case when S.request_id is not null then 'Y' else 'N' end,
...
using utf8)) from <<tenant>>_mdwdb.f_request DWH);
