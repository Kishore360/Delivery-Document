...
SELECT CASE WHEN count(1) > 0 THEN 'FAILURE' ELSE 'SUCCESS' END as Result,
 ...
 LEFT JOIN <<tenant>>_mdwdb.d_lov_change_request_review_status_view LKP 
 ON ( CONCAT('REVIEW_STATUS','~','CHANGE_REQUEST','~','~','~',UPPER(review_status)) using utf8)= convert(LKP.src_rowid using utf8)
