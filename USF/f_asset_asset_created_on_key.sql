...
on (convert(LKP.row_id using utf8) = convert(date_format(convert_tz(SRC.sys_created_on,source_time_zone,target_time_zone),'%Y%m%d') using utf8) and LKP.source_id=0
)
WHERE CONVERT(COALESCE(LKP.row_key,CASE WHEN SRC.sys_created_on IS NULL THEN 0 else '-1' end) using utf8) <> CONVERT(COALESCE(TRGT.asset_created_on_key,'') using utf8)
