...
END AS Message
FROM (SELECT (SELECT COUNT(1) FROM usf_mdsdb.core_company_final) + (SELECT COUNT(1) FROM usf_mdsdb.vendor_final) AS count_mds) MDS,
(SELECT COUNT(1) Count_DWH, 'd_supplier'  Table_DWH 
FROM usf_mdwdb.d_supplier WHERE row_key not in (0,-1)) DWH
