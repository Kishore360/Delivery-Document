...
, CASE WHEN COUNT(1) > 0 THEN 'reopened_flag not set correctly in dwh_t_task_activity' ELSE 'SUCCESS' END AS Message
from #DWH_TABLE_SCHEMA.dwh_t_task_activity ta   
join #DWH_TABLE_SCHEMA.dwh_d_lov_map lm   
on (ta.task_attribute_wh_old_value = lm.dimension_code and lm.dimension_class = 'STATE~INCIDENT')
join #DWH_TABLE_SCHEMA.dwh_d_lov_map new_lm    
