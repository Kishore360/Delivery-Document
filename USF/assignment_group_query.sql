...
case when count(*) > 0 then' dwh_dh_all_hierarchies not populated correctly for ''USER GROUP CLASSIFICATION'' ' else 'SUCCESS' end as Message

from #MDS_TABLE_SCHEMA.c_mds_usergrouphierarchy SRC
left join #DWH_TABLE_SCHEMA.dwh_d_internal_organization IO1
on Level1=IO1.row_id 
left join #DWH_TABLE_SCHEMA.dwh_d_internal_organization IO2
on Level2=IO2.row_id
left join #DWH_TABLE_SCHEMA.dwh_d_internal_organization IO3
on Level3=IO3.row_id
left join #DWH_TABLE_SCHEMA.dwh_d_internal_organization IO4
...
from #DWH_TABLE_SCHEMA.dwh_dh_all_hierarchies  DH
