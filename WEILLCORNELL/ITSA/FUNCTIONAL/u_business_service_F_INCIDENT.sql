...
(select count(1) as src_count from weillcornell_mdwdb.f_incident where soft_deleted_flag <>'Y')A,
(select count(1) as tar_count from weillcornell_mdsdb.incident_final where (sys_id,number) not in (
