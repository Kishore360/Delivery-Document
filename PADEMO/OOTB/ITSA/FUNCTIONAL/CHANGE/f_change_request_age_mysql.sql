...
*/
 ...
 CASE WHEN count(1) >0 THEN 'MDS to DWH data validation failed for f_change_request.closed_by_key' ELSE 'SUCCESS' END as Message
  FROM 
(select sys_id,sourceinstance,opened_at from <<tenant>>_mdsdb.change_request_final where opened_at < closed_at) SRC 
...
FROM <<tenant>>_mdwdb.d_o_data_freshness WHERE sourcename like 'ServiceNow%'))<> f.change_request_age
